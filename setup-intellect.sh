...
echo ""
echo -e "${CYAN}⚕ Intellect Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/intellect-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink intellect into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up intellect command..."

INTELLECT_BIN="$SCRIPT_DIR/venv/bin/intellect"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$INTELLECT_BIN" "$COMMAND_LINK_DIR/intellect"
echo -e "${GREEN}✓${NC} Symlinked intellect → $COMMAND_LINK_DISPLAY_DIR/intellect"
...
# ============================================================================
# Seed bundled skills into ~/.intellect/skills/
# ============================================================================

INTELLECT_SKILLS_DIR="${INTELLECT_HOME:-$HOME/.intellect}/skills"
mkdir -p "$INTELLECT_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.intellect/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$INTELLECT_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  intellect status        # Check configuration"
if is_termux; then
    echo "  intellect gateway       # Run gateway in foreground"
else
    echo "  intellect gateway install # Install gateway service (messaging + cron)"
fi
echo "  intellect cron list     # View scheduled jobs"
echo "  intellect doctor        # Diagnose issues"
echo ""
