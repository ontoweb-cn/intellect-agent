#!/usr/bin/env python3
"""Live test harness for Intellect Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any intellect imports.
ORIGINAL_HOME = os.environ.get("INTELLECT_HOME")
ORIGINAL_AUTH = Path.home() / ".intellect" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.intellect/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.intellect/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="intellect_ts_live_"))
    intellect_home = home_dir / ".intellect"
    intellect_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, intellect_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".intellect" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, intellect_home / ".env")
        ...
        load_intellect_dotenv(intellect_home=str(Path.home() / ".intellect"))
    ...
    (intellect_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
...
def reset_module_state():
    """Drop cached modules so the new INTELLECT_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "intellect_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    if ORIGINAL_HOME is not None:
        os.environ["INTELLECT_HOME"] = ORIGINAL_HOME
    else:
