...
Environment:
    intellect_TEST_WORKERS  Override worker count (default: os.cpu_count())
    intellect_TEST_PATHS    Override discovery roots (colon-sep, default: 'tests')
...
#                        ontoweb/intellect-agent:test image, via
#                        ``INTELLECT_TEST_IMAGE`` so the fixture skips
...
# via --file-timeout or intellect_TEST_FILE_TIMEOUT.
...
_NO_ISOLATE_BATCH_SIZE = 24


def _file_requests_no_isolate(path: Path) -> bool:
    """True when the test module opts out of per-file subprocess isolation."""
    try:
        head = path.read_text(encoding="utf-8", errors="ignore")[:16384]
    except OSError:
        return False
    return "pytest.mark.no_isolate" in head or "mark.no_isolate" in head


def _partition_files(files: List[Path]) -> Tuple[List[Path], List[List[Path]]]:
    """Split into per-file isolated paths and batched no_isolate groups."""
    isolated: List[Path] = []
    fast: List[Path] = []
    for path in files:
        if _file_requests_no_isolate(path):
            fast.append(path)
        else:
            isolated.append(path)
    batches: List[List[Path]] = []
    for i in range(0, len(fast), _NO_ISOLATE_BATCH_SIZE):
        batches.append(fast[i : i + _NO_ISOLATE_BATCH_SIZE])
    return isolated, batches
...
def _run_pytest_cmd(
    ...
    """Run a pytest argv list; return (rc, output, summary, wall_seconds)."""
    ...
    return rc, output, _parse_pytest_summary(output), subproc_wall
...
def _run_one_file(
    ...
    rc, output, summary, subproc_wall = _run_pytest_cmd(cmd, repo_root, file_timeout)
...
def _run_file_batch(
    ...
    repo_root: Path,
    file_timeout: float,
) -> List[Tuple[Path, int, str, dict[str, int], float]]:
    """Run several no_isolate modules in one pytest subprocess."""
    if not files:
        return []
    if len(files) == 1:
        one = _run_one_file(files[0], pytest_args, repo_root, file_timeout)
        return [one]

    cmd = [sys.executable, "-m", "pytest", *[str(f) for f in files], *pytest_args]
    rc, output, summary, subproc_wall = _run_pytest_cmd(cmd, repo_root, file_timeout)
    if rc == 0:
        per_wall = subproc_wall / len(files)
        return [(f, 0, output, summary, per_wall) for f in files]

    # Batch failed — rerun individually so failures attribute to the right file.
    results: List[Tuple[Path, int, str, dict[str, int], float]] = []
    for path in files:
        results.append(_run_one_file(path, pytest_args, repo_root, file_timeout))
    return results
...
def _format_file(file: Path, repo_root: Path) -> str:
    ...
    ``/home/runner/work/intellect-agent/intellect-agent/tests/acp/test_auth.py``.
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "--jobs",
        type=int,
        default=int(os.environ.get("intellect_TEST_WORKERS") or (os.cpu_count() or 4) * 2),
        help="Parallel worker count (default: $intellect_TEST_WORKERS or cpu_count*2)",
    )
    parser.add_argument(
        "--paths",
        default=os.environ.get("intellect_TEST_PATHS", ":".join(_DEFAULT_ROOTS)),
    ...
    parser.add_argument(
        ...
        type=float,
        default=float(
            os.environ.get("intellect_TEST_FILE_TIMEOUT", _DEFAULT_FILE_TIMEOUT_SECONDS)
        ),
        help=(
            "Per-file wall-clock cap in seconds. On timeout, the pytest "
            "subprocess and its full process tree are SIGKILL'd. "
            "Default: 600 (10 min), env: intellect_TEST_FILE_TIMEOUT."
        ),
    )
    parser.add_argument(
        "--slice",
        ...
        help=(
            ...
            "Env: intellect_TEST_SLICE (format: I/N)."
        ),
    )
    parser.add_argument(
        "paths_positional",
    ...
    # Parse --slice (or intellect_TEST_SLICE) early so we can exit on bad input
    # before doing any expensive discovery.
    slice_raw = args.slice or os.environ.get("intellect_TEST_SLICE")
    ...
    isolated_files, fast_batches = _partition_files(files)
    fast_file_count = sum(len(b) for b in fast_batches)
    batch_note = ""
    if fast_file_count:
        batch_note = (
            f"; {fast_file_count} no_isolate file(s) in {len(fast_batches)} batched run(s)"
        )
    print(
        ...
        f"running with -j {args.jobs}{batch_note}",
        flush=True,
    )
    ...
    def _on_batch_done(batch: List[Path], started_at: float, fut: "Future[List[Tuple[Path, int, str, dict[str, int], float]]]") -> None:
        try:
            results = fut.result()
            ...
            for file in batch:
                _on_done(
                    file,
                    started_at,
                    _ImmediateFuture((file, 1, f"batch runner crashed: {exc!r}", {}, 0.0)),
                )
            return
        for file in batch:
            row = next((r for r in results if r[0] == file), None)
            if row is None:
                _on_done(
                    file,
                    started_at,
                    _ImmediateFuture((file, 1, "batch result missing file", {}, 0.0)),
                )
            else:
                _on_done(file, started_at, _ImmediateFuture(row))

    class _ImmediateFuture:
        """Minimal future wrapper for synchronous callback reuse."""
    ...
    with ThreadPoolExecutor(max_workers=args.jobs) as pool:
        ...
        for batch in fast_batches:
            t0 = time.monotonic()
            fut = pool.submit(
                _run_file_batch, batch, pytest_passthrough, repo_root, args.file_timeout
            )
            fut.add_done_callback(lambda f, batch=batch, t0=t0: _on_batch_done(batch, t0, f))
            futures.append(fut)
