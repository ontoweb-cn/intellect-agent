...
# Bootstrap Open WebUI against Intellect Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.intellect/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-intellect.sh
...
#   OPEN_WEBUI_NAME='Johnny Intellect'
...
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Intellect Agent WebUI}"
...
intellect_API_BASE_URL="http://${intellect_API_CONNECT_HOST}:${intellect_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-intellect.sh"
LOG_DIR="$HOME/.intellect/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$intellect_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$intellect_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.intellect.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.intellect</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.intellect"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.intellect"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-intellect.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Intellect Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-intellect.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.intellect/logs/openwebui.log
StandardError=append:%h/.intellect/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-intellect.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$intellect_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Intellect API server is configured...'
  upsert_env API_SERVER_ENABLED true "$intellect_ENV_FILE"
  upsert_env API_SERVER_HOST "$intellect_API_HOST" "$intellect_ENV_FILE"
  upsert_env API_SERVER_PORT "$intellect_API_PORT" "$intellect_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$intellect_API_MODEL_NAME" "$intellect_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$intellect_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Intellect gateway so API server settings take effect...'
  intellect gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${intellect_API_CONNECT_HOST}:${intellect_API_PORT}/health" >/dev/null; then
    log 'Intellect API server did not answer on the first check. Trying to start gateway in the background...'
    nohup intellect gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${intellect_API_CONNECT_HOST}:${intellect_API_PORT}/health" >/dev/null
  ...
  log "Intellect API endpoint: ${intellect_API_BASE_URL}"
