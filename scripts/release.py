...
VERSION_FILE = REPO_ROOT / "intellect_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "intellect@marian.local": "Schrotti77",
    ...
    "274096618+intellect-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "mind-dragon@ontoweb.research": "Mind-Dragon",
    ...
    "shannon@ontoweb.cn": "shannonsands",
    ...
    "intellect@noushq.ai": "benbarclay",
    ...
    "intellect@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "agent@intellect.local": "jacdevos",
    ...
    "teknium@intellect-agent": "teknium1",
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (INTELLECT_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (INTELLECT_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: intellect-eval community link)
    ...
    "clement@ontoweb.cn": "lemassykoi",  # PR #27042 (model-switch probe keyless providers)
    ...
    "vigo@intellect": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.intellect@agentmail.to": "oseftg",
    ...
    "ethie@ontoweb": "ethernet8023",  # PR #29342 (TUI clipboard copy on linux/wayland)
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Intellect runtime files)
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"intellect-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "intellect@ontoweb.cn"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://gitee.com/ontoweb/intellect-agent",
    ...
    lines.append(f"# Intellect Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Intellect Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Intellect Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Intellect Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Intellect Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
