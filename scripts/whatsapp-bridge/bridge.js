#!/usr/bin/env node
/**
 * Intellect Agent WhatsApp Bridge
 *
 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.intellect/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.intellect', 'whatsapp', 'session'));
const IMAGE_CACHE_DIR = path.join(process.env.HOME || '~', '.intellect', 'image_cache');
const DOCUMENT_CACHE_DIR = path.join(process.env.HOME || '~', '.intellect', 'document_cache');
const AUDIO_CACHE_DIR = path.join(process.env.HOME || '~', '.intellect', 'audio_cache');
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['Intellect Agent', 'Chrome', '120.0'],
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Ignore Intellect' own reply messages in self-chat mode to avoid loops.
