...
# Python-driven tool session (e.g. Intellect terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@gitee.com:ontoweb/intellect-agent.git"
REPO_URL_HTTPS="https://gitee.com/ontoweb/intellect-agent.git"
INTELLECT_HOME="${INTELLECT_HOME:-$HOME/.intellect}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${intellect_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$intellect_INSTALL_DIR"
...
#   code at /usr/local/lib/intellect-agent, command at /usr/local/bin/intellect,
#   data still at /root/.intellect (INTELLECT_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.intellect/intellect-agent"
            echo "                   default (root, Linux): /usr/local/lib/intellect-agent"
            echo "  --intellect-home PATH  Data directory (default: ~/.intellect, or \$INTELLECT_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Intellect installs the code under"
            echo "  /usr/local/lib/intellect-agent and links the command into"
            echo "  /usr/local/bin/intellect (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$INTELLECT_HOME"
            echo "  (default /root/.intellect).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$INTELLECT_HOME/intellect-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs intellect setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# Decide where the repo checkout + venv live, and where the `intellect` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $INTELLECT_HOME/intellect-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $INTELLECT_HOME/intellect-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/intellect-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $INTELLECT_HOME/intellect-agent — then preserve it)
#
# Always no-op when the user set --dir or $intellect_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in INTELLECT_HOME.
    if is_termux; then
        INSTALL_DIR="$INTELLECT_HOME/intellect-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$INTELLECT_HOME/intellect-agent/.git" ]; then
            INSTALL_DIR="$INTELLECT_HOME/intellect-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/intellect-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/intellect-agent"
        ...
        # /usr/local/bin/intellect wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/intellect"
        log_info "  Data:    $INTELLECT_HOME (unchanged)"
...
get_intellect_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/intellect" ]; then
        echo "$link_dir/intellect"
    else
        echo "intellect"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.giteeusercontent.com/ontoweb/intellect-agent/raw/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/intellect-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/intellect-uv-installer.$$.sh")"
...
# module compiled in. Intellect' session store (intellect_state.py) creates FTS5
...
ensure_fts5() {
    ...
    if [ -n "${PYTHON_PATH:-}" ] && _python_has_fts5 "$PYTHON_PATH"; then
        ...
        # etc.). Install proceeds — Intellect degrades gracefully and disables
        # only full-text session search — but warn so it isn't a silent gap.
        log_warn "Could not obtain an FTS5-capable Python. Intellect will run, but"
        log_warn "full-text session search will be disabled until FTS5 is present."
    fi
...
check_node() {
    ...
    if [ -x "$INTELLECT_HOME/node/bin/node" ]; then
        export PATH="$INTELLECT_HOME/node/bin:$PATH"
        local found_ver=$("$INTELLECT_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Intellect-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.intellect/node/..."
    ...
    # Place into ~/.intellect/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$INTELLECT_HOME/node"
    mkdir -p "$INTELLECT_HOME"
    mv "$extracted_dir" "$INTELLECT_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$INTELLECT_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$INTELLECT_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$INTELLECT_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$INTELLECT_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$INTELLECT_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.intellect/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Intellect install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Intellect Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="intellect-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Intellect behaves unexpectedly."
                    else
...
install_deps() {
    ...
    install_rust_extension || { log_error "Rust extension is required. Aborting."; exit 1; }
...
setup_path() {
    log_info "Setting up intellect command..."

    if [ "$USE_VENV" = true ]; then
        INTELLECT_BIN="$INSTALL_DIR/venv/bin/intellect"
    else
        INTELLECT_BIN="$(which intellect 2>/dev/null || echo "")"
        if [ -z "$INTELLECT_BIN" ]; then
            log_warn "intellect not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$INTELLECT_BIN" ]; then
        log_warn "intellect entry point not found at $INTELLECT_BIN"
    ...
    # Create a user-facing shim for the intellect command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $INTELLECT_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$INTELLECT_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/intellect"
    cat > "$command_link_dir/intellect" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$INTELLECT_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/intellect"
    log_success "Installed intellect launcher → $command_link_display_dir/intellect"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "intellect command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v intellect' \
                >/dev/null 2>&1; then
        ...
        log_info "intellect not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Intellect Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Intellect Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Intellect Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so intellect works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.intellect directory structure (config at top level, code in subdir)
    mkdir -p "$INTELLECT_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.intellect/.env (top level, easy to find)
    if [ ! -f "$INTELLECT_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$INTELLECT_HOME/.env"
            log_success "Created ~/.intellect/.env from template"
        else
            touch "$INTELLECT_HOME/.env"
            log_success "Created ~/.intellect/.env"
        fi
    else
        log_info "~/.intellect/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$INTELLECT_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.intellect/config.yaml (top level, easy to find)
    if [ ! -f "$INTELLECT_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$INTELLECT_HOME/config.yaml"
            log_success "Created ~/.intellect/config.yaml from template"
        fi
    else
        log_info "~/.intellect/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$INTELLECT_HOME/SOUL.md" ]; then
        cat > "$INTELLECT_HOME/SOUL.md" << 'SOUL_EOF'
...
-->
SOUL_EOF
        log_success "Created ~/.intellect/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.intellect/"

    # Seed bundled skills into ~/.intellect/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.intellect/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.intellect/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$INTELLECT_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$INTELLECT_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.intellect/skills/"
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    local env_file="$INTELLECT_HOME/.env"
    ...
    {
        echo ""
        echo "# Intellect Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Intellect will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (intellect --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'intellect setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m intellect_cli.main setup < /dev/tty
    else
        python -m intellect_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$INTELLECT_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Intellect to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$INTELLECT_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'intellect whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                intellect_CMD="$(get_intellect_command_path)"
                $intellect_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'intellect whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'intellect gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $intellect_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $intellect_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: intellect gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: intellect gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $intellect_CMD gateway > "$INTELLECT_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.intellect/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: intellect gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: intellect gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $INTELLECT_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $INTELLECT_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $INTELLECT_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}intellect${NC}              Start chatting"
    echo -e "   ${GREEN}intellect setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}intellect config${NC}       View/edit configuration"
    echo -e "   ${GREEN}intellect config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}intellect gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}intellect update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'intellect' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'intellect' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'intellect' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$INTELLECT_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$INTELLECT_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$INTELLECT_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$INTELLECT_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$INTELLECT_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Intellect for pip install"
    ...
    intellect_CMD="$(command -v intellect 2>/dev/null || echo "")"
    if [ -n "$intellect_CMD" ]; then
        log_info "Running intellect setup..."
        "$intellect_CMD" setup
    else
        log_warn "intellect command not found on PATH"
        log_info "Try: python -m intellect_cli.main setup"
    fi
}
...
main() {
    ...
    echo "git" > "$INTELLECT_HOME/.install_method"
}
