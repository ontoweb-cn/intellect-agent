"""
Intellect MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Intellect-specific extra)

Usage:
    intellect mcp serve
    intellect mcp serve --verbose
...
logger = logging.getLogger("intellect.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using INTELLECT_HOME."""
    try:
        from intellect_constants import get_intellect_home
        return get_intellect_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("INTELLECT_HOME", Path.home() / ".intellect")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from intellect_constants import get_intellect_home
        directory_file = get_intellect_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("INTELLECT_HOME", Path.home() / ".intellect")
...
class EventBridge:
    ...
    This is the Intellect equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        Uses mtime checks on sessions.json and (SQLite only) state.db to skip work
        when nothing has changed — makes 200ms polling essentially free.
        PostgreSQL has no on-disk mtime; always poll the session store.
        """
        ...
        try:
            from intellect_constants import get_intellect_home
            db_file = get_intellect_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("INTELLECT_HOME", Path.home() / ".intellect")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Intellect MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "intellect",
        instructions=(
            "Intellect Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Intellect MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
