...
_INTELLECT_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (INTELLECT_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `intellect tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "video_gen": {
        "description": (
            ...
            "``intellect tools`` → Video Generation."
        ),
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (INTELLECT_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full Intellect toolsets (CLI + messaging platforms)
    #
    ...
    "intellect-cli": {
        ...
        "tools": _INTELLECT_CORE_TOOLS,
        "includes": []
    },

    "intellect-cron": {
        # Mirrors intellect-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `intellect tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as intellect-cli; gated by `intellect tools`",
        "tools": _INTELLECT_CORE_TOOLS,
        "includes": []
    },
    ...
    "intellect-bluebubbles": {
        "description": "BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server",
        "tools": _INTELLECT_CORE_TOOLS,
        "includes": []
    },

    "intellect-homeassistant": {
    ...
    "intellect-email": {
        "description": "Email bot toolset - interact with Intellect via email (IMAP/SMTP)",
        "tools": _INTELLECT_CORE_TOOLS,
        "includes": []
    },

    "intellect-mattermost": {
    ...
    "intellect-sms": {
        "description": "SMS bot toolset - interact with Intellect via SMS (Twilio)",
        "tools": _INTELLECT_CORE_TOOLS,
        "includes": []
    },
    ...
    "intellect-webhook": {
        ...
        "tools": _intellect_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "intellect-gateway": {
        ...
        "tools": [],
        "includes": ["intellect-telegram", "intellect-discord", "intellect-whatsapp", "intellect-slack", "intellect-signal", "intellect-bluebubbles", "intellect-homeassistant", "intellect-email", "intellect-sms", "intellect-mattermost", "intellect-matrix", "intellect-dingtalk", "intellect-feishu", "intellect-wecom", "intellect-wecom-callback", "intellect-weixin", "intellect-qqbot", "intellect-webhook", "intellect-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (intellect-<name>).
        # Gives them _INTELLECT_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("intellect-"):
            platform_name = name[len("intellect-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_INTELLECT_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
