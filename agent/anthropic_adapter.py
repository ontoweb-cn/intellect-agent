"""Anthropic Messages API adapter for Intellect Agent.

Translates between Intellect's internal OpenAI-style message format and
...
# Intellect effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _is_kimi_family_endpoint(base_url: str | None, model: str | None = None) -> bool:
    ...
    See intellect-agent#13848, #17057.
    """
...
def _write_claude_code_credentials(
    ...
    *,
    ...
    # DB write (so resolve_anthropic_token DB-first path sees fresh tokens)
    try:
        from agent.oauth.model_tokens import try_persist_model_token
        if access_token:
            expires_in = max(0, int((expires_at_ms - time.time() * 1000) / 1000)) if expires_at_ms else None
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Intellect historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Intellect)
      2. CLAUDE_CODE_OAUTH_TOKEN env var
      3. ``oauth_tokens`` row (Intellect-managed PKCE / linked Claude Code)
    ...
    # 1. Intellect-managed OAuth/setup token env var
    ...
    # 3. Encrypted model token in state.db (PR-A8)
    try:
        from agent.oauth.model_tokens import resolve_runtime_access_token

        db_token = resolve_runtime_access_token("anthropic")
        if db_token:
            preferred = _prefer_refreshable_claude_code_token(db_token, creds)
            if preferred:
                return preferred
            return db_token
    except Exception:
    ...
    # This remains as a compatibility fallback for pre-migration Intellect configs.
...
# ── Intellect-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.intellect/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_intellect_OAUTH_FILE = get_intellect_home() / ".anthropic_oauth.json"
...
def run_intellect_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Intellect-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Intellect with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from intellect_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def save_intellect_oauth_credentials(creds: Dict[str, Any]) -> None:
    """Persist Intellect PKCE OAuth to ``oauth_tokens`` and legacy JSON file (A8)."""
    import time

    access = str(creds.get("access_token") or "").strip()
    if not access:
        return
    refresh = str(creds.get("refresh_token") or "").strip() or None
    expires_ms = int(creds.get("expires_at_ms") or 0)
    expires_in = 0
    if expires_ms:
    ...
    try:
        ...
        try_persist_model_token(
            ...
            expires_in=expires_in,
            metadata={"expires_at_ms": expires_ms},
        )
    except Exception as exc:
        logger.debug("persist Anthropic OAuth to oauth_tokens failed: %s", exc)
    ...
    try:
        ...
        store = SecretStore()
        store.write_json(".anthropic_oauth.json", payload)
    except OSError as exc:
        logger.warning("Failed to write Intellect OAuth credentials: %s", exc)


def read_intellect_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Intellect-managed OAuth from state.db, then ~/.intellect/.anthropic_oauth.json."""
    try:
        from agent.oauth.model_tokens import try_load_model_token_row

        row = try_load_model_token_row("anthropic")
        if row and row.get("access_token"):
            meta = row.get("metadata") if isinstance(row.get("metadata"), dict) else {}
            expires_at = row.get("expires_at")
            expires_ms = int(expires_at * 1000) if expires_at else int(meta.get("expires_at_ms") or 0)
            return {
    ...
    try:
        from agent.secret_store import SecretStore
        store = SecretStore()
        data = store.read_json(".anthropic_oauth.json")
        if data.get("accessToken"):
            return data
    except Exception as e:
        # Fall back to plaintext read for unmigrated files
        if _intellect_OAUTH_FILE.exists():
            try:
                data = json.loads(_intellect_OAUTH_FILE.read_text(encoding="utf-8"))
                if data.get("accessToken"):
                    return data
            except (json.JSONDecodeError, OSError, IOError) as e2:
                logger.debug("Failed to read Intellect OAuth credentials: %s", e2)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See intellect-agent#13848.
    #
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See intellect-agent#13848 (Kimi) and
    intellect-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Intellect Agent", "Claude Code")
                text = text.replace("Intellect agent", "Claude Code")
                text = text.replace("intellect-agent", "claude-code")
                text = text.replace("ONTOWEB", "Anthropic")
                block["text"] = text
    ...
    # silently hides reasoning text that Intellect surfaces in its CLI. We
