...
Plugins built on Intellect Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Intellect
...
:class:`~intellect_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Intellect' main config
...
handles every provider, fallback chain, and per-task override Intellect
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`intellect_cli.plugins.PluginContext`
