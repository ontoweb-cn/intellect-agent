...
Storage (``~/.intellect/auth/google_oauth.json``, chmod 0o600):
...
from intellect_constants import get_intellect_home, secure_parent_dir
...
#
# Resolution order:
#   1. intellect_GEMINI_CLIENT_ID / intellect_GEMINI_CLIENT_SECRET env vars (power users)
...
ENV_CLIENT_ID = "intellect_GEMINI_CLIENT_ID"
ENV_CLIENT_SECRET = "intellect_GEMINI_CLIENT_SECRET"
...
_HEADLESS_ENV_VARS = ("SSH_CONNECTION", "SSH_CLIENT", "SSH_TTY", "intellect_HEADLESS")
...
def _credentials_path() -> Path:
    return get_intellect_home() / "auth" / "google_oauth.json"
...
def _require_client_id() -> str:
    ...
    if not cid:
        raise GoogleOAuthError(
            ...
            "Intellect looks for a locally installed gemini-cli to source the OAuth client. "
            "Either:\n"
            "  1. Install it: npm install -g @google/gemini-cli  (or brew install gemini-cli)\n"
            "  2. Set intellect_GEMINI_CLIENT_ID and intellect_GEMINI_CLIENT_SECRET in ~/.intellect/.env\n"
            "\n"
...
def _load_credentials_from_db() -> Optional[GoogleCredentials]:
    """PR-A8: load Gemini OAuth from ``oauth_tokens`` when present."""
    try:
        from agent.oauth.model_tokens import try_load_model_token_row

        row = try_load_model_token_row("google-gemini-cli")
        if not row or not row.get("access_token"):
            return None
        meta = row.get("metadata") if isinstance(row.get("metadata"), dict) else {}
        expires_at = row.get("expires_at")
        expires_ms = int(expires_at * 1000) if expires_at else int(meta.get("expires_ms") or 0)
        return GoogleCredentials(
            ...
            managed_project_id=str(meta.get("managed_project_id") or ""),
        )
    except Exception:
        return None


def _sync_credentials_to_db(creds: GoogleCredentials) -> None:
    try:
        from agent.oauth.model_tokens import try_persist_model_token

        expires_in = 0
        if creds.expires_ms:
            expires_in = max(60, int(creds.expires_unix_seconds() - time.time()))
        ...
        logger.debug("sync Google OAuth to oauth_tokens failed: %s", exc)


def load_credentials() -> Optional[GoogleCredentials]:
    """Load credentials from state.db first, then disk (PR-A8)."""
...
def save_credentials(creds: GoogleCredentials) -> Path:
    """Persist creds to state.db; optionally mirror to disk when legacy write is enabled."""
    path = _credentials_path()
    _sync_credentials_to_db(creds)
    try:
...
def clear_credentials() -> None:
    """Remove creds from DB and disk. Idempotent."""
    try:
        from agent.oauth.model_tokens import try_delete_model_token
...
def get_valid_access_token(*, force_refresh: bool = False) -> str:
    ...
    if creds is None:
        raise GoogleOAuthError(
            "No Google OAuth credentials found. Run `intellect auth add google-gemini-cli` first.",
...
<html><head><meta charset="utf-8"><title>Intellect — signed in</title>
<style>
...
<html><head><meta charset="utf-8"><title>Intellect — sign-in failed</title>
<style>
body {{ font: 16px/1.5 system-ui, sans-serif; margin: 10vh auto; max-width: 32rem; text-align: center; color: #222; }}
h1 {{ color: #b42318; }} p {{ color: #555; }}
</style></head>
<body><h1>Sign-in failed</h1><p>{message}</p>
<p>Return to your terminal — Intellect will walk you through a manual paste fallback.</p></body></html>
"""
...
def start_oauth_flow(
    *,
    ...
    auth_url = AUTH_ENDPOINT + "?" + urllib.parse.urlencode(params) + "#intellect"
