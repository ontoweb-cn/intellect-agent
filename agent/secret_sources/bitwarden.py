...
Intellect pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.intellect/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<intellect_home>/bin/bws`` on
  first use.  Intellect pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.intellect/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``intellect`` invocations don't
  hammer the API.
* Failures NEVER block Intellect startup.  Missing binary, no network,
...
# In-process cache so repeated load_intellect_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, "_CachedFetch"] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `intellect chat -q ...`
...
# <intellect_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.intellect/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under intellect_home/cache/.

    `home_path` is what `load_intellect_dotenv()` already resolved; falling back
    to `$INTELLECT_HOME` / `~/.intellect` keeps direct callers working too.
    """
    if home_path is None:
        home_path = Path(os.getenv("INTELLECT_HOME", Path.home() / ".intellect"))
...
def _intellect_bin_dir() -> Path:
    """Where Intellect stores its managed binaries.  Profile-aware."""
    from intellect_constants import get_intellect_home

    return get_intellect_home() / "bin"
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<intellect_home>/bin/bws``  (our managed copy — preferred)
    ...
    """
    managed = _intellect_bin_dir() / _platform_binary_name()
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``intellect secrets bitwarden setup``
    surface lets them propagate so the wizard can show a clear error.
    """
    bin_dir = _intellect_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="intellect-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "intellect-agent"})
    try:
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<intellect_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
    ...
    back to ``$INTELLECT_HOME`` / ``~/.intellect``.
...
# ---------------------------------------------------------------------------
# Public entry point — called from intellect_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_intellect_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `intellect secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `intellect secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `intellect secrets bitwarden setup` to install."
        )
        return result
