    ...
    try:
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    ...
                    "compression will drop middle turns without a summary. "
                    "Run `intellect setup` or set OPENROUTER_API_KEY."
                )
        ...
        if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Intellect "
...
def compress_context(
    ...
    *,
    ...
    # long-lived ``intellect_state.SessionDB`` class still bound to the
        ...
        try:
            ...
            if getattr(agent, "_last_compression_lock_error_sid", None) != _lock_sid:
                ...
                logger.warning(
                    ...
                    "process (or `intellect update`) to resync.",
    ...
    if agent._session_db:
        try:
            ...
            # Propagate member_id from the parent session for multi-user isolation.
            _existing_member_id = None
            try:
                _existing = agent._session_db.get_session(old_session_id)
                if _existing:
    ...
    # compression (not a fresh /new). Plugin engines (e.g. intellect-lcm) use
    # boundary_reason="compression" to preserve DAG lineage across the
    # rollover instead of re-initializing fresh per-session state.
    # See intellect-lcm#68. Built-in ContextCompressor ignores kwargs.
    try:
    ...
    def _shrink_data_url(url: str) -> Optional[str]:
        ...
        try:
            ...
            tmp = tempfile.NamedTemporaryFile(
                prefix="intellect_shrink_", suffix=suffix, delete=False,
            )
            try:
                tmp.write(raw)
                tmp.close()
