...
def _bucket_label_node(bucket: _ChartBucket) -> Optional[dict[str, Any]]:
    if not bucket.nodes:
        return None
    return max(bucket.nodes, key=lambda node: _node_score(node, bucket.rec))
...
def render_graph(payload: dict[str, Any], *, cols: int = 80, rows: int = 16, reveal: float = 1.0) -> dict[str, Any]:
    ...
    if not nodes:
        placeholder = [["no learning yet — keep using Intellect and it maps out here", STYLE_DIM, 0.7]]
