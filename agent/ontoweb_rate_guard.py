"""Cross-session rate limit guard for ONTOWEB Portal.

Writes rate limit state to a shared file so all sessions (CLI, gateway,
cron, auxiliary) can check whether ONTOWEB Portal is currently rate-limited
before making requests.  Prevents retry amplification when RPH is tapped.

Each 429 from the OntoWeb Portal triggers up to 9 API calls per conversation turn
(3 SDK retries x 3 Intellect retries), and every one of those calls counts
...
_STATE_FILENAME = "ontoweb.json"


def _state_path() -> str:
    """Return the path to the OntoWeb rate limit state file."""
    try:
        from intellect_constants import get_intellect_home
        base = get_intellect_home()
    except ImportError:
        base = os.path.join(os.path.expanduser("~"), ".intellect")
...
def record_ontoweb_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    error_context: Optional[dict[str, Any]] = None,
    default_cooldown: float = 300.0,
) -> None:
    """Record that ONTOWEB Portal is rate-limited.
    ...
    try:
        ...
        logger.info(
            "OntoWeb rate limit recorded: resets in %.0fs (at %.0f)",
            reset_at - now, reset_at,
        )
    except Exception as exc:
        logger.debug("Failed to write OntoWeb rate limit state: %s", exc)


def ontoweb_rate_limit_remaining() -> Optional[float]:
    """Check if ONTOWEB Portal is currently rate-limited.
...
def clear_ontoweb_rate_limit() -> None:
    """Clear the rate limit state (e.g., after a successful OntoWeb request)."""
    try:
        os.unlink(_state_path())
    except FileNotFoundError:
        pass
    except OSError as exc:
        logger.debug("Failed to clear OntoWeb rate limit state: %s", exc)
...
def is_genuine_ontoweb_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    last_known_state: Optional[Any] = None,
) -> bool:
    """Decide whether a 429 from ONTOWEB Portal is a real account rate limit.

    ONTOWEB Portal multiplexes multiple upstream providers (DeepSeek, Kimi,
    MiMo, Intellect, ...) behind one endpoint.  A 429 can mean either:

      (a) The caller's own RPM / RPH / TPM / TPH bucket on OntoWeb is
      ...
      (b) The upstream provider is out of capacity for a specific model
          ...
          the caller's quota on OntoWeb.

    Tripping the cross-session breaker on (b) blocks ALL OntoWeb requests
    (and all models, since OntoWeb is one provider key) for minutes even
    ...
    We tell the two apart by looking at:
      ...
      1. The 429 response's own ``x-ratelimit-*`` headers.  OntoWeb emits
