...
from intellect_constants import get_intellect_home, get_skills_dir, is_wsl
...
_intellect_MD_NAMES = (".intellect.md", "INTELLECT.md")


def _find_intellect_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.intellect.md`` or ``INTELLECT.md``.
    ...
    for directory in [current, *current.parents]:
        for name in _intellect_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are Intellect Agent, an intelligent AI assistant created by ONTOWEB. "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.intellect/kanban.db`. Your task id is in "
    "`$INTELLECT_KANBAN_TASK`; your workspace is `$intellect_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $intellect_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `intellect kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Intellect WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Intellect itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Intellect runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Intellect itself is running. The host OS, home, and cwd "
                f"of the Intellect process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Intellect "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Intellect
    ...
    extra = (os.getenv("intellect_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_intellect_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.intellect/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("intellect_PLATFORM")
        or get_session_env("intellect_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Intellect Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `intellect-agent` skill "
            "first. It has the actual commands (e.g. `intellect config set …`, `intellect tools`, "
            "`intellect setup`) so you don't have to guess or invent workarounds.\n"
...
def build_ontoweb_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    """Build a compact OntoWeb subscription capability block for the system prompt."""
    try:
        ...
        logger.debug("Failed to import OntoWeb subscription helper: %s", exc)
        return ""
    ...
    features = get_ontoweb_subscription_features()

    def _status_line(feature) -> str:
        if feature.managed_by_nous:
            return f"- {feature.label}: active via OntoWeb subscription"
        if feature.active:
            current = feature.current_provider or "configured provider"
            return f"- {feature.label}: currently using {current}"
        if feature.included_by_default and features.ontoweb_auth_present:
            return f"- {feature.label}: included with OntoWeb subscription, not currently selected"
        if feature.key == "modal" and features.ontoweb_auth_present:
            return f"- {feature.label}: optional via OntoWeb subscription"
        return f"- {feature.label}: not currently available"

    lines = [
        "# OntoWeb Subscription",
        "OntoWeb subscription includes managed web tools (Firecrawl), image generation (FAL), OpenAI TTS, and browser automation (Browser Use) by default. Modal execution is optional.",
        "Current capability status:",
    ]
    lines.extend(_status_line(feature) for feature in features.items())
    lines.extend(
        [
            "When a OntoWeb-managed feature is active, do not ask the user for Firecrawl, FAL, OpenAI TTS, or Browser-Use API keys.",
            "If the user is not subscribed and asks for a capability that OntoWeb subscription would unlock or simplify, suggest OntoWeb subscription as one option alongside direct setup or local alternatives.",
            "Do not mention subscription unless the user asks about it or it directly solves the current missing capability.",
            "Useful commands: intellect setup, intellect setup tools, intellect setup terminal, intellect status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from INTELLECT_HOME and return its content, or None.
    ...
    """
    try:
        from intellect_cli.config import ensure_intellect_home
        ensure_intellect_home()
    except Exception as e:
        logger.debug("Could not ensure INTELLECT_HOME before loading SOUL.md: %s", e)

    soul_path = get_intellect_home() / "SOUL.md"
...
def _load_intellect_md(cwd_path: Path) -> str:
    """.intellect.md / INTELLECT.md — walk to git root."""
    intellect_md_path = _find_intellect_md(cwd_path)
    if not intellect_md_path:
        return ""
    try:
        content = intellect_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = intellect_md_path.name
        try:
            rel = str(intellect_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".intellect.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", intellect_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .intellect.md / INTELLECT.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from INTELLECT_HOME is independent and always included when present.
    ...
    # SOUL.md from INTELLECT_HOME only — skip when already loaded as identity
    if not skip_soul:
