...
def _oauth_auth_json_sync_allowed() -> bool:
    """PR-A10: skip auth.json OAuth sync when runtime reads are disabled."""
    try:
        from agent.oauth.runtime_settings import should_read_auth_json

        return should_read_auth_json()
    except Exception:
        return False
...
class PooledCredential:
    ...
    def runtime_api_key(self) -> str:
        if self.provider == "ontoweb":
            # OntoWeb stores the runtime inference credential in agent_key for
            ...
            for token, expires_at in (
                ...
                if (
                    ...
                    and token.strip()
                    and auth_mod._ontoweb_invoke_jwt_is_usable(
                        token,
...
class CredentialPool:
    ...
    @contextmanager
    def _pool_lock(self):
        """Hold the pool mutex; flush auth.json outside the critical section."""
        ...
        with self._lock:
            try:
                ...
                if self._dirty:
                    snapshot = [entry.to_dict() for entry in self._entries]
                    self._dirty = False
        if snapshot is not None:
            with self._persist_lock:
                write_credential_pool(provider, snapshot)
    ...
    def _persist(self) -> None:
        """Mark pool state dirty; disk write runs when ``_pool_lock`` exits."""
        self._dirty = True

    def _persist_now(self) -> None:
        """Synchronous persist for callers that do not use ``_pool_lock``."""
        with self._lock:
            snapshot = [entry.to_dict() for entry in self._entries]
            provider = self.provider
            self._dirty = False
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``intellect model`` / ``intellect auth`` which
        ...
        Mirrors the OntoWeb/Anthropic resync paths above.  Only applies to
        device_code-sourced entries; env/API-key-sourced entries have no
        auth.json shadow to sync from.
        """
        if not _oauth_auth_json_sync_allowed():
            return entry
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Intellect process
    ...
    def _sync_ontoweb_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        """Sync a OntoWeb pool entry from auth.json if tokens differ.

        OntoWeb OAuth refresh tokens are single-use.  When another process
        (e.g. a concurrent cron) refreshes the token via
        ``resolve_ontoweb_runtime_credentials``, it writes fresh tokens to
        auth.json under ``_auth_store_lock``.  The pool entry's tokens
        become stale.  This method detects that and adopts the newer pair,
        avoiding a "refresh token reuse" revocation on the ONTOWEB Portal.
        """
        if not _oauth_auth_json_sync_allowed():
            return entry
        if self.provider != "ontoweb" or entry.source != "device_code":
            return entry
        try:
            with _auth_store_lock():
                auth_store = _load_auth_store()
            ...
            if should_sync:
                logger.debug(
                    "Pool entry %s: syncing OntoWeb state from auth.json",
                    entry.id,
                )
                field_updates: Dict[str, Any] = {
                    "last_status": None,
            ...
            logger.debug("Failed to sync OntoWeb entry from auth.json: %s", exc)
        return entry
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (currently OntoWeb, OpenAI Codex, and xAI Grok OAuth).
        ...
        here would mean every OntoWeb/Codex/xAI refresh in a multi-provider
        setup silently flips the ``active_provider`` flag — the next
        ``intellect`` invocation that defaults to the active provider
        (e.g. setup wizard, ``intellect auth status``) would land on
        whatever provider happened to refresh last, not whatever the
        user actually chose.

        PR-A10: no-op when auth.json OAuth I/O is disabled.
        """
        if not _oauth_auth_json_sync_allowed():
            return
        ...
        try:
            with _auth_store_lock():
                ...
                if self.provider == "ontoweb":
                    ...
                    _store_provider_state(auth_store, "ontoweb", state, set_active=False)
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("intellect_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another Intellect
            ...
            elif self.provider == "ontoweb":
                synced = self._sync_ontoweb_entry_from_auth_store(entry)
                if synced is not entry:
                    entry = synced
                auth_mod.resolve_ontoweb_runtime_credentials(
                    force_refresh=force,
                )
                updated = self._sync_ontoweb_entry_from_auth_store(entry)
            else:
                return entry
        except Exception as exc:
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("intellect_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
                    ...
                    return synced
            # For xai-oauth: same race as ontoweb — another process may have
            ...
            if self.provider == "xai-oauth":
                ...
                # in-memory pool.  Mirrors the OntoWeb quarantine path above.
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    self._persist()
                    return None
            # For openai-codex: same race as xAI/ontoweb — another Intellect process
            ...
            if self.provider == "openai-codex":
                ...
                # in-memory pool.  Mirrors the xAI and OntoWeb quarantine paths.
                if auth_mod._is_terminal_codex_oauth_refresh_error(exc):
                    ...
                    self._persist()
                    return None
            # For ontoweb: another process may have consumed the refresh token
            # between our proactive sync and the HTTP call.  Re-sync from
            # auth.json and adopt the fresh tokens if available.
            if self.provider == "ontoweb":
                synced = self._sync_ontoweb_entry_from_auth_store(entry)
                if synced.refresh_token != entry.refresh_token:
                    logger.debug("OntoWeb refresh failed but auth.json has newer tokens — adopting")
                    updated = replace(
                        synced,
                        last_status=STATUS_OK,
                        last_status_at=None,
                    ...
                    return updated
                if auth_mod._is_terminal_ontoweb_refresh_error(exc):
                    logger.debug("OntoWeb refresh token is terminally invalid; clearing local token state")
                    try:
                        with _auth_store_lock():
                            auth_store = _load_auth_store()
                            state = _load_provider_state(auth_store, "ontoweb") or {
                            ...
                            entry_refresh = str(entry.refresh_token or "").strip()
                            if not store_refresh or store_refresh == entry_refresh:
                                auth_mod._quarantine_ontoweb_oauth_state(
                                    state,
                                    exc,
                                    reason="credential_pool_refresh_failure",
                                )
                                auth_mod._quarantine_ontoweb_pool_entries(
                                    auth_store,
                                    exc,
                                    reason="credential_pool_refresh_failure",
                                )
                        ...
                        logger.debug("Failed to clear terminal OntoWeb OAuth state: %s", clear_exc)

                    singleton_sources = {
                        auth_mod.ONTOWEB_DEVICE_CODE_SOURCE,
                        f"manual:{auth_mod.ONTOWEB_DEVICE_CODE_SOURCE}",
                    }
                    self._entries = [
                        item for item in self._entries
    ...
    def _entry_needs_refresh(self, entry: PooledCredential) -> bool:
        ...
        if self.provider == "ontoweb":
            # OntoWeb refresh can require network access and should happen when
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Intellect profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
                ...
                if synced is not entry:
                    entry = synced
                    cleared_any = True
            # For ontoweb entries, sync from auth.json before status checks.
            # Another process may have successfully refreshed via
            # resolve_ontoweb_runtime_credentials(), making this entry's
            ...
            # re-authed via `intellect model` / `intellect auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `intellect model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``intellect auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `intellect auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `intellect auth remove <provider> <N>` is stable across all source types.
    try:
        from intellect_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Intellect PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `intellect setup` ("Claude
        ...
        # Claude Code CLI, intellect_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.intellect/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `intellect setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"intellect_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_intellect_oauth_credentials

        for source_name, creds in (
            ("intellect_pkce", read_intellect_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # (set by persist_ontoweb_credentials(label=...) when the user ran
            # `intellect auth add ontoweb --label <name>`).  Fall back to the
        ...
        # Intellect auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from intellect_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.intellect/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `intellect auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `intellect auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Intellect auth store.  Without this gate the removal is instantly
        ...
        # Intellect owns its own Codex auth state — we do NOT auto-import from
        ...
        # via `intellect auth openai-codex`.
        ...
        # When the user logs in via ``intellect model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``intellect auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from intellect_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.intellect/.env over os.environ — the user's config file is the
    # authoritative source for Intellect credentials. Stale env vars from parent
    ...
    # won't be re-seeded from the user's shell environment or ~/.intellect/.env.
    ...
    try:
        from intellect_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from intellect_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.intellect/.env over os.environ
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            # Intellect PKCE is Intellect-owned/persistable while present, but it is
            # still a file-backed singleton and should disappear from the pool
            # when the backing OAuth file is gone.
            or entry.source == "intellect_pkce"
        )
    ]
    if len(retained) == len(entries):
        return False
