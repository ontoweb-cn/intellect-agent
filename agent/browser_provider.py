...
``kind: backend``) or ``~/.intellect/plugins/browser/<name>/`` (user, opt-in via
``plugins.enabled``).
...
class BrowserProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``intellect tools``. Defaults to ``name``."""
        return self.name
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        Used by :mod:`intellect_cli.tools_config` to inject this provider as a
        ...
        expose API key prompts, badges, managed-OntoWeb gating, and the
