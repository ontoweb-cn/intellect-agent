...
def _deterministic_call_id(fn_name: str, arguments: str, index: int = 0) -> str:
    ...
    digest = hashlib.sha256(seed.encode("utf-8", errors="replace")).hexdigest()[:12]  # codeql[py/weak-sensitive-data-hashing]: seed hash for request dedup
...
def _chat_messages_to_responses_input(
    ...
    *,
    ...
    explicitly relies on Intellect threading encrypted reasoning back across
                ...
                if isinstance(codex_reasoning, list):
                    for ri in codex_reasoning:
                        if isinstance(ri, dict) and ri.get("encrypted_content"):
                            ...
                            # it is an Intellect-side metadata key and not part
