...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Intellect with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.intellect/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Intellect
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``intellect tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``intellect setup`` for
