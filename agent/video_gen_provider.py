...
as ``kind: backend``) or ``~/.intellect/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``intellect tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$INTELLECT_HOME/cache/videos/``, creating parents as needed."""
    from intellect_constants import get_intellect_home

    path = get_intellect_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$INTELLECT_HOME/cache/videos/``.
