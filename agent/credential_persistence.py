...
``auth.json``.  They intentionally have no dependency on ``intellect_cli.auth`` so
...
# Sources Intellect owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Intellect-owned OAuth/device-code state)
