...
Owns one Codex thread per Intellect session. Drives `turn/start`, consumes
...
# Intellect' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_intellect_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Intellect/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Intellect session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _intellect_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("intellect_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="intellect",
            client_title="Intellect Agent",
            client_version=_get_intellect_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Intellect' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Intellect' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own intellect-tools callback we
            # auto-accept — the user already approved Intellect' tools
            ...
            if server_name == "intellect-tools":
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Intellect approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Intellect returns 'once', 'session', 'always', or 'deny'.
...
def _get_intellect_version() -> str:
    """Best-effort Intellect version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("intellect-agent")
