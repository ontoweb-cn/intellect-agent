...
projection into Intellect' display, approval bridging, transcript projection into
...
"codex_app_server"`. Intellect' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        if spawn_env.get("INTELLECT_KANBAN_TASK"):
            kanban_db = spawn_env.get("intellect_KANBAN_DB")
            kanban_root = (
                os.path.dirname(kanban_db)
                if kanban_db
                else spawn_env.get(
                    "intellect_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("INTELLECT_HOME", os.path.expanduser("~/.intellect")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "intellect",
        client_title: str = "Intellect Agent",
