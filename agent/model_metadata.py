...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Intellect also honours intellect_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("intellect_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
# Minimum context length required to run Intellect Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent Intellect from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    "inference-api.ontoweb.cn": "ontoweb",
    ...
    # (often 8K) makes it unusable for Intellect' system prompt, but mapping
    ...
    "open.volcengineapi.com": "volcengine",
...
def _get_context_cache_path() -> Path:
    ...
    return get_intellect_home() / "context_length_cache.yaml"
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Intellect grow conversations past the runtime limit and Ollama
...
def _resolve_ontoweb_context_length(
    model: str,
    base_url: str = "",
    api_key: str = "",
) -> Tuple[Optional[int], str]:
    """Resolve ONTOWEB Portal model context length.

    Tries the live OntoWeb inference endpoint first (authoritative), then falls
    back to OpenRouter metadata with suffix/version matching.

    OntoWeb model IDs are bare after prefix-stripping (e.g. 'qwen3.6-plus',
    ...
    Returns ``(context_length, source)`` where ``source`` is one of:
      ...
      - ``""``           — could not resolve
    """
    # Portal first — the OntoWeb /models endpoint is authoritative for what our
    ...
    def _safe_ctx(or_id: str, entry: dict) -> Optional[int]:
        ...
        if ctx <= 32768 and _model_name_suggests_kimi(or_id):
            logger.info(
                ...
                "(Kimi-family underreport, OntoWeb path); falling through to hardcoded defaults",
                ctx, or_id,
            )
            return None
        return ctx
...
def get_model_context_length(
    ...
    1. Persistent cache (previously discovered via probing).  OntoWeb URLs
    ...
    5. Provider-aware lookups (before generic OpenRouter cache):
       ...
       b. OntoWeb: live /v1/models probe first (authoritative), then OR
    ...
    if base_url and provider != "lmstudio":
        ...
        if cached is not None:
            ...
            # Invalidate stale 32k cache entries for Kimi-family models.
                ...
                _invalidate_cached_context_length(model, base_url)
            # ONTOWEB Portal: the portal /v1/models endpoint is authoritative.
            ...
            elif _infer_provider_from_url(base_url) == "ontoweb":
                logger.debug(
                    "Bypassing persistent cache for %s@%s (OntoWeb portal authoritative)",
                    model, base_url,
                )
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from intellect_cli.models import get_copilot_model_context
    ...
    if effective_provider == "ontoweb":
        ...
        if ctx:
            ...
            # want it leaking into the persistent cache for OntoWeb URLs.
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Intellect sends to providers:
