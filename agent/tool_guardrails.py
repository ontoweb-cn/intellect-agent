...
# Platforms where the user cannot interrupt a runaway tool loop in real time.
_INTERACTIVE_PLATFORMS = frozenset({"cli", "tui", "acp"})


def merge_tool_loop_guardrails_for_platform(
    section: Mapping[str, Any] | None,
    platform: str | None,
) -> dict[str, Any]:
    """Apply non-interactive defaults: hard_stop on gateway/cron/messaging surfaces."""
    base: dict[str, Any] = dict(section) if isinstance(section, Mapping) else {}
    plat = (platform or "").strip().lower()
    if not plat or plat in _INTERACTIVE_PLATFORMS:
        return base
    merged = dict(base)
    merged["hard_stop_enabled"] = True
    return merged
