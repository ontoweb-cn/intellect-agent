...
def _intellect_home_path() -> Path:
    """Resolve the active INTELLECT_HOME (profile-aware) without circular imports."""
    try:
        from intellect_constants import get_intellect_home  # local import to avoid cycles
        return get_intellect_home()
    except Exception:
        return Path(os.path.expanduser("~/.intellect"))


def _intellect_root_path() -> Path:
    """Resolve the Intellect root dir (always the parent of any profile, never per-profile)."""
    try:
        from intellect_constants import get_default_intellect_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    intellect_home = _intellect_home_path()
    intellect_root = _intellect_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(intellect_home / ".env"),
            # Top-level .env, even when running under a profile — overwriting it
            # leaks credentials across every profile that inherits from root (#15981).
            str(intellect_root / ".env"),
            # Active profile Anthropic PKCE credential store.
            str(intellect_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(intellect_root / ".anthropic_oauth.json"),
...
def get_safe_write_root() -> Optional[str]:
    """Return the resolved intellect_WRITE_SAFE_ROOT path, or None if unset."""
    root = os.getenv("intellect_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    # Intellect control-plane files: block both the ACTIVE profile's view
    # (intellect_home) AND the global root view. Without the root pass, a
    ...
    intellect_dirs = []
    for base in (_intellect_home_path(), _intellect_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in intellect_dirs:
                intellect_dirs.append(real)
        except Exception:
            continue

    for base_real in intellect_dirs:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Intellect path.

    Three categories are blocked:

      * Internal Intellect cache files under ``INTELLECT_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under INTELLECT_HOME and the global Intellect
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.intellect/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active INTELLECT_HOME (profile-aware) AND the global
    # Intellect root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (INTELLECT_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    intellect_dirs: list[Path] = []
    for base in (_intellect_home_path(), _intellect_root_path()):
        try:
            real = base.resolve()
            if real not in intellect_dirs:
    ...
    for hd in intellect_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Intellect cache file "
                "and cannot be read directly to prevent prompt injection. "
                "Use the skills_list or skill_view tools instead."
            )

    # Credential / secret stores. Exact-file matches under either
    # INTELLECT_HOME or <root>.
    ...
    for hd in intellect_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is an Intellect credential store "
    ...
    for hd in intellect_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Intellect MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is an Intellect MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Intellect profiles are separate INTELLECT_HOME dirs under
...
#
# Reference: May 2026 incident where an intellect-security profile session
# edited skills under both ``~/.intellect/profiles/intellect-security/skills/``
# AND ``~/.intellect/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under INTELLECT_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    ...
    """
    try:
        home_real = _intellect_home_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Intellect scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Intellect entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Intellect profile {info['target_profile']!r}, but the "
