...
# `export INTELLECT_REDACT_SECRETS=false`) cannot disable redaction
# mid-session.  ON by default — secure default per issue #17691. Users who
# need raw credential values in tool output (e.g. working on the redactor
# itself) can opt out via `security.redact_secrets: false` in config.yaml
# (bridged to this env var in intellect_cli/main.py, gateway/run.py, and
# cli.py) or `INTELLECT_REDACT_SECRETS=false` in ~/.intellect/.env. An opt-out
# warning is logged at gateway and CLI startup so operators see the
# downgrade — see `_log_redaction_status()` in gateway/run.py and cli.py.
_REDACT_ENABLED = os.getenv("INTELLECT_REDACT_SECRETS", "true").lower() in {"1", "true", "yes", "on"}
...
_PREFIX_PATTERNS = [
    ...
    r"xapp-\d+-[A-Za-z0-9-]{10,}",      # Slack app-level token (Socket Mode)
...
_JSON_KEY_NAMES = r"(?:api_?[Kk]ey|token|secret|password|access_token|refresh_token|auth_token|bearer|secret_value|raw_secret|secret_input|key_material|corp_id|app_secret|encoding_aes_key|dev_key|bot_token|webhook_url|signing_secret)"
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across Intellect — used by
    ``intellect config``, ``intellect status``, ``intellect dump``, and anywhere
...
def redact_sensitive_text(text: str, *, force: bool = False, code_file: bool = False) -> str:
    ...
    for URLs, ``"eyJ" in text`` for JWTs). On a typical intellect log line
    ...
    # NOTE: The redaction below only affects LOG OUTPUT — it does NOT modify
    # the actual HTTP requests the agent sends.  Pre-flight URL safety checks
    # (is_safe_url etc.) continue to receive the original, unredacted URL.
    # Redacting query params in logs prevents credentials from leaking into
    # agent.log / errors.log / gateway.log while keeping OAuth callbacks,
    # magic-link checkouts, and pre-signed share URLs fully functional.
