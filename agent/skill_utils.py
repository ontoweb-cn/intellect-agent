...
from intellect_constants import get_config_path, get_skills_dir, is_termux
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``intellect_PLATFORM`` or
            ``intellect_SESSION_PLATFORM`` env vars.  Falls back to the
    ...
    resolved_platform = (
        platform
        or os.getenv("intellect_PLATFORM")
...
# which becomes the dominant cost of ``intellect`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.intellect/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``intellect`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against INTELLECT_HOME, not cwd
        if not p.is_absolute():
            p = (intellect_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.intellect/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    intellect = metadata.get("intellect") or {}
    if not isinstance(intellect, dict):
        intellect = {}
    return {
        "fallback_for_toolsets": intellect.get("fallback_for_toolsets", []),
        "requires_toolsets": intellect.get("requires_toolsets", []),
        "fallback_for_tools": intellect.get("fallback_for_tools", []),
        "requires_tools": intellect.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    intellect = metadata.get("intellect")
    if not isinstance(intellect, dict):
        return []
    raw = intellect.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Intellect metadata, VCS, virtualenv/dependency, and cache
