"""Assemble the "learning made visible" graph for WebUI / CLI / gateway.

Profile-learned skills (agent-created or used) plus MEMORY.md / USER.md chunks.
Skill links come from ``related_skills``; memory-to-skill links from lexical overlap.
...
_SKIP_PARTS = {".archive", "node_modules", ".git"}
...
def _intellect_meta(fm: dict[str, Any]) -> dict[str, Any]:
    """``metadata.intellect`` as a dict, tolerant of malformed frontmatter."""
    meta = fm.get("metadata")
    intellect = meta.get("intellect") if isinstance(meta, dict) else None
    return intellect if isinstance(intellect, dict) else {}


def _related(fm: dict[str, Any]) -> list[str]:
    raw = fm.get("related_skills") or _intellect_meta(fm).get("related_skills")
...
def _category(fm: dict[str, Any], skill_md: Path) -> str:
    cat = fm.get("category") or _intellect_meta(fm).get("category")
    if cat:
        return str(cat)
    ...
    for source, root in roots:
        ...
        for path in root.rglob("SKILL.md"):
            ...
            if parts & _SKIP_PARTS:
                continue
            if source == "profile" and ".hub" in parts:
                continue
            yield source, path
...
def _load_usage() -> dict[str, dict[str, Any]]:
    try:
        ...
        path = get_intellect_home() / "skills" / ".usage.json"
        try:
...
def _skill_roots() -> list[tuple[str, Path]]:
    """Profile skills + hub installs only — bundled repo ``skills/`` excluded (M3)."""
    home = get_intellect_home()
    roots: list[tuple[str, Path]] = [("profile", home / "skills")]
    hub = home / "skills" / ".hub"
    if hub.is_dir():
...
def build_learning_graph() -> dict[str, Any]:
    """Full payload for Journey panels and CLI."""
    all_skills = build_skill_nodes(_skill_roots())
    learned_skills = {
        name: node
        for name, node in all_skills.items()
        if node.created_by == "agent" or node.use_count > 0
    }
