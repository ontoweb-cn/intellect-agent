...
from intellect_cli.timeouts import get_provider_request_timeout
...
def init_agent(
    ...
    Args:
        ...
        load_soul_identity (bool): If True, still use ~/.intellect/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from intellect_cli.config import load_config as _load_pc_cfg
    ...
    # both live under ~/.intellect/logs/.  Idempotent, so gateway mode
    ...
    if agent.verbose_logging:
        ...
        # on the fact that intellect_logging.setup_logging() does not
        ...
        # handler level inside intellect_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from intellect_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from intellect_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `intellect model` to "
                        "select a provider, or run `intellect setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff INTELLECT_KANBAN_TASK is set).
    ...
    # Subagents set publish_session_context=False and publish from the
    # delegate worker thread so the parent process env is not overwritten.
    if publish_session_context:
        try:
            from gateway.session_context import set_current_session_id

            set_current_session_id(agent.session_id)
        except Exception:
            os.environ["intellect_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.intellect/sessions/ alongside gateway sessions
    intellect_home = get_intellect_home()
    agent.logs_dir = intellect_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.intellect/sessions/session_{sid}.json)
    ...
    try:
        from intellect_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from intellect_cli.config import load_config as _load_agent_config
        _agent_cfg = _load_agent_config()
    except Exception:
        _agent_cfg = {}
    try:
        from agent.tool_guardrails import merge_tool_loop_guardrails_for_platform

        agent._tool_guardrails = ToolCallGuardrailController(
            ToolCallGuardrailConfig.from_mapping(
                merge_tool_loop_guardrails_for_platform(
        ...
        try:
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    # so this is additive and back-compatible.
                    _rt_ctx = getattr(agent, "runtime_context", None)
                    if _rt_ctx is not None:
                        _mid = getattr(_rt_ctx, "member_id", None)
                        if _mid:
                            _init_kwargs["member_id"] = _mid
                        _tid = getattr(_rt_ctx, "team_id", None)
                        if _tid:
                            _init_kwargs["team_id"] = _tid
                        _pid = getattr(_rt_ctx, "project_id", None)
                        if _pid:
                            _init_kwargs["project_id"] = _pid
                    if _agent_cfg:
    ...
    # RAG provider plugin (document knowledge — independent of skip_memory).
    agent._rag_manager = None
    try:
        _rag_cfg = _agent_cfg.get("rag", {}) if _agent_cfg else {}
        _rag_enabled = _rag_cfg.get("enabled", True)
        _rag_provider_name = (_rag_cfg.get("provider") or "").strip()
        if _rag_enabled and _rag_provider_name:
            from agent.rag_manager import RAGManager as _RAGManager
            from plugins.rag import load_rag_provider as _load_rag
            agent._rag_manager = _RAGManager()
            agent._rag_manager.set_max_prefetch_tokens(
                int(_rag_cfg.get("max_prefetch_tokens") or 2000)
            )
            _rp = _load_rag(_rag_provider_name)
            if _rp and _rp.is_available():
                agent._rag_manager.add_provider(_rp)
            if agent._rag_manager.providers:
                _rag_init_kwargs = {
                    "session_id": agent.session_id,
                    "platform": platform or "cli",
                    "intellect_home": str(get_intellect_home()),
                ...
                for _k in (
                    ...
                    _v = getattr(agent, f"_{_k}", None)
                    if _v:
                        _rag_init_kwargs[_k] = _v
                try:
                    from intellect_cli.profiles import get_active_profile_name
                ...
                if _rt_ctx is not None:
                    for _rk, _attr in (
                        ...
                        _rv = getattr(_rt_ctx, _attr, None)
                        if _rv:
                            _rag_init_kwargs[_rk] = _rv
                agent._rag_manager.initialize_all(**_rag_init_kwargs)
                _ra().logger.info("RAG provider '%s' activated", _rag_provider_name)
            else:
                agent._rag_manager = None
    except Exception as _rpe:
        _ra().logger.warning("RAG provider plugin init failed: %s", _rpe)
        agent._rag_manager = None

    if agent._rag_manager and agent.tools is not None and (
        agent.enabled_toolsets is None or "rag" in agent.enabled_toolsets
    ):
        _existing_tool_names = {
            t.get("function", {}).get("name")
            for t in agent.tools
        ...
        for _schema in agent._rag_manager.get_all_tool_schemas():
            _tname = _schema.get("name", "")
            if _tname and _tname in _existing_tool_names:
                continue
            _wrapped = {"type": "function", "function": _schema}
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from intellect_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            try:
                from intellect_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Intellect Agent.  Choose a model with at least "
