...
from intellect_constants import PARTIAL_STREAM_STUB_ID
from intellect_logging import set_session_context
...
from agent.conversation_helpers import (
    _ollama_context_limit_error,
    _ra,
    _ontoweb_entitlement_message,
    _print_ontoweb_entitlement_guidance,
    _is_ontoweb_inference_route,
    _billing_or_entitlement_message,
    _print_billing_or_entitlement_guidance,
    _try_refresh_ontoweb_paid_entitlement_credentials,
...
def run_conversation(
    ...
    # are reused.  The system prompt is Intellect's territory; plugins
    ...
    # all run inside Codex). Default Intellect path is bypassed entirely.
    ...
    # ==================================================================
        ...
        from agent.conversation_helpers import _drain_pre_api_steer
        ...
        for idx, msg in enumerate(messages):
            ...
            if idx == current_turn_user_idx and msg.get("role") == "user":
                ...
                if _rag_prefetch_cache:
                    from agent.rag_manager import build_rag_context_block
                    _rag_fenced = build_rag_context_block(_rag_prefetch_cache)
                    if _rag_fenced:
                        _injections.append(_rag_fenced)
        ...
        # ── Assemble system message + prefill prefix ────────────────────
        from agent.conversation_helpers import _assemble_system_message
        _sys_prefix = _assemble_system_message(
            active_system_prompt or "",
            agent.ephemeral_system_prompt,
            agent.prefill_messages,
        )
        if _sys_prefix:
            api_messages = _sys_prefix + api_messages
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Intellect tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        ontoweb_auth_retry_attempted=False
        ...
        while retry_count < max_retries:
            # ── ONTOWEB Portal rate limit guard ──────────────────────
            # If another session already recorded that OntoWeb is rate-
            # limited, skip the API call entirely.  Each attempt
            # (including SDK-level retries) counts against RPH and
            # deepens the rate limit hole.
            if agent.provider == "ontoweb":
                try:
                    from agent.ontoweb_rate_guard import (
                        ontoweb_rate_limit_remaining,
                        format_remaining as _fmt_ontoweb_remaining,
                    )
                    _ontoweb_remaining = ontoweb_rate_limit_remaining()
                    if _ontoweb_remaining is not None and _ontoweb_remaining > 0:
                        _ontoweb_msg = (
                            f"ONTOWEB Portal rate limit active — "
                            f"resets in {_fmt_ontoweb_remaining(_ontoweb_remaining)}."
                        )
                        agent._buffer_vprint(
                            f"⏳ {_ontoweb_msg} Trying fallback..."
                        )
                        agent._buffer_status(f"⏳ {_ontoweb_msg}")
            ...
            try:
                ...
                # Clear OntoWeb rate limit state on successful request —
                # proves the limit has reset and other sessions can
                # resume hitting OntoWeb.
                if agent.provider == "ontoweb":
                    try:
    ...
    # ==================================================================
                ...
                if (
                    ...
                    and _is_ontoweb_inference_route(
                        getattr(agent, "provider", "") or "",
                    ...
                    if _try_refresh_ontoweb_paid_entitlement_credentials(agent):
                        agent._vprint(
                            f"{agent.log_prefix}🔐 OntoWeb paid access verified — "
                ...
                if (
                    ...
                    and agent.provider == "ontoweb"
                    and status_code == 401
                    and not ontoweb_auth_retry_attempted
                ):
                    ontoweb_auth_retry_attempted = True
                    if agent._try_refresh_ontoweb_client_credentials(force=True):
                        print(f"{agent.log_prefix}🔐 OntoWeb agent key refreshed after 401. Retrying request...")
                        continue
                    ...
                    if not _print_ontoweb_entitlement_guidance(agent, "OntoWeb model access"):
                        print(f"{agent.log_prefix}   Most likely: Portal OAuth expired, account out of credits, or agent key revoked.")
                    print(f"{agent.log_prefix}   Troubleshooting:")
                    print(f"{agent.log_prefix}     • Re-authenticate: intellect auth add ontoweb")
                    print(f"{agent.log_prefix}     • Check credits / billing: https://portal.ontoweb.cn")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `intellect doctor` for credential-chain diagnostics, or")
                    ...
                    from intellect_constants import display_intellect_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Intellect-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: intellect config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: intellect config set ANTHROPIC_API_KEY \"\"")
                ...
                # ── ONTOWEB Portal: record rate limit & skip retries ─────
                # When OntoWeb returns a 429 that is a genuine account-
                ...
                #
                # IMPORTANT: ONTOWEB Portal multiplexes multiple upstream
                # providers (DeepSeek, Kimi, MiMo, Intellect).  A 429 can
                # also mean an UPSTREAM provider is out of capacity
                # for one specific model -- transient, clears in
                # seconds, nothing to do with the caller's quota.
                # Tripping the cross-session breaker on that would
                # block every OntoWeb model for minutes.  We use
                # ``is_genuine_ontoweb_rate_limit`` to tell the two
                ...
                if (
                    ...
                    _genuine_ontoweb_rate_limit = False
                    try:
                        from agent.ontoweb_rate_guard import (
                            is_genuine_ontoweb_rate_limit,
                            record_ontoweb_rate_limit,
                        )
                        ...
                        _genuine_ontoweb_rate_limit = is_genuine_ontoweb_rate_limit(
                            headers=_err_hdrs,
                        ...
                        if _genuine_ontoweb_rate_limit:
                            ...
                            logger.info(
                                "OntoWeb 429 looks like upstream capacity "
                ...
                # which Intellect' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Intellect' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`intellect",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            pass
                        elif _provider == "ontoweb" and _print_ontoweb_entitlement_guidance(
                            agent,
                            "OntoWeb model access",
                        ):
                            pass
                        elif _provider in {"openai-codex", "xai-oauth", "ontoweb"} and status_code == 401:
                            if _provider == "openai-codex":
                                agent._vprint(f"{agent.log_prefix}   💡 Codex OAuth token was rejected (HTTP 401). Your token may have been", force=True)
                                agent._vprint(f"{agent.log_prefix}      refreshed by another client (Codex CLI, VS Code). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Run `codex` in your terminal to generate fresh tokens.", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Then run `intellect auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `intellect model`.", force=True)
                            else:  # ontoweb
                                agent._vprint(f"{agent.log_prefix}   💡 ONTOWEB Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: intellect auth add ontoweb --type oauth", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Check your portal account: https://portal.ontoweb.cn", force=True)
                                # ``:free`` is OpenRouter slug syntax; ONTOWEB Portal will reject
                                ...
                                if isinstance(_model, str) and _model.endswith(":free"):
                                    ...
                                    agent._vprint(f"{agent.log_prefix}         ONTOWEB Portal won't recognize that model name. Either switch to a", force=True)
                                    agent._vprint(f"{agent.log_prefix}         OntoWeb catalog model, or run `/model openrouter:{_model}` to use OpenRouter.", force=True)
                        else:
                            agent._vprint(f"{agent.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: intellect setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        intellect fallback add   (interactive picker — same as `intellect model`)",
                            force=True,
                        )
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        _policy_response = (
                            ...
                            f"(not an Intellect/gateway failure).\n\n"
                            f"Provider message: {_summary}\n\n"
                            f"Try rephrasing the request, narrowing the context, or "
                            f"adding a fallback provider with `intellect fallback add`."
                        )
                        return {
    ...
    if final_response is None and (
        ...
        _kanban_task = os.environ.get("INTELLECT_KANBAN_TASK")
        if _kanban_task:
            try:
    ...
    from agent.conversation_helpers import _apply_file_mutation_verifier_footer
    final_response = _apply_file_mutation_verifier_footer(
        agent, final_response, interrupted,
    )

    # ── Turn-completion explainer ──────────────────────────────────
    from agent.conversation_helpers import _apply_turn_completion_explainer
    final_response = _apply_turn_completion_explainer(
