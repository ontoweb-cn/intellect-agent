...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Intellect starts and finds ``~/.openclaw/``.

    Points users at ``intellect claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``intellect claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Intellect, run "
        "`intellect claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `intellect claw cleanup` (renames it to ~/.openclaw.pre-migration — "
