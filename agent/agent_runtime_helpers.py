...
def try_recover_primary_transport(
    ...
    Skipped for proxy/aggregator providers (OpenRouter, OntoWeb) which
    ...
    if provider_lower in {"ontoweb", "ontoweb-research"}:
        return False
...
def dump_api_request_debug(
    ...
    *,
    ...
    try:
        ...
        if env_var_enabled("intellect_DUMP_REQUEST_STDOUT"):
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    # ONTOWEB Portal proxies to OpenRouter behind the scenes — identical
    # OpenAI-wire envelope cache_control semantics. Treat it as an
    # OpenRouter-equivalent endpoint for caching layout purposes.
    is_ontoweb_portal = "ontoweb" in eff_base_url.lower()
    ...
    if is_native_anthropic:
        return True, True
    if (is_openrouter or is_ontoweb_portal) and is_claude:
        return True, False
    # ONTOWEB Portal Qwen (e.g. qwen3.6-plus) takes the same envelope-layout
    ...
    if is_ontoweb_portal and "qwen" in model_lower:
        return True, False
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    if str(client_kwargs.get("base_url", "")).startswith("acp://"):
        # Generic ACP sub-agent provider (P1a): base_url acp://<command> routes
        # to ACPSubagentOpenAIFacade. The command is derived from the provider
        # (e.g. claude-code/opencode) or resolved via env/acp_command.
        from agent.acp_subagent_client import ACPSubagentOpenAIFacade

        client = ACPSubagentOpenAIFacade(**client_kwargs)
        _ra().logger.info(
            "Generic ACP sub-agent client created (%s, shared=%s) %s",
            reason,
            shared,
            agent._client_log_context(),
        )
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    turn-scoped).
    """
    from intellect_cli.providers import determine_api_mode
    ...
    if hasattr(agent, "context_compressor") and agent.context_compressor:
        ...
        try:
            from intellect_cli.config import load_config, get_compatible_custom_providers
            _sm_cfg = load_config()
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    _rbac_tokens = None
    try:
        from agent.member_rbac import bind_tool_rbac_context

        _cfg = getattr(agent, "_cli_config", None) or getattr(agent, "_config", None)
        if _cfg is None:
            try:
                ...
                _cfg = None
        _rbac_tokens = bind_tool_rbac_context(
            getattr(agent, "runtime_context", None),
            _cfg,
        )
    except Exception:
        _rbac_tokens = None
    ...
    def _clear_rbac():
        if _rbac_tokens is not None:
            try:
                from agent.member_rbac import reset_tool_rbac_context
                reset_tool_rbac_context(_rbac_tokens)
            except Exception:
                pass

    try:
        return _invoke_tool_body(
            agent,
            function_name,
            function_args,
            effective_task_id,
...
def _invoke_tool_body(
    ...
    *,
    ...
    if not pre_tool_block_checked:
        try:
            from intellect_cli.plugins import get_pre_tool_call_block_message
    ...
    try:
        from agent.member_rbac import check_member_tool_permission

        tool_denied = check_member_tool_permission(function_name, function_args)
        if tool_denied:
            return json.dumps({"error": tool_denied}, ensure_ascii=False)
    except Exception:
        pass
