"""Unified removal contract for every credential source Intellect reads from.

Intellect seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.intellect/.env
    claude_code   — ~/.claude/.credentials.json
    intellect_pkce   — ~/.intellect/.anthropic_oauth.json
    device_code   — auth.json providers.<provider> (ontoweb, openai-codex, ...)
    qwen-cli      — ~/.qwen/oauth_creds.json
    gh_cli        — gh auth token
    config:<name> — custom_providers config entry
    model_config  — model.api_key when model.provider == "custom"
    manual        — user ran `intellect auth add`
...
unify here is **removal**:
    ...
    ``intellect auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, ontoweb device_code (partial), intellect_pkce, copilot gh_cli, and
...
class RemovalStep:
    ...
    Attributes:
        provider: Provider pool key (``"xai"``, ``"anthropic"``, ``"ontoweb"``, ...).
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.intellect/.env  → clear it
    ...
    """
    from intellect_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            f"(not in ~/.intellect/.env).",
            "  Unset it there (shell profile, systemd EnvironmentFile, "
            "launchd plist, etc.) or it will keep being visible to Intellect.",
            f"  The pool entry is now suppressed — Intellect will ignore "
            f"{env_var} until you run `intellect auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Intellect stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `intellect auth add anthropic` to re-enable if needed.",
    ])


def _remove_intellect_pkce(provider: str, removed) -> RemovalResult:
    """~/.intellect/.anthropic_oauth.json is ours — delete it outright."""
    from intellect_constants import get_intellect_home

    result = RemovalResult()
    oauth_file = get_intellect_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Intellect Anthropic OAuth credentials")
...
def _remove_ontoweb_device_code(provider: str, removed) -> RemovalResult:
    """OntoWeb OAuth lives in auth.json providers.ontoweb — clear it and suppress.

    We suppress in addition to clearing because nothing else stops a future
    `intellect auth add ontoweb` (or any other path that writes providers.ontoweb)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `intellect auth add ontoweb` to re-engage, which is the
...
def _remove_minimax_oauth(provider: str, removed) -> RemovalResult:
    ...
    Same pattern as OntoWeb: single-source OAuth state with refresh tokens.
...
def _remove_xai_oauth_loopback_pkce(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``intellect auth remove xai-oauth <N>`` silently undoes
    ...
    Belt-and-braces against the manual entry path: ``intellect auth add
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `intellect model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Intellect auth store is not enough — _seed_from_singletons() would
    ...
    via ``intellect auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `intellect auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `intellect auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Intellect stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `intellect auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="intellect_pkce",
        remove_fn=_remove_intellect_pkce,
        description="~/.intellect/.anthropic_oauth.json",
    ))
    register(RemovalStep(
        provider="ontoweb", source_id="device_code",
        remove_fn=_remove_ontoweb_device_code,
        description="auth.json providers.ontoweb",
    ))
    register(RemovalStep(
