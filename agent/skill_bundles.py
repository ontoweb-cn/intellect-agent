...
Storage
-------
Bundles live in ``~/.intellect/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``INTELLECT_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``intellect bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under INTELLECT_HOME.

    Honors ``intellect_BUNDLES_DIR`` for tests; falls back to
    ``<INTELLECT_HOME>/skill-bundles``.
    """
    override = os.environ.get("intellect_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_intellect_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `intellect bundles` CLI subcommand.
# ---------------------------------------------------------------------------
