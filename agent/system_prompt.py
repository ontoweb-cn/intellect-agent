...
upstream prefix cache warm.  See ``intellect-agent-dev``'s
...
* ``stable``   — identity (SOUL.md or DEFAULT_AGENT_IDENTITY), tool
  guidance, computer-use guidance, ontoweb subscription block, tool-use
...
def _ra():
    ...
    ``build_context_files_prompt``, ``build_ontoweb_subscription_prompt``,
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Intellect never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want INTELLECT_HOME persona while keeping
    ...
    # ── Multi-user SOUL layers (team → project → member) ──────────────
    # If the agent has a runtime_context, layer in team/project/member SOUL
    # below the profile-level SOUL already loaded above.
    _rt_ctx = getattr(agent, "runtime_context", None)
    if _rt_ctx is not None:
        try:
            from agent.runtime_context import assemble_soul
            _layer_souls = assemble_soul(_rt_ctx)
            for _layer in _layer_souls:
                if _layer:
                    stable_parts.append(_layer)
                    _soul_loaded = True
        except Exception:
            pass
        _mid = getattr(_rt_ctx, "member_id", None)
        if _mid:
            _mrole = getattr(_rt_ctx, "member_role", None) or "member"
            _ctx_bits = [f"member_id={_mid}", f"role={_mrole}"]
            if getattr(_rt_ctx, "team_id", None):
                _ctx_bits.append(f"team={_rt_ctx.team_id}")
            if getattr(_rt_ctx, "project_id", None):
                _ctx_bits.append(f"project={_rt_ctx.project_id}")
            stable_parts.append(
                "Runtime member context (session-scoped): " + ", ".join(_ctx_bits) + "."
            )
    ...
    # Pointer to the intellect-agent skill + docs for user questions about Intellect itself.
    stable_parts.append(intellect_AGENT_HELP_GUIDANCE)
    ...
    # INTELLECT_KANBAN_TASK env var). Normal chat sessions never see
    ...
    ontoweb_subscription_prompt = _r.build_ontoweb_subscription_prompt(agent.valid_tool_names)
    if ontoweb_subscription_prompt:
        stable_parts.append(ontoweb_subscription_prompt)
    ...
    # Active-profile hint — names the Intellect profile the agent is running
    # under so it doesn't conflate ~/.intellect/skills/ (default profile) with
    # ~/.intellect/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Intellect profile: default. Other profiles (if any) live "
            "under ~/.intellect/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Intellect profile: {active_profile}. This session reads "
            f"and writes ~/.intellect/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.intellect/skills/, ~/.intellect/plugins/, "
            f"~/.intellect/cron/, ~/.intellect/memories/ — those belong to a "
    ...
    if not agent.skip_context_files:
        ...
        # mode).  The gateway process runs from the intellect-agent install
    ...
    if agent._rag_manager:
        try:
            ...
            if _rag_block:
                volatile_parts.append(_rag_block)
        except Exception:
            pass
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Intellect never rebuilds or reinjects parts of it
