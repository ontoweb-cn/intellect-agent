...
Resolution order for text tasks (auto mode):
  ...
  3. ONTOWEB Portal (~/.intellect/auth.json active provider)
...
def _compression_threshold_for_model(model: Optional[str]) -> Optional[float]:
    ...
    consumed before Intellect triggers summarization.  Higher values delay
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``intellect_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``intellect_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("intellect_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("intellect_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "IntellectAgent",
}
...
# ONTOWEB Portal extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed by ONTOWEB Portal.
#
# The tags are computed from agent.portal_tags so the client= marker stays
# in lockstep with intellect_cli.__version__ across every Portal call site
# (main loop, aux, compression, web_extract). Do not inline a literal here;
# see agent/portal_tags.py for the rationale.
from agent.portal_tags import ontoweb_portal_tags as _ontoweb_portal_tags


def _ontoweb_extra_body() -> dict:
    """Return a fresh ONTOWEB Portal ``extra_body`` dict.

    Computed at call time so a hot-reloaded ``intellect_cli.__version__`` is
    reflected without restarting long-running processes.
    """
    return {"tags": _ontoweb_portal_tags()}
...
# ``_ontoweb_extra_body()`` or import ``ontoweb_portal_tags`` directly.
ONTOWEB_EXTRA_BODY = _ontoweb_extra_body()
NOUS_EXTRA_BODY = ONTOWEB_EXTRA_BODY  # deprecated alias (pre-OntoWeb rename)

# Set at resolve time — True if the auxiliary client points to ONTOWEB Portal
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Intellect Agent)",
...
def _pool_runtime_api_key(entry: Any) -> str:
    ...
    # provider-specific fallback (e.g. agent_key for ontoweb).
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    # runtime_base_url handles provider-specific logic (e.g. ontoweb prefers inference_base_url).
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``intellect_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_ontoweb_auth() -> Optional[dict]:
    """Read and validate ~/.intellect/auth.json for an active OntoWeb provider.

    Returns the provider state dict if OntoWeb is active with tokens,
    otherwise None.
    """
    pool_present, entry = _select_pool_entry("ontoweb")
    if pool_present:
        if entry is None:
            return None
        return {
            ...
            "inference_base_url": _pool_runtime_base_url(entry, _ONTOWEB_DEFAULT_BASE_URL),
    ...
    try:
        ...
        if data.get("active_provider") != "ontoweb":
            return None
        provider = data.get("providers", {}).get("ontoweb", {})
        # Must have at least an access_token or agent_key
        if not provider.get("agent_key") and not provider.get("access_token"):
            return None
        return provider
    except Exception as exc:
        logger.debug("Could not read OntoWeb auth: %s", exc)
        return None


def _ontoweb_api_key(provider: dict) -> str:
    """Extract a usable OntoWeb inference JWT from stored auth state."""
    from intellect_cli.auth import _ontoweb_invoke_jwt_is_usable
    ...
    for token_key, expiry_key in (
        ...
        if not isinstance(token, str) or not token.strip():
            continue
        if _ontoweb_invoke_jwt_is_usable(
            token,
...
def _ontoweb_base_url() -> str:
    """Resolve the OntoWeb inference base URL from env or default."""
    return os.getenv("ONTOWEB_INFERENCE_BASE_URL", _ONTOWEB_DEFAULT_BASE_URL)


def _resolve_ontoweb_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Return fresh OntoWeb runtime credentials when available.
    ...
    """
    try:
        from intellect_cli.auth import resolve_ontoweb_runtime_credentials

        creds = resolve_ontoweb_runtime_credentials(
            timeout_seconds=float(os.getenv("INTELLECT_ONTOWEB_TIMEOUT_SECONDS", "15")),
            force_refresh=force_refresh,
        )
    except Exception as exc:
        logger.debug("Auxiliary OntoWeb runtime credential resolution failed: %s", exc)
        return None
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``intellect auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``intellect_cli.auth``'s singleton runtime resolver for older
    ...
    try:
        from intellect_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Intellect auth store.
    ...
    try:
        from intellect_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from intellect_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from intellect_cli.models import copilot_default_headers
...
def _try_ontoweb(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    # Check cross-session rate limit guard before attempting OntoWeb —
    # if another session already recorded a 429, skip OntoWeb entirely
    # to avoid piling more requests onto the tapped RPH bucket.
    try:
        from agent.ontoweb_rate_guard import ontoweb_rate_limit_remaining
        _remaining = ontoweb_rate_limit_remaining()
        if _remaining is not None and _remaining > 0:
            logger.debug(
                "Auxiliary: skipping ONTOWEB Portal (rate-limited, resets in %.0fs)",
                _remaining,
            )
    ...
    if runtime is None and not ontoweb:
        logger.warning(
            "Auxiliary OntoWeb client unavailable: no OntoWeb authentication found "
            "(run: intellect auth)."
        )
        _mark_provider_unhealthy("ontoweb", ttl=60)
        return None, None
    if runtime is None and ontoweb:
        logger.debug(
            "Auxiliary OntoWeb: runtime JWT refresh failed; checking stored "
            "auth.json token."
        )
    global auxiliary_is_ontoweb
    auxiliary_is_ontoweb = True
    logger.debug("Auxiliary client: ONTOWEB Portal")

    # Ask the Portal which model it currently recommends for this task type.
    # The /api/ontoweb/recommended-models endpoint is the authoritative source:
    # it distinguishes paid vs free tier recommendations, and get_ontoweb_recommended_aux_model
    # auto-detects the caller's tier via check_ontoweb_free_tier().  Fall back to
    # _ONTOWEB_MODEL (google/gemini-3-flash-preview) when the Portal is unreachable
    # or returns a null recommendation for this task type.
    model = _ONTOWEB_MODEL
    try:
        from intellect_cli.models import get_ontoweb_recommended_aux_model
        recommended = get_ontoweb_recommended_aux_model(vision=vision)
        if recommended:
            model = recommended
            logger.debug(
    ...
    if runtime is not None:
        ...
        api_key = _ontoweb_api_key(ontoweb or {})
        if not api_key:
            logger.warning(
                "Auxiliary OntoWeb client unavailable: no usable inference JWT found "
                "(run: intellect auth add ontoweb)."
            )
            _mark_provider_unhealthy("ontoweb", ttl=60)
            return None, None
        base_url = str((ontoweb or {}).get("inference_base_url") or _ontoweb_base_url()).rstrip("/")
    return (
        OpenAI(
            api_key=api_key,
            base_url=base_url,
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `intellect setup` or `intellect model` and enter a valid http(s) base URL."
        ) from exc
...
def _try_azure_foundry(
    *,
    ...
    Mirrors the ``_try_anthropic`` / ``_try_ontoweb`` shape but delegates to
    :func:`intellect_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from intellect_cli.runtime_provider import _resolve_azure_foundry_runtime
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second intellect
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``intellect config aux reset``)."""
...
def _ontoweb_portal_account_has_fresh_paid_access() -> bool:
    """Return True only when the fresh OntoWeb account API says paid access is allowed."""
    try:
        ...
        logger.debug("Auxiliary OntoWeb paid-entitlement refresh check failed: %s", exc)
        return False
...
def _recoverable_pool_provider(
    ...
    if base_url_host_matches(base, "openrouter.ai"):
        return "openrouter"
    if base_url_host_matches(base, "inference-api.ontoweb.cn"):
        return "ontoweb"
...
def _try_payment_fallback(
    ...
    _alias_to_label = {"openrouter": "openrouter", "ontoweb": "ontoweb",
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    Priority:
      1. User's main provider + main model, regardless of provider type.
         ...
         chat.  Users on OpenRouter/OntoWeb get their chosen chat model; users
      ...
      2. OpenRouter → OntoWeb → custom → Codex → API-key providers (fallback
         chain, only used when the main provider has no working client).
    """
    global auxiliary_is_ontoweb, _stale_base_url_warned
    auxiliary_is_ontoweb = False  # Reset — _try_ontoweb() will set True if it wins
    runtime = _normalize_main_runtime(main_runtime)
    ...
    #    scenario where a user switches providers via `intellect model` but the
    #    old OPENAI_BASE_URL lingers in ~/.intellect/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: intellect model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.intellect/.env",
    ...
    # on aggregators (OpenRouter, OntoWeb) who previously got routed to a
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from intellect_cli.copilot_auth import copilot_request_headers
...
def resolve_provider_client(
    ...
    Args:
        provider: Provider identifier.  One of:
            "openrouter", "ontoweb", "openai-codex" (or "codex"),
    ...
    if provider == "ontoweb":
        ...
        client, default = _try_ontoweb(vision=_is_vision)
        if client is None:
            logger.warning("resolve_provider_client: ontoweb requested "
                           "but ONTOWEB Portal not configured (run: intellect auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: intellect model)")
                return None, None
    ...
    # OpenRouter / OntoWeb bills for side tasks they thought were running on
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: intellect model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    try:
        from intellect_cli.runtime_provider import _get_named_custom_provider
        ...
        # entries that coincidentally match a canonical provider (e.g. ``ontoweb``)
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: intellect doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("intellect_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from intellect_cli.models import _should_use_copilot_responses_api
    ...
    if pconfig.auth_type == "external_process":
        ...
        if provider == "ontoweb":
            return resolve_provider_client("ontoweb", model, async_mode)
        if provider == "openai-codex":
...
def _resolve_strict_vision_backend(
    ...
    if provider == "ontoweb":
        return _try_ontoweb(vision=True)
    if provider == "openai-codex":
...
def get_available_vision_backends() -> List[str]:
    ...
    Order: active provider → OpenRouter → OntoWeb → stop.  This is the single
    ...
    if main_provider and main_provider not in {"auto", ""}:
        if main_provider in _VISION_AUTO_PROVIDER_ORDER:
            ...
            if client is not None:
                available.append(main_provider)
    # 2. OpenRouter, 3. OntoWeb — skip if already covered by main provider.
...
def resolve_vision_provider_client(
    ...
    *,
    ...
    if requested == "auto":
        ...
        #      zai → glm-5v-turbo). OntoWeb is the exception: it has a dedicated
        #      strict vision backend with tier-aware defaults, so it must not
        #      fall through to the user's text chat model here.
        #   2. OpenRouter  (vision-capable aggregator fallback)
        #   3. ONTOWEB Portal (vision-capable aggregator fallback)
        #   4. Stop
        main_provider = _read_main_provider()
        main_model = _read_main_model()
        if main_provider and main_provider not in {"auto", ""}:
            vision_model = _PROVIDER_VISION_MODELS.get(main_provider, main_model)
            if main_provider == "ontoweb":
...
def get_auxiliary_extra_body() -> dict:
    ...
    Includes ONTOWEB Portal product tags when the auxiliary client is backed
    by ONTOWEB Portal. Returns empty dict otherwise.
    """
    return _ontoweb_extra_body() if auxiliary_is_ontoweb else {}
...
def auxiliary_max_tokens_param(value: int) -> dict:
    ...
    if (not or_key
            and _read_ontoweb_auth() is None
...
def _refresh_ontoweb_auxiliary_client(
    *,
    cache_provider: str,
    model: Optional[str],
    async_mode: bool,
    ...
    """Refresh OntoWeb runtime creds, rebuild the client, and replace the cache entry."""
    runtime = _resolve_ontoweb_runtime_api(force_refresh=True)
    if runtime is None:
        return None, model
...
def _aux_task_config_is_default(task: str, task_config: Dict[str, Any]) -> bool:
    """True when *task_config* matches DEFAULT_CONFIG auxiliary defaults."""
    if not task_config:
        return True
    try:
    ...
    defaults = aux.get(task) if isinstance(aux, dict) else None
    if not isinstance(defaults, dict):
        return False
    for key, default_val in defaults.items():
        user_val = task_config.get(key)
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`intellect_cli.plugins.PluginContext.register_auxiliary_task`) the
    ...
    When ``auxiliary.lightrag`` is still at factory defaults and the user has
    customized ``auxiliary.compression``, compression settings are inherited
    once (migration from the pre-lightrag-task ingest path).
    """
    if not task:
        return {}
    try:
    ...
    if (
        task == "lightrag"
        and _aux_task_config_is_default("lightrag", task_config)
    ):
        compression_cfg = aux.get("compression", {}) if isinstance(aux, dict) else {}
        if isinstance(compression_cfg, dict) and compression_cfg:
            if not _aux_task_config_is_default("compression", compression_cfg):
                try:
                    from intellect_cli.config import DEFAULT_CONFIG
                    lightrag_defaults = (
                        (DEFAULT_CONFIG.get("auxiliary") or {}).get("lightrag") or {}
                    )
                except ImportError:
                    lightrag_defaults = {}
                merged = dict(compression_cfg)
                for key, val in task_config.items():
                    if val is None or val == "":
                        continue
                    default_val = (
                        lightrag_defaults.get(key)
                        if isinstance(lightrag_defaults, dict)
                        else None
                    )
...
def _build_call_kwargs(
    ...
    if provider == "ontoweb" or auxiliary_is_ontoweb:
        merged_extra.setdefault("tags", []).extend(_ontoweb_portal_tags())
    if merged_extra:
    ...
    *,
    ...
    Args:
        task: Auxiliary task name ("compression", "vision", "web_extract",
              "lightrag", "session_search", "skills_hub", "mcp",
              "title_generation").
        ...
        if client is None:
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                raise RuntimeError(
                    ...
                    f"variable, or switch to a different provider with `intellect model`."
                )
    ...
    try:
        ...
        # ── OntoWeb auth refresh parity with main agent ──────────────────
        client_is_ontoweb = (
            resolved_provider == "ontoweb"
            or base_url_host_matches(_base_info, "inference-api.ontoweb.cn")
        )
        if (
            _is_payment_error(first_err)
            and client_is_ontoweb
            and _ontoweb_portal_account_has_fresh_paid_access()
        ):
            refreshed_client, refreshed_model = _refresh_ontoweb_auxiliary_client(
                cache_provider=resolved_provider or "ontoweb",
                model=final_model,
                async_mode=False,
                base_url=resolved_base_url,
                api_key=resolved_api_key,
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s: refreshed OntoWeb runtime credentials after paid account check, retrying",
                    task or "call",
                )
        ...
        if _is_auth_error(first_err) and client_is_ontoweb:
            refreshed_client, refreshed_model = _refresh_ontoweb_auxiliary_client(
                cache_provider=resolved_provider or "ontoweb",
                model=final_model,
                async_mode=False,
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s: refreshed OntoWeb runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
...
async def async_call_llm(
    ...
    *,
    ...
    try:
        ...
        client_is_ontoweb = (
            resolved_provider == "ontoweb"
            or base_url_host_matches(_client_base, "inference-api.ontoweb.cn")
        )
        if (
            _is_payment_error(first_err)
            and client_is_ontoweb
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s (async): refreshed OntoWeb runtime credentials after paid account check, retrying",
                    task or "call",
                )
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
        ...
        if _is_auth_error(first_err) and client_is_ontoweb:
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s (async): refreshed OntoWeb runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
