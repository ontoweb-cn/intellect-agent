...
#[pyfunction]
pub fn is_fts5_unavailable_error(exc: &PyAny) -> PyResult<bool> {
    let msg: String = exc
        .str()
        .unwrap_or_default()
        .to_string()
...
#[pyfunction]
pub fn drop_fts_triggers_py(cursor: &PyAny) -> PyResult<()> {
...
#[pyfunction]
pub fn fts_trigger_count_py(cursor: &PyAny) -> PyResult<usize> {
    ...
    let row = cursor.call_method1("execute", (sql.as_str(),))?;
    let result: &PyAny = row.call_method0("fetchone")?;
...
#[pyfunction]
pub fn rebuild_fts_indexes_py(cursor: &PyAny) -> PyResult<()> {
