# nix/nixosModules.nix — NixOS module for intellect-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: intellect runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $INTELLECT_HOME/.env
# and read by intellect at startup — no container recreation needed for env changes.
#
# Tool resolution: the intellect wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.intellect-agent;
    effectivePackage =
    ...
    intellect-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-intellect-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "intellect-config-attrs";
      description = "Intellect YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "intellect-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "intellect-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'intellect_DOC_EOF'\n${value}\nintellect_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "intellect-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/intellect";
    ...
    # intellect user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "intellect-container-entrypoint" ''
      set -eu

      INTELLECT_UID="''${INTELLECT_UID:?INTELLECT_UID must be set}"
      INTELLECT_GID="''${INTELLECT_GID:?INTELLECT_GID must be set}"

      # ── Group: ensure a group with GID=$INTELLECT_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$INTELLECT_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="intellect"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$INTELLECT_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$INTELLECT_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$INTELLECT_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$INTELLECT_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="intellect"
        TARGET_HOME="/home/intellect"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$INTELLECT_UID" -g "$INTELLECT_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$INTELLECT_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$INTELLECT_UID:$INTELLECT_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure INTELLECT_HOME is owned by the target user.
      ...
      if [ -n "''${INTELLECT_HOME:-}" ] && [ -d "$INTELLECT_HOME" ]; then
        find "$INTELLECT_HOME" \! -user "$INTELLECT_UID" -exec chown "$INTELLECT_UID:$INTELLECT_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/intellect-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/intellect-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/intellect ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/intellect
        chmod 0440 /etc/sudoers.d/intellect
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$INTELLECT_UID" --regid="$INTELLECT_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $INTELLECT_HOME/.env.
    ...
    # Default: /var/lib/intellect/workspace → /data/workspace.
  ...
  in {
    options.services.intellect-agent = with lib; {
      enable = mkEnableOption "Intellect Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = intellect-agent;
        description = "The intellect-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/intellect";
        description = "State directory. Contains .intellect/ subdir (INTELLECT_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Intellect config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $INTELLECT_HOME/.env at activation time.
          Intellect reads this file on every startup via load_intellect_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $INTELLECT_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $INTELLECT_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `intellect gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the intellect user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the intellect plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Intellect discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          intellect_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as intellect (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in intellect-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the intellect CLI to environment.systemPackages and export
          INTELLECT_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.intellect symlink to the service
            stateDir. These users are automatically added to the intellect group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.intellect-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the intellect CLI to system PATH and export INTELLECT_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.intellect/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.INTELLECT_HOME = "${cfg.stateDir}/.intellect";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.intellect-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the intellect user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.intellect-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed intellect
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure intellect is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.intellect        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.intellect/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.intellect/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.intellect/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.intellect/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.intellect/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."intellect-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.intellect
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.intellect ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.intellect ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.intellect -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.intellect/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.intellect/$_subdir"
            chmod 2770 "${cfg.stateDir}/.intellect/$_subdir"
            find "${cfg.stateDir}/.intellect/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.intellect/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.intellect/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.intellect/config.yaml
            chmod 0640 ${cfg.stateDir}/.intellect/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.intellect/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.intellect/.managed
          chmod 0644 ${cfg.stateDir}/.intellect/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.intellect/.container-mode <<'intellect_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    intellect_bin=${containerDataDir}/current-package/bin/intellect
    intellect_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.intellect/.container-mode
            chmod 0644 ${cfg.stateDir}/.intellect/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.intellect/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.intellect";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.intellect" ]; then
                  rm -f "${symlinkPath}"
                  echo "intellect-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.intellect -> stateDir/.intellect for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.intellect";
                target = "${cfg.stateDir}/.intellect";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "intellect-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.intellect/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.intellect/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.intellect/auth.json
              fi
            ''}
          ''}
          ...
          # Intellect reads $INTELLECT_HOME/.env at startup via load_intellect_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.intellect/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'intellect_NIX_ENV_EOF'
    ${envFileContent}
    intellect_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
                echo "" >> "$ENV_FILE"
                cat "${f}" >> "$ENV_FILE"
        ...
        find ${cfg.stateDir}/.intellect/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.intellect/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.intellect/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.intellect-agent = {
          description = "Intellect Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            INTELLECT_HOME = "${cfg.stateDir}/.intellect";
            intellect_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $INTELLECT_HOME/.env by the activation script. load_intellect_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/intellect"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the intellect group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.intellect-agent = {
          description = "Intellect Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              INTELLECT_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              INTELLECT_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env INTELLECT_UID="$INTELLECT_UID" \
                --env INTELLECT_GID="$INTELLECT_GID" \
                --env INTELLECT_HOME=${containerDataDir}/.intellect \
                --env intellect_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/intellect gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
