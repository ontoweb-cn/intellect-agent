# nix/intellect-agent.nix — Overridable Intellect Agent package
#
...
let
  nodejs = nodejs_22;
  intellectVenv = callPackage ./python.nix {
    inherit uv2nix pyproject-nix pyproject-build-systems;
    dependency-groups = [ "all" ] ++ extraDependencyGroups;
  };

  intellectNpmLib = callPackage ./lib.nix {
    inherit npm-lockfile-fix nodejs;
  };

  intellectTui = callPackage ./tui.nix {
  ...
  # to a dev checkout — the loader reads them from INTELLECT_BUNDLED_PLUGINS.
  ...
  checkPackageCollisions = ''
    ...
    core = set()
    venv_sp = pathlib.Path('${intellectVenv}/${sitePackagesPath}')
        ...
        for di in sp.glob('*.dist-info'):
            ...
            for line in meta.read_text().splitlines():
                if line.startswith('Name:'):
                    ...
                    if pkg in core:
                        print(f'ERROR: plugin package \"{pkg}\" collides with a package in intellect sealed venv', file=sys.stderr)
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    ${lib.concatMapStringsSep "\n"
      (name: ''
        makeWrapper ${intellectVenv}/bin/${name} $out/bin/${name} \
          ...
          ${lib.optionalString (rev != null) ''--set INTELLECT_REVISION ${rev} \''}
    ...
    ${lib.optionalString (extraPythonPackages != [ ]) ''
      ...
      ${intellectVenv}/bin/python3 -c "${checkPackageCollisions}"
      echo "=== No collisions ==="
    ''}
  ...
  passthru = {
    ...
    devShellHook = ''
      STAMP=".nix-stamps/intellect-agent"
      STAMP_VALUE="${pyprojectHash}:${uvLockHash}"
      if [ ! -f "$STAMP" ] || [ "$(cat "$STAMP")" != "$STAMP_VALUE" ]; then
        echo "intellect-agent: installing Python dependencies..."
        ...
        source .venv/bin/activate
        export INTELLECT_PYTHON=${intellectVenv}/bin/python3
      fi
    '';
  };
