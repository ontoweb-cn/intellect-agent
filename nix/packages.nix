# nix/packages.nix — Intellect Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    { pkgs, lib, inputs', ... }:
    let
      intellectAgent = pkgs.callPackage ./intellect-agent.nix {
