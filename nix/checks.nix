...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      intellect-agent = self'.packages.default;
      intellectVenv = intellect-agent.intellectVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "intellect-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${intellectVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "intellect-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "intellect-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${intellect-agent}/bin/intellect || (echo "FAIL: intellect binary missing"; exit 1)
          test -x ${intellect-agent}/bin/intellect-agent || (echo "FAIL: intellect-agent binary missing"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "intellect-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in intellect intellect-agent intellect-acp; do
            test -x ${intellect-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "intellect-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${intellect-agent}/share/intellect-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${intellect-agent}/share/intellect-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "INTELLECT_BUNDLED_SKILLS" ${intellect-agent}/bin/intellect || \
            (echo "FAIL: INTELLECT_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: INTELLECT_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "intellect-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${intellect-agent}/share/intellect-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${intellect-agent}/share/intellect-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "INTELLECT_BUNDLED_PLUGINS" ${intellect-agent}/bin/intellect || \
            (echo "FAIL: INTELLECT_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: INTELLECT_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "intellect-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${intellect-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${intellect-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "INTELLECT_TUI_DIR" ${intellect-agent}/bin/intellect || \
            (echo "FAIL: INTELLECT_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: INTELLECT_TUI_DIR set in wrapper"
        ...
        # Verify INTELLECT_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        intellect-node = pkgs.runCommand "intellect-node-version" { } ''
          set -e
          echo "=== Checking INTELLECT_NODE in wrapper ==="
          grep -q "INTELLECT_NODE" ${intellect-agent}/bin/intellect || \
            (echo "FAIL: INTELLECT_NODE not set in wrapper"; exit 1)
          echo "PASS: INTELLECT_NODE present in wrapper"

          INTELLECT_NODE=$(sed -n "s/^export INTELLECT_NODE='\(.*\)'/\1/p" ${intellect-agent}/bin/intellect)
          test -x "$INTELLECT_NODE" || (echo "FAIL: INTELLECT_NODE=$INTELLECT_NODE not executable"; exit 1)
          echo "PASS: INTELLECT_NODE executable at $INTELLECT_NODE"

          NODE_MAJOR=$("$INTELLECT_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All INTELLECT_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify intellect_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "intellect-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(intellect_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking intellect_MANAGED guards ==="
          check_blocked "config set" ${intellect-agent}/bin/intellect config set model foo
          check_blocked "config edit" ${intellect-agent}/bin/intellect config edit
        ...
        extra-python-packages = let
          ...
          intellectWithExtra = intellect-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "intellect-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${intellectWithExtra}/bin/intellect || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${intellectWithExtra}/bin/intellect || \
          ...
          if grep -q "PYTHONPATH" ${intellect-agent}/bin/intellect; then
        ...
        extra-dependency-groups = let
          intellectWithGroups = intellect-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "intellect-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${intellectWithGroups}"
          echo "venv: ${intellectWithGroups.intellectVenv}"
        ...
        messaging-variant = pkgs.runCommand "intellect-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.intellectVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "intellect-config-roundtrip" {
          ...
          merge_and_load() {
            local intellect_home="$1"
            export INTELLECT_HOME="$intellect_home"
            ${configMergeScript} ${nixSettings} "$intellect_home/config.yaml"
            ${intellectVenv}/bin/python3 -c '
import json, sys
