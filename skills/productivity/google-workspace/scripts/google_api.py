#!/usr/bin/env python3
"""Google Workspace API CLI for Intellect Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Intellect-facing JSON contract and falls back to the Python client
...
from _intellect_home import get_intellect_home
...
def _gws_binary() -> str | None:
    override = os.getenv("intellect_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Intellect Agent")
