...
INTELLECT_HOME_DIR="${INTELLECT_HOME:-$HOME/.intellect}"
intellect_CFG="${INTELLECT_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Intellect config has twozero_td MCP entry ──
if [[ ! -f "$intellect_CFG" ]]; then
    echo -e " ${FAIL} Intellect config not found at ${intellect_CFG}"
    manual_steps+=("Create ${intellect_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$intellect_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Intellect config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Intellect config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$intellect_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${intellect_CFG} manually"); }
    manual_steps+=("Restart Intellect session to pick up config change")
fi
