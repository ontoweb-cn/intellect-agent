...
## 5. Intellect Configuration

### Config location

`$INTELLECT_HOME/config.yaml` (defaults to `~/.intellect/config.yaml` when `INTELLECT_HOME` is unset)
...
Restart the Intellect session for changes to take effect. The MCP connection is
