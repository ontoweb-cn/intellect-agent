...
## 5. GODMODE FAST — Intellect 4 405B (Zero Refusal)

**Model:** `ontoweb/intellect-4-405b`
**Strategy:** Intellect models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Intellect Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.intellect/config.yaml`:
