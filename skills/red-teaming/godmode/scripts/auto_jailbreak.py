...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("INTELLECT_HOME", os.path.expanduser("~/.intellect")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("INTELLECT_HOME", Path.home() / ".intellect")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Intellect config paths
# ═══════════════════════════════════════════════════════════════════

INTELLECT_HOME = Path(os.getenv("INTELLECT_HOME", Path.home() / ".intellect"))
CONFIG_PATH = INTELLECT_HOME / "config.yaml"
PREFILL_PATH = INTELLECT_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # OntoWeb/Intellect models — already uncensored, just needs clean prompt
    "intellect": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "intellect" in model_lower or "ontoweb" in model_lower:
        return "intellect"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Intellect config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.intellect/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Intellect for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Intellect for changes to take effect.")
