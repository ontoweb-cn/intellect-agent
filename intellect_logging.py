"""Centralized logging setup for Intellect Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.intellect/logs/`` (profile-aware via ``get_intellect_home()``).
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_intellect_session_injector", False):
    ...
    _session_record_factory._intellect_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``intellect logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "intellect_plugins"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("intellect_cli", "cli"),
    "cron": ("cron",),
}


# ---------------------------------------------------------------------------
# JSON log formatter (P4-4)
# ---------------------------------------------------------------------------

class JsonLogFormatter(logging.Formatter):
    """Emit log records as JSON Lines for structured-log ingestion.

    Activated via ``INTELLECT_LOG_FORMAT=json`` or ``config.yaml``
    ``logging.format: json``.  Each line is a standalone JSON object
    with keys: ``ts``, ``level``, ``logger``, ``msg``, ``session``,
    ``file``, ``line``, ``exc``.
    """
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        if sid:
            payload["session"] = sid.strip(" []")
        if record.exc_info and record.exc_info[1]:
            payload["exc"] = redact_sensitive_text(self.formatException(record.exc_info))
        payload["file"] = f"{record.filename}:{record.lineno}"
        return _json.dumps(payload, ensure_ascii=False, default=str)

    @staticmethod
    def is_enabled(config: dict | None = None) -> bool:
        """Return True if JSON logging is active."""
        from os import environ
        if environ.get("INTELLECT_LOG_FORMAT", "").lower() == "json":
            return True
        cfg = config or {}
        log_cfg = cfg.get("logging") if isinstance(cfg.get("logging"), dict) else {}
        return str(log_cfg.get("format", "")).lower() == "json"
...
def setup_logging(
    *,
    ...
    """Configure the Intellect logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    intellect_home
        Override for the Intellect home directory.  Falls back to
        ``get_intellect_home()`` (profile-aware).
    log_level
    ...
    _fmt = JsonLogFormatter() if _use_json else RedactingFormatter(_LOG_FORMAT)
    if _use_json:
        logger.debug("Logging in JSON Lines format (INTELLECT_LOG_FORMAT=json)")
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_intellect_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._intellect_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the intellect group. However, both ``_open()`` (initial creation)
