...
running under the dispatcher (env var ``INTELLECT_KANBAN_TASK`` set) or when
the active profile explicitly enables the ``kanban`` toolset for
orchestrator work. A normal ``intellect chat`` session still sees **zero**
kanban tools in its schema unless configured.

Why tools instead of just shelling out to ``intellect kanban``?

1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``intellect kanban complete …``
   inside the container, where ``intellect`` isn't installed and the DB
   isn't mounted. Tools run in the agent's Python process, so they
   always reach ``~/.intellect/kanban.db`` regardless of terminal backend.
...
Humans continue to use the CLI (``intellect kanban …``), the slash command
(``/kanban …``), and the TUI — all three bypass the agent entirely. The tools are for dispatcher-spawned
...
def _check_kanban_mode() -> bool:
    ...
    1. ``INTELLECT_KANBAN_TASK`` is set (dispatcher-spawned worker), OR
    2. The current profile has ``kanban`` in its toolsets config
       (orchestrator profiles like techlead that route work via Kanban).

    Humans running ``intellect chat`` without the kanban toolset see zero
...
def _default_task_id(arg: Optional[str]) -> Optional[str]:
    ...
    if arg:
        return arg
    env_tid = os.environ.get("INTELLECT_KANBAN_TASK")
...
def _stamp_worker_session_metadata(
    ...
    try:
        ...
        session_id = get_session_env("intellect_SESSION_ID", "")
    except Exception:
        session_id = os.environ.get("intellect_SESSION_ID", "")
    if not session_id:
        return metadata
...
def _enforce_worker_task_ownership(tid: str) -> Optional[str]:
    ...
    A process spawned by the dispatcher has ``INTELLECT_KANBAN_TASK`` set
    ...
    ``INTELLECT_KANBAN_TASK`` in env) aren't subject to this check — their
...
def _connect(board: Optional[str] = None):
    ...
    (``intellect_KANBAN_DB`` → ``INTELLECT_KANBAN_BOARD`` env → current symlink
    → ``default``). Per-tool ``board`` lets a Telegram-side agent override
    the env-pinned active board without restarting Intellect.
    """
...
#   - No-op outside dispatcher-spawned worker context (no ``INTELLECT_KANBAN_TASK``).
...
def heartbeat_current_worker_from_env() -> bool:
    ...
    Identity comes from:
      * ``INTELLECT_KANBAN_TASK`` — task id (required; absence means no-op)
      * ``intellect_KANBAN_RUN_ID`` — pins the run row so we don't heartbeat
        a stale run that may have already been reclaimed
      * ``intellect_KANBAN_CLAIM_LOCK`` — claim lock for ``heartbeat_claim``;
    ...
    tid = os.environ.get("INTELLECT_KANBAN_TASK")
    if not tid:
        return False
    ...
    try:
        kb, conn = _connect()
        try:
            claim_lock = os.environ.get("intellect_KANBAN_CLAIM_LOCK")
            try:
                kb.heartbeat_claim(conn, tid, claimer=claim_lock)
            except Exception:
                logger.debug("auto-heartbeat: heartbeat_claim failed", exc_info=True)
            run_id_raw = os.environ.get("intellect_KANBAN_RUN_ID")
            run_id: Optional[int]
            try:
...
def _handle_show(args: dict, **kw) -> str:
    ...
    if not tid:
        return tool_error(
            "task_id is required (or set INTELLECT_KANBAN_TASK in the env)"
        )
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            # intellect_KANBAN_CLAIM_LOCK in the worker env at spawn time
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    # an authoritative-looking name like ``intellect-system`` and poison
    # the future-worker context with what reads as a system directive.
    # Cross-task commenting itself remains unrestricted (see #19713) —
    # comments are the deliberate handoff channel between tasks.
    author = os.environ.get("intellect_PROFILE") or "worker"
    board = args.get("board")
    try:
        kb, conn = _connect(board=board)
        try:
...
def _handle_create(args: dict, **kw) -> str:
    ...
    tenant = args.get("tenant") or os.environ.get("intellect_TENANT")
    # Stamp the originating session id when the agent loop runs under
    # ACP (which sets intellect_SESSION_ID before invoking tools). NULL on
    # CLI / TUI paths and on legacy hosts that don't set the env.
    session_id = args.get("session_id")
    if not session_id:
        try:
            from gateway.session_context import get_session_env

            session_id = get_session_env("intellect_SESSION_ID", "") or None
        except Exception:
            session_id = os.environ.get("intellect_SESSION_ID")
    ...
    try:
        ...
        try:
            new_tid = kb.create_task(
                ...
                created_by=os.environ.get("intellect_PROFILE") or "worker",
...
_DESC_TASK_ID_DEFAULT = (
    "Task id. If omitted, defaults to INTELLECT_KANBAN_TASK from the env "
    "(the task the dispatcher spawned you to work on)."
)

_DESC_BOARD = (
    "Kanban board slug to target. When omitted, the call resolves the "
    "active board the usual way: intellect_KANBAN_DB env → "
    "INTELLECT_KANBAN_BOARD env → the 'current' symlink under the kanban "
        ...
        "properties": {
            ...
            "summary": {
                ...
                "description": (
                    ...
                    "Run History in the TUI and in downstream "
                    "workers' context."
                ),
            },
            "metadata": {
        ...
        "properties": {
            ...
            "tenant": {
                ...
                "description": (
                    ...
                    "Defaults to intellect_TENANT env if set."
                ),
            },
            "priority": {
                "type": "integer",
