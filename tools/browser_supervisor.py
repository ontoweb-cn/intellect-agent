...
One ``CDPSupervisor`` runs per Intellect ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "intellect-dialog-bridge.invalid"
...
(() => {
  if (window.__intellectDialogBridgeInstalled) return;
  window.__intellectDialogBridgeInstalled = true;
  const ENDPOINT = "http://intellect-dialog-bridge.invalid/";
