...
def get_memory_dir() -> Path:
    ...
    When ``members.memory_scope`` is ``member``, returns the per-member
    directory ``members/<id>/memories/`` resolved from the CLI session file
    or the single default member.  Otherwise returns the profile-level
    ``~/.intellect/memories/``.
    """
    home = get_intellect_home()
    # Check if member scope is enabled
    try:
        from intellect_cli.config import load_config
        config = load_config()
        scope = config.get("members", {}).get("memory_scope", "profile") if config else "profile"
        if scope == "member":
            member_id = _resolve_current_member_id()
            if member_id:
                member_dir = home / "members" / member_id / "memories"
...
def _resolve_current_member_id() -> str | None:
    """Get the current member ID from runtime env, CLI session file, or sole member."""
    env_mid = (os.environ.get("INTELLECT_MEMBER_ID") or "").strip()
    if env_mid:
        return env_mid
    home = get_intellect_home()
    try:
        session_file = home / ".cli-session.json"
...
class MemoryStore:
    ...
    def __init__(self, memory_char_limit: int = 2200, user_char_limit: int = 1375):
        ...
        # Pre-compute mem_dir once to avoid repeated load_config() calls
        self._mem_dir: Path = get_memory_dir()
    ...
    def load_from_disk(self):
        ...
        self.memory_entries = self._read_file(self._mem_dir / "MEMORY.md")
        self.user_entries = self._read_file(self._mem_dir / "USER.md")
