...
from tools.environments.local import _intellect_PROVIDER_ENV_BLOCKLIST
...
def _load_intellect_env_vars() -> dict[str, str]:
    """Load ~/.intellect/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active Intellect profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale intellect-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=intellect-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Intellect process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=intellect-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a intellect process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Intellect
    process leaves its container behind permanently because there's no
    subsequent Intellect process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=intellect-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=intellect-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``INTELLECT_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("INTELLECT_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using INTELLECT_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'intellect'
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.intellect/sandboxes/). Non-persistent
        ...
        container_name = f"intellect-{uuid.uuid4().hex[:8]}"
        # Labels make intellect-created containers identifiable to:
        #   * the orphan reaper (`intellect-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`intellect-task-id`, `intellect-profile`)
        #   * operators running `docker ps --filter label=intellect-agent=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Intellect profile is captured at
        ...
        label_args = [
            "--label", "intellect-agent=1",
            "--label", f"intellect-task-id={task_label}",
            "--label", f"intellect-profile={profile_name}",
        ]
        self._labels = {
            "intellect-agent": "1",
            "intellect-task-id": task_label,
            "intellect-profile": profile_name,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior Intellect process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Intellect secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _intellect_PROVIDER_ENV_BLOCKLIST)
        intellect_env = _load_intellect_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if value is None:
                value = intellect_env.get(key)
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``intellect-*`` but were
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "--filter", "label=intellect-agent=1",
                    "--filter", f"label=intellect-task-id={task_label}",
                    "--filter", f"label=intellect-profile={profile_label}",
                    "--format", "{{.ID}}\t{{.State}}",
                ],
                capture_output=True,
                text=True,
        ...
        # container) but can happen if a previous Intellect process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Intellect exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Intellect process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Intellect startup. That covers the
        ...
        # outlives Intellect processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"intellect-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``intellect /quit``
