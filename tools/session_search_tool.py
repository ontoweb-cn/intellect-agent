...
the get_anchored_view / get_messages_around primitives in intellect_state.
...
# Third-party integrations tag their sessions with INTELLECT_SESSION_SOURCE=tool
# so they don't clutter the user's session history.
_HIDDEN_SESSION_SOURCES = ("tool",)


def _resolve_actor_member_id() -> Optional[str]:
    """Runtime member for session_search scoping (WebUI/gateway workers)."""
    env_mid = (os.environ.get("INTELLECT_MEMBER_ID") or "").strip()
    if env_mid:
        return env_mid
    try:
        from tools.memory_tool import _resolve_current_member_id

        return _resolve_current_member_id()
    except Exception:
        return None


def _session_visible_to_actor(db, session_id: str, actor_member_id: Optional[str]) -> bool:
    if not actor_member_id or not session_id:
        return True
    try:
        meta = db.get_session(session_id, actor_member_id=actor_member_id)
    except TypeError:
        meta = db.get_session(session_id)
    except Exception:
        logging.debug("session visibility check failed for %s", session_id, exc_info=True)
        return False
    return bool(meta)
...
def _list_recent_sessions(
    db, limit: int, current_session_id: str = None, actor_member_id: str = None,
) -> str:
    ...
    try:
        ...
        if actor_member_id:
            list_kwargs["member_id"] = actor_member_id
...
def _scroll(
    ...
    # Session existence + member visibility
    if not _session_visible_to_actor(db, session_id, actor_member_id):
        return tool_error(f"session_id not found: {session_id}", success=False)
    try:
        if actor_member_id:
            session_meta = db.get_session(session_id, actor_member_id=actor_member_id) or {}
        else:
...
def _discover(
    ...
    for r in raw_results:
        ...
        if resolved_sid not in seen_sessions:
            if actor_member_id and not _session_visible_to_actor(
                db, resolved_sid, actor_member_id,
            ):
                continue
            row = dict(r)
...
def session_search(
    ...
    actor_member_id = _resolve_actor_member_id()
    ...
    if not query or not isinstance(query, str) or not query.strip():
        return _list_recent_sessions(
            db, limit, current_session_id, actor_member_id=actor_member_id,
        )
...
def check_session_search_requirements() -> bool:
    ...
    try:
        from intellect_state import DEFAULT_DB_PATH
