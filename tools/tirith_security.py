...
it is automatically downloaded from GitHub releases to $INTELLECT_HOME/bin/tirith.
...
def _get_intellect_home() -> str:
    """Return the Intellect home directory, respecting INTELLECT_HOME env var."""
    return str(get_intellect_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_intellect_home(), ".tirith-install-failed")
...
def _intellect_bin_dir() -> str:
    """Return $INTELLECT_HOME/bin, creating it if needed."""
    d = os.path.join(_get_intellect_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $INTELLECT_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_intellect_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $INTELLECT_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $INTELLECT_HOME/bin/tirith
    ...
    intellect_bin = os.path.join(_intellect_bin_dir(), "tirith")
    if os.path.isfile(intellect_bin) and os.access(intellect_bin, os.X_OK):
        _resolved_path = intellect_bin
        _install_failure_reason = ""
        _clear_install_failed()
