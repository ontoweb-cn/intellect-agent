...
def _is_intellect_provider_credential(name: str) -> bool:
    """True if ``name`` is an Intellect-managed provider credential (API key,
    token, or similar) per ``_intellect_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Intellect API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    try:
        ...
        return False
    return name in _intellect_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Intellect-managed provider credentials (from
    ``_intellect_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to an Intellect-managed
    ...
    Non-Intellect third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_intellect_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Intellect provider "
                "credential %r (blocked by _intellect_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Intellect-managed provider credentials must not be passed
                ...
                if _is_intellect_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Intellect "
                        "provider credential %r from config.yaml (blocked "
                        "by _intellect_PROVIDER_ENV_BLOCKLIST). Operator "
