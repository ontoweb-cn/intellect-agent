...
Lets the LLM write a Python script that calls Intellect tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `intellect_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `intellect_tools.py` with file-based RPC stubs
...
# available on every platform Intellect itself runs on.
...
# match a safe prefix, the operational intellect_ allowlist, or (on Windows) an
# OS-essential name.
#
# NB: the broad "intellect_" prefix was deliberately removed (#27303) — it leaked
# intellect_*-named config that lacks a secret substring (e.g. intellect_BASE_URL,
# intellect_KANBAN_DB, intellect_*_WEBHOOK).  The child only needs the few
# location/profile vars in _intellect_CHILD_ALLOWED below; intellect_RPC_SOCKET /
# intellect_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational intellect_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set intellect_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_intellect_CHILD_ALLOWED = frozenset({
    "INTELLECT_HOME",
    "intellect_PROFILE",
    "intellect_CONFIG",
    "intellect_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational intellect_* vars (_intellect_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret intellect_* vars dropped by the tightened allowlist (#27303). The
    # broad "intellect_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like intellect_KANBAN_DB / intellect_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _intellect_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("intellect_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped intellect_* var.
            _dropped_intellect.append(k)
    if _dropped_intellect:
        logger.debug(
            "execute_code: dropped %d non-allowlisted intellect_* var(s) from the "
            ...
            len(_dropped_intellect),
            ", ".join(sorted(_dropped_intellect)),
        )
    return scrubbed
...
# ---------------------------------------------------------------------------
# intellect_tools.py code generator
# ---------------------------------------------------------------------------
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Intellect-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Intellect-profile soft guard."""',
...
def generate_intellect_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the intellect_tools.py stub module.
...
"""Auto-generated Intellect tools RPC stubs."""
...
def _connect():
    ...
    intellect_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["intellect_RPC_SOCKET"]
...
"""Auto-generated Intellect tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("intellect_RPC_DIR") or os.path.join(tempfile.gettempdir(), "intellect_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated intellect_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/intellect_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_intellect_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/intellect_tools.py", tools_src)
        ...
        env_prefix = (
            f"intellect_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("INTELLECT_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    to a subset of Intellect tools.
    ...
    # --- Set up temp directory with intellect_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="intellect_sandbox_")
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"intellect_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated intellect_tools module.
        ...
        tools_src = generate_intellect_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "intellect_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  intellect_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["intellect_RPC_SOCKET"] = rpc_endpoint
        ...
        # Ensure the intellect-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from intellect_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _intellect_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _intellect_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # INTELLECT_TIMEZONE is an internal Intellect setting and must not leak
        # into child processes.
        _tz_name = os.getenv("INTELLECT_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("INTELLECT_TIMEZONE", None)

        # Per-profile HOME isolation: redirect system tool configs into
        # {INTELLECT_HOME}/home/ when that directory exists.
        from intellect_constants import get_subprocess_home
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.intellect/.env')).
...
def _load_config() -> dict:
    ...
    ``intellect --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (intellect-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``intellect tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and intellect-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.intellect/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Intellect tools programmatically. "
        ...
        f"Available via `from intellect_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into intellect_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from intellect_tools import {import_str}` "
