...
provider** — turn on a backend by enabling a plugin (``intellect plugins
enable video_gen/<name>``) and selecting it in ``intellect tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/intellect-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `intellect tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `intellect tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `intellect tools` → "
...
# `intellect tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`intellect tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `intellect tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
