...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``intellect tools`` repaint, tool-registration scans,
    ...
    2. ``~/.intellect/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_intellect_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.intellect/.env`` first, then ``os.environ``.

    Wraps :func:`intellect_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from intellect_cli.config import get_env_value as _intellect_get_env_value

        value = _intellect_get_env_value(name)
...
def intellect_xai_user_agent() -> str:
    """Return a stable Intellect-specific User-Agent for xAI HTTP calls."""
    try:
        from intellect_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Intellect-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Intellect-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``intellect_cli.config.get_env_value`` so keys
    stored in ``~/.intellect/.env`` (the standard Intellect location) are honored —
