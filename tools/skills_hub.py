...
Used by intellect_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _guarded_http_get(url: str, *, timeout: int = 20) -> Optional[httpx.Response]:
    ...
    for _ in range(_MAX_SKILL_FETCH_REDIRECTS + 1):
        ...
        try:
            # Use safe_httpx_client to validate the actual TCP peer IP
            # after connect, closing the DNS rebinding TOCTOU window where
            # is_safe_url() pre-flight check passes a public IP but the
            # live connection lands on a private IP (TTL=0 attack).
            from tools.safe_http import safe_httpx_client, SSRFConnectBlocked
            with safe_httpx_client(timeout=timeout) as safe_client:
                resp = safe_client.get(current_url, follow_redirects=False)
        except (httpx.HTTPError, SSRFConnectBlocked, OSError) as exc:
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            intellect_meta = metadata.get("intellect", {})
            if isinstance(intellect_meta, dict):
                tags = intellect_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            intellect_meta = metadata.get("intellect", {})
            if isinstance(intellect_meta, dict):
                raw_tags = intellect_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by ONTOWEB) but not activated
    by default — they don't appear in the system prompt and aren't copied to
    ~/.intellect/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                intellect_meta = meta_block.get("intellect", {})
                if isinstance(intellect_meta, dict):
                    tags = intellect_meta.get("tags", [])
...
def _load_intellect_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for intellect_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if intellect_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - intellect_INDEX_CACHE_FILE.stat().st_mtime
            if age < intellect_INDEX_TTL:
                return json.loads(intellect_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(intellect_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Intellect index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Intellect index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        intellect_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        intellect_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class intellectIndexSource(SkillSource):
    """Skill source backed by the centralized Intellect Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_intellect_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "intellect-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "intellect-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        intellectIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if source_filter == "all":
        for src in sources:
            if (src.source_id() == "intellect-index"
