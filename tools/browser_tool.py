...
supports multiple backends — **Browser Use** (cloud, default for OntoWeb
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    intellect_node_bin = str(intellect_home / "node" / "bin")
    intellect_node_root = str(intellect_home / "node")
    intellect_nm_bin = str(intellect_home / "node_modules" / ".bin")
    return [intellect_node_bin, intellect_node_root, intellect_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
# plugins drop in under ``~/.intellect/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    fallback. If unset, fall back to Browser Use (managed OntoWeb gateway or
    ...
    browser plugins (``~/.intellect/plugins/browser/<vendor>/``) participate
    ...
    if resolved is None:
        # Auto-detect path: Browser Use first (managed OntoWeb gateway or
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Intellect silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Intellect CLI where sys.stdout and sys.stderr both report
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-intellect_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed intellect processes — this way every clean intellect exit sweeps
    ...
    # Sweep orphans from other crashed intellect processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live intellect processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current intellect PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live intellect process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning intellect process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced intellect PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent intellect instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-intellect_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live intellect process.
            continue
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Intellect-managed
    ...
    # Build an extended search PATH including Intellect-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            ...
            if not recheck:
                intellect_nm = str(get_intellect_home() / "node_modules" / ".bin")
                recheck = shutil.which("agent-browser", path=intellect_nm)
            if not recheck:
                intellect_node_bin = str(get_intellect_home() / "node" / "bin")
                recheck = shutil.which("agent-browser", path=intellect_node_bin)
            if not recheck:
                intellect_node_root = str(get_intellect_home() / "node")
                recheck = shutil.which("agent-browser", path=intellect_node_root)
            if recheck:
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this intellect PID as the session owner (cross-process safe
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = intellect_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Intellect
    ...
    from intellect_constants import get_intellect_dir
    screenshots_dir = get_intellect_dir("cache/screenshots", "browser_screenshots")
