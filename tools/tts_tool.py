...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.intellect/config.yaml``. Intellect
...
Configuration is loaded from ~/.intellect/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``intellect_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from intellect_cli.config import get_env_value as _get_env_value
    except ImportError:
...
from tools.xai_http import intellect_xai_user_agent
...
def _get_default_output_dir() -> str:
    ...
    return str(get_intellect_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.intellect/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.intellect/config.yaml.
    ...
    try:
        ...
        logger.debug("intellect_cli.config not available, using default TTS config")
        return {}
    except Exception as e:
...
# custom voice-cloning scripts, etc.) into Intellect without any Python code
...
#           output_format: wav
#
# Intellect writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__intellect_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `intellect model` or set XAI_API_KEY.")
    ...
    # output_format when Intellect actually needs a non-default format/override.
    ...
    response = requests.post(
        ...
        headers={
            ...
            "User-Agent": intellect_xai_user_agent(),
        },
        json=payload,
        timeout=60,
    )
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Intellect caches Piper voice models.

    Resolves to ``~/.intellect/cache/piper-voices/`` under the active
    INTELLECT_HOME so voice downloads follow profile boundaries.
    """
    from intellect_constants import get_intellect_dir
    root = Path(get_intellect_dir("cache/piper-voices", "piper_voices_cache"))
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.intellect/config.yaml (tts: section).
    ...
    platform = get_session_env("intellect_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'intellect-agent[mistral]'"
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run intellect setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'intellect setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'intellect tools' and select Piper under TTS, or install manually: "
...
def _resolve_openai_audio_client_config() -> tuple[str, str]:
    ...
    if managed_gateway is None:
        ...
        if managed_ontoweb_tools_enabled() or prefers_gateway("tts"):
            message += (
                ". "
                + ontoweb_tool_gateway_unavailable_message(
    ...
    return managed_gateway.ontoweb_user_token, urljoin(
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_intellect_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
