...
# All skills live in ~/.intellect/skills/ (seeded from bundled skills/ on install).
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from INTELLECT_HOME/.env."""
    env_path = get_intellect_home() / ".env"
...
def _is_gateway_surface() -> bool:
    if env_var_enabled("intellect_GATEWAY_SESSION"):
        return True
    from gateway.session_context import get_session_env
    return bool(get_session_env("intellect_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_intellect_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.intellect/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``intellect_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
        return get_session_env("intellect_SESSION_PLATFORM") or ""
    except Exception:
        return ""
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``intellect_PLATFORM`` environment variable
    3. ``intellect_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from intellect_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("intellect_PLATFORM") or _get_session_platform()
...
# ── Skills scan cache ────────────────────────────────────────────────────
# Skills change rarely at runtime; a full scan is ~50+ file reads + YAML
# parses. Cache per process with mtime-based invalidation.
_SKILLS_CACHE: dict[str, Any] = {"mtime": 0.0, "data": None, "data_all": None}


def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.intellect/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``intellect skills`` config UI). Default False
            filters out disabled skills.

    Returns:
        List of skill metadata dicts (name, description, category).
    """
    # Check cache: invalidate when SKILLS_DIR mtime changes
    try:
        _mtime = SKILLS_DIR.stat().st_mtime if SKILLS_DIR.exists() else 0.0
        if _SKILLS_CACHE["mtime"] == _mtime:
            _key = "data_all" if skip_disabled else "data"
            if _SKILLS_CACHE[_key] is not None:
                return list(_SKILLS_CACHE[_key])  # shallow copy for safety
    except OSError:
        pass
    ...
    # Scan order (later wins on name conflict): profile → team → project → member
    dirs_to_scan = []
    if SKILLS_DIR.exists():
        dirs_to_scan.append(SKILLS_DIR)

    # ── Multi-user skill directories ────────────────────────────────────
    # Scan team skills/, project skills/, and member skills/ so that
    # skills defined in any layer are discoverable.  Later layers override
    # earlier ones on name conflict (handled by seen_names dedup in the
    # scan loop below).
    _home = SKILLS_DIR.parent  # INTELLECT_HOME
    for _pattern, _label in [
        ...
        try:
            for _skill_dir in sorted(_home.glob(_pattern)):
                if _skill_dir.is_dir():
                    dirs_to_scan.append(_skill_dir)
        except Exception:
            logger.debug('non-critical operation failed', exc_info=True)
    ...
    try:
        if SKILLS_DIR.exists():
            _SKILLS_CACHE["mtime"] = SKILLS_DIR.stat().st_mtime
        if skip_disabled:
            _SKILLS_CACHE["data_all"] = list(skills)
        else:
            _SKILLS_CACHE["data"] = list(skills)
    except OSError:
        pass
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": f"No skills found. Skills directory created at {display_intellect_home()}/skills/",
                },
                ensure_ascii=False,
            )
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from intellect_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: intellect plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.intellect/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `intellect skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.intellect.* first (agentskills.io convention), fall back to top-level
        intellect_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            intellect_meta = metadata.get("intellect", {}) or {}
