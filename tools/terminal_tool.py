...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Intellect exit
...
# display_intellect_home imported lazily at call site (stale-module safety during intellect update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of intellect directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "intellect-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = get_session_env("INTELLECT_SESSION_KEY", "")
    except Exception:
        session_key = os.getenv("INTELLECT_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("intellect_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (intellect_INTERACTIVE=1).
    ...
    try:
        os.environ["intellect_SPINNER_PAUSE"] = "1"
        time.sleep(0.2)
        ...
        if "intellect_SPINNER_PAUSE" in os.environ:
            del os.environ["intellect_SPINNER_PAUSE"]
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (intellect_INTERACTIVE=1):
    ...
    # interactive Intellect password prompt or the sudo -S password-pipe path.
    ...
    # silently without Intellect prompting.
    if not has_configured_password and not sudo_password and _sudo_nopasswd_works():
        return command, None

    if not has_configured_password and not sudo_password and env_var_enabled("intellect_INTERACTIVE"):
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so Intellect can track lifecycle and output.
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``intellect-agent=1`` for the current
    profile that match the issue #20561 leak class — containers left behind
    by Intellect processes that exited without firing ``atexit`` (SIGKILL,
    ...
    * ``terminal.docker_orphan_reaper: false`` disables it entirely (the
      ...
      Intellect processes in the same profile and don't trust the
    ...
    * ``_docker_orphan_reaper_ran`` flag — sweep runs once per Python
    ...
    # 2 × lifetime_seconds gives sibling Intellect processes a generous grace
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, intellectSweEnv, ...)
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.intellect/.env or environment variables."
        )


_WIKI_DOCKER_FORWARD_ENV = (
    "WIKI_PATH",
    "WIKI_SCOPE",
    "WIKI_SCOPE_ID",
    "WIKI_WRITE_MODE",
...
def _merge_docker_forward_env(configured: list) -> list:
    """Always forward scoped wiki env vars into Docker sandboxes."""
    merged: list = []
    seen: set[str] = set()
    for item in list(configured or []) + list(_WIKI_DOCKER_FORWARD_ENV):
...
def _get_env_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "docker_forward_env": _merge_docker_forward_env(
            _parse_env_var("TERMINAL_DOCKER_FORWARD_ENV", "[]", json.loads, "valid JSON")
        ),
        ...
        # Startup orphan reaper for intellect-tagged containers left behind by
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        # prior Intellect processes that hit SIGKILL / OOM / a closed terminal
        ...
        if modal_state["selected_backend"] != "direct":
            if modal_state["managed_mode_blocked"]:
                raise ValueError(
                    ...
                    "OntoWeb Tool Gateway access is not currently available and no direct "
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "Use terminal(background=true) so Intellect can track the process, then run "
    ...
    try:
        ...
        if background:
            ...
            try:
                ...
                # intellect-agent dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in intellect-agent dev work (PRs #31329, "
                            ...
                            "name='github/intellect-agent-dev', "
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    _gw_platform = _gse("intellect_SESSION_PLATFORM", "")
                    if _gw_platform:
                        _gw_chat_id = _gse("intellect_SESSION_CHAT_ID", "")
                        _gw_thread_id = _gse("intellect_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("intellect_SESSION_USER_ID", "")
                        _gw_user_name = _gse("intellect_SESSION_USER_NAME", "")
                        _gw_message_id = _gse("intellect_SESSION_MESSAGE_ID", "")
            ...
            try:
                from intellect_cli.plugins import invoke_hook
