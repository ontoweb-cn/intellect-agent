...
This module provides the glue:
    - ``intellectTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Intellect Agent"
"""
...
_USER_SKIPPED_SENTINEL = "__intellect_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses INTELLECT_HOME so each profile gets its own OAuth tokens.
    Layout: ``INTELLECT_HOME/mcp-tokens/``
    """
    try:
        from intellect_constants import get_intellect_home
        base = Path(get_intellect_home())
    except ImportError:
        base = Path(os.environ.get("INTELLECT_HOME", str(Path.home() / ".intellect")))
...
# ---------------------------------------------------------------------------
# intellectTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class intellectTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        INTELLECT_HOME/mcp-tokens/<server_name>.json         -- tokens
        INTELLECT_HOME/mcp-tokens/<server_name>.client.json   -- client info
        INTELLECT_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Intellect records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Intellect.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://intellect-agent.ontoweb.cn/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `intellect mcp login <server>` later to "
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Intellect Agent")
    scope = cfg.get("scope")
