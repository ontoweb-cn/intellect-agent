...
def managed_ontoweb_tools_enabled(*, force_fresh: bool = False) -> bool:
    """Return True when the user has paid ONTOWEB Portal service access.
...
def ontoweb_tool_gateway_unavailable_message(
    capability: str = "the OntoWeb Tool Gateway",
    *,
    force_fresh: bool = False,
) -> str:
    """Return account-aware guidance for an unavailable OntoWeb Tool Gateway path."""
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `intellect model` to refresh your "
        "ONTOWEB Portal login and billing status."
    )
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.intellect/.env`` (via
    ``intellect_cli.config.get_env_value`` when available) so tool-side
