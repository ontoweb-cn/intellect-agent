...
Backend is selected during ``intellect tools`` setup (web.backend in config.yaml).
When available, Intellect can route Firecrawl calls through a OntoWeb-hosted tool-gateway
for OntoWeb Subscribers only.
...
- Firecrawl: https://docs.firecrawl.dev/introduction (search, extract; direct or derived firecrawl-gateway.<domain> for OntoWeb Subscribers)
...
from tools.managed_tool_gateway import (  # noqa: F401 — backward-compat names for tests
    ...
    peek_ontoweb_access_token as _peek_ontoweb_access_token,
    read_ontoweb_access_token as _read_ontoweb_access_token,
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.intellect/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``intellect tools``).
    ...
    # tool gateway is configured for OntoWeb subscribers.
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `intellect tools` repaint.
        try:
...
def _web_requires_env() -> list[str]:
    ...
    set.  Gating them on ``managed_ontoweb_tools_enabled()`` only saved
    string noise in the metadata list, but cost a synchronous HTTP
    refresh against the OntoWeb portal on every CLI startup (invoked at
...
def _is_ontoweb_auxiliary_client(client: Any) -> bool:
    """Return True when the resolved auxiliary backend is ONTOWEB Portal."""
    from urllib.parse import urlparse

    base_url = str(getattr(client, "base_url", "") or "")
    host = (urlparse(base_url).hostname or "").lower()
    return host == "ontoweb.cn" or host.endswith(".ontoweb.cn")
...
def _resolve_web_extract_auxiliary(model: Optional[str] = None) -> tuple[Optional[Any], Optional[str], Dict[str, Any]]:
    ...
    if client is not None and _is_ontoweb_auxiliary_client(client):
        from agent.auxiliary_client import get_auxiliary_extra_body
        from agent.portal_tags import ontoweb_portal_tags
        extra_body = get_auxiliary_extra_body() or {"tags": ontoweb_portal_tags()}
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `intellect tools` to set one up."
                ),
            }
        else:
            logger.info(
