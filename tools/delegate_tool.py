...
# Must match intellect_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
# (intellect-* prefixed), and scenario toolsets.
#
...
_SUBAGENT_TOOLSETS = sorted(
    ...
    and not name.startswith("intellect-")
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``intellect-cli`` (which bundles
...
def _build_child_agent(
    ...
    model on OpenRouter while the parent runs on ONTOWEB Portal).
    """
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. intellect-cli) so that individual
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.intellect/logs/subagent-<sid>-<ts>.log``
...
def _parent_session_id_for_delegate(parent_agent) -> Optional[str]:
    """Resolve the parent's session id for post-delegation restore."""
    sid = getattr(parent_agent, "session_id", None) if parent_agent else None
    ...
    try:
        ...
        sid = str(get_session_env("intellect_SESSION_ID", "") or "").strip()
        return sid or None
    except Exception:
        return None


def _restore_parent_session_context(parent_session_id: Optional[str]) -> None:
    """Restore parent ``intellect_SESSION_ID`` after a subagent worker finishes."""
    ...
    try:
        ...
        os.environ["intellect_SESSION_ID"] = parent_session_id


def _publish_child_session_context(child) -> None:
    """Publish the child's session id in the current (worker) thread."""
    child_sid = str(getattr(child, "session_id", "") or "").strip()
    if not child_sid:
        return
    try:
        from gateway.session_context import set_current_session_id

        set_current_session_id(child_sid)
    except Exception:
        import os

        os.environ["intellect_SESSION_ID"] = child_sid
    ...
    **_kwargs,
    ...
    try:
        ...
        def _run_with_thread_capture():
            ...
            _publish_child_session_context(child)
        ...
        _restore_parent_session_context(parent_session_id)
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Available providers: openrouter, ontoweb, zai, kimi-coding, minimax."
        ) from exc
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'intellect auth'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (intellect_cli/config.py load_config()) so that
        ...
        "properties": {
            ...
            "acp_command": {
                ...
                "description": (
                    ...
                    "Leave empty to use the parent's default transport (Intellect subagents)."
                ),
            },
            "acp_args": {
                "type": "array",
