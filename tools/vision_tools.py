...
OntoWeb, Codex, native Anthropic, or a custom OpenAI-compatible endpoint.
...
def _resolve_download_timeout() -> float:
    env_val = os.getenv("intellect_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
            ...
            pass
    try:
        from intellect_cli.config import cfg_get, load_config
        cfg = load_config()
...
async def _download_image(image_url: str, destination: Path, max_retries: int = 3) -> Path:
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Connect-time peer validation (safe_httpx_async_client) plus redirect hooks.
            from tools.safe_http import safe_httpx_async_client

            async with safe_httpx_async_client(
...
def _supports_media_in_tool_results(provider: str, model: str) -> bool:
    ...
    Providers covered today (per spec docs verified Apr-2026):
      ...
      * Anthropic Messages API (``anthropic`` provider, plus aggregators that
        proxy Claude — ``openrouter``, ``ontoweb``, ``vertex``, ``bedrock``):
    ...
    # them.
    _AGGREGATORS = {
        "openrouter", "ontoweb", "vertex", "bedrock", "anthropic-vertex",
        "google-vertex",
    }
...
async def _vision_analyze_native(
    ...
    try:
        ...
        if local_path.is_file():
            ...
            temp_dir = get_intellect_dir("cache/vision", "temp_vision_images")
...
async def vision_analyze_tool(
    ...
    Note:
        - For URLs, temporary images are stored under $INTELLECT_HOME/cache/vision/ and cleaned up
...
def check_vision_requirements() -> bool:
    ...
    and if that fails, the auto chain (main provider → openrouter → ontoweb).
...
async def video_analyze_tool(
    ...
    try:
        ...
        if local_path.is_file():
            ...
            if blocked:
                raise PermissionError(blocked["message"])
            temp_dir = get_intellect_dir("cache/video", "temp_video_files")
