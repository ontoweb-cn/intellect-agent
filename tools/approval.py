...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("intellect_YOLO_MODE", ""))
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("INTELLECT_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return os.getenv("intellect_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set intellect_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind intellect_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds intellect_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("intellect_CRON_SESSION"):
        return False
...
# via shell expansions like $HOME or $INTELLECT_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_intellect_ENV_PATH = (
    r'(?:~\/\.intellect/|'
    r'(?:\$home|\$\{home\})/\.intellect/|'
    r'(?:\$intellect_home|\$\{intellect_home\})/)'
    r'\.env\b'
)
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("intellect_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set intellect_INTERACTIVE or intellect_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("intellect_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("intellect_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("intellect_EXEC_ASK")
