...
transport, discovers their tools, and registers them into the intellect-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.intellect/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.intellect/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            intellect_home = os.path.expanduser(
                os.getenv(
                    "INTELLECT_HOME", os.path.join(os.path.expanduser("~"), ".intellect")
                )
            )
            candidates = [
                os.path.join(intellect_home, "node", "bin", resolved_command),
                ...
                # image (which the Intellect Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Intellect MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Intellect gateways know how to render.
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "  pip install 'intellect-agent[mcp]'\n"
                "or (full install):\n"
    ...
    async def run(self, config: dict):
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Intellect is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `intellect mcp login {server_name}` (or delete the tokens "
            f"file under ~/.intellect/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Intellect config file.
    ...
    ``os.environ`` (which includes ``~/.intellect/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Intellect' MEDIA: tag convention and out to messaging
            ...
            # Intellect' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Intellect's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Intellect registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``intellect tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Intellect registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple intellect
