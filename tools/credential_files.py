...
def _resolve_intellect_home() -> Path:
...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.intellect",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``INTELLECT_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside INTELLECT_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    intellect_home = _resolve_intellect_home()

    # Reject absolute paths — they bypass the INTELLECT_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to INTELLECT_HOME)",
            relative_path,
        )
        return False

    host_path = intellect_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape INTELLECT_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, intellect_home)
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.intellect/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="intellect-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching intellect_constants.get_intellect_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_intellect_dir()`` for backward compatibility with old directory layouts.
    """
    from intellect_constants import get_intellect_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_intellect_dir(new_subpath, old_name)
        if host_dir.is_dir():
