...
`mcp_tool.py`, and `intellect_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# intellectMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_intellect_provider_class() -> Optional[type]:
    ...
    class intellectMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._intellect_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`intellectTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, intellectTokenStorage)
            ...
            if (
                ...
                try:
                    ...
                    logger.debug(
                        ...
                        "failed (non-fatal): %s",
                        self._intellect_server_name, exc,
                    )
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._intellect_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import intellectTokenStorage
                        if isinstance(storage, intellectTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._intellect_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import intellectTokenStorage
            if not isinstance(storage, intellectTokenStorage):
                return
    ...
    return intellectMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_intellect_PROVIDER_CLS: Optional[type] = _make_intellect_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`intellectMCPOAuthProvider` directly using the helpers
        ...
        storage = intellectTokenStorage(server_name)
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``intellect mcp remove <name>`` and (indirectly) by
        ``intellect mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
