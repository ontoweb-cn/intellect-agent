...
the actual `setup.sh` that creates the project workspace, configures Intellect
...
> [ONTOWEB/kanban-video-pipeline](https://github.com/ONTOWEB/kanban-video-pipeline).
...
6. **Fire the initial kanban task** — `intellect kanban create` assigned to the director
...
PyYAML must be installed in the user's Python (it ships with most Intellect
...
```bash
intellect kanban create "Direct production of <video title>" \
    --assignee director \
...
`~/.intellect/.env` and macOS Keychain (if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    if grep -q "^${var}=" ~/.intellect/.env 2>/dev/null && \
       [ -n "$(grep "^${var}=" ~/.intellect/.env | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ~/.intellect/.env or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY intellect ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY intellect OPENROUTER_API_KEY || exit 1
# ...
```
