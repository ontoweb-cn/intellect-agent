...
Maps each role archetype to the Intellect skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public intellect-agent
...
## Intellect skills relevant to video production

### Visual / rendering skills (`intellect-agent/skills/creative/`)
...
### Design / pre-production skills (`intellect-agent/skills/creative/`)
...
### Audio / media skills (`intellect-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`intellect-agent/skills/devops/`)
...
These are **not** Intellect skills but external CLIs / APIs that profiles invoke.
...
## Built-in Intellect tools for media review

These are native Intellect tools — not invoked via terminal but through their own
...
key is present in `~/.intellect/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Intellect profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.intellect/.env` → environment variable.
...
When a new Intellect-public video skill ships:
