...
intellect kanban watch --tenant <project-slug>

# Snapshot of the board
intellect kanban list --tenant <project-slug>
intellect kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
intellect kanban stats --tenant <project-slug>

# Visual dashboard (browser)
intellect dashboard

# Inspect a specific task (includes comments + events)
intellect kanban show <task-id>

# Follow a single task's event stream
intellect kanban tail <task-id>
```

Verify available subcommands with `intellect kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `intellect kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `intellect kanban block <id>` then `intellect kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `intellect kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `intellect kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `intellect profile list` (profile exists?), `intellect status` (gateway/dispatcher up?), and verify tenant. |
...
intellect kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
intellect kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(intellect kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
intellect kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
intellect kanban block <task-id>
intellect kanban archive <task-id>

# Diagnose what happened
intellect kanban show <task-id>      # task body, comments, recent events
intellect kanban tail <task-id>      # follow the live event stream
intellect kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `intellect kanban create` for the director
...
while true; do
    clear
    intellect kanban list --tenant <slug>
    echo "---"
    intellect kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `intellect kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `intellect profile list` and start
