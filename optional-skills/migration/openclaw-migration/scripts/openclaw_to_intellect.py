...
into Intellect Agent, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into intellect.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into intellect memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into intellect memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import intellect-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of intellect-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into Intellect command_allowlist.",
    },
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.intellect/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.intellect/tts/.",
    },
    "discord-settings": {
        "label": "Discord settings",
        "description": "Import Discord bot token and allowlist into intellect .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into intellect .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into intellect .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into intellect .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into intellect .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into Intellect config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into Intellect config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into intellect.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into intellect MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into Intellect config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'intellect cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into Intellect config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "session-config": {
        ...
        "description": "Import session reset policies (daily/idle) into Intellect session_reset config.",
    },
    "full-providers": {
        "label": "Full model provider definitions",
        "description": "Import custom model providers (baseUrl, apiType, headers) into intellect custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into Intellect config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into Intellect config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into Intellect config.yaml approvals section.",
    },
    "memory-backend": {
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    if yaml is None:
        raise RuntimeError("PyYAML is required to update Intellect config.yaml")
    ensure_parent(path)
    path.write_text(
...
# ── Brand rewriting ─────────────────────────────────────────
# Replace OpenClaw brand names with intellect in migrated text so that
# memory entries, user profiles, SOUL.md, and workspace instructions
# read as self-referential to the new agent identity.
#
# Case-preserving: ``OpenClaw`` → ``intellect`` (prose), but lowercase matches
# like ``openclaw`` → ``intellect`` (so filesystem paths like ``~/.openclaw``
# become ``~/.intellect`` — the real Intellect home — not the broken ``~/.intellect``).
_REBRAND_PATTERNS: List[Tuple[re.Pattern, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'intellect'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'intellect'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'intellect'),
]
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``intellect`` but maps ``openclaw`` → ``intellect`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.intellect/config.yaml`` (the real Intellect home) instead of the broken
...
def rebrand_text(text: str) -> str:
    """Replace OpenClaw / ClawdBot / MoltBot brand names with intellect.
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> intellect Migration Report",
        "",
...
class Migrator:
    ...
    # Option ids that mutate the Intellect config.yaml file.  Once any one of
    ...
    def _build_next_steps(self, summary: Dict[str, int]) -> List[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new Intellect session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "Intellect config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # that path will be stale after migration and would cause the intellect
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No intellect-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted intellect-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: intellect config set",
                        )
                    continue
        ...
        for oc_key, intellect_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and intellect_key not in secret_additions:
                secret_additions[intellect_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, intellect_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and intellect_key not in secret_additions:
                        secret_additions[intellect_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        intellect_config = load_yaml_file(destination)
        current_model = intellect_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = intellect_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                intellect_config["model"] = {"default": model_str}
            dump_yaml_file(destination, intellect_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(provider, str) and provider in {"elevenlabs", "openai", "edge", "microsoft"}:
            # OpenClaw renamed "edge" to "microsoft"; intellect still uses "edge"
        ...
        intellect_config = load_yaml_file(destination)
        existing_tts = intellect_config.get("tts", {})
        ...
        if self.execute:
            ...
            intellect_config["tts"] = merged_tts
            dump_yaml_file(destination, intellect_config)
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct intellect destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected intellect-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected intellect-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        intellect_cfg_path = self.target_root / "config.yaml"
        intellect_cfg = load_yaml_file(intellect_cfg_path)
        existing_mcp = intellect_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in Intellect config")
                continue

            intellect_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                intellect_srv["command"] = srv["command"]
                if srv.get("args"):
                    intellect_srv["args"] = srv["args"]
                if srv.get("env"):
                    intellect_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    intellect_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                intellect_srv["url"] = srv["url"]
                if srv.get("headers"):
                    intellect_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    intellect_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                intellect_srv["enabled"] = False
            if srv.get("timeout"):
                intellect_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                intellect_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                intellect_srv["tools"] = {}
                if tools_cfg.get("include"):
                    intellect_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    intellect_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
            sampling = srv.get("sampling")
            if sampling and isinstance(sampling, dict):
                intellect_srv["sampling"] = {
                    k: v for k, v in {
            ...
            existing_mcp[name] = intellect_srv
            added += 1
        ...
        if added > 0 and self.execute:
            self.maybe_backup(intellect_cfg_path)
            intellect_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(intellect_cfg_path, intellect_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'intellect cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = intellect_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> intellect reasoning_effort
        ...
        if compaction:
            compression = intellect_cfg.get("compression") or {}
            ...
            if compaction.get("model"):
                aux = intellect_cfg.setdefault("auxiliary", {})
                aux_comp = aux.setdefault("compression", {})
                aux_comp["model"] = compaction["model"]
            intellect_cfg["compression"] = compression
            changes = True

        # Map humanDelay
        human_delay = defaults.get("humanDelay") or {}
        if human_delay:
            hd = intellect_cfg.get("human_delay") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'intellect gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("intellect_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        intellect_cfg_path = self.target_root / "config.yaml"
        intellect_cfg = load_yaml_file(intellect_cfg_path)
        sr = intellect_cfg.get("session_reset") or {}
        changes = False
        ...
        if changes:
            intellect_cfg["session_reset"] = sr
            if self.execute:
                self.maybe_backup(intellect_cfg_path)
                dump_yaml_file(intellect_cfg_path, intellect_cfg)
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        intellect_cfg_path = self.target_root / "config.yaml"
        intellect_cfg = load_yaml_file(intellect_cfg_path)
        custom_providers = intellect_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(intellect_cfg_path)
            intellect_cfg["custom_providers"] = custom_providers
            dump_yaml_file(intellect_cfg_path, intellect_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to Intellect config
        discord_cfg = channels.get("discord") or {}
        if discord_cfg:
            intellect_cfg_path = self.target_root / "config.yaml"
            intellect_cfg = load_yaml_file(intellect_cfg_path)
            discord_intellect = intellect_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_intellect["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_intellect["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                intellect_cfg["discord"] = discord_intellect
                dump_yaml_file(intellect_cfg_path, intellect_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        intellect_cfg_path = self.target_root / "config.yaml"
        intellect_cfg = load_yaml_file(intellect_cfg_path)
        browser_intellect = intellect_cfg.get("browser") or {}
        changed = False

        # Map fields that have intellect equivalents
        if browser.get("cdpUrl"):
            browser_intellect["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_intellect["headless"] = browser["headless"]
            changed = True

        if changed:
            intellect_cfg["browser"] = browser_intellect
            if self.execute:
                self.maybe_backup(intellect_cfg_path)
                dump_yaml_file(intellect_cfg_path, intellect_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            intellect_mode = mode_map.get(mode, "manual")
            intellect_cfg.setdefault("approvals", {})["mode"] = intellect_mode
            if self.execute:
                self.maybe_backup(intellect_cfg_path)
                dump_yaml_file(intellect_cfg_path, intellect_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{intellect_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# OpenClaw -> intellect Migration Notes",
            "",
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in intellect. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing intellect Config Not Overwritten)",
                "",
                "These items already existed in your Intellect config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the intellect agent discovers these",
            "directories, it may read/write to them instead of the intellect state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `intellect claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `intellect claw cleanup` to fix it.",
            "",
            "## intellect-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `intellect claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `intellect setup` to configure any remaining settings",
            "- Run `intellect mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `intellect cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `intellect cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `intellect gateway install` if you need the gateway service",
            "- Review `~/.intellect/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into Intellect Agent.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=os.environ.get("INTELLECT_HOME") or str(Path.home() / ".intellect"), help="Intellect home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing Intellect targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of intellect-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    print()
    print(f"  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   OpenClaw -> intellect Migration   [{mode_label:>8s}]   ║")
    print(f"  ╠══════════════════════════════════════════════════════╣")
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.intellect/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.intellect/config.yaml")
        print("    2. Run: intellect mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: intellect cron")
