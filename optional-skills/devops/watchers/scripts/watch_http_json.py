...
Usage (via cron with --no-agent):
    ...
    intellect cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$INTELLECT_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Intellect-Watcher/1.0"})
