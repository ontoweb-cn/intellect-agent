...
Usage (via cron with --no-agent):
    ...
    intellect cron create intellect-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$INTELLECT_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name intellect-issues --repo ONTOWEB/intellect-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.intellect/.env to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "Intellect-Watcher/1.0",
    }
