"""Persistent slash-command worker — one IntellectCLI per TUI session.
...
def _run(cli: IntellectCLI, command: str) -> str:
...
def main():
    ...
    os.environ["INTELLECT_SESSION_KEY"] = args.session_key
    os.environ["intellect_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = IntellectCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
