...
# intellect modules.  intellect_cli sets INTELLECT_PYTHON_SRC_ROOT before spawning this
# subprocess; inserting it first ensures the installed packages win.
_src_root = os.environ.get("INTELLECT_PYTHON_SRC_ROOT", "")
...
from tui_gateway.server import _CRASH_LOG, dispatch, event_epoch, resolve_skin, write_json
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `intellect_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("intellect_TUI_SIDECAR_URL")
...
# tested; override via ``intellect_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("intellect_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``intellect_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``intellect --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
...
def main():
    ...
    # server stalled the whole shell on "summoning intellect…" for the full
    ...
    if not write_json({
        "jsonrpc": "2.0",
        "method": "event",
        "params": {"type": "gateway.ready", "payload": {"skin": resolve_skin(), "epoch": event_epoch()}},
    }):
