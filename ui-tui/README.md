...
React + Ink terminal UI for Intellect. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `INTELLECT_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.intellect/.intellect_history` or under `INTELLECT_HOME`.
...
```text
ui-tui/
  packages/intellect-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      intellect-ink.d.ts    type declarations for @intellect/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent IntellectCLI subprocess for slash commands
```
