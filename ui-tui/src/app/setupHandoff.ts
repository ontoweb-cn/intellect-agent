import type { RunExternalProcess } from '@intellect/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`intellect ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching intellect: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`intellect ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
