import type { InputEvent, Key } from '@intellect/ink'
import * as Ink from '@intellect/ink'
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.intellect_TUI_TERMUX_FAST_ECHO ?? '').trim().toLowerCase()
