...
// Mirrors @intellect/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @intellect/ink is bundled separately from packages/.
...
// Terminal. Explicit INTELLECT_TUI_THEME / COLORFGBG signals above still win,
...
// `INTELLECT_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `intellect_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `INTELLECT_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `INTELLECT_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default Intellect palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.intellect_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.INTELLECT_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.INTELLECT_TUI_BACKGROUND ?? '')
