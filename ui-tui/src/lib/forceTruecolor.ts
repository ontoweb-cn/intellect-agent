...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.intellect_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Intellect on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via intellect_TUI_TRUECOLOR=1.
