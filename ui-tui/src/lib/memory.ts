...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.intellect_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.intellect', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `intellect-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
