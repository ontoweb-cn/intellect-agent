...
//
// Both gate on intellect_DEV_PERF=1 and dump JSON-lines (default ~/.intellect/perf.log,
// override intellect_DEV_PERF_LOG). Tagged { src: 'react' | 'frame' } for jq.
// intellect_DEV_PERF_MS (default 2) skips sub-ms idle frames; set 0 to capture all.
//
...
import type { FrameEvent } from '@intellect/ink'
import { scrollFastPathStats } from '@intellect/ink'
import { Profiler, type ProfilerOnRenderCallback, type ReactNode } from 'react'

const ENABLED = /^(?:1|true|yes|on)$/i.test((process.env.intellect_DEV_PERF ?? '').trim())
const THRESHOLD_MS = Number(process.env.intellect_DEV_PERF_MS ?? '2') || 0
const LOG_PATH = process.env.intellect_DEV_PERF_LOG?.trim() || join(homedir(), '.intellect', 'perf.log')
