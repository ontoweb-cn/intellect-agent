...
await build({
  ...
  // Skip the prebuilt @intellect/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@intellect/ink': resolve(root, 'packages/intellect-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The intellect_cli launcher
