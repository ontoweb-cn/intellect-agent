# Intellect Agent Security Policy

This document describes Intellect Agent's trust model, names the one
...
- Environment details (`intellect version`, commit SHA, OS, Python
  version).
...
Intellect Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Intellect Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Intellect Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Intellect Agent's documentation
  or code about how a consuming layer (adapter, UI, file writer,
  shell) should treat agent output — e.g. "the TUI renders
...
Intellect Agent supports two OS-level isolation postures. They address
...
Intellect Agent supports this in two ways:

- **Intellect Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Intellect Agent's in-process heuristics
...
Intellect Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Intellect Agent
itself. Bugs in Intellect Agent's plugin-install or plugin-discovery
...
- **Network-exposed HTTP surfaces.** The API server adapter,
  and any plugin that binds a listening socket.
...
4. **Within the authorized set, all callers are equally trusted.**
   Intellect Agent does not model per-caller capabilities inside a
...
5. **Binding a local-only surface to a non-loopback interface is a
   break-glass operator decision (§3.2).** Plugin HTTP servers default
   to loopback; exposing them via
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Intellect Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Intellect Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  flags on the TUI or other components, disabled approvals,
  local backend in production, development profiles that bypass
  intellect-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Intellect Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Intellect Agent. Bugs in Intellect
...
- Intellect Agent includes supply-chain guards for MCP server
...
### 6.2 Layer 1 — Rust Regex (always on)

**File:** `rust-core/src/sandbox.rs` — 69 patterns in two tiers.

**Hardline (14):** `rm -rf /`, `shutdown`, `reboot`, `mkfs`, `dd of=/dev/sd*`,
fork bombs, `kill -9 -1`. Cannot be bypassed by any approval mode.

**Dangerous (55):** Recursive delete, chmod 777, SQL DROP/DELETE/TRUNCATE,
curl|sh, file overwrite, git force push, docker lifecycle, sudo -S,
SSH authorized_keys overwrite, script execution (31 tokens).

**Token Registry** (`SCRIPT_EXEC_TOKEN_ENTRIES`): 31 Python/Perl/Ruby/Node
APIs in 10 categories with individual descriptions. Adding a token:
append one line to `rust-core/src/sandbox.rs`.

### 6.3 Layer 2 — AST Analysis

**File:** `tools/approval.py` → `_check_python_ast()`

Structural analysis of `execute_code` Python payloads: direct calls
(exec/eval/compile), attribute calls (os.system/subprocess.call),
getattr obfuscation, `__dict__[]` access, importlib chains,
dangerous imports. AST detection triggers **auto-deny**.

### 6.4 Layer 3 — Path & Network Guards

| Module | Protection |
|--------|-----------|
| `path_security.py` | System files, SSH keys, credentials |
| `url_safety.py` | SSRF: private IPs, loopback, cloud metadata |
| `file_tools.py` | Write guard: `/etc/`, `/boot/`, docker socket |

### 6.5 Coverage & Performance

- 30 attack vectors tested, 0 bypasses in penetration testing
- RegexSet: ~10 µs, AST: ~27 µs, full pipeline: ~8 µs
