...
> **日期**：2026-07-12  
> **状态**：**REVISED → 执行中**（已吸收技术评审 Request changes）  
...
- [ ] 0.1 本文件状态 **REVISED/执行中**；修订历史追加评审行  
- [ ] 0.2 **校验** W10 计划已 WONTFIX（幂等）  
- [ ] 0.3 **校验** parity 索引指向 W11（幂等）  
- [ ] 0.4 `AGENTS.md`：永久单用户；W10.1 WONTFIX  
- [ ] 0.5 `website/docs/user-guide/profiles.md`（+ zh-Hans）：隔离 = profile；`members.enabled` 无效  
- [ ] 0.6 `CHANGELOG.md` Unreleased  
- [ ] 0.7 Docs PR merge  
...
- [ ] 1.1 删除 CommandDef：`team`, `teams`, `project`, `projects`, `join`, `join-project`, `login`, `logout`  
- [ ] 1.2 **保留** `gateway/run.py` 内联 deprecation「removed in v0.5.0」  
- [ ] 1.3 测试：`resolve_command` 上述名为 None；∉ `GATEWAY_KNOWN_COMMANDS` / `gateway_help_lines()`；yaml `members.enabled: true` 仍不出现  
...
- [ ] 2.1 卸 `routes.py` list/get/create/post 挂载（今日 401 → 卸后 **404**）  
- [ ] 2.2 删 `wiki_contributions_handlers.py`  
- [ ] 2.3 清 `wiki-panel.js` pending badge；`_build_wiki_global_catalog_entry` pending count  
- [ ] 2.4 更新 `website/docs/.../llm-wiki.md` + `skills/research/llm-wiki/SKILL.md`（及 bundled 镜像）：去掉 contribution review API  
- [ ] 2.5 `wiki_merge.py`：**延期**（S12），计划注明  
- [ ] 2.6 测试：`GET/POST /api/wiki/contributions` → 未处理/404  

### Task 3 — Auth carve-out（钉死 status）

**Files:** `webui/api/auth.py`

- [ ] 3.1 从 `PUBLIC_PATHS` 移除：`/api/members/oauth/providers`、`register*`、`/api/members/login`（**保留** `/api/members/status`）  
- [ ] 3.2 删 `check_auth` 分支：`/api/members/oauth/`、`/api/members/redeem`、`register`/`login` 前缀  
- [ ] 3.3 **不**改 CSRF exempt 列表（S13）  
- [ ] 3.4 测试：auth 开启时未登录 → oauth/providers + login + register → **401/302**；status → **200** `enabled:false`  
- [ ] 3.5 （可选）`_members_flag_from_config` raw yaml 残差 → 已知 S12 或随手收紧  

### Task 4 — FE（loaded 表面）

**Files:** `login.js`；`register.js`；`routes.py` login/register HTML；`i18n.js`；**勿伤** `member-auth.js`

- [ ] 4.1 隐藏/折叠 multi-user login（OAuth/password/redeem tabs）与 register 流程  
- [ ] 4.2 i18n `members_*` → removed / 指向 `intellect oauth` 或 provider OAuth UI  
- [ ] 4.3 确认 `member-auth.js` `fetchMembersStatus` 仍工作  
- [ ] 4.4 （可选）删未加载的 `members.js` / `teams.js` / `projects.js`  
- [ ] 4.5 **勿动** `oauth-providers.js`（S4）  

### Task 5 — 回归

- [ ] 5.1 `pytest`：membership stub / members thin / members security + 本拍新测  
- [ ] 5.2 Sweep PR merge  
- [ ] 5.3 可选 M1–M4  
