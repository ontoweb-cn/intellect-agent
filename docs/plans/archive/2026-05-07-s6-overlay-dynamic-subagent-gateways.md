...
> [ONTOWEB/intellect-agent#30136](https://gitee.com/ontoweb/intellect-agent/pull/30136)
...
> (`git log --oneline a957ef083..a6f7171a5 -- 'docker/*' 'intellect_cli/service_manager.py' …`).
> Open Questions are collapsed into a single Decision Log table; full
> deliberations live in PR review comments.

**Goal:** Replace `tini` with s6-overlay as PID 1 in the Intellect Docker image so
that the main intellect process, the dashboard, and dynamically-created
...
s6-svscan as PID 1. Main intellect and the dashboard are declared as static
s6-rc services at image build time. Per-profile gateways — which users create
*after* the image is built (`intellect profile create coder` →
...
- Existing systemd/launchd/windows surface in `intellect_cli/gateway.py` and
  `intellect_cli/gateway_windows.py`.
...
- Interactive TUI must keep working:
  `docker run -it --rm ontoweb/intellect-agent:latest --tui`.
- Dynamic registration is limited to per-profile gateways — one service per
  ...
  `gateway-default` slot is always registered for the root INTELLECT_HOME
  profile so `intellect gateway start` (no `-p`) has somewhere to land.
...
1. `gosu` privilege drop from root → `intellect` UID.
2. Copied `.env.example`, `cli-config.yaml.example`, `SOUL.md` into
   `$INTELLECT_HOME` if missing.
3. Synced bundled skills via `tools/skills_sync.py`.
4. Optionally backgrounded `intellect dashboard` in a subshell when
   `intellect_DASHBOARD=1` — **not supervised**, no restart.
5. `exec intellect "$@"` — tini's sole direct child.
...
All init-system logic lives in **`intellect_cli/gateway.py`** (~5,400 LOC at
re-validation). The systemd/launchd code is ~1,500 lines of that, plus a
separate **`intellect_cli/gateway_windows.py`** (~690 LOC) for Windows
Scheduled Tasks.
...
- `intellect_cli/status.py` — gained an `s6` branch for in-container runs.
- `intellect_cli/profiles.py` — `create_profile` / `delete_profile` register and
  unregister with s6 inside the container (no-op on host).
- `intellect_cli/doctor.py` — `_check_gateway_service_linger` skips on s6, and a
  new "Service Supervisor" section reports main-intellect / dashboard /
  profile-gateway counts via the ServiceManager.
- `intellect_cli/gateway.py::gateway_command` — the
...
`intellect gateway start`, `coder gateway start` (profile alias), and
`intellect -p <profile> gateway start` all spawn a gateway process scoped to a
given profile. See
[Profiles: Running Gateways](https://intellect-agent.ontoweb.cn/docs/user-guide/profiles#running-gateways).
On host, lifecycle is managed via per-profile systemd units
(`intellect-gateway-<profile>.service`); inside the container, an s6 service at
...
`intellect_cli/container_boot.py`, and auto-starts those whose last recorded
...
- **Root/non-root model:** `/init` runs as root to set up the supervision
  ...
  `s6-setuidgid intellect` in its `run` script. The per-service `s6-supervise`
  monitor stays root so it can signal its child regardless of UID. Net
  effect: intellect and all its subprocesses run as UID 10000 exactly as
...
- `/command/with-contenv` sources `/run/s6/container_environment/*` into
  service env — convenient for passing `INTELLECT_HOME` etc.
...
- Zombie reaping: PID 1 (s6-svscan) reaps all zombies non-blockingly on
  SIGCHLD. Any subagent subprocess spawned by the main intellect process is
...
Container ENTRYPOINT is `/init`, PID 1 is s6-svscan. The main intellect
...
### D2. Main intellect is an s6 service with container-exit semantics

The contract "container exits when `intellect` exits" is preserved via a
...
|---|---|
| (no args) | `intellect` with no args, container exits when intellect exits |
| `chat -q "..."` | `intellect chat -q "..."`, container exits with intellect exit code |
...
- **s6-rc** (declarative, compile-then-swap): used for main intellect and the
...
(tmpfs, intellect-writable). s6-svscan picks them up on rescan.
...
(`intellect -p <profile> gateway run`, or `intellect gateway run` for the default
...
and silently failed for the unprivileged intellect user (UID 10000), making
...
Existing per-profile `intellect -p <profile> gateway start/stop/restart` CLI
...
`intellect gateway start` (no `-p`) targets a special `gateway-default` slot
that's always registered by the cont-init reconciler. Its run script omits
the `-p` flag and runs against the root `$INTELLECT_HOME` profile.

`--all` lifecycle (`intellect gateway stop --all`, `... restart --all`)
...
main-intellect service so the intellect CMD becomes that main program.
...
_is_tui_invocation() {
    ...
    case "${intellect_TUI:-}" in 1|true|TRUE|yes) return 0 ;; esac
    if [ -t 0 ] && [ $# -eq 0 ]; then return 0; fi
    return 1
}
```

And in `docker/s6-rc.d/main-intellect/run`:

```sh
if [ -f /var/run/s6/container_environment/intellect_TUI_MODE ]; then
    exec sleep infinity   # s6-overlay will exec CMD as the TTY-connected main
fi
exec s6-setuidgid intellect intellect ${intellect_ARGS:-}
```

In TUI mode main intellect is effectively unsupervised (same as the pre-s6
...
| Profile gateway crash loop masks a real config error | Low | Medium | s6 `finish` script `max_restarts` cap (planned follow-up); operators see crash-looping logs in `$INTELLECT_HOME/logs/gateways/<profile>/` |
...
| `docker restart` silently loses per-profile gateway registrations (tmpfs scandir wiped) | High (without mitigation) | High | Cont-init reconciliation re-registers from persistent `$INTELLECT_HOME/profiles/` and auto-starts those last seen `running`; outcome recorded to `$INTELLECT_HOME/logs/container-boot.log` (size-bounded, rotates to `.1` at 256 KiB) |
| A `running` gateway that's actually broken auto-restarts into a crash loop after every container restart | Low | Medium | s6 `finish` script `max_restarts` cap (planned); follow-up: `intellect doctor` alerts when N consecutive container restarts ended in `startup_failed` |
| `_s6_running()` detection works as root but silently fails for unprivileged intellect user, making runtime-registration path inert | High (without mitigation) | High | **Caught in PR review.** Detection now probes `/proc/1/comm` (world-readable) + `/run/s6/basedir`. Docker integration tests refactored to `docker exec -u intellect` so the realistic runtime user is exercised |
| `s6-svscanctl` from intellect hits EACCES on the root-owned control FIFO | Medium | Medium | `02-reconcile-profiles` chowns `/run/service/.s6-svscan/{control,lock}` to intellect after stage1 creates them |
| Per-service `supervise/control` FIFO is root-owned by s6-supervise, blocking `s6-svc` from intellect | Known | Medium | Surfaced cleanly as `S6CommandError` (with rc + stderr) instead of raw `CalledProcessError`. Permission fix tracked as a follow-up (small SUID helper, polling chown loop in cont-init.d, or replace `s6-svc` with `down`-marker manipulation) |
...
| # | Question | Decision |
|---|---|---|
| OQ1 | Gate Phase 2 behind env var? | Ship directly (Intellect is pre-1.0; users can pin the previous image) |
| OQ2 | s6 root model | Root `/init`, drop per-service via `s6-setuidgid intellect` |
| OQ3 | Dashboard opt-in mechanism | Always declared as an s6 service; `03-dashboard-toggle` cont-init script writes a `down` marker when `intellect_DASHBOARD` is unset so `s6-svstat` reports the slot's real state |
| OQ4 | Podman rootless | Supported, fix reactively |
| OQ5 | Service naming | `gateway-<profile>` (matches pre-existing `intellect-gateway-<profile>.service` systemd convention) |
...
| OQ8 | Log persistence | `$INTELLECT_HOME/logs/gateways/<profile>/`. The log path is sourced from runtime `$INTELLECT_HOME` via `with-contenv`, NOT Python-substituted at registration time |
...
- **`gateway-default` slot:** always registered by the reconciler so
  `intellect gateway start` (no `-p`) has somewhere to land.
...
- [x] `docker run -it --rm intellect-agent --tui` starts the Ink TUI with
      working keyboard input, cursor control, and resize (SIGWINCH)
- [x] Dashboard crashes are recovered by s6 within ~2s
- [x] `intellect profile create test` inside a container creates
      `/run/service/gateway-test/`
- [x] `intellect -p test gateway start` inside a container dispatches through s6
- [x] `intellect -p test gateway stop` inside a container cleanly stops via s6
- [x] `intellect profile delete test` inside a container removes
      `/run/service/gateway-test/`
- [x] Profile gateway logs persist at
      `$INTELLECT_HOME/logs/gateways/test/current`
- [x] `intellect status` inside the container shows `Manager: s6`
- [x] `intellect gateway start` (no `-p`) inside a container targets
      `gateway-default` and runs against the root profile
- [x] `intellect gateway stop --all` / `... restart --all` iterate every
...
- [x] `intellect gateway install/start/stop` on Linux host and macOS host
