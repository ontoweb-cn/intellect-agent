# Plans Index & Status

> Updated 2026-06-10 | 49 documents (28 archived to `archive/`)

## Status Key
- ✅ Complete (archived) | 📋 Planned | 📦 Archive

## Active Plans (📋)

| Status | Date | Document | Topic |
|--------|------|----------|-------|
| 📋 | 2026-06-10 | `rag-memory-collaboration-architecture.md` | RAG+Memory 协同架构 |
| 📋 | 2026-06-02 | `2026-06-02-model-registry-agent-implementation.md` | Agent-side registry design |
| 📋 | 2026-06-02 | `2026-06-02-provider-registry-db-unification-design.md` | Provider registry design |
| 📋 | 2026-06-01 | `2026-06-01-release-summary.md` | v0.4.2 release summary |
| 📋 | — | `graphiti-memory-plugin-dev-plan.md` | Graphiti memory plugin |
| 📋 | — | `graphiti-memory-plugin-gap-analysis.md` | Graphiti gap analysis |
| 📋 | — | `lightrag-memory-plugin-design.md` | LightRAG memory design |
| 📋 | — | `ontoweb-provider-architecture.md` | ONTOWEB provider architecture |
| 📋 | — | `port-hermes-2026-05-31-updates.md` | Hermes→Intellect port notes |
| 📋 | — | `2026-06-profile-management-disabled-restore.md` | Profile restore guide |
| 📋 | — | `oauth-device-code-msal-p3.md` | Device code + MSAL (P3) |
| 📋 | — | `2026-06-02-model-registry-migration-runbook.md` | Model migration runbook |

## Archived (📦)

28 completed plans moved to `archive/`. See individual files for implementation history.

| Year | Count | Topics |
|------|-------|--------|
| 2026-05 | 10 | OAuth design, multi-entity spec, Telegram topics |
| 2026-04 | 18 | Architecture, RBAC, storage, teams, webui |
