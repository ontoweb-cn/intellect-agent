...
When running Intellect inside Docker, the default `network_mode: host` gives the
...
The Intellect [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ intellect-agent │   │ intellect-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=intellect,intellect-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://intellect-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Intellect trust model and vulnerability reporting
