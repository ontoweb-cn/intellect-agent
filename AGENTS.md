# Intellect Agent - Development Guide

Instructions for AI coding assistants and developers working on the intellect-agent codebase.
...
├── intellect_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── intellect-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `intellect --tui`
...
**User config:** `~/.intellect/config.yaml` (settings), `~/.intellect/.env` (API keys only).
**Logs:** `~/.intellect/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
...
- **Skin engine** (`intellect_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
...
- **Slack** — `slack_subcommand_map()` generates `/intellect` subcommand routing
...
```
2. Add handler in `IntellectCLI.process_command()` in `cli.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `intellect --tui` or `intellect_TUI=1`.
...
npm run dev       # watch mode (rebuilds intellect-ink + tsx --watch)
npm start         # production
npm run build     # full build (intellect-ink + tsc)
...
For most custom or local-only tools, do **not** edit Intellect core. Use the plugin
route instead: create `~/.intellect/plugins/<name>/plugin.yaml` and
`~/.intellect/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Intellect tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_INTELLECT_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_INTELLECT_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_intellect_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `INTELLECT_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_intellect_home()` for the base directory — never `Path.home() / ".intellect"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `intellect_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `intellect_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `intellect tools`, `intellect setup`, most CLI subcommands | `intellect_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`intellect_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
intellect_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.intellect/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic Intellect gold/kawaii (the current look)
...
Add to `_BUILTIN_SKINS` dict in `intellect_cli/skin_engine.py`:
...
Users create `~/.intellect/skins/<name>.yaml`:
...
Intellect has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.intellect/plugins/` and pip-installed entry points.

### General plugins (`intellect_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.intellect/plugins/`, `./.intellect/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `intellect` at startup so
  `intellect <pluginname> <subcmd>` works with no change to `main.py`
...
`post_setup(intellect_home, config)` for setup-wizard integration.

**CLI commands via `plugins/memory/<name>/cli.py`:** if a memory plugin
defines `register_cli(subparser)`, `discover_plugin_cli_commands()` finds
it at argparse setup time and wires it into `intellect <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `intellect --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `intellect_cli/main.py`, etc.).
...
into `~/.intellect/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `intellect memory setup` / `post_setup()` without
...
2. User: `$INTELLECT_HOME/plugins/model-providers/<name>/`
...
(`strike-freedom-cockpit`, `plugin-llm-example`,
`plugin-llm-async-example`) live in the
[`intellect-example-plugins`](https://github.com/ONTOWEB/intellect-example-plugins)
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `intellect skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.intellect.tags`, `metadata.intellect.category`,
`metadata.intellect.related_skills`, `metadata.intellect.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.intellect.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Intellect tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Intellect Agent" is the secondary collaborator. If the
   contributor's commit shows "Intellect Agent" as author (because they
   used Intellect to draft the skill), replace it with their actual name
...
lives in the `intellect-agent-dev` skill at
...
`"messaging"`); `_INTELLECT_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `intellect tools` (the curses UI) or the
...
go to `~/.intellect/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `intellect_cli/curator.py` wires `intellect curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.intellect/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `intellect cron <verb>`
...
- File lock at `~/.intellect/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
- **CLI:** `intellect_cli/kanban.py` wires `intellect kanban` with verbs
...
- **Board** is the hard boundary — workers are spawned with
  `INTELLECT_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
Intellect-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `intellect_BACKGROUND_NOTIFICATIONS` env var):
...
3. **Module-level constants are fine** — they cache `get_intellect_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_intellect_home()`,
   not `Path.home() / ".intellect"`.

4. **Tests that mock `Path.home()` must also set `INTELLECT_HOME`** — since code now uses
   `get_intellect_home()` (reads env var), not `Path.home() / ".intellect"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"INTELLECT_HOME": str(tmp_path / ".intellect")}):
       ...
   ```
...
Use `get_intellect_home()` from `intellect_constants` for code paths. Use `display_intellect_home()`
for user-facing print/log messages. Hardcoding `~/.intellect` breaks profiles — each profile
has its own `INTELLECT_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### DO NOT introduce new `simple_term_menu` usage
Existing call sites in `intellect_cli/main.py` remain for legacy fallback only;
the preferred UI is curses (stdlib) because `simple_term_menu` has
ghost-duplication rendering bugs in tmux/iTerm2 with arrow keys. New
interactive menus must use `intellect_cli/curses_ui.py` — see
`intellect_cli/tools_config.py` for the canonical pattern.
...
with actual imports (not mocks) against a temp `INTELLECT_HOME`.

### Tests must not write to `~/.intellect/`
The `_isolate_intellect_home` autouse fixture in `tests/conftest.py` redirects `INTELLECT_HOME` to a temp dir. Never hardcode `~/.intellect/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_intellect_home()` resolve within the temp dir.
Use the pattern from `tests/intellect_cli/test_profiles.py`:
```python
@pytest.fixture
...
- The plugin disables itself in child processes (sentinel envvar
  `intellect_ISOLATE_CHILD=1`), so there's no fork-bomb risk.
...
| HOME / `~/.intellect/` | Your real config+auth.json | Temp dir per test |
