"""ACP auth helpers — detect and advertise Intellect authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "intellect-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Intellect runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Intellect can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Intellect.
    ...
    not have Intellect provider credentials configured yet, so Intellect always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Intellect using the currently configured "
