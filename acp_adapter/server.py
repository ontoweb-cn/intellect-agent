"""ACP agent server — exposes Intellect Agent via the Agent Client Protocol."""
...
try:
    from intellect_cli import __version__ as intellect_VERSION
except Exception:
    intellect_VERSION = "0.0.0"
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Intellect running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Intellect cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into an Intellect/OpenAI-compatible user content payload."""
...
class intellectACPAgent(acp.Agent):
    ...
    _SLASH_COMMANDS = {
        ...
        "version": "Show Intellect version",
    }
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Intellect version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Intellect maps edit approval
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from intellect_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Intellect estimates ``used`` from the same
    ...
    async def _send_session_info_update(self, session_id: str) -> None:
        """Send ACP native session metadata after Intellect changes it."""
        if not self._conn:
            return
        try:
        ...
        # intellect_state.py schema — only started_at/ended_at). Use "now" as
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["intellect-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="intellect-agent", version=intellect_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Intellect' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Intellect setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Intellect
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Intellect on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Intellect' local kawaii waiting/status
        ...
        # not the executor's. Also set intellect_INTERACTIVE so approval.py
        # takes the CLI-interactive path (which calls the registered
        # callback via prompt_dangerous_approval) instead of the
        # non-interactive auto-approve branch (GHSA-96vc-wcxf-jjff).
        # ACP's conn.request_permission maps cleanly to the interactive
        # callback shape — not the gateway-queue intellect_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind intellect_SESSION_KEY for this session so per-session caches
            ...
            previous_interactive = os.environ.get("intellect_INTERACTIVE")
            ...
            previous_session_id = os.environ.get("intellect_SESSION_ID")
            ...
            try:
                ...
                # Restore intellect_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("intellect_INTERACTIVE", None)
                else:
                    os.environ["intellect_INTERACTIVE"] = previous_interactive
                # Restore intellect_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("intellect_SESSION_ID", None)
                else:
                    os.environ["intellect_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            ...
            # stomp on each other's ContextVar writes (intellect_SESSION_KEY in
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Intellect has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
