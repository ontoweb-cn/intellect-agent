"""CLI entry point for the intellect-agent ACP adapter.

Loads environment variables from ``~/.intellect/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from INTELLECT_HOME (default ``~/.intellect``)."""
    from intellect_cli.env_loader import load_intellect_dotenv

    intellect_home = get_intellect_home()
    loaded = load_intellect_dotenv(intellect_home=intellect_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", intellect_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="intellect-acp",
        description="Run Intellect Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Intellect version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _print_version() -> None:
    ...
    print(intellect_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import intellectACPAgent  # noqa: F401

    print("Intellect ACP check OK")


def _run_setup() -> None:
    from intellect_cli.main import main as intellect_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "intellect", "model"]
        intellect_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``intellect postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting intellect-agent ACP adapter")
    ...
    import acp
    from .server import intellectACPAgent
