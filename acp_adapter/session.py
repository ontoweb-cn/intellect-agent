"""ACP session manager — maps ACP sessions to Intellect AIAgent instances.

Sessions are persisted to the shared SessionDB (``storage.backend`` store) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Intellect itself is running in WSL.

    Windows ACP clients can launch ``intellect acp`` inside WSL while still sending
    ...
    """
    from intellect_constants import is_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Intellect from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["intellect-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Intellect agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Intellect AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Intellect runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, SessionDB is
                           lazily opened via the storage factory (SQLite or PostgreSQL).
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``INTELLECT_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_intellect_home``).
        """
