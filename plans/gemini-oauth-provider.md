...
## Client ID
- Need to register a "Desktop app" OAuth client on a ONTOWEB GCP project
...
## Token Lifecycle
- Store at `~/.intellect/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `intellect_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `intellect_cli/models.py` — Add Gemini model catalog
4. `intellect_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `intellect_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `intellect_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
## Prerequisites
- ONTOWEB GCP project with Desktop OAuth client registered
- OR: accept user-provided client_id via intellect_GEMINI_CLIENT_ID env var
...
- intellect-agent Copilot OAuth: `intellect_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
