"""
LINE Messaging API platform adapter for Intellect Agent.
...
to Intellect Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``INTELLECT_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            intellect_home = Path.home().joinpath(".intellect").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``intellect status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``intellect status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``intellect setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.intellect/.env`` via ``intellect_cli.config``.
    """
    print()
    ...
    try:
        from intellect_cli.config import get_env_var, set_env_var
    except ImportError:
        print("intellect_cli.config not available; set LINE_* vars manually in ~/.intellect/.env")
        return
