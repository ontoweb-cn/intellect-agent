"""
Microsoft Teams platform adapter for Intellect Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Intellect"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        intellect_action = data.get("intellect_action", "")
        session_key = data.get("session_key", "")

        if not intellect_action or not session_key:
        ...
        choice = choice_map.get(intellect_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
    ...
    async def send_exec_approval(
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="intellect_approve",
                    data={**btn_data_base, "intellect_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="intellect_approve",
                    data={**btn_data_base, "intellect_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="intellect_approve",
                    data={**btn_data_base, "intellect_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="intellect_approve",
                    data={**btn_data_base, "intellect_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Intellect\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.intellect/.env")
