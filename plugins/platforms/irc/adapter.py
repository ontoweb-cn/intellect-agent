"""
IRC Platform Adapter for Intellect Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Intellect agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "intellect-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Intellect Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Intellect Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: intellect_, intellect_1, intellect_2...
...
def check_requirements() -> bool:
    ...
    # intellect setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `intellect gateway setup` flow for the IRC platform.

    Lazy-imports ``intellect_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Intellect to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    nickname = prompt(
        "Bot nickname (e.g. intellect-bot)",
    ...
    channel = prompt(
        "Channel to join (e.g. #intellect — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.intellect/.env")
    print_info("Restart the gateway for changes to take effect: intellect gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "intellect-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "intellect-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Intellect Agent (cron)")
