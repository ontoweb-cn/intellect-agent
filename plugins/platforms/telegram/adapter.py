...
def check_telegram_requirements() -> bool:
    """Check if Telegram dependencies are available. Delegates to shared helper."""
    global TELEGRAM_AVAILABLE
    if TELEGRAM_AVAILABLE:
        return True
    from gateway.platforms.helpers import check_platform_requirements
    return check_platform_requirements("platform.telegram", _reimport_telegram)
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = float(os.getenv("intellect_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", "0.8"))
        ...
        self._text_batch_delay_seconds = self._env_float_clamped(
            "intellect_TELEGRAM_TEXT_BATCH_DELAY_SECONDS",
            0.3,
            min_value=0.08,
            max_value=2.0,
        )
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "intellect_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS",
            1.0,
    ...
    def _thread_kwargs_for_send(
        ...
        metadata. Intellect-created private-chat topic lanes are marked with
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `intellect update` or `--replace` handoffs)
        ...
        message = (
            ...
            "To recover: ensure no other Intellect or OpenClaw instance is running "
            "with this token, then restart the gateway with 'intellect gateway restart'."
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # readable by the Intellect process (shared mount, same machine, etc.).
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("intellect_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("intellect_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("intellect_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("intellect_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("intellect_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            }

            disable_fallback = (os.getenv("intellect_TELEGRAM_DISABLE_FALLBACK_IPS", "").strip().lower() in {"1", "true", "yes", "on"})
            ...
            try:
                ...
                from intellect_cli.commands import telegram_menu_commands
    ...
    def _build_provider_keyboard(self, providers: list):
        ...
        CLI ``intellect model`` picker via the shared ``group_providers`` fold,
        so all surfaces stay consistent.
        """
        try:
            from intellect_cli.models import group_providers
        except Exception:
    ...
    async def _handle_model_picker_callback(
        ...
        if data.startswith("mp:"):
            ...
            group_id = data[4:]
            try:
                from intellect_cli.models import PROVIDER_GROUPS
    ...
    # Scripts live in ~/.intellect/scripts/gmail-triage/. `arg` from the callback
    ...
    async def _handle_gmail_triage_callback(
        ...
        *,
        ...
        script_path = _Path.home() / ".intellect" / "scripts" / "gmail-triage" / script_name
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # raw substring matches like "foo@intellect_bot.example" are not mentions
    ...
    def _explicit_bot_mentions_exclude_self(self, message: Message) -> bool:
        ...
        Telegram groups can contain several Intellect bot profiles. A message like
