...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-Intellect-Session-Id header; opt-in long-term memory scoping via X-Intellect-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-Intellect-Session-Key supported)
...
from agent.cache.response_store import SqliteResponseStore as ResponseStore
from agent.cache.response_store import create_response_store
...
from agent.cache.idempotency import create_idempotency_cache


def _load_api_server_config() -> dict:
    try:
...
_idem_cache = create_idempotency_cache(_load_api_server_config())
...
def _derive_chat_session_id(
    ...
    the same Intellect session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through intellect-agent's AIAgent.
    """
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._response_store = create_response_store(
            _load_api_server_config(),
            max_size=MAX_STORED_RESPONSES,
        )
        ...
        from agent.cache.run_status import create_run_status_store

        self._run_status_store = create_run_status_store(_load_api_server_config())
    ...
    def _parse_cors_origins(value: Any) -> tuple[str, ...]:
        ...
        Warns when ``*`` is configured — wildcard CORS allows any website
        to make credentialed requests to the API server on localhost.
        """
        if not value:
            return ()
        ...
        if "*" in result:
            logger.warning(
                "CORS origin '*' configured — any website can access the API. "
                "Restrict to specific origins for production use."
            )
        return result
    ...
    def _check_auth(self, request: "web.Request") -> Optional["web.Response"]:
        ...
        Accepts both the profile-level API_SERVER_KEY and member tokens
        (``imt_*``) when multi-user features are enabled.  Returns None
        if auth is OK, or a 401 web.Response on failure.
        """
        ...
        if auth_header.startswith("Bearer "):
            ...
            # 1. Profile-level API key (legacy)
            if hmac.compare_digest(token, self._api_key):
                request["auth_mode"] = "profile"
                return None
            # 2. Member token (imt_*) — multi-user
            if token.startswith("imt_"):
                member_info = self._verify_member_token(token)
                if member_info:
                    request["member_info"] = member_info
                    request["auth_mode"] = "member"
                    return None
            # 3. Project token (imt_p_*) — project-scoped
            if token.startswith("imt_p_"):
                proj_info = self._verify_project_token(token)
                if proj_info:
                    request["project_info"] = proj_info
                    request["auth_mode"] = "project"
                    return None
    ...
    def _verify_member_token(self, token: str) -> dict | None:
        """Verify a member-scoped bearer token (imt_*).  Returns member row or None."""
        db = None
        try:
            from agent.membership import MembershipDB
            from gateway.run import _load_gateway_config

            db = MembershipDB(config=_load_gateway_config())
            row = db.verify_token(token)
            if row and row.get("member_id") and row["member_id"] != "__project__":
                member = db.get_member(row["member_id"])
                if not member or not member.get("enabled", True):
                    return None
                return {
                    "member_id": row["member_id"],
                    "role": member.get("role"),
                    "token_id": row.get("id"),
                    "scopes": row.get("permissions"),
                }
        except Exception:
            pass
        finally:
    ...
    def _check_member_chat_rbac(self, request: "web.Request") -> Optional["web.Response"]:
        """Enforce Action.CHAT when members.enabled and auth is a member token."""
        try:
            from gateway.run import _load_gateway_config
            from agent.member_rbac import check_member_chat_permission

            cfg = _load_gateway_config()
            err = check_member_chat_permission(
                config=cfg,
                member_id=(request.get("member_info") or {}).get("member_id"),
                role=(request.get("member_info") or {}).get("role"),
                profile_key_auth=request.get("auth_mode") == "profile",
                project_token_auth=request.get("auth_mode") == "project",
            )
            if err:
                return web.json_response(
                    _openai_error(err, code="insufficient_permissions"),
                    status=403,
                )
        except Exception:
            pass
    ...
    def _verify_project_token(self, token: str) -> dict | None:
        """Verify a project-scoped bearer token (imt_p_*).  Returns project info or None."""
        try:
            from agent.projects import ProjectDB
            from gateway.run import _load_gateway_config

            db = ProjectDB(config=_load_gateway_config())
            row = db.verify_token(token)
            if row and row.get("scope_type") == "project":
                proj_id = row.get("scope_id")
                if proj_id:
                    proj = db.get_project(proj_id)
                    db.close()
                    if proj:
                        return {"project_id": proj.get("slug", proj_id), "scopes": row.get("permissions")}
            db.close()
        except Exception:
            pass
        return None

    def _resolve_member_context(self, request: "web.Request") -> dict:
        """Resolve member/team/project context from request headers and auth token.

        Returns a dict with optional ``member_id``, ``team_id``, ``project_id``.
        Callers can use this to build a RuntimeContext for the agent.
        """
        ctx: dict = {}

        # Trusted reverse-proxy header (before bearer token)
        if "member_id" not in ctx:
            try:
                from agent.members_oauth import resolve_trusted_header_member_from_headers
                from agent.membership import MembershipDB

                cfg = getattr(self, "config", None) or {}
                headers = {k: v for k, v in request.headers.items()}
                db = MembershipDB(config=cfg) if cfg else None
                try:
                    th_mid = resolve_trusted_header_member_from_headers(
                        headers, config=cfg, db=db,
                    )
                    if th_mid:
                        ctx["member_id"] = th_mid
                finally:
                    if db:
                        db.close()
            except Exception:
        ...
        if member_info.get("member_id"):
            ctx["member_id"] = member_info["member_id"]

        # From project token
        proj_info = request.get("project_info", {})
        if proj_info.get("project_id"):
            ctx["project_id"] = proj_info["project_id"]

        # From headers
        team_header = request.headers.get("X-Intellect-Team", "").strip()
        if team_header:
            ctx["team_id"] = team_header

        proj_header = request.headers.get("X-Intellect-Project", "").strip()
        if proj_header and "project_id" not in ctx:
            ctx["project_id"] = proj_header
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-Intellect-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-Intellect-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-Intellect-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-Intellect-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-Intellect-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``intellect sessions list``
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"status": "ok", "platform": "intellect-agent"})

    async def _handle_health_detailed(self, request: "web.Request") -> "web.Response":
        """GET /health/detailed — rich status for dashboard probing.

        Unauthenticated: returns basic status only (safe for load balancers).
        Authenticated: includes gateway state, PID, and platform list.
        """
        from gateway.status import read_runtime_status

        runtime = read_runtime_status() or {}
        authed = self._check_auth(request) is None

        payload: dict = {
            "status": "ok",
            "platform": "intellect-agent",
            "gateway_state": runtime.get("gateway_state") if authed else None,
            "platforms": runtime.get("platforms", {}) if authed else None,
            "active_agents": runtime.get("active_agents", 0) if authed else None,
            "exit_reason": runtime.get("exit_reason") if authed else None,
            "updated_at": runtime.get("updated_at"),
            "pid": os.getpid() if authed else None,
        }
        return web.json_response(payload)

    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — return intellect-agent as an available model."""
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every Intellect version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err

        # Check multi-user feature flags for capabilities
        _members_enabled = False
        _teams_enabled = False
        _projects_enabled = False
        try:
            from intellect_cli.config import load_config
            _cfg = load_config()
            _members_enabled = bool(_cfg.get("members", {}).get("enabled", False))
            _teams_enabled = _members_enabled and bool(_cfg.get("members", {}).get("teams", {}).get("enabled", False))
            _projects_enabled = _members_enabled and bool(_cfg.get("members", {}).get("projects", {}).get("enabled", False))
        except Exception:
            pass

        return web.json_response({
            "object": "intellect.api_server.capabilities",
            "platform": "intellect-agent",
            "model": self._model_name,
            "auth": {
                "type": "bearer",
                "required": bool(self._api_key),
                "member_tokens": _members_enabled,
                "team_header": "X-Intellect-Team" if _teams_enabled else None,
                "project_header": "X-Intellect-Project" if _projects_enabled else None,
            },
            "members": {
                "enabled": _members_enabled,
                "teams": _teams_enabled,
                "projects": _projects_enabled,
            },
            "runtime": {
                "mode": "server_agent",
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side Intellect AIAgent; "
            ...
            "features": {
                ...
                "session_key_header": "X-Intellect-Session-Key",
    ...
    async def _handle_oauth_providers(self, request: "web.Request") -> "web.Response":
        """GET /api/oauth/providers"""
        from intellect_cli.oauth_api import list_providers
        import json
        result = list_providers()
        return web.json_response(result)

    async def _handle_oauth_provider(self, request: "web.Request") -> "web.Response":
        """GET /api/oauth/providers/{provider_id}"""
        from intellect_cli.oauth_api import get_provider
        import json
        pid = request.match_info.get("provider_id", "")
        result = get_provider(pid)
        if not result:
            return web.json_response({"error": "Not found"}, status=404)
        return web.json_response(result)

    async def _handle_oauth_add_provider(self, request: "web.Request") -> "web.Response":
        """POST /api/oauth/providers"""
        from intellect_cli.oauth_api import add_provider
        import json
        data = await request.json()
        result = add_provider(data)
        status = result.pop("status", 200) if isinstance(result, tuple) else 200
        if isinstance(result, tuple):
            result, status = result
        return web.json_response(result, status=status if isinstance(status, int) else 200)

    async def _handle_oauth_update_provider(self, request: "web.Request") -> "web.Response":
        """PUT /api/oauth/providers/{provider_id}"""
        from intellect_cli.oauth_api import update_provider
        import json
        pid = request.match_info.get("provider_id", "")
        data = await request.json()
        result = update_provider(pid, data)
        status = 200
        if isinstance(result, tuple):
            result, status = result
        return web.json_response(result, status=status if isinstance(status, int) else 200)

    async def _handle_oauth_delete_provider(self, request: "web.Request") -> "web.Response":
        """DELETE /api/oauth/providers/{provider_id}"""
        from intellect_cli.oauth_api import delete_provider
        import json
        pid = request.match_info.get("provider_id", "")
        result = delete_provider(pid)
        status = 200
        if isinstance(result, tuple):
            result, status = result
        return web.json_response(result, status=status if isinstance(status, int) else 200)

    async def _handle_oauth_provider_logo(self, request: "web.Request") -> "web.Response":
        """GET /api/oauth/providers/{provider_id}/logo"""
        from intellect_cli.oauth_api import get_provider_logo
        pid = request.match_info.get("provider_id", "")
        result = get_provider_logo(pid)
        if not result:
            return web.json_response({"error": "Not found"}, status=404)
    ...
    async def _handle_oauth_authorize(self, request: "web.Request") -> "web.Response":
        ...
        from intellect_cli.oauth_api import start_authorize
        import json
        data = await request.json()
        result = start_authorize(data)
        status = 200
        if isinstance(result, tuple):
            result, status = result
        return web.json_response(result, status=status if isinstance(status, int) else 200)
    ...
    async def _handle_oauth_callback(self, request: "web.Request") -> "web.Response":
        ...
        from intellect_cli.oauth_api import handle_callback
        import json
    ...
    async def _handle_toolsets(self, request: "web.Request") -> "web.Response":
        ...
        try:
            from intellect_cli.config import load_config
            from intellect_cli.tools_config import (
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted Intellect sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        # Scope to the authenticated member when in multi-user mode.
        member_id = (request.get("member_info") or {}).get("member_id")
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions — create an empty Intellect session row."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "intellect.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        if auth_err:
            return auth_err
        rbac_err = self._check_member_chat_rbac(request)
        if rbac_err:
            return rbac_err
        ...
        headers = {"X-Intellect-Session-Id": effective_session_id or session_id}
        if gateway_session_key:
            headers["X-Intellect-Session-Key"] = gateway_session_key
        return web.json_response(
            {
                "object": "intellect.session.chat.completion",
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-Intellect-Session-Key.  This
        # is independent of X-Intellect-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-Intellect-Session-Id.
        ...
        provided_session_id = request.headers.get("X-Intellect-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-Intellect-Session-Id rejected: "
            ...
            # Sanitize: whitelist alphanumeric + dash + underscore. Reject
            # control chars and other injection vectors (was blacklist-only).
            if not re.fullmatch(r'[a-zA-Z0-9_-]+', provided_session_id):
                return web.json_response(
            ...
            # conversation map to the same Intellect session.  The first user
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``intellect.tool.progress`` with ``status: running``.
        ...
        response_headers = {
            "X-Intellect-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-Intellect-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            err_body["error"]["intellect"] = {
                "completed": completed,
                "partial": is_partial,
                "failed": is_failed,
            }
            response_headers["X-Intellect-Completed"] = "false"
            response_headers["X-Intellect-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + Intellect-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-Intellect-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-Intellect-Error"] = err_msg[:200]
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-Intellect-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-Intellect-Session-Key"] = gateway_session_key
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: intellect.tool.progress`` SSE event so
                ...
                if isinstance(item, tuple) and len(item) == 2 and item[0] == "__tool_progress__":
                    ...
                    await response.write(
                        f"event: intellect.tool.progress\ndata: {event_data}\n\n".encode()
                    )
                else:
                    content_chunk = {
                        "id": completion_id, "object": "chat.completion.chunk",
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = {"X-Intellect-Session-Id": session_id}
        if gateway_session_key:
            response_headers["X-Intellect-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
    ...
    async def _run_agent(
        ...
        def _run():
            ...
            # (e.g. X-Intellect-Session-Id header) can track compression-
    ...
    def _set_run_status(self, run_id: str, status: str, **fields: Any) -> Dict[str, Any]:
        ...
        now = time.time()
        current = self._run_status_store.get(run_id) or {}
        current.update({
            "object": "intellect.run",
            "run_id": run_id,
            "status": status,
            "updated_at": now,
        })
        ...
        return self._run_status_store.set(run_id, current)
    ...
    def _make_run_event_callback(self, run_id: str, loop: "asyncio.AbstractEventLoop"):
        ...
        def _push(event: Dict[str, Any]) -> None:
            self._set_run_status(
                run_id,
                (self._run_status_store.get(run_id) or {}).get("status", "running"),
                last_event=event.get("event"),
            )
            try:
                from agent.events.channels import gateway_run_events_channel
                from agent.events.factory import get_events_backend_name
                from agent.events.redis_sync import publish_sync

                cfg = _load_api_server_config()
                if get_events_backend_name(cfg) == "redis":
                    publish_sync(
                        gateway_run_events_channel(run_id),
                        event,
                        config=cfg,
                    )
            except Exception:
    ...
    async def _sweep_orphaned_runs(self) -> None:
        ...
        while True:
            ...
            for run_id, _status in self._run_status_store.items_terminal_before(cutoff):
                self._run_status_store.pop(run_id)
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # OAuth unified platform
            self._app.router.add_get("/api/oauth/providers", self._handle_oauth_providers)
            self._app.router.add_get("/api/oauth/providers/{provider_id}", self._handle_oauth_provider)
            self._app.router.add_post("/api/oauth/providers", self._handle_oauth_add_provider)
            self._app.router.add_put("/api/oauth/providers/{provider_id}", self._handle_oauth_update_provider)
            self._app.router.add_delete("/api/oauth/providers/{provider_id}", self._handle_oauth_delete_provider)
            self._app.router.add_get("/api/oauth/providers/{provider_id}/logo", self._handle_oauth_provider_logo)
            self._app.router.add_post("/api/oauth/authorize", self._handle_oauth_authorize)
            self._app.router.add_get("/api/oauth/callback", self._handle_oauth_callback)
