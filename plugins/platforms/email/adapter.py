"""
Email platform adapter for the Intellect gateway.

Allows users to interact with Intellect by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        try:
            ...
            _intellect_version = "0"
        imap.xatom(
            "ID",
            f'("name" "intellect-agent" "version" "{_intellect_version}" '
            '"vendor" "ONTOWEB" '
            '"support-email" "noreply@ontoweb.cn")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Intellect Agent")
        ...
        msg_id = f"<intellect-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
