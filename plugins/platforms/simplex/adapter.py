"""SimpleX Chat platform adapter (Intellect plugin).
...
This adapter ships as an Intellect platform plugin under
``plugins/platforms/simplex/``. The Intellect plugin loader scans the
...
discoverable and `intellect setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Intellect — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"intellect-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``intellect setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.intellect/.env`` via ``intellect_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("intellect_cli.config not available; set SIMPLEX_* vars manually in ~/.intellect/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `intellect gateway status` without
