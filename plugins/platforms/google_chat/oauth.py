...
- Per-user tokens (keyed by sender email):
    ``${INTELLECT_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${INTELLECT_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${INTELLECT_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${INTELLECT_HOME}/google_chat_user_oauth_pending.json``
- Shared OAuth client (one per host):
    ``${INTELLECT_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's INTELLECT_HOME helper so the token follows the user's
# profile (e.g. tests can override via INTELLECT_HOME=/tmp/...).
try:
    from intellect_constants import display_intellect_home, get_intellect_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where intellect_constants isn't importable
    # (mirrors the same fallback used by the google-workspace skill's
    # _intellect_home.py shim).
...
def _intellect_home() -> Path:
    """Resolve INTELLECT_HOME at call time (NOT module import).

    Tests and ``INTELLECT_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.intellect/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _intellect_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _intellect_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _intellect_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _intellect_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _intellect_home() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install intellect-agent[google_chat]."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'intellect-agent[google_chat]'")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into INTELLECT_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_intellect_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_intellect_home()}/google_chat_user_token.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for Intellect (native attachment delivery)"
    )
