...
# ``model_tools`` import time, so ``intellect status``, ``intellect chat``, etc.
...
# ``intellect_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Intellect is thinking…" card during the small window between
...
def check_google_chat_requirements() -> bool:
    ...
    on first call.  This is the canonical "are the deps available" probe
    used by the plugin registry and the adapter's own startup gate.
    """
    ...
    def _reimport_google_chat() -> None:
        # Rebind the module-level globals populated by the initial import
        # block so a lazy install takes effect without a restart.
        if _load_google_modules():
            globals()["GOOGLE_CHAT_AVAILABLE"] = True
...
# else gets rejected by _is_google_owned_host to block SSRF scenarios where a
# crafted event points downloadUri at a non-Google endpoint (e.g. the
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to an intellect MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${INTELLECT_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _intellect_home = _get_intellect_home()
        except (ModuleNotFoundError, ImportError):
            _intellect_home = _Path.home() / ".intellect"
        self._thread_count_store = _ThreadCountStore(
            _intellect_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Intellect is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("INTELLECT_HOME", str(_Path.home() / ".intellect"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${INTELLECT_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into an intellect MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Intellect is thinking…' marker message.
        ...
        "Intellect is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Intellect is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Intellect is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Intellect is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Intellect is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``intellect setup``.

    The setup wizard at ``intellect_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.intellect/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. intellect-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.intellect/.env")
    print_info("Restart the gateway: intellect gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``intellect cron`` running as a
    separate process from ``intellect gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Intellect plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'intellect-agent[google_chat]'",
        ...
        # the "Intellect is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Intellect is thinking…' marker message "
