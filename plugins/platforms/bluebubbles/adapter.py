...
class BlueBubblesAdapter(BasePlatformAdapter):
    ...
    def _auth_headers(self) -> Dict[str, str]:
        """Return auth headers for outbound API requests.

        Sends the password via ``X-BlueBubbles-Password`` header so the
        server can authenticate without relying on URL query parameters.
        The BlueBubbles inbound webhook handler already accepts
        ``x-password`` headers (see :meth:`_handle_webhook`); this aligns
        outbound auth with that pattern as defense-in-depth.  The URL query
        parameter is kept for backward compatibility with callers that
        bypass :meth:`_api_get` / :meth:`_api_post`.
        """
        return {"X-BlueBubbles-Password": self.password}
    ...
    async def _api_get(self, path: str) -> Dict[str, Any]:
        ...
        res = await self.client.get(
            self._api_url(path), headers=self._auth_headers(),
        )
    ...
    async def _api_post(self, path: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        res = await self.client.post(
            self._api_url(path), json=payload, headers=self._auth_headers(),
        )
        res.raise_for_status()
        return res.json()
    ...
    async def connect(self) -> bool:
        ...
        # P0 security: warn when credentials are transmitted over cleartext HTTP.
        if not self.server_url.startswith("https://"):
            logger.warning(
                "[bluebubbles] server_url is not HTTPS (%s). "
                "Password will be transmitted in cleartext over the network. "
                "Configure an HTTPS endpoint to protect credentials in transit.",
                self.server_url,
            )
