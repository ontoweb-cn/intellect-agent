"""ntfy platform adapter (Intellect plugin).
...
an Intellect dependency.

This adapter ships as an Intellect platform plugin under
``plugins/platforms/ntfy/``. The Intellect plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "intellect-in"              # subscribe topic (incoming)
          publish_topic: "intellect-out"     # optional — defaults to topic
...
_ECHO_TAG = "intellect-agent"  # tag added to outgoing messages for echo-loop prevention
...
class NtfyAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            from plugins.http_deps import require_httpx

            _http = require_httpx(httpx, plugin="ntfy")
            self._http_client = _http.AsyncClient(timeout=None)
...
async def _standalone_send(
    ...
    *,
    ...
    ``intellect cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already an Intellect dependency",
