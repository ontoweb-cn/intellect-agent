...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: intellect)
...
Or via $INTELLECT_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Intellect can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $INTELLECT_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_intellect_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "intellect": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "intellect"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Intellect config."""
    profile = config.get("profile", "intellect")
    return str(profile or "intellect")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Intellect profile name (from agent_identity)
      {workspace} — Intellect workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``intellect-{user}`` with no user becomes ``intellect``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Intellect Agent and the User"
    ...
    def save_config(self, values, intellect_home):
        """Write config to $INTELLECT_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(intellect_home) / "hindsight"
    ...
    def post_setup(self, intellect_home: str, config: dict) -> None:
        ...
        from intellect_cli.memory_setup import _curses_select
        ...
        if mode == "cloud":
            ...
            if llm_key:
                ...
                env_path = Path(intellect_home) / ".env"
                existing_llm_key = ""
                if env_path.exists():
        ...
        provider_config.setdefault("bank_id", "intellect")
        ...
        self.save_config(provider_config, intellect_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(intellect_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(intellect_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "intellect"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: intellect-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Intellect Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "intellect"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "intellect"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "intellect", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "intellect")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Intellect Agent and the User")
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See intellect-agent#6672.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Intellect created/used that client on the shared async loop,
