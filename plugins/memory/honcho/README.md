...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/intellect>
...
```bash
intellect memory setup honcho   # configure Honcho directly (works on a fresh install)
intellect memory setup          # generic picker, choose Honcho from the list
```

Or manually:
```bash
intellect config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.intellect/.env
```

> `intellect honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `intellect memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$INTELLECT_HOME/honcho.json` | Profile-local (isolated Intellect instances) |
| 2 | `~/.intellect/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Intellect profile: `intellect` (default) or `intellect_<profile>`.
...
**Deployment shapes** (`intellect memory setup honcho` asks one prompt to set these):

- **Single-operator** — `pinUserPeer: true`. All gateway users → `peerName`. Recommended for personal use where you connect Intellect to your own Telegram/Discord/etc.
...
| 4 | `per-session` strategy | Intellect session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`intellect-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`intellect`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-intellect-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening intellect in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Intellect session ID (timestamp + hex). Every `intellect` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Intellect profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`intellect`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Intellect profile: `intellect` (default) or `intellect_<profile>` (e.g. `intellect -p coder` -> host key `intellect_coder`). Older `intellect.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `intellect_HONCHO_HOST` | Host key override |
...
|---------|-------------|
| `intellect memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `intellect honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `intellect memory setup`) |
| `intellect honcho status` | Show resolved config for active profile |
| `intellect honcho enable` / `disable` | Toggle Honcho for active profile |
| `intellect honcho mode <mode>` | Change recall or observation mode |
| `intellect honcho peer --user <name>` | Update user peer name |
| `intellect honcho peer --ai <name>` | Update AI peer name |
| `intellect honcho tokens --context <N>` | Set context token budget |
| `intellect honcho tokens --dialectic <N>` | Set dialectic max chars |
| `intellect honcho map <name>` | Map current directory to a session name |
| `intellect honcho sync` | Create host blocks for all Intellect profiles |
