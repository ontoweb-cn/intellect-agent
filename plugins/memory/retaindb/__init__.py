...
Config (env vars or intellect config.yaml under retaindb:):
...
class _Client:
    ...
    def _headers(self, path: str) -> dict:
        ...
        h = {
            ...
            "x-sdk-runtime": "intellect-plugin",
        }
    ...
    def upload_file(self, data: bytes, filename: str, remote_path: str, mime_type: str, scope: str, project_id: str | None) -> dict:
        ...
        token = self.api_key.replace("Bearer ", "").strip()
        headers = {"Authorization": f"Bearer {token}", "x-sdk-runtime": "intellect-plugin"}
    ...
    def read_file_content(self, file_id: str) -> bytes:
        ...
        resp = requests.get(url, headers={"Authorization": f"Bearer {token}", "x-sdk-runtime": "intellect-plugin"}, timeout=30, allow_redirects=True)
        resp.raise_for_status()
        return resp.content
...
class RetainDBMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        # Project resolution: RETAINDB_PROJECT > intellect-<profile> > "default"
        # If unset, the API auto-creates and uses the "default" project — no config required.
        explicit = os.environ.get("RETAINDB_PROJECT")
        if explicit:
            project = explicit
        else:
            intellect_home = str(kwargs.get("intellect_home", ""))
            profile_name = os.path.basename(intellect_home) if intellect_home else ""
            project = f"intellect-{profile_name}" if (profile_name and profile_name not in {"", ".intellect"}) else "default"

        self._client = _Client(api_key, base_url, project)
        self._session_id = session_id
        self._user_id = kwargs.get("user_id", "default") or "default"
        self._agent_id = kwargs.get("agent_id", "intellect") or "intellect"

        from intellect_constants import get_intellect_home
        intellect_home_path = get_intellect_home()
        db_path = intellect_home_path / "retaindb_queue.db"
        self._queue = _WriteQueue(self._client, db_path)

        # Seed agent identity from SOUL.md in background
        soul_path = intellect_home_path / "SOUL.md"
        if soul_path.exists():
