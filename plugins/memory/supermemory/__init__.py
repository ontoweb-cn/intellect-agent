...
_DEFAULT_CONTAINER_TAG = "intellect"
...
def _load_supermemory_config(intellect_home: str) -> dict:
    config = _default_config()
    config_path = Path(intellect_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, intellect_home: str) -> None:
    config_path = Path(intellect_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `intellect memory setup`.
        # All other options are documented for $INTELLECT_HOME/supermemory.json
    ...
    def save_config(self, values, intellect_home):
        ...
        _save_supermemory_config(sanitized, intellect_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from intellect_constants import get_intellect_home
        self._intellect_home = kwargs.get("intellect_home") or str(get_intellect_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._intellect_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "intellect", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "intellect_memory", "target": target, "type": "explicit_memory"},
    ...
    def _tool_store(self, args: dict) -> str:
        ...
        metadata["source"] = "intellect_tool"
        try:
