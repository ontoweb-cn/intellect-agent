...
Config file: `$INTELLECT_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `intellect` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `intellect-{identity}` → `intellect-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Intellect profile:

```json
{
  "container_tag": "intellect-{identity}"
}
```

For a profile named `coder`, this resolves to `intellect-coder`. The default profile resolves to `intellect-default`. Without `{identity}`, all profiles share the same container.
