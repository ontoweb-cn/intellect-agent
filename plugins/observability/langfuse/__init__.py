"""langfuse — Intellect plugin for Langfuse observability.

Traces Intellect conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Intellect plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``intellect plugins enable
observability/langfuse``, or by checking the box in the interactive
``intellect plugins`` UI). At runtime the plugin also requires the
``langfuse`` SDK and credentials; if either is missing the hooks are inert.

Required env vars (set in ~/.intellect/.env):
  intellect_LANGFUSE_PUBLIC_KEY  - Langfuse project public key (pk-lf-...)
  intellect_LANGFUSE_SECRET_KEY  - Langfuse project secret key (sk-lf-...)
  intellect_LANGFUSE_BASE_URL    - Langfuse server URL (default: https://cloud.langfuse.com)

Optional env vars:
  intellect_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  intellect_LANGFUSE_RELEASE     - release/version tag
  intellect_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  intellect_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  intellect_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
def _debug_enabled() -> bool:
    return _env_bool("intellect_LANGFUSE_DEBUG")
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Intellect plugin system —
    ...
    public_key = _env("intellect_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
    secret_key = _env("intellect_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
    ...
    placeholder_issues = [
        msg
        for msg in (
            _validate_langfuse_key("intellect_LANGFUSE_PUBLIC_KEY", public_key),
            _validate_langfuse_key("intellect_LANGFUSE_SECRET_KEY", secret_key),
        )
        if msg
    ]
    if placeholder_issues:
        logger.warning(
            ...
            "or unset intellect_LANGFUSE_PUBLIC_KEY / intellect_LANGFUSE_SECRET_KEY to "
    ...
    base_url = _env("intellect_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
    environment = _env("intellect_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
    release = _env("intellect_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
    sample_rate = _env("intellect_LANGFUSE_SAMPLE_RATE")
    ...
    if sample_rate:
        try:
            ...
            logger.warning("Invalid intellect_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("intellect_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(
                ...
                trace_name="Intellect turn",
                tags=["intellect", "langfuse"],
            ):
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Intellect branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Intellect also has a turn-scoped
    ...
    # messages is a list only for legacy Intellect branches that fired
    # pre_llm_call with API messages directly. Current Intellect fires
...
def register(ctx) -> None:
    ...
    # Intellect versions.  pre_api_request / post_api_request fire per API
