...
``{title, url, description, position}`` rows every other Intellect web
provider produces.
...
prefers Intellect-managed xAI Grok OAuth (via ``intellect auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.intellect/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``intellect tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `intellect auth` to sign in with "
