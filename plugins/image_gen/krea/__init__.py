...
# Intellect uses 3 abstract aspect ratios. Map to Krea's enum (which is wider).
...
class KreaImageGenProvider(ImageGenProvider):
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        _req = require_requests(requests, plugin="krea-image-gen")
        ...
        if not api_key:
            return error_response(
                error=(
                    "KREA_API_KEY not set. Run `intellect tools` → Image "
        ...
        headers = {
            ...
            "User-Agent": "Intellect-Agent/1.0 (krea-image-gen)",
        }
        ...
        while True:
            ...
            try:
                poll_resp = _req.get(job_url, headers=poll_headers, timeout=30)
