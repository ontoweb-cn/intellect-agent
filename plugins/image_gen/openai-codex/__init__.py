...
Output is saved as PNG under ``$INTELLECT_HOME/cache/images/``.
"""
...
class OpenAICodexImageGenProvider(ImageGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            ...
            "env_vars": [],
            "post_setup_hint": (
                "Sign in with `intellect auth codex` (or `intellect setup` → Codex) "
        ...
        if not _read_codex_access_token():
            return error_response(
                error=(
                    ...
                    "`intellect auth codex` (or `intellect setup` → Codex) to sign in."
                ),
