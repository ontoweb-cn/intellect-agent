...
Auto-tracks and cleans up ephemeral files created during Intellect Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$INTELLECT_HOME` and `/tmp/intellect-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `INTELLECT_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `INTELLECT_HOME` or `/tmp/intellect-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$INTELLECT_HOME/disk-cleanup/` is itself excluded
- `$INTELLECT_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
