"""disk_cleanup — ephemeral file cleanup for Intellect Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under INTELLECT_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly INTELLECT_HOME and /tmp/intellect-*
Never touches: ~/.intellect/logs/ or any system directory.
"""
...
try:
    ...
    def get_intellect_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("INTELLECT_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".intellect").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$INTELLECT_HOME/logs/``."""
    return get_intellect_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$INTELLECT_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under INTELLECT_HOME or ``/tmp/intellect-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    intellect_home = get_intellect_home()
    try:
        path.resolve().relative_to(intellect_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/intellect-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("intellect-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside INTELLECT_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under INTELLECT_HOME (but leave INTELLECT_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "intellect-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(intellect_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == intellect_home:
                continue
            try:
                rel_parts = dirpath.relative_to(intellect_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    intellect_home = get_intellect_home()
    try:
        rel = path.resolve().relative_to(intellect_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "intellect-agent",
        }:
            return None
        ...
        # Path isn't under INTELLECT_HOME (e.g. /tmp/intellect-*) — fall through.
        pass
