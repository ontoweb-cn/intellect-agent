...
``BROWSER_USE_API_KEY`` for self-billed users, or the managed OntoWeb tool
gateway (which Intellect uses to bill Browser Use sessions to a OntoWeb
...
Auth env vars (one of)::
    ...
    # OR a managed OntoWeb gateway entry (configured via 'intellect setup')
"""
...
# Idempotency tracking for managed-mode session creation. The managed OntoWeb
...
class BrowserUseBrowserProvider(BrowserProvider):
    ...
    to the managed OntoWeb tool gateway when ``tool_gateway.browser`` config
    ...
    def _get_config_or_none(self, *, refresh_token: bool = True) -> Optional[Dict[str, Any]]:
        ...
        managed = resolve_managed_tool_gateway(
            "browser-use",
            token_reader=None if refresh_token else peek_ontoweb_access_token,
        )
        if managed is None:
            return None

        return {
            "api_key": managed.ontoweb_user_token,
    ...
    def create_session(self, task_id: str) -> Dict[str, object]:
        from plugins.http_deps import require_requests

        _req = require_requests(requests, plugin="browser-use")
        ...
        # default to a long Browser-Use timeout when Intellect only needs a task-
        ...
        if managed_mode:
            _clear_pending_create_key(task_id)
        session_name = f"intellect_{task_id}_{uuid.uuid4().hex[:8]}"
        external_call_id = (
