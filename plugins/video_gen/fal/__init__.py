...
Authentication via ``FAL_KEY`` or the managed OntoWeb gateway. Output is an
...
def _submit_fal_video_request(endpoint: str, arguments: Dict[str, Any]):
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            raise ValueError(
                f"OntoWeb Subscription gateway rejected endpoint '{endpoint}' "
                f"(HTTP {status}). This model may not yet be enabled on "
                f"the ONTOWEB Portal's FAL proxy. Either:\n"
                f"  • Set FAL_KEY in your environment to use FAL.ai directly, or\n"
                f"  • Pick a different model via `intellect tools` → Video Generation."
            ) from exc
        raise
        ...
        if not _check_fal_video_available():
            return error_response(
                error=(
                    ...
                    "(run `intellect tools` → Video Generation → FAL to configure) "
                    "or sign in to the OntoWeb Portal (`intellect setup`) for managed gateway access."
                ),
            ...
            if not endpoint:
                return error_response(
                    error=(
                        ...
                        f"via `intellect tools` → Video Generation."
                    ),
