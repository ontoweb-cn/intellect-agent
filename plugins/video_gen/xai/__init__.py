...
def _xai_user_agent() -> str:
    try:
        ...
        return intellect_xai_user_agent()
    except Exception:
        return "intellect-agent/video_gen"
...
class XAIVideoGenProvider(VideoGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        # hook (``intellect_cli/tools_config.py``) so the picker doesn't blindly
    ...
    async def _generate_async(
        ...
        *,
        ...
        if not api_key:
            return error_response(
                error=(
                    "No xAI credentials found. Sign in via `intellect auth add xai-oauth` "
        ...
        _http = require_httpx(httpx, plugin="xai-video-gen")
        async with _http.AsyncClient() as client:
            try:
