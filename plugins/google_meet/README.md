...
Let the intellect agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where intellect runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `intellect meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `intellect meet` CLI |
...
| `cli.py` | `intellect meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$INTELLECT_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `intellect meet node {run,list,approve,remove,status,ping}` |
...
intellect meet install                                      # pip + Chromium
intellect meet setup                                        # preflight
intellect meet auth                                         # optional
intellect meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
intellect meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.intellect/.env
intellect meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
intellect meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
intellect meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.intellect/.env
intellect meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, intellect will **not** switch your system audio input automatically — the
...
intellect plugins enable google_meet
intellect meet node run --display-name my-mac --host 0.0.0.0 --port 18789
...
intellect meet node ping my-mac
