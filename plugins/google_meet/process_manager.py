...
session-scoped state file under ``$INTELLECT_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $INTELLECT_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_intellect_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    guest_name: str = "Intellect Agent",
    ...
    If a bot is already running for this intellect install, leave it first —
    ...
    env = os.environ.copy()
    env["intellect_MEET_URL"] = url
    env["intellect_MEET_OUT_DIR"] = str(out)
    env["intellect_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["intellect_MEET_HEADED"] = "1"
    if auth_state:
        env["intellect_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["intellect_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if intellect_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["intellect_MEET_MODE"] = mode
    if realtime_model:
        env["intellect_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["intellect_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["intellect_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["intellect_MEET_REALTIME_KEY"] = realtime_api_key
