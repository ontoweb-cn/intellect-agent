"""`intellect meet node ...` subcommand tree.

Wired into the existing ``intellect meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``intellect meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="intellect-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``intellect meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             intellect meet node approve <name> ws://<host>:{args.port} {token}")
        try:
