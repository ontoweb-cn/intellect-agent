"""Windows UTF-8 bootstrap for Intellect entry points.
...
should be imported at the very top of every Intellect entry point
(``intellect``, ``intellect-agent``, ``intellect-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import intellect_bootstrap``
# (or ``from intellect_bootstrap import apply_windows_utf8_bootstrap``) at
