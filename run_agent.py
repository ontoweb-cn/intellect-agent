...
_loaded_env_paths = load_intellect_dotenv(intellect_home=_intellect_home, project_env=_project_env)
...
from intellect_rust import TokenAccumulator as _TokenAccumulator
...
def _routermint_headers() -> dict:
    ...
    return {
        "User-Agent": f"IntellectAgent/{_intellect_VERSION}",
    }
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[intellect-agent: tool call arguments were corrupted in this session and "
    ...
    def __init__(
        ...
        publish_session_context: bool = True,
    ):
        ...
        init_agent(
            ...
            publish_session_context=publish_session_context,
        )
    ...
    @staticmethod
    def _resolve_member_id() -> str | None:
        """Read the active member_id from .cli-session.json (CLI) or
        environment for gateway/API sessions.
        """
        try:
        ...
        if gw_member:
            return gw_member.strip() or None
        return None
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            ...
            _mid = self._resolve_member_id()
            self._session_db.create_session(
                session_id=self.session_id,
                source=self.platform or os.environ.get("INTELLECT_SESSION_SOURCE", "cli"),
                model=self.model,
        ...
        **extra_context,
        ...
        if should_start and target_session_id and hasattr(engine, "on_session_start"):
            start_context = {
                ...
                "platform": getattr(self, "platform", None) or os.environ.get("INTELLECT_SESSION_SOURCE", "cli"),
    ...
    def reset_session_state(
        ...
        # ── Rust TokenAccumulator (reset or create) ─────────────────────
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        """
        Preload the LM Studio model with at least Intellect' minimum context.
        """
        if (self.provider or "").strip().lower() != "lmstudio":
            return
        try:
            from agent.model_metadata import MINIMUM_CONTEXT_LENGTH
            from intellect_cli.models import ensure_lmstudio_model_loaded
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``intellect chat -q``. In that mode,
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``INTELLECT_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``INTELLECT_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return float(os.getenv("INTELLECT_API_TIMEOUT", 1800.0))
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``INTELLECT_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("INTELLECT_API_CALL_STALE_TIMEOUT")
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        intellect-agent #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See intellect-agent#21444 for symptom history."
        )
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        # OntoWeb serves GPT-5.x models via its OpenAI-compatible chat
        # completions endpoint; its /v1/responses endpoint returns 404.
        if normalized_provider == "ontoweb":
            return False
    ...
    def _redact_message_content(content):
        ...
        Respects ``INTELLECT_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.intellect/sessions/session_{sid}.json``
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects INTELLECT_REDACT_SECRETS via
    ...
    def _file_mutation_verifier_enabled(self) -> bool:
        ...
        ``intellect_FILE_MUTATION_VERIFIER`` env var overrides config.  Exposed
        as a method so tests can patch a single seam without reaching into
        the private ``_turn_failed_file_mutations`` state dict.
        """
        try:
            import os as _os
            env = _os.environ.get("intellect_FILE_MUTATION_VERIFIER")
            if env is not None:
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.intellect/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``intellect_TURN_COMPLETION_EXPLAINER`` env var overrides
        config.  Exposed as a method so tests can patch a single seam,
        mirroring ``_file_mutation_verifier_enabled``.
        """
        try:
            import os as _os
            env = _os.environ.get("intellect_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
            # Read from the persisted config.yaml so gateway and CLI share
            # the same setting.  Import lazily to avoid a startup-time cycle.
    ...
    def _touch_activity(self, desc: str) -> None:
        ...
        process is a dispatcher-spawned worker (INTELLECT_KANBAN_TASK set),
    ...
    def _sync_external_memory_for_turn(
        ...
        *,
        ...
        if interrupted:
            return
        if not final_response or not original_user_message:
            return
        try:
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``intellect auth add xai-oauth``) whose tokens belong to
    ...
    def _try_refresh_ontoweb_client_credentials(
        self,
        *,
        force: bool = True,
    ) -> bool:
        if self.api_mode != "chat_completions" or self.provider != "ontoweb":
            return False
        ...
        try:
            ...
            logger.debug("OntoWeb credential refresh failed: %s", exc)
            return False
        ...
        self._client_kwargs["api_key"] = self.api_key
        self._client_kwargs["base_url"] = self.base_url
        # OntoWeb requests should not inherit OpenRouter-only attribution headers.
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from intellect_cli.copilot_auth import resolve_copilot_token
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        known reasoning-capable model families and direct ONTOWEB Portal.
        """
        if base_url_host_matches(self._base_url_lower, "ontoweb.cn"):
            return True
        if (
            ...
            try:
                from intellect_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from intellect_cli.models import lmstudio_model_reasoning_options
