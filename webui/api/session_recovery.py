...
def _recovery_uses_session_bridge(state_db_path: Path | None) -> bool:
    """Production callers pass ``state_db_path=None`` to use :mod:`api.storage_bridge`."""
    return state_db_path is None


def _state_db_has_session(session_id: str, state_db_path: Path | None) -> bool:
    """Return whether the session store still knows this session.

    The check is deliberately fail-open: recovery must not be prevented by a
    locked, absent, or older-schema DB. When a store is readable and has no
    row, treat the orphan backup as a tombstoned/deleted session and skip it.
    """
    if _recovery_uses_session_bridge(state_db_path):
        try:
            from api.storage_bridge import open_session_db, state_storage_available
            ...
            logger.debug("session store tombstone check failed for %s: %s", session_id, exc)
            return True
...
def _read_missing_sidecar_rows_from_conn(
    conn,
    session_dir: Path,
    *,
    include_empty: bool = False,
    session_cols: set[str] | None = None,
    message_cols: set[str] | None = None,
    dialect: str | None = None,
) -> list[dict]:
    if session_cols is None or message_cols is None:
        # conn may be raw sqlite3 or SessionDB-backed; table_column_names needs db wrapper
        raise ValueError("session_cols and message_cols required")

    if dialect is None:
        try:
            from api.storage_bridge import infer_connection_dialect
    ...
    try:
        from api.storage_bridge import (
            messages_order_by_sql,
            optional_column_sql,
            sessions_order_by_started_desc_sql,
        )
    except ImportError:
        messages_order_by_sql = None  # type: ignore[assignment]
        optional_column_sql = None  # type: ignore[assignment]
        sessions_order_by_started_desc_sql = None  # type: ignore[assignment]

    if optional_column_sql is not None:
        title_expr = optional_column_sql("title", session_cols, dialect=dialect)
        model_expr = optional_column_sql("model", session_cols, dialect=dialect)
        started_expr = optional_column_sql("started_at", session_cols, "0", dialect=dialect)
        parent_expr = optional_column_sql("parent_session_id", session_cols, dialect=dialect)
        msg_count_expr = optional_column_sql("message_count", session_cols, "0", dialect=dialect)
        workspace_expr = optional_column_sql("workspace", session_cols, dialect=dialect)
        worktree_path_expr = optional_column_sql("worktree_path", session_cols, dialect=dialect)
        worktree_branch_expr = optional_column_sql("worktree_branch", session_cols, dialect=dialect)
        worktree_repo_root_expr = optional_column_sql("worktree_repo_root", session_cols, dialect=dialect)
        worktree_created_at_expr = optional_column_sql("worktree_created_at", session_cols, dialect=dialect)
        order_by = sessions_order_by_started_desc_sql(dialect=dialect)
    else:
        ...
        order_by = "ORDER BY COALESCE(started_at, 0) DESC"
    ...
    for row in session_rows:
        ...
        if {"session_id", "role", "content"}.issubset(message_cols):
            if messages_order_by_sql is not None:
                order = messages_order_by_sql(message_cols, dialect=dialect)
            else:
                order = (
                    "timestamp, id"
            ...
            msg_sql = (
                f"SELECT role, content, {ts_expr} FROM messages "
                f"WHERE session_id = ? ORDER BY {order}"
            )
            if execute is not None:
                msg_rows = execute(msg_sql, (sid,)).fetchall()
            else:
                msg_rows = conn.execute(msg_sql, (sid,)).fetchall()
            for msg in msg_rows:
                msg_data = _row_to_dict(msg)
                message = {
                    "role": msg_data.get("role"),
                    "content": msg_data.get("content") or "",
                }
                if msg_data.get("timestamp") is not None:
...
def _read_state_db_missing_sidecar_rows(
    ...
    *,
    ...
    """Return WebUI-origin session-store rows whose JSON sidecar is missing."""
    if _recovery_uses_session_bridge(state_db_path):
        try:
            from api.storage_bridge import open_session_db, state_storage_available, table_column_names
            ...
            with open_session_db() as db:
                if db is None:
                    return []
                session_cols = table_column_names(db, "sessions")
                message_cols = table_column_names(db, "messages")
                dialect = getattr(getattr(db, "_storage_backend", None), "dialect", None)
                if dialect is None:
                    from api.storage_bridge import infer_connection_dialect

                    dialect = infer_connection_dialect(db._conn)
                return _read_missing_sidecar_rows_from_conn(
                    db._conn,
                    session_dir,
            ...
            logger.warning("session store sidecar reconciliation scan failed: %s", exc)
            return []
...
def _main() -> int:
    parser = argparse.ArgumentParser(description="Audit Intellect WebUI session recovery state")
    ...
    parser.add_argument("--state-db", type=Path, default=None, help="optional Intellect state.db path")
