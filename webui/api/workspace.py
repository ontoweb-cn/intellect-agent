"""
Intellect Web UI -- Workspace and file system helpers.
...
def _profile_state_dir() -> Path:
    ...
    INTELLECT_WEBUI_STATE_DIR env var for test isolation).
    For named profiles, returns {profile_home}/webui_state/.
    """
    try:
        from api.profiles import get_active_profile_name, get_active_intellect_home
        name = get_active_profile_name()
        if name and name != 'default':
            d = get_active_intellect_home() / 'webui_state'
...
def _profile_default_workspace() -> str:
    ...
    Checks keys in priority order:
      ...
      3. 'terminal.cwd'      — intellect-agent terminal working dir (most common)
...
def _clean_workspace_list(workspaces: list) -> list:
    ...
    - Remove entries whose paths live inside another profile's directory
      (e.g. ~/.intellect/profiles/X/... should not appear on a different profile).
...
def _workspace_access_error(candidate: Path, *, missing_label: str = "Path does not exist") -> str | None:
    ...
    try:
        ...
        return (
            ...
            f"Folders permission to the Intellect/WebUI app or server process, then try again."
        )
...
def list_workspace_suggestions(prefix: str = "", limit: int = 12) -> list[str]:
    ...
    _tilde_prefix = ""
    if raw.startswith("~"):
        _tilde_prefix = raw.split("/", 1)[0]  # ~ or ~user
    ...
    for child in children:
        ...
        if len(suggestions) >= limit:
            break
    # Preserve tilde prefix in output: when the user typed ~/..., rewrite
    # the expanded home path back to ~ so the suggestion matches their input.
    if _tilde_prefix:
        _home = str(Path.home())
        suggestions = [
            s.replace(_home, _tilde_prefix, 1) if s.startswith(_home) else s
