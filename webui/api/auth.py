"""
Intellect Web UI -- Optional password authentication.
Off by default. Enable by setting INTELLECT_WEBUI_PASSWORD env var
...
def _resolve_session_ttl() -> int:
    ...
    Priority mirrors get_password_hash(): INTELLECT_WEBUI_SESSION_TTL env var
    ...
    """
    env_v = os.getenv('INTELLECT_WEBUI_SESSION_TTL', '').strip()
    if env_v.isdigit():
        val = int(env_v)
        if 60 <= val <= 86400 * 365:
            return val
...
COOKIE_NAME = 'intellect_session'
CSRF_HEADER_NAME = 'X-Intellect-CSRF-Token'
...
def get_password_hash() -> str | None:
    ...
    with _AUTH_HASH_LOCK:
        ...
        env_pw = os.getenv('INTELLECT_WEBUI_PASSWORD', '').strip()
        if env_pw:
...
def _passkey_feature_flag_enabled() -> bool:
    ...
    endpoints, no credential storage. To enable:
      ...
      - Set ``INTELLECT_WEBUI_PASSKEY=1`` in the environment, OR
    ...
    """
    env_value = os.getenv("INTELLECT_WEBUI_PASSKEY", "")
    if env_value:
...
def csrf_token_for_session(cookie_value: str) -> str | None:
    ...
    in ``X-Intellect-CSRF-Token`` on unsafe API requests. The token is derived
...
def verify_csrf_token(cookie_value: str, csrf_token: str) -> bool:
    ...
    session_valid = bool(cookie_value) and verify_session(cookie_value)
    if not cookie_value or not csrf_token or not session_valid:
        logger.debug("verify_csrf_token early fail has_cookie=%s has_token=%s session_valid=%s",
                     bool(cookie_value), bool(csrf_token), session_valid)
        return False
    expected = csrf_token_for_session(cookie_value)
    ok = bool(expected and hmac.compare_digest(str(csrf_token), expected))
    if not ok:
        logger.debug("verify_csrf_token HMAC mismatch expected_len=%s submitted_len=%s",
                     len(expected or ''), len(str(csrf_token)))
    return ok
...
def is_loopback_host(host: str) -> bool:
    """True when host is localhost / loopback (member dev login)."""
...
def is_loopback_client(handler) -> bool:
    return is_loopback_host((handler.client_address or ("127.0.0.1", 0))[0])


def _multi_user_members_enabled() -> bool:
    try:
...
def build_login_context(handler, parsed) -> dict:
    """Public login page context (legacy vs multi_user — mutually exclusive)."""
    from api.members import agent_membership_available, resolve_member_id

    webui_auth = is_auth_enabled()
    webui_logged_in = False
    if webui_auth:
        cv = parse_cookie(handler)
        webui_logged_in = bool(cv and verify_session(cv))

    mode = "legacy"
    members_block: dict = {"enabled": False}
    actor: str | None = None
    agent_ok = agent_membership_available()

    if agent_ok:
        from agent.membership import is_members_enabled
        from api.members import get_status, load_members_config

        config = load_members_config()
        if is_members_enabled(config):
            mode = "multi_user"
            status = get_status(handler, parsed)
            actor = status.get("actor_member_id")
            members_block = {
                "enabled": True,
                "teams_enabled": bool(status.get("teams_enabled")),
                "oauth_enabled": bool(status.get("oauth_enabled")),
                "oauth_required": bool(status.get("oauth_required")),
                "oauth_providers": status.get("oauth_providers") or [],
                "oauth_callback_uri": status.get("oauth_callback_uri"),
            }
    else:
        members_block["agent_available"] = False
        if _members_flag_from_config():
            members_block["enabled"] = True
            members_block["config_enabled"] = True

    show_local = False  # local dev picker is not used in multi-user mode
    ready = False
    if mode == "legacy":
        ready = (not webui_auth) or webui_logged_in
    else:
        ready = bool(actor)

    return {
        "mode": mode,
        "webui_auth_enabled": webui_auth,
        "webui_logged_in": webui_logged_in,
        "member_logged_in": bool(actor),
        "actor_member_id": actor,
        "show_local_dev_form": show_local,
...
def build_register_context(handler, parsed) -> dict:
    """Public register page context (multi-user registration)."""
    from api.members import local_registration_requires_approval, load_members_config

    ctx = build_login_context(handler, parsed)
    ctx["registration_open"] = ctx.get("mode") == "multi_user"
    if ctx["registration_open"]:
        config = load_members_config()
        local_enabled = local_registration_requires_approval(config)
        ctx["local_registration_requires_approval"] = local_enabled
        default_tab = "local" if local_enabled else "oauth"
        ctx["register_default_tab"] = default_tab
    return ctx
...
def check_auth(handler, parsed) -> bool:
    ...
    if parsed.path.startswith('/api/'):
        ...
        from api.helpers import login_redirect_location

        handler.send_header('Location', login_redirect_location(parsed))
        handler.end_headers()
    return False
...
def _is_secure_context(handler=None) -> bool:
    ...
    Behaviour is overridable via INTELLECT_WEBUI_SECURE env var for
    ...
    """
    env = os.getenv('INTELLECT_WEBUI_SECURE', '').strip().lower()
...
def set_auth_cookie(handler, cookie_value) -> None:
    ...
    handler.send_header('Set-Cookie', webui_auth_cookie_line(handler, cookie_value))
