"""
Intellect Web UI -- Gateway session watcher.
...
requiring any changes to intellect-agent.
"""
import hashlib
import json
import logging
...
from api.agent_sessions import read_importable_agent_session_rows_for_profile
...
def _gateway_session_from_row(row: dict) -> dict:
    return {
        'session_id': row['id'],
        'title': row.get('title') or 'Agent Session',
...
def _get_agent_sessions_from_db() -> list:
    """Read importable agent sessions from the active storage backend (W6)."""
    try:
        rows = read_importable_agent_session_rows_for_profile(
            None,
            limit=200,
            log=logger,
            exclude_sources=("cron", "webui"),
        )
        return [_gateway_session_from_row(row) for row in rows]
    except Exception:
        logger.debug("gateway watcher session list failed", exc_info=True)
        return []
