"""
Intellect Web UI -- Shared configuration, constants, and global state.
...
# ── Network config (env-overridable) ─────────────────────────────────────────
HOST = os.getenv("INTELLECT_WEBUI_HOST", "127.0.0.1")
PORT = int(os.getenv("INTELLECT_WEBUI_PORT", "9119"))

# ── TLS/HTTPS config (optional, env-overridable) ────────────────────────────
TLS_CERT = os.getenv("INTELLECT_WEBUI_TLS_CERT", "").strip() or None
...
# ── Intellect agent directory discovery ─────────────────────────────────────────
def _discover_agent_dir() -> Path:
    """
    Locate the intellect-agent checkout using a multi-strategy search.

    Priority:
      1. INTELLECT_WEBUI_AGENT_DIR env var  -- explicit override always wins
      2. INTELLECT_HOME / intellect-agent      -- e.g. ~/.intellect/intellect-agent
      3. Sibling of this repo            -- ../intellect-agent
      4. Parent of this repo             -- ../../intellect-agent (nested layout)
      5. Common install paths            -- ~/.intellect/intellect-agent (again as fallback)
      6. HOME / intellect-agent             -- ~/intellect-agent (simple flat layout)
    """
    candidates = []

    # 1. Explicit env var
    if os.getenv("INTELLECT_WEBUI_AGENT_DIR"):
        candidates.append(
            Path(os.getenv("INTELLECT_WEBUI_AGENT_DIR")).expanduser().resolve()
        )

    # 2. INTELLECT_HOME / intellect-agent
    intellect_home = os.getenv("INTELLECT_HOME", str(HOME / ".intellect"))
    candidates.append(Path(intellect_home).expanduser() / "intellect-agent")

    # 3. Parent is the agent repo itself (webui/ lives inside the agent project)
    candidates.append(REPO_ROOT.parent)

    # 4. Sibling: <agent-root>/../intellect-agent (legacy standalone webui layout)
    if (REPO_ROOT.parent.parent / "intellect-agent" / "run_agent.py").exists():
        candidates.append(REPO_ROOT.parent.parent / "intellect-agent")

    # 5. ~/.intellect/intellect-agent (explicit common path)
    candidates.append(HOME / ".intellect" / "intellect-agent")

    # 6. ~/intellect-agent
    candidates.append(HOME / "intellect-agent")

    # 7. XDG_DATA_HOME / intellect-agent  (e.g. ~/.local/share/intellect-agent)
    xdg_data = Path(os.getenv("XDG_DATA_HOME", str(HOME / ".local" / "share")))
    candidates.append(xdg_data.expanduser() / "intellect-agent")

    # 8. System-wide install paths (e.g. /opt/intellect-agent, /usr/local/intellect-agent)
    for sys_prefix in ("/opt", "/usr/local", "/usr/local/share"):
        candidates.append(Path(sys_prefix) / "intellect-agent")
...
def _discover_python(agent_dir: Path) -> str:
    """
    Locate a Python executable that has the Intellect agent dependencies installed.
...
# `pip install -t .` inside the intellect-agent checkout, third-party
# package directories (openai/, pydantic/, requests/, etc.) end up
# alongside real Intellect source files.  Putting _AGENT_DIR at the
...
# and Intellect-specific modules (run_agent, intellect/, etc.) still
...
def _get_config_path() -> Path:
    ...
    env_override = os.getenv("INTELLECT_CONFIG_PATH")
    if env_override:
        return Path(env_override).expanduser()
    try:
        from api.profiles import get_active_intellect_home

        return get_active_intellect_home() / "config.yaml"
    except ImportError:
        return HOME / ".intellect" / "config.yaml"
...
def _save_yaml_config_file(config_path: Path, config_data: dict) -> None:
    try:
        ...
        raise RuntimeError("PyYAML is required to write Intellect config.yaml") from exc
...
def _workspace_candidates(raw: str | Path | None = None) -> list[Path]:
    ...
    add(raw)
    if os.getenv("INTELLECT_WEBUI_DEFAULT_WORKSPACE"):
        add(os.getenv("INTELLECT_WEBUI_DEFAULT_WORKSPACE"))
...
def resolve_default_workspace(raw: str | Path | None = None) -> Path:
    ...
    raise RuntimeError(
        ...
        "Set INTELLECT_WEBUI_DEFAULT_WORKSPACE to a writable path."
    )
...
def _discover_default_workspace() -> Path:
    ...
    Resolve the default workspace in order:
      1. INTELLECT_WEBUI_DEFAULT_WORKSPACE env var
...
DEFAULT_MODEL = os.getenv("INTELLECT_WEBUI_DEFAULT_MODEL", "")  # Empty = use provider default; avoids showing unavailable OpenAI model to non-OpenAI users (#646)
...
def print_startup_config() -> None:
    ...
    lines = [
        "",
        "  Intellect Web UI -- startup config",
        "  --------------------------------",
    ...
    if not _INTELLECT_FOUND:
        print(
            f"{err}  Could not find the Intellect agent directory.\n"
...
def verify_intellect_imports() -> tuple:
    """
    Attempt to import the key Intellect modules.
...
MAX_UPLOAD_BYTES = _env_mb_bytes("INTELLECT_WEBUI_MAX_UPLOAD_MB", 20)
...
_LEGACY_CLI_TOOLSET_ALIASES = {
    # Older Intellect configs used "intellect" as the CLI composite toolset. Modern
    # Intellect Agent exposes that split as these two registered composites; keep
    # WebUI sessions usable when pointed at an older shared config.yaml.
    "intellect": ("intellect-cli", "intellect-api-server"),
}
...
_FALLBACK_MODELS = [
    ...
    # are visible even when the tool-support filter in intellect_cli strips
...
# Provider display names for known Intellect provider IDs
_PROVIDER_DISPLAY = {
    ...
    "volcengine-coding-plan": "Volcano Engine Coding Plan",
    "volcengine-agent-plan": "Volcano Engine Agent Plan",
    "bedrock": "AWS Bedrock",
}
...
#
# This table is authoritative for the WebUI.  When ``intellect_cli.models``
...
# not on ``sys.path`` (CI, installs without intellect-agent cloned
# alongside the WebUI).
_PROVIDER_ALIASES = {
    "glm": "zai",
    "z-ai": "zai",
    ...
    "volc-coding": "volcengine-coding-plan",
    "doubao-coding": "volcengine-coding-plan",
    "volcengine-coding": "volcengine-coding-plan",
    "volc-agent": "volcengine-agent-plan",
    "doubao-agent": "volcengine-agent-plan",
    ...
    # ``intellect_cli.auth.PROVIDER_REGISTRY``. Routing it through ``custom``
...
def _resolve_provider_alias(name: str) -> str:
    ...
    additional aliases the agent provides (when intellect_cli is on
    ...
    try:
        from intellect_cli.models import _PROVIDER_ALIASES as _agent_aliases
...
def _canonicalise_provider_id(name: object) -> str:
    ...
    (the intellect_cli alias target which the WebUI doesn't index by).
    ...
    # at non-canonical strings (legacy, intellect_cli-specific) from leaking
...
_PROVIDER_MODELS = {
    ...
    # 'gemini' is the intellect_cli provider ID for Google AI Studio
    ...
    # Volcengine (火山引擎) — Doubao models via open.volcengineapi.com
    "volcengine": [
    ...
    # Volcengine Coding Plan — dedicated coding tier
    ...
    # Volcengine Agent Plan — dedicated agent tier
    ...
    # intellect_cli.models.provider_model_ids("bedrock") when available (#2720).
    "bedrock": [
...
# ── Xiaomi MiMo URL configuration ─────────────────────────────────────────
# API (SK prefix) and Token Plan (TP prefix) use different base URLs.
_XIAOMI_URLS = {
...
# Key prefix → default URL mapping for auto-detection
_XIAOMI_KEY_PREFIX_MAP = {
    "sk": "api",
    "tp": "token_plan_cn",  # Default to China cluster for Token Plan
}
...
_LOCAL_SERVER_PROVIDERS = {
    "lmstudio",     # canonical (in intellect_cli.models.CANONICAL_PROVIDERS)
...
def resolve_model_provider(model_id: str) -> tuple:
    """Resolve model name, provider, and base_url for WebUI routing and persistence."""
    model, provider, base_url = _resolve_model_provider(model_id)
...
def get_effective_default_model(config_data: dict | None = None) -> str:
    """Resolve the effective Intellect default model from config, then env overrides."""
    ...
    env_model = (
        os.getenv("INTELLECT_MODEL") or os.getenv("OPENAI_MODEL") or os.getenv("LLM_MODEL")
    )
    if env_model:
...
# Mirrors intellect_constants.parse_reasoning_effort so WebUI can validate without
...
def _heuristic_reasoning_efforts(model_id: str, provider_id: str) -> list[str]:
    """Fallback when intellect_cli is unavailable."""
    ...
    if provider in {"copilot", "github-copilot"}:
        if bare.startswith(("gpt-5", "o1", "o3", "o4")):
            if bare.startswith(("o1", "o3", "o4")):
                return ["low", "medium", "high"]
            return list(VALID_REASONING_EFFORTS)
    # Session-stored models are often bare (e.g. "deepseek-v4-pro"), but the
    # prefix list expects provider/model form (e.g. "deepseek/").  Reconstruct
    # the full path when the bare form doesn't match.
    candidates = [model]
    ...
    # #3202: also match known reasoning-capable model families when they
    # appear as bare names (no provider prefix) or inside a dot-separated
    # custom-provider model name.  Previously a bare "deepseek-v4-pro" or
    # "volcano.doubao-pro-256k" would return [] because the prefix list
    # didn't have anything for those providers.
    _reasoning_families = (
        "deepseek",
        "claude",
        "gpt-",
    ...
    for candidate in candidates:
        if any(candidate.startswith(prefix) for prefix in prefixes):
            return list(VALID_REASONING_EFFORTS)
        if any(family in candidate for family in _reasoning_families):
            return list(VALID_REASONING_EFFORTS)
    return []
...
def resolve_model_reasoning_efforts(
    ...
    if "/" not in model_lower and provider:
        candidates.append(f"{provider}/{model_lower}")
    prefixes = (
        "deepseek/",
        "anthropic/",
        "openai/",
...
def set_intellect_default_model(model_id: str) -> dict:
    """Persist the Intellect default model in config.yaml and reload runtime config."""
    ...
    with _cfg_lock:
        ...
        # prefix is a WebUI-internal routing hint that the intellect-agent CLI does
        # not understand — if we wrote `@nous:anthropic/claude-opus-4.6` to
        # config.yaml, a user who ran `intellect` in the terminal right after
...
# Canonical auxiliary task slots. Keep in sync with intellect_cli/config.py
# DEFAULT_CONFIG["auxiliary"] and intellect_cli/web_server.py _AUX_TASK_SLOTS.
...
# INTELLECT_WEBUI_STATE_DIR / port) has its own file and test runs never
...
def _get_auth_store_path() -> Path:
    """Return the auth.json path for the active Intellect profile."""
    try:
        from api.profiles import get_active_intellect_home as _gah

        return _gah() / "auth.json"
    except ImportError:
        return HOME / ".intellect" / "auth.json"
...
def _read_live_provider_model_ids(provider_id: str) -> list[str]:
    """Return live model IDs from Intellect CLI for a provider, or [] on failure.
    ...
    groups should ask ``intellect_cli.models.provider_model_ids()`` first (#1240).
    Provider aliases are tried as a secondary lookup because WebUI keeps a few
    display-facing IDs (for example ``google`` / ``x-ai``) that Intellect CLI may
    ...
    if not pid:
        return []
    try:
        from intellect_cli.models import provider_model_ids as _provider_model_ids
    except Exception:
        return []
    ...
    for candidate in candidates:
        try:
            ...
            logger.debug("Failed to load %s models from intellect_cli", candidate)
            continue
        result: list[str] = []
        for mid in live_ids:
...
def _models_from_live_provider_ids(provider_id: str, live_ids: list[str]) -> list[dict]:
    """Convert Intellect CLI model ids into WebUI picker model entries."""
...
def _read_custom_endpoint_models(
    ...
    provider: str,
    api_key: str = "",
    trusted_base_urls: tuple = (),
) -> tuple[list[dict], dict | None]:
    """Fetch model list from an OpenAI-compatible /v1/models endpoint.

    Returns (models_list, error_dict). On success error_dict is None.
    Each model is ``{"id": ..., "label": ...}``.
    """
    import ipaddress
    import socket
    import urllib.error
    ...
    if base.endswith("/v1"):
        ...
        endpoint_url = base.rstrip("/") + "/v1/models"
        ...
        try:
            ...
            for _, _, _, _, addr in resolved:
                ...
                if addr_obj.is_private or addr_obj.is_loopback or addr_obj.is_link_local:
                    ...
                    is_known = (
                        any(k in host_l for k in ("ollama", "localhost", "127.0.0.1", "lmstudio", "lm-studio"))
                        or host_l in {urlparse(b if "://" in b else f"http://{b}").hostname for b in trusted_base_urls if b}
                    )
                    if not is_known:
                        return [], {"error": f"SSRF blocked: resolved to private IP {addr[0]}"}
    ...
    for m in raw_models:
        ...
        if not mid or mid in seen:
            continue
        seen.add(mid)
        label = str(m.get("name") or m.get("label") or mid).strip()
...
def get_available_models() -> dict:
    ...
    Discovery order:
      ...
      2. Check for known API keys in env or ~/.intellect/.env
    ...
    def _build_available_models_uncached() -> dict:
        ...
        _intellect_auth_used = False
        try:
            from intellect_cli.models import list_available_providers as _lap
            from intellect_cli.auth import get_auth_status as _gas
            ...
            _intellect_auth_used = True
        except Exception:
            logger.debug("Failed to detect auth providers from intellect")

        if not _intellect_auth_used:
            try:
                from api.profiles import get_active_intellect_home as _gah2

                intellect_env_path = _gah2() / ".env"
            except ImportError:
                intellect_env_path = HOME / ".intellect" / ".env"
            env_keys = {}
            if intellect_env_path.exists():
                try:
                    for line in intellect_env_path.read_text(encoding="utf-8").splitlines():
                        line = line.strip()
                        if line and not line.startswith("#") and "=" in line:
                            k, v = line.split("=", 1)
                            env_keys[k.strip()] = v.strip().strip('"').strip("'")
                except Exception:
                    logger.debug("Failed to parse intellect env file")
            all_env = {**env_keys}
            for k in (
                "ANTHROPIC_API_KEY",
                "OPENAI_API_KEY",
            ...
            # Volcengine (火山引擎)
            if all_env.get("VOLCENGINE_CODING_PLAN_API_KEY"):
                detected_providers.add("volcengine-coding-plan")
            if all_env.get("VOLCENGINE_AGENT_PLAN_API_KEY"):
                detected_providers.add("volcengine-agent-plan")
            if all_env.get("VOLCENGINE_API_KEY"):
                detected_providers.add("volcengine")
            # LM Studio: detect via LM_API_KEY + LM_BASE_URL in ~/.intellect/.env
                ...
                try:
                    ...
                    if addr.is_private or addr.is_loopback or addr.is_link_local:
                        if "ollama" in host or "127.0.0.1" in host or "localhost" in host:
                            ...
                            # ``intellect_cli.auth.PROVIDER_REGISTRY`` — see #1384.
                            provider = "custom"
                except ValueError:
                    pass
        ...
        if isinstance(_custom_providers_cfg, list):
            ...
            for _cp in _custom_providers_cfg:
                ...
                if _slug and _cp_base_url:
                    ...
                    # discovery (same as intellect-agent model_switch.py Section 4
        ...
        if detected_providers:
            for pid in sorted(detected_providers):
                ...
                if pid == "openrouter":
                    ...
                    #   (1) curated tool-supporting catalog via intellect_cli.models.fetch_openrouter_models()
                    ...
                    try:
                        ...
                        logger.warning("Failed to load OpenRouter curated catalog from intellect_cli")
                    ...
                    try:
                        ...
                        logger.warning("Failed to load Ollama Cloud models from intellect_cli")
                    ...
                    try:
                        ...
                        logger.warning("Failed to load OpenAI Codex models from intellect_cli")
                    ...
                    # CI environments where intellect_cli isn't importable —
                    ...
                    try:
                        ...
                        logger.debug("intellect_cli LM Studio lookup unavailable; using urlopen fallback")
                    ...
                    # Otherwise, ask Intellect CLI first so WebUI tracks the same
...
# ── Static file path ─────────────────────────────────────────────────────────
STATIC_DIR = os.getenv("INTELLECT_WEBUI_STATIC_DIR", "static")
_INDEX_HTML_PATH = REPO_ROOT / STATIC_DIR / "index.html"
...
def session_agent_cache_key(session_id: str, member_id: str | None = None) -> str:
    """Cache key scoped by member to prevent cross-member agent reuse."""
...
def _evict_session_agent(session_id: str, member_id: str | None = None) -> None:
    ...
    """
    agent = None
    keys = {session_id, session_agent_cache_key(session_id, member_id)}
    with SESSION_AGENT_CACHE_LOCK:
        for key in list(SESSION_AGENT_CACHE.keys()):
            if key in keys or key.endswith(f":{session_id}"):
                entry = SESSION_AGENT_CACHE.pop(key, None)
                if entry is not None and agent is None:
...
_SETTINGS_DEFAULTS = {
    ...
    "ignore_agent_updates": False,  # U10: persist ignore-agent toggle (was bool-key-only)
    "update_channel": "stable",  # W8: stable | experimental
    "whats_new_summary_enabled": False,  # show an LLM-written What's New summary before diff links
    "theme": "dark",  # light | dark | system
    "skin": "default",  # accent color skin: default | ares | mono | slate | poseidon | sisyphus | charizard | sienna | catppuccin
    ...
    "dashboard_title": os.getenv(
        "INTELLECT_WEBUI_DASHBOARD_TITLE", "Intellect Agent Harness"
    ),  # title-bar brand label (logo + text, left side)
    ...
    "members_local_requires_approval": True,  # show local self-registration tab; pending admin approval
...
# Exception: if INTELLECT_WEBUI_DEFAULT_WORKSPACE is explicitly set in the
...
try:
    ...
    pass  # intellect_cli not available -- default profile only
