"""Intellect Web UI -- Session model and in-memory session store."""
import collections
import copy
import datetime
import hashlib
...
from api.agent_sessions import (
    ...
    read_importable_agent_session_rows_for_profile,
    read_session_lineage_metadata_for_profile,
)
...
# Centralised session-id character allowlist.  See #3023.
...
class Session:
    def __init__(self, session_id: str=None, title: str='Untitled',
                 ...
                 member_id: str=None, team_id: str=None,
        ...
        self.member_id = str(member_id).strip() if member_id else None
        self.team_id = str(team_id).strip() if team_id else None
    ...
    def save(self, touch_updated_at: bool = True, skip_index: bool = False) -> None:
        ...
        try:
            from api.session_visibility import (
                ensure_session_owner,
                members_require_member_id_on_save,
            )

            ensure_session_owner(self)
            if members_require_member_id_on_save() and not getattr(self, "member_id", None):
                raise RuntimeError(
                    f"Refusing to save session {self.session_id!r} without member_id "
                    f"while multi-user mode is enabled"
                )
        except ImportError:
            logger.debug(
                "Session.save: member ownership stamp skipped (session_visibility unavailable)",
                exc_info=True,
            )
        if touch_updated_at:
        ...
        METADATA_FIELDS = [
            ...
            'pinned', 'archived', 'project_id', 'profile', 'member_id', 'team_id',
...
def _get_profile_home(profile) -> Path:
    """Resolve the intellect agent home directory for the given profile.

    Prefers the profile-specific helper from api.profiles; falls back to the
    INTELLECT_HOME environment variable or ~/.intellect, expanding ~ correctly.
    """
    try:
        from api.profiles import get_intellect_home_for_profile
        return Path(get_intellect_home_for_profile(profile))
    except ImportError:
        return Path(os.environ.get('INTELLECT_HOME') or '~/.intellect').expanduser()
...
def _enforce_request_session_access(session, sid) -> None:
    try:
        from api.session_visibility import enforce_session_access

        enforce_session_access(session, sid)
    except ImportError:
        pass
...
def get_session(sid, metadata_only=False):
    ...
    with LOCK:
        if sid in SESSIONS:
            ...
            cached = SESSIONS[sid]
            _enforce_request_session_access(cached, sid)
            return cached
    if metadata_only:
...
def _profile_default_model_state(profile=None):
    ...
    try:
        from api.profiles import get_intellect_home_for_profile
        config_path = Path(get_intellect_home_for_profile(profile)) / "config.yaml"
...
def _active_state_db_path() -> Path | None:
    """Return on-disk SQLite path for the active profile, or None when backend is PostgreSQL."""
    try:
        from api.storage_bridge import resolve_state_db_path
        ...
        try:
            ...
            intellect_home = Path(os.getenv('INTELLECT_HOME', str(HOME / '.intellect'))).expanduser().resolve()
        return intellect_home / 'state.db'


def _resolve_state_db_profile(profile) -> str | None:
...
def _state_messages_via_session_db(
    sid,
    *,
    profile=None,
    ...
    """Load messages through :func:`api.storage_bridge.get_session_db` (SQLite or PG)."""
    try:
    ...
    try:
        ...
        resolved_member = resolve_actor_member_id_for_state_db_reads(member_id)
        if resolved_member == "":
            return []
    ...
    prof = _resolve_state_db_profile(profile)
    db = get_session_db(prof)
    if db is None:
        return []

    try:
        session = db.get_session(str(sid), actor_member_id=member_id)
        if session is None:
            return []
        if member_id:
            owner = str(session.get("member_id") or "").strip() or None
            if owner is not None and owner != member_id:
                return []

        session_chain = [str(sid)]
        if stitch_continuations:
            rows_by_id = {str(sid): session}
            current_id = str(sid)
            seen = {current_id}
            for _ in range(20):
                ...
                rows_by_id[parent_id] = parent_dict
                if not _is_continuation_session(parent_dict, current):
                    break
                session_chain.insert(0, parent_id)
        ...
        msgs = []
        for chain_sid in session_chain:
            for row in db.get_messages(chain_sid):
                msg = {
                    "role": row.get("role"),
        ...
        logger.debug("PG state.db message read failed", exc_info=True)
        return []
    finally:
        try:
            db.close()
...
def _sidebar_title_is_generic_webui(title: str | None) -> bool:
    ...
    if text == 'Intellect WebUI':
        return True
    prefix = 'Intellect WebUI #'
...
def _enrich_sidebar_lineage_metadata(sessions: list[dict]) -> None:
    ...
    try:
        metadata = read_session_lineage_metadata_for_profile(
            _resolve_state_db_profile(None),
...
def _default_claude_code_projects_dir() -> Path | None:
    ...
    override = os.getenv('INTELLECT_WEBUI_CLAUDE_PROJECTS_DIR')
    if override:
        return Path(override).expanduser()
    if os.getenv('INTELLECT_WEBUI_TEST_STATE_DIR'):
        return None
...
def _resolve_cli_sessions_context():
    # Use the active WebUI profile's INTELLECT_HOME to find state.db.
    # The active profile is determined by what the user has selected in the UI
    # (stored in the server's runtime config). This means:
    #   - default profile  -> ~/.intellect/state.db
    #   - named profile X  -> ~/.intellect/profiles/X/state.db
    # We resolve the active profile's home directory rather than just using
    # INTELLECT_HOME (which is the server's launch profile, not necessarily the
    ...
    try:
        ...
        db_path = resolve_state_db_path(cli_profile)
        backend = resolve_storage_backend_name(cli_profile)
        storage_ok = state_storage_available(cli_profile)
    except Exception:
        db_path = intellect_home / 'state.db'
        backend = 'sqlite'
        storage_ok = db_path.is_file()

    projects_dir = _default_claude_code_projects_dir()
    try:
        from api.session_visibility import cli_sessions_scope_cache_token

        scope_token = cli_sessions_scope_cache_token()
    except ImportError:
        scope_token = "__no_scope__"
    cache_key = (
        str(intellect_home),
        str(cli_profile or ''),
        str(backend),
        str(db_path or ""),
        _sqlite_file_stat_cache_key(db_path) if backend == 'sqlite' else None,
        storage_ok,
        scope_token,
    ...
    return intellect_home, db_path, cli_profile, cache_key, storage_ok


def _load_cli_sessions_uncached(
    intellect_home: Path, db_path: Path, _cli_profile, *, storage_ok: bool = True,
) -> list:
    cli_sessions = []
    try:
    ...
    try:
        ...
        actor_member_id = resolve_actor_member_id_for_state_db_reads()
        if actor_member_id == "":
            return cli_sessions
    except ImportError:
        actor_member_id = None

    for row in read_importable_agent_session_rows_for_profile(
        _cli_profile,
        ...
        if not _title and _source == 'cron' and sid.startswith('cron_'):
            ...
            if len(parts) >= 3:
                _job_id = parts[1]
                try:
                    _jobs_path = intellect_home / 'cron' / 'jobs.json'
...
def get_cli_sessions() -> list:
    ...
    """
    intellect_home, db_path, cli_profile, cache_key, storage_ok = _resolve_cli_sessions_context()
        ...
        with _CLI_SESSIONS_CACHE_LOCK:
            ...
            try:
                sessions = _load_cli_sessions_uncached(
                    intellect_home, db_path, cli_profile, storage_ok=storage_ok,
                )
            except Exception as _cli_err:
                logger.warning(
                    "get_cli_sessions() failed — check state storage (%s): %s",
                    db_path, _cli_err,
                )
                return []
...
def get_state_db_session_messages(
    sid, *, stitch_continuations: bool = False, profile=None, member_id: str = None,
) -> list:
    """Read messages for an Intellect session from state.db.
    ...
    through another Intellect surface such as the Gateway API Server.  When
    ...
    When *member_id* is provided (multi-user mode), the session's owner is
    validated before reading messages — returns an empty list if the session
    belongs to a different member.
    """
    return _state_messages_via_session_db(
        sid,
        profile=profile,
        member_id=member_id,
...
def get_state_db_session_summary(sid, *, profile=None) -> dict:
    ...
    empty = {"message_count": 0, "last_message_at": 0.0}
    if not sid:
        return empty
        ...
        for row in rows:
            ...
            if ts is not None:
                try:
                    ...
                    pass
        return {"message_count": len(rows), "last_message_at": last_ts}
    except Exception:
        return empty
    finally:
        try:
...
def count_conversation_rounds(sid: str, since: float | None = None) -> int:
    ...
    db = get_session_db(get_active_profile_name())
    if db is None:
        return 0
    try:
        rows = db.get_messages(str(sid))
...
def delete_cli_session(sid) -> bool:
    ...
    try:
        ...
        if db is None:
            return False
        try:
            return bool(db.delete_session(str(sid)))
        finally:
            try:
                db.close()
            except Exception:
