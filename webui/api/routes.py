"""
Intellect Web UI -- Route handlers for GET and POST endpoints.
...
from api.agent_sessions import (
    ...
    read_session_lineage_report_for_profile,
)
...
# until the OS-level timeout fires. Force-import the authoritative tuple so
# downstream users (server.py, SSE sinks) don't need their own copy.
from api.helpers import _CLIENT_DISCONNECT_ERRORS  # noqa: F401 — re-export for server.py
...
def _active_skills_dir() -> Path:
    """Return the skills directory for the request's active Intellect profile.
    ...
    ``get_active_intellect_home()`` for every request instead of reading that
    process-global constant.
    """
    try:
        from api.profiles import get_active_intellect_home

        return Path(get_active_intellect_home()) / "skills"
    except Exception:
        try:
            ...
            return Path(os.getenv("INTELLECT_HOME", str(Path.home() / ".intellect"))).expanduser() / "skills"
...
def _skill_view_from_file(skill_dir: Path | None, skill_md: Path) -> dict:
    ...
    intellect_meta = metadata.get("intellect", {}) if isinstance(metadata, dict) else {}
    tags = _parse_tags(intellect_meta.get("tags") or frontmatter.get("tags", ""))
    related_skills = _parse_tags(
        intellect_meta.get("related_skills") or frontmatter.get("related_skills", "")
    )
    try:
...
def _gateway_session_metadata_path():
    try:
        from api.profiles import get_active_intellect_home
        intellect_home = Path(get_active_intellect_home()).expanduser().resolve()
    except Exception:
        intellect_home = Path(os.getenv("INTELLECT_HOME", str(Path.home() / ".intellect"))).expanduser().resolve()
    return intellect_home / "sessions" / "sessions.json"
...
def _profile_home_for_cron_job(job: dict):
    ...
    """
    from api.profiles import get_active_intellect_home, get_intellect_home_for_profile

    raw = str((job or {}).get("profile") or "").strip()
    if not raw:
        return get_active_intellect_home()
    if raw not in _available_cron_profile_names():
        ...
        return get_active_intellect_home()
    return get_intellect_home_for_profile(raw)
...
def _run_cron_job_in_profile_subprocess(job, execution_profile_home):
    ...
    cron.scheduler/cron.jobs still rely on process-global INTELLECT_HOME and module
    ...
    try:
        ...
        def _persist_success():
            ...
            try:
                ...
                # not expose the newer delivery_error parameter. Real Intellect
...
# Used as fallback when intellect_cli.provider_model_ids() is unavailable or
...
# specific model filtering happens downstream in intellect_cli.
#
...
def _allowed_public_origins() -> set[str]:
    """Parse INTELLECT_WEBUI_ALLOWED_ORIGINS env var (comma-separated) into a set.
    ...
    """
    raw = os.getenv('INTELLECT_WEBUI_ALLOWED_ORIGINS', '')
    result = set()
    for value in raw.split(','):
        ...
        if not (value.startswith('http://') or value.startswith('https://')):
            import sys
            print(
                f"[webui] WARNING: INTELLECT_WEBUI_ALLOWED_ORIGINS entry {value!r} is missing "
...
def _api_path_normalized(path: str) -> str:
    """Normalize API paths for routing (strip trailing slash)."""
    if not path or path == "/":
        return path or "/"
    return path.rstrip("/") or "/"


_CSRF_FAILURE_ATTR = "_intellect_csrf_failure_reason"
...
def _check_csrf(handler) -> bool:
    ...
    if not is_auth_enabled():
        return True
    # Member-authenticated users have an HttpOnly + SameSite=Lax cookie from the
    # member-auth flow.  That cookie provides built-in CSRF protection (cross-site
    # requests cannot attach it), so a separate webui-session CSRF token is not
    # required.  Without this bypass every POST/PUT/PATCH/DELETE from a member-
    # authenticated browser would fail with "Session expired".
    try:
        from api.auth import _multi_user_members_enabled
        from api.members import resolve_member_id
        from urllib.parse import urlparse as _urlparse
        if _multi_user_members_enabled() and resolve_member_id(handler, _urlparse(handler.path)):
            return True
    except ImportError:
        pass
    except Exception as exc:
        logger.warning("Member CSRF bypass check failed unexpectedly: %s", exc)
...
def _resolve_context_length_for_session_model(
    ...
    try:
        ...
        try:
            ...
            # Older intellect-agent builds: legacy 2-arg form.
...
def _should_hide_stale_messaging_session(
    ...
    Intellect Gateway keeps the external conversation identity in sessions.json.
...
def _session_attention_summary(session_id: str) -> dict | None:
    """Return attention metadata for a session, or None if nothing needs attention.

    Checks pending approvals first (critical), then pending clarify prompts
    (question). The frontend renders a coloured badge and hides the timestamp
    when an attention state is active.
    """
    if not session_id:
        return None
    # ── Pending approvals ──────────────────────────────────────────────────
    try:
        pending_approvals = resolve_gateway_approval(session_id)
    ...
    try:
        ...
        clarify_pending = pending_count(session_id)
        if clarify_pending:
            return {
                "kind": "clarify",
                "severity": "question",
...
def _approval_sse_notify_locked(
    session_id: str,
    head: dict | None,
    total: int,
    *,
    ...
    if publish_redis:
        try:
            from api.approval_events import publish_approval_changed

            publish_approval_changed(session_id, head, total)
        except Exception:
            logger.debug("Approval Redis publish failed", exc_info=True)
...
_LOGIN_LOCALE = {
    "en": {
        ...
        "member_title": "Member sign-in",
        "member_subtitle": "Use your organization account to access this Intellect profile.",
        "invite_label": "Invite code (optional)",
        "redeem_btn": "Redeem invite",
        "redeem_code_required": "Enter an invite code",
        "local_dev_title": "Local development",
        "local_dev_hint": "localhost only — member ID plus WebUI access password.",
        "member_id_placeholder": "Member id",
        "local_dev_btn": "Local sign in",
        "member_id_required": "Enter a member id",
        "continue_btn": "Continue",
        "signed_in_as": "Signed in as {id}",
        "no_oauth_providers": "No OAuth providers configured.",
        ...
        "register_oauth_hint": "Fill in member id, display name, and password, then pick a provider.",
        "or_divider": "or",
        "members_agent_unavailable": "Members are enabled in config.yaml but intellect-agent is unavailable. Restart WebUI with intellect-agent on PYTHONPATH.",
        "oauth_callback_register": "Register this callback URL in your OAuth app (e.g. Gitee):",
        "login_with_password_title": "Member sign-in",
        "login_with_password_hint": "Sign in with your member id and password.",
        "member_password_placeholder": "Password",
        "member_login_btn": "Sign in",
        "register_link": "Create an account",
        "login_tab_oauth": "OAuth",
        "login_tab_password": "Account",
        "login_tab_local": "Local dev",
        "register_title": "Create account",
        "register_subtitle": "Register with a local account, OAuth, or an invite code.",
        "register_tab_local": "Local account",
        "register_tab_oauth": "OAuth",
        "register_tab_invite": "Invite code",
        "register_local_hint": "Your account will be created after an administrator approves it.",
        "register_submitted_pending": "Registration submitted. Sign in after an admin approves your account.",
        "login_pending_approval": "Your account is pending admin approval.",
        "register_member_id": "Member id",
        "register_display_name": "Display name",
        "register_password": "Password",
        "register_password_confirm": "Confirm password",
        "register_invite_code": "Invite code",
        "register_submit": "Create account",
        "register_continue_oauth": "Continue with OAuth",
        "register_login_link": "Already have an account? Sign in",
        "register_member_id_taken": "Member id is already taken",
        "register_display_name_taken": "Display name is already taken",
    ...
    "zh": {
        ...
        "conn_failed": "\u8fde\u63a5\u5931\u8d25",
        "member_title": "\u6210\u5458\u767b\u5f55",
        "member_subtitle": "\u4f7f\u7528\u7ec4\u7ec7\u8d26\u53f7\u8bbf\u95ee\u6b64 Intellect \u914d\u7f6e\u3002",
        "invite_label": "\u9080\u8bf7\u7801\uff08\u53ef\u9009\uff09",
        "redeem_btn": "\u5151\u6362\u9080\u8bf7\u7801",
        "local_dev_title": "\u672c\u5730\u5f00\u53d1",
        "local_dev_hint": "\u4ec5 localhost \u2014 \u6210\u5458 ID + WebUI \u8bbf\u95ee\u5bc6\u7801\u3002",
        "member_id_placeholder": "\u6210\u5458 ID",
        "local_dev_btn": "\u672c\u5730\u767b\u5f55",
        "continue_btn": "\u7ee7\u7eed",
        "login_with_password_title": "\u6210\u5458\u767b\u5f55",
        "login_with_password_hint": "\u4f7f\u7528\u6210\u5458 ID \u548c\u5bc6\u7801\u767b\u5f55\u3002",
        "member_password_placeholder": "\u5bc6\u7801",
        "member_login_btn": "\u767b\u5f55",
        "register_link": "\u521b\u5efa\u8d26\u53f7",
        "login_tab_oauth": "\u7b2c\u4e09\u65b9\u767b\u5f55",
        "login_tab_password": "\u8d26\u53f7\u5bc6\u7801",
        "login_tab_local": "\u672c\u5730\u5f00\u53d1",
        "register_title": "\u521b\u5efa\u8d26\u53f7",
        "register_subtitle": "\u4f7f\u7528\u672c\u5730\u8d26\u53f7\u3001OAuth \u6216\u9080\u8bf7\u7801\u6ce8\u518c\u3002",
        "register_tab_local": "\u672c\u5730\u8d26\u53f7",
        "register_tab_oauth": "\u7b2c\u4e09\u65b9 OAuth",
        "register_tab_invite": "\u9080\u8bf7\u7801",
        "register_local_hint": "\u63d0\u4ea4\u540e\u9700\u7b49\u5f85\u7ba1\u7406\u5458\u5ba1\u6838\u901a\u8fc7\u540e\u624d\u53ef\u767b\u5f55\u3002",
        "register_submitted_pending": "\u6ce8\u518c\u5df2\u63d0\u4ea4\u3002\u7b49\u5f85\u7ba1\u7406\u5458\u5ba1\u6838\u901a\u8fc7\u540e\u518d\u767b\u5f55\u3002",
        "login_pending_approval": "\u8d26\u53f7\u5c1a\u672a\u901a\u8fc7\u7ba1\u7406\u5458\u5ba1\u6838\u3002",
        "no_oauth_providers": "\u672a\u914d\u7f6e OAuth \u63d0\u4f9b\u5546\u3002",
        ...
        "register_member_id": "\u6210\u5458 ID",
        "register_display_name": "\u663e\u793a\u540d\u79f0",
        "register_password": "\u5bc6\u7801",
        "register_password_confirm": "\u786e\u8ba4\u5bc6\u7801",
        "register_invite_code": "\u9080\u8bf7\u7801",
        "register_submit": "\u521b\u5efa\u8d26\u53f7",
        "register_continue_oauth": "\u7ee7\u7eed OAuth \u6ce8\u518c",
        "register_login_link": "\u5df2\u6709\u8d26\u53f7\uff1f\u53bb\u767b\u5f55",
        "register_member_id_taken": "\u6210\u5458 ID \u5df2\u88ab\u4f7f\u7528",
        "register_display_name_taken": "\u663e\u793a\u540d\u79f0\u5df2\u88ab\u4f7f\u7528",
    ...
    "zh-Hant": {
        ...
        "conn_failed": "\u9023\u63a5\u5931\u6557",
        "member_title": "\u6210\u54e1\u767b\u5165",
        "member_subtitle": "\u4f7f\u7528\u7d44\u7e54\u5e33\u865f\u5b58\u53d6\u6b64 Intellect \u914d\u7f6e\u3002",
        "invite_label": "\u9080\u8acb\u78bc\uff08\u9078\u586b\uff09",
        "redeem_btn": "\u514c\u63db\u9080\u8acb\u78bc",
        "local_dev_title": "\u672c\u5730\u958b\u767c",
        "member_id_placeholder": "\u6210\u54e1 ID",
        "local_dev_btn": "\u672c\u5730\u767b\u5165",
        "continue_btn": "\u7e7c\u7e8c",
    },
...
def _merged_login_strings(raw_lang: str | None) -> dict[str, str]:
    """Locale strings for /login; missing multi-user keys fall back to English."""
    key = _resolve_login_locale_key(raw_lang)
    merged = dict(_LOGIN_LOCALE["en"])
    merged.update(_LOGIN_LOCALE.get(key, {}))
    return merged
...
.card{background:#16213e;border:1px solid rgba(255,255,255,.08);border-radius:16px;padding:36px 32px;
  width:100%;max-width:400px;text-align:center;box-shadow:0 8px 32px rgba(0,0,0,.3)}
.logo{width:64px;height:64px;margin:0 auto 12px}
.logo img{width:100%;height:100%;border-radius:14px;object-fit:contain}
h1{font-size:18px;font-weight:600;margin-bottom:4px}
.sub{font-size:12px;color:#8888aa;margin-bottom:20px;line-height:1.45}
input{width:100%;padding:10px 14px;border-radius:10px;border:1px solid rgba(255,255,255,.1);
  background:rgba(255,255,255,.04);color:#e8e8f0;font-size:14px;outline:none;margin-bottom:10px;
...
button.oauth-btn{margin-bottom:8px}
button:disabled{opacity:.45;cursor:not-allowed}
.err{color:#e94560;font-size:12px;margin-top:10px;display:none;text-align:left}
.section{margin-bottom:16px;text-align:left}
.section-title{font-size:11px;text-transform:uppercase;letter-spacing:.06em;color:#8888aa;margin-bottom:8px}
.divider{height:1px;background:rgba(255,255,255,.08);margin:16px 0}
.hint{font-size:11px;color:#8888aa;margin-bottom:8px;line-height:1.4}
.continue-box{text-align:left;margin-bottom:16px;padding:12px;border-radius:10px;border:1px solid rgba(255,255,255,.1)}
.continue-box p{font-size:13px;margin-bottom:10px}
.hidden{display:none!important}
.oauth-provider-grid{display:flex;flex-wrap:wrap;gap:12px;justify-content:center;margin:8px 0 4px}
.oauth-provider-btn{display:inline-flex;align-items:center;justify-content:center;width:48px;height:48px;padding:0;border-radius:12px;border:1px solid rgba(255,255,255,.12);background:var(--oauth-brand-bg,#3d4663);cursor:pointer;transition:transform .12s,box-shadow .12s,border-color .12s;text-decoration:none;color:inherit}
a.oauth-provider-btn{box-sizing:border-box}
.oauth-provider-btn:hover{transform:translateY(-1px);border-color:rgba(124,185,255,.45);box-shadow:0 4px 14px rgba(0,0,0,.25)}
.oauth-provider-btn:focus-visible{outline:2px solid rgba(124,185,255,.7);outline-offset:2px}
.oauth-provider-icon,.oauth-provider-img{display:inline-flex;align-items:center;justify-content:center}
.oauth-provider-img{width:28px;height:28px;object-fit:contain}
.oauth-provider-hint{margin-bottom:8px}
.oauth-provider-empty{text-align:center}
.oauth-callback-details{margin-top:12px;text-align:left;font-size:11px;color:#8888aa}
.oauth-callback-details summary{cursor:pointer;color:#7cb9ff;user-select:none}
.tabs{display:flex;gap:8px;margin-bottom:14px}
.tab{flex:1;padding:8px 10px;border-radius:10px;border:1px solid rgba(255,255,255,.12);
  background:rgba(255,255,255,.03);color:#8888aa;font-size:13px;font-weight:600;cursor:pointer}
.tab.active{background:rgba(124,185,255,.15);border-color:rgba(124,185,255,.35);color:#7cb9ff}
.field-hint{font-size:11px;color:#8888aa;margin:-6px 0 10px;text-align:left}
.field-hint.err{color:#e94560}
.link-row{margin-top:14px;font-size:12px}
.link-row a{color:#7cb9ff;text-decoration:none}
.link-row a:hover{text-decoration:underline}
</style></head><body>
<div class="card">
  <div class="logo"><img src="{{STATIC_DIR}}/logo.png" alt="{{BOT_NAME}}"></div>
  <div id="login-continue" class="continue-box hidden">
    <p id="login-continue-text"></p>
    <button type="button" id="login-continue-btn">{{LOGIN_CONTINUE_BTN}}</button>
  </div>
  <div id="login-single" class="hidden">
    <h1>{{BOT_NAME}}</h1>
  ...
  <div id="login-multi" class="hidden">
    <h1 id="login-multi-title">{{LOGIN_MEMBER_TITLE}}</h1>
    <p class="sub" id="login-multi-sub">{{LOGIN_MEMBER_SUBTITLE}}</p>
    <div class="tabs" id="login-tabs">
      <button type="button" class="tab active" id="login-tab-password" data-tab="password">{{LOGIN_TAB_PASSWORD}}</button>
      <button type="button" class="tab" id="login-tab-oauth" data-tab="oauth">{{LOGIN_TAB_OAUTH}}</button>
      <button type="button" class="tab hidden" id="login-tab-local" data-tab="local">{{LOGIN_TAB_LOCAL}}</button>
    </div>
    <div id="login-panel-password" class="login-panel">
      <p class="hint">{{LOGIN_PASSWORD_HINT}}</p>
      <form id="login-member-form">
        <input type="text" id="login-member-id" placeholder="{{LOGIN_MEMBER_ID_PLACEHOLDER}}" autocomplete="username">
        <input type="password" id="login-member-pw" placeholder="{{LOGIN_MEMBER_PASSWORD_PLACEHOLDER}}" autocomplete="current-password">
        <button type="submit">{{LOGIN_MEMBER_BTN}}</button>
      </form>
    </div>
    <div id="login-panel-oauth" class="login-panel hidden">
      <div id="login-oauth-providers"></div>
    </div>
    <div id="login-panel-local" class="login-panel hidden">
      <p class="hint">{{LOGIN_LOCAL_DEV_HINT}}</p>
      <form id="login-local-form">
        <input type="text" id="login-local-member-id" placeholder="{{LOGIN_MEMBER_ID_PLACEHOLDER}}" autocomplete="username">
        <input type="password" id="login-local-pw" placeholder="{{LOGIN_PLACEHOLDER}}" autocomplete="current-password">
        <button type="submit">{{LOGIN_LOCAL_DEV_BTN}}</button>
      </form>
    </div>
    <p class="link-row"><a href="register">{{LOGIN_REGISTER_LINK}}</a></p>
  </div>
  <div class="err" id="err"></div>
</div>
<script>window.__LOGIN_I18N__={{LOGIN_I18N_JSON}};window.__LOGIN_CONTEXT__={{LOGIN_CONTEXT_JSON}};</script>
<script src="{{STATIC_DIR}}/member-oauth-providers.js?v={{WEBUI_VERSION}}"></script>
<script src="{{STATIC_DIR}}/login.js?v={{WEBUI_VERSION}}"></script>
</body></html>"""


_REGISTER_PAGE_HTML = """<!doctype html>
<html lang="{{LANG}}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>{{BOT_NAME}} — {{REGISTER_TITLE}}</title>
<style>
*{box-sizing:border-box;margin:0;padding:0}
body{background:#1a1a2e;color:#e8e8f0;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",system-ui,sans-serif;
  min-height:100vh;display:flex;align-items:center;justify-content:center;padding:24px}
.card{background:#16213e;border:1px solid rgba(255,255,255,.08);border-radius:16px;padding:36px 32px;
  width:100%;max-width:440px;text-align:center;box-shadow:0 8px 32px rgba(0,0,0,.3)}
.logo{width:64px;height:64px;margin:0 auto 12px}
.logo img{width:100%;height:100%;border-radius:14px;object-fit:contain}
h1{font-size:18px;font-weight:600;margin-bottom:4px}
.sub{font-size:12px;color:#8888aa;margin-bottom:20px;line-height:1.45}
...
<div class="card">
  <div class="logo"><img src="{{STATIC_DIR}}/logo.png" alt="{{BOT_NAME}}"></div>
  <div id="register-closed" class="hidden">
    <p class="sub">{{REGISTER_CLOSED}}</p>
    <p class="link-row"><a href="login">{{REGISTER_LOGIN_LINK}}</a></p>
  </div>
  <div id="register-open" class="hidden">
    <h1>{{REGISTER_TITLE}}</h1>
    <p class="sub">{{REGISTER_SUBTITLE}}</p>
    <div class="tabs">
      <button type="button" class="tab active" id="register-tab-local" data-tab="local">{{REGISTER_TAB_LOCAL}}</button>
      <button type="button" class="tab" id="register-tab-oauth" data-tab="oauth">{{REGISTER_TAB_OAUTH}}</button>
      <button type="button" class="tab" id="register-tab-invite" data-tab="invite">{{REGISTER_TAB_INVITE}}</button>
    </div>
    <form id="register-form" autocomplete="off">
      <div id="register-local-hint" class="hint hidden">{{REGISTER_LOCAL_HINT}}</div>
      <input type="text" id="register-display-name" placeholder="{{REGISTER_DISPLAY_NAME}}" autocomplete="nickname">
      <div class="field-hint" id="register-display-name-hint"></div>
      <input type="password" id="register-password" placeholder="{{REGISTER_PASSWORD}}" autocomplete="new-password">
      <input type="password" id="register-password-confirm" placeholder="{{REGISTER_PASSWORD_CONFIRM}}" autocomplete="new-password">
      <div id="register-invite-fields" class="hidden">
        <input type="text" id="register-invite-code" placeholder="{{REGISTER_INVITE_CODE}}" autocomplete="off">
      </div>
      <button type="submit" id="register-submit-btn">{{REGISTER_SUBMIT}}</button>
    </form>
    <div id="register-oauth-host" class="hidden"></div>
    <p class="link-row"><a href="login">{{REGISTER_LOGIN_LINK}}</a></p>
  </div>
  <div class="err" id="err"></div>
</div>
<script>window.__REGISTER_I18N__={{REGISTER_I18N_JSON}};window.__REGISTER_CONTEXT__={{REGISTER_CONTEXT_JSON}};</script>
<script src="{{STATIC_DIR}}/member-oauth-providers.js?v={{WEBUI_VERSION}}"></script>
<script src="{{STATIC_DIR}}/register.js?v={{WEBUI_VERSION}}"></script>
</body></html>"""
...
def _handle_logs(handler, parsed) -> bool:
    """Return a bounded tail window for an active-profile Intellect log file."""
    ...
    try:
        ...
        intellect_home = Path(get_active_intellect_home()).expanduser()
    except Exception:
        intellect_home = Path(os.environ.get("INTELLECT_HOME") or (Path.home() / ".intellect")).expanduser()

    log_dir = intellect_home / "logs"
...
_LLM_WIKI_DOCS_URL = "https://docs.ontoweb.cn/intellect-agent/skills/bundled/research/research-llm-wiki"
_LLM_WIKI_PAGE_DIRS = ("entities", "concepts", "comparisons", "queries")


def _llm_wiki_active_intellect_home() -> Path:
    try:
        from api.profiles import get_active_intellect_home
        return Path(get_active_intellect_home()).expanduser()
    except Exception:
        return Path(os.getenv("INTELLECT_HOME", str(Path.home() / ".intellect"))).expanduser()


def _llm_wiki_env_file_path(intellect_home: Path) -> str | None:
...
def _llm_wiki_agent_config() -> dict:
    try:
        from api.config import get_config as _get_cfg
        cfg = _get_cfg()
...
def _llm_wiki_env_override(member_id: str | None = None) -> str | None:
    from intellect_cli.wiki_scaffold import env_wiki_path_for_scoping

    scoped = env_wiki_path_for_scoping(
        _llm_wiki_agent_config(), member_id=member_id
    )
    if scoped:
        return scoped
    return _llm_wiki_env_file_path(_llm_wiki_active_intellect_home())


def _llm_wiki_tenant_context(handler, parsed) -> tuple[str | None, str | None, str | None]:
    member_id = None
    team_id = None
    project_id = None
    try:
        from api.members import resolve_member_id, resolve_team_id, resolve_project_id
        member_id = resolve_member_id(handler, parsed)
        if member_id:
    ...
    if qs.get("team_id"):
        team_id = qs["team_id"][0].strip() or None
    if qs.get("project_id"):
        project_id = qs["project_id"][0].strip() or None
    return member_id, team_id, project_id


def _llm_wiki_resolve_target(
    ...
    *,
    ...
    return resolve_wiki_target(
        intellect_home=_llm_wiki_active_intellect_home(),
        member_id=member_id,
        team_id=team_id,
        project_id=project_id,
        target_scope=target_scope,
        env_wiki_path=_llm_wiki_env_override(member_id=member_id),
        config=_llm_wiki_agent_config(),
    )


def _llm_wiki_safe_path_hint(wiki_path: Path, path_source: str) -> str:
    from intellect_cli.wiki_scaffold import redact_path_hint
    intellect_home = _llm_wiki_active_intellect_home()
    try:
        resolved = wiki_path.expanduser().resolve()
        if resolved.is_relative_to(intellect_home.expanduser().resolve()):
...
def _llm_wiki_wiki_settings() -> tuple[bool, bool]:
    cfg = _llm_wiki_agent_config()
    skills = cfg.get("skills") if isinstance(cfg.get("skills"), dict) else {}
    config_section = skills.get("config") if isinstance(skills.get("config"), dict) else {}
    wiki_cfg = config_section.get("wiki") if isinstance(config_section.get("wiki"), dict) else {}
    has_config = bool(wiki_cfg) or bool(_llm_wiki_config_path()) or bool(_llm_wiki_env_override())
    enabled = bool(wiki_cfg.get("enabled", True)) if wiki_cfg else has_config
...
def _llm_wiki_resolve_path() -> tuple[Path, str, bool]:
    """Legacy wrapper — calls scoped resolver with no tenant context."""
    path, source, _scope, _scope_id, configured = _llm_wiki_resolve_scoped_path()
    return path, source, configured


def _llm_wiki_resolve_scoped_path(
    member_id: str | None = None,
    team_id: str | None = None,
    project_id: str | None = None,
) -> tuple[Path, str, str, str | None, bool]:
    """Resolve the LLM Wiki path with tenant scoping.

    Resolution order:
      1. WIKI_PATH env var                     — explicit global override (backward compat)
      2. Project-scoped: projects/{slug}/wiki/  — when project_id is set and dir exists
      3. Team-scoped:    teams/{slug}/wiki/     — when team_id is set and dir exists
      4. Member-scoped:  members/{id}/wiki/     — when member_id is set and dir exists
      5. .env / config.yaml / ~/wiki            — existing fallback

    Returns:
      (wiki_path, source, scope, scope_id, configured)
      scope is one of: "project", "team", "member", "global"
    """
    intellect_home = _llm_wiki_active_intellect_home()

    # 1. Explicit WIKI_PATH override (highest priority, backward compat)
    env_override = os.getenv("WIKI_PATH")
    if env_override:
        return Path(os.path.expandvars(env_override)).expanduser(), "WIKI_PATH", "global", None, True

    # 2. Project-scoped wiki
    if project_id and _llm_wiki_safe_slug(project_id):
        project_wiki = intellect_home / "projects" / project_id / "wiki"
        if project_wiki.exists() and project_wiki.is_dir():
            return project_wiki, "project", "project", project_id, True

    # 3. Team-scoped wiki
    if team_id and _llm_wiki_safe_slug(team_id):
        team_wiki = intellect_home / "teams" / team_id / "wiki"
        if team_wiki.exists() and team_wiki.is_dir():
            return team_wiki, "team", "team", team_id, True

    # 4. Member-scoped wiki
    if member_id and _llm_wiki_safe_slug(member_id):
        member_wiki = intellect_home / "members" / member_id / "wiki"
        if member_wiki.exists() and member_wiki.is_dir():
            return member_wiki, "member", "member", member_id, True

    # 5. Global fallback (existing behavior)
    raw = _llm_wiki_env_file_path(intellect_home)
    ...
    if not raw:
        raw = "~/wiki"
    return Path(os.path.expandvars(raw)).expanduser(), source, "global", None, configured


def _llm_wiki_safe_slug(value: str) -> bool:
    """Basic safety check: reject slugs containing path traversal or suspicious chars."""
...
def _build_llm_wiki_status(
    ...
    *,
    ...
    from intellect_cli.wiki_scaffold import is_forbidden_path
    try:
        target = _llm_wiki_resolve_target(
            member_id=member_id,
            team_id=team_id,
            project_id=project_id,
            target_scope=target_scope,
        )
        wiki_path = target.path
        path_source = target.path_source
        scope = target.scope
        scope_id = target.scope_id
        path_configured = path_source not in ("default",)
        enabled, has_config = _llm_wiki_wiki_settings()
        path_hint = _llm_wiki_safe_path_hint(wiki_path, path_source)
        init_block_reason = None
        init_available = False
        if is_forbidden_path(wiki_path):
            init_block_reason = "forbidden_path"
        elif not wiki_path.exists():
            init_available = True
        elif not wiki_path.is_dir():
            init_block_reason = "not_directory"
        elif (wiki_path / "SCHEMA.md").exists() or (wiki_path / "index.md").exists():
            init_block_reason = "wiki_already_exists"
        else:
            init_available = True
        ...
        base = {
            ...
            "init_available": init_available,
            "init_block_reason": init_block_reason,
            "has_config": has_config,
            "toggle_available": has_config,
            "toggle_reason": None if has_config else "No wiki config section found in config.yaml.",
        ...
        base.update({
            ...
            "init_available": False,
            "init_block_reason": "wiki_already_exists" if init_block_reason == "wiki_already_exists" else None,
        })
        return base
    except Exception as exc:
        return {
...
def _handle_llm_wiki_status(handler, parsed) -> bool:
    """Return LLM Wiki status with tenant-scoped path resolution."""
    member_id, team_id, project_id = _llm_wiki_tenant_context(handler, parsed)
...
def _vault_url_for_scope(scope: str, scope_id: str | None) -> str:
    if scope in _VAULT_SCOPE_DIRS and scope_id:
        return f"/vault/{scope[0]}/{scope_id}/"
    return "/vault/"


def _llm_wiki_members_enabled() -> bool:
    try:
        from agent.membership import is_members_enabled

        return bool(is_members_enabled(_llm_wiki_agent_config()))
    except Exception:
        return False


def _llm_wiki_tenant_context_safe(handler, parsed_url) -> tuple[str | None, str | None, str | None]:
    """Resolve tenant context, returning all None when members are not enabled."""
    member_id, team_id, project_id = _llm_wiki_tenant_context(handler, parsed_url)
    if not _llm_wiki_members_enabled():
        return None, None, None
    return member_id, team_id, project_id


def _vault_intellect_home() -> Path:
    """Return INTELLECT_HOME for vault paths (delegates to canonical resolver)."""
    return _llm_wiki_active_intellect_home()


def _vault_global_output_dir() -> Path:
    return _vault_intellect_home() / "vaults" / "global"


def _vault_static_bundle_dir() -> Path:
    return Path(__file__).parent.parent / "static" / "vault"


def _resolve_global_vault_serve_dir() -> Path:
    """Prefer user-built vaults/global; fall back to bundled static/vault demo."""
    user_global = _vault_global_output_dir()
    if (user_global / "index.html").is_file():
        return user_global
    static = _vault_static_bundle_dir()
    if static.exists():
        return static
    return user_global


def _vault_built_status(scope: str, scope_id: str | None) -> tuple[bool, str | None]:
    """Return whether vault index.html exists and its mtime as ISO."""
    if scope == "global" or not scope_id:
        vault_dir = _resolve_global_vault_serve_dir()
    else:
        vault_dir = _vault_dir_for_scope(scope, scope_id)
    index = vault_dir / "index.html"
    if index.is_file():
        return True, _llm_wiki_safe_iso(index.stat().st_mtime)
    return False, None


def _latest_build_job(scope: str, scope_id: str | None) -> dict | None:
    job_key = _vault_build_job_key(scope, scope_id)
    with _VAULT_BUILD_LOCK:
        recent = None
        for job in _VAULT_BUILD_JOBS.values():
            if not isinstance(job, dict) or job.get("job_key") != job_key:
                continue
            if recent is None or (job.get("started_at") or 0) > (recent.get("started_at") or 0):
                recent = job
    return recent


def _estimate_build_progress(job: dict) -> int:
    ...
    if status == "running":
        ...
        if started:
            ...
            timeout = int(_vault_config().get("build_timeout_seconds", 300))
            pct = 15 + int(min(elapsed / max(timeout, 1), 1.0) * 75)
            return min(pct, 90)
        return 15
    return 0


def _wiki_catalog_badge(wiki_status: str, vault_built: bool, build_status: str | None) -> str:
...
def _scope_ids_for_wiki_status(scope: str, scope_id: str | None) -> tuple[str | None, str | None, str | None]:
    member_id = team_id = project_id = None
    if scope == "member":
        member_id = scope_id
    elif scope == "team":
        team_id = scope_id
...
def _build_wiki_global_catalog_entry(handler, actor: str | None, members_on: bool) -> dict:
    from intellect_cli.wiki_scaffold import global_wiki_dir, wiki_write_mode
    ...
    if actor and members_on:
        try:
            ...
            from api.members import _member_authorize, _store
...
def _build_wiki_catalog_entry(scope: str, scope_id: str | None, label: str) -> dict:
    if scope == "global":
        members_on = _llm_wiki_members_enabled()
        # Single-user personal wiki is also labeled scope=global but lives at ~/wiki
        # (or WIKI_PATH). Organization wiki/global/ is multi-user only.
        if members_on:
            full_wiki = _build_llm_wiki_status(target_scope="global")
        else:
            full_wiki = _build_llm_wiki_status()
    else:
        member_id, team_id, project_id = _scope_ids_for_wiki_status(scope, scope_id)
        full_wiki = _build_llm_wiki_status(
            member_id=member_id, team_id=team_id, project_id=project_id
        )
    effective_scope = scope or full_wiki.get("scope") or "global"
    effective_id = scope_id if scope_id is not None else full_wiki.get("scope_id")
    wiki_summary = {
        "status": full_wiki.get("status"),
        "page_count": full_wiki.get("page_count", 0),
        "init_available": bool(full_wiki.get("init_available")),
        "target_path_hint": full_wiki.get("target_path_hint"),
    }
    built, last_built_at = _vault_built_status(effective_scope, effective_id)
    recent = _latest_build_job(effective_scope, effective_id)
    build_status = recent.get("status") if recent else None
    build_block: dict = {
        "status": build_status if recent else "idle",
        "job_id": recent.get("job_id") if recent else None,
    }
    if recent and build_status in ("queued", "running", "failed"):
        build_block["error"] = recent.get("error")
    badge = _wiki_catalog_badge(
        str(wiki_summary.get("status") or "missing"),
        built,
        build_status,
    )
    return {
        ...
        "vault": {
            ...
            "url": _vault_url_for_scope(effective_scope, effective_id),
            "build": build_block,
        },
    }


def _handle_wiki_catalog(handler, parsed) -> bool:
    """Return wiki/vault catalog for the current member (sidebar data source)."""
    from urllib.parse import parse_qs

    from api.members import _load_config, _resolve_actor_display_name, _store, resolve_member_id

    qs = parse_qs(parsed.query or "")
    include_empty = qs.get("include_empty", ["1"])[0].strip() != "0"
    actor = resolve_member_id(handler, parsed)
    config = _load_config()
    teams_list: list[dict] = []
    projects_list: list[dict] = []
    members_on = teams_on = projects_on = False
    try:
        from agent.membership import is_members_enabled, is_projects_enabled, is_teams_enabled

        members_on = is_members_enabled(config)
    ...
    if actor and members_on:
        personal = _build_wiki_catalog_entry(
            "member", actor, _resolve_actor_display_name(actor)
        )
    else:
        target = _llm_wiki_resolve_target()
        personal = _build_wiki_catalog_entry(
            target.scope, target.scope_id, "My Wiki"
        )

    if actor and teams_on:
        store = _store()
        try:
            for row in store.list_teams_brief(actor):
                tid = row.get("team_id") or row.get("id")
                if not tid or not _vault_access_check(actor, "team", str(tid)):
                    continue
                entry = _build_wiki_catalog_entry(
                    "team", str(tid), row.get("display_name") or str(tid)
                )
                if include_empty or entry.get("badge") != "missing":
                    teams_list.append(entry)
        finally:
            store.close()

    if actor and projects_on:
        store = _store()
        try:
            for row in store.list_projects_brief(actor):
                pid = row.get("project_id") or row.get("id")
                if not pid or not _vault_access_check(actor, "project", str(pid)):
                    continue
                entry = _build_wiki_catalog_entry(
                    "project", str(pid), row.get("display_name") or row.get("name") or str(pid)
                )
                if include_empty or entry.get("badge") != "missing":
    ...
    global_entry = _build_wiki_global_catalog_entry(handler, actor, members_on)
    j(handler, {
        "ok": True,
        "personal": personal,
        "teams": teams_list,
...
# Mapping from logical scope name to filesystem directory component.
_VAULT_SCOPE_DIRS: dict[str, str] = {
    "project": "projects",
    "team": "teams",
    "member": "members",
}
...
_VAULT_DEFAULTS = {
    "routing": "context",             # "context" | "explicit"
    "build_trigger": "auto",          # "auto" | "manual" | "scheduled"
    "build_schedule": "0 * * * *",    # cron (only when build_trigger=scheduled)
    "build_throttle_seconds": 600,    # min interval between builds (10 min)
    "build_timeout_seconds": 300,     # max build duration (5 min)
}


def _vault_config() -> dict:
    """Return vault configuration with defaults merged."""
    cfg = {}
    try:
        from api.config import get_config as _get_cfg
        agent_cfg = _get_cfg()
...
def _vault_dir_for_scope(scope: str, scope_id: str | None) -> Path:
    """Resolve vault build output directory for a given scope."""
    home = _vault_intellect_home()
    if scope in _VAULT_SCOPE_DIRS and scope_id:
        dir_key = _VAULT_SCOPE_DIRS[scope]
        return home / "vaults" / dir_key / scope_id
    return _vault_global_output_dir()


# ── Vault build management ────────────────────────────────────────────────

_VAULT_BUILD_JOBS: dict[str, dict] = {}
_VAULT_BUILD_LOCK = threading.Lock()
_VAULT_BUILD_LAST_RUN: dict[str, float] = {}  # scope:scope_id → last build timestamp


def _vault_build_job_key(scope: str, scope_id: str | None) -> str:
    return f"{scope}:{scope_id or 'global'}"


def _vault_resolve_wiki_path_for_build(
    handler,
    scope: str,
    scope_id: str | None,
    ...
    members_on = _llm_wiki_members_enabled()
    home = _llm_wiki_active_intellect_home()

    if scope == "global":
        if members_on:
            from intellect_cli.wiki_scaffold import global_wiki_dir

            return global_wiki_dir(home)
        return _llm_wiki_resolve_target(
            member_id=None, team_id=None, project_id=None
        ).path

    if scope in _VAULT_SCOPE_DIRS and scope_id:
        dir_key = _VAULT_SCOPE_DIRS[scope]
        return home / dir_key / scope_id / "wiki"

    # Fallback: resolve tenant context for scoped path resolution
    from urllib.parse import urlsplit

    member_id, team_id, project_id = _llm_wiki_tenant_context_safe(
        handler, urlsplit(handler.path)
    )
    return _llm_wiki_resolve_target(
        member_id=member_id, team_id=team_id, project_id=project_id
    ).path
...
def _queue_vault_build(handler, body, *, wiki_path: Path | None = None) -> dict:
    """Queue a Quartz vault build. Returns a JSON-serializable payload."""
    scope = body.get("scope", "") if body else ""
    scope_id = body.get("scope_id") if body else None
    ...
    job_key = _vault_build_job_key(scope, scope_id)
    vcfg = _vault_config()
    throttle = int(vcfg.get("build_throttle_seconds", 600))
    with _VAULT_BUILD_LOCK:
        last = _VAULT_BUILD_LAST_RUN.get(job_key, 0)
        if time.time() - last < throttle:
            remaining = int(throttle - (time.time() - last))
            return {
                "ok": False,
                "queued": False,
                "error": f"Build throttled. Next build available in {remaining}s.",
                "job_id": None,
                "status": "throttled",
            }

    with _VAULT_BUILD_LOCK:
        for existing_job in _VAULT_BUILD_JOBS.values():
            if existing_job.get("job_key") == job_key and existing_job.get("status") == "running":
                return {
                    "ok": False,
                    "queued": False,
                    "error": "A build is already in progress for this vault.",
                    "job_id": existing_job.get("job_id"),
                    "status": "running",
                }

    resolved_wiki = _vault_resolve_wiki_path_for_build(handler, scope, scope_id, wiki_path)
    vault_output = _vault_dir_for_scope(scope, scope_id)

    build_host = "127.0.0.1:9119"
    try:
    ...
    with _VAULT_BUILD_LOCK:
        _VAULT_BUILD_JOBS[job_id] = job

    def _run_build():
        job["status"] = "running"
        job["started_at"] = time.time()
        with _VAULT_BUILD_LOCK:
            _VAULT_BUILD_LAST_RUN[job_key] = time.time()
        try:
            quartz_dir = Path(__file__).parent.parent / "quartz"
            if sys.platform == "win32":
                script = quartz_dir / "build.ps1"
                script_cmd = ["powershell", "-NoProfile", "-NonInteractive", "-File", str(script)]
            else:
                script = quartz_dir / "build.sh"
                script_cmd = ["bash", str(script)]

            if not script.exists():
                job["status"] = "failed"
                job["error"] = f"Build script not found: {script}"
            ...
            base_path = "/vault"
            if scope in _VAULT_SCOPE_DIRS and scope_id:
                base_path = f"/vault/{scope[0]}/{scope_id}"

            timeout = int(vcfg.get("build_timeout_seconds", 300))
            build_env = os.environ.copy()
            build_env["QUARTZ_BASE_HOST"] = build_host
            build_args = script_cmd + [str(resolved_wiki), str(vault_output), title, base_path]
            result = subprocess.run(
                build_args,
                capture_output=True, text=True, timeout=timeout, env=build_env,
            )
            if result.returncode == 0:
                ...
                job["error"] = err_text[:500] if err_text else f"Exit code: {result.returncode}"
        except subprocess.TimeoutExpired:
            job["status"] = "failed"
            job["error"] = f"Build timed out after {timeout}s"
        except Exception as e:
            job["status"] = "failed"
            job["error"] = str(e)[:500]
        job["finished_at"] = time.time()
    ...
    return {"ok": True, "queued": True, "job_id": job_id, "status": "queued"}


def _handle_vault_build(handler, body) -> bool:
    """Trigger a Quartz vault build for the specified scope."""
    return j(handler, _queue_vault_build(handler, body))


def _handle_vault_build_status(handler, parsed) -> bool:
    """Query build job status."""
    from urllib.parse import parse_qs
    qs = parse_qs(parsed.query or "")
    ...
    if job_id:
        with _VAULT_BUILD_LOCK:
            job = _VAULT_BUILD_JOBS.get(job_id)
        if not job:
            return j(handler, {"error": "Job not found"}, status=404)
        return j(handler, {
            "job_id": job.get("job_id") if isinstance(job, dict) else job_id,
            "status": job.get("status") if isinstance(job, dict) else "unknown",
            "scope": job.get("scope") if isinstance(job, dict) else None,
            "scope_id": job.get("scope_id") if isinstance(job, dict) else None,
            "started_at": _llm_wiki_safe_iso(job.get("started_at")) if isinstance(job, dict) else None,
            "finished_at": _llm_wiki_safe_iso(job.get("finished_at")) if isinstance(job, dict) else None,
            "error": job.get("error") if isinstance(job, dict) else None,
        })

    # No job_id — return latest build status per scope
    scope = qs.get("scope", ["global"])[0]
    scope_id = qs.get("scope_id", [None])[0]
    job_key = _vault_build_job_key(scope, scope_id)
    with _VAULT_BUILD_LOCK:
        last_ts = _VAULT_BUILD_LAST_RUN.get(job_key, 0)
    recent = _latest_build_job(scope, scope_id)
    vault_built, vault_mtime = _vault_built_status(scope, scope_id)
    last_built_at = _llm_wiki_safe_iso(last_ts) if last_ts else vault_mtime
    payload: dict = {
        "last_built_at": last_built_at,
        "vault_built": vault_built,
        "current_job": None,
    }
    if recent:
        status = recent.get("status")
        payload["current_job"] = {
            "job_id": recent.get("job_id"),
            "status": status,
            "started_at": _llm_wiki_safe_iso(recent.get("started_at")),
            "finished_at": _llm_wiki_safe_iso(recent.get("finished_at")),
            "error": recent.get("error"),
            "progress_phase": "quartz_build" if status == "running" else None,
            "progress_pct": _estimate_build_progress(recent),
        }
...
def _vault_resolve_static_file(vault_dir: Path, file_path: str) -> Path | None:
    """Resolve a Quartz pretty URL to an on-disk file (adds .html / index.html)."""
...
def _handle_vault_serve(handler, parsed) -> bool:
    """Serve Quartz vault static files with tenant-aware routing.

    Routing modes (configured via vault.routing in config.yaml):
      "context" (default) — /vault/ auto-selects vault based on user context
      "explicit"           — /vault/ only serves global vault
    """
    vcfg = _vault_config()
    routing = vcfg.get("routing", "context")
    path = parsed.path

    # Parse vault path: /vault[/{type}/{id}][/path...]
    #   /vault/              → global or context-redirect
    #   /vault/p/{slug}/...  → project vault
    #   /vault/t/{slug}/...  → team vault
    #   /vault/m/{id}/...    → member vault
    #   /vault/g/...         → explicit global vault

    rest = path[6:] if path.startswith("/vault") else ""  # strip "/vault"
    if rest.startswith("/"):
        rest = rest[1:]

    # Reverse mapping: url prefix char → scope type
    _VAULT_URL_PREFIX_TO_SCOPE = {s[0]: s for s in _VAULT_SCOPE_DIRS}
    ...
    # p/{id}/..., t/{id}/..., m/{id}/...
    if len(rest) > 2 and rest[1:2] == "/":
        prefix = rest[0:1]
        if prefix in _VAULT_URL_PREFIX_TO_SCOPE:
            vault_type = _VAULT_URL_PREFIX_TO_SCOPE[prefix]
            parts = rest[2:].split("/", 1)
            vault_id = parts[0]
            file_path = parts[1] if len(parts) > 1 else ""
    elif rest.startswith("g/") and len(rest) > 2:
        vault_type = "global"
        file_path = rest[2:]
    elif rest == "" or rest == "g":
        # /vault/ or /vault/g — context redirect (multi-user only) or global
        if routing == "context" and _llm_wiki_members_enabled():
            try:
                from api.members import resolve_member_id, resolve_team_id, resolve_project_id
                from urllib.parse import urlsplit
                parsed_url = urlsplit(handler.path)
                member_id = resolve_member_id(handler, parsed_url)
                if member_id:
                    project_id = resolve_project_id(handler, parsed_url, member_id=member_id)
                    if project_id:
                        from api.helpers import redirect
                        redirect(handler, f"/vault/p/{project_id}/")
                        return True
                    team_id = resolve_team_id(handler, parsed_url, member_id=member_id)
                    if team_id:
                        from api.helpers import redirect
    ...
    if vault_type == "global":
        ...
        vault_dir = _vault_dir_for_scope(vault_type, vault_id)
        if not vault_dir.exists():
            # Fall back to global vault (built or bundled demo)
            fallback = _resolve_global_vault_serve_dir()
            if fallback.exists():
                vault_dir = fallback
                vault_type = "global"
                vault_id = None
            else:
                return j(handler, {"error": "Vault not found"}, status=404)

    # Access control for non-global vaults
    if vault_type != "global":
        try:
            from api.members import resolve_member_id
            from urllib.parse import urlsplit
            actor = resolve_member_id(handler, urlsplit(handler.path))
    ...
    if target is None:
        # 404 fallback: serve vault's 404.html if available
        fallback = vault_dir / "404.html"
...
def _vault_access_check(member_id: str, vault_type: str, vault_id: str) -> bool:
    """Check if a member has access to a scoped vault."""
    if vault_type == "global":
        return True
    if vault_type == "member":
        return member_id == vault_id
    try:
        from api.members import MembershipStore
        store = MembershipStore()
        if vault_type == "team":
            return store.is_team_member(vault_id, member_id)
...
def _handle_analytics(handler, parsed) -> bool:
    """Return skill lifecycle analytics from the agent (P6-1)."""
    try:
...
def _handle_wiki_toggle(handler, body) -> bool:
    """Toggle LLM Wiki enabled/disabled for the active profile (P3-9)."""
    enabled = bool(body.get("enabled", False)) if body else False
    try:
        from api.members import _load_config as _wiki_cfg_load
        from intellect_cli.config import save_config as _wiki_cfg_save
        cfg = _wiki_cfg_load()
        skills = cfg.get("skills") if isinstance(cfg.get("skills"), dict) else {}
        config_section = skills.get("config") if isinstance(skills.get("config"), dict) else {}
        wiki_cfg = config_section.get("wiki") if isinstance(config_section.get("wiki"), dict) else {}
        wiki_cfg["enabled"] = enabled
        config_section["wiki"] = wiki_cfg
        skills["config"] = config_section
        cfg["skills"] = skills
        _wiki_cfg_save(cfg)
        return j(handler, {"enabled": enabled, "ok": True})
    except Exception as exc:
        return j(handler, {"enabled": enabled, "ok": False, "error": str(exc)}, status=500)


def _handle_wiki_init(handler, body) -> bool:
    """Initialize the scoped LLM Wiki directory scaffold."""
    from intellect_cli.wiki_scaffold import init_wiki, resolve_wiki_target
    from urllib.parse import urlsplit

    member_id, team_id, project_id = _llm_wiki_tenant_context(handler, urlsplit(handler.path))
    domain = str((body or {}).get("domain") or "General knowledge").strip() or "General knowledge"
    trigger_build = bool((body or {}).get("trigger_build", True))
    req_scope = str((body or {}).get("scope") or "").strip().lower()

    members_on = _llm_wiki_members_enabled()

    # scope=global in the request body means Organization wiki (wiki/global/) only when
    # multi-user mode is on. In single-user mode the catalog still labels personal wiki
    # as scope "global" (legacy ~/wiki path) — do not require admin or use wiki/global/.
    if req_scope == "global" and members_on:
        try:
            from agent.membership import Action
            from api.members import _member_authorize, _store
            ...
            store = _store()
            try:
                if not member_id or not _member_authorize(store, member_id, Action.ADMIN):
                    return j(handler, {"ok": False, "error": "admin required for global wiki init"}, status=403)
            finally:
                store.close()
        except Exception:
            return j(handler, {"ok": False, "error": "admin required"}, status=403)
    ...
    result = init_wiki(target.path, domain=domain)
    if not result.ok:
        status = 403 if result.error_code == "forbidden_path" else 400
        return j(handler, {
            "ok": False,
            "error": result.error,
    ...
    build_payload: dict = {"queued": False}
    vcfg = _vault_config()
    if trigger_build and vcfg.get("build_trigger") != "manual":
        build_body = {"scope": target.scope, "scope_id": target.scope_id}
        build_resp = _queue_vault_build(handler, build_body, wiki_path=target.path)
        if isinstance(build_resp, dict):
            build_payload = build_resp
    elif trigger_build:
        build_payload = {"queued": False, "build_skipped_reason": "manual_build_trigger"}
    ...
    return j(handler, {
        ...
        "scope": target.scope,
        "scope_id": target.scope_id,
        "path_hint": _llm_wiki_safe_path_hint(target.path, target.path_source),
...
def _handle_vault_config_get(handler, parsed) -> bool:
    """GET /api/vault/config — return merged vault configuration."""
    from intellect_cli.vault_build import get_scheduler_status

    merged = _vault_config()
    raw_keys: set[str] = set()
    try:
        from api.config import get_config as _get_cfg
        agent_cfg = _get_cfg()
        if isinstance(agent_cfg, dict):
            vault_cfg = agent_cfg.get("vault")
    ...
    j(handler, {
        **merged,
        ...
        "scheduler": scheduler,
        "members_enabled": members_on,
        "routing_available": members_on,
        "ok": True,
    })
    return True


def _handle_vault_config_save(handler, body) -> bool:
    """POST/PATCH /api/vault/config — persist vault routing / build trigger settings."""
    if not body:
        j(handler, {"ok": False, "error": "Missing request body"}, status=400)
        return True

    allowed = {"routing", "build_trigger", "build_schedule"}
    updates = {k: body[k] for k in allowed if k in body}
    if not updates:
        j(handler, {"ok": False, "error": "No supported vault fields in body"}, status=400)
        return True
    ...
    if "routing" in updates:
        if not _llm_wiki_members_enabled():
            j(handler, {
                "ok": False,
                "error": "Vault URL routing applies only when multi-user mode is enabled",
            }, status=400)
            return True
        routing = str(updates["routing"]).strip().lower()
        if routing not in ("context", "explicit"):
            j(handler, {"ok": False, "error": "routing must be 'context' or 'explicit'"}, status=400)
            return True
        updates["routing"] = routing

    if "build_trigger" in updates:
        trigger = str(updates["build_trigger"]).strip().lower()
        if trigger not in ("auto", "manual", "scheduled"):
            j(handler, {"ok": False, "error": "build_trigger must be auto, manual, or scheduled"}, status=400)
            return True
        updates["build_trigger"] = trigger

    if "build_schedule" in updates:
        schedule = str(updates["build_schedule"]).strip()
        try:
            from intellect_cli.vault_build import validate_cron_schedule
            validate_cron_schedule(schedule)
        except ValueError as exc:
            j(handler, {"ok": False, "error": str(exc)}, status=400)
            return True
        updates["build_schedule"] = schedule

    if updates.get("build_trigger") == "scheduled" and "build_schedule" not in updates:
        current = _vault_config()
        if not str(current.get("build_schedule", "")).strip():
            j(handler, {"ok": False, "error": "build_schedule required when build_trigger is scheduled"}, status=400)
            return True

    try:
        from api.members import _load_config as _vcfg_load
        from intellect_cli.config import save_config as _vcfg_save
        ...
        j(handler, {
            "ok": True,
            **updates,
            "scheduler": get_scheduler_status(_vault_config(), _llm_wiki_active_intellect_home()),
        })
    except Exception as exc:
        j(handler, {"ok": False, "error": str(exc)}, status=500)
    return True


def _handle_vault_tick(handler, body) -> bool:
    """POST /api/vault/tick — run one scheduled vault build tick."""
    from intellect_cli.vault_build import run_scheduled_vault_tick

    force = bool((body or {}).get("force", False))
    result = run_scheduled_vault_tick(_vault_config(), intellect_home=_llm_wiki_active_intellect_home(), force=force)
    return j(handler, {
        "ok": result.ok,
        "ran": result.ran,
...
def _handle_search(handler, parsed) -> bool:
    """Proxy a web search query to a configured SearXNG instance (P3-4)."""
    from api.search import get_searxng_config, search_searxng
    from api.members import _load_config
    from urllib.parse import parse_qs

    cfg = _load_config()
    searxng = get_searxng_config(cfg)
    if not searxng["enabled"] or not searxng["url"]:
        return j(handler, {"error": "SearXNG search is not enabled. Set features.searxng_enabled and features.searxng_url in config.yaml."}, status=400)

    query = parse_qs(parsed.query).get("q", [""])[0].strip()
    if not query:
        return j(handler, {"error": "Missing ?q= parameter"}, status=400)

    import asyncio
    loop = asyncio.new_event_loop()
    try:
        results = loop.run_until_complete(search_searxng(query, searxng["url"]))
    finally:
        loop.close()
    return j(handler, results)


def _handle_search_probe(handler, parsed) -> bool:
    """Test connectivity to the configured SearXNG instance (P3-4)."""
    from api.search import get_searxng_config, probe_searxng
    from api.members import _load_config

    cfg = _load_config()
    searxng = get_searxng_config(cfg)
    if not searxng["url"]:
        return j(handler, {"ok": False, "error": "No SearXNG URL configured"}, status=400)

    import asyncio
    loop = asyncio.new_event_loop()
    try:
        status = loop.run_until_complete(probe_searxng(searxng["url"]))
    finally:
        loop.close()
...
def _handle_search_config_get(handler, parsed) -> bool:
    """GET /api/search/config — return current search configuration."""
    from api.search import get_searxng_config
    from api.members import _load_config
    cfg = _load_config()
    return j(handler, get_searxng_config(cfg))


def _handle_search_config_save(handler, body) -> bool:
    """POST /api/search/config — persist search configuration."""
    try:
        from api.members import _load_config
        from intellect_cli.config import save_config as _scfg_save
        cfg = _load_config()
        features = cfg.get("features") if isinstance(cfg.get("features"), dict) else {}
        if body and "enabled" in body:
            features["searxng_enabled"] = bool(body["enabled"])
        if body and "url" in body:
            features["searxng_url"] = str(body.get("url", "")).strip()
        cfg["features"] = features
        _scfg_save(cfg)
        return j(handler, {"ok": True, "enabled": features.get("searxng_enabled", False), "url": features.get("searxng_url", "")})
    except Exception as exc:
...
def _canvas_find_file(handler) -> Path | None:
    """Resolve the canvas save file for the active profile."""
    try:
        from api.profiles import get_active_intellect_home
        d = Path(get_active_intellect_home()) / _CANVAS_DIR
    except Exception:
        d = Path(os.getenv("INTELLECT_HOME", str(Path.home() / ".intellect"))) / _CANVAS_DIR
    d.mkdir(parents=True, exist_ok=True)
    return d / "canvas.json"


def _canvas_load_from_disk(handler) -> dict:
    """Read the saved canvas from disk."""
    path = _canvas_find_file(handler)
    if not path.exists():
        return {"content": "", "updated_at": None, "filename": "canvas.md"}
    try:
        data = json.loads(path.read_text())
        return data if isinstance(data, dict) else {"content": "", "updated_at": None, "filename": "canvas.md"}
    except Exception:
        return {"content": "", "updated_at": None, "filename": "canvas.md"}


def _handle_canvas_load(handler, parsed) -> bool:
    """GET /api/canvas/load — return saved canvas content (F3)."""
    return j(handler, _canvas_load_from_disk(handler))


def _handle_canvas_save(handler, body) -> bool:
    """POST /api/canvas/save — persist canvas content to disk (F3)."""
    path = _canvas_find_file(handler)
    try:
        data = {
            "content": str(body.get("content", "") if body else ""),
            "updated_at": (body or {}).get("updated_at") or time.time(),
            "filename": str((body or {}).get("filename", "canvas.md")),
        }
        path.write_text(json.dumps(data))
        path.chmod(0o600)
        return j(handler, {"ok": True, "updated_at": data["updated_at"]})
    except Exception as exc:
        return j(handler, {"ok": False, "error": str(exc)}, status=500)


def _handle_code_status(handler, parsed) -> bool:
    """Return code execution availability and supported languages (P3-6)."""
    try:
        from tools.code_execution import get_executor
        executor = get_executor()
        return j(handler, {
            "docker_available": executor.is_available(),
            "languages": executor.list_languages(),
        })
    except ImportError:
        return j(handler, {"docker_available": False, "languages": []})


def _handle_code_execute(handler, body) -> bool:
    """Execute code in a sandboxed container (P3-6)."""
    try:
        from tools.code_execution import execute_code_cell
    except ImportError:
        return j(handler, {"error": "Code execution engine not available"}, status=503)

    code = str(body.get("code", "") if body else "").strip()
    language = str(body.get("language", "python") if body else "python").strip()
    if not code:
        return j(handler, {"error": "No code provided"}, status=400)

    result = execute_code_cell(code, language=language)
...
def _deep_health_checks(stream_check: dict | None = None) -> tuple[dict, bool]:
    ...
    shared stream map, sidebar/session path, project state, and Intellect state.db
    ...
    t0 = time.time()
    try:
        from api.session_visibility import apply_member_scope_to_session_rows
        sessions = apply_member_scope_to_session_rows(all_sessions())
        checks["sessions"] = {
            "status": "ok",
            "count": len(sessions),
    ...
    try:
        ...
        backend = resolve_storage_backend_name()
        if not state_storage_available():
            checks["session_store"] = {
                "status": "error" if backend == "postgresql" else "missing",
                "backend": backend,
                "error": "dsn_not_configured" if backend == "postgresql" else None,
                "ms": round((time.time() - t0) * 1000, 1),
            }
        else:
            with open_session_db() as db:
                if db is None:
                    checks["session_store"] = {
                        "status": "error",
                        "backend": backend,
        ...
        checks["state_db"] = checks.get("session_store", {"status": "missing"})
    except Exception as exc:
        checks["session_store"] = {
            "status": "error",
            "error": type(exc).__name__,
        ...
        checks["state_db"] = checks["session_store"]
...
def _get_plugin_manager_for_visibility():
    """Return Intellect Agent's plugin manager for read-only WebUI visibility."""
...
def _plugin_visibility_payload(manager=None) -> dict:
    ...
    The Intellect Agent manager stores manifests and callback objects internally.
...
<head>
  ...
  <title>Intellect is restarting</title>
</head>
<body style=\"margin:0;padding:2rem;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#111827;color:#e5e7eb;\">
  <main style=\"max-width:40rem;margin:10vh auto;line-height:1.5;\">
    <h1 style=\"font-size:1.5rem;margin:0 0 0.75rem;\">Intellect is restarting…</h1>
...
def _serve_manifest(handler) -> bool:
    ...
    """
    static_root = Path(__file__).parent.parent / STATIC_DIR
...
def handle_get(handler, parsed) -> bool:
    ...
    if parsed.path.startswith("/api/oauth/providers"):
        ...
        if oauth_p_api.handle_get(handler, parsed):
            return True

    if parsed.path.startswith("/api/storage/"):
        from api import storage_api

        if storage_api.handle_get(handler, parsed):
            return True
    ...
    if (
        ...
        if members_api.handle_get(handler, parsed):
            return True

    if parsed.path.startswith("/api/user/"):
        from api import user_profile as user_profile_api

        if user_profile_api.handle_get(handler, parsed):
            return True

    if parsed.path.startswith(f"/session/{STATIC_DIR}/"):
        # Strip the leading "/session" so _serve_static() sees a path that
        # starts with "/{STATIC_DIR}/" (its required prefix). _serve_static enforces
    ...
    # Mis-resolved auth redirect (relative login?next= from /session/<id> → /session/login).
    if parsed.path == "/session/login":
        from api.helpers import redirect

        loc = "../login" + ("?" + parsed.query if parsed.query else "")
    ...
    if parsed.path in ("/", "/index.html") or parsed.path.startswith("/session/"):
        try:
            ...
            # If the member must change their password, hide the main UI and
            # show the password overlay before the first paint — no flash.
            try:
                from api.members import resolve_member_id, _member_password_change_blocks_request
                actor = resolve_member_id(handler, parsed)
                if actor and _member_password_change_blocks_request(handler, parsed, actor):
                    css = (
                        "<style>"
                        ".layout{display:none!important}"
                        "#memberPasswordOverlay{display:flex!important}"
                        "#memberPasswordOverlay[hidden]{display:flex!important}"
                        "</style>"
                    )
    ...
    if parsed.path == "/login":
        ...
        _bn = _html.escape(_settings.get("bot_name") or "Intellect")
        _lang = _settings.get("language", "en")
        _login_strings = _merged_login_strings(_lang)
        from api.auth import build_login_context
        from urllib.parse import quote
        from api.updates import WEBUI_VERSION
        version_token = quote(WEBUI_VERSION, safe="")
        _i18n_json = json.dumps(_login_strings, ensure_ascii=False).replace("<", "\\u003c")
        _ctx_json = json.dumps(
            build_login_context(handler, parsed), ensure_ascii=False
        ).replace("<", "\\u003c")
        _page = (
            _LOGIN_PAGE_HTML.replace("{{BOT_NAME}}", _bn)
            .replace("{{STATIC_DIR}}", STATIC_DIR)
            ...
            .replace(
                "{{LOGIN_MEMBER_TITLE}}",
                _html.escape(_login_strings.get("member_title", "Member sign-in")),
            )
            .replace(
                "{{LOGIN_MEMBER_SUBTITLE}}",
                _html.escape(_login_strings.get("member_subtitle", "")),
            )
            .replace(
                "{{LOGIN_INVITE_LABEL}}",
                _html.escape(_login_strings.get("invite_label", "Invite code")),
            )
            .replace(
                "{{LOGIN_REDEEM_BTN}}",
                _html.escape(_login_strings.get("redeem_btn", "Redeem invite")),
            )
            .replace(
                "{{LOGIN_PASSWORD_TITLE}}",
                _html.escape(_login_strings.get("login_with_password_title", "Member sign-in")),
            )
            .replace(
                "{{LOGIN_PASSWORD_HINT}}",
                _html.escape(_login_strings.get("login_with_password_hint", "")),
            )
            .replace(
                "{{LOGIN_MEMBER_PASSWORD_PLACEHOLDER}}",
                _html.escape(_login_strings.get("member_password_placeholder", "Password")),
            )
            .replace(
                "{{LOGIN_MEMBER_BTN}}",
                _html.escape(_login_strings.get("member_login_btn", "Sign in")),
            )
            .replace(
                "{{LOGIN_REGISTER_LINK}}",
                _html.escape(_login_strings.get("register_link", "Create an account")),
            )
            .replace(
                "{{LOGIN_TAB_OAUTH}}",
                _html.escape(_login_strings.get("login_tab_oauth", "OAuth")),
            )
            .replace(
                "{{LOGIN_TAB_PASSWORD}}",
                _html.escape(_login_strings.get("login_tab_password", "Account")),
            )
            .replace(
                "{{LOGIN_TAB_LOCAL}}",
                _html.escape(_login_strings.get("login_tab_local", "Local dev")),
            )
            .replace(
                "{{LOGIN_LOCAL_DEV_TITLE}}",
                _html.escape(_login_strings.get("local_dev_title", "Local development")),
            )
            .replace(
                "{{LOGIN_LOCAL_DEV_HINT}}",
                _html.escape(_login_strings.get("local_dev_hint", "")),
            )
            .replace(
                "{{LOGIN_MEMBER_ID_PLACEHOLDER}}",
                _html.escape(_login_strings.get("member_id_placeholder", "Member id")),
            )
            .replace(
                "{{LOGIN_LOCAL_DEV_BTN}}",
                _html.escape(_login_strings.get("local_dev_btn", "Local sign in")),
            )
            .replace(
                "{{LOGIN_CONTINUE_BTN}}",
                _html.escape(_login_strings.get("continue_btn", "Continue")),
            )
            .replace("{{LOGIN_I18N_JSON}}", _i18n_json)
            .replace("{{LOGIN_CONTEXT_JSON}}", _ctx_json)
        )
    ...
    if parsed.path == "/register":
        ...
        _lang = _settings.get("language", "en")
        _login_strings = _merged_login_strings(_lang)
        from api.auth import build_register_context
        from urllib.parse import quote
        from api.updates import WEBUI_VERSION
        version_token = quote(WEBUI_VERSION, safe="")
        _i18n_json = json.dumps(_login_strings, ensure_ascii=False).replace("<", "\\u003c")
        _ctx_json = json.dumps(
            build_register_context(handler, parsed), ensure_ascii=False
        ).replace("<", "\\u003c")
        _page = (
            _REGISTER_PAGE_HTML.replace("{{BOT_NAME}}", _bn)
            .replace("{{STATIC_DIR}}", STATIC_DIR)
            .replace("{{WEBUI_VERSION}}", version_token)
            .replace("{{LANG}}", _html.escape(_login_strings["lang"]))
            .replace("{{REGISTER_TITLE}}", _html.escape(_login_strings.get("register_title", "Create account")))
            .replace("{{REGISTER_SUBTITLE}}", _html.escape(_login_strings.get("register_subtitle", "")))
            .replace("{{REGISTER_TAB_LOCAL}}", _html.escape(_login_strings.get("register_tab_local", "Local account")))
            .replace("{{REGISTER_TAB_OAUTH}}", _html.escape(_login_strings.get("register_tab_oauth", "OAuth")))
            .replace("{{REGISTER_TAB_INVITE}}", _html.escape(_login_strings.get("register_tab_invite", "Invite code")))
            .replace("{{REGISTER_LOCAL_HINT}}", _html.escape(_login_strings.get("register_local_hint", "")))
            .replace("{{REGISTER_MEMBER_ID}}", _html.escape(_login_strings.get("register_member_id", "Member id")))
            .replace("{{REGISTER_DISPLAY_NAME}}", _html.escape(_login_strings.get("register_display_name", "Display name")))
            .replace("{{REGISTER_PASSWORD}}", _html.escape(_login_strings.get("register_password", "Password")))
            .replace(
                "{{REGISTER_PASSWORD_CONFIRM}}",
                _html.escape(_login_strings.get("register_password_confirm", "Confirm password")),
            )
            .replace("{{REGISTER_INVITE_CODE}}", _html.escape(_login_strings.get("register_invite_code", "Invite code")))
            .replace("{{REGISTER_SUBMIT}}", _html.escape(_login_strings.get("register_submit", "Create account")))
            .replace("{{REGISTER_LOGIN_LINK}}", _html.escape(_login_strings.get("register_login_link", "Sign in")))
            .replace("{{REGISTER_CLOSED}}", _html.escape(_login_strings.get("register_closed", "")))
            .replace("{{REGISTER_I18N_JSON}}", _i18n_json)
            .replace("{{REGISTER_CONTEXT_JSON}}", _ctx_json)
        )
        return t(handler, _page, content_type="text/html; charset=utf-8")

    if parsed.path == "/api/auth/login-context":
        from api.auth import build_login_context

        j(handler, build_login_context(handler, parsed))
        return True

    if parsed.path == "/api/auth/register-context":
        from api.auth import build_register_context

        j(handler, build_register_context(handler, parsed))
        return True
    ...
    if parsed.path == "/api/analytics":
        return _handle_analytics(handler, parsed)
    if parsed.path == "/api/search":
        return _handle_search(handler, parsed)
    if parsed.path == "/api/search/probe":
        return _handle_search_probe(handler, parsed)
    if parsed.path == "/api/search/config":
        return _handle_search_config_get(handler, parsed)
    if parsed.path == "/api/canvas/load":
        return _handle_canvas_load(handler, parsed)
    if parsed.path == "/api/code/status":
        return _handle_code_status(handler, parsed)
    ...
    if _api_path_normalized(parsed.path) == "/api/vault/config":
        return _handle_vault_config_get(handler, parsed)

    # ── Vault static serving ──
    if parsed.path == "/vault" or parsed.path.startswith("/vault/"):
        return _handle_vault_serve(handler, parsed)
    ...
    if parsed.path == "/api/settings":
        ...
        settings["password_env_var"] = bool(
            os.getenv("INTELLECT_WEBUI_PASSWORD", "").strip()
        )
    ...
    if parsed.path.startswith(f"/{STATIC_DIR}/"):
    ...
    if parsed.path == "/api/session":
        ...
        import time as _time
        _t0 = _time.monotonic()
        _debug_slow = os.environ.get("INTELLECT_DEBUG_SLOW", "")
        query = parse_qs(parsed.query)
        sid = query.get("session_id", [""])[0]
        if not sid:
        ...
        try:
            ...
            if is_messaging_session:
                ...
                state_db_messages = get_state_db_session_messages(sid, profile=_session_profile, member_id=getattr(s, 'member_id', None))
            ...
            # WebUI session missing — try CLI / state.db import path
            cli_meta = _lookup_cli_session_metadata(sid)
            if cli_meta:
                from api.session_visibility import session_row_visible_for_request

                if not session_row_visible_for_request(cli_meta):
                    return bad(handler, "Session not found", 404)
    ...
    if parsed.path == "/api/session/lineage/report":
        ...
        profile = parse_qs(parsed.query).get("profile", [None])[0] or None
        report = read_session_lineage_report_for_profile(profile, sid)
    ...
    if parsed.path == "/api/session/recovery/audit":
        ...
        return j(handler, audit_session_recovery(SESSION_DIR, state_db_path=None))
    ...
    if parsed.path == "/api/sessions":
        ...
        try:
            ...
            from api.profiles import get_active_profile_name, is_profile_management_enabled
            active_profile = get_active_profile_name()
            # TEMPORARY: cross-profile session list disabled while profile
            # management is off (no "show other profiles" toggle).
            all_profiles = (
                _all_profiles_query_flag(parsed)
            ...
            scoped = apply_member_scope_to_session_rows(scoped)
            ...
            for s in scoped:
                ...
                # Attach attention metadata for pending approvals / clarify
                try:
                    attention = _session_attention_summary(item.get("session_id", ""))
                    if attention:
    ...
    if parsed.path == "/api/projects":
        ...
        # TEMPORARY: same gate as /api/sessions — no aggregate profile view.
        all_profiles = (
            _all_profiles_query_flag(parsed)
            if is_profile_management_enabled()
            else False
    ...
    if parsed.path == "/api/personalities":
        ...
        # (matches intellect-agent CLI behavior, not filesystem SOUL.md approach)
    ...
    # ── OAuth provider management (GET) ──
    if parsed.path == "/api/providers/oauth":
        from api.oauth import list_oauth_providers
        return j(handler, list_oauth_providers(), extra_headers={"Cache-Control": "no-store"})

    if parsed.path.startswith("/api/providers/oauth/") and "/poll/" in parsed.path:
        rest = parsed.path[len("/api/providers/oauth/"):]
        parts = rest.split("/")
        if len(parts) == 3 and parts[1] == "poll":
            from api.oauth import _poll_oauth_session
            try:
                return j(handler, _poll_oauth_session(parts[0], parts[2]),
                         extra_headers={"Cache-Control": "no-store"})
            except KeyError as e:
                return bad(handler, str(e), 404)
            except ValueError as e:
    ...
    # ── Cron API (GET) ──
    # All cron handlers touch cron.jobs which resolves INTELLECT_HOME from
    ...
    if parsed.path == "/api/profile/active":
        ...
        return j(
            handler,
            {
                ...
                "path": str(get_active_intellect_home()),
                "management_enabled": is_profile_management_enabled(),
            },
        )
    ...
    # ── Memory Provider API (GET) ──
    if parsed.path == "/api/memory/providers":
        return _handle_memory_providers_list(handler)

    if parsed.path.startswith("/api/memory/providers/"):
        provider_name = parsed.path[len("/api/memory/providers/"):]
        if provider_name and "/" not in provider_name:
            return _handle_memory_provider_get(handler, provider_name)
...
def handle_post(handler, parsed) -> bool:
    ...
    if (
        ...
        if members_api.handle_post(handler, parsed):
            if diag:
                diag.finish()
            return True
    ...
    if parsed.path.startswith("/api/oauth/providers"):
        from api import oauth_providers as oauth_p_api
        from api.helpers import read_body as _read_body_post

        body = _read_body_post(handler)
        if oauth_p_api.handle_post(handler, parsed, body):
            if diag:
                diag.finish()
            return True
    ...
    if parsed.path.startswith("/api/user/"):
        ...
        if user_profile_api.handle_post(handler, parsed):
            if diag:
                diag.finish()
            return True
    ...
    if parsed.path.startswith("/api/storage/"):
        ...
        if storage_api.handle_post(handler, parsed, body):
            if diag:
                diag.finish()
            return True
    ...
    if parsed.path == "/api/session/recovery/repair-safe":
        ...
        result = repair_safe_session_recovery(SESSION_DIR, state_db_path=None)
    ...
    if parsed.path == "/api/session/new":
        ...
        if prev_session_id:
            try:
                ...
                _prev_mid = None
                try:
                    _prev_sess = get_session(prev_session_id, metadata_only=True)
                    _prev_mid = getattr(_prev_sess, "member_id", None)
                except KeyError:
                    pass
                _prev_key = session_agent_cache_key(prev_session_id, _prev_mid)
                with SESSION_AGENT_CACHE_LOCK:
                    _cached = SESSION_AGENT_CACHE.get(_prev_key)
                    if not _cached:
        ...
        from api.session_visibility import ensure_session_owner

        ensure_session_owner(s)
    ...
    if parsed.path == "/api/session/duplicate":
        try:
            ...
            copied_session = Session(
                ...
                # deepcopied so the duplicate has its own independent context
                context_messages=copy.deepcopy(getattr(session, "context_messages", None) or []),
                # Gateway routing — if the user customized routing, the duplicate
                ...
                # Preserve LLM-generated title flag so we don't regenerate title.
    ...
    if parsed.path == "/api/default-model":
        try:
            return j(handler, set_intellect_default_model(body.get("model")))
        except ValueError as e:
            return bad(handler, str(e))
    ...
    if parsed.path == "/api/models/refresh":
        ...
        try:
            ...
            return bad(handler, f"Failed to refresh models: {exc}", status=500)
    ...
    if parsed.path == "/api/model/set":
        ...
        if scope == "main":
            try:
                return j(handler, set_intellect_default_model(model))
            except ValueError as exc:
                return bad(handler, str(exc), status=400)
    ...
    if parsed.path == "/api/providers":
        ...
        base_url = body.get("base_url")  # Optional: for Xiaomi Token Plan URL
        if not provider_id:
        ...
        result = set_provider_key(provider_id, api_key, base_url=base_url)
    ...
    if parsed.path == "/api/personality/set":
        ...
        # (matches intellect-agent CLI behavior)
        prompt = ""
        if name:
            ...
            # Resolve prompt using the same logic as intellect-agent cli.py
            if isinstance(value, dict):
    ...
    if parsed.path == "/api/session/branch":
        ...
        branch = Session(
            ...
            session_source="fork",
            member_id=getattr(source, "member_id", None),
    ...
    # ── Vault Build (POST) ──
    if parsed.path == "/api/wiki/build":
        return _handle_vault_build(handler, body)
    if parsed.path == "/api/wiki/init":
    ...
    if parsed.path == "/api/vault/tick":
        return _handle_vault_tick(handler, body)
    if parsed.path == "/api/canvas/save":
        return _handle_canvas_save(handler, body)
    if parsed.path == "/api/search/config":
        return _handle_search_config_save(handler, body)
    ...
    if parsed.path == "/api/settings":
        ...
        if "bot_name" in body:
            body["bot_name"] = (str(body["bot_name"]) or "").strip() or "Intellect"
        if "dashboard_title" in body:
            body["dashboard_title"] = (
                (str(body["dashboard_title"]) or "").strip() or "Intellect Agent Harness"
            )
        ...
        # #1560: INTELLECT_WEBUI_PASSWORD env var takes precedence in
        ...
        if requested_password or requested_clear_password:
            if os.getenv("INTELLECT_WEBUI_PASSWORD", "").strip():
                return bad(
                    handler,
                    "INTELLECT_WEBUI_PASSWORD env var is set — it overrides the settings password. "
        ...
        if requested_passwordless:
            ...
            if not _passkey_feature_flag_enabled():
                return bad(handler, "Passkey support is disabled. Enable INTELLECT_WEBUI_PASSKEY before going passwordless.", 409)
    ...
    if parsed.path == "/api/config":
        ...
        if not is_loopback_client(handler):
            return bad(handler, "Config changes require localhost access", 403)
        key = str(body.get("key") or "").strip()
        value = body.get("value")
        if not key:
            return bad(handler, "key required", 400)
        allowed = ("members.",)
        if not any(key.startswith(p) for p in allowed):
            return bad(handler, f"Config key {key!r} is not allowed via API", 403)
        try:
            from intellect_cli.config import set_config_value
            set_config_value(key, str(value).lower() if isinstance(value, bool) else str(value))
        except Exception as exc:
            return bad(handler, str(exc), 500)
        from api.config import reload_config
        reload_config()
        return j(handler, {"ok": True, "key": key, "value": value})
    ...
    if parsed.path == "/api/onboarding/oauth/start":
        ...
        if not is_auth_enabled() and not _os.getenv("INTELLECT_WEBUI_ONBOARDING_OPEN"):
            ...
            try:
                ...
                is_local = False
            if not is_local:
                return bad(handler, "Onboarding OAuth is only available from local networks when auth is not enabled. To bypass this on a remote server, set INTELLECT_WEBUI_ONBOARDING_OPEN=1.", 403)
        try:
    ...
    # ── OAuth provider management (POST) ──
    if parsed.path.endswith("/start") and parsed.path.startswith("/api/providers/oauth/"):
        provider_id = parsed.path[len("/api/providers/oauth/"):].rstrip("/").rsplit("/start")[0]
        from api.oauth import _start_oauth_login
        try:
            return j(handler, _start_oauth_login(provider_id),
                     extra_headers={"Cache-Control": "no-store"})
        except ValueError as e:
            return bad(handler, str(e))
        except RuntimeError as e:
    ...
    if parsed.path.endswith("/submit") and parsed.path.startswith("/api/providers/oauth/"):
        provider_id = parsed.path[len("/api/providers/oauth/"):].rstrip("/").rsplit("/submit")[0]
        from api.oauth import _submit_anthropic_pkce
        try:
            session_id = str((body or {}).get("session_id") or "").strip()
            code = str((body or {}).get("code") or "").strip()
            return j(handler, _submit_anthropic_pkce(session_id, code),
                     extra_headers={"Cache-Control": "no-store"})
        except ValueError as e:
            return bad(handler, str(e))
        except RuntimeError as e:
    ...
    if parsed.path == "/api/onboarding/setup":
        ...
        # INTELLECT_WEBUI_ONBOARDING_OPEN=1 lets operators on remote servers explicitly bypass
        ...
        if not is_auth_enabled() and not _os.getenv("INTELLECT_WEBUI_ONBOARDING_OPEN"):
            ...
            try:
                ...
                is_local = False
            if not is_local:
                return bad(handler, "Onboarding setup is only available from local networks when auth is not enabled. To bypass this on a remote server, set INTELLECT_WEBUI_ONBOARDING_OPEN=1.", 403)
        try:
    ...
    if parsed.path == "/api/onboarding/probe":
        ...
        if not is_auth_enabled() and not _os.getenv("INTELLECT_WEBUI_ONBOARDING_OPEN"):
            ...
            try:
                ...
                is_local = False
            if not is_local:
                return bad(handler, "Onboarding probe is only available from local networks when auth is not enabled. To bypass this on a remote server, set INTELLECT_WEBUI_ONBOARDING_OPEN=1.", 403)
    ...
    if parsed.path == "/api/session/pin":
        ...
        if pin_requested and not getattr(s, "pinned", False):
            ...
            persisted_pinned_ids = {
                ...
                for existing in apply_member_scope_to_session_rows(all_sessions())
                if _session_field(existing, "pinned", False)
                and not _session_field(existing, "archived", False)
                and not _is_hidden_empty_session(existing)
            }
            with LOCK:
        ...
        def _llm_update_summary(system_prompt: str, user_prompt: str) -> str:
            ...
            with profiles_api.profile_env_for_background_worker(
                ...
                try:
                    ...
                    # discover in the Intellect Agent setup/config UI.
    ...
    # ── Memory Provider API (POST) ──
    if parsed.path == "/api/memory/providers":
        return _handle_memory_provider_set_active(handler, body)

    if parsed.path.startswith("/api/memory/providers/"):
        provider_name = parsed.path[len("/api/memory/providers/"):]
        if provider_name and "/" not in provider_name:
            return _handle_memory_provider_save(handler, body, provider_name)
...
def handle_delete(handler, parsed) -> bool:
    ...
    if parsed.path.startswith("/api/oauth/providers/"):
        from api import oauth_providers as oauth_p_api
        return oauth_p_api.handle_delete(handler, parsed)
    ...
    # ── MCP server management (DELETE) ──
    if parsed.path.startswith("/api/mcp/servers/"):
        from urllib.parse import unquote
        name = unquote(parsed.path[len("/api/mcp/servers/"):])
        if name:
            return _handle_mcp_server_delete(handler, name)

    # ── OAuth provider management (DELETE) ──
    if parsed.path.startswith("/api/providers/oauth/"):
        from api.oauth import disconnect_oauth_provider, _cancel_oauth_session
        rest = parsed.path[len("/api/providers/oauth/"):]
        # DELETE /api/providers/oauth/sessions/{session_id}
        if rest.startswith("sessions/") and len(rest) > 9:
            session_id = rest[len("sessions/"):]
            result = _cancel_oauth_session(session_id)
            return j(handler, result)
        # DELETE /api/providers/oauth/{provider_id}
        if "/" not in rest and rest:
            provider_id = rest
            try:
                return j(handler, disconnect_oauth_provider(provider_id))
            except ValueError as e:
                return bad(handler, str(e), 400)
...
def _serve_static(handler, parsed):
    static_root = (Path(__file__).parent.parent / STATIC_DIR).resolve()
    # Strip the leading '/{STATIC_DIR}/' prefix, then resolve and sandbox
    rel = parsed.path[len(f"/{STATIC_DIR}/"):]
...
def _handle_session_export(handler, parsed):
    ...
    handler.send_header("Content-Type", "application/json; charset=utf-8")
    handler.send_header(
        "Content-Disposition", f'attachment; filename="intellect-{sid}.json"'
    )
...
def _session_search_message_text(message) -> str:
    """Extract searchable plain text from a WebUI message object."""
...
def _session_search_preview(body, query: str, *, max_len: int = 92) -> str:
    """Build a short preview around the first case-insensitive match."""
    if not body or not query:
        return ""
    text = str(body)
    idx = text.lower().find(str(query).lower())
    if idx < 0:
        return ""
    half = max(10, (max_len - len(query)) // 2)
...
def _handle_sessions_search_fts(handler, parsed, q: str):
    """Option B — FTS via state.db ``search_messages()`` + JSON hybrid fallback."""
    from api.session_visibility import apply_member_scope_to_session_rows
    from api.storage_bridge import open_session_db

    qs = parse_qs(parsed.query)
    limit = max(1, min(50, int(qs.get("limit", ["20"])[0] or 20)))
    offset = max(0, int(qs.get("offset", ["0"])[0] or 0))
    sort = (qs.get("sort", ["newest"])[0] or "newest").strip() or "newest"

    scoped = {s.get("session_id"): s for s in apply_member_scope_to_session_rows(all_sessions())}
    results = []
    seen: set[str] = set()

    try:
        with open_session_db() as db:
            if db is not None and getattr(db, "_fts_enabled", False):
                ...
                for hit in hits or []:
                    ...
                    if not sid or sid in seen or sid not in scoped:
                        continue
                    seen.add(sid)
                    item = dict(scoped[sid])
                    snippet = str(hit.get("snippet") or "")
                    if snippet:
                        item["match_preview"] = _redact_text(snippet.replace(">>>", "").replace("<<<", ""))
                    item["match_type"] = "fts"
                    item["match_message_id"] = hit.get("id")
                    item["matched_role"] = hit.get("role")
                    if isinstance(item.get("title"), str):
                        item["title"] = _redact_text(item["title"])
                    results.append(item)
    except Exception:
    ...
    if len(results) < limit:
        substring = _handle_sessions_search_substring_rows(
            q,
            content_search=True,
            depth=5,
            scoped_rows=list(scoped.values()),
        )
    ...
    return j(handler, {"sessions": results, "query": q, "mode": "fts", "count": len(results)})


def _handle_sessions_search_substring_rows(
    q: str,
    *,
    content_search: bool,
    depth: int,
    ...
    """Option A substring search over pre-scoped session rows."""
    q_lower = q.lower().strip()
    if not q_lower:
        return list(scoped_rows)
    results = []
    for s in scoped_rows:
        title_match = q_lower in (s.get("title") or "").lower()
        if title_match:
        ...
        if content_search:
            try:
                ...
                for m in msgs:
                    ...
                    if q_lower in str(c).lower():
                        item = dict(s, match_type="content")
                        preview = _session_search_preview(c, q_lower)
                        if preview:
...
def _handle_sessions_search(handler, parsed):
    ...
    qs = parse_qs(parsed.query)
    q_raw = qs.get("q", [""])[0].strip()
    mode = (qs.get("mode", ["substring"])[0] or "substring").strip().lower()
    if mode == "fts" and q_raw:
        return _handle_sessions_search_fts(handler, parsed, q_raw)
    q = q_raw.lower()
    content_search = qs.get("content", ["1"])[0] == "1"
    depth = int(qs.get("depth", ["5"])[0])
    if not q:
        safe_sessions = []
        for s in apply_member_scope_to_session_rows(all_sessions()):
            item = dict(s)
            if isinstance(item.get("title"), str):
                item["title"] = _redact_text(item["title"])
            safe_sessions.append(item)
        return j(handler, {"sessions": safe_sessions})
    scoped = apply_member_scope_to_session_rows(all_sessions())
    results = _handle_sessions_search_substring_rows(
        q, content_search=content_search, depth=depth, scoped_rows=scoped,
    )
...
def _handle_list_dir(handler, parsed):
    ...
    try:
        ...
        logger.exception("list_dir failed for sid=%s path=%s workspace=%s", sid, rel_path, workspace)
        return bad(handler, f"Internal server error: {type(e).__name__}: {e}", 500)
...
def _handle_gateway_sse_stream(handler, parsed):
    ...
    try:
        ...
        initial = apply_member_scope_to_session_rows(get_cli_sessions())
        ...
        while True:
            ...
            if isinstance(event_data, dict) and event_data.get('sessions'):
                event_data = {
                    **event_data,
                    'sessions': apply_member_scope_to_session_rows(
                        event_data.get('sessions') or []
                    ),
                }
...
def _handle_media(handler, parsed):
    ...
    Security:
    - Path must resolve to an allowed root (intellect home, /tmp, common dirs)
    ...
    _INTELLECT_HOME = Path(_os.getenv("INTELLECT_HOME", str(_HOME / ".intellect"))).expanduser()
    ...
    def _path_is_within_root(child: Path, root: Path) -> bool:
        ...
        try:
            return _os.path.commonpath([str(child), str(root)]) == str(root)
        except ValueError:
            return False
    ...
    # ── #3234: hard-deny Intellect's own state + secret/config files ─────────
    # The allowlist above grants the whole Intellect home (and base ~/.intellect),
    # so an authenticated session rendering attacker-influenced agent output that
    ...
    # browser reaches them), even when a workspace happens to live under an
    # Intellect root. The deny rules target Intellect's OWN internal state, which
    # lives OUTSIDE any workspace. So: if the target is inside the active
    # workspace, it is never denied here; otherwise we deny known secret/config
    # basenames and the internal state subdirectories across every Intellect root
    # the allowlist accepts (INTELLECT_HOME, base ~/.intellect, and STATE_DIR).
    _DENY_FILENAMES = {
    ...
    for _r in (
        ...
        if _r is not None and _r not in _intellect_roots:
            _intellect_roots.append(_r)

    # Active-workspace carve-out: a file inside the active workspace is the
    # user's own content, not Intellect internal state — never deny it here.
    ...
    # any Intellect root, an ancestor of an Intellect root, STATE_DIR, a
    # */profiles dir, a named profile root, or an internal state subdir),
    # honoring it would re-open the disclosure. In that case we DISABLE the
    # carve-out so the deny rules below still fire.
    ...
    def _workspace_is_safe_carveout(ws):
        if ws is None:
            return False
        # Never trust a workspace that IS, CONTAINS, or is CONTAINED BY an
        # Intellect root, or that is $HOME / a profiles dir / a state subdir.
        if ws == _HOME.resolve():
            return False
        for _root in _intellect_roots:
    ...
    if not _in_active_workspace:
        ...
        # cannot bypass the deny on macOS/Windows filesystems.
        def _within_ci(child, root):
            try:
                c = _os.path.normcase(str(Path(child).resolve()))
                r = _os.path.normcase(str(Path(root).resolve()))
                return _os.path.commonpath([c, r]) == r
            except (ValueError, OSError):
                return False
        _deny_names_ci = {n.casefold() for n in _DENY_FILENAMES}
        _under_intellect_root = any(
            _within_ci(target, _root) for _root in _intellect_roots
        )
        # State-subdir deny: any DENY_SUBDIR directly under an Intellect root,
        # and the STATE_DIR itself (sensitive in its entirety).
        _deny_dirs = []
        ...
        _denied = (
            (_under_intellect_root and target.name.casefold() in _deny_names_ci)
...
# ─── /api/folder/download ───────────────────────────────────────────────────
# Configurable caps. Match the INTELLECT_WEBUI_MAX_UPLOAD_MB style used elsewhere
...
def _folder_zip_max_bytes() -> int:
    try:
        mb = int(os.getenv("INTELLECT_WEBUI_FOLDER_ZIP_MAX_MB", "1024"))
    except ValueError:
        mb = 1024
...
def _folder_zip_max_files() -> int:
    try:
        return max(1, int(os.getenv("INTELLECT_WEBUI_FOLDER_ZIP_MAX_FILES", "50000")))
    except ValueError:
        return 50000
...
def _handle_folder_download(handler, parsed):
    ...
    Respects INTELLECT_WEBUI_FOLDER_ZIP_MAX_MB and INTELLECT_WEBUI_FOLDER_ZIP_MAX_FILES.
    ...
    if limit_hit == "max_files":
        return j(handler, {
            ...
            "limit": max_files,
            "configure": "INTELLECT_WEBUI_FOLDER_ZIP_MAX_FILES",
        }, status=413)
    if limit_hit == "max_bytes":
        return j(handler, {
            "error": "folder too large",
            "limit_bytes": max_bytes,
            "configure": "INTELLECT_WEBUI_FOLDER_ZIP_MAX_MB",
        }, status=413)
...
def _handle_approval_sse_stream(handler, parsed):
    ...
    try:
        from api.approval_events import subscribe_approval_channel

        def _remote_approval(msg: dict) -> None:
            with _lock:
        ...
        subscribe_approval_channel(sid, _remote_approval)
    except Exception:
        logger.debug("Approval Redis subscribe failed for %s", sid, exc_info=True)
...
def _handle_clarify_sse_stream(handler, parsed):
    ...
    try:
        from api.approval_events import subscribe_clarify_channel
        from api.clarify import _clarify_sse_notify

        def _remote_clarify(msg: dict) -> None:
            _clarify_sse_notify(
                sid,
                msg.get("head"),
                int(msg.get("total") or 0),
        ...
        subscribe_clarify_channel(sid, _remote_clarify)
    except Exception:
        logger.debug("Clarify Redis subscribe failed for %s", sid, exc_info=True)
...
def _handle_live_models(handler, parsed):
    ...
    - ONTOWEB: live fetch from inference-api.ontoweb.cn/v1/models
    ...
    try:
        ...
        # works even when intellect_cli is not on sys.path.
        ...
        try:
            ...
            _agent_dir = _os.path.join(_os.path.dirname(_os.path.dirname(_os.path.abspath(__file__))),
                                       "..", "..", ".intellect", "intellect-agent")
            _agent_dir = _os.path.normpath(_agent_dir)
            if _agent_dir not in _sys.path:
                _sys.path.insert(0, _agent_dir)
            from intellect_cli.models import provider_model_ids as _pmi
            ids = _pmi(provider)
        ...
        if not ids:
            ...
            # returns [] because they aren't real intellect_cli endpoints.
...
def _handle_memory_read(handler):
    try:
        ...
        mem_dir = get_active_intellect_home() / "memories"
    except ImportError:
        mem_dir = Path.home() / ".intellect" / "memories"
    mem_file = mem_dir / "MEMORY.md"
    user_file = mem_dir / "USER.md"
    soul_file = mem_dir.parent / "SOUL.md"
    memory = (
...
# ── Memory Provider API helpers ────────────────────────────────────────────


def _handle_memory_providers_list(handler):
    """List available memory providers and current active provider."""
    try:
        ...
        return j(handler, {
            ...
            "configured": False,
            "error": "intellect-agent not found",
        })
    ...
    active_provider = mem_config.get("provider", "") or None
    ...
    providers = []
    for name, desc, available in discovered:
        providers.append({
            "name": name,
            "description": desc,
...
def _handle_memory_provider_get(handler, provider_name):
    """Get a specific memory provider's config schema and current values."""
    try:
        from plugins.memory import load_memory_provider
        from intellect_cli.config import load_config, get_env_value
        from api.profiles import get_active_intellect_home
    except ImportError:
        return j(handler, {"error": "intellect-agent not found"}, status=503)

    provider = load_memory_provider(provider_name)
    if not provider:
        return j(handler, {"error": f"Provider '{provider_name}' not found"}, status=404)

    try:
        schema = provider.get_config_schema()
    except Exception as e:
        logger.exception("Failed to get config schema for %s", provider_name)
        return j(handler, {"error": str(e)}, status=500)
    ...
    try:
        from api.profiles import get_active_intellect_home
        intellect_home = get_active_intellect_home()
    except Exception:
        from pathlib import Path
    ...
    for field in schema:
        ...
        if not key:
            continue
        env_var = field.get("env_var") or key
        ...
        if secret:
            try:
                ...
                env_val = None
            env_values[env_var] = "(set)" if env_val else None
        else:
            if key in provider_config:
                values[key] = provider_config[key]
...
def _handle_memory_provider_set_active(handler, body):
    """Set or clear the active memory provider without changing its config."""
    try:
        from intellect_cli.config import load_config, save_config
    except ImportError:
        return j(handler, {"error": "intellect-agent not found"}, status=503)

    provider = body.get("provider", "") if body else ""
    ...
    try:
        ...
        logger.exception("Failed to save memory provider config")
        return j(handler, {"error": str(e)}, status=500)

    return j(handler, {"ok": True, "provider": provider or None})


def _handle_memory_provider_save(handler, body, provider_name):
    """Save memory provider configuration (non-secret fields) and secrets."""
    try:
        from plugins.memory import load_memory_provider
        from intellect_cli.config import load_config, save_config, save_env_value
    except ImportError:
        return j(handler, {"error": "intellect-agent not found"}, status=503)

    if not body:
        return j(handler, {"error": "request body is required"}, status=400)
    ...
    # Save non-secret config via provider's save_config method
    try:
        if hasattr(provider, 'save_config') and fields:
            provider.save_config(fields, intellect_home)
    except Exception as e:
        logger.exception("Failed to save provider config via save_config")
        return j(handler, {"error": f"Failed to save config: {e}"}, status=500)

    # Write secrets to .env
    try:
        for env_var, value in secrets.items():
            if value:  # Only save non-empty secrets
                save_env_value(env_var, str(value))
    except Exception as e:
        logger.exception("Failed to save env values")
        return j(handler, {"error": f"Failed to save secrets: {e}"}, status=500)

    # Update config.yaml with provider name
    try:
        config = load_config()
        if config is None:
            config = {}
        ...
        logger.exception("Failed to update config.yaml")
        return j(handler, {"error": f"Failed to update config: {e}"}, status=500)

    return j(handler, {"ok": True, "provider": provider_name})
...
def _handle_btw(handler, body):
    ...
    thr = threading.Thread(
        ...
        kwargs={"ephemeral": True, "model_provider": model_provider,
                "member_id": getattr(ephemeral, 'member_id', None)},
        daemon=True,
    )
    thr.start()
...
def _handle_background(handler, body):
    ...
    bg.parent_session_id = s.session_id
    ...
    def _run_bg_and_notify():
        ...
        try:
            _run_agent_streaming(
                ...
                None,
                model_provider=model_provider,
                member_id=getattr(bg, 'member_id', None),
            )
    ...
    *,
    ...
    try:
        ...
        if load_settings().get("sync_to_insights"):
            from api.state_sync import sync_session_start
        ...
        logger.debug("state.db session start sync failed", exc_info=True)
...
def _start_chat_stream_for_session(
    ...
    *,
    ...
    # Capture member context from the request thread before spawning the
    # worker thread.  _tls (threading.local) is NOT inherited by new threads,
    # so we must pass member_id explicitly.
    _worker_member_id = getattr(s, 'member_id', None)
    _worker_team_id = getattr(s, 'team_id', None)
    _worker_project_id = None
    if not _worker_member_id:
        try:
            from api.members import get_bound_runtime_context
            _ctx = get_bound_runtime_context()
            if _ctx and _ctx.member_id:
                _worker_member_id = str(_ctx.member_id)
        ...
        try:
            ...
            _ctx = get_bound_runtime_context()
            if _ctx:
                if not _worker_team_id and _ctx.team_id:
                    _worker_team_id = str(_ctx.team_id)
                if _ctx.project_id:
    ...
    _worker_scope = None
    try:
        from api.session_visibility import capture_worker_session_scope

        _worker_scope = capture_worker_session_scope(
            member_id=_worker_member_id,
    ...
    thr = threading.Thread(
        ...
        kwargs={
            ...
            "member_id": _worker_member_id,
            "member_team_id": _worker_team_id,
            "member_project_id": _worker_project_id,
            "session_scope": _worker_scope,
        },
        daemon=True,
    )
    thr.start()
...
def _handle_goal_command(handler, body):
    ...
    try:
        from api.profiles import get_intellect_home_for_profile

        profile_home = get_intellect_home_for_profile(getattr(s, "profile", None))
    except Exception:
        profile_home = None
...
def _normalize_chat_attachments(raw_attachments):
    ...
    to Intellect as native multimodal inputs for the current turn.
    """
    normalized = []
...
def _handle_chat_sync(handler, body):
    ...
    with _ENV_LOCK:
        ...
        old_exec_ask = os.environ.get("INTELLECT_EXEC_ASK")
        old_session_key = os.environ.get("INTELLECT_SESSION_KEY")
        os.environ["INTELLECT_EXEC_ASK"] = "1"
        os.environ["INTELLECT_SESSION_KEY"] = s.session_id
    try:
        from run_agent import AIAgent
        ...
        with CHAT_LOCK:
            ...
            # Resolve API key via Intellect runtime provider (matches gateway behaviour)
            _api_key = None
            try:
                from api.oauth import resolve_runtime_provider_with_anthropic_env_lock
                from intellect_cli.runtime_provider import resolve_runtime_provider
            ...
            agent = AIAgent(
                ...
                base_url=_base_url,
                api_key=_api_key,
                # Identify browser-originated sessions as WebUI so Intellect Agent
        ...
        with _ENV_LOCK:
            ...
            if old_exec_ask is None:
                os.environ.pop("INTELLECT_EXEC_ASK", None)
            else:
                os.environ["INTELLECT_EXEC_ASK"] = old_exec_ask
            if old_session_key is None:
                os.environ.pop("INTELLECT_SESSION_KEY", None)
            else:
                os.environ["INTELLECT_SESSION_KEY"] = old_session_key
    with _get_session_agent_lock(s.session_id):
...
def _handle_cron_run(handler, body):
    ...
    #
    # Resolve directly without a try/except: get_active_intellect_home() does
    ...
    # run unpinned against the process-global INTELLECT_HOME — so we'd
    # rather let any unexpected exception 500 the request than corrupt
    # cross-profile state.
    from api.profiles import get_active_intellect_home

    _profile_home = get_active_intellect_home()
...
def _handle_file_open_vscode(handler, body):
    ...
    Intellect WebUI inside Docker still open files in their local editor.
    """
    try:
        require(body, "session_id", "path")
    except ValueError as e:
...
def _handle_session_compress(handler, body):
    ...
    try:
        ...
        from api.oauth import resolve_runtime_provider_with_anthropic_env_lock
        import intellect_cli.runtime_provider as _runtime_provider
...
def _handle_handoff_summary(handler, body):
    ...
    def _agent_text_completion(agent, system_prompt, user_text, max_tokens=700):
        """Use the current Intellect Agent transport without mutating conversation history."""
        api_messages = [
            {"role": "system", "content": system_prompt},
...
def _handle_skill_toggle(handler, body):
    ...
    ``INTELLECT_SESSION_PLATFORM`` is set).
    """
    try:
    ...
    with _cfg_lock:
        ...
        # platform-specific list first when INTELLECT_SESSION_PLATFORM=webui).
...
def _handle_session_import_cli(handler, body):
    ...
    try:
        from api.session_visibility import resolve_actor_member_id_for_state_db_reads
        from api.storage_bridge import get_session_db

        actor = resolve_actor_member_id_for_state_db_reads()
        if actor == "":
            return bad(handler, "Session not found in CLI store", 404)
        db = get_session_db(None)
        if db is None:
            return bad(handler, "Session not found in CLI store", 404)
        try:
            row = db.get_session(sid, actor_member_id=actor or None)
        finally:
            try:
                db.close()
            except Exception:
...
def _parse_mcp_enabled(value) -> bool:
    """Parse Intellect MCP ``enabled`` values without raising on bad config."""
    if value is None:
        return True
...
def _mcp_runtime_status_by_name() -> dict[str, dict]:
    ...
    configuration; it does not probe or spawn MCP subprocesses. If Intellect Agent
...
def _external_notes_sources_enabled(config_data: dict | None = None) -> bool:
    ...
    """
    env_value = os.getenv("INTELLECT_WEBUI_EXTERNAL_NOTES_SOURCES", "")
    if env_value:
