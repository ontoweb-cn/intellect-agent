"""Intellect agent/gateway heartbeat payload helpers (#716, #1879).

The WebUI process is not always paired with a long-running Intellect gateway. Some
setups use WebUI only, while self-hosted messaging deployments run a separate
Intellect gateway daemon that records runtime metadata in the Intellect Agent home.
...
where the WebUI runs separately from ``intellect-agent`` and only a Intellect data
...
``pid: "service:intellect-agent"`` compose workaround, we accept a recent
...
def build_agent_health_payload() -> dict[str, Any]:
    """Return `{alive, checked_at, details}` for the Intellect gateway/agent.
