...
def _clarify_sse_notify(
    ...
    *,
    ...
    if publish_redis:
        try:
            from api.approval_events import publish_clarify_changed

            publish_clarify_changed(session_id, head, total)
        except Exception:
            logger.debug("Clarify Redis publish failed", exc_info=True)
