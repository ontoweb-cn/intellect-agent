"""Intellect Web UI -- first-run onboarding helpers."""
...
_SUPPORTED_PROVIDER_SETUPS = {
    ...
    "lmstudio": {
        "label": "LM Studio",
        # Canonical env var matches the agent CLI runtime (intellect_cli/auth.py:182,
    ...
    "volcengine": {
        ...
        "requires_base_url": False,
        "models": list(_PROVIDER_MODELS.get("volcengine", [])),
        "category": "specialized",
    },
    "volcengine-coding-plan": {
        ...
        "requires_base_url": False,
        "models": list(_PROVIDER_MODELS.get("volcengine-coding-plan", [])),
        "category": "specialized",
    },
    "volcengine-agent-plan": {
        "label": "Volcano Engine Agent Plan",
        "env_var": "VOLCENGINE_AGENT_PLAN_API_KEY",
        "default_model": "",
        "default_base_url": "https://ark.cn-beijing.volces.com/api/plan",
        "requires_base_url": False,
        "models": list(_PROVIDER_MODELS.get("volcengine-agent-plan", [])),
        "category": "specialized",
    },
    "zai": {
        "label": "Z.AI / GLM (智谱)",
...
_UNSUPPORTED_PROVIDER_NOTE = (
    "Advanced provider flows such as GitHub Copilot are still "
    "terminal-first. OpenAI Codex and Anthropic Claude Code can be authenticated in this onboarding flow "
    "when your Intellect config selects the corresponding provider."
)
...
def probe_provider_endpoint(
    ...
    headers = {
        ...
        "User-Agent": "intellect-webui-onboarding-probe",
    }
    if api_key:
...
def _provider_api_key_present(
    ...
    # xai, etc.), ask the intellect_cli auth registry — it knows every provider's env
...
def _provider_oauth_authenticated(provider: str, intellect_home: "Path") -> bool:
    ...
    requested Intellect home. Known OAuth providers may store auth either in the
    legacy providers[provider_id] singleton state or in credential_pool entries
    used by current Intellect runtime auth resolution.
    """
    provider = (provider or "").strip().lower()
    ...
    try:
        from api.storage_bridge import state_storage_available  # type: ignore[import-not-found]

        if state_storage_available():
            from agent.membership import MembershipStore  # type: ignore[import-not-found]
            from agent.oauth.model_tokens import provider_has_db_token  # type: ignore[import-not-found]
            from agent.oauth.pool_storage import read_pool_entries  # type: ignore[import-not-found]
            from intellect_cli.config import load_config  # type: ignore[import-not-found]

            store = MembershipStore(config=load_config())
            try:
                if provider_has_db_token(store, provider):
                    return True
                for entry in read_pool_entries(store, provider):
...
def _status_from_runtime(cfg: dict, imports_ok: bool) -> dict:
    ...
    env_values = _load_env_file(_get_active_intellect_home() / ".env")
    ...
    if provider_configured:
        ...
        if provider in _SUPPORTED_PROVIDER_SETUPS:
            ...
            if meta.get("key_optional"):
                ...
                if not provider_ready and meta.get("oauth_provider"):
                    provider_ready = _provider_oauth_authenticated(
                        str(meta.get("oauth_provider")), _get_active_intellect_home()
                    )
        else:
            ...
            provider_ready = (
                ...
                or _provider_oauth_authenticated(provider, _get_active_intellect_home())
            )

    chat_ready = bool(_INTELLECT_FOUND and imports_ok and provider_ready)

    if not _INTELLECT_FOUND or not imports_ok:
        state = "agent_unavailable"
        note = (
            "Intellect is not fully importable from the Web UI yet. Finish bootstrap or fix the "
            "agent install before provider setup will work."
        )
    elif chat_ready:
        state = "ready"
        provider_name = _PROVIDER_DISPLAY.get(
            provider, provider.title() if provider else "Intellect"
        )
        note = f"Intellect is minimally configured and ready to chat via {provider_name}."
    elif provider_configured:
        state = "provider_incomplete"
        if provider == "custom" and not base_url:
            note = (
                "Intellect has a saved provider/model selection but still needs the "
            ...
            note = (
                ...
                "Run 'intellect auth' or 'intellect model' in a terminal to complete "
        ...
        note = "Intellect is installed, but you still need to choose a provider and save working credentials."
    ...
    return {
        ...
        "env_path": str(_get_active_intellect_home() / ".env"),
    }
...
def _build_setup_catalog(cfg: dict) -> dict:
    ...
    # of a key input when the user has already authenticated via 'intellect auth'.
    current_is_oauth = (
        current_provider not in _SUPPORTED_PROVIDER_SETUPS and bool(current_provider)
    ) or _provider_oauth_authenticated(current_provider, _get_active_intellect_home())
...
def get_onboarding_status() -> dict:
    ...
    cfg = get_config()
    imports_ok, missing, errors = verify_intellect_imports()
    runtime = _status_from_runtime(cfg, imports_ok)
    workspaces = load_workspaces()
    last_workspace = get_last_workspace()
    available_models = get_available_models()

    # INTELLECT_WEBUI_SKIP_ONBOARDING=1 lets hosting providers (e.g. Agent37) ship
    ...
    skip_env = os.environ.get("INTELLECT_WEBUI_SKIP_ONBOARDING", "").strip()
    skip_requested = skip_env in {"1", "true", "yes"}
    auto_completed = skip_requested  # unconditional: operator says skip, we skip

    # Auto-complete for existing Intellect users: if config.yaml already exists
    # AND the provider is configured (or the system is chat_ready), treat onboarding
    # as done.  These users configured Intellect via the CLI before the Web UI existed;
    ...
    # a git branch switch in the intellect-agent repo).  Without this, a CLI-configured
    ...
    return {
        ...
        "settings": {
            ...
            "bot_name": settings.get("bot_name") or "Intellect",
        },
        "system": {
            "intellect_found": bool(_INTELLECT_FOUND),
...
def apply_onboarding_setup(body: dict) -> dict:
    ...
    if provider not in _SUPPORTED_PROVIDER_SETUPS:
        # Unsupported providers (openai-codex, copilot, etc.) are already
    ...
    if Path(config_path).exists() and not body.get("confirm_overwrite"):
        return {
            "error": "config_exists",
            "message": (
                "Intellect is already configured (config.yaml exists). "
    ...
    env_path = _get_active_intellect_home() / ".env"
    ...
    if not api_key and not _provider_api_key_present(provider, cfg, env_values):
        ...
        oauth_ready = bool(provider_meta.get("oauth_provider")) and _provider_oauth_authenticated(
            str(provider_meta.get("oauth_provider")), _get_active_intellect_home()
        )
    ...
    # Reload the intellect_cli provider/config cache so the next streaming call
    # picks up the new key without requiring a server restart.
    try:
        from api.profiles import _reload_dotenv
        _reload_dotenv(_get_active_intellect_home())
    except Exception:
    ...
    try:
        # intellect_cli may cache config at import time; ask it to reload if possible.
        from intellect_cli.config import reload as _cli_reload
        _cli_reload()
    except Exception:
        logger.debug("Failed to reload intellect_cli config")
