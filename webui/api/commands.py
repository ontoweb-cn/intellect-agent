"""Expose intellect-agent's COMMAND_REGISTRY to the webui frontend.

This module is the single integration point with intellect_cli.commands.
If intellect-agent is unavailable the endpoint degrades to an empty list
...
def list_commands(_registry=None) -> list[dict[str, Any]]:
    ...
    Returns empty list if intellect_cli is not installed (graceful
    degradation -- the frontend has its own fallback minimum set).

    Args:
        _registry: Optional injected registry for testing. When None
            (production), imports COMMAND_REGISTRY from intellect_cli.
    """
    if _registry is None:
        try:
            from intellect_cli.commands import COMMAND_REGISTRY as _registry
        except ImportError:
            logger.warning("intellect_cli.commands not importable -- /api/commands returns []")
            return []
...
def execute_plugin_command(command: str) -> str:
    ...
    transport errors, matching Intellect' existing slash-command UX.
    """
