"""
Intellect Web UI -- HTTP helper functions.
"""
...
# networks, or Tailscale leaves the socket half-closed.  ssl.SSLError specifically
# catches SSL-level disconnects without swallowing unrelated OSError subtypes.
...
def _security_headers(handler):
    ...
    handler.send_header(
        ...
        "img-src 'self' data: https: blob:; font-src 'self' data: https://fonts.gstatic.com https://cdn.jsdelivr.net; connect-src 'self' https://cdn.jsdelivr.net; "
...
def _safe_write(handler, body: bytes) -> None:
    ...
    try:
        ...
        import logging
        logging.getLogger("intellect.webui").debug(
...
def login_redirect_location(parsed) -> str:
    """Build a Location header value for redirect-to-login (subpath-safe).

    Relative ``login?next=`` from ``/session/<id>`` resolves to ``/session/login``
    per RFC 3986 (same-directory relative URL), which re-enters the session shell
    and causes a redirect loop. Use ``../login`` when leaving a session deep-link.
    """
    import urllib.parse as _urlparse

    from api.members import _is_session_deeplink_path

    path = parsed.path or "/"
    path_with_query = path
    if parsed.query:
        path_with_query += "?" + parsed.query
    next_q = _urlparse.quote(path_with_query, safe="/")
    prefix = "../login" if _is_session_deeplink_path(path) else "login"
    return f"{prefix}?next={next_q}"


def redirect(handler, location: str, status: int = 302, cookies: list[str] | None = None) -> None:
    """Send an HTTP redirect with optional Set-Cookie headers."""
    handler.send_response(status)
    ...
    if cookies:
        for line in cookies:
            handler.send_header('Set-Cookie', line)
    handler.end_headers()


def j_with_cookies(handler, payload, status: int = 200, cookies: list[str] | None = None) -> None:
    """JSON response with multiple Set-Cookie lines (dict extra_headers cannot)."""
    body = _json.dumps(payload, ensure_ascii=False, indent=2).encode('utf-8')
    handler.send_response(status)
    handler.send_header('Content-Type', 'application/json; charset=utf-8')
    handler.send_header('Content-Length', str(len(body)))
...
def _build_redact_fn():
    """Return a redactor backed by intellect-agent plus local fallback patterns."""
    # Fallback mirrors the agent's known credential prefixes so WebUI API
    # responses remain a hard redaction boundary even without intellect-agent.
    # Keep this active even when intellect-agent is importable so API responses do
    ...
    def _combined_redact(text: str) -> str:
        ...
        # INTELLECT_REDACT_SECRETS opt-in. The local fallback then handles the
        # common short-prefix shapes the agent omits (ghp_, sk-, hf_, AKIA).
        try:
            agent_redacted = redact_sensitive_text(text, force=True)
        except TypeError:
            # Older intellect-agent builds that predate the force kwarg.
...
def drain_body(handler) -> None:
    """Consume request body bytes so keep-alive does not corrupt the next request.

    Idempotent: sets ``handler._body_drained`` so repeated calls are no-ops.
    ``_body_drained`` is only set to True when the body was actually consumed.
    """
...
PROFILE_COOKIE_NAME = 'intellect_profile'
