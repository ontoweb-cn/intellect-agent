"""WebUI bridge for Intellect persistent session goals."""
...
try:  # Exposed as a module attribute so tests can monkeypatch it directly.
    from intellect_cli.goals import (  # type: ignore
...
except Exception:  # pragma: no cover - depends on installed intellect-agent
...
def _default_max_turns() -> int:
    """Return the configured /goal turn budget, defaulting to Intellect' 20 turns."""
    try:
...
def _profile_db(profile_home: str | Path):
    """Return a SessionDB for *profile_home* via the storage factory.

    Scopes INTELLECT_HOME to the profile directory without mutating
    ``os.environ`` so concurrent WebUI workers stay isolated.
    """
    ...
    try:
        ...
        try:
            from intellect_state import SessionDB  # type: ignore
...
class _ProfileGoalManager:
    ...
    def __init__(self, session_id: str, *, profile_home: str | Path, default_max_turns: int = 20):
        if GoalState is None:
            raise RuntimeError("Intellect goal state unavailable")
...
def restore_goal_state(session_id: str, snapshot: Any, *, profile_home: str | Path | None = None) -> None:
    ...
    if isinstance(mgr, _ProfileGoalManager):
        ...
        return
    try:
        from intellect_cli.goals import save_goal  # type: ignore
