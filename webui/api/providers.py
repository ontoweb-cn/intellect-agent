"""Intellect Web UI -- provider management endpoints.
...
# extension — the same technique used throughout the Intellect codebase.
...
def _codex_usage_headers(access_token):
    headers = {
        ...
        "User-Agent": "codex_cli_rs/0.0.0 (Intellect WebUI)",
...
_PROVIDER_ENV_VAR: dict[str, str] = {
    ...
    "volcengine-agent-plan": "VOLCENGINE_AGENT_PLAN_API_KEY",
    "opencode-zen": "OPENCODE_ZEN_API_KEY",
    "opencode-go": "OPENCODE_GO_API_KEY",
    # NOTE: bare "ollama" (local) deliberately omitted — local Ollama is keyless
    # by default and the runtime in intellect_cli/runtime_provider.py only consumes
    ...
    # runtime reads (intellect_cli/auth.py:182, api_key_env_vars=("LM_API_KEY",)).
...
# Maps canonical provider slug → env var name for base_url.
# When set_provider_key receives a base_url for these providers, it writes to
# the corresponding env var so that the agent's
# resolve_api_key_provider_credentials() picks it up at runtime.
_PROVIDER_BASE_URL_ENV_VAR: dict[str, str] = {
    "xiaomi": "XIAOMI_BASE_URL",
    "volcengine": "VOLCENGINE_BASE_URL",
    "volcengine-coding-plan": "VOLCENGINE_CODING_PLAN_BASE_URL",
    "volcengine-agent-plan": "VOLCENGINE_AGENT_PLAN_BASE_URL",
}
...
# through the Intellect CLI, not via API keys.  The WebUI cannot set these.
...
def _get_intellect_home() -> Path:
    """Return the active Intellect home directory."""
    try:
...
def _provider_has_shadowed_codex_oauth_value(provider_id: str) -> bool:
    ...
    if env_var:
        env_path = _get_intellect_home() / ".env"
...
def _write_env_file(env_path: Path, updates: dict[str, str | None]) -> None:
    ...
    with _ENV_LOCK:
        ...
        # ``~/.intellect/.env`` is also written by ``intellect_cli.config.save_env_value``
...
def _maybe_update_active_model_base_url(provider_id: str, base_url: str) -> None:
    """When *provider_id* is the active provider, update ``model.base_url`` in config.yaml.

    ``runtime_provider.py:1617`` gives ``model.base_url`` priority over the
    provider-specific env var, so without this update a user who changes the
    URL in the WebUI settings will still see the old base_url at inference time.
    """
    try:
        import yaml
        ...
        if not isinstance(model_cfg, dict):
            return
        active_provider = str(model_cfg.get("provider") or "").strip().lower()
        if active_provider != provider_id.strip().lower():
            return
        model_cfg["base_url"] = base_url
        _save_yaml_config_file(config_path, cfg)
        reload_config()
    except Exception:
        logger.exception("Failed to update model.base_url for provider %s", provider_id)


def _save_provider_base_url(provider_id: str, base_url: str) -> None:
    """Save provider base_url to config.yaml.

    Writes to ``providers.<provider_id>.base_url`` in the config file.
    Creates the providers section if it doesn't exist.
    """
    import yaml
    ...
    if provider_id not in cfg["providers"] or not isinstance(cfg["providers"][provider_id], dict):
...
def _provider_has_key(provider_id: str) -> bool:
    ...
    Checks (in order):
    1. ``~/.intellect/.env`` for the known env var
...
def _account_usage_subprocess_env(home: Path, provider: str, api_key: str | None) -> dict[str, str]:
    env = dict(os.environ)
    env["INTELLECT_HOME"] = str(Path(home))
...
def _account_usage_cache_key(provider: str, home: Path, api_key: str | None) -> tuple[str, str, str]:
    key_fingerprint = ""
    if api_key:
        key_fingerprint = hashlib.sha256(api_key.encode("utf-8", "ignore")).hexdigest()  # codeql[py/weak-sensitive-data-hashing]: account-usage polling cache key
...
def get_provider_quota(provider_id: str | None = None, *, refresh: bool = False) -> dict[str, Any]:
    ...
    providers reuse Intellect Agent's /usage account-limits abstraction so WebUI
...
def _provider_is_oauth(provider_id: str) -> bool:
    ...
    return (provider_id or "").strip().lower() in _OAUTH_PROVIDERS
...
def _cost_snapshots_dir() -> Path:
    ...
    Uses the Intellect home directory (profile-aware) so snapshots are
    isolated per profile, matching the existing STATE_DIR convention.
    """
    return _get_intellect_home() / _COST_SNAPSHOTS_DIR_NAME
...
def _append_cost_snapshot(provider: str, usage: int | float | None, limit: int | float | None) -> list[dict[str, Any]]:
    ...
    # deployments that share one Intellect home/state directory.
...
def get_providers() -> dict[str, Any]:
    ...
    for pid in sorted(known_ids):
        ...
        if is_oauth:
            key_source = "oauth"
            # Check if actually authenticated via intellect_cli.
            ...
            # returns logged_in=False (e.g. token not in the intellect credential pool,
            ...
            try:
                ...
                # key just because the intellect_cli auth module is unavailable.
                logger.debug("intellect_cli auth check failed for %s", pid, exc_info=True)
        ...
        # The model picker already prefers intellect_cli + Codex local cache for
        ...
        if pid == "openai-codex":
            ...
            if live_models:
                models = live_models
                models_total = len(models)
        # Ontoweb Portal: prefer the live catalog so the providers card matches
        # the dropdown picker (#1538). Same fallback shape as the static-only
        # case below — when intellect_cli is unavailable or its lookup raises,
        ...
        if pid == "lmstudio":
            try:
                ...
                logger.debug("Failed to load LM Studio models from intellect_cli")
...
def set_provider_key(provider_id: str, api_key: str | None, base_url: str | None = None) -> dict[str, Any]:
    ...
    Writes the key to ``~/.intellect/.env`` using the standard env var name.
    If ``api_key`` is None or empty, the key is removed.

    For Xiaomi provider, optionally saves ``base_url`` to config.yaml
    under ``providers.xiaomi.base_url``.
    ...
    if _provider_is_oauth(provider_id):
        return {
            ...
            "error": f"'{_PROVIDER_DISPLAY.get(provider_id, provider_id)}' uses OAuth authentication. "
                     f"Use `intellect model` in the terminal to configure it.",
        }
    ...
    # Save base_url to the provider-specific env var so the agent's
    # resolve_api_key_provider_credentials() picks it up at runtime.
    # Also write to providers.<id>.base_url in config.yaml, AND update
    # model.base_url if this is the active provider (runtime_provider.py
    # gives model.base_url priority over the env var).
    if base_url:
        base_url_env = _PROVIDER_BASE_URL_ENV_VAR.get(provider_id)
        if base_url_env:
            try:
                _write_env_file(env_path, {base_url_env: base_url})
            except Exception as exc:
                logger.exception("Failed to write base_url env var for provider %s", provider_id)
        _save_provider_base_url(provider_id, base_url)
        # Also update model.base_url when this is the active provider,
        # since runtime_provider.py:1617 gives cfg_base_url priority.
        _maybe_update_active_model_base_url(provider_id, base_url)
...
def remove_provider_key(provider_id: str) -> dict[str, Any]:
    ...
    Removes the key from ``~/.intellect/.env`` (via ``set_provider_key``)
