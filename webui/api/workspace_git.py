...
WORKSPACE_GIT_DESTRUCTIVE_ENV = "INTELLECT_WEBUI_WORKSPACE_GIT_DESTRUCTIVE"
_GIT_ENV_SCRUB_KEYS = (
    "GIT_DIR",
    "GIT_WORK_TREE",
    "GIT_CONFIG_GLOBAL",
...
_INTELLECT_BRANCH_SWITCH_STASH_PREFIX = "intellect-webui branch switch"
...
def _stash_subject_parts(subject: str) -> tuple[str, str] | None:
    ...
    if not branch or not message.startswith(_INTELLECT_BRANCH_SWITCH_STASH_PREFIX):
        return None
    return branch, message


def _intellect_branch_switch_stashes(ctx: GitContext) -> list[dict]:
...
def _restore_branch_switch_stash_locked(ctx: GitContext, branch: str) -> dict:
    if _dirty_worktree(ctx):
        return {}
    for item in _intellect_branch_switch_stashes(ctx):
...
def git_stash_and_checkout(
    ...
    stash_name = f"{_INTELLECT_BRANCH_SWITCH_STASH_PREFIX} to {target_label}".strip()
    restored: dict = {}
    with _git_mutation_lock(ctx):
        _validate_checkout_request_locked(ctx, ref, mode, new_branch)
        stashed = False
...
def _selected_temp_index_env(ctx: GitContext, specs: list[str]) -> tuple[dict[str, str], str]:
    fd, index_path = tempfile.mkstemp(prefix="intellect-webui-git-index-")
    os.close(fd)
