...
def _resolve_custom_provider_runtime_overrides(
    ...
    Intellect Agent treats named custom providers as routing hints around an
...
def _prewarm_skill_tool_modules():
    ...
    in the intellect-agent package which may not be on ``sys.path`` at
...
# Lazy import to avoid circular deps -- intellect-agent is on sys.path via api/config.py
try:
...
- Match the normal Intellect messaging style; do not add extra status updates solely because this is a browser session.
...
def _prefill_script_timeout(config_data: dict) -> float:
    raw = os.getenv("INTELLECT_WEBUI_PREFILL_MESSAGES_SCRIPT_TIMEOUT", "") or str(config_data.get("webui_prefill_messages_script_timeout") or "")
    try:
...
def _load_prefill_messages_script(config_data: dict) -> dict:
    script_raw = os.getenv("INTELLECT_WEBUI_PREFILL_MESSAGES_SCRIPT", "") or config_data.get("webui_prefill_messages_script")
    if not script_raw:
...
def _load_webui_prefill_context(
    ...
    Supports the same bounded JSON-file shape used by Intellect Agent.  WebUI also
    ...
    file_raw = os.getenv("INTELLECT_PREFILL_MESSAGES_FILE", "") or str(cfg.get("prefill_messages_file") or "")
    if file_raw:
...
def _preferred_agent_display_name() -> str:
    ...
    try:
        ...
        name = ''
    return name or 'Intellect'


def _cancelled_turn_hint(agent_name: str | None = None) -> str:
    name = str(agent_name or _preferred_agent_display_name()).strip() or 'Intellect'
...
def _classify_provider_error(err_str: str, exc=None, *, silent_failure: bool = False) -> dict:
    ...
    _is_not_found = (
        # model_not_found hints mention Settings / `intellect model` below.
        '404' in err_str
        or 'not found' in _err_lower
        or 'does not exist' in _err_lower
        or 'model not found' in _err_lower
        or 'model_not_found' in _err_lower  # hint below points to Settings / `intellect model`
    ...
    if _is_quota:
        return {
            ...
            'hint': 'Your provider account is out of credits or usage. Top up, wait for the plan window to reset, or switch providers via `intellect model`.',
        }
    if _is_rate_limit:
        return {
    ...
    if _is_auth:
        return {
            ...
            'type': 'auth_mismatch',
            'hint': 'The selected model may not be supported by your configured provider or your API key is invalid. Run `intellect model` in your terminal to update credentials, then restart the WebUI.',
        }
    if _is_not_found:
        return {
...
def _aiagent_import_error_detail() -> str:
    ...
    The bare ImportError ("AIAgent not available -- check that intellect-agent is
    ...
    var if set, the sys.path entries that mention 'intellect', and the most-common
    ...
    lines = ["AIAgent not available -- check that intellect-agent is on sys.path"]
    lines.append("")
    lines.append(f"  python:  {_sys.executable}")
    agent_dir = _os.environ.get("INTELLECT_WEBUI_AGENT_DIR")
    if agent_dir:
        lines.append(f"  INTELLECT_WEBUI_AGENT_DIR: {agent_dir}")
    else:
        lines.append("  INTELLECT_WEBUI_AGENT_DIR: (not set)")

    # Show only the sys.path entries that look relevant — full sys.path is noisy.
    relevant = [p for p in _sys.path if "intellect" in p.lower() or "agent" in p.lower()]
    if relevant:
        lines.append("  sys.path entries mentioning intellect/agent:")
        for entry in relevant[:6]:
            lines.append(f"    - {entry}")
        if len(relevant) > 6:
            lines.append(f"    ... and {len(relevant) - 6} more")
    else:
        lines.append("  sys.path: (no entries mention intellect or agent)")

    lines.append("")
    lines.append("  Most common fix: install the agent in editable mode so its modules")
    lines.append("  appear on sys.path:")
    lines.append("")
    lines.append("    cd /path/to/intellect-agent")
...
def _build_agent_thread_env(profile_runtime_env: dict | None, workspace: str, session_id: str, profile_home: str) -> dict:
    ...
    **kwargs alongside an explicit TERMINAL_CWD raises TypeError before the
    ...
    env.update({
        ...
        'INTELLECT_EXEC_ASK': '1',
        'INTELLECT_SESSION_KEY': session_id,
        'INTELLECT_SESSION_ID': session_id,
        'INTELLECT_SESSION_PLATFORM': 'webui',
...
def _build_native_multimodal_message(workspace_ctx: str, msg_text: str, attachments, workspace: str, *, cfg: dict = None):
    ...
    bytes to Intellect as OpenAI-style image_url data URLs so vision-capable main
    ...
    # land in ~/.intellect/webui/attachments/<sid>/ (outside workspace_root by
...
def _fallback_title_from_exchange(user_text: str, assistant_text: str) -> Optional[str]:
    ...
    if topic_name:
        if not _contains_latin(topic_name):
            ...
            if any(k in combined for k in ('intellect', 'codex', 'ai')):
...
def _merge_display_messages_after_agent_result(previous_display, previous_context, result_messages, msg_text):
    ...
    If Intellect Agent returns a normal append-only history, append that delta to
...
def _attempt_credential_self_heal(
    ...
    Steps:
    1. Re-read ``~/.intellect/auth.json`` to pick up fresh credentials that
       may have been written by a concurrent ``intellect model`` CLI invocation.
    ...
    try:
        ...
        # 2. Evict the cached agent for this session (all member-scoped keys)
...
def _run_agent_streaming(
    ...
    *,
    ...
    old_intellect_home = None
    old_profile_env = {}

    # MCP discovery moved to AFTER the per-profile INTELLECT_HOME mutation below
    # (was here at v0.51.30) — the previous placement always read the default
    # profile's mcp_servers because os.environ['INTELLECT_HOME'] hadn't been
    # rewritten yet.
    ...
    try:
        from api.members import bind_worker_session_scope
        from api.session_visibility import capture_worker_session_scope

        bind_worker_session_scope(
            session_scope
            or capture_worker_session_scope(member_id=member_id)
        )
    except Exception:
        logger.debug("Failed to bind worker session scope", exc_info=True)
    ...
    try:
        ...
        try:
            from api.profiles import (
                ...
                get_intellect_home_for_profile,
                get_profile_runtime_env,
            )
            _profile_home_path = get_intellect_home_for_profile(getattr(s, 'profile', None))
            _profile_home = str(_profile_home_path)
            _profile_runtime_env = get_profile_runtime_env(_profile_home_path)
        except ImportError:
            _profile_home = os.environ.get('INTELLECT_HOME', '')
        ...
        _member_env_snapshot: dict[str, Optional[str]] = {}
        _worker_rt_ctx = None
        ...
        with _ENV_LOCK:
            ...
            old_session_id = os.environ.get('INTELLECT_SESSION_ID')
            old_session_platform = os.environ.get('INTELLECT_SESSION_PLATFORM')
            old_intellect_home = os.environ.get('INTELLECT_HOME')
            os.environ.update(_profile_runtime_env)
            os.environ['TERMINAL_CWD'] = str(s.workspace)
            os.environ['INTELLECT_EXEC_ASK'] = '1'
            os.environ['INTELLECT_SESSION_KEY'] = session_id
            os.environ['INTELLECT_SESSION_ID'] = session_id
            os.environ['INTELLECT_SESSION_PLATFORM'] = 'webui'
            try:
                # Set member context from the parameter passed by the request
                # thread.  push_member_runtime_env() reads thread-local storage
                # (_tls) which is NOT inherited by this worker thread, so we
                # inject the value that was captured on the request thread.
                from api.members import (
                    apply_runtime_context_to_agent,
                    build_runtime_context,
                    get_bound_runtime_context,
                    push_member_runtime_env,
                )
                _ctx = get_bound_runtime_context()
                if not _ctx and member_id:
                    _ctx = build_runtime_context(
                        member_id=member_id,
                        team_id=member_team_id,
                        project_id=member_project_id,
                    )
                _member_env_snapshot = push_member_runtime_env(_ctx)
                _worker_rt_ctx = _ctx
            except Exception:
                _member_env_snapshot = {}
                _worker_rt_ctx = None
            if _profile_home:
                os.environ['INTELLECT_HOME'] = _profile_home
                # Patch module-level caches to match the active profile.
                # _set_intellect_home() does this for process-wide switches
        ...
        # MUST run AFTER the INTELLECT_HOME mutation above — `discover_mcp_tools()`
        # reads `~/.intellect/config.yaml` via `get_intellect_home()`, which uses
        # `os.environ['INTELLECT_HOME']`.  Calling it before the mutation always
        ...
        # `_servers` by `(profile_home, name)` upstream in intellect-agent; that
        ...
        def _clarify_callback_impl(question, choices, sid, cancel_evt, put_event):
            """Bridge Intellect clarify prompts to the WebUI."""
        ...
        try:
            ...
            def on_tool(*cb_args, **cb_kwargs):
                ...
                # Modern Intellect Agent builds can call both tool_progress_callback
            ...
            # Initialize SessionDB so session_search works in WebUI sessions (W1 bridge)
            _session_db = None
            try:
            ...
            # degrades gracefully when run against an older intellect-agent build.
            ...
            # after the operator raises Intellect' global turn budget.
            ...
            try:
                ...
                if _raw_max_iterations is None and isinstance(_cfg, dict):
                    # Back-compat for older Intellect config files that used a
            ...
            # Newer intellect-agent builds accept reasoning_effort as a direct
            # string (e.g. "high") in addition to the reasoning_config dict.
            # This lets agent metadata determine the canonical effort level when
            # the profile config.yaml does not specify one explicitly.
            if 'reasoning_effort' in _agent_params and _reasoning_config is not None:
                _agent_kwargs['reasoning_effort'] = _reasoning_config.get('effort') if _reasoning_config.get('enabled') else None
            ...
            # Params added in newer intellect-agent — skip if not supported
            ...
            if ephemeral:
                ...
                try:
                    from api.members import apply_runtime_context_to_agent
                    apply_runtime_context_to_agent(agent, _worker_rt_ctx)
                except Exception:
                    logger.debug("apply_runtime_context_to_agent failed", exc_info=True)
                ...
                _cache_key = session_agent_cache_key(
                    session_id,
                    member_id or getattr(s, "member_id", None),
                )
                ...
                _sig_blob = _json.dumps([
                    ...
                    # at construction time, sourced from INTELLECT_HOME. Same-session
                    ...
                    _profile_home or '',
                    str(getattr(_worker_rt_ctx, "project_id", None) or ""),
                    str(getattr(_worker_rt_ctx, "team_id", None) or ""),
                ], sort_keys=True)
                _agent_sig = _hashlib.sha256(_sig_blob.encode()).hexdigest()[:16]

                agent = None
                with SESSION_AGENT_CACHE_LOCK:
                    _cached = SESSION_AGENT_CACHE.get(_cache_key)
                    if _cached and _cached[1] == _agent_sig:
                        agent = _cached[0]
                        SESSION_AGENT_CACHE.move_to_end(_cache_key)
                ...
                if agent is not None:
                    try:
                        _s_check = get_session(session_id)
                        _owner = getattr(_s_check, "member_id", None)
                        if member_id and _owner and str(_owner) != str(member_id):
                            agent = None
                            with SESSION_AGENT_CACHE_LOCK:
                                SESSION_AGENT_CACHE.pop(_cache_key, None)
                    except KeyError:
                ...
                if agent is not None:
                    ...
                    _evicted_items = []
                    with SESSION_AGENT_CACHE_LOCK:
                        SESSION_AGENT_CACHE[_cache_key] = (agent, _agent_sig)
                        SESSION_AGENT_CACHE.move_to_end(_cache_key)
            ...
            # (matches intellect-agent CLI behavior — passes via ephemeral_system_prompt)
            ...
            _external_state_messages = get_state_db_session_messages(getattr(s, 'session_id', None), member_id=getattr(s, 'member_id', None))
            ...
            with _agent_lock:
                    ...
                    if _is_quota:
                        ...
                        if not _assistant_added:
                            ...
                            _err_hint = (
                                ...
                                'your API key is invalid. Run `intellect model` in your terminal to '
                ...
                if not getattr(s, 'context_length', 0):
                    try:
                        ...
                        # Older intellect-agent builds whose get_model_context_length
                ...
                if not ephemeral:
                    ...
                    # every completed turn.  This mirrors Intellect CLI where
            ...
            if not usage.get('context_length'):
                try:
                    ...
                    try:
                        ...
                        # Older intellect-agent builds: fall back to legacy 2-arg form.
                        _fb_cl = _get_cl(
                            getattr(agent, 'model', resolved_model or '') or '',
                            getattr(agent, 'base_url', '') or '',
                        )
            ...
            # /goal parity: after a successful assistant turn, run the Intellect
            ...
            with _ENV_LOCK:
                ...
                if old_exec_ask is None: os.environ.pop('INTELLECT_EXEC_ASK', None)
                else: os.environ['INTELLECT_EXEC_ASK'] = old_exec_ask
                if old_session_key is None: os.environ.pop('INTELLECT_SESSION_KEY', None)
                else: os.environ['INTELLECT_SESSION_KEY'] = old_session_key
                if old_session_id is None: os.environ.pop('INTELLECT_SESSION_ID', None)
                else: os.environ['INTELLECT_SESSION_ID'] = old_session_id
                if old_session_platform is None: os.environ.pop('INTELLECT_SESSION_PLATFORM', None)
                else: os.environ['INTELLECT_SESSION_PLATFORM'] = old_session_platform
                if old_intellect_home is None: os.environ.pop('INTELLECT_HOME', None)
                else: os.environ['INTELLECT_HOME'] = old_intellect_home
                try:
        ...
        # The user hint still points to Settings / `intellect model` from _classify_provider_error().
        if _exc_is_quota:
            ...
            _exc_label, _exc_type, _exc_hint = (
                ...
                'Run `intellect model` in your terminal to switch providers, then restart the WebUI.',
            )
...
def _handle_chat_steer(handler, body: dict) -> bool:
    ...
    try:
        ...
        return j(handler, {"accepted": False, "fallback": "session_not_found",
                           "stream_id": None})
    from api.config import session_agent_cache_key

    _steer_key = session_agent_cache_key(sid, getattr(s_meta, "member_id", None))
    with _cfg.SESSION_AGENT_CACHE_LOCK:
        cached = _cfg.SESSION_AGENT_CACHE.get(_steer_key)
        if not cached:
    ...
    if not hasattr(agent, "steer"):
        # Older intellect-agent that pre-dates the steer() method
