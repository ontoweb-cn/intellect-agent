"""Intellect Web UI -- startup helpers."""
...
def fix_credential_permissions() -> None:
    """Ensure sensitive files in INTELLECT_HOME have safe permissions.

    Respects:
      - INTELLECT_SKIP_CHMOD=1  → bypass entirely
      - INTELLECT_HOME_MODE     → group bits are allowed if set by the operator,
                               only world-readable/world-writable files are fixed
    """
    if os.environ.get('INTELLECT_SKIP_CHMOD', '').strip() in ('1', 'true'):
        return

    # Parse operator-declared mode to know if group bits are intentional
    declared_mode = None
    raw_mode = os.environ.get('INTELLECT_HOME_MODE', '').strip()
    if raw_mode:
        try:
    ...
    intellect_home = Path(os.environ.get('INTELLECT_HOME', str(Path.home() / '.intellect')))
    if not intellect_home.is_dir():
        return
    for name in _SENSITIVE_FILES:
        fpath = intellect_home / name
        if not fpath.exists():
            continue
        try:
...
def _agent_dir() -> Path | None:
    intellect_home = Path(os.environ.get('INTELLECT_HOME', str(Path.home() / '.intellect')))
    for raw in [os.environ.get('INTELLECT_WEBUI_AGENT_DIR', '').strip(), str(intellect_home / 'intellect-agent')]:
        if not raw:
            continue
...
def _trusted_agent_dir(agent_dir: Path) -> bool:
    ...
    the dir to be ~/.intellect/intellect-agent), so custom INTELLECT_WEBUI_AGENT_DIR
    paths work correctly when INTELLECT_WEBUI_AUTO_INSTALL=1 is set.
    """
    try:
        st = agent_dir.stat()
...
def auto_install_agent_deps() -> bool:
    enabled = os.environ.get('INTELLECT_WEBUI_AUTO_INSTALL', '').strip().lower() in ('1', 'true', 'yes')
    if not enabled:
        print('[!!] Auto-install disabled. Set INTELLECT_WEBUI_AUTO_INSTALL=1 to enable.', flush=True)
        return False
