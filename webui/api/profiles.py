"""
Intellect Web UI -- Profile state management.
Wraps intellect_cli.profiles to provide profile switching for the web UI.

The web UI maintains a process-level "active profile" that determines which
INTELLECT_HOME directory is used for config, skills, memory, cron, and API keys.
Profile switches update os.environ['INTELLECT_HOME'] and monkey-patch module-level
cached paths in intellect-agent modules (skills_tool, skill_manager_tool,
cron/jobs) that snapshot INTELLECT_HOME at import time.
"""
import json
import logging
import os
...
# ── Constants (match intellect_cli.profiles upstream) ─────────────────────────
...
# reads its own profile from the intellect_profile cookie instead of the
...
def snapshot_skill_home_modules() -> dict[str, dict[str, object]]:
    ...
    for module_name in _SKILL_HOME_MODULES:
        ...
        snapshot[module_name] = {
            "module_present": True,
            "has_INTELLECT_HOME": hasattr(module, "INTELLECT_HOME"),
            "INTELLECT_HOME": getattr(module, "INTELLECT_HOME", None),
...
def patch_skill_home_modules(home: Path) -> None:
    """Patch imported skill modules that cache INTELLECT_HOME at import time."""
    for module_name in _SKILL_HOME_MODULES:
        module = sys.modules.get(module_name)
        if module is None:
            continue
        try:
            module.INTELLECT_HOME = home
...
def restore_skill_home_modules(snapshot: dict[str, dict[str, object]]) -> None:
    ...
    for module_name, values in snapshot.items():
        ...
        if module is None:
            continue
        for attr in ("INTELLECT_HOME", "SKILLS_DIR"):
...
def _resolve_base_intellect_home() -> Path:
    ...
    if intellect_home:
        p = Path(intellect_home).expanduser()
        # If INTELLECT_HOME points to a profiles/ subdir, walk up two levels to the base
        return _unwrap_profile_home_to_base(p)

    # Platform default. On Windows this includes the #2905 migration-safety
    # fallback (prefer the populated legacy %USERPROFILE%\.intellect over an
    # empty %LOCALAPPDATA%\intellect). Delegate to config so the base-home
    # resolution used for the active-profile pointer can never drift from the
    # one STATE_DIR is derived from.
    try:
        from api.config import _platform_default_intellect_home
        return _platform_default_intellect_home()
    except Exception:
        # Defensive: never let a config import problem break profile resolution.
        return Path.home() / '.intellect'

_DEFAULT_INTELLECT_HOME = _resolve_base_intellect_home()
...
#
# Intellect Agent allows the root/default profile (~/.intellect itself) to have a
# display name other than the legacy literal 'default'.  When that happens,
# WebUI must NOT resolve the display name as ~/.intellect/profiles/<name> — that
# directory doesn't exist, and every site that does `if name == 'default':`
# will fall through to the wrong filesystem path.
#
# `_is_root_profile(name)` answers "does this name resolve to ~/.intellect?" and
# is the canonical replacement for scattered `if name == 'default':` checks
# in switch_profile, get_active_intellect_home, _validate_profile_name, etc.
#
# Cost note: list_profiles_api() shells out via intellect_cli (non-trivial), so
...
def _is_root_profile(name: str) -> bool:
    """True if *name* resolves to the Intellect Agent root profile (~/.intellect).
    ...
    # avoid holding it across a intellect_cli subprocess call.
    try:
...
def is_profile_management_enabled() -> bool:
    ...
    try:
        ...
        # Agent tree unavailable (minimal checkout): do not block WebUI.
        return True
...
def get_active_profile_name() -> str:
    ...
    Priority:
      1. Thread-local (set per-request from intellect_profile cookie) — issue #798
      2. Process-level default (_active_profile)

    While profile management is disabled, always ``default`` so cookies cannot
    switch INTELLECT_HOME away from the primary instance.
    """
    if not is_profile_management_enabled():
        return "default"
...
def set_request_profile(name: str) -> None:
    ...
    Called by server.py at the start of each request when a intellect_profile
    ...
    TEMPORARY: ignore non-default cookies while profile management is disabled.
    """
    if not is_profile_management_enabled():
        name = "default"
    _tls.profile = name
...
def _resolve_profile_home_for_name(name: str) -> Path:
    """Resolve a logical profile name to its Intellect home path.

    Root/default aliases resolve to _DEFAULT_INTELLECT_HOME.  Valid named profiles
    resolve to _DEFAULT_INTELLECT_HOME/profiles/<name> even when the directory has
    ...
    if not name or _is_root_profile(name):
        return _DEFAULT_INTELLECT_HOME
    if not _PROFILE_ID_RE.fullmatch(name):
        return _DEFAULT_INTELLECT_HOME
    return _resolve_named_profile_home(name)


def get_active_intellect_home() -> Path:
    """Return the INTELLECT_HOME path for the currently active profile.
...
# `cron.jobs` reads INTELLECT_HOME from os.environ (process-global) at function-
...
# This context manager swaps INTELLECT_HOME (and the cached module-level constants
...
# all webui access to INTELLECT_HOME goes through one thread at a time. Any
...
# (own-profile) INTELLECT_HOME, never a half-swapped state.
...
def _home_for_scheduled_cron_job(job: dict) -> Path:
    ...
    Jobs pinned to a named profile execute under that profile's INTELLECT_HOME, so
...
def install_cron_scheduler_profile_isolation() -> None:
    ...
    INTELLECT_HOME isolation as the manual /api/crons/run path.
    """
    try:
...
class cron_profile_context_for_home:
    """Context manager that pins INTELLECT_HOME to an explicit profile home path.
    ...
    def __enter__(self):
        ...
        try:
            ...
            try:
                ...
                self._prev_cj = (_cj.INTELLECT_DIR, _cj.CRON_DIR, _cj.JOBS_FILE, _cj.OUTPUT_DIR)
                _cj.INTELLECT_DIR = self._home
            ...
            # cron.scheduler snapshots _intellect_home at import time and run_job()
            ...
            try:
                import cron.scheduler as _cs
                self._prev_cs = (
                    getattr(_cs, '_intellect_home', None),
                    getattr(_cs, '_LOCK_DIR', None),
                    getattr(_cs, '_LOCK_FILE', None),
                )
                _cs._intellect_home = self._home
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        try:
            if self._prev_env is None:
                os.environ.pop('INTELLECT_HOME', None)
            else:
                os.environ['INTELLECT_HOME'] = self._prev_env
            if self._prev_cj is not None:
                try:
                    import cron.jobs as _cj
                    _cj.INTELLECT_DIR, _cj.CRON_DIR, _cj.JOBS_FILE, _cj.OUTPUT_DIR = self._prev_cj
                except (ImportError, AttributeError):
                    pass
...
class cron_profile_context:
    """Context manager that pins INTELLECT_HOME to the TLS-active profile.
    ...
    def __enter__(self):
        ...
        try:
            self._prev_env = os.environ.get('INTELLECT_HOME')
            home = get_active_intellect_home()
            os.environ['INTELLECT_HOME'] = str(home)
...
def get_intellect_home_for_profile(name: str) -> Path:
    """Return the INTELLECT_HOME Path for *name* without mutating any process state.
    ...
    Falls back to _DEFAULT_INTELLECT_HOME (same as 'default') when *name* is None,
...
def get_profile_runtime_env(home: Path) -> dict[str, str]:
    ...
    environment variables (matching ``intellect -p <profile>``), so streaming must
...
def profile_env_for_background_worker(
    ...
    try:
        ...
        profile_home_path = Path(get_intellect_home_for_profile(profile))
    ...
    thread_env["INTELLECT_HOME"] = str(profile_home_path)
    ...
    # worker body because several production Intellect readers still call
    ...
    try:
        ...
        with _ENV_LOCK:
            ...
            if had_intellect_home:
                os.environ["INTELLECT_HOME"] = old_intellect_home or ""
            else:
                os.environ.pop("INTELLECT_HOME", None)
...
def _set_intellect_home(home: Path):
    """Set INTELLECT_HOME env var and monkey-patch cached module-level paths."""
    os.environ['INTELLECT_HOME'] = str(home)
...
def switch_profile(name: str, *, process_wide: bool = True) -> dict:
    ...
    # Process-wide profile switches mutate INTELLECT_HOME, module-level path caches,
...
def list_profiles_api() -> list:
    ...
    try:
        ...
        # intellect_cli not available -- return just the default
...
def _default_profile_dict() -> dict:
    """Fallback profile dict when intellect_cli is not importable."""
    return {
        'name': 'default',
        'path': str(_DEFAULT_INTELLECT_HOME),
        'is_default': True,
        'is_active': True,
        'gateway_running': False,
        'model': None,
        'provider': None,
        'has_env': (_DEFAULT_INTELLECT_HOME / '.env').exists(),
        'skill_count': 0,
    }
...
def create_profile_api(name: str, clone_from: str = None,
    ...
    # TEMPORARY: profile creation disabled via profiles.management_enabled.
    _require_profile_management_enabled()
    _validate_profile_name(name)
    ...
    try:
        from intellect_cli.profiles import create_profile
        create_profile(
            name,
    ...
    # When intellect_cli is not importable, list_profiles_api() also falls back
...
def delete_profile_api(name: str) -> dict:
    ...
    try:
        from intellect_cli.profiles import delete_profile
