...
function _renderOnboardingProviderOAuthField(provider){
  ...
  return `<div class="onboarding-oauth-card onboarding-oauth-pending" style="margin-top:12px">
    ...
    <div style="flex:1">
      ...
      <p style="margin-top:6px;color:var(--muted);font-size:13px"><strong>Claude Code subscription credentials are not the same as an Anthropic API key.</strong> Use this path only when you want Intellect to use Claude Code credentials already available on the server, or start a short polling flow while you complete <code>claude setup-token</code> on the host.</p>
...
function _renderOnboardingBody(){
  ...
  if(key==='system'){
    const intellectOk=system.intellect_found&&system.imports_ok;
    const setupOk=!!system.chat_ready;
    _setOnboardingNotice(system.provider_note|| (setupOk?t('onboarding_notice_system_ready'):t('onboarding_notice_system_unavailable')),setupOk?'success':(intellectOk?'info':'warn'));
    body.innerHTML=`
      <div class="onboarding-panel-grid">
        <div class="onboarding-check ${intellectOk?'ok':'warn'}"><strong>${t('onboarding_check_agent')}</strong><span>${intellectOk?t('onboarding_check_agent_ready'):t('onboarding_check_agent_missing')}</span></div>
...
async function _saveOnboardingDefaults(){
  ...
  try{localStorage.setItem('intellect-webui-model',model)}catch{}
...
async function _pollCodexOAuth(){
  ...
  try{
    ...
    _setCodexOAuthButton(true);
    if(status==='success'){
      _renderCodexOAuthTerminal('success','Credentials saved to the Intellect credential pool. Refreshing provider status…');
...
async function _pollAnthropicOAuth(){
  ...
  try{
    ...
    _setAnthropicOAuthButton(true);
    if(status==='success'){
      _renderAnthropicOAuthTerminal('success','Intellect is now linked to Claude Code credentials. Refreshing provider status…');
...
async function startAnthropicOAuth(){
  ...
  flowDiv.style.display='block';
  flowDiv.innerHTML=`<div class="onboarding-oauth-card onboarding-oauth-pending"><div class="onboarding-oauth-icon">⏳</div><div><strong>Checking Claude Code credentials…</strong><p>Intellect is checking for existing Claude Code OAuth credentials on this server.</p></div></div>`;
  try{
    ...
    flowDiv.innerHTML=`
      <div class="onboarding-oauth-card onboarding-oauth-pending">
        ...
        <div style="flex:1">
          ...
          <p style="margin-top:6px">${esc(action_required||"Run 'claude setup-token' on the server, then return here. Intellect will detect the credential automatically.")}</p>
