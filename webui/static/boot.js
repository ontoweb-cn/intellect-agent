(function(){
  ...
  localStorage.removeItem('intellect-webui-server-stopped');
  // Listen for shutdown broadcast from other tabs
  try {
    var _stopChan = new BroadcastChannel('intellect-webui-shutdown');
...
(function(){
  ...
  function _setRecording(on){
    ...
    _setButtonTooltip(btn, on ? (typeof _rawAudioMode!=='undefined'&&_rawAudioMode ? t('voice_recording_active') : t('voice_dictate_active')) : t('voice_dictate'));
...
$('btnNewChat').onclick=async()=>{
  ...
  try{
    ...
    if(typeof showToast==='function') showToast((e&&e.message)||'Failed to create new conversation');
  }
};
$('btnDownload').onclick=()=>{
  if(!S.session)return;
...
$('msg').addEventListener('keydown',e=>{
  ...
  if(e.key==='Enter'){
    if(_isImeEnter(e)){return;}
    // #3076: only force mobile Enter-as-newline when the device has NO fine
    // pointer (mouse/trackpad). Tablets with physical keyboards and stylus-
    // equipped devices often match (pointer:coarse) AND (pointer:fine)
    // simultaneously — they should keep desktop Enter=sends behavior.
    const _mobileDefault=matchMedia('(pointer:coarse)').matches&&!matchMedia('(pointer:fine)').matches&&window._sendKey==='enter'&&_isVirtualKeyboardLikelyOpen();
...
document.addEventListener('keydown',async e=>{
  ...
  // P5-12: Ctrl+` (backtick) toggles between chat and last-used panel
  if(e.key==='`'&&(e.metaKey||e.ctrlKey)&&!S.busy){
    e.preventDefault();
    if(_currentPanel!=='chat'){switchPanel('chat');}else if(_lastUsedPanel&&_lastUsedPanel!=='chat'){switchPanel(_lastUsedPanel);}
    return;
  }
  if(e.key==='Escape'){
  ...
  // P4-2: Arrow key navigation in session sidebar — Up/Down to move between
  // sessions, Enter to select. Only active when the sidebar search input
  // is focused (so normal page scrolling is unaffected).
  if((e.key==='ArrowDown'||e.key==='ArrowUp')&&document.activeElement===$('sessionSearch')&&!e.ctrlKey&&!e.metaKey){
    const items=document.querySelectorAll('.session-item:not([style*=\"display: none\"])');
    if(!items.length) return;
    let idx=Array.from(items).findIndex(el=>el.classList.contains('session-item-focused'));
    if(idx===-1) idx=0;
    else idx=e.key==='ArrowDown'?Math.min(idx+1,items.length-1):Math.max(idx-1,0);
    items.forEach(el=>el.classList.remove('session-item-focused'));
    items[idx].classList.add('session-item-focused');
    items[idx].scrollIntoView({block:'nearest'});
    e.preventDefault();
  }
  if(e.key==='Enter'&&document.activeElement===$('sessionSearch')){
    const focused=document.querySelector('.session-item-focused');
...
(async()=>{
  ...
  // Update profile chip label immediately (chip hidden when management disabled)
  const profileLabel=$('profileChipLabel');
  if(profileLabel) profileLabel.textContent=S.activeProfile||'default';
  // Fetch available models without blocking session restore. The static HTML
  // options are enough for first paint; the dynamic provider list can settle
  // after the saved session is visible.
  // #2726: wrap in a reusable helper so later calls (profile switch, saved-
  // session restore) can re-hydrate the catalog without duplicating the
  // error-handling + label-contract dance.
  const _hydrateBootModelDropdown=()=>populateModelDropdown({preferProfileDefaultOnFreshBoot:true}).then(()=>{
    // Prefer the active session's model over localStorage so the composer
    // chip always reflects the conversation you're actually viewing. Fall
    // back to the persisted model state for a fresh page with no session.
    const sessionModelState = S.session && S.session.model
      ? {model: S.session.model, model_provider: S.session.model_provider || null}
      : null;
    const savedState = sessionModelState || (
    ...
    if(savedModel && $('modelSelect')){
      const applied=(typeof _applyModelToDropdown==='function')
        ...
        : null;
      if(!applied && savedModel && typeof _ensureModelOptionInDropdown==='function'){
        _ensureModelOptionInDropdown(savedModel, $('modelSelect'));
        _applyModelToDropdown(savedModel, $('modelSelect'), savedState.model_provider||null);
      }
  ...
  // _startBootModelDropdown is the canonical hydrated-boot ready signal.
  // It is safe to call multiple times (profile switch, saved-session restore).
  // The fire-and-forget prime below runs at boot to pre-warm the catalog
  // without blocking sidebar render (#2726).
  const _startBootModelDropdown=()=>{
    window._modelDropdownReady=null;
    return _hydrateBootModelDropdown().then(()=>{
      window._modelDropdownReady=Promise.resolve();
    });
  };
  window._ensureModelDropdownReady=_startBootModelDropdown;
  // Fire-and-forget prime — must be a bare Promise.resolve so the sidebar
  // can render before /api/models returns.
  Promise.resolve(_startBootModelDropdown()).catch(()=>{});
  // Pre-load workspace list so sidebar name is correct from first render.
  // Fire-and-forget (no await) so the session list can render before the
  // API response returns — see test_boot_renders_session_list_before_workspace.
  ...
  const _onboardingReady=_bootSettings.onboarding_completed
    ?Promise.resolve(false)
    :loadOnboardingWizard();
  // Resolve actor (localStorage + status cache) before listing sessions or resuming
  // a saved session id — avoids loading the previous member's session after switch.
  if(typeof initMemberAuth==='function') await initMemberAuth();
  await Promise.all([renderSessionList(),_onboardingReady]);
  _initResizePanels();
  ...
  let urlSession=(typeof _sessionIdFromLocation==='function')?_sessionIdFromLocation():null;
  let savedLocal=(typeof getSavedWebuiSessionId==='function')?getSavedWebuiSessionId():'';
  if(
    savedLocal
    &&typeof _sessionIdVisibleInSidebar==='function'
    &&!_sessionIdVisibleInSidebar(savedLocal)
  ){
  ...
  if(
    urlSession
    &&typeof _sessionIdVisibleInSidebar==='function'
    &&!_sessionIdVisibleInSidebar(urlSession)
  ){
    if(typeof clearSavedWebuiSessionIds==='function') clearSavedWebuiSessionIds();
    if(typeof _abandonInaccessibleSession==='function') _abandonInaccessibleSession(urlSession, {});
    urlSession=null;
  }
  ...
  if(saved){
    try{
      ...
      // Apply the saved session's model to the select IMMEDIATELY using static
      // HTML options so the chip is correct on first paint, before the /api/models
      // catalog fetch even starts.
      const s = S.session; // shorthand — loadSession just populated S.session
      if(s && s.default_model){
        ...
        if(sel){
          ...
          if(existingDefaultOpt){
            ...
            // The session's model is not in the static HTML list (custom provider,
            // remote model, etc.). Inject a temporary option so it appears in the
            // chip until the live catalog hydrates and replaces it.
            const opt = document.createElement('option');
            opt.value = s.default_model;
            opt.textContent = s.default_model;
      ...
      // Re-apply the session's model after the live catalog is hydrated so the
      // composer chip and select always reflect the ACTIVE session, not the
      // localStorage-persisted default or the static HTML fallback.
      if(S.session && S.session.model){
        const _sel = $('modelSelect');
        if(_sel){
          _applyModelToDropdown(S.session.model, _sel, S.session.model_provider || null);
          if(typeof syncModelChip==='function') syncModelChip();
        }
      }
    ...
    catch(e){if(typeof clearSavedWebuiSessionIds==='function')clearSavedWebuiSessionIds();else localStorage.removeItem('intellect-webui-session');}
  }
  ...
  if(typeof startGatewaySSE==='function') startGatewaySSE();
    ...
    if (typeof membersApplyStartupQuery === 'function') {
      try { await membersApplyStartupQuery(); } catch (_) {}
    }
  }
})().catch(e=>{
...
window.addEventListener('pageshow', async (event) => {
  ...
  const _bfcacheChrome = (typeof initMemberAuth === 'function')
    ? initMemberAuth().catch(() => {})
    : Promise.resolve();
  if (typeof renderSessionList === 'function') {
    try { await Promise.all([_bfcacheChrome, renderSessionList()]); } catch (_) {}
  } else {
    try { await _bfcacheChrome; } catch (_) {}
...
async function shutdownServer() {
  const ok = await showConfirmDialog({
    title: (typeof t === 'function' ? t('settings_shutdown_confirm_title') : 'Stop Intellect WebUI'),
    message: (typeof t === 'function' ? t('settings_shutdown_confirm_message') : 'Stop the Intellect WebUI server?'),
    confirmLabel: (typeof t === 'function' ? t('settings_shutdown_confirm_btn') : 'Stop'),
    danger: true,
  });
  if (!ok) return;
  localStorage.setItem('intellect-webui-server-stopped', '1');
  try { var bc = new BroadcastChannel('intellect-webui-shutdown'); bc.postMessage('stop'); bc.close(); } catch(_) {}
  _showServerStopped();
