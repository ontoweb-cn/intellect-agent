...
function getSavedWebuiSessionId(){
  try{
    const key=memberScopedSessionStorageKey();
    if(!key) return '';
    return localStorage.getItem(key)||'';
  }catch(_){return '';}
}

function setSavedWebuiSessionId(sid){
  try{
    const key=memberScopedSessionStorageKey();
    if(!key) return;
...
function isWebuiSessionStorageKey(key){
  return !!key&&(key===WEBUI_SESSION_LS_PREFIX||key.indexOf(WEBUI_SESSION_LS_PREFIX+':')===0);
}
if(typeof window!=='undefined'){
  window.getSavedWebuiSessionId=getSavedWebuiSessionId;
  window.setSavedWebuiSessionId=setSavedWebuiSessionId;
  window.clearSavedWebuiSessionIds=clearSavedWebuiSessionIds;
  window.rememberWebuiActorMemberId=rememberWebuiActorMemberId;
  window._resetSessionsForMemberContextChange=_resetSessionsForMemberContextChange;
}
...
function _isSessionActivelyViewedForList(sid) {
  ...
  // Allow SSE reconnect even when the page is not focused — background
  // tabs/panes still need to stay in sync.  The visibility check is still
  // enforced below because hidden tabs can throttle their poll interval.
...
// ── Session attention badges (#3205) ──────────────────────────────────────
// When the server returns s.attention (kind + severity + count), render a
// small coloured badge in the sidebar row so the user can see at a glance
// which sessions need approval or clarification.
function _sessionAttentionState(s) {
  if (!s || !s.attention || !s.attention.kind) return null;
  var a = s.attention;
  var labels = {
    approval:   (typeof t === 'function' ? t('session_attention_approval', a.count || 1) : 'Approval'),
    clarify:    (typeof t === 'function' ? t('session_attention_clarify', a.count || 1) : 'Clarify'),
    attention:  (typeof t === 'function' ? t('session_attention_generic', a.count || 1) : 'Attention'),
  };
  var titles = {
    approval:   (typeof t === 'function' ? t('session_attention_approval_title', a.count || 1) : 'Pending approval'),
    clarify:    (typeof t === 'function' ? t('session_attention_clarify_title', a.count || 1) : 'Needs clarification'),
    attention:  (typeof t === 'function' ? t('session_attention_generic_title', a.count || 1) : 'Needs attention'),
  };
  return {
    kind: a.kind,
    severity: a.severity || 'attention',
    count: a.count || 1,
    label: labels[a.kind] || labels.attention,
    title: titles[a.kind] || titles.attention,
  };
}
...
function clearChatUiForSignOut(){
  clearSavedWebuiSessionIds({allMembers:true});
  rememberWebuiActorMemberId(null);
...
async function newSession(flash, options={}){
  ...
  setSavedWebuiSessionId(S.session.session_id);
...
async function loadSession(sid, options={}){
  ...
  try {
    ...
    if(_msgInner){
      if(e.status===404){
        const inaccessibleMsg='Session not available in web UI.';
        const savedMatch=getSavedWebuiSessionId()===sid;
        const deeplinkSid=(typeof _sessionIdFromLocation==='function')?_sessionIdFromLocation():null;
        const deeplink=deeplinkSid===sid;
        if(savedMatch&&typeof clearSavedWebuiSessionIds==='function') clearSavedWebuiSessionIds();
        if(deeplink||savedMatch){
          if(typeof _abandonInaccessibleSession==='function'){
            _abandonInaccessibleSession(sid, {toast: inaccessibleMsg});
          }else if(_msgInner){
            _msgInner.innerHTML='<div style="display:flex;align-items:center;justify-content:center;height:100%;color:var(--text-muted);font-size:14px;padding:40px;text-align:center;">'+inaccessibleMsg+'</div>';
          }
  ...
  if (_rejectSessionIfWrongMember(data.session, sid)) return;
  ...
  if(INFLIGHT[sid]){
    ...
    try {
      ...
      console.error('_ensureMessagesLoaded failed (INFLIGHT path) for sid='+sid+':', e&&e.message, e);
    }
    ...
    try {
      ...
      console.error('_ensureMessagesLoaded failed (idle path) for sid='+sid+':', errMsg, e);
      const _msgInner = $('msgInner');
      if (_msgInner) {
      ...
      if (typeof showToast === 'function') showToast('Failed to load conversation messages: ' + errMsg, 3000, 'error');
      if (_loadingSessionId === sid) _loadingSessionId = null;
      return;
    }
...
async function _loadOlderMessages() {
  ...
  try {
    ...
    // W4 C4: under virt, do not expand the legacy tail window — data is in
    // memory and the variable-height window will pan to include new rows.
    if(typeof _isTranscriptVirtualWindowEnabled==='function'&&_isTranscriptVirtualWindowEnabled()){
      if(typeof _messageVirtForceStart!=='undefined'){
        // Keep showing from near the previous top of the in-memory transcript.
...
/** True when path is /session/<id> (not login/manifest/static subpaths). */
function isSessionDeeplinkPath(path){
  if(!path) return false;
  const p=String(path).split('?')[0];
  if(p.indexOf('/session/')!==0) return false;
  const tail=p.slice('/session/'.length).replace(/^\/+|\/+$/g,'');
...
if(typeof window!=='undefined') window.isSessionDeeplinkPath=isSessionDeeplinkPath;
  ...
  deleteBtn.onclick=async()=>{
    ...
    try{
      ...
      if(S.session&&ids.includes(S.session.session_id)){
        S.session=null;S.messages=[];S.entries=[];clearSavedWebuiSessionIds();
...
function _abandonInaccessibleSession(sid, opts={}){
  ...
  if(typeof clearSavedWebuiSessionIds==='function') clearSavedWebuiSessionIds();
  else try{localStorage.removeItem(memberScopedSessionStorageKey());}catch(_){}
  if(INFLIGHT&&sid&&INFLIGHT[sid]){
    delete INFLIGHT[sid];
    if(typeof clearInflightState==='function') clearInflightState(sid);
  }
  if(_loadingSessionId===sid) _loadingSessionId=null;
...
const _activeSessionExternalRefreshMs = 30000;  // #3107: throttle from 5s to 30s to reduce idle load
...
function ensureActiveSessionExternalRefreshPoll(){
  ...
  if(typeof document !== 'undefined' && !document._intellectExternalRefreshVisibilityHook){
    document.addEventListener('visibilitychange', () => {
      if(!document.hidden) void refreshActiveSessionIfExternallyUpdated('visible');
    });
    document._intellectExternalRefreshVisibilityHook = true;
  }
  if(typeof window !== 'undefined' && !window._intellectExternalRefreshFocusHook){
    window.addEventListener('focus', () => { void refreshActiveSessionIfExternallyUpdated('focus'); });
    window._intellectExternalRefreshFocusHook = true;
  }
}
...
function ensureSessionEventsSSE(){
  if(typeof document !== 'undefined' && !document._intellectSessionEventsVisibilityHook){
    document.addEventListener('visibilitychange', () => {
      if(!document.hidden){
        ensureSessionEventsSSE();
        void refreshSessionList('visible');
      }
      // When going hidden, keep the SSE connection alive so background tabs
      // continue receiving session-changed events. Closing on hide and
      // re-opening on show caused missed updates and unnecessary reconnect
      // delays every time the user switched tabs.
    });
    document._intellectSessionEventsVisibilityHook = true;
  }
...
function filterSessions(){
  ...
  // Show/hide the clear button based on whether the filter has content
  ...
  _searchDebounceTimer = setTimeout(async () => {
    const requestedQ = q;
    try {
      const data = await api(`/api/sessions/search?q=${encodeURIComponent(requestedQ)}&mode=fts&content=1&depth=5&limit=20`);
      ...
      _contentSearchResults = (data.sessions||[]).filter(s => (s.match_type === 'content' || s.match_type === 'fts') && !titleIds.has(s.session_id));
      renderSessionListFromCache();
    } catch(e) { /* ignore */ }
  }, 350);
}
...
function renderSessionListFromCache(){
  ...
  // TEMPORARY: "show sessions from other profiles" disabled while
  // profiles.management_enabled is false (server ignores ?all_profiles=1 too).
  // Profile filter toggle was here; re-enable with profiles.management_enabled.
  ...
  function _renderOneSession(s, isPinnedGroup=false){
    ...
    title.title=readOnly
      ? (typeof t==='function'?t('session_readonly_title_hint',displayTitle):'Read-only imported session — '+displayTitle)
      : ('Double-click to rename  ·  Click to open  ·  Right-click for options');
    const tsMs=_sessionTimestampMs(s);
    const ts=document.createElement('span');
    // Attention badge: highlight sessions with pending approvals or clarify prompts
    const attention=_sessionAttentionState(s);
    const hasAttentionState=isStreaming||hasUnread||!!attention;
    ...
    if(segmentCount>0){
      ...
      segmentCountEl.title=(typeof t==='function'
        ?(canExpandLineageSegments?t('session_lineage_toggle_hint',segmentLabel):t('session_lineage_static_hint',segmentLabel))
        :segmentLabel);
    ...
    if(childCount>0){
      ...
      childCountEl.title=(typeof t==='function'?t('session_child_toggle_hint',childLabel):childLabel);
    ...
    // Attention badge — lives after the timestamp
    if(attention){
      var badge = document.createElement('span');
      badge.className = 'session-attention-badge session-attention-badge-' + attention.kind;
      badge.textContent = attention.label;
      badge.title = attention.title;
      el.classList.add('needs-attention', 'attention-' + attention.kind);
...
async function _handleActiveSessionStorageEvent(e){
  if(!e || !isWebuiSessionStorageKey(e.key)) return;
