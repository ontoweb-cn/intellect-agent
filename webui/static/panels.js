...
const APP_TITLEBAR_KEYS = {
  ...
  profiles: 'tab_profiles', todos: 'tab_todos', wiki: 'tab_wiki', journey: 'tab_journey', insights: 'tab_insights', logs: 'tab_logs', settings: 'tab_settings',
};
...
async function switchPanel(name, opts = {}) {
  ...
  if (prevPanel === 'wiki' && nextPanel !== 'wiki') {
    if (typeof _wikiStopBuildPoll === 'function') _wikiStopBuildPoll();
  }
  if (_currentPanel !== 'chat') _lastUsedPanel = _currentPanel;
  ...
  if (mainEl) {
    ['settings','skills','memory','tasks','kanban','workspaces','profiles','wiki','journey','insights','logs','todos'].forEach(p => {
  ...
  // TEMPORARY: Profiles sidebar/panel hidden while profile management is disabled.
  ...
  if (nextPanel === 'journey' && typeof loadJourney === 'function') await loadJourney(true);
...
function _cronGatewayNoticeHtml(status) {
  ...
  const body = notConfigured
    ? 'In Intellect WebUI, scheduled jobs require the Intellect gateway daemon. If this is a single-container Docker install, jobs can be created and run manually here, but scheduled ticks need a gateway container or `intellect gateway` running outside the WebUI.'
    : 'In Intellect WebUI, scheduled jobs require the Intellect gateway daemon to be running. Start the gateway container or `intellect gateway` before relying on offline scheduled runs.';
  const docsHref = 'https://gitee.com/ontoweb/intellect-webui/blob/master/docs/docker.md#scheduled-jobs-and-the-gateway-daemon';
...
async function loadInsights(animate) {
  ...
  try {
    const [data, wikiStatus, buildStatus, analytics] = await Promise.all([
      api(`/api/insights?days=${period}`),
      api('/api/wiki/status').catch(err => ({status:'error', error: err.message || String(err)})),
      api('/api/wiki/build/status').catch(() => ({})),
      api('/api/analytics').catch(() => null),
      _loadVaultConfig(),
    ]);
    await _loadSearchConfig();
    _renderInsights(data, box, wikiStatus, buildStatus, analytics);
    if (typeof _syncSystemHealthMonitorVisibility === 'function') _syncSystemHealthMonitorVisibility();
...
async function _toggleLlmWiki(enabled) {
  try {
    var r = await api('/api/wiki/toggle', {method:'POST', body:JSON.stringify({enabled:!!enabled})});
    if (r && r.ok) {
      if (typeof showToast === 'function') showToast(enabled ? 'LLM Wiki enabled' : 'LLM Wiki disabled', 2000);
      setTimeout(function(){ if(typeof loadInsights==='function') loadInsights(); }, 500);
    } else {
      if (typeof showToast === 'function') showToast('Failed to toggle LLM Wiki' + (r && r.error ? ': ' + r.error : ''), 4000);
    }
  } catch(e) {
    if (typeof showToast === 'function') showToast('Failed to toggle LLM Wiki: ' + (e.message || String(e)), 4000);
  }
}
window._toggleLlmWiki = _toggleLlmWiki;

// ── SearXNG search settings (module scope — called from loadInsights) ──
var _searchConfig = null;

async function _loadSearchConfig() {
  try { _searchConfig = await api("/api/search/config"); } catch(_) { _searchConfig = {enabled:false, url:""}; }
}

function _renderSearchSettings() {
  var sc = _searchConfig || {enabled: false, url: ""};
  var html = "<section class='insights-card' id='searxngSettings'>";
  html += "<div class='insights-card-title'>" + (typeof t==="function"?t("settings_search_title"):"Built-in Web Search") + "</div>";
  html += "<div class='wiki-status-note'>" + (typeof t==="function"?t("settings_search_desc"):"Self-hosted SearXNG meta-search engine. No API key required.") + "</div>";
  html += "<div style='display:flex;align-items:center;gap:8px;margin-top:8px'>";
  html += "<label style='font-size:12px;display:flex;align-items:center;gap:4px;cursor:pointer'>";
  html += "<input type='checkbox' id='searxngEnabled' " + (sc.enabled?"checked":"") + " onchange='_saveSearchConfig()'>";
  html += (typeof t==="function"?t("settings_search_enable"):"Enable SearXNG search") + "</label>";
  html += "</div>";
  html += "<div style='margin-top:4px'>";
  html += "<input type='text' id='searxngUrl' value='" + esc(sc.url) + "' placeholder='https://searxng.example.com' style='width:100%;font-size:12px;padding:4px 8px;border:1px solid var(--border-muted);border-radius:var(--radius-sm);background:var(--input-bg);color:var(--text)' onchange='_saveSearchConfig()'>";
  html += "</div>";
  html += "<div style='margin-top:6px;display:flex;align-items:center;gap:8px'>";
  html += "<button class='sm-btn' style='font-size:11px' onclick='void _probeSearxng()'>" + (typeof t==="function"?t("settings_search_test"):"Test Connection") + "</button>";
  html += "<span id='searxngProbeStatus' style='font-size:11px'></span>";
  html += "</div></section>";
  return html;
}

async function _saveSearchConfig() {
  var enabled = document.getElementById("searxngEnabled");
  var url = document.getElementById("searxngUrl");
  var body = { enabled: enabled ? enabled.checked : false, url: url ? url.value.trim() : "" };
  try {
    await api("/api/search/config", { method: "POST", body: JSON.stringify(body) });
    _searchConfig = body;
  } catch(_) {}
}

async function _probeSearxng() {
  var status = document.getElementById("searxngProbeStatus");
  if (status) { status.textContent = "Testing…"; status.style.color = "var(--muted)"; }
  try {
    var r = await api("/api/search/probe");
    if (status) {
      status.textContent = r.ok ? ("Connected (" + esc(r.version) + ")") : ("Failed: " + esc(r.error));
      status.style.color = r.ok ? "var(--success)" : "var(--error)";
    }
  } catch(e) {
    if (status) { status.textContent = "Failed: " + esc(e.message||String(e)); status.style.color = "var(--error)"; }
  }
}

function _renderLlmWikiStatus(d, buildStatus) {
  ...
  const rawDocsUrl = status.docs_url || 'https://docs.ontoweb.cn/intellect-agent/skills/bundled/research/research-llm-wiki';
  ...
  const pathHint = status.target_path_hint || '';
  const missingMsg = (typeof t === 'function' ? t('wiki_status_missing_at') : 'No LLM Wiki directory was found at {path}.')
    .replace('{path}', pathHint || 'the configured path');
  ...
  const scopeLabels = {
    project: 'Project: ' + (status.scope_id || ''),
    team: 'Team: ' + (status.scope_id || ''),
    member: 'Personal',
    global: 'Global',
    unknown: 'Unknown'
  };
  ...
  // Vault link
  var vaultHref = '/vault/';
  if (scope === 'project' && status.scope_id) vaultHref = '/vault/p/' + encodeURIComponent(status.scope_id) + '/';
  else if (scope === 'team' && status.scope_id) vaultHref = '/vault/t/' + encodeURIComponent(status.scope_id) + '/';
  else if (scope === 'member' && status.scope_id) vaultHref = '/vault/m/' + encodeURIComponent(status.scope_id) + '/';

  // Build status
  var buildInfo = '';
  var bs = buildStatus || {};
  if (bs.current_job && bs.current_job.status === 'running') {
    buildInfo = '<span style="color:var(--accent);font-size:11px">⏳ Building vault...</span>';
  } else if (bs.last_built_at) {
    var lastBuilt = _formatLlmWikiTimestamp(bs.last_built_at);
    buildInfo = '<span style="color:var(--muted);font-size:11px">Vault built: ' + esc(lastBuilt) + '</span>';
  } else {
    buildInfo = '<span style="color:var(--muted);font-size:11px">Vault not built yet</span>';
  }

  // Build button
  var buildBtn = '';
  if (isReady || isEmpty) {
    var btnDisabled = (bs.current_job && bs.current_job.status === 'running') ? ' disabled' : '';
    buildBtn = '<button class="btn-tiny" onclick="triggerVaultBuild(\'' + esc(scope) + '\',\'' + esc(status.scope_id || '') + '\')" style="font-size:10px;padding:2px 8px"' + btnDisabled + '>\u{1F504} Rebuild Vault</button>';
  }

  var initBtn = '';
  if (status.init_available) {
    initBtn = '<button type="button" class="sm-btn" style="font-size:11px" onclick="void initLlmWiki()">' + esc(typeof t === 'function' ? t('wiki_init_btn') : 'Initialize Wiki') + '</button>';
  }

  var routingHint = '';
  if (_vaultConfig && _vaultConfig.routing && (_vaultConfig.members_enabled || _vaultConfig.routing_available)) {
    var modeLabel = _vaultConfig.routing === 'explicit'
      ? (typeof t === 'function' ? t('vault_routing_explicit') : 'Explicit paths only')
      : (typeof t === 'function' ? t('vault_routing_context') : 'Context-aware');
    routingHint = '<span style="font-size:10px;color:var(--muted)">' + esc((typeof t === 'function' ? t('vault_routing_current') : 'Vault routing: {mode}').replace('{mode}', modeLabel)) + '</span>';
  }
    ...
    <div class="insights-card wiki-status-card" id="llmWikiStatusCard">
      <div class="wiki-status-head">
        <div>
          ...
          <div class="wiki-status-sub">Knowledge-base observability ${scope !== 'global' ? '· <span style="color:var(--accent)">' + esc(scopeLabel) + '</span>' : ''}</div>
        </div>
      ...
      <div class="wiki-status-grid">
        <div><span>Scope</span><strong style="color:var(--${scope === 'project' ? 'accent' : scope === 'team' ? 'accent' : scope === 'member' ? 'green' : 'muted'})">${esc(scopeLabel)}</strong></div>
      ...
      <div class="wiki-status-footer">
        <span>${buildInfo}${routingHint ? ' · ' + routingHint : ''}</span>
        <span style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
          ${initBtn}
          ${buildBtn}
          ${status.has_config ? '<button type="button" class="sm-btn" style="font-size:11px" onclick="void _toggleLlmWiki(' + JSON.stringify(!status.enabled) + ')">' + (status.enabled ? 'Disable' : 'Enable') + '</button>' : ''}
          <a href="${esc(vaultHref)}" target="_blank" rel="noopener noreferrer" style="font-size:11px">Browse Vault →</a>
          <a href="${esc(docsUrl)}" target="_blank" rel="noopener noreferrer" style="font-size:11px">Docs</a>
        </span>
      </div>
    </div>`;
}

var _vaultConfig = null;

async function _loadVaultConfig() {
  try {
    _vaultConfig = await api('/api/vault/config');
  } catch (_) {
    _vaultConfig = { routing: 'context' };
  }
}

async function initLlmWiki() {
  var pathHint = '';
  try {
    var st = await api('/api/wiki/status');
    pathHint = (st && st.target_path_hint) || '';
  } catch (_) {}
  var confirmTpl = (typeof t === 'function' ? t('wiki_init_confirm_body') : 'Create wiki at {path}?');
  if (!window.confirm(confirmTpl.replace('{path}', pathHint || 'this location'))) return;
  var domain = '';
  if (typeof window.prompt === 'function') {
    var prompted = window.prompt((typeof t === 'function' ? t('wiki_init_domain_prompt') : 'Domain (optional):'), '');
    if (prompted === null) return;
    domain = String(prompted).trim();
  }
  try {
    var body = { trigger_build: true };
    if (domain) body.domain = domain;
    var r = await api('/api/wiki/init', { method: 'POST', body: JSON.stringify(body) });
    if (r && r.ok) {
      if (typeof showToast === 'function') showToast(typeof t === 'function' ? t('wiki_init_success') : 'Wiki initialized.', 2500);
      if (typeof loadInsights === 'function') setTimeout(function() { loadInsights(); }, 400);
      if (typeof loadVaultWikiSettings === 'function') loadVaultWikiSettings();
    } else {
      if (typeof showToast === 'function') showToast((typeof t === 'function' ? t('wiki_init_failed') : 'Failed to initialize wiki.') + (r && r.error ? ': ' + r.error : ''), 'error');
    }
  } catch (e) {
    if (typeof showToast === 'function') showToast((typeof t === 'function' ? t('wiki_init_failed') : 'Failed to initialize wiki.') + ': ' + (e.message || String(e)), 'error');
  }
}
window.initLlmWiki = initLlmWiki;

async function loadVaultWikiSettings() {
  var card = $('vaultWikiSettingsCard');
  if (!card) return;
  var membersOn = false;
  try {
    var cfg = await api('/api/vault/config');
    _vaultConfig = cfg;
    var descEl = document.getElementById('vaultWikiSettingsDesc');
    if (descEl) {
      membersOn = !!(cfg && (cfg.members_enabled || cfg.routing_available));
      descEl.textContent = typeof t === 'function'
        ? t(membersOn ? 'vault_wiki_settings_desc' : 'vault_wiki_settings_desc_single')
        : (membersOn
          ? 'Initialize your knowledge base and configure how /vault/ URLs are routed.'
          : 'Initialize your knowledge base and configure vault rebuild triggers.');
    }
    var wikiStatus = await api('/api/wiki/status').catch(function() { return {}; });
    var routing = (cfg && cfg.routing) || 'context';
    var trigger = (cfg && cfg.build_trigger) || 'auto';
    var schedule = (cfg && cfg.build_schedule) || '0 * * * *';
    var sched = (cfg && cfg.scheduler) || {};
    var initBtn = (wikiStatus && wikiStatus.init_available)
      ? '<button type="button" class="sm-btn" style="font-size:11px;margin-top:8px" onclick="void initLlmWiki()">' + esc(typeof t === 'function' ? t('wiki_init_btn') : 'Initialize Wiki') + '</button>'
      : '';
    var schedInfo = '';
    if (trigger === 'scheduled') {
      var lastTick = sched.last_tick_summary ? esc(sched.last_tick_summary) : '—';
      var nextTick = sched.next_tick_estimate ? esc(new Date(sched.next_tick_estimate).toLocaleString()) : '—';
      schedInfo = '<div style="font-size:11px;color:var(--muted);margin-top:8px;line-height:1.5">' +
        esc(typeof t === 'function' ? t('vault_scheduler_last') : 'Last scheduled run') + ': ' + lastTick + '<br>' +
        esc(typeof t === 'function' ? t('vault_scheduler_next') : 'Next estimated run') + ': ' + nextTick + '<br>' +
        esc(typeof t === 'function' ? t('vault_scheduler_gateway_hint') : 'Scheduled builds require intellect gateway.') +
        '</div>';
    }
    var routingBlock = '';
    if (membersOn) {
      routingBlock =
        '<div style="font-size:12px;font-weight:500;margin-bottom:6px">' + esc(typeof t === 'function' ? t('vault_routing_title') : 'Vault URL routing') + '</div>' +
        '<label style="display:flex;gap:8px;align-items:flex-start;font-size:12px;margin-bottom:6px;cursor:pointer">' +
          '<input type="radio" name="vaultRouting" value="context" ' + (routing === 'context' ? 'checked' : '') + ' onchange="void saveVaultConfig({routing:this.value})">' +
          '<span><strong>' + esc(typeof t === 'function' ? t('vault_routing_context') : 'Context-aware') + '</strong><br><span style="color:var(--muted);font-size:11px">' + esc(typeof t === 'function' ? t('vault_routing_context_desc') : '') + '</span></span>' +
        '</label>' +
        '<label style="display:flex;gap:8px;align-items:flex-start;font-size:12px;margin-bottom:10px;cursor:pointer">' +
          '<input type="radio" name="vaultRouting" value="explicit" ' + (routing === 'explicit' ? 'checked' : '') + ' onchange="void saveVaultConfig({routing:this.value})">' +
          '<span><strong>' + esc(typeof t === 'function' ? t('vault_routing_explicit') : 'Explicit paths only') + '</strong><br><span style="color:var(--muted);font-size:11px">' + esc(typeof t === 'function' ? t('vault_routing_explicit_desc') : '') + '</span></span>' +
        '</label>';
    }
    card.innerHTML =
      routingBlock +
      '<div style="font-size:12px;font-weight:500;margin-bottom:6px">' + esc(typeof t === 'function' ? t('vault_build_trigger_title') : 'Build trigger') + '</div>' +
      '<select id="vaultBuildTrigger" class="settings-select" style="width:100%;font-size:12px;margin-bottom:6px" onchange="void onVaultBuildTriggerChange()">' +
        '<option value="auto"' + (trigger === 'auto' ? ' selected' : '') + '>' + esc(typeof t === 'function' ? t('vault_build_trigger_auto') : 'Auto (after sessions)') + '</option>' +
        '<option value="manual"' + (trigger === 'manual' ? ' selected' : '') + '>' + esc(typeof t === 'function' ? t('vault_build_trigger_manual') : 'Manual only') + '</option>' +
        '<option value="scheduled"' + (trigger === 'scheduled' ? ' selected' : '') + '>' + esc(typeof t === 'function' ? t('vault_build_trigger_scheduled') : 'Scheduled (cron)') + '</option>' +
      '</select>' +
      '<div id="vaultScheduleRow" style="display:' + (trigger === 'scheduled' ? 'block' : 'none') + ';margin-bottom:6px">' +
        '<label style="font-size:11px;color:var(--muted)">' + esc(typeof t === 'function' ? t('vault_build_schedule_label') : 'Cron schedule') + '</label>' +
        '<input type="text" id="vaultBuildSchedule" value="' + esc(schedule) + '" placeholder="0 * * * *" style="width:100%;font-size:12px;padding:4px 8px;margin-top:4px;border:1px solid var(--border-muted);border-radius:var(--radius-sm);background:var(--input-bg);color:var(--text)" onchange="void saveVaultConfig({build_schedule:this.value})">' +
        '<div style="font-size:10px;color:var(--muted);margin-top:4px">' + esc(typeof t === 'function' ? t('vault_build_schedule_hint') : '5-field cron, e.g. 0 * * * * = hourly') + '</div>' +
      '</div>' + schedInfo + initBtn;
  } catch (e) {
    card.innerHTML = '<div style="color:var(--error);font-size:12px">' + esc((typeof t === 'function' ? t('vault_routing_save_failed') : 'Failed to load vault settings.') + ': ' + (e.message || String(e))) + '</div>';
  }
}

function onVaultBuildTriggerChange() {
  var sel = document.getElementById('vaultBuildTrigger');
  var row = document.getElementById('vaultScheduleRow');
  if (!sel) return;
  if (row) row.style.display = sel.value === 'scheduled' ? 'block' : 'none';
  var body = { build_trigger: sel.value };
  if (sel.value === 'scheduled') {
    var inp = document.getElementById('vaultBuildSchedule');
    if (inp && inp.value.trim()) body.build_schedule = inp.value.trim();
  }
  void saveVaultConfig(body);
}

async function saveVaultConfig(updates) {
  try {
    var r = await api('/api/vault/config', { method: 'POST', body: JSON.stringify(updates || {}) });
    if (r && r.ok) {
      _vaultConfig = Object.assign({}, _vaultConfig || {}, updates || {}, { scheduler: r.scheduler });
      if (typeof showToast === 'function') showToast(typeof t === 'function' ? t('vault_config_saved') : 'Vault settings saved.', 2000);
      if ((updates || {}).build_trigger) loadVaultWikiSettings();
    } else {
      if (typeof showToast === 'function') showToast((typeof t === 'function' ? t('vault_routing_save_failed') : 'Failed to save vault settings.') + (r && r.error ? ': ' + r.error : ''), 'error');
    }
  } catch (e) {
    if (typeof showToast === 'function') showToast((typeof t === 'function' ? t('vault_routing_save_failed') : 'Failed to save vault settings.') + ': ' + (e.message || String(e)), 'error');
  }
}

async function saveVaultRouting(routing) {
  return saveVaultConfig({ routing: routing });
}
window.saveVaultRouting = saveVaultRouting;
window.saveVaultConfig = saveVaultConfig;
window.onVaultBuildTriggerChange = onVaultBuildTriggerChange;
 ...
 */
async function triggerVaultBuild(scope, scopeId) {
  try {
    var body = {scope: scope || 'global'};
    if (scopeId) body.scope_id = scopeId;
    var r = await api('/api/wiki/build', {method:'POST', body:JSON.stringify(body)});
    if (r && r.ok) {
      showToast('Vault build queued. Refresh to check status.');
      // Re-fetch build status after a short delay
      setTimeout(async function(){
        try {
          var bs = await api('/api/wiki/build/status?scope=' + encodeURIComponent(scope) + '&scope_id=' + encodeURIComponent(scopeId || ''));
          // Trigger insights refresh to update the card
          if (typeof refreshInsights === 'function') refreshInsights();
        } catch(e) {}
      }, 2000);
    } else {
      showToast((r && r.error) || 'Failed to trigger vault build.', 'error');
    }
  } catch(e) {
    showToast('Build request failed: ' + (e.message || 'network error'), 'error');
  }
}
...
function _renderInsights(d, box, wikiStatus, buildStatus, analytics) {
  ...
  box.innerHTML = `
    ...
    ${_renderLlmWikiStatus(wikiStatus, buildStatus)}
    ${_renderSearchSettings()}
    ...
    ${dowHtml}
    ${hodHtml}
    ${_renderSkillsAnalytics(analytics)}
...
function syncWorkspaceDisplays(){
  ...
  if(composerChip){
    // Enable the chip after workspace list is loaded so the user can always
    // open the dropdown to browse and select workspaces. Only disable when
    // the workspace list hasn't been fetched yet (initial page load).
    composerChip.disabled=!_workspaceList;
    composerChip.title=hasWorkspace?ws:(_workspaceList?(t('no_workspace')||'Select workspace'):'Loading workspaces…');
...
async function loadWorkspaceList(){
  try{
    ...
    // Even if the API fails, mark the list as loaded (empty) so the
    // workspace chip is enabled — the user can still type a path manually
    // or retry by re-opening the dropdown.
    if(!_workspaceList) _workspaceList = [];
    syncWorkspaceDisplays();
...
function isProfileManagementEnabled() {
  return _profileManagementEnabled !== false;
}

function _applyProfileManagementUiGate(enabled) {
  _profileManagementEnabled = enabled !== false;
  if (_profileManagementEnabled) return;
  document.querySelectorAll('[data-panel="profiles"]').forEach((el) => {
    el.classList.add('nav-tab-hidden');
    el.setAttribute('aria-hidden', 'true');
  });
  ['profileChipWrap', 'panelProfiles', 'mainProfiles'].forEach((id) => {
    const el = $(id);
...
async function activateCurrentProfile(){
  // TEMPORARY: profile activation disabled (profiles.management_enabled).
  if (typeof isProfileManagementEnabled === 'function' && !isProfileManagementEnabled()) {
    return;
  }
...
function toggleProfileDropdown() {
  // TEMPORARY: composer profile chip hidden when management disabled.
  if (typeof isProfileManagementEnabled === 'function' && !isProfileManagementEnabled()) {
    return;
  }
  const dd = $('profileDropdown');
...
function openProfileCreate(){
  // TEMPORARY: profile creation UI disabled (profiles.management_enabled).
  if (typeof isProfileManagementEnabled === 'function' && !isProfileManagementEnabled()) {
    return;
  }
...
const _HIDDEN_TABS_LS_KEY = 'intellect-webui-hidden-tabs';
...
async function loadSettingsPanel(){
  try{
    ...
    const skinVal=(localStorage.getItem('intellect-skin')||settings.skin||'default').toLowerCase();
...
var _providersPanelLoaded = false;

async function loadProvidersPanel(force){
  const list=$('providersList');
  const empty=$('providersEmpty');
  if(!list) return;
  // Avoid duplicate load — called both from settings init and tab click
  if (_providersPanelLoaded && !force) return;
  _providersPanelLoaded = true;
  // Show loading indicator
  list.innerHTML = '<div style="text-align:center;padding:24px;color:var(--muted)">' + esc(t('loading') || 'Loading...') + '</div>';
  try{
    // Fetch providers and quota in parallel
    ...
    _providersPanelLoaded = false;  // allow retry on error
...
function _buildProviderCard(p){
  ...
  if(p.configurable){
    ...
    // Xiaomi-specific: Base URL selector for API vs Token Plan
    // Always show for Xiaomi regardless of backend url_options
    if(p.id==='xiaomi'){
      // Hardcoded URL options (matches api/config.py _XIAOMI_URLS)
      const XIAOMI_URLS={
      ...
      urlRow.className='provider-card-row';

      // URL selector dropdown
      const urlSelect=document.createElement('select');
      urlSelect.className='provider-card-select';

      // Add API option
      const apiOpt=document.createElement('option');
      apiOpt.value=XIAOMI_URLS.api;
      apiOpt.textContent='API ('+XIAOMI_URLS.api+')';
      urlSelect.appendChild(apiOpt);

      // Add Token Plan options
      const tpLabel=document.createElement('optgroup');
      tpLabel.label='Token Plan';
      const clusters=[
        {key:'token_plan_cn',label:'中国集群'},
        {key:'token_plan_sgp',label:'新加坡集群'},
        {key:'token_plan_ams',label:'欧洲集群'}
      ];
      for(const c of clusters){
        const opt=document.createElement('option');
        opt.value=XIAOMI_URLS[c.key];
        opt.textContent=c.label+' ('+XIAOMI_URLS[c.key]+')';
        tpLabel.appendChild(opt);
      }
      urlSelect.appendChild(tpLabel);

      // Set current value from provider config or default to API
      const currentBaseUrl=p.base_url||XIAOMI_URLS.api;
      // Find matching option or set to API default
      let found=false;
      for(const opt of urlSelect.options){
        if(opt.value===currentBaseUrl){
          urlSelect.value=currentBaseUrl;
          found=true;
          break;
        }
      }
      if(!found) urlSelect.value=XIAOMI_URLS.api;

      urlRow.appendChild(urlSelect);
      urlField.appendChild(urlRow);
      body.appendChild(urlField);

      // Auto-detect on key input
      input.addEventListener('input',()=>{
        const key=input.value.trim().toLowerCase();
        if(key.startsWith('sk')){
          urlSelect.value=XIAOMI_URLS.api;
        }else if(key.startsWith('tp')){
          // Default to China cluster for Token Plan
          urlSelect.value=XIAOMI_URLS.token_plan_cn;
        }
      });

      // Store reference for save function
      _providerCardEls.set(p.id,{
        ...(_providerCardEls.get(p.id)||{}),
        urlSelect:urlSelect
      });
    }

    // Volcano Engine: Base URL text input
    if(p.id==='volcengine'||p.id==='volcengine-coding-plan'||p.id==='volcengine-agent-plan'){
      const VOLC_URLS={
      ...
      urlInput.value=VOLC_URLS[p.id]||'';
      urlInput.placeholder='https://ark.cn-beijing.volces.com/api/v3';
    ...
    hint.textContent=p.is_custom
      ? 'Custom provider loaded from config.yaml / intellect model. Edit it from the CLI or config file.'
  ...
  if(input&&saveBtn){
    // Merge with existing elements (Xiaomi may have already set urlSelect)
    const existing=_providerCardEls.get(p.id)||{};
    _providerCardEls.set(p.id,{...existing,card,input,saveBtn,hasKey:p.has_key});
...
async function _saveProviderKey(providerId){
  ...
  try{
    ...
    // Include base_url for Xiaomi provider if URL selector exists
    if(providerId==='xiaomi' && els.urlSelect){
      body.base_url=els.urlSelect.value;
    }

    // Include base_url for Volcano Engine providers if URL input exists
    if((providerId==='volcengine'||providerId==='volcengine-coding-plan'||providerId==='volcengine-agent-plan')&&els.urlInput){
      body.base_url=els.urlInput.value;
    }
    ...
    if(res.ok){
      ...
      await loadProvidersPanel(true); // force refresh after save
    }else{
...
async function _refreshProviderModels(providerId, btn){
  ...
  try{
    ...
    if(res.ok){
      ...
      // Refresh the model dropdowns so the composer picks up the new models
      try{
        if(typeof window._invalidateSlashModelCache==='function'){
          window._invalidateSlashModelCache();
        }
        // Clear live model cache so stale entries don't block fresh fetches
        if(window._liveModelCache) Object.keys(window._liveModelCache).forEach(k=>delete window._liveModelCache[k]);
        if(typeof window._ensureModelDropdownReady==='function'){
          window._modelDropdownReady=null;
          await window._ensureModelDropdownReady();
        }else if(typeof populateModelDropdown==='function'){
          await populateModelDropdown();
        }
        // Force re-render the composer's visible dropdown so new models appear
        // even if the dropdown was closed during refresh
        if(typeof renderModelDropdown==='function') renderModelDropdown();
        if(typeof syncModelChip==='function') syncModelChip();
      }catch(_e){
        // Swallow — dropdown refresh is best-effort
      }
    }else{
...
async function signOut(){
  ...
  }catch(e){/* ignore — redirect regardless */}
...
let _mcpServerListDelegationBound=false;
function _bindMcpServerListActions(){
  if(_mcpServerListDelegationBound) return;
  const list=$('mcpServerList');
  if(!list) return;
  list.addEventListener('click',e=>{
    const deleteBtn=e.target.closest('.mcp-delete-btn');
    if(!deleteBtn) return;
    const name=deleteBtn.getAttribute('data-mcp-name');
    if(name) deleteMcpServer(name);
  });
  _mcpServerListDelegationBound=true;
}
async function deleteMcpServer(name){
  const ok=await showConfirmDialog({
    title:t('mcp_delete_confirm_title'),
    message:t('mcp_delete_confirm_message',name),
...
function loadMcpServers(){
  ...
  api('/api/mcp/servers').then(r=>{
    ...
    list.innerHTML=r.servers.map(s=>{
      ...
      const deleteBtn=r.toggle_supported
        ?`<button type="button" class="mcp-delete-btn" data-mcp-name="${esc(s.name)}" title="${esc(t('mcp_delete_confirm_title'))}">${esc(t('delete_title'))}</button>`
        :'';
      return `<div class="mcp-server-row">
        ...
        <div class="mcp-server-meta"><span class="mcp-tool-count">${esc(t('mcp_tool_count',toolCount))}</span>${toggleBtn}${deleteBtn}</div>
      </div>`;
    }).join('');
...
// ── Memory Provider (System tab) ─────────────────────────────────────────
var _memoryProviderCache=null;

async function loadMemoryProviders(){
  const card=$('memoryProviderCard');
  if(!card) return;
  try{
    const data=await api('/api/memory/providers');
    _memoryProviderCache=data;
    renderMemoryProviderCard(card,data);
  }catch(e){
    card.innerHTML='<div style="color:var(--muted);font-size:12px">'+esc(t('memory_provider_unavailable')||'Memory provider API unavailable (intellect-agent required).')+'</div>';
  }
}

function renderMemoryProviderCard(card,data){
  if(!data||data.error){
    card.innerHTML='<div style="color:var(--muted);font-size:12px">'+esc(data&&data.error||t('memory_provider_unavailable')||'Memory provider API unavailable.')+'</div>';
    return;
  }
  var providers=data.providers||[];
  var active=data.active_provider||null;
  ...
  if(active){
    var activeInfo=null;
    for(var i=0;i<providers.length;i++){if(providers[i].name===active){activeInfo=providers[i];break;}}
    html+='<div style="display:flex;align-items:center;gap:8px;margin-bottom:10px">'+
      '<span style="font-weight:600;font-size:13px">'+esc(t('memory_provider_active')||'Active')+':</span>'+
      '<span class="badge" style="background:var(--accent);color:#fff;padding:2px 8px;border-radius:10px;font-size:11px">'+esc(active)+'</span>'+
      (activeInfo&&!activeInfo.is_available?'<span style="color:#ef4444;font-size:11px">('+esc(t('memory_provider_not_available')||'not available')+')</span>':'')+
    '</div>';
  }else{
    html+='<div style="margin-bottom:10px"><span style="font-weight:600;font-size:13px">'+esc(t('memory_provider_active')||'Active')+':</span> <span style="color:var(--muted);font-size:12px">'+esc(t('memory_provider_builtin')||'Built-in only (MEMORY.md + USER.md)')+'</span></div>';
  }

  // Provider selector
  html+='<div style="margin-bottom:10px">'+
    '<label style="font-size:11px;color:var(--muted);display:block;margin-bottom:4px">'+esc(t('memory_provider_select')||'Select provider')+'</label>'+
    '<select id="memoryProviderSelect" onchange="onMemoryProviderChange(this.value)" style="width:100%;padding:6px;border:1px solid var(--border);border-radius:4px;background:var(--input-bg,var(--bg));color:var(--text);font-size:13px">'+
      '<option value="">'+esc(t('memory_provider_builtin')||'Built-in only (MEMORY.md + USER.md)')+'</option>';

  for(var i=0;i<providers.length;i++){
    var p=providers[i];
    var sel=p.name===active?' selected':'';
    var label=p.name+(p.description?' — '+p.description:'')+(p.is_available?'':' ['+(t('memory_provider_inactive')||'Not configured')+']');
    html+='<option value="'+esc(p.name)+'"'+sel+'>'+esc(label)+'</option>';
  }
  html+='</select></div>';

  // Config form container
  html+='<div id="memoryProviderConfigForm"></div>';

  // Restart hint
  html+='<div class="mcp-restart-hint" style="margin-top:8px">'+esc(t('memory_provider_restart_hint')||'Provider changes require an agent restart to take effect.')+'</div>';

  card.innerHTML=html;

  // Auto-load config form if a provider is active
  if(active){
    loadMemoryProviderConfig(active);
  }
}

async function onMemoryProviderChange(name){
  if(!name){
    // Switch to built-in only
    try{
      var r=await api('/api/memory/providers',{method:'POST',body:JSON.stringify({provider:''})});
      if(r&&r.ok){
        showToast(t('memory_provider_saved')||'Switched to built-in memory.');
        loadMemoryProviders();
      }
    }catch(e){
      showToast(t('memory_provider_save_failed')||'Failed to switch provider.','error');
    }
    return;
  }
  // Load config form for selected provider
  loadMemoryProviderConfig(name);
}

async function loadMemoryProviderConfig(name){
  var container=$('memoryProviderConfigForm');
  if(!container) return;
  container.innerHTML='<div style="color:var(--muted);font-size:12px;padding:8px 0">'+esc(t('loading'))+'</div>';
  try{
    var data=await api('/api/memory/providers/'+encodeURIComponent(name));
    renderProviderConfigForm(container,data);
  }catch(e){
    container.innerHTML='<div style="color:#ef4444;font-size:12px">'+esc(t('memory_provider_load_failed')||'Failed to load provider config.')+'</div>';
  }
}

function renderProviderConfigForm(container,providerData){
  if(!providerData||providerData.error){
    container.innerHTML='<div style="color:#ef4444;font-size:12px">'+esc(providerData&&providerData.error||'Failed to load config')+'</div>';
    return;
  }
  var schema=providerData.schema||[];
  var values=providerData.values||{};
  var envValues=providerData.env_values||{};
  var name=providerData.name||'';

  if(!schema.length){
    container.innerHTML='<div style="color:var(--muted);font-size:12px;padding:8px 0">'+esc(t('memory_provider_no_config')||'This provider has no configurable options.')+'</div>';
    return;
  }

  var html='<div class="memory-config-form" style="margin-top:8px">';

  // Top-level fields (no 'when' condition)
  for(var i=0;i<schema.length;i++){
    var field=schema[i];
    if(field.when) continue;
    html+=_renderMemoryField(field,values,envValues,name);
  }

  // Conditional fields grouped by their 'when' condition
  var condGroups={};
  for(var i=0;i<schema.length;i++){
    var field=schema[i];
    if(!field.when) continue;
    var condKey=JSON.stringify(field.when);
    if(!condGroups[condKey]) condGroups[condKey]={when:field.when,fields:[]};
    condGroups[condKey].fields.push(field);
  }

  for(var ck in condGroups){
    var group=condGroups[ck];
    var watchField=Object.keys(group.when)[0];
    var watchValue=group.when[watchField];
    var currentVal=values[watchField]||'';
    var show=currentVal===watchValue;
    html+='<div class="memory-field-conditional" data-watch-field="'+esc(watchField)+'" data-watch-value="'+esc(watchValue)+'" style="'+(show?'':'display:none')+'">';
    for(var j=0;j<group.fields.length;j++){
      html+=_renderMemoryField(group.fields[j],values,envValues,name);
    }
    html+='</div>';
  }

  html+='<button class="btn-tiny" onclick="saveMemoryProviderConfig(\''+esc(name)+'\')" style="margin-top:10px;padding:6px 16px;font-weight:600">'+esc(t('memory_provider_save')||'Save Configuration')+'</button>';
  html+='</div>';

  container.innerHTML=html;

  // Wire up conditional field visibility
  var selects=container.querySelectorAll('select[data-memory-key]');
  for(var i=0;i<selects.length;i++){
    selects[i].addEventListener('change',function(){
      _updateMemoryConditionalFields(container);
    });
  }
}

function _renderMemoryField(field,values,envValues,providerName){
  var key=field.key||'';
  var desc=field.description||'';
  var required=field.required||false;
  var secret=field.secret||false;
  var choices=field.choices||null;
  var defVal=field.default!==undefined?field.default:'';
  var url=field.url||'';
  var envVar=field.env_var||key;

  var currentVal=secret?(envValues[envVar]||''):(values[key]!==undefined?values[key]:defVal);
  var fieldId='mem-field-'+providerName+'-'+key;

  var html='<div class="memory-field-row" style="margin-bottom:10px">';
  html+='<label for="'+esc(fieldId)+'" style="display:block;font-size:12px;font-weight:500;margin-bottom:3px">'+esc(key)+(required?' <span style="color:#ef4444">*</span>':'')+'</label>';

  if(secret){
    var isSet=currentVal==='(set)';
    if(isSet){
      html+='<div style="display:flex;align-items:center;gap:6px">'+
        '<span style="color:var(--muted);font-size:12px">'+esc(t('memory_provider_secret_set')||'(set)')+'</span>'+
        '<button type="button" class="btn-tiny memory-secret-toggle" onclick="toggleMemorySecret(\''+esc(fieldId)+'\')" style="font-size:10px">'+esc(t('memory_provider_secret_change')||'Change')+'</button>'+
      '</div>';
      html+='<input type="password" id="'+esc(fieldId)+'" data-memory-key="'+esc(key)+'" data-memory-secret="true" data-memory-env-var="'+esc(envVar)+'" placeholder="'+esc(defVal)+'" style="display:none;width:100%;padding:5px;border:1px solid var(--border);border-radius:3px;background:var(--input-bg,var(--bg));color:var(--text);font-size:12px;margin-top:4px">';
    }else{
      html+='<input type="password" id="'+esc(fieldId)+'" data-memory-key="'+esc(key)+'" data-memory-secret="true" data-memory-env-var="'+esc(envVar)+'" placeholder="'+esc(defVal)+'" style="width:100%;padding:5px;border:1px solid var(--border);border-radius:3px;background:var(--input-bg,var(--bg));color:var(--text);font-size:12px">';
    }
  }else if(choices&&choices.length){
    html+='<select id="'+esc(fieldId)+'" data-memory-key="'+esc(key)+'" style="width:100%;padding:5px;border:1px solid var(--border);border-radius:3px;background:var(--input-bg,var(--bg));color:var(--text);font-size:12px">';
    for(var i=0;i<choices.length;i++){
      var c=String(choices[i]);
      var sel=c===String(currentVal)?' selected':'';
      html+='<option value="'+esc(c)+'"'+sel+'>'+esc(c)+'</option>';
    }
    html+='</select>';
  }else{
    html+='<input type="text" id="'+esc(fieldId)+'" data-memory-key="'+esc(key)+'" value="'+esc(String(currentVal))+'" placeholder="'+esc(defVal)+'" style="width:100%;padding:5px;border:1px solid var(--border);border-radius:3px;background:var(--input-bg,var(--bg));color:var(--text);font-size:12px">';
  }

  if(desc) html+='<div style="font-size:10px;color:var(--muted);margin-top:2px">'+esc(desc)+'</div>';
  if(url) html+='<div style="font-size:10px;margin-top:1px"><a href="'+esc(url)+'" target="_blank" rel="noopener" style="color:var(--accent)">'+esc(url)+'</a></div>';
  html+='</div>';
  return html;
}

function toggleMemorySecret(fieldId){
  var input=$(fieldId);
  if(!input) return;
  var row=input.closest('.memory-field-row');
  var toggleBtn=row?row.querySelector('.memory-secret-toggle'):null;
  var setSpan=row?row.querySelector('span'):null;
  if(input.style.display==='none'){
    input.style.display='';
    if(setSpan) setSpan.style.display='none';
    if(toggleBtn) toggleBtn.textContent=t('memory_provider_secret_hide')||'Hide';
  }else{
    input.style.display='none';
    if(setSpan) setSpan.style.display='';
    if(toggleBtn) toggleBtn.textContent=t('memory_provider_secret_change')||'Change';
  }
}

function _updateMemoryConditionalFields(container){
  var condDivs=container.querySelectorAll('.memory-field-conditional');
  for(var i=0;i<condDivs.length;i++){
    var div=condDivs[i];
    var watchField=div.getAttribute('data-watch-field');
    var watchValue=div.getAttribute('data-watch-value');
    var select=container.querySelector('select[data-memory-key="'+watchField+'"]');
    var currentVal=select?select.value:'';
    div.style.display=currentVal===watchValue?'':'none';
  }
}

async function saveMemoryProviderConfig(providerName){
  var form=$('memoryProviderConfigForm');
  ...
  var inputs=form.querySelectorAll('input[data-memory-key]:not([data-memory-secret]), select[data-memory-key]');
  for(var i=0;i<inputs.length;i++){
    var inp=inputs[i];
    var key=inp.getAttribute('data-memory-key');
    if(key) fields[key]=inp.value;
  }

  // Collect secret fields (only if changed)
  var secretInputs=form.querySelectorAll('input[data-memory-secret]');
  for(var i=0;i<secretInputs.length;i++){
    var inp=secretInputs[i];
    if(inp.style.display==='none') continue;
    var key=inp.getAttribute('data-memory-env-var')||inp.getAttribute('data-memory-key');
    var val=inp.value;
    if(key&&val) secrets[key]=val;
  }

  try{
    var r=await api('/api/memory/providers/'+encodeURIComponent(providerName),{
      method:'POST',
      body:JSON.stringify({fields:fields,secrets:secrets})
    });
    if(r&&r.ok){
      showToast(t('memory_provider_saved')||'Memory provider configuration saved. Restart agent to apply.');
      loadMemoryProviders();
    }else{
      showToast((r&&r.error)||t('memory_provider_save_failed')||'Failed to save configuration.','error');
    }
  }catch(e){
    showToast(t('memory_provider_save_failed')||'Failed to save configuration.','error');
  }
}
...
function _renderSkillsAnalytics(analytics) {
  if (!analytics || !analytics.summary) return '';
  var s = analytics.summary;
  var top = (analytics.top_used || []).slice(0, 8);
  if (!top.length) return '';
  return '<div class="insights-card" style="margin-top:16px">'
    + '<div class="insights-card-title">' + esc(String(s.total_skills)) + ' Skills &middot; ' + esc(String(s.active_30d)) + ' active (30d) &middot; ' + esc(String(s.total_usage_count)) + ' uses</div>'
    + '<div class="insights-bars">'
    + top.map(function(t) {
        var maxVal = Math.max(1, top[0] ? top[0].total : 0);
        var pct = Math.min(100, Math.round(t.total / maxVal * 100));
        return '<div class="insights-bar-row">'
          + '<span class="insights-bar-label" style="flex:0 0 160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:11px">' + esc(t.name) + '</span>'
          + '<span class="insights-bar" style="flex:1"><span class="insights-bar-fill" style="width:' + pct + '%"></span></span>'
          + '<span class="insights-bar-value" style="flex:0 0 45px;text-align:right;font-size:11px">' + esc(String(t.total)) + '</span></div>';
      }).join('')
    + '</div></div>';
}
