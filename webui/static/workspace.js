async function api(path,opts={}){
  ...
  const projectId=typeof getWebuiActiveProjectId==='function'?getWebuiActiveProjectId():'';
  if(projectId&&!headers['X-Intellect-Project'])headers['X-Intellect-Project']=projectId;
...
// Canonical Intellect mutators plus MCP filesystem aliases that can create/edit files.
...
async function loadDir(path, opts={}){
  ...
  try{
    ...
    // When an open workspace preview is being preserved (e.g. after a chat
    // turn), reload the file if agent write/edit tools touched that path,
    // unless the preview editor has unsaved local edits (#3262).
    if(preservePreview && typeof _previewCurrentPath!=='undefined' && _previewCurrentPath
        && !(typeof _previewDirty!=='undefined' && _previewDirty)){
      const normalizedTitle=_normalizeArtifactPath(_previewCurrentPath);
      if(normalizedTitle){
        const touched=collectSessionArtifacts().some(a=>_normalizeArtifactPath(a&&a.path)===normalizedTitle);
        if(touched && typeof openFile==='function'){
