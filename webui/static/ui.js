// lgtm[js/xss-through-dom]: DOM updates use server-returned HTML builder, not raw user input
...
async function populateModelDropdown(opts={}){
  ...
  try{
    ...
    // Build new options in a fragment first — only replace the existing
    // content atomically after all options are constructed. This prevents
    // the select from being left empty when a mid-loop error occurs.
    ...
    for(const g of groups){
      ...
      _fragment.appendChild(og);
    }
    // Atomically replace the select content — old options stay visible
    // until the fragment is fully built, avoiding a blank intermediate state.
    sel.innerHTML='';
    sel.appendChild(_fragment);
...
// Expose for external invalidation (e.g. after provider key save / refresh)
window._liveModelCache=_liveModelCache;
...
async function toggleModelDropdown(){
  ...
  // Wait for the model catalog to be ready before rendering — avoids showing
  // an empty dropdown when the user clicks before the async /api/models fetch
  // has completed (populateModelDropdown cleared sel.innerHTML on its first
  // pass but hasn't inserted the new options yet).
  if(typeof window._ensureModelDropdownReady==='function'){
    try { await window._ensureModelDropdownReady(); } catch(_) {}
  }
...
function _showAgentHealthAlert(payload){
  ...
  const scope=payload&&payload.details&&payload.details.probe_scope?` Probe: ${payload.details.probe_scope} (active profile).`:'';
...
// ── Tool card open-state preservation across DOM rebuilds ──────────────────
// When renderMessages() rebuilds the DOM (history scroll, stream settle),
// any .tool-card.open state is lost. Capture a stable signature for each
// open card before the rebuild and restore it after, so the user's expanded
// tool cards stay open rather than collapsing on every re-render.

function _toolCardSignature(card) {
  // Build a fingerprint from the visible header content — stable across
  // live→settled transitions and history re-renders.
  var name = '';
  var preview = '';
  var nameEl = card.querySelector('.tool-card-name');
  if (nameEl) name = (nameEl.textContent || '').trim();
  var previewEl = card.querySelector('.tool-card-preview');
  if (previewEl) preview = (previewEl.textContent || '').trim();
  // Truncate long previews to a reasonable length for fingerprinting.
  if (preview.length > 120) preview = preview.slice(0, 120);
  return (name + '|' + preview).toLowerCase();
}

function _captureOpenToolCardSignatures() {
  var signatures = new Set();
  var container = $('msgInner');
  if (!container) return signatures;
  var openCards = container.querySelectorAll('.tool-card.open');
  for (var i = 0; i < openCards.length; i++) {
    var sig = _toolCardSignature(openCards[i]);
    if (sig && sig !== '|') signatures.add(sig);
  }
  return signatures;
}

function _restoreOpenToolCardSignatures(signatures) {
  if (!signatures || !signatures.size) return;
  var container = $('msgInner');
  if (!container) return;
  var allCards = container.querySelectorAll('.tool-card');
  for (var i = 0; i < allCards.length; i++) {
    var card = allCards[i];
    if (card.classList.contains('open')) continue; // already open
    var sig = _toolCardSignature(card);
    if (sig && signatures.has(sig)) {
      card.classList.add('open');
      // Once all captured signatures are restored, exit early.
      signatures.delete(sig);
      if (!signatures.size) break;
    }
  }
}
...
function renderMessages(options){
  ...
  const openToolCardSignatures = preserveScroll ? _captureOpenToolCardSignatures() : null;
  const inner=$('msgInner');
  ...
  if(openToolCardSignatures && openToolCardSignatures.size > 0){
    _restoreOpenToolCardSignatures(openToolCardSignatures);
  }
...
function buildToolCard(tc){
  ...
  // When the only detail is a snippet that mirrors the preview text, skip the
  // detail section entirely — otherwise the user sees the same content twice.
