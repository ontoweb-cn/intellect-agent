"""
Intellect Web UI -- Main server entry point.
...
# ── Module path setup ─────────────────────────────────────────────────────
# When running as `python -m webui.server` from the agent project root, the
# webui/api/ modules use top-level ``from api.xxx import ...`` imports.
# Add the webui directory to sys.path so those resolve correctly.
...
# mockable).
if os.environ.get("INTELLECT_WEBUI_TEST_NETWORK_BLOCK", "").strip() in ("1", "true", "yes"):
    ...
    def _blocked_create_connection(address, *a, **kw):
        ...
        raise OSError(
            f"intellect test network isolation (server.py): outbound to {address!r} blocked"
        )
    ...
    def _blocked_socket_connect(self, address):
        ...
        raise OSError(
            f"intellect test network isolation (server.py): socket.connect to {address!r} blocked"
        )
...
def _csp_extra_connect_src() -> str:
    raw = os.getenv("INTELLECT_WEBUI_CSP_CONNECT_EXTRA", "").strip()
    if not raw:
        return ""
    sources = raw.split()
    if not sources or any(not _valid_csp_extra_connect_source(src) for src in sources):
        logger.warning("Ignoring invalid INTELLECT_WEBUI_CSP_CONNECT_EXTRA value")
        return ""
    return " " + " ".join(sources)
...
class QuietHTTPServer(ThreadingHTTPServer):
    ...
    def handle_error(self, request, client_address):
        ...
        if exc_type in _CLIENT_DISCONNECT_ERRORS:
            return
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        try:
            ...
            from api.members import (
                ...
                check_member_access,
                clear_request_member_context,
                maybe_redirect_oauth_canonical_host,
            )
            if maybe_redirect_oauth_canonical_host(self, parsed):
                return
            bind_request_member_context(self, parsed)
            if not check_auth(self, parsed): return
            if not check_member_access(self, parsed): return
            ...
            j(self, {'error': 'Session not found'}, status=404)
            return
        except Exception:
    ...
    def _handle_write(self, route_func) -> None:
        ...
        # into the next request on the same connection (501 Unsupported method).
        ...
        try:
            ...
            from api.members import bind_request_member_context, check_member_access, clear_request_member_context
            bind_request_member_context(self, parsed)
            if not _is_csp_report_post and not check_auth(self, parsed):
                from api.helpers import drain_body
                drain_body(self)
                return
            if not _is_csp_report_post and not check_member_access(self, parsed):
                from api.helpers import drain_body
                drain_body(self)
                return
            ...
            # response. This is expected for probes, tab closes, and SSE
            # reconnect races; do not convert it into a misleading server 500.
            # Drain any unread body so keep-alive connections are not corrupted.
            from api.helpers import drain_body
            drain_body(self)
            return
        except SessionAccessDenied:
            ...
            from api.helpers import drain_body
            drain_body(self)
            drained = getattr(self, '_body_drained', False)
            if not drained:
...
def main() -> None:
    ...
    # W4b / T5: multi-worker requires PostgreSQL + Redis cache/events.
    try:
        from agent.webui_ha import format_webui_ha_errors, validate_webui_ha_startup
        from intellect_cli.config import load_config

        ha_errors = validate_webui_ha_startup(load_config())
        if ha_errors:
            print(format_webui_ha_errors(ha_errors), flush=True)
            raise SystemExit(1)
    except SystemExit:
        raise
    except Exception as exc:
        print(f'[!!] WARNING: WebUI HA config check skipped: {exc}', flush=True)
    ...
    if HOST not in ('127.0.0.1', '::1', 'localhost') and not is_auth_enabled():
        ...
        print(f'     Set a password via Settings or INTELLECT_WEBUI_PASSWORD env var.', flush=True)
        ...
        print(f'        and memory via the local API. Set INTELLECT_WEBUI_PASSWORD to', flush=True)
        print(f'        enable authentication.', flush=True)

    ok, missing, errors = verify_intellect_imports()
    if not ok and _INTELLECT_FOUND:
        print(f'[!!] Warning: Intellect agent found but missing modules: {missing}', flush=True)
    ...
    try:
        from api.session_events import start_session_events_bridge
        start_session_events_bridge()
    except Exception as e:
        print(f'[!!] WARNING: Session events bridge failed to start: {e}', flush=True)

    try:
        from api.kanban_events import start_kanban_events_bridge
        start_kanban_events_bridge()
    except Exception as e:
        print(f'[!!] WARNING: Kanban events bridge failed to start: {e}', flush=True)

    try:
        from agent.webui_ha import parse_webui_worker_count

        if parse_webui_worker_count() > 1:
            print(
                '[ok] Multi-worker mode: member auth uses state.db member_sessions '
                '(.member-sessions files are not shared across workers)',
                flush=True,
            )
    except Exception:
    ...
    print(f'  Intellect Web UI listening on {scheme}://{HOST}:{PORT}', flush=True)
    ...
    try:
        ...
        try:
            from api.session_events import stop_session_events_bridge
            stop_session_events_bridge()
        except Exception:
            logger.debug("Failed to stop session events bridge during shutdown")
