# OpenAI-Compatible API Server for Intellect Agent
...
SSE streaming. By exposing this endpoint, intellect-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  intellect-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.intellect/.env`
2. Run `intellect gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with intellect-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns intellect-agent as a model) |
...
Authorization: Bearer intellect-api-key-here
...
But intellect-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "intellect-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured intellect-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of intellect-agent's core prompt
...
Once implemented, intellect-agent works as a drop-in backend for:
