"""
MCP Server Management CLI — ``intellect mcp`` subcommand.

Implements ``intellect mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.intellect/config.yaml under the ``mcp_servers`` key.
"""
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from intellect_cli.cli_output import prompt as _shared_prompt
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``intellect mcp login`` to distinguish a genuine authentication
    ...
    """
    try:
        from tools.mcp_oauth import intellectTokenStorage
        return intellectTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # intellect_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  intellect mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  intellect mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  intellect mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_intellect_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: intellect mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_intellect_home()}/config.yaml ({tool_count}/{total} tools enabled)")
    _info("Start a new session to use these tools.")


# ─── intellect mcp remove ───────────────────────────────────────────────────────
...
def cmd_mcp_remove(args):
    ...
    # earlier `intellect mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  intellect mcp add <name> --url <endpoint>')
        _info('  intellect mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `intellect mcp remove` + `intellect mcp add` to reconfigure auth.")
        return
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `intellect mcp login " + name + "`.")
            return
        if tools:
...
# ─── intellect mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'intellect mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    ...
    if action == "picker":
        from intellect_cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from intellect_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from intellect_cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `intellect plugin`.
        from intellect_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("intellect mcp                                    Open the catalog picker (default)")
        _info("intellect mcp catalog                            List ONTOWEB-approved MCPs")
        _info("intellect mcp install <name>                     Install a catalog MCP")
        _info("intellect mcp serve                              Run as MCP server")
        _info("intellect mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("intellect mcp add <name> --command <cmd>         Add a stdio server")
        _info("intellect mcp add <name> --preset <preset>       Add from a known preset")
        _info("intellect mcp remove <name>                      Remove a server")
        _info("intellect mcp list                               List configured servers")
        _info("intellect mcp test <name>                        Test connection")
        _info("intellect mcp configure <name>                   Toggle tools")
        _info("intellect mcp login <name>                       Re-authenticate OAuth")
        print()
