...
``<root>`` is the **shared Intellect root** (the parent of any active
...
``intellect -p <profile>`` joins the same board as the dispatcher that
...
the profile's Intellect home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``INTELLECT_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``intellect_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``intellect kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.intellect``. In Docker / custom
deployments where ``INTELLECT_HOME`` points outside ``~/.intellect`` (e.g.
``/opt/intellect``), ``<root>`` is ``INTELLECT_HOME``. Legacy env-var
overrides still work:

* ``intellect_KANBAN_DB`` — pin the database file path directly.
* ``intellect_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``intellect_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``intellect_KANBAN_DB``,
``intellect_KANBAN_WORKSPACES_ROOT``, and ``INTELLECT_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/intellect-kanban-v1-spec.pdf`` for the full
...
# ``intellect_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``intellect_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("intellect_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``intellect_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("intellect_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
# enough that kebab-case names like ``atm10-server`` or ``intellect-agent``
...
def kanban_home() -> Path:
    """Return the shared Intellect root that anchors the kanban board.

    Resolution order:

    1. ``intellect_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_intellect_root()``, which already returns ``<root>``
       when ``INTELLECT_HOME`` is ``<root>/profiles/<name>``, and returns
       ``INTELLECT_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``INTELLECT_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("intellect_KANBAN_HOME", "").strip()
    if override:
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``intellect kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``INTELLECT_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``intellect kanban boards
    ...
    """
    env = os.environ.get("INTELLECT_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``intellect kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``intellect_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("intellect_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
    try:
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``intellect_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("intellect_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``intellect_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("intellect_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``intellect kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``intellect_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``intellect_SESSION_ID``. NULL
    -- for tasks created from the CLI, dashboard, or any path that doesn't
    -- set the env var. Indexed so per-session list queries stay cheap on
    -- larger boards.
    session_id           TEXT,
    -- Unified-storage board namespace (legacy per-board kanban.db ignores this).
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("intellect_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Intellect treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``intellect_KANBAN_DB`` env → ``INTELLECT_KANBAN_BOARD`` env →
    ...
    if db_path is not None:
        ...
        try:
            from agent.kanban_unified import connect_unified, kanban_storage_mode

            if kanban_storage_mode() == "unified":
                return connect_unified(board=board)
        except ImportError:
            pass
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See intellect_state._WAL_INCOMPAT_MARKERS for detection logic.
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``intellect kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``intellect_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from intellect_cli.profiles import normalize_profile_name

    return normalize_profile_name(assignee)


def _kanban_unified_storage() -> bool:
    try:
        from agent.kanban_unified import kanban_storage_mode

        return kanban_storage_mode() == "unified"
    except ImportError:
        return False


def _resolve_board_id(board: Optional[str] = None) -> str:
    return _normalize_board_slug(board) or get_current_board()


def _append_board_scope(
    query: str, params: list[Any], *, board: Optional[str] = None
) -> tuple[str, list[Any]]:
    if not _kanban_unified_storage():
        return query, params
    query += " AND board_id = ?"
    params.append(_resolve_board_id(board))
    return query, params
...
def create_task(
    ...
    *,
    ...
    each name to ``intellect --skills ...`` alongside the built-in
    ...
    # `intellect --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
    ...
    if idempotency_key:
        ...
        idem_params: list[Any] = [idempotency_key]
        idem_q, idem_params = _append_board_scope(idem_q, idem_params, board=board)
        idem_q += " ORDER BY created_at DESC LIMIT 1"
        row = conn.execute(idem_q, tuple(idem_params)).fetchone()
        if row:
            return row["id"]
...
# Canonical sort-order mappings for ``intellect kanban list --sort``.
...
def list_tasks(
    ...
    *,
    ...
    params: list[Any] = []
    query, params = _append_board_scope(query, params, board=board)
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `intellect kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``intellect kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``intellect kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``intellect kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``intellect kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``intellect_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Intellect' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Intellect-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``intellect kanban show <id>`` and the dashboard.
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than an Intellect
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``intellect kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Intellect profile.
    ...
    if not rows:
        return False
    try:
        from intellect_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def dispatch_once(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from intellect_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Intellect profile.
        # `_default_spawn` invokes ``intellect -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than an Intellect
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `intellect kanban tail` — without
...
def _module_intellect_argv() -> list[str]:
    """Return the interpreter-bound Intellect CLI invocation."""
    # ``intellect_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``intellect`` package — there is no
    # ``intellect`` package to import.
    return [sys.executable, "-m", "intellect_cli.main"]


def _absolute_intellect_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Intellect shim."""
...
def _intellect_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Intellect executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_intellect_argv()
    return [_absolute_intellect_path(path)]


def _resolve_intellect_argv() -> list[str]:
    """Resolve the ``intellect`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$INTELLECT_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("intellect")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\intellect.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m intellect_cli.main`` — fallback for setups where
       Intellect is launched from a venv and the ``intellect`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_intellect_bin`` for the same reason. Kept
    local (not imported from gateway) because ``intellect_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("INTELLECT_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _intellect_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _intellect_path_argv(resolved_env_bin)
        return _module_intellect_argv()

    intellect_bin = _safe_which_no_cwd("intellect") if _IS_WINDOWS else shutil.which("intellect")
    if intellect_bin:
        return _intellect_path_argv(intellect_bin)
    return _module_intellect_argv()


def _kanban_worker_skill_available(intellect_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``intellect -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset INTELLECT_HOME means the worker falls back to the default root
    # home (``~/.intellect``), which ships the bundled skill.
    base = _Path(intellect_home) if intellect_home else (_Path.home() / ".intellect")
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``intellect -p <profile> chat -q ...`` subprocess.
    ...
    ``intellect_KANBAN_DB`` / ``INTELLECT_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject INTELLECT_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `intellect -p <name>` the
    # _apply_profile_override() runs *before* intellect_constants is imported.
    # If INTELLECT_HOME is absent from the child's env, get_intellect_home() falls
    # back to Path.home() / ".intellect" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from intellect_cli.profiles import resolve_profile_env
    try:
        env["INTELLECT_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via intellect_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # INTELLECT_HOME never had profiles created.
        pass
    if task.tenant:
        env["intellect_TENANT"] = task.tenant
    env["INTELLECT_KANBAN_TASK"] = task.id
    env["intellect_KANBAN_WORKSPACE"] = workspace
    if task.branch_name:
        env["intellect_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["intellect_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["intellect_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["INTELLECT_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["INTELLECT_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`intellect -p <name>`
    # rewrites INTELLECT_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_intellect_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["intellect_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["intellect_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["INTELLECT_KANBAN_BOARD"] = resolved_board
    # intellect_PROFILE is the author the kanban_comment tool defaults to.
    # `intellect -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["intellect_PROFILE"] = profile_arg

    cmd = [
        *_resolve_intellect_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped INTELLECT_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("INTELLECT_HOME")):
    ...
    # `intellect kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`intellect` executable not found on PATH. "
            "Install Intellect Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``intellect kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled intellect_PROFILE values like "intellect-system"
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``intellect kanban assignees`` for the terminal.
