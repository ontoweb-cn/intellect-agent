...
Usage:
    intellect                     # Interactive chat (default)
    intellect chat                # Interactive chat
    intellect gateway             # Run gateway in foreground
    intellect gateway start       # Start gateway as service
    intellect gateway stop        # Stop gateway service
    intellect gateway status      # Show gateway status
    intellect gateway install     # Install gateway service
    intellect gateway uninstall   # Uninstall gateway service
    intellect setup               # Interactive setup wizard
    intellect logout              # Clear stored authentication
    intellect status              # Show status of all components
    intellect cron                # Manage cron jobs
    intellect cron list           # List cron jobs
    intellect cron status         # Check if cron scheduler is running
    intellect doctor              # Check configuration and dependencies
    intellect honcho setup                    # Configure Honcho AI memory integration
    intellect honcho status                   # Show Honcho config and connection status
    intellect honcho sessions                 # List directory → session name mappings
    intellect honcho map <name>               # Map current directory to a session name
    intellect honcho peer                     # Show peer names and dialectic settings
    intellect honcho peer --user NAME         # Set user peer name
    intellect honcho peer --ai NAME           # Set AI peer name
    intellect honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    intellect honcho mode                     # Show current memory mode
    intellect honcho mode [hybrid|honcho|local]  # Set memory mode
    intellect honcho tokens                   # Show token budget settings
    intellect honcho tokens --context N       # Set session.context() token cap
    intellect honcho tokens --dialectic N     # Set dialectic result char cap
    intellect honcho identity                 # Show AI peer identity representation
    intellect honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    intellect honcho migrate                  # Step-by-step migration guide: OpenClaw native → Intellect + Honcho
    intellect version             Show version
    intellect update              Update to latest version
    intellect uninstall           Uninstall Intellect Agent
    intellect acp                 Run as an ACP server for editor integration
    intellect sessions browse     Interactive session picker with search

    intellect claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: intellect_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``intellect_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``intellect update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``intellect_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, intellect crashes on import and the user can't run
# ``intellect update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
def _set_process_title() -> None:
    """Set the process title to 'intellect' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``intellect tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``intellect.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("intellect")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"intellect", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
...
# `entry.tsx`; this is just the earlier cousin. ``intellect_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("intellect_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not (os.environ.get("intellect_TUI") == "1" or "--tui" in sys.argv[1:]):
        return
    try:
        # Skip when stdout is redirected (`intellect --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from intellect_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Intellect Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``intellect --version`` before config/logging imports on Termux."""
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to INTELLECT_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (intellect tools, intellect setup, intellect model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'intellect {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any intellect module import.
#
# Many modules cache INTELLECT_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("INTELLECT_HOME", ...)`` resolves correctly.
...
def _apply_profile_override() -> None:
    ...
    if profile_name is not None:
        try:
            ...
            intellect_home = resolve_profile_env(profile_name)
            ...
            return
        os.environ["INTELLECT_HOME"] = intellect_home
...
# Load .env from ~/.intellect/.env first, then project root as dev fallback.
...
load_intellect_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → INTELLECT_REDACT_SECRETS env
# var BEFORE intellect_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_intellect_home() / "config.yaml"
    if _cfg_path.exists():
        with open(_cfg_path, encoding="utf-8") as _f:
            _early_cfg_raw = _yaml_early.safe_load(_f) or {}
        if "INTELLECT_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["INTELLECT_REDACT_SECRETS"] = str(_early_redact).lower()
...
if _FORCE_IPV4_EARLY:
    try:
        from intellect_constants import apply_ipv4_preference as _apply_ipv4
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_intellect_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("INTELLECT_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("INTELLECT_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from intellect_cli.config import get_env_path, get_intellect_home, load_config
    from intellect_cli.auth import get_auth_status

    # Determine whether Intellect itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_intellect_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    # Only count these if Intellect has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Intellect to use their tokens.
    if _has_intellect_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, intellect_bin
        cli_args: the original CLI arguments (everything after 'intellect')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    intellect_bin = container_info["intellect_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo intellect {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo intellect {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  intellect --tui --resume {target}")
    if title:
        print(f'  intellect --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @intellect/ink is missing or node_modules is behind package-lock.json.
    ...
    ink = root / "node_modules" / "@intellect" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/intellect-ink/package.json",
    "packages/intellect-ink/package-lock.json",
    "packages/intellect-ink/index.js",
    "packages/intellect-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``intellect_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("intellect_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``INTELLECT_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("INTELLECT_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    intellect_home = os.environ.get("INTELLECT_HOME") or str(Path.home() / ".intellect")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "INTELLECT_HOME": intellect_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(intellect_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(intellect_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if intellect_cli_dir is None:
        intellect_cli_dir = Path(__file__).parent
    bundled = intellect_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (INTELLECT_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("INTELLECT_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from intellect_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("INTELLECT_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with INTELLECT_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset INTELLECT_TUI_DIR (e.g. `unset INTELLECT_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if _tui_need_npm_install(tui_dir):
        npm = _node_bin("npm")
        if not os.environ.get("INTELLECT_QUIET"):
    ...
    if tui_dev:
        # Keep the local @intellect/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @intellect/ink resolves through
        # packages/intellect-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "intellect-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from intellect_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="intellect-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["intellect_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["INTELLECT_PYTHON_SRC_ROOT"] = os.environ.get(
        "INTELLECT_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("INTELLECT_PYTHON", sys.executable)
    env.setdefault("intellect_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["intellect_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["intellect_MODEL"] = model
        env["intellect_INFERENCE_MODEL"] = model
    if provider:
        env["intellect_TUI_PROVIDER"] = provider
        env["INTELLECT_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["intellect_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["intellect_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["intellect_TUI_SKILLS"] = value
    if query:
        env["intellect_TUI_QUERY"] = query
    if image:
        env["intellect_TUI_IMAGE"] = image
    if checkpoints:
        env["intellect_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["intellect_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["intellect_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["intellect_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["intellect_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["INTELLECT_ACCEPT_HOOKS"] = "1"
    ...
    # intellect_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `intellect --tui` try to
    ...
    # still set intellect_TUI_RESUME themselves.
    env.pop("intellect_TUI_RESUME", None)
    if resume_session_id:
        env["intellect_TUI_RESUME"] = resume_session_id
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``INTELLECT_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``intellect kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``intellect kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("INTELLECT_KANBAN_BOARD"):
        return
    try:
        from intellect_cli.kanban_db import get_current_board
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'intellect sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        ...
        from intellect_cli.config import load_config as _load_config_for_xai_check
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Intellect isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  intellect setup")
        print()

        from intellect_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'intellect setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from intellect_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["intellect_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.intellect/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["INTELLECT_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["INTELLECT_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["INTELLECT_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from intellect_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from intellect_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Intellect?")
        print()
    ...
    # has no creds.json.  Every subsequent `intellect gateway` then paid a 30s
    ...
    session_dir = get_intellect_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: intellect gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `intellect gateway` skips it cleanly instead of paying a 30s
        ...
        print("⚠ Pairing may not have completed. Run 'intellect whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from intellect_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from intellect_cli.config import stamp_install_method
    from intellect_cli.dep_ensure import ensure_dependency
...
def cmd_login(args):
    """Authenticate Intellect CLI with a provider."""
    from intellect_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from intellect_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from intellect_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from intellect_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from intellect_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from intellect_cli.webhook import webhook_command

    webhook_command(args)


def cmd_vault(args):
    """Vault build scheduling helpers."""
    from intellect_cli.vault_cmd import vault_command
...
def cmd_slack(args):
    ...
    Dispatches ``intellect slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: intellect slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `intellect slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from intellect_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from intellect_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from intellect_cli.hooks import hooks_command
...
def cmd_security(args):
    """Dispatch `intellect security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from intellect_cli.security_audit import cmd_security_audit
...
def cmd_members(args):
    ...
    if sub == "bootstrap":
        return _cmd_members_bootstrap(args, config)
    elif sub == "invite":
        return _cmd_members_invite(args, config)
    elif sub == "register":
        return _cmd_members_register(args, config)
    elif sub == "list":
        return _cmd_members_list(config)
    elif sub == "show":
        return _cmd_members_show(args, config)
    elif sub == "add":
        return _cmd_members_add(args, config)
    elif sub == "activate":
        return _cmd_members_activate(args, config)
    elif sub == "deactivate":
        return _cmd_members_deactivate(args, config)
    elif sub == "delete":
        return _cmd_members_delete(args, config)
    elif sub == "grant-owner":
        return _cmd_members_grant_owner(args, config)
    elif sub == "audit":
        return _cmd_members_audit(args, config)
    elif sub == "bind":
        return _cmd_members_bind(args, config)
    elif sub == "identities":
        return _cmd_members_identities(args, config)

    elif sub == "workspace":
        return _cmd_members_workspace(args, config)

    elif sub == "login":
        return _cmd_members_login(args, config)

    elif sub == "logout":
        return _cmd_members_logout(args, config)

    elif sub == "whoami":
        return _cmd_members_whoami(args, config)

    elif sub == "role":
        return _cmd_members_role(args, config)

    elif sub == "passwd":
        return _cmd_members_passwd(args, config)

    elif sub == "reset":
        return _cmd_members_reset(args, config)

    elif sub == "teams":
        return _cmd_teams(args, config)

    elif sub == "projects":
        return _cmd_members_projects(args, config)

    elif sub == "sessions":
        from intellect_cli.members_sessions import (
            cmd_members_sessions_audit_null,
            cmd_members_sessions_migrate_ownership,
        )

        sessions_sub = getattr(args, "sessions_command", None)
        if sessions_sub == "migrate-ownership":
            return cmd_members_sessions_migrate_ownership(args)
        if sessions_sub == "audit-null":
            return cmd_members_sessions_audit_null(args)
        print(
            "Usage: intellect members sessions <audit-null|migrate-ownership> ...",
            file=sys.stderr,
        )
        return 1
    else:
        print("Usage: intellect members <list|show|create|login|teams|projects|sessions>", file=sys.stderr)
        return 1


def cmd_oauth(args):
    """Dispatch ``intellect oauth <subcmd>``."""
    from agent.oauth import OAuthEngine
    from agent.membership import MembershipDB

    config = __import__("intellect_cli.config", fromlist=["load_config"]).load_config()
    db = MembershipDB(config=config)
    engine = OAuthEngine(config=config, db=db)

    sub = getattr(args, "oauth_command", None)

    if sub == "list":
        providers = engine.list_providers()
        if not providers:
            print("No OAuth providers found.")
        else:
            print(f"{'ID':20s} {'NAME':22s} {'USAGE':10s} {'FLOW':16s} {'ENABLED'}")
            print("-" * 78)
            for p in providers:
                print(f"{p.id:20s} {p.name[:20]:22s} {p.usage:10s} {p.auth_flow:16s} {'✓' if p.enabled else '—'}")
        db.close()
        return 0

    elif sub == "enable":
        pid = getattr(args, "provider_id", "")
        if not pid:
            print("Usage: intellect oauth enable <provider_id>", file=sys.stderr)
            db.close()
            return 1
        _toggle_provider(db, pid, True)
        db.close()
        return 0
        ...
        pid = getattr(args, "provider_id", "")
        if not pid:
            print("Usage: intellect oauth disable <provider_id>", file=sys.stderr)
            db.close()
            return 1
        _toggle_provider(db, pid, False)
        db.close()
        return 0

    elif sub == "show":
        pid = getattr(args, "provider_id", "")
        p = engine.get_provider(pid)
        if not p:
            print(f"Provider '{pid}' not found.", file=sys.stderr)
            db.close()
            return 1
        print(f"ID:              {p.id}")
        print(f"Name:            {p.name}")
        print(f"Usage:           {p.usage}")
        print(f"Auth Flow:       {p.auth_flow}")
        print(f"Enabled:         {'yes' if p.enabled else 'no'}")
        print(f"Client ID:       {p.client_id or '(not set)'}")
        print(f"Scopes:          {', '.join(p.scopes)}")
        if p.authorize_url:  print(f"Authorize URL:   {p.authorize_url}")
        if p.token_url:      print(f"Token URL:       {p.token_url}")
        if p.userinfo_url:   print(f"Userinfo URL:    {p.userinfo_url}")
        db.close()
        return 0

    elif sub == "add":
        _cmd_oauth_add(args, db)
        db.close()
        return 0

    elif sub == "remove":
        pid = getattr(args, "provider_id", "")
        _remove_provider(db, pid)
        db.close()
        return 0

    elif sub == "seed-builtin":
        from agent.oauth.builtin_catalog import seed_builtin_oauth_providers_db

        force = bool(getattr(args, "refresh_metadata", False))
        stats = seed_builtin_oauth_providers_db(db, force_metadata=force)
        print(
            f"Catalog {stats.get('catalog_id') or '(unknown)'}: "
            f"{stats.get('total', 0)} providers, "
            f"{stats.get('inserted', 0)} inserted"
            + (
                f", {stats.get('metadata_updated', 0)} metadata refreshed"
                if force
                else ""
            )
        )
    ...
    elif sub == "migrate-from-config":
        from agent.oauth.migrate_from_config import (
            clear_yaml_providers_in_config,
            migrate_yaml_providers_to_db,
        ...
        if not entries:
            print("No members.oauth.providers entries found in config.yaml.")
            db.close()
            return 0

        dry_run = bool(getattr(args, "dry_run", False))
        stats = migrate_yaml_providers_to_db(
            config,
            db,
            dry_run=dry_run,
            force_secrets=bool(getattr(args, "force_secrets", False)),
            force_client_id=bool(getattr(args, "force_client_id", False)),
        )
        label = "Dry run" if dry_run else "Migrated"
        print(
            f"{label}: {stats.get('yaml_count', 0)} YAML provider(s) — "
            f"{stats.get('inserted', 0)} inserted, "
            f"{stats.get('updated', 0)} updated, "
            f"{stats.get('skipped', 0)} skipped."
        )
        for item in stats.get("providers", []):
            detail = item.get("action", "")
            if item.get("fields"):
                detail += f" ({', '.join(item['fields'])})"
            print(f"  {item.get('id')}: {detail}")
        for err in stats.get("errors", []):
            print(f"  Error {err.get('id')}: {err.get('error')}", file=sys.stderr)

        if getattr(args, "write_config", False) and not dry_run and not stats.get("errors"):
            cfg = load_config()
            if clear_yaml_providers_in_config(cfg):
                save_config(cfg)
                print(
                    "Cleared members.oauth.providers in config.yaml (set to []). "
                    "Manage providers via Settings or `intellect oauth`."
                )
        db.close()
        return 1 if stats.get("errors") else 0

    elif sub == "migrate-from-auth-json":
        ...
        if not summary.get("provider_count") and not dry_run:
            print("No OAuth providers or credential_pool entries in auth.json.")
            db.close()
            return 0

        stats = migrate_auth_json_to_db(
            db,
            dry_run=dry_run,
            prune_auth_json=bool(getattr(args, "prune_auth_json", False)),
        )
        label = "Dry run" if dry_run else "Migrated"
        print(
            f"{label}: auth.json — {summary.get('provider_count', 0)} provider(s), "
            f"{summary.get('pool_entry_count', 0)} pool entry(ies)."
        )
        if not dry_run:
            print(f"  oauth_tokens rows written/updated: {stats.get('migrated', 0)}")
            if stats.get("pruned"):
                print("  Cleared providers/credential_pool in auth.json.")
        for err in stats.get("errors", []):
        ...
        print(
            "Usage: intellect oauth <list|enable|disable|show|add|remove|"
            "seed-builtin|migrate-from-config|migrate-from-auth-json>",
            file=sys.stderr,
        )
        db.close()
        return 1
...
def _toggle_provider(db, provider_id: str, enabled: bool) -> None:
    """Enable or disable an OAuth provider in the database."""
    import json, time
    now = time.time()
    label = "enabled" if enabled else "disabled"
    affected = [0]  # mutable to track inside closure

    def _update(cursor):
        cursor.execute(
            "UPDATE oauth_providers SET enabled=?, updated_at=? WHERE id=?",
            (1 if enabled else 0, now, provider_id),
        )
        affected[0] = cursor.rowcount
        if cursor.rowcount == 0:
            # Provider not in DB yet — seed a row from the builtin list
            from agent.oauth import OAuthEngine
            engine = OAuthEngine(db=db)
            bp = engine.get_provider(provider_id)
            if bp:
                cursor.execute(
                    ...
                    "token_storage, is_builtin, display_order, created_at, updated_at) "
                    "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,1,?,?,?)",
                    (bp.id, bp.name, bp.usage, bp.auth_flow,
                     1 if enabled else 0,
                     bp.authorize_url, bp.token_url, bp.userinfo_url,
                     json.dumps(bp.scopes),
                     bp.claim_sub, bp.claim_email, bp.claim_name,
                     bp.token_storage, bp.display_order, now, now),
                )
                affected[0] = cursor.rowcount
    ...
    try:
        ...
        if affected[0] > 0:
            ...
            print(f"Provider '{provider_id}' not found.", file=__import__("sys").stderr)
    except Exception as e:
        print(f"Error: {e}", file=__import__("sys").stderr)


def _cmd_oauth_add(args, db) -> None:
    """Add a custom OAuth provider to the database."""
    ...
    existing = db._conn.execute(
        "SELECT id FROM oauth_providers WHERE id=?", (pid,)
    ).fetchone()
    if existing:
        print(f"Error: Provider '{pid}' already exists.", file=sys.stderr)
        return

    scopes = [s.strip() for s in getattr(args, "scopes", "openid").split(",") if s.strip()]
    now = time.time()

    def _insert(cursor):
        cursor.execute(
            "INSERT INTO oauth_providers (id, name, usage, auth_flow, enabled, "
            "client_id, authorize_url, token_url, userinfo_url, scopes, "
            "oidc_discovery_url, is_builtin, created_at, updated_at) "
            "VALUES (?, ?, ?, ?, 1, ?, ?, ?, ?, ?, ?, 0, ?, ?)",
            (pid, getattr(args, "name", pid),
             getattr(args, "usage", "login"),
             getattr(args, "flow", "pkce_loopback"),
             getattr(args, "client_id", ""),
             getattr(args, "authorize_url", ""),
             getattr(args, "token_url", ""),
             getattr(args, "userinfo_url", ""),
             json.dumps(scopes),
             getattr(args, "discovery_url", ""),
             now, now),
        )
    db._execute_write(_insert)
    print(f"Provider '{pid}' added. Enable it with: intellect oauth enable {pid}")


def _remove_provider(db, provider_id: str) -> None:
    """Remove a custom OAuth provider from the database."""
    ...
    if not row:
        ...
        return
    if row["is_builtin"]:
        print(f"Error: '{provider_id}' is a built-in provider and cannot be removed.", file=sys.stderr)
        return

    def _delete(cursor):
        cursor.execute("DELETE FROM oauth_tokens WHERE provider_id=?", (provider_id,))
        cursor.execute("DELETE FROM oauth_providers WHERE id=?", (provider_id,))
    db._execute_write(_delete)
    print(f"Provider '{provider_id}' removed.")


def _cmd_members_bootstrap(args, config):
    """Bootstrap the full multi-user environment: default member + team + project."""
    import sys

    from agent.members_bootstrap import run_members_bootstrap
    from intellect_cli.config import load_config, save_config
    ...
    if storage != "sqlite":
        print(
            "Error: --storage must be sqlite.",
            file=sys.stderr,
        )
        return 1

    cfg = load_config()
    storage_block = cfg.setdefault("storage", {})
    if not isinstance(storage_block, dict):
        storage_block = {}
        cfg["storage"] = storage_block
    storage_block["backend"] = "sqlite"
    save_config(cfg)
    config = cfg

    try:
        result = run_members_bootstrap(
            config,
            admin_login=getattr(args, "admin_login", None),
            team_slug=getattr(args, "team", "default"),
            project_slug=getattr(args, "project", "default"),
        )
    except RuntimeError as exc:
        print(f"Error: {exc}", file=sys.stderr)
        return 1
    if result.created_member:
        print(f"Created member: {result.admin_login}")
    else:
        print(f"Member '{result.admin_login}' already exists.")
    if result.created_team:
        print(f"Created team: {getattr(args, 'team', 'default')}")
    if result.created_project:
        print(f"Created project: {getattr(args, 'project', 'default')}")
    print(f"\nBootstrap complete. Logged in as '{result.admin_login}'.")
    print(f"Session: {get_intellect_home() / '.cli-session.json'}")
    return 0


def _cmd_members_audit(args, config):
    """List or tail member management audit log."""
    from agent.membership import MembershipDB, can_view_member_audit
    from datetime import datetime, timezone

    sub = getattr(args, "audit_command", None) or "tail"
    member_id, _role = _resolve_current_member_id()
    db = MembershipDB(config=config)
    if not can_view_member_audit(db, member_id, config):
        print("Error: Profile admin access required for audit log.", file=sys.stderr)
        db.close()
        return 1
    if sub == "list":
        rows = db.list_member_admin_audit(
            actor_member_id=getattr(args, "actor", None),
            target_member_id=getattr(args, "target", None),
            action=getattr(args, "action", None),
            limit=getattr(args, "limit", 50),
        )
    else:
        rows = db.list_member_admin_audit(limit=getattr(args, "n", 20))
    for row in rows:
        ...
        print(
            f"{when}  {row.get('action')}  actor={row.get('actor_member_id')}  "
            f"target={row.get('target_member_id') or '-'}  source={row.get('source')}{extra}"
        )
    db.close()
    return 0


def _cmd_members_invite(args, config):
    """Create an invite code for a new member (owner, admin, or profile admin)."""
    from agent.membership import MembershipDB, can_create_invites

    member_id, actor_role = _resolve_current_member_id()
    db = MembershipDB(config=config)
    if not can_create_invites(db, member_id, config):
        print(
            "Error: Only a profile owner, admin, or profile admin can create invite codes.",
            file=sys.stderr,
        )
        db.close()
        return 1

    ttl_str = getattr(args, "ttl", "168h")
    ...
    code = db.create_invite(member_id, email=getattr(args, "email", None), ttl_seconds=ttl_seconds, actor_role=actor_role)
    print(f"Invite code: {code}")
    print(f"  Expires in: {ttl_str}")
    print(f"  Register with: intellect members register {code}")
    db.close()
    return 0


def _cmd_members_register(args, config):
    """Register a new member using an invite code, with password.

    The invite is validated first (no member created). All interactive input
    is collected before any DB writes to avoid ghost members on crash/Ctrl+C.
    """
    from agent.membership import MembershipDB
    from intellect_cli.secret_prompt import masked_secret_prompt
    ...
    # Phase 2: collect all interactive input before touching DB
    try:
        ...
        return 1
    if not login:
        print("Error: Login name is required.", file=sys.stderr)
        db.close()
        return 1
    if db.get_member_by_login(login):
        print(f"Error: Member '{login}' already exists.", file=sys.stderr)
        db.close()
        return 1
    ...
    # Phase 3: atomically create member, set password, and mark invite used
    member_id, email = db.redeem_invite(code, login)
    if not member_id:
        print("Error: Could not create member.", file=sys.stderr)
        db.close()
        return 1
    db.set_member_password(member_id, pw1)

    from agent.membership import ensure_member_dirs
    ensure_member_dirs(member_id)

    # Write session + bind CLI identity + record online presence
    _write_cli_session({"id": member_id, "login_name": login, "display_name": login.capitalize()}, db)

    # OAuth binding if requested
    oauth_provider = getattr(args, "oauth", None)
    if oauth_provider:
        print()
        try:
            _oauth_register_bind(member_id, login, oauth_provider, config)
        except Exception as exc:
            print(f"  OAuth binding skipped: {exc}")
            print(f"  Bind later with: intellect members bind {login} --provider {oauth_provider}")

    print(f"Registered as '{login}'. You are now logged in.")
    db.close()
    return 0


def _oauth_register_bind(member_id: str, login: str, provider_id: str, config: dict) -> None:
    """Run OAuth flow and bind identity to a newly registered member."""
    from agent.members_oauth import (
        is_oauth_enabled, resolve_provider, generate_pkce_pair,
        create_oauth_state, build_authorization_url,
        exchange_code_for_tokens, extract_claims,
    )
    from agent.membership import MembershipDB

    if not is_oauth_enabled(config):
        print("  OAuth is not enabled. Skipping.")
        return

    provider = resolve_provider(config, provider_id)
    if not provider:
        print(f"  Unknown OAuth provider '{provider_id}'. Skipping.")
        return
    ...
    state = create_oauth_state(provider_id, verifier)
    auth_url = build_authorization_url(provider, redirect_uri, state, challenge or None)
    if not auth_url:
        print("  Could not build authorization URL. Skipping.")
        return

    import webbrowser, time, json, urllib.parse
    ...
    server = HTTPServer(("127.0.0.1", 18923), CallbackHandler)
    server.timeout = 120
    try:
    ...
    if result.get("error") or not result.get("code"):
        print("  OAuth authorization was not completed.")
        return

    token_resp = exchange_code_for_tokens(provider, result["code"], redirect_uri, verifier)
    if not token_resp:
        print("  Token exchange failed. Skipping.")
        return

    claims = extract_claims(provider, token_resp)
    if not claims:
        print("  Could not extract identity. Skipping.")
        return

    provider_name = f"oauth:{provider_id}"
    ...
    def _bind(cursor):
        cursor.execute(
            ...
            "(id, member_id, provider, provider_id, email, display_name, raw, created_at) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (f"{provider_name}:{external_id}", member_id, provider_name, external_id,
             claims.get("email", ""), claims.get("name", ""), json.dumps(token_resp), time.time()),
        )
    db._execute_write(_bind)
    db.close()
    print(f"  Identity linked: {login} ↔ {provider_id}:{claims.get('preferred_username', external_id)}")

def _cmd_members_list(config):
    """List all members with online status."""
    from agent.membership import MembershipDB
    import time
    db = MembershipDB(config=config)
    ...
    if not members:
        ...
        print(f"{'LOGIN':16s} {'NAME':20s} {'ROLE':8s} {'STATUS':8s} {'LAST SEEN'}")
        print("-" * 80)
        now = time.time()
        for m in members:
            status = m.get("online_status", "offline") or "offline"
            last_at = m.get("last_active_at")
            if last_at:
                diff = int(now - last_at)
                if diff < 60:
                    seen = "now"
                elif diff < 3600:
                    seen = f"{diff // 60}m ago"
                elif diff < 86400:
                    seen = f"{diff // 3600}h ago"
                else:
                    seen = f"{diff // 86400}d ago"
                platform = m.get("last_active_platform", "") or ""
                if platform:
                    seen += f" ({platform})"
            else:
                seen = "never"
            print(
                f"{m.get('login_name', '')[:14]:16s} "
                f"{m.get('display_name', '')[:18]:20s} "
                f"{m.get('role', 'member') or 'member':8s} "
                f"{status:8s} "
                f"{seen}"
            )
    db.close()
...
def _cmd_members_show(args, config):
    """Show member details."""
    from agent.membership import MembershipDB, normalize_member_lookup
    db = MembershipDB(config=config)
    db.cleanup_expired_sessions()  # Ensure online status is current
    m = normalize_member_lookup(args.login, db)
    if not m:
        print(f"Error: Member '{args.login}' not found.", file=sys.stderr)
        db.close()
        return 1
    print(f"Login:       {m.get('login_name', '')}")
    print(f"Name:        {m.get('display_name', '')}")
    print(f"Email:       {m.get('email', '') or '(not set)'}")
    online = m.get("online_status", "offline") or "offline"
    print(f"Enabled:     {'yes' if m.get('enabled') else 'no'}")
    print(f"Online:      {online}")
    if online == "online":
        import time
        sessions = db.list_active_sessions(m["id"])
        if sessions:
            print(f"Active sessions ({len(sessions)}):")
            for s in sessions:
                plat = s.get("platform", "?")
                since = s.get("login_at", "")
                print(f"  {plat:12s} since {since}")
    print(f"ID:          {m['id']}")
    print(f"Created:     {m.get('created_at', '')}")
    # List tokens
    tokens = db.list_member_tokens(m["id"])
    if tokens:
...
def _cmd_members_add(args, config):
    """Add a new member (owner only)."""
    from agent.membership import MembershipDB, authorize, Action
    db = MembershipDB(config=config)
    member_id, actor_role = _resolve_current_member_id()
    if not authorize(actor_role, Action.ADMIN):
        print("Error: Only an owner can add members.", file=sys.stderr)
        db.close()
        return 1
    existing = db.get_member_by_login(args.login)
    if existing:
        print(f"Error: Member '{args.login}' already exists.", file=sys.stderr)
        db.close()
        return 1
    mid = db.create_member(
        display_name=getattr(args, "name", None) or args.login,
        login_name=args.login,
        email=getattr(args, "email", None),
        platform="cli",
        member_id=getattr(args, "id", None),
    )
    if not mid:
        print("Error: Could not create member.", file=sys.stderr)
        db.close()
    ...
    print(f"Member '{args.login}' added (id={mid}).")
    print(f"  They can login with: intellect members login {args.login}")
    db.close()
    return 0


def _cmd_members_activate(args, config):
    """Activate a disabled member (owner only)."""
    from agent.membership import MembershipDB, authorize, Action, normalize_member_lookup
    db = MembershipDB(config=config)
    member_id, actor_role = _resolve_current_member_id()
    if not authorize(actor_role, Action.ADMIN):
        print("Error: Only an owner can activate members.", file=sys.stderr)
        db.close()
        return 1
    target = normalize_member_lookup(args.login, db)
    if not target:
        print(f"Error: Member '{args.login}' not found.", file=sys.stderr)
        db.close()
        return 1
    db.activate_member(target["id"], actor_role=actor_role)
    print(f"Member '{args.login}' activated.")
    db.close()
    return 0


def _cmd_members_deactivate(args, config):
    """Deactivate a member (owner only)."""
    from agent.membership import MembershipDB, authorize, Action, normalize_member_lookup
    db = MembershipDB(config=config)
    member_id, actor_role = _resolve_current_member_id()
    if not authorize(actor_role, Action.ADMIN):
        print("Error: Only an owner can deactivate members.", file=sys.stderr)
        db.close()
        return 1
    target = normalize_member_lookup(args.login, db)
    if not target:
        ...
        db.close()
        return 1
    if target["id"] == member_id:
        print("Error: Cannot deactivate yourself.", file=sys.stderr)
        db.close()
        return 1
    db.deactivate_member(target["id"], actor_role=actor_role)
    print(f"Member '{args.login}' deactivated.")
    db.close()
    return 0


def _cmd_members_delete(args, config):
    """Permanently delete a member (owner only)."""
    from agent.membership import MembershipDB, authorize, Action, normalize_member_lookup
    db = MembershipDB(config=config)
    member_id, actor_role = _resolve_current_member_id()
    if not authorize(actor_role, Action.ADMIN):
        print("Error: Only an owner can delete members.", file=sys.stderr)
        db.close()
        return 1
    target = normalize_member_lookup(args.login, db)
    if not target:
        ...
        return 1
    if target["id"] == member_id:
        print("Error: Cannot delete yourself.", file=sys.stderr)
        db.close()
        return 1
    try:
        confirm = input(f"Type '{args.login}' to confirm permanent deletion: ").strip()
    except (EOFError, KeyboardInterrupt):
        print()
        db.close()
        return 1
    if confirm != args.login:
        print("Confirmation does not match. Aborted.")
        db.close()
        return 1
    if not db.delete_member(target["id"], actor_role=actor_role):
        print("Error: Failed to delete member.", file=sys.stderr)
        db.close()
        return 1
    print(f"Member '{args.login}' permanently deleted.")
    db.close()
    return 0


def _cmd_members_grant_owner(args, config):
    """Grant owner role to a member (owner only)."""
    from agent.membership import MembershipDB, authorize, Action, normalize_member_lookup
    db = MembershipDB(config=config)
    member_id, actor_role = _resolve_current_member_id()
    if not authorize(actor_role, Action.ADMIN):
        print("Error: Only an owner can grant owner privileges.", file=sys.stderr)
        db.close()
        return 1
    target = normalize_member_lookup(args.login, db)
    if not target:
    ...
    try:
        confirm = input(
            f"Grant '{args.login}' full owner privileges? [y/N]: "
        ).strip().lower()
    except (EOFError, KeyboardInterrupt):
        print()
        db.close()
    ...
    if confirm not in ("y", "yes"):
        ...
        db.close()
        return 1
    db.grant_owner(target["id"], actor_role=actor_role)
    print(f"'{args.login}' is now an owner.")
    db.close()
    return 0


def _cmd_members_bind(args, config):
    """Link an OAuth provider to a member: members bind --oauth <provider> [--login <login>]"""
    oauth_provider = getattr(args, "oauth", None)
    if not oauth_provider:
        print("Error: --oauth <provider> is required.", file=sys.stderr)
        print("  Example: intellect members bind --oauth github", file=sys.stderr)
        return 1

    from agent.membership import MembershipDB, Action, authorize
    import json
    from intellect_constants import get_intellect_home

    member_id = None
    specified_login = getattr(args, "login", None)
    db = MembershipDB(config=config)
    actor_id, actor_role = _resolve_current_member_id()
    if specified_login:
        m = db.get_member_by_login(specified_login)
        if m:
            member_id = m["id"]
        if not member_id:
            print(f"Error: Member '{specified_login}' not found.", file=sys.stderr)
            db.close()
            return 1
    else:
    ...
    if not member_id:
        print("Error: No member specified. Use --login <login> or log in first.", file=sys.stderr)
        db.close()
        return 1

    if actor_id and actor_id != member_id:
        if not authorize(actor_role, Action.ADMIN, config=config):
            print("Error: Only owner/admin can bind OAuth for another member.", file=sys.stderr)
            db.close()
            return 1
    db.close()

    print(f"Starting OAuth flow with {oauth_provider} to link identity...")
    print(f"This will add {oauth_provider} as a sign-in method for your account.")
    return _oauth_login(oauth_provider, config, link_member_id=member_id)


def _cmd_members_identities(args, config):
    """List linked OAuth identities for a member."""
    import json
    from intellect_constants import get_intellect_home
    ...
    try:
        rows = db._conn.execute(
            "SELECT provider, provider_id, email, display_name, created_at "
            "FROM identities WHERE member_id = ? ORDER BY created_at",
            (member_id,),
        ).fetchall()
    except Exception:
        rows = []
    ...
    m = MembershipDB(config=config).get_member(member_id) if member_id else None
    login = m.get("login_name", member_id) if m else member_id

    print(f"Identities for '{login}':")
    if not rows:
        print("  (none)")
    else:
        for r in rows:
            plat = r["provider"]
            ext_id = r["provider_id"] or ""
            email = r["email"] or ""
            name = r["display_name"] or ""
            detail = f"{name} <{email}>" if name else ext_id[:16]
            print(f"  {plat:20s} {detail}")

    # Show stored OAuth tokens
    try:
        from agent.oauth_tokens import list_member_tokens
        tokens = list_member_tokens(member_id)
        if tokens:
            print(f"\nStored OAuth tokens (git auth):")
            for t in tokens:
                print(f"  {t}")
    except Exception:
        pass
...
def _cmd_members_workspace(args, config):
    """Show a member's workspace path."""
    from agent.membership import MembershipDB, get_member_dir, ensure_member_dirs, normalize_member_lookup
    db = MembershipDB(config=config)
    m = normalize_member_lookup(args.login, db)
    if not m:
        print(f"Error: Member '{args.login}' not found.", file=sys.stderr)
        db.close()
        return 1
    member_dir = ensure_member_dirs(m["id"])
    ws = member_dir / "workspace"
    print(ws)
    db.close()
    return 0


def _cmd_members_login(args, config):
    """Set active member for CLI session (writes .cli-session.json).

    Supports:
    - ``intellect members login <login>`` — login with password
    - ``intellect members login --oauth github`` — OAuth login
    - ``intellect members login --oauth github --device`` — device code flow
    """
    import json
    from agent.membership import MembershipDB, normalize_member_lookup
    from intellect_constants import get_intellect_home
    ...
    oauth_provider = getattr(args, "oauth", None)
    if oauth_provider:
        return _oauth_login(oauth_provider, config, device=getattr(args, "device", False))

    login = getattr(args, "login", None)
    if not login:
        print("Usage: intellect members login <login>  OR  intellect members login --oauth <provider>", file=sys.stderr)
        return 1

    db = MembershipDB(config=config)
    m = normalize_member_lookup(login, db)
    if not m:
        print(f"Error: Member '{login}' not found.", file=sys.stderr)
        db.close()
        return 1
    ...
    # Check lockout
    failed, locked_until = db.get_member_lock_status(member_id)
    if locked_until and time.time() < locked_until:
        remaining = int(locked_until - time.time())
        print(f"Account locked. Try again in {remaining}s.", file=sys.stderr)
        db.close()
        return 1

    # Check for pending reset
    reset_code = m.get("password_reset_code")
    reset_expiry = m.get("password_reset_expiry")
    if reset_code and reset_expiry and time.time() < reset_expiry:
        print("Your password has been reset by an administrator.")
        try:
            ...
            db.close()
            return 1
        if not db.verify_reset_code(member_id, code):
            print("Invalid or expired reset code.", file=sys.stderr)
            db.close()
            return 1
        _prompt_new_password(db, member_id, confirm_label="Logged in as")
        db.clear_reset_code(member_id)
        _write_cli_session(m, db)
        print(f"Logged in as '{login}' ({m['display_name']}).")
        db.close()
        return 0
    ...
    if not pw_hash:
        print("No password set. Create one now.")
        _prompt_new_password(db, member_id, confirm_label="Password set.")
        # Re-read member after password save
    ...
    from intellect_cli.secret_prompt import masked_secret_prompt
    try:
        pw = masked_secret_prompt("Password: ")
    except (EOFError, KeyboardInterrupt):
        print()
        db.close()
        return 1

    if not db.verify_member_password(member_id, pw):
...
def _prompt_new_password(db, member_id: str, confirm_label: str = "Password updated.") -> None:
    """Prompt for new password twice and persist."""
    from intellect_cli.secret_prompt import masked_secret_prompt
    while True:
        try:
            pw1 = masked_secret_prompt("New password: ")
...
def _cmd_members_logout(args, config):
    """Clear the active CLI session (deletes .cli-session.json)."""
    from intellect_constants import get_intellect_home
    ...
    try:
        data = json.loads(session_file.read_text())
        login_name = data.get("login_name", "unknown")
        member_id = data.get("member_id")
    except Exception:
        login_name = "unknown"
        member_id = None

    # End online session BEFORE deleting the session file (so DB state is
    # consistent even if the DB write fails).
    if member_id:
        from agent.membership import MembershipDB
        db = MembershipDB(config=config)
        try:
            db.end_session(member_id, "cli")
        except Exception:
            pass  # Best-effort: DB may be locked or table missing
        finally:
            db.close()
    session_file.unlink()
    # Prevent auto re-login via step 4 (single member fallback)
    logout_marker = get_intellect_home() / ".cli-logged-out"
    logout_marker.touch()
...
def _cmd_members_whoami(args, config):
    """Show the active CLI member identity."""
    from agent.membership import MembershipDB, is_members_enabled
    from agent.runtime_context import resolve_member_id
    from intellect_constants import get_intellect_home

    if not is_members_enabled(config):
        print("Members mode is disabled.")
        return 0
    ...
    if not member_id:
        print("Not logged in. Use: intellect members login <login>")
        return 0

    db = MembershipDB(config=config)
    m = db.get_member(member_id)
    if not m:
        print("Session member not found in database.")
        db.close()
        return 1

    login = m.get("login_name") or member_id
    display = m.get("display_name") or login
    email = m.get("email") or ""
    print(f"Logged in as: {login} ({display})")
    if role:
        print(f"Role: {role}")
    if email:
        print(f"Email: {email}")
    ...
    try:
        ...
        teams = store.list_member_team_memberships(member_id)
        active_teams = [t.get("slug") or t.get("team_id") for t in teams if t.get("status") == "active"]
        if active_teams:
            print(f"Teams: {', '.join(str(x) for x in active_teams if x)}")
        store.close()
    except Exception:
        pass
    ...
    try:
        ...
        pdb = ProjectDB(config=config)
        projects = pdb.list_projects(member_id=member_id)
        if projects:
            slugs = [p.get("slug", "") for p in projects]
            print(f"Projects: {', '.join(s for s in slugs if s)}")
        pdb.close()
    except Exception:
        pass
...
def _cmd_members_role(args, config):
    """Manage custom roles (v2 RBAC): list/show/create/delete/grant/revoke."""
    from agent.membership import MembershipDB, Action, authorize, is_rbac_v2_enabled

    if not is_rbac_v2_enabled(config):
        print(
            "Database-driven RBAC is disabled. Set members.rbac.version: 2 in config.yaml.",
            file=sys.stderr,
        )
        return 1
    ...
    if sub == "list":
        rows = db.list_role_definitions()
        if not rows:
            print("No roles defined.")
        else:
            print(f"{'ID':12s} {'NAME':12s} {'BUILTIN':8s} DISPLAY")
            for r in rows:
                print(
                    f"{r.get('id',''):12s} {r.get('name',''):12s} "
                    f"{'yes' if r.get('is_builtin') else 'no':8s} {r.get('display_name','')}"
                )
        db.close()
        return 0
    ...
    if sub == "show":
        ...
        row = db.get_role_definition(role_id) if role_id else None
        db.close()
        if not row:
            print(f"Role '{role_id}' not found.", file=sys.stderr)
            return 1
        import json as _json
        perms = _json.loads(row.get("permissions") or "[]")
        print(f"Role: {row.get('name')} ({row.get('id')})")
        print(f"  {row.get('description') or ''}")
        print(f"  Permissions ({len(perms)}): {', '.join(perms)}")
        return 0

    if sub == "create":
        if not authorize(actor_role, Action.ADMIN, config=config):
            print("Error: owner/admin required.", file=sys.stderr)
            db.close()
            return 1
        perms = [p.strip() for p in (getattr(args, "permissions", "") or "").split(",") if p.strip()]
        rid = db.create_role_definition(
            getattr(args, "name", ""),
            perms,
        ...
        db.close()
        if not rid:
            print("Error: could not create role (name may exist).", file=sys.stderr)
            return 1
        print(f"Created role '{args.name}' ({rid}).")
        return 0
    ...
    if sub == "delete":
        if not authorize(actor_role, Action.ADMIN, config=config):
            ...
            db.close()
            return 1
        ok = db.delete_role_definition(getattr(args, "role_id", ""), actor_role=actor_role)
        db.close()
        if not ok:
            print("Error: could not delete role (builtin or missing).", file=sys.stderr)
            return 1
        print("Role deleted.")
        return 0
    ...
    if sub in ("grant", "revoke"):
        ...
        login = getattr(args, "login", None)
        role_id = getattr(args, "role_id", None)
        m = db.get_member_by_login(login) if login else None
        if not m:
            print(f"Member '{login}' not found.", file=sys.stderr)
            db.close()
            return 1
        scope = getattr(args, "scope", None)
        scope_id = getattr(args, "scope_id", None)
        if sub == "grant":
            ok = db.grant_role_binding(
                m["id"], role_id,
                scope_type=scope, scope_id=scope_id,
                granted_by=actor_id, actor_role=actor_role,
            )
        else:
            ok = db.revoke_role_binding(
                m["id"], role_id,
                scope_type=scope, scope_id=scope_id,
                actor_role=actor_role,
            )
        ...
        if not ok:
            ...
            return 1
        print(f"Role {sub} succeeded for '{login}'.")
        return 0

    db.close()
    print(
        "Usage: intellect members role <list|show|create|delete|grant|revoke> ...",
        file=sys.stderr,
    )
    return 1


def _cmd_members_passwd(args, config):
    """Change password for the currently logged-in member."""
    from agent.membership import MembershipDB
    from intellect_cli.secret_prompt import masked_secret_prompt

    member_id, _role = _resolve_current_member_id()
    if not member_id:
        print("Not logged in. Use 'intellect members login <name>' first.", file=sys.stderr)
        return 1

    db = MembershipDB(config=config)
    m = db.get_member(member_id)
    if not m:
        print("Error: Member not found.", file=sys.stderr)
        db.close()
        return 1

    # If password is set, verify old one
    if m.get("password_hash"):
        try:
            old = masked_secret_prompt("Current password: ")
        except (EOFError, KeyboardInterrupt):
            print()
            db.close()
            return 1
        if not db.verify_member_password(member_id, old):
            print("Invalid password.", file=sys.stderr)
            db.close()
            return 1

    try:
        _prompt_new_password(db, member_id, confirm_label="Password changed.")
    except (EOFError, KeyboardInterrupt):
        db.close()
        return 1
...
def _cmd_members_reset(args, config):
    """Reset a member's password (owner only). Generates a reset code."""
    from agent.membership import MembershipDB, authorize, Action, normalize_member_lookup
    ...
    db = MembershipDB(config=config)
    if not authorize(actor_role, Action.ADMIN):
        print("Only an owner can reset member passwords.", file=sys.stderr)
        db.close()
        return 1

    login = getattr(args, "login", None)
    if not login:
        print("Usage: intellect members reset <login>", file=sys.stderr)
        db.close()
        return 1

    target = normalize_member_lookup(login, db)
    if not target:
        print(f"Error: Member '{login}' not found.", file=sys.stderr)
        db.close()
        return 1

    code = db.set_member_reset_code(target["id"])
    print(f"Reset code: {code}")
    print(f"Give this code to '{login}'. It expires in 24 hours.")
    db.close()
    return 0


def _resolve_current_member_id() -> tuple[str | None, str | None]:
    """Get the current (member_id, role) from the CLI session file."""
    from intellect_constants import get_intellect_home
    ...
    try:
        data = json.loads(session_file.read_text())
        return data.get("member_id"), data.get("role")
    except Exception:
        return None, None


def _write_cli_session(m: dict, db=None) -> Path:
    """Write .cli-session.json, bind CLI identity, and record online presence.

    This is the single convergence point for ALL login paths (password,
    OAuth, reset-code, first-login, and register). Returns session file path.
    """
    import json, time
    from intellect_constants import get_intellect_home
    ...
    if logout_marker.exists():
        try:
            ...
            pass
    from agent.membership import get_session_ttl

    session_data = {
        "member_id": m["id"],
        "login_name": m.get("login_name", ""),
        "display_name": m.get("display_name", ""),
        "device_id": f"cli-{m['id'][:8]}",
        "expires_at": time.time() + get_session_ttl("cli"),
    }
    ...
    if db:
        def _upsert(cursor):
            cursor.execute(
                "INSERT OR REPLACE INTO identities (id, member_id, provider, provider_id, display_name, created_at) "
                "VALUES (?, ?, 'cli', ?, ?, ?)",
                (f"cli-{m['id']}", m["id"], f"device:{m['id'][:8]}", m.get("display_name", ""), time.time()),
            )
        try:
...
def _oauth_login(
    ...
    *,
    device: bool = False,
    link_member_id: str | None = None,
) -> int:
    """Perform OAuth login flow (loopback or device code)."""
    from agent.members_oauth import (
        is_oauth_enabled, resolve_provider, generate_pkce_pair,
        create_oauth_state, build_authorization_url,
        exchange_code_for_tokens, extract_claims,
        resolve_oauth_member, OAuthMemberNotLinkedError,
        provider_oauth_login_ready,
    )
    from agent.membership import MembershipDB
    from agent.oauth_tokens import store_token as store_oauth_token

    if not is_oauth_enabled(config):
        print("Error: OAuth is not enabled. Set members.oauth.enabled: true in config.yaml", file=sys.stderr)
        return 1

    provider = resolve_provider(config, provider_id)
    if not provider:
        print(f"Error: Unknown OAuth provider '{provider_id}'.", file=sys.stderr)
        return 1
    if not provider.get("enabled"):
        print(f"Error: Provider '{provider_id}' is not enabled.", file=sys.stderr)
        return 1
    if not provider_oauth_login_ready(provider):
        if provider_id == "wecom":
            print(
                "Error: WeCom requires corp_id, agent_id, and "
                f"{provider.get('client_secret_env', 'WECOM_OAUTH_CLIENT_SECRET')} in .env.",
                file=sys.stderr,
            )
        elif provider_id == "dingtalk":
            print(
                "Error: DingTalk requires client_id (app_key) and client secret in .env.",
                file=sys.stderr,
            )
        else:
            print(f"Error: Provider '{provider_id}' has no client_id configured.", file=sys.stderr)
        return 1
    ...
    state = create_oauth_state(
        provider_id, verifier, link_member_id=link_member_id,
    )
    auth_url = build_authorization_url(provider, redirect_uri, state, challenge or None)
    if not auth_url:
        print(f"Error: Could not build authorization URL for '{provider_id}'.", file=sys.stderr)
        return 1

    if device:
        return _oauth_device_flow(provider, config)
    else:
        return _oauth_loopback_flow(provider, config, auth_url, redirect_uri, verifier, state)


def _oauth_loopback_flow(provider, config, auth_url, redirect_uri, verifier, state) -> int:
    """Loopback OAuth: open browser, start temp HTTP server, wait for callback."""
    import json, time, webbrowser, urllib.parse
    from http.server import HTTPServer, BaseHTTPRequestHandler
    from agent.members_oauth import (
        exchange_code_for_tokens, extract_claims,
        resolve_oauth_member, verify_and_consume_oauth_state,
        OAuthMemberNotLinkedError, OAuthIdentityConflictError, is_git_host_provider,
    )
    from agent.membership import MembershipDB
    from agent.oauth_tokens import store_token as store_oauth_token
    ...
    class CallbackHandler(BaseHTTPRequestHandler):
        def do_GET(self):
            ...
            if error:
                result["error"] = error
                self._respond("Authentication failed", f"Error: {error}")
                return
            ...
            self._respond("Authentication successful!", "You may close this window and return to the terminal.")

        def _respond(self, title, message):
            self.send_response(200)
            self.send_header("Content-Type", "text/html; charset=utf-8")
            self.end_headers()
            self.wfile.write(f"<html><body><h2>{title}</h2><p>{message}</p></body></html>".encode())

        def log_message(self, format, *args):
            pass  # Suppress HTTP server logs

    # Open browser
    print(f"Opening browser for {provider['display_name']} login...")
    ...
    print("Waiting for OAuth callback (timeout: 2 min)...")
    start = time.time()
    while not result and (time.time() - start) < 120:
        server.handle_request()
    ...
    if "code" not in result:
        print("Error: OAuth timed out (no callback received).", file=sys.stderr)
        return 1

    # Verify state
    if result.get("state") != state:
        print("Error: OAuth state mismatch — possible CSRF attack.", file=sys.stderr)
        return 1
    state_payload = verify_and_consume_oauth_state(state)
    if not state_payload:
        print("Error: OAuth state expired or invalid.", file=sys.stderr)
        return 1

    # Exchange code for tokens
    print("Exchanging authorization code for tokens...")
    token_resp = exchange_code_for_tokens(
        provider, result["code"], redirect_uri,
    ...
    claims = extract_claims(provider, token_resp)
    if not claims:
        print("Error: Could not extract identity claims from token response.", file=sys.stderr)
        return 1

    db = MembershipDB(config=config)
    try:
        member_id = resolve_oauth_member(
            provider["id"], claims, config=config, db=db,
            invite_code=state_payload.get("invite_code"),
            link_member_id=state_payload.get("link_member_id"),
        )
    except OAuthMemberNotLinkedError:
        db.close()
        print(f"\nError: No member linked to this {provider['display_name']} account.", file=sys.stderr)
        print(f"  Register first: intellect members register <code> --oauth {provider['id']}", file=sys.stderr)
        print(f"  Or bind manually: intellect members bind --oauth {provider['id']} [--login <login>]", file=sys.stderr)
        return 1
    ...
    # Store OAuth token for git/auth reuse
    pid = provider["id"]
    ...
    # Write session
    session_file = _write_cli_session(m, db)

    # Resolve project context
    project_msg = ""
    try:
        from agent.runtime_context import resolve_project_id
        proj = resolve_project_id(config=config, member_id=member_id, db=db)
        if proj:
            project_msg = f"\n  Active project: {proj}"
    except Exception:
        pass
    ...
    git_msg = ""
    if is_git_host_provider(pid):
        from agent.members_oauth import get_git_host_for_provider
        git_msg = f"\n  Git: auto-authenticated for {get_git_host_for_provider(pid)} repos"

    print(f"\n✓ Logged in as '{m.get('login_name', '')}' ({provider['display_name']}: {claims.get('name', claims.get('sub', ''))})")
    print(f"  Session: {session_file}{project_msg}{git_msg}")

    db.close()
    return 0


def _oauth_device_flow(provider, config) -> int:
    """Device code OAuth flow for headless/remote machines."""
    import json, time, urllib.request, urllib.parse

    device_endpoint = None
    pid = provider["id"]
    # Map known device endpoints
    if pid == "github":
        device_endpoint = "https://github.com/login/device/code"
        token_endpoint = "https://github.com/login/oauth/access_token"
    elif pid == "gitee":
        # Gitee doesn't support device flow — use manual
        print(f"Error: {provider['display_name']} does not support device code flow.", file=sys.stderr)
        print("  Use loopback mode instead: intellect members login --oauth gitee", file=sys.stderr)
        return 1
    else:
        # Try OIDC device authorization endpoint
        device_endpoint = provider.get("device_authorization_endpoint")
    ...
    try:
        ...
        print(f"Error: Device code request failed: {e}", file=sys.stderr)
        return 1
    ...
    if not user_code or not device_code:
        print("Error: Invalid device code response from provider.", file=sys.stderr)
        return 1

    print(f"\nDevice code flow for {provider['display_name']}:")
    ...
    while (time.time() - start) < 900:  # 15 min
        ...
        try:
            req2 = urllib.request.Request(token_endpoint, data=poll_data,
                headers={"Accept": "application/json", "Content-Type": "application/x-www-form-urlencoded"})
        ...
        if "access_token" in poll_resp:
            # Got token! Extract claims and finish
            from agent.members_oauth import extract_claims, resolve_oauth_member, OAuthMemberNotLinkedError
            from agent.membership import MembershipDB

            claims = extract_claims(provider, poll_resp)
            if not claims:
                print("Error: Could not extract claims.", file=sys.stderr)
                return 1

            db = MembershipDB(config=config)
            try:
                member_id = resolve_oauth_member(provider["id"], claims, config=config, db=db)
            except OAuthMemberNotLinkedError:
                db.close()
                print(f"\nError: No member linked to this {provider['display_name']} account.", file=sys.stderr)
                return 1
            ...
            if m:
                session_file = _write_cli_session(m, db)
                print(f"\n✓ Logged in as '{m.get('login_name', '')}' ({provider['display_name']})")
        ...
        if error == "authorization_pending":
            print(f"  Still waiting... ({int((time.time() - start) // 60)}m elapsed)")
        elif error in ("slow_down",):
            interval += 5
        elif error in ("access_denied", "expired_token"):
            print(f"Error: Device code {error}.", file=sys.stderr)
            return 1

    print("Error: Device code flow timed out.", file=sys.stderr)
    return 1


def _cmd_teams(args, config):
    """Handle ``intellect members teams <subcmd>``."""
    from agent.membership import MembershipStore, is_teams_enabled

    if not is_teams_enabled(config):
        print("Error: Teams are not enabled.\n  Run: intellect config set members.teams.enabled true", file=sys.stderr)
        return 1

    member_id, actor_role = _resolve_current_member_id()
    store = MembershipStore(config=config)
    sub = getattr(args, "teams_command", None)

    if sub == "create":
        slug = args.slug
        name = getattr(args, "name", None) or slug
        if not member_id:
            print("Error: Not logged in. Run: intellect members login <id>", file=sys.stderr)
            store.close()
            return 1
        if store.get_team_by_slug(slug):
            print(f"Error: Team '{slug}' already exists.", file=sys.stderr)
            store.close()
            return 1
        team = store.create_team_for_webui(
            slug, name, member_id, actor_role=actor_role,
        )
        if not team:
            print("Error: Could not create team (permission denied or teams disabled).", file=sys.stderr)
            store.close()
            return 1
        print(f"Team '{slug}' created.")
        store.close()
        return 0

    elif sub == "list":
        member_login = getattr(args, "member", None)
        list_member_id = member_id
        if member_login:
            m = store.get_member_by_login(member_login)
            if not m:
                print(f"Error: Member '{member_login}' not found.", file=sys.stderr)
                store.close()
                return 1
            list_member_id = m["id"]
        teams = store.list_teams(member_id=list_member_id)
        if not teams:
            print("No teams found.")
        else:
            print(f"{'SLUG':20s} {'NAME':24s} {'STATUS'}")
            print("-" * 60)
            for t in teams:
                status = "active" if t.get("enabled") else "archived"
                print(f"{t['slug']:20s} {t.get('display_name','')[:22]:24s} {status}")
        store.close()
        return 0

    elif sub == "show":
        slug = args.slug
        t = store.get_team_by_slug(slug)
        if not t:
            print(f"Error: Team '{slug}' not found.", file=sys.stderr)
            store.close()
            return 1
        print(f"Slug:    {t['slug']}")
        print(f"Name:    {t['display_name']}")
        print(f"Status:  {'active' if t.get('enabled') else 'archived'}")
        members = store.get_team_members(t["id"], actor_role=actor_role)
        if members:
            print(f"\nMembers ({len(members)}):")
            for m in members:
                print(f"  {m.get('login_name', m.get('member_id','')):20s} role={m['role']}")
        else:
            print("\nNo members.")
        store.close()
        return 0
        ...
        if not member_id:
            ...
            return 1
        try:
            store.archive_team_by_ref(slug, actor_role=actor_role)
        except ValueError as exc:
            print(f"Error: {exc}", file=sys.stderr)
            store.close()
            return 1
        print(f"Team '{slug}' archived.")
        store.close()
        return 0
        ...
        try:
            ...
            store.close()
            return 1
        print(f"Join request submitted for team '{slug}' (pending approval).")
        store.close()
        return 0
        ...
        try:
            store.leave_team(
                slug, member_id, actor_role=actor_role, actor_member_id=member_id,
            )
        except ValueError as exc:
            print(f"Error: {exc}", file=sys.stderr)
            store.close()
            return 1
        print(f"Left team '{slug}'.")
        store.close()
        return 0

    elif sub == "approve":
        slug = args.slug
        member_login = args.member_login
        if not member_id:
            print("Error: Not logged in. Run: intellect members login <id>", file=sys.stderr)
            store.close()
            return 1
        ...
        if not authorize(
            ...
            resource=Resource.for_scope("team", slug),
        ):
            print("Error: Team admin permission required.", file=sys.stderr)
            store.close()
            return 1
        m = store.get_member_by_login(member_login)
        if not m:
            ...
            return 1
        try:
            store.approve_team_join(slug, m["id"], approved_by=member_id)
        except ValueError as exc:
            print(f"Error: {exc}", file=sys.stderr)
            store.close()
            return 1
        print(f"Member '{member_login}' approved for team '{slug}'.")
        store.close()
        return 0

    elif sub == "admin":
        admin_cmd = getattr(args, "team_admin_command", None)
        slug = args.slug
        member_login = args.member_login
        if not member_id:
            print("Error: Not logged in. Run: intellect members login <id>", file=sys.stderr)
        ...
        if not m:
            ...
            store.close()
            return 1
        promote = admin_cmd == "add"
        if admin_cmd not in ("add", "remove"):
            print("Usage: intellect members teams admin <add|remove> <slug> <member>", file=sys.stderr)
            store.close()
            return 1
        try:
            store.set_team_admin(
                slug, m["id"], promote=promote,
                actor_role=actor_role, actor_member_id=member_id,
            )
        except ValueError as exc:
            print(f"Error: {exc}", file=sys.stderr)
            store.close()
            return 1
        if promote:
            print(f"Member '{member_login}' is now admin of team '{slug}'.")
        else:
            print(f"Member '{member_login}' is no longer admin of team '{slug}'.")
        store.close()
        return 0

    elif sub == "soul":
        soul_cmd = getattr(args, "team_soul_command", None)
        if soul_cmd == "refresh":
            slug = args.slug
            if not store.get_team_by_slug(slug):
                print(f"Error: Team '{slug}' not found.", file=sys.stderr)
                store.close()
                return 1
            from agent.team_soul import synthesize_team_soul
            ok, msg = synthesize_team_soul(slug, config=config)
            print(msg)
            store.close()
            return 0 if ok else 1
        else:
            print("Usage: intellect members teams soul refresh <slug>", file=sys.stderr)
            store.close()
            return 1
        ...
        if not t:
            ...
            store.close()
            return 1
        from agent.teams import ensure_team_dirs
        team_dir = ensure_team_dirs(slug)
        print(team_dir / "workspace")
        store.close()
        return 0

    else:
        print("Usage: intellect members teams <create|list|show|archive|join|leave|approve|admin|soul|workspace>", file=sys.stderr)
        store.close()
        return 1


def _cmd_members_projects(args, config):
    """Handle ``intellect members projects <subcmd>``."""
    from agent.membership import is_projects_enabled
    from agent.projects import ProjectDB, ensure_project_dirs

    if not is_projects_enabled(config):
        print(
            "Error: Projects are not enabled.\n"
            "  Run:  intellect config set members.projects.enabled true",
            file=sys.stderr,
        )
        return 1
    ...
    # ── bootstrap ─────────────────────────────────────────────────────────
    if sub == "bootstrap":
        return _projects_bootstrap(args, config, db)

    # ── create ─────────────────────────────────────────────────────────────
    elif sub == "create":
        slug = args.slug
        name = getattr(args, "name", None) or slug
        team_slug = getattr(args, "team", None)
        ...
        members = db.list_members()
        if not members:
            owner_id = db.create_member("default", login_name="default", platform="cli")
            if not owner_id:
                print("Error: Could not create owner member.", file=sys.stderr)
                db.close()
                return 1
        else:
        ...
        if team_slug:
            ...
            if not t:
                print(f"Error: Team '{team_slug}' not found.", file=sys.stderr)
                db.close()
                return 1
            team_id = t["id"]

        pid = db.create_project(slug, name, owner_id, team_id=team_id)
        if not pid:
            print("Error: Could not create project.", file=sys.stderr)
            db.close()
            return 1
        db.add_project_member(pid, owner_id, role="admin")
        ensure_project_dirs(slug, config)
    ...
    # ── show ───────────────────────────────────────────────────────────────
        ...
        print(f"Status:      {'archived' if p.get('archived') else 'active'}")
        ...
        if members:
            ...
            print("\nNo members.")
        # Teams
        teams = db.get_project_teams(p["id"])
        if teams:
            print(f"\nLinked teams ({len(teams)}):")
            for t in teams:
                print(f"  {t['slug']}")
        db.close()
        return 0
    ...
    # ── join ───────────────────────────────────────────────────────────────
        ...
        members = db.list_members()
        if not members:
            print("Error: No members found. Run 'intellect members projects bootstrap' first.", file=sys.stderr)
            db.close()
            return 1
        member_id = members[0]["id"]
        mid = db.add_project_member(p["id"], member_id, role="member")
        if mid:
            print(f"Joined project '{slug}'.")
        else:
            print(f"Already a member of '{slug}' or request pending.")
        db.close()
        return 0
    ...
    # ── leave ──────────────────────────────────────────────────────────────
        ...
        if not members:
            ...
            return 1
        member_id = members[0]["id"]
        db.remove_project_member(p["id"], member_id)
        print(f"Left project '{slug}'.")
        db.close()
        return 0
    ...
    # ── approve ────────────────────────────────────────────────────────────
        ...
        if not p:
            ...
            db.close()
            return 1
        m = db.get_member_by_login(member_login)
        if not m:
            print(f"Error: Member '{member_login}' not found.", file=sys.stderr)
            db.close()
            return 1
        # Check if membership exists
        role = db.get_member_role(p["id"], m["id"])
        if role:
            print(f"Member '{member_login}' is already in project '{slug}' with role '{role}'.")
        else:
            db.add_project_member(p["id"], m["id"], role="member")
            print(f"Member '{member_login}' approved for project '{slug}'.")
        db.close()
        return 0
    ...
    # ── reject ─────────────────────────────────────────────────────────────
        ...
        if not m:
            ...
            db.close()
            return 1
        db.remove_project_member(p["id"], m["id"])
        print(f"Member '{member_login}' removed from project '{slug}'.")
        db.close()
        return 0
    ...
    # ── admin add/remove ───────────────────────────────────────────────────
        ...
        if admin_cmd == "add":
            role = db.get_member_role(p["id"], m["id"])
            if role:
                # Update role to admin (remove + re-add)
                db.remove_project_member(p["id"], m["id"])
            db.add_project_member(p["id"], m["id"], role="admin")
            print(f"Member '{member_login}' is now admin of project '{slug}'.")
        elif admin_cmd == "remove":
            db.remove_project_member(p["id"], m["id"])
            db.add_project_member(p["id"], m["id"], role="member")
            print(f"Member '{member_login}' is no longer admin of project '{slug}'.")
        else:
            print("Usage: intellect members projects admin <add|remove> <slug> <member_login>", file=sys.stderr)
            db.close()
            return 1
        db.close()
        return 0
    ...
    # ── link-team ──────────────────────────────────────────────────────────
        ...
        if not t:
            ...
            db.close()
            return 1
        db.link_project_team(p["id"], t["id"])
        print(f"Team '{team_slug}' linked to project '{slug}'.")
        db.close()
        return 0

    # ── unlink-team ────────────────────────────────────────────────────────
    elif sub == "unlink-team":
        slug = args.slug
        team_slug = args.team_slug
        p = db.get_project_by_slug(slug)
        if not p:
        ...
        if not t:
            ...
            db.close()
            return 1
        db.unlink_project_team(p["id"], t["id"])
        print(f"Team '{team_slug}' unlinked from project '{slug}'.")
        db.close()
        return 0

    # ── list ───────────────────────────────────────────────────────────────
    elif sub == "list":
        return _projects_list(args, db)

    # ── env ────────────────────────────────────────────────────────────────
    elif sub == "env":
        return _projects_env(args, db)

    # ── soul ───────────────────────────────────────────────────────────────
    elif sub == "soul":
        return _projects_soul(args, db)

    # ── clone ──────────────────────────────────────────────────────────────
    elif sub == "clone":
        return _projects_clone(args, db)

    # ── workspace ──────────────────────────────────────────────────────────
    elif sub == "workspace":
        return _projects_workspace(args, db)

    # ── token ──────────────────────────────────────────────────────────────
    elif sub == "token":
        return _projects_token(args, db)

    else:
        print(
            "Usage: intellect members projects "
            "<bootstrap|create|show|list|archive|join|leave|approve|reject|admin|link-team|unlink-team|env|soul>",
            file=sys.stderr,
        )
        db.close()
        return 1
...
def _projects_env(args, db):
    """Handle ``intellect members projects env <set|unset|list>``."""
    ...
    from agent.project_env import (
        write_project_env, delete_project_env, list_project_env_keys
    )

    if env_cmd == "set":
        write_project_env(slug, args.key, args.value)
        print(f"Set {args.key} in project '{slug}' env.")
    elif env_cmd == "unset":
        ok = delete_project_env(slug, args.key)
        if ok:
            print(f"Removed {args.key} from project '{slug}' env.")
        else:
            print(f"Key '{args.key}' not found in project '{slug}' env.")
    elif env_cmd == "list":
        keys = list_project_env_keys(slug)
        if keys:
            for k in keys:
                print(f"  {k}=***")
        else:
        ...
        print("Usage: intellect members projects env <set|unset|list>", file=sys.stderr)
        return 1
    return 0


def _projects_soul(args, db):
    """Handle ``intellect members projects soul <show|edit>``."""
    soul_cmd = getattr(args, "soul_command", None)
    slug = args.slug
    p = db.get_project_by_slug(slug)
    if not p:
        print(f"Error: Project '{slug}' not found.", file=sys.stderr)
    ...
    from agent.project_env import read_project_soul, write_project_soul, _soul_path

    if soul_cmd == "show":
        content = read_project_soul(slug)
        if content:
            print(content)
        else:
            print(f"(no SOUL.md for project '{slug}')")
    elif soul_cmd == "edit":
        import subprocess as _sp
        editor = os.environ.get("EDITOR", os.environ.get("VISUAL", "vi"))
        soul_path = _soul_path(slug)
        soul_path.parent.mkdir(parents=True, exist_ok=True)
        if not soul_path.exists():
            soul_path.write_text(f"# {p['display_name']} — SOUL\n\n", encoding="utf-8")
        _sp.call([editor, str(soul_path)])
        print(f"SOUL.md updated for project '{slug}'.")
    else:
        print("Usage: intellect members projects soul <show|edit>", file=sys.stderr)
        return 1
    return 0


def _projects_clone(args, db):
    """Clone or pull a git repo into the project workspace."""
    slug = args.slug
    p = db.get_project_by_slug(slug)
    if not p:
        print(f"Error: Project '{slug}' not found.", file=sys.stderr)
    ...
    repo_url = getattr(args, "url", None) or p.get("repo_url")
    if not repo_url:
        print(
            f"Error: No repo_url set for project '{slug}'.\n"
            f"  Set it with: intellect members projects env set {slug} GIT_REPO_URL <url>\n"
            f"  Or pass:    intellect members projects clone {slug} --url <url>",
            file=sys.stderr,
        )
        return 1

    from agent.project_workspace import clone_project_repo
    branch = getattr(args, "branch", None)
    ok, msg = clone_project_repo(slug, repo_url, branch=branch)
    print(msg)
    if ok and not p.get("repo_url"):
        db.set_project_repo_url(p["id"], repo_url)
    return 0 if ok else 1


def _projects_workspace(args, db):
    ...
    from agent.project_workspace import get_workspace_path
    ws = get_workspace_path(slug)
    if ws.exists():
        print(ws)
    else:
...
def _projects_token(args, db):
    """Handle ``intellect members projects token <create|list|revoke>``."""
    ...
    if token_cmd == "create":
        ...
        result = db.create_project_token(p["id"], name)
        if not result:
            print("Error: Could not create token.", file=sys.stderr)
            return 1
        token_id, raw_token = result
        print(f"Token created (show once — save it now):")
        print(f"  Token ID: {token_id}")
        print(f"  Token:    {raw_token}")
    elif token_cmd == "list":
        tokens = db.list_project_tokens(p["id"])
        if tokens:
            print(f"{'TOKEN ID':16s} {'NAME':20s} {'CREATED':20s}")
            print("-" * 60)
            for t in tokens:
                print(
                    f"{t['id']:16s} "
                    f"{t.get('name', '')[:18]:20s} "
                    f"{str(t.get('created_at', ''))[:18]:20s}"
                )
        else:
            print("No tokens for this project.")
    elif token_cmd == "revoke":
        db.revoke_project_token(args.token_id)
        print(f"Token '{args.token_id}' revoked.")
    else:
        print("Usage: intellect members projects token <create|list|revoke>", file=sys.stderr)
        return 1
    return 0


def _projects_bootstrap(args, config, db):
    """Bootstrap a default project."""
    from agent.projects import ensure_project_dirs

    slug = getattr(args, "slug", "default")
    name = getattr(args, "name", "Default Project")
    owner_login = getattr(args, "owner_login", None)

    members = db.list_members()
    if not members:
        owner_id = db.create_member(
            display_name="Default User",
            login_name=owner_login or "default",
            platform="cli",
        )
        if not owner_id:
            print("Error: Could not create default member.", file=sys.stderr)
            return 1
        print(f"Created default member: {owner_id}")
    elif owner_login:
        owner = db.get_member_by_login(owner_login)
        if not owner:
            print(f"Error: Member '{owner_login}' not found.", file=sys.stderr)
            return 1
        owner_id = owner["id"]
    else:
        owner_id = members[0]["id"]
    ...
    if existing:
        print(f"Project '{slug}' already exists (id={existing['id']}).")
        return 0

    pid = db.create_project(slug, name, owner_id)
    if not pid:
        print("Error: Could not create project.", file=sys.stderr)
        return 1
    db.add_project_member(pid, owner_id, role="admin")
    project_dir = ensure_project_dirs(slug, config)
    print(f"Project '{slug}' created (id={pid})")
    print(f"  Directory: {project_dir}")
    print(f"  Owner: {owner_id}")
    return 0


def _projects_list(args, db):
    """List projects."""
    member_login = getattr(args, "member", None)
    ...
    projects = db.list_projects(member_id=member_id, include_archived=include_all)
    if not projects:
        print("No projects found.")
    else:
        print(f"{'SLUG':24s} {'NAME':32s} {'OWNER':16s} {'STATUS'}")
        print("-" * 80)
        for p in projects:
            status = "archived" if p.get("archived") else "active"
            print(
                f"{p.get('slug', ''):24s} "
                f"{p.get('display_name', '')[:30]:32s} "
                f"{p.get('owner_member_id', '')[:14]:16s} "
                f"{status}"
            )
    return 0
...
def cmd_dump(args):
    ...
    from intellect_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from intellect_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from intellect_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Intellect home directory to a zip file."""
    if getattr(args, "quick", False):
        from intellect_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from intellect_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore an Intellect backup from a zip file."""
    from intellect_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from intellect_cli.banner import check_for_updates
...
def cmd_uninstall(args):
    """Uninstall Intellect Agent."""
    _require_tty("uninstall")
    from intellect_cli.uninstall import run_uninstall
...
# Critical files that every ``intellect`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``intellect update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "intellect_cli/main.py",
    "intellect_cli/config.py",
    "intellect_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
    "intellect_constants.py",
)
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``intellect`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="intellect-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``intellect update --gateway`` so interactive prompts (stash restore,
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``intellect update`` to stash the
    lockfile — repeatedly.
    """
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `intellect update`.
    ...
    print("  Preview now:  intellect curator run --dry-run")
    print("  Pause it:     intellect curator pause")
    print(
        "  Docs:         https://intellect-agent.ontoweb.cn/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``intellect update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``intellect update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: intellect curator status)"
    )
...
def _update_via_zip(args):
    """Update Intellect Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `intellect update "
            f"--branch {branch}`, or update against main with `intellect update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://gitee.com/ontoweb/intellect-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="intellect-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"intellect-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for intellect-agent itself
        ...
        # GitHub ZIPs extract to intellect-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"intellect-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "intellect-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Intellect behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes intellect completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Intellect couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Intellect couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Intellect behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `intellect update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://gitee.com/ontoweb/intellect-agent.git",
    "git@gitee.com:ontoweb/intellect-agent.git",
    "https://gitee.com/ontoweb/intellect-agent",
    "git@github.com:ONTOWEB/intellect-agent",
}
OFFICIAL_REPO_URL = "https://gitee.com/ontoweb/intellect-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_intellect_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Intellect repository.")
        print("  This means you may miss updates from ONTOWEB/intellect-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://gitee.com/ontoweb/intellect-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://gitee.com/ontoweb/intellect-agent.git' to add later."
            )
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``intellect update`` is still progressing even if pip/uv itself is silent.
    """
...
def _intellect_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "intellect.exe",
        scripts_dir / "intellect-gateway.exe",
    ]


def _detect_concurrent_intellect_instances(
    ...
    chain are excluded so the running ``intellect update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``intellect.exe``) is a separate process from the Python
    ...
    for shim in _intellect_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated intellect.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``intellect update``
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "intellect.exe"
    lines = ["✗ Another intellect.exe is running:"]
    ...
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `intellect update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_intellect_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``intellect.exe``.
    ...
    when ``intellect update`` runs, ``intellect.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``intellect.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next intellect invocation by ``_cleanup_quarantined_exes``.
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_intellect_exe`` if uv didn't write replacements."""
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``intellect.exe.old.*`` left by prior updates.

    Called early on every intellect invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``intellect update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `intellect update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``intellect.exe`` / ``intellect-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_intellect_exe`` for the rationale.
    """
    ...
    def _install(args: list[str]) -> None:
        ...
        if scripts_dir is not None:
            moved = _quarantine_running_intellect_exe(scripts_dir)
        try:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from intellect_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _update_node_dependencies() -> None:
    ...
    for label, path in paths:
        ...
        # streamed output to ``~/.intellect/logs/update.log`` so nothing is lost.
...
class _UpdateOutputStream:
    """Stream wrapper used during ``intellect update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.intellect/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``intellect update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``intellect update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.intellect/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``intellect update --gateway``) the update is already
    ...
    try:
        ...
        # intellect_cli.config.get_intellect_home to simulate setup failure.
        from intellect_cli.config import get_intellect_home as _get_intellect_home

        logs_dir = _get_intellect_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``intellect update --check``: fetch and report without installing.
    ...
    dropping the flag.
    """
    from intellect_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``intellect update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from intellect_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from intellect_cli.config import recommended_update_command
        from intellect_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``intellect update`` without requiring a reinstall.  The
    ...
    ``intellect`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/intellect.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v intellect'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/intellect, code at /usr/local/lib/intellect-agent).
    fhs_link = Path("/usr/local/bin/intellect")
    ...
    path_comment = (
        "# Intellect Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of INTELLECT_HOME before running the update.
    ...
    INTELLECT_HOME directories.  The ``--backup`` flag on ``intellect update``
    ...
    try:
        from intellect_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_intellect_home so the user sees ~/.intellect/...
    try:
        from intellect_constants import get_intellect_home, display_intellect_home

        home = get_intellect_home()
        try:
            display_path = f"{display_intellect_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  intellect import {out_path}")
...
def cmd_update(args):
    """Update Intellect Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Intellect Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `intellect update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Intellect via pip (for PyPI installs)."""
    from intellect_cli import __version__
    from intellect_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "intellect-agent"]
        ...
        cmd = [pipx, "upgrade", "intellect-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "intellect-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "intellect-agent"]
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_intellect_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        # though `git pull` can't touch $INTELLECT_HOME, this is cheap
        ...
        try:
            from intellect_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in intellect_cli/config.py bricked
        # every user who ran ``intellect update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``intellect
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``intellect update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_intellect_home added to intellect_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # attributes like display_intellect_home() added since the last release.
        try:
            import importlib
            import intellect_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit INTELLECT_HOME so
        # it is not affected by sync_skills()'s module-level INTELLECT_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's INTELLECT_HOME env var points at the default or a named profile.
        try:
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'intellect config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                # cannot prompt for API keys. Still run the non-interactive
                # migration pass before restarting
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: intellect config migrate")
            else:
                print()
                print("Skipped. Run 'intellect config migrate' later to configure.")
        else:
        ...
        try:
            from intellect_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `intellect curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``intellect update`` gives users a predictable
        ...
        # When running as ``intellect update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_intellect_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            try:
                from intellect_constants import (
            ...
            # Discover all intellect-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. intellect-gateway.service or intellect-gateway-coder.service
                            ...
                            # path in `intellect gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: intellect gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: intellect -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Intellect gateway unit files are still installed.
        # When both intellect.service (from a pre-rename install) and the
        # current intellect-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `intellect update` surfaces the issue until the user migrates.
        try:
            from intellect_cli.gateway import (
                has_legacy_intellect_units,
                _find_legacy_intellect_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_intellect_units():
                print()
                print("⚠ Legacy Intellect gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_intellect_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (intellect.service) fight the current")
                print("  intellect-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    intellect gateway migrate-legacy")
                print()
        ...
        print("  intellect model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``intellect -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.intellect)")
            else:
        ...
        from intellect_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from intellect_constants import get_intellect_home as _hh
        ...
        from intellect_cli import profile_describer as _pd
        ...
        from intellect_cli.profiles import rename_profile
        ...
        from intellect_cli.profiles import export_profile
        ...
        from intellect_cli.profiles import import_profile
        ...
        import tempfile
        from intellect_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="intellect_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
        ...
        from intellect_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        from intellect_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("intellect_requires"):
            print(f"Requires:     Intellect {data['intellect_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from intellect_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.intellect_requires:
        print(f"  Requires: Intellect {mf.intellect_requires}")
...
def cmd_completion(args, parser=None):
    ...
    from intellect_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from intellect_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Intellect log files."""
    from intellect_cli.logs import tail_log, list_logs
