"""``intellect logs`` — view and filter Intellect log files.

Supports tailing, following, session filtering, level filtering,
component filtering, and relative time ranges.  All log files live
under ``~/.intellect/logs/``.

Usage examples::

    intellect logs                    # last 50 lines of agent.log
    intellect logs -f                 # follow agent.log in real time
    intellect logs errors             # last 50 lines of errors.log
    intellect logs gateway -n 100    # last 100 lines of gateway.log
    intellect logs --level WARNING    # only WARNING+ lines
    intellect logs --session abc123   # filter by session ID substring
    intellect logs --component tools  # only tool-related lines
    intellect logs --since 1h         # lines from the last hour
    intellect logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_intellect_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Intellect runs — try 'intellect chat' first)")
        sys.exit(1)
    ...
    if component:
        from intellect_logging import COMPONENT_PREFIXES
    ...
    if follow:
        print(f"--- {display_intellect_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_intellect_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_intellect_home()}/logs/")
        return

    print(f"Log files in {display_intellect_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'intellect chat' to generate logs)")
