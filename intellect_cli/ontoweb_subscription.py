"""Helpers for OntoWeb subscription managed-tool capabilities."""
...
from intellect_cli.config import get_env_value, load_config
from intellect_cli.ontoweb_account import (
    OntowebPortalAccountInfo,
...
@dataclass(frozen=True)
class OntowebSubscriptionFeatures:
    subscribed: bool
    ontoweb_auth_present: bool
    provider_is_nous: bool
    features: Dict[str, OntowebFeatureState]
    account_info: Optional[OntowebPortalAccountInfo] = None

    @property
    def web(self) -> OntowebFeatureState:
        return self.features["web"]

    @property
    def image_gen(self) -> OntowebFeatureState:
        return self.features["image_gen"]

    @property
    def tts(self) -> OntowebFeatureState:
        return self.features["tts"]

    @property
    def browser(self) -> OntowebFeatureState:
        return self.features["browser"]

    @property
    def video_gen(self) -> OntowebFeatureState:
        return self.features["video_gen"]

    @property
    def modal(self) -> OntowebFeatureState:
        return self.features["modal"]

    def items(self) -> Iterable[OntowebFeatureState]:
...
def get_ontoweb_subscription_features(
    config: Optional[Dict[str, object]] = None,
    *,
    force_fresh: bool = False,
) -> OntowebSubscriptionFeatures:
    if config is None:
        config = load_config() or {}
    config = dict(config)
    model_cfg = _model_config_dict(config)
    provider_is_nous = str(model_cfg.get("provider") or "").strip().lower() == "ontoweb"

    try:
        if force_fresh:
            account_info = get_ontoweb_portal_account_info(force_fresh=True)
        else:
            account_info = get_ontoweb_portal_account_info()
    except Exception:
        account_info = None
    ...
    ontoweb_auth_present = bool(account_info and account_info.logged_in)
    subscribed = provider_is_nous or ontoweb_auth_present
    ...
    # Tool Gateway via `intellect model`, so direct credentials should NOT
    ...
    managed_web_available = managed_tools_flag and ontoweb_auth_present and is_managed_tool_gateway_ready("firecrawl")
    managed_image_available = managed_tools_flag and ontoweb_auth_present and is_managed_tool_gateway_ready("fal-queue")
    # Video gen uses the same fal-queue gateway as image gen.
    managed_video_available = managed_image_available
    managed_tts_available = managed_tools_flag and ontoweb_auth_present and is_managed_tool_gateway_ready("openai-audio")
    managed_browser_available = managed_tools_flag and ontoweb_auth_present and is_managed_tool_gateway_ready("browser-use")
    managed_modal_available = managed_tools_flag and ontoweb_auth_present and is_managed_tool_gateway_ready("modal")
    ...
    features = {
        ...
        "image_gen": OntowebFeatureState(
            ...
            current_provider="FAL" if direct_fal else ("OntoWeb Subscription" if image_managed else ""),
            explicit_configured=direct_fal,
        ),
        "video_gen": OntowebFeatureState(
            key="video_gen",
            ...
            current_provider="FAL" if direct_fal_video else ("OntoWeb Subscription" if video_managed else ""),
            explicit_configured=direct_fal_video,
        ),
        "tts": OntowebFeatureState(
            key="tts",
            label="OpenAI TTS",
            included_by_default=True,
    ...
    return OntowebSubscriptionFeatures(
        subscribed=subscribed,
        ontoweb_auth_present=ontoweb_auth_present,
...
def apply_ontoweb_managed_defaults(
...
def get_gateway_eligible_tools(
    ...
    *,
    ...
    All lists are empty when the user is not a paid OntoWeb subscriber or
    is not using OntoWeb as their provider.
    """
    if force_fresh:
        managed_enabled = managed_ontoweb_tools_enabled(force_fresh=True)
    else:
        managed_enabled = managed_ontoweb_tools_enabled()
    ...
    # Quick provider check without the heavy get_ontoweb_subscription_features call
    model_cfg = config.get("model")
    if not isinstance(model_cfg, dict) or str(model_cfg.get("provider") or "").strip().lower() != "ontoweb":
        return [], [], []
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from intellect_cli.setup import prompt_choice
    except Exception:
        return set()
    ...
    desc_parts: list[str] = [
        ...
        "  text-to-speech, and browser automation through your OntoWeb subscription.",
    ...
    try:
        idx = prompt_choice(
            "Your OntoWeb subscription includes the Tool Gateway.",
            choices,
            default_idx,
    ...
    if changed:
        ...
        for key in sorted(newly_switched):
            ...
            print(f"  ✓ {label}: enabled via OntoWeb subscription")
...
# ---------------------------------------------------------------------------
# Inline login gate for `intellect tools`
# ---------------------------------------------------------------------------


def ensure_ontoweb_portal_access(*, capability: str = "the OntoWeb Tool Gateway") -> bool:
    """Make sure the user has paid OntoWeb Portal access, logging in if needed.

    Used by ``intellect tools`` when a user selects a OntoWeb-managed Tool Gateway
    backend.  Unlike ``intellect model``'s OntoWeb login, this:
    ...
    It only performs the OntoWeb Portal device-code OAuth (when the user isn't
    ...
    try:
        info = get_ontoweb_portal_account_info(force_fresh=True)
    except Exception:
        info = None
    ...
    if info is None or not info.logged_in:
        if not _run_ontoweb_portal_login_only(capability=capability):
            return False
        try:
            info = get_ontoweb_portal_account_info(force_fresh=True)
        except Exception:
    ...
    message = format_ontoweb_portal_entitlement_message(info, capability=capability)
    if message:
...
def _run_ontoweb_portal_login_only(*, capability: str) -> bool:
    """Run the OntoWeb Portal device-code OAuth and persist credentials only.
    ...
    try:
        from intellect_cli.auth import (
            _auth_store_lock,
            _load_auth_store,
            _ontoweb_device_code_login,
            _read_shared_ontoweb_state,
            _save_auth_store,
            _save_provider_state,
            _sync_ontoweb_pool_from_auth_store,
            _try_import_shared_ontoweb_state,
            _write_shared_ontoweb_state,
        )
    except Exception as exc:  # pragma: no cover - defensive
        print(f"  Could not start OntoWeb Portal login: {exc}")
        return False

    print()
    print(f"  {capability} requires a OntoWeb Portal login.")
    try:
        proceed = input("  Log in to OntoWeb Portal now? [Y/n]: ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        print()
        return False
    if proceed not in {"", "y", "yes"}:
        print("  Skipped OntoWeb Portal login.")
        return False

    try:
        # Snapshot the active_provider so a tool-config login never silently
        # switches the user's inference provider to OntoWeb.
        ...
        if shared:
            try:
                do_import = input(
                    "  Found existing OntoWeb OAuth credentials. Import them? [Y/n]: "
                ).strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
            if do_import in {"", "y", "yes"}:
                auth_state = _try_import_shared_ontoweb_state(timeout_seconds=15.0)
        ...
        _sync_ontoweb_pool_from_auth_store()
        print("  OntoWeb Portal login successful.")
        return True
    except KeyboardInterrupt:
        print("\n  Login cancelled.")
        return False
    except SystemExit:
        # _ontoweb_device_code_login raises SystemExit on subscription_required;
        # it already printed billing guidance.
        return False
    except Exception as exc:
        print(f"  OntoWeb Portal login failed: {exc}")
        return False
