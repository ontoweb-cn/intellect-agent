"""``intellect plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.intellect/plugins/``.
...
def _resolve_git_executable() -> Optional[str]:
    ...
    Matches other Intellect subprocess resolution: :func:`shutil.which` first,
...
def _plugins_dir() -> Path:
    ...
    plugins = get_intellect_home() / "plugins"
...
def _missing_requires_env_names(manifest: dict) -> list[str]:
    """Return declared ``requires_env`` names that are unset in ``~/.intellect/.env``."""
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from intellect_cli.config import get_env_value, save_env_value  # noqa: F811
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_intellect_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_intellect_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_intellect_home()}/.env later)[/dim]")
...
def _install_plugin_core(identifier: str, *, force: bool) -> tuple[Path, dict, str]:
    """Clone Git plugin into ``~/.intellect/plugins``.
        ...
        if mv is not None:
            ...
            if mv_int > _SUPPORTED_MANIFEST_VERSION:
                ...
                raise PluginOperationError(
                    ...
                    f"Run {recommended_update_command()} to update Intellect.",
                ) from None
        ...
        if target.exists():
            if not force:
                raise PluginOperationError(
                    ...
                    f"or run `intellect plugins update {plugin_name}`.",
                )
...
def cmd_install(
    ...
    if not (target / "plugin.yaml").exists() and not (target / "plugin.yml").exists() and not (
        ...
        console.print(
            ...
            f"or __init__.py. It may not be a valid Intellect plugin.",
        )
    ...
    if should_enable:
        ...
        console.print(
            ...
            f"Run `intellect plugins enable {installed_name}` to activate.[/dim]",
        )

    console.print("[dim]Restart the gateway for the plugin to take effect:[/dim]")
    console.print("[dim]  intellect gateway restart[/dim]")
    console.print()
    ...
    if user_dir.is_dir():
        ...
        for child in user_dir.iterdir():
            ...
            if manifest.get("name") == name:
                return True
    # Bundled: <repo>/plugins/<name>/ (or INTELLECT_BUNDLED_PLUGINS on Nix).
    from intellect_cli.plugins import get_bundled_plugins_dir
...
def _discover_all_plugins() -> list:
    ...
    user types into ``intellect plugins enable <key>``. For category-namespaced
...
def _filter_plugin_entries(entries: list, args: Any, enabled: set, disabled: set) -> list:
    """Apply ``intellect plugins list`` CLI filters."""
    filtered = entries
...
def cmd_list(args: Any | None = None) -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] intellect plugins install owner/repo")
        return
    ...
    console.print(table)
    console.print()
    console.print("[dim]Compact view:[/dim] intellect plugins list --plain --no-bundled")
    console.print("[dim]Interactive toggle:[/dim] intellect plugins")
    console.print("[dim]Enable/disable:[/dim] intellect plugins enable/disable <name>")
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    plugin-registered engines (third-party engines installed as Intellect
...
def _run_composite_ui(curses, plugin_names, plugin_labels, plugin_selected,
    ...
    from intellect_cli.curses_ui import flush_stdin
...
def plugins_command(args) -> None:
    """Dispatch intellect plugins subcommands."""
