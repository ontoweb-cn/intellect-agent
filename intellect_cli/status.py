"""
Status command for intellect CLI.

Shows the status of all Intellect Agent components.
"""
...
from intellect_cli.auth import AuthError, resolve_provider
from intellect_cli.colors import Colors, color
from intellect_cli.config import get_env_path, get_env_value, get_intellect_home, load_config
...
from intellect_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``intellect config``'s
...
def show_status(args):
    """Show status of all Intellect Agent components."""
    ...
    if (
        ...
        try:
            ontoweb_account_info = get_ontoweb_portal_account_info()
        except Exception:
            ontoweb_account_info = None

    nous_logged_in = bool(
        nous_status.get("logged_in")
        or (ontoweb_account_info and ontoweb_account_info.logged_in)
    )
    nous_inference_present = bool(
        nous_status.get("inference_credential_present")
        or (ontoweb_account_info and ontoweb_account_info.inference_credential_present)
    )
    ...
    inference_url = (
        ...
        or (ontoweb_account_info.inference_base_url if ontoweb_account_info else None)
    )
    ...
    if managed_ontoweb_tools_enabled():
        ...
        for feature in features.items():
            ...
            elif feature.included_by_default and features.ontoweb_auth_present:
                state = "included by subscription, not currently selected"
            elif feature.key == "modal" and features.ontoweb_auth_present:
    ...
    if _effective_provider_label() == "LM Studio":
        from intellect_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
