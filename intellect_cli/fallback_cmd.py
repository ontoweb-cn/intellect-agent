"""
intellect fallback — manage the fallback provider chain.
...
Subcommands:
  intellect fallback [list]   Show the current fallback chain (default when no subcommand)
  intellect fallback add      Pick provider + model via the same picker as `intellect model`,
                           then append the selection to the chain
  intellect fallback remove   Pick an entry to delete from the chain
  intellect fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.intellect/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `intellect model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  intellect fallback add")
        print()
        return
    ...
    print("  Docs: https://intellect-agent.ontoweb.cn/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `intellect model`, then append the selection to the chain."""
    from intellect_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `intellect model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `intellect fallback list` to view, or `intellect fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``intellect fallback [subcommand]``."""
