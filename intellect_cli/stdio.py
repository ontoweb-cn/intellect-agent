...
on Japanese locales, etc.).  Intellect's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``intellect_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Intellect is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``intellect_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("intellect_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Intellect-managed Git install directories so
    ...
    # already-running shells don't see the change — which means intellect
    ...
    # install without Intellect-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Intellect's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Intellect's
    ``intellect config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Intellect (User env var in Windows Settings, or
    export in a PowerShell profile) and Intellect picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Intellect-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\intellect\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``intellect`` from right after install) retain their old PATH.

    Any subprocess Intellect spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Intellect-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "intellect", "git", "cmd"),
        os.path.join(local_appdata, "intellect", "git", "bin"),
        os.path.join(local_appdata, "intellect", "git", "usr", "bin"),
        # Intellect venv Scripts directory — host of the intellect.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes intellect, but harmless to include.
        os.path.join(local_appdata, "intellect", "intellect-agent", "venv", "Scripts"),
