"""intellect webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.intellect/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _intellect_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: intellect gateway run
"""
...
def webhook_command(args):
    """Entry point for 'intellect webhook' subcommand."""
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from intellect webhook test"}'
