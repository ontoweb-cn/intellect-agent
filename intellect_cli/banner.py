...
Pure display functions with no IntellectCLI state dependency.
"""
...
[bold #FFD700] |_ _| \\ | |_   _| ____| |   | |   | ____/ ___|_   _|[/]
[#FFBF00]  | ||  \\| | | | |  _| | |   | |   |  _|| |     | |  [/]
[#FFBF00]  | || |\\  | | | | |___| |___| |___| |__| |___  | |  [/]
[#CD7F32] |___|_| \\_| |_| |_____|_____|_____|_____\\____| |_|  [/]"""

intellect_CADUCEUS = """[bold #FFD700]WUST·1898[/]
[bold #CD7F32]青山·沁水[/]"""
...
# (e.g. nix-built intellect — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://gitee.com/ontoweb/intellect-agent.git"
...
def _fetch_pypi_latest(package: str = "intellect-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether an Intellect update is available.

    Two paths: if ``INTELLECT_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = intellect_home / ".update_check"
    embedded_rev = os.environ.get("INTELLECT_REVISION") or None
    ...
    if embedded_rev:
        ...
        # $INTELLECT_HOME/intellect-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = intellect_home / "intellect-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Intellect git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$INTELLECT_HOME/intellect-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``intellect_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from intellect_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://gitee.com/ontoweb/intellect-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Intellect checkout. Cached per-process. Release URL always points at the
    canonical ONTOWEB/intellect-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Intellect Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    """Build and print a welcome banner with branding on left and info on right.
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else intellect_CADUCEUS
    except Exception:
        _bskin = None
        _hero = intellect_CADUCEUS
    left_lines = ["", "", _hero, "", ""]
    ...
    left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]ONTOWEB[/]")

    if os.getenv("intellect_YOLO_MODE"):
    ...
    try:
        from intellect_cli.codex_runtime_switch import get_current_runtime
        from intellect_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from intellect_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built intellect; we know an update
