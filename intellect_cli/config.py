"""
Configuration management for Intellect Agent.

Config files are stored in ~/.intellect/ for easy access:
- ~/.intellect/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.intellect/.env         - API keys and secrets

This module provides:
- intellect config          - Show current configuration
- intellect config edit     - Open config in editor
- intellect config set      - Set a specific value
- intellect config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.intellect/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``intellect logs``
...
#   the next ``subprocess.run([...])`` Intellect makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Intellect itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``intellect update``, the TUI build.
# * ``PATH`` — too broad to allow. If a tool can't be found, the fix is
#   to add an absolute path in the integration config, not to mutate PATH
#   globally.
# * ``GIT_SSH_COMMAND`` / ``GIT_EXEC_PATH`` — git rewrites that fire
#   on every plugin install / ``intellect update``.
...
# * ``INTELLECT_HOME`` / ``intellect_PROFILE`` / ``intellect_CONFIG`` /
#   ``intellect_ENV`` — Intellect runtime location flags. Writing these into
#   ``.env`` would relocate state in ways the user did not request.
#   ``config.yaml`` is the supported surface for these.
#
# IMPORTANT: ``intellect_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (intellect_GEMINI_CLIENT_ID,
# intellect_LANGFUSE_PUBLIC_KEY, intellect_SPOTIFY_CLIENT_ID, ...). The
...
# point is that a writable surface cannot escalate by planting them.
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Intellect runtime location — never via env writer.
    # NOT a intellect_* blanket: integration credentials (intellect_GEMINI_*,
    # intellect_LANGFUSE_*, intellect_SPOTIFY_*, ...) ARE allowed.
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Intellect runtime location "
            "(INTELLECT_HOME, intellect_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.intellect/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `intellect plugins enable
    # observability/langfuse`); credentials gate the plugin at runtime.
    "intellect_LANGFUSE_ENV",
    "intellect_LANGFUSE_RELEASE",
    "intellect_LANGFUSE_SAMPLE_RATE",
    "intellect_LANGFUSE_MAX_CHARS",
    "intellect_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from intellect_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("intellect_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_intellect_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Intellect is running in package-manager-managed mode.

    Two signals: the intellect_MANAGED env var (set by the systemd service),
    or a .managed marker file in INTELLECT_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade intellect-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Intellect was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.intellect/.install_method`` file (written by installers)
    2. intellect_MANAGED env / .managed marker (NixOS, Homebrew)
    ...
    container detection here:
      ...
      - the published ``ontoweb/intellect-agent`` image stamps ``docker``
    ...
    so ``intellect update`` bailed with "doesn't apply inside the Docker
    ...
    """
    stamp = get_intellect_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.intellect/.install_method."""
    stamp = get_intellect_home() / ".install_method"
    try:
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Intellect lives in a ``uv tool`` layout.

    ``uv tool install intellect-agent`` places the install at
    ``.../uv/tools/intellect-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``intellect-agent`` happens to be uv-tool-installed on the machine while
    the *active* Intellect is a regular pip/venv install, causing
    ``intellect update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/intellect-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade intellect-agent"
    if method == "docker":
        return "docker pull ontoweb/intellect-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade intellect-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade intellect-agent"
        return "pip install --upgrade intellect-agent"
...
# Long-form text for ``intellect update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# intellect_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Intellect, it doesn't update
...
✗ ``intellect update`` doesn't apply inside the Docker container.

Intellect Agent runs as a published image (ontoweb/intellect-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull ontoweb/intellect-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate intellect-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm ontoweb/intellect-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/ontoweb/intellect-agent/tags
  • Your config and session history live under ``$INTELLECT_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``intellect update`` inside Docker.
...
def format_managed_message(action: str = "modify this Intellect installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("intellect_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Intellect installation is managed by NixOS "
            f"(intellect_MANAGED={env_hint}).\n"
            "Edit services.intellect-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Intellect installation is managed by Homebrew "
            f"(intellect_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade intellect-agent"
        )

    return (
        f"Cannot {action}: this Intellect installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Intellect."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from INTELLECT_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, intellect_bin
    or None if container mode is not active, we're already inside the
    container, or intellect_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("intellect_DEV") == "1":
        return None

    from intellect_constants import is_container
    if is_container():
        return None

    container_mode_file = get_intellect_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "intellect-agent")
    exec_user = info.get("exec_user", "intellect")
    intellect_bin = info.get("intellect_bin", "/data/current-package/bin/intellect")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "intellect_bin": intellect_bin,
    }
...
# Re-export from intellect_constants — canonical definition lives there.
from intellect_constants import get_intellect_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_intellect_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_intellect_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the intellect group can
    share state with the gateway service.

    The mode can be overridden via the INTELLECT_HOME_MODE environment variable
    (e.g. INTELLECT_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse INTELLECT_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("INTELLECT_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Intellect runs in a container with volume-mounted config files, forcing
    0o600 permissions breaks multi-process setups where the gateway runs as a
    different UID or the volume mount requires broader permissions.
    """
    # Explicit opt-out
    if os.environ.get("intellect_CONTAINER") or os.environ.get("intellect_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    Skipped in managed/container modes where chmod can break shared-volume
    setups (Docker/Podman) or OS-managed permissions (NixOS, Homebrew).
    Set ``INTELLECT_SKIP_CHMOD=1`` to force-skip on other unusual systems.
    """
    if is_managed() or _is_container():
        return
    if os.environ.get("INTELLECT_SKIP_CHMOD"):
        return
    try:
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into INTELLECT_HOME if the user doesn't have one yet."""
...
def ensure_intellect_home():
    """Ensure ~/.intellect directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_intellect_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_intellect_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"INTELLECT_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Intellect-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Intellect
        ...
        # identity slot (SOUL.md). Empty by default. The intellect_ENVIRONMENT_HINT
    ...
    "terminal": {
        ...
        # skipped. When empty, Intellect auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Intellect sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Intellect
        ...
        # bundled Intellect image, which drops to the `intellect` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Intellect sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Intellect should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``intellect chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.intellect/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: intellect sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Intellect truncates the
    ...
    "auxiliary": {
        "vision": {
            "provider": "auto",    # auto | openrouter | ontoweb | codex | custom
        ...
        # LightRAG conversation summary ingest (plugins/rag/lightrag/ingest.py).
        # Defaults to auto (main model chain). Pin a cheap model for per-turn
        # summary when ingest.auto_mode is summary|full.
        "lightrag": {
            "provider": "auto",
            "model": "",
            "base_url": "",
        ...
        # Invoked by ``intellect kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``intellect kanban decompose`` and the kanban
        ...
        # ``intellect profile describe <name> --auto``. Short, cheap call.
        "profile_describer": {
            "provider": "auto",
            "model": "",
            "base_url": "",
        ...
        # model; override via `intellect model` → auxiliary → Curator to route
    ...
    # All default to safe values.  See agent/feature_flags.py for the
    # authoritative registry and per-flag documentation.
    "features": {
        "credential_encryption_enforce": True,
        "dns_rebinding_protection": True,
        "url_query_log_redaction": True,
        "bluebubbles_header_auth": True,
        "searxng_enabled": False,
        "searxng_url": "",
        "structured_logging": False,
        "experimental_inference_registry": False,
        "skill_analytics_collect_on_startup": False,
    },
    ...
    "display": {
        ...
        # When true, `intellect --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `intellect -c` muscle memory.  Default off so existing
        # users aren't surprised.  intellect_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `intellect --tui` drops a one-time hint
        # ("subagents working · /agents to watch live") the first time a turn
        # starts delegating, nudging the user toward the live spawn-tree
        # view. Set false to suppress the hint.
        ...
        # e.g. `model · 68% · ~/projects/intellect`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.intellect/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.intellect/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # Document RAG — parallel to memory.provider (e.g. lightrag + graphiti).
    "rag": {
        "enabled": True,
        "provider": "",
    ...
    # response. If not, Intellect feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Intellect auto-pauses the goal and
    ...
    # cron daemon.
    #
    # See `intellect curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.intellect/skills/ into
        # ~/.intellect/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `intellect curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for intellect-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Intellect Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (intellect_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.intellect/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or INTELLECT_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `intellect doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``intellect_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Intellect to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Cron execution provider (HP-408).
        #   "builtin"  — the gateway ticks the scheduler every 60s (default).
        #   "chronos"  — the in-process ticker is disabled; an external
        #                scheduler (systemd .timer, cloud scheduler, k8s
        #                CronJob, hosted Chronos) drives due jobs by POSTing
        ...
        "chronos": {
            ...
            # provider=chronos on a network bind is rejected at startup.
            "trigger_token": "",
    ...
    # todos to ready, and fires `intellect -p <assignee> chat -q ...` for
    # each claimable ready task. One dispatcher per profile is sufficient;
    # running more than one on the same kanban.db will race for claims.
    "kanban": {
        # ``legacy`` — separate ``kanban.db`` per board (default).
        # ``unified`` — kanban tables live in ``state.db`` / PostgreSQL (T1).
        "storage": "legacy",
        ...
        # falls back to the default profile (the one `intellect` launches with
        ...
        # decomposition is manual via `intellect kanban decompose <id>`.
        "auto_decompose": True,
    ...
    "code_execution": {
        ...
        #     intellect-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Intellect tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.intellect/logs/.
    ...
    # curated model lists for OpenRouter and ONTOWEB Portal from this URL,
    # falling back to the in-repo snapshot on network failure.  Lets us
    # update model picker lists without shipping a intellect-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://intellect.ontoweb.cn/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `intellect model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.intellect/.env,
        ...
        # behavior: files must live under the Intellect cache, under
        ...
        # user. Bridged to intellect_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Intellect
        # cache (~/.intellect/cache/{documents,images,audio,video,screenshots})
        ...
        # to intellect_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # intellect_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        ...
        # platforms are rejected at startup validation (HP-406c).
        "scale_to_zero": {
            "enabled": False,
    ...
    # Session storage — controls automatic cleanup of ~/.intellect/state.db.
    ...
    "sessions": {
        ...
        # default of ``intellect sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.intellect/sessions/session_{sid}.json`` on every turn
    ...
    # ``intellect update`` behaviour.
    "updates": {
        # Run a full ``intellect backup``-style zip of INTELLECT_HOME before every
        # ``intellect update``.  Backups land in ``<INTELLECT_HOME>/backups/`` and
        # can be restored with ``intellect import <path>``.  Off by default —
        # on large INTELLECT_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<INTELLECT_HOME>/lsp/bin/`` on first use.
    ...
    # `intellect tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.intellect/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.intellect/.env (or your shell) and never in config.yaml.
            ...
            # ~/.intellect/bin/ on first use.  When False you must install
            ...
            # `intellect secrets bitwarden setup`.
            "server_url": "",
        },
    },
...
# selection step (ONTOWEB Portal / OpenRouter / Custom endpoint), so this
...
OPTIONAL_ENV_VARS = {
    ...
    "NOUS_BASE_URL": {
        "description": "ONTOWEB Portal base URL override",
        "prompt": "ONTOWEB Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "intellect_GEMINI_CLIENT_SECRET": {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'intellect model' for endpoint-specific config)",
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override for OntoWeb Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for OntoWeb Subscribers only, used to derive vendor hosts, e.g. ontoweb.cn -> firecrawl-gateway.ontoweb.cn",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme for OntoWeb Subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit OntoWeb Subscriber access token for tool-gateway requests (optional; otherwise read from the Intellect auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @intellect:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. intellect_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #intellect)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: intellect-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'intellect-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Intellect API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # intellect_TOOL_PROGRESS and intellect_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "intellect_TOOL_PROGRESS": {
    ...
    "intellect_PREFILL_MESSAGES_FILE": {
    ...
    "INTELLECT_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.intellect.config`` entries, then checks
    ...
    try:
        ...
        # should never break `intellect update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Intellect versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``intellect_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'intellect setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Intellect won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'intellect doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("INTELLECT_HOME", "~/.intellect")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("intellect_TOOL_PROGRESS")
            old_mode = get_env_value("intellect_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from intellect_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from intellect_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("INTELLECT_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from INTELLECT_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$INTELLECT_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_intellect_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `intellect plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `intellect curator status` has no
    ...
    #   3. Creates `~/.intellect/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_intellect_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_intellect_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: intellect config set <key> <value>")
    ...
    # metadata.intellect.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _intellect_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.intellect/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.intellect/config.yaml.
    ...
    (which change ``INTELLECT_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def load_config_readonly() -> "MappingProxyType":
    ...
    Returns a ``MappingProxyType`` wrapping the cached config — enforced
    read-only. Mutating the result raises ``TypeError``.  This is safe for
    all read-only callers (``.get()``, ``[]`` access, iteration) and
    prevents accidental cache corruption.

    If you need to mutate or pass to ``save_config``, call ``load_config()``
    instead (which returns a deep-copied dict).
    """
    from types import MappingProxyType

    raw = _load_config_impl(want_deepcopy=False)
...
#   openai-codex (OAuth — intellect auth) — OpenAI Codex
#   ontoweb         (OAuth — intellect auth) — ONTOWEB Portal
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.intellect/.env.
    ...
    interactive menu render (`intellect tools`, `intellect setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `intellect tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Intellect env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.intellect/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.intellect/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.intellect/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.intellect/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Intellect — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Intellect vars that are no longer in .env
...
def show_config():
    ...
    print(color("│              ⚕ Intellect Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  intellect config edit     # Edit config file", Colors.DIM))
    print(color("  intellect config set <key> <value>", Colors.DIM))
    print(color("  intellect setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: intellect config set <key> <value>")
            print()
            print("Examples:")
            print("  intellect config set model anthropic/claude-sonnet-4")
            print("  intellect config set terminal.backend docker")
            print("  intellect config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'intellect config migrate' to add them")
        ...
        print("  intellect config           Show current configuration")
        print("  intellect config edit      Open config in editor")
        print("  intellect config set <key> <value>   Set a config value")
        print("  intellect config check     Check for missing/outdated config")
        print("  intellect config migrate   Update config with new options")
        print("  intellect config path      Show config file path")
        print("  intellect config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``intellect config`` UI so users
