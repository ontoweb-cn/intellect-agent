"""
Unified tool configuration for Intellect Agent.

`intellect tools` and `intellect setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.intellect/config.yaml under
...
from intellect_cli.ontoweb_subscription import (
    apply_ontoweb_managed_defaults,
    get_ontoweb_subscription_features,
)
from intellect_cli.ontoweb_account import format_ontoweb_portal_entitlement_message
...
from intellect_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _INTELLECT_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `intellect tools` → Video Generation, which walks
...
# `intellect tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from intellect_cli.auth import _read_xai_oauth_tokens
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``intellect tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from intellect_cli.plugins import discover_plugins, get_plugin_toolsets
...
from intellect_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    "tts": {
        ...
        "providers": [
            {
                "name": "OntoWeb Subscription",
                "badge": "subscription",
                "tag": "Managed OpenAI TTS billed to your subscription",
                "env_vars": [],
                "tts_provider": "openai",
                "requires_ontoweb_auth": True,
    ...
    "web": {
        ...
        #   - "OntoWeb Subscription" — managed Firecrawl billed via OntoWeb
        #     subscription (requires_ontoweb_auth + override_env_vars).
    ...
    "image_gen": {
        ...
        #   - "OntoWeb Subscription" — managed FAL billed via the OntoWeb
        #     subscription (requires_ontoweb_auth + override_env_vars).
    ...
    "video_gen": {
        ...
        "icon": "🎬",
        # "OntoWeb Subscription" row mirrors the image_gen pattern — managed
        # FAL video generation billed via the ONTOWEB Portal.  Plugin-backed
        ...
        "providers": [
            {
                ...
                # "OntoWeb Subscription" we set video_gen.provider = "fal"
    ...
    "x_search": {
        ...
        "setup_note": (
            "Intellect routes X searches through xAI's built-in x_search "
    ...
    "browser": {
        ...
        #   - "OntoWeb Subscription (Browser Use cloud)" — managed Browser Use
        #     billed via OntoWeb subscription (requires_ontoweb_auth +
        ...
        "providers": [
            {
                "name": "OntoWeb Subscription (Browser Use cloud)",
                "badge": "subscription",
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. intellect_CUA_DRIVER_VERSION is an
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("intellect_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``intellect update`` and
      by ``intellect computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `intellect update` calls this for every
    ...
    try:
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                _print_info("      System Settings > Privacy & Security > Accessibility")
                _print_info("      System Settings > Privacy & Security > Screen Recording")
                _print_info("    Both must allow the terminal / Intellect process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_intellect_home()}/intellect-agent && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in intellect-agent directory)")
            return
        ...
        if not camofox_dir.exists() and _npm_bin:
            ...
            # package.json so that `intellect update` does not silently pull
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.intellect/config.yaml")
        ...
        # Run the full `intellect auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.intellect/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from intellect_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: intellect auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `intellect auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: intellect auth spotify")
        except Exception as exc:
        ...
        try:
            ...
            _print_info("    Run later: intellect auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `intellect auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: intellect auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: intellect auth add xai-oauth"
                )
        else:
    ...
    *,
    ...
    if toolset_names is None or not isinstance(toolset_names, list):
        ...
        if plat_info:
            ...
            default_ts = f"intellect-{platform}"
    ...
    # "intellect-cli" (which include all _INTELLECT_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[intellect-cli, spotify]`` after enabling Spotify via ``intellect
        ...
        # (e.g. "intellect-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``intellect tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `intellect tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"intellect-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("intellect-")}
    ...
    # shipped as a bundled plugin but user must opt in via `intellect tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `intellect tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by intellect tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (intellect-cli, intellect-telegram, etc.)
    ...
    # Opening `intellect tools` is the user's opt-in to reconfigure tools, so treat
...
def _toolset_has_keys(
    ...
    *,
    ...
    if ts_key in {"web", "image_gen", "video_gen", "tts", "browser"}:
        features = get_ontoweb_subscription_features(config, force_fresh=force_fresh)
...
# ("OntoWeb Subscription" managed-gateway entry, "Firecrawl Self-Hosted")
...
# ("OntoWeb Subscription", "Local Browser", "Camofox") — see the comment block
...
def _visible_providers(
    ...
    *,
    ...
    OntoWeb-managed Tool Gateway rows (``managed_nous_feature``) are always
    shown — even to logged-out / unentitled users — so the picker advertises
    that the capability exists.  Selecting one drives an inline OntoWeb Portal
    ...
    *activates* the gateway once paid access is confirmed.
    ...
    for provider in cat.get("providers", []):
        # OntoWeb-managed Tool Gateway rows stay visible regardless of auth —
        # selecting one drives an inline Portal login. A `requires_ontoweb_auth`
        # row that is NOT a managed gateway feature (pure pre-auth UX) is
        # still hidden until the user is logged in.
        if (
            provider.get("requires_ontoweb_auth")
            and not provider.get("managed_nous_feature")
            and not features.ontoweb_auth_present
        ):
            continue
        visible.append(provider)

    # Inject plugin-registered image_gen backends (OpenAI today, more
    # later) so the picker lists them alongside FAL / OntoWeb Subscription.
    ...
    # remaining hardcoded rows ("OntoWeb Subscription", "Firecrawl
    ...
    # the hardcoded "OntoWeb Subscription" / "Local Browser" / "Camofox" rows
...
def _hidden_ontoweb_gateway_message(
    cat: dict,
    config: dict,
    capability: str,
    *,
    ...
    """Deprecated: OntoWeb Tool Gateway rows are no longer hidden.

    Previously this returned a "log in / upgrade" banner shown above a
    category when its OntoWeb-managed rows were filtered out for unentitled
    ...
    (``ensure_ontoweb_portal_access``). Kept as a no-op so call sites stay simple;
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `intellect tools` gets a silent no-op
...
def _configure_tool_category(
    ...
    *,
    ...
    hidden_ontoweb_message = _hidden_ontoweb_gateway_message(
        cat,
        config,
        f"the OntoWeb Subscription provider for {name}",
    ...
    if len(providers) == 1:
        ...
        if hidden_ontoweb_message:
            for line in hidden_ontoweb_message.splitlines():
        ...
        # When the user is logged into the OntoWeb Portal, surface a marker on providers
        # whose access is included in their subscription so it's visually
        # obvious which options cost extra vs. cost nothing on top of OntoWeb.
        try:
            _ontoweb_logged_in = bool(
                get_ontoweb_subscription_features(
                    config,
                    force_fresh=force_fresh,
                ).ontoweb_auth_present
            )
        except Exception:
            _ontoweb_logged_in = False
        ...
        for p in providers:
            ...
            # Mark OntoWeb-managed entries. Logged-in paid subscribers get the
            # "included" star; everyone else gets a "via OntoWeb Portal" hint so
            ...
            if p.get("managed_nous_feature"):
                if _ontoweb_logged_in:
                    sub_marker = "  ★ Included with your OntoWeb subscription"
                else:
                    sub_marker = "  ★ via OntoWeb Portal (login on select)"
...
def _configure_provider(
    ...
    *,
    ...
    # OntoWeb-managed Tool Gateway backends are always listed (see
    # _visible_providers), but only *activate* once the user has paid OntoWeb
    ...
    # "enable all tools" prompt (that lives in `intellect model`).
    if managed_feature:
        from intellect_cli.ontoweb_subscription import ensure_ontoweb_portal_access

        if not ensure_ontoweb_portal_access(
            capability=f"{provider.get('name', 'the OntoWeb Tool Gateway')}"
        ):
            _print_warning(
                "  Not enabled — OntoWeb Portal paid access is required for this backend."
            )
            return

    # Pure pre-auth UX rows (requires_ontoweb_auth without a managed gateway
    # feature) keep the old gate. Managed rows are handled by the inline
    # login above, so don't double-check them here.
    if provider.get("requires_ontoweb_auth") and not managed_feature:
        features = get_ontoweb_subscription_features(config, force_fresh=force_fresh)
        entitled = bool(
            features.account_info and features.account_info.paid_service_access is True
        )
        if not features.ontoweb_auth_present or not entitled:
            message = format_ontoweb_portal_entitlement_message(
                features.account_info,
                capability=f"{provider.get('name', 'OntoWeb Subscription')}",
            )
            _print_warning(
                f"  {message or 'OntoWeb Subscription is only available after logging into OntoWeb Portal.'}"
            )
            return
    ...
    if not env_vars:
        ...
        if managed_feature:
            _print_info("  Requests for this tool will be billed to your OntoWeb subscription.")
    ...
    # OntoWeb-managed sibling, show a single dim hint so users know
    # they can avoid the key entirely via a Portal subscription.
    # Suppressed when the user is already authed to the OntoWeb Portal.
    _show_portal_hint = False
    if env_vars and not managed_feature and not provider.get("requires_ontoweb_auth"):
        try:
            ...
            if _has_managed_sibling:
                _features = get_ontoweb_subscription_features(
                    config,
                    force_fresh=force_fresh,
                )
                _show_portal_hint = not _features.ontoweb_auth_present
        except Exception:
            _show_portal_hint = False

    if _show_portal_hint:
        _print_info("  Available through ONTOWEB Portal subscription.")
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `intellect tools` and `intellect setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Intellect Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://intellect-agent.ontoweb.cn/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    if first_install:
        for pkey in enabled_platforms:
            ...
            auto_configured = apply_ontoweb_managed_defaults(
                config,
            ...
            for ts_key in sorted(auto_configured):
                ...
                print(color(f"  ✓ {label}: using your OntoWeb subscription defaults", Colors.GREEN))
    ...
    print()
    from intellect_constants import display_intellect_home
    print(color(f"  Tool configuration saved to {display_intellect_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'intellect' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, intellect mcp
