"""
Backup and import commands for intellect CLI.

`intellect backup` creates a zip archive of the entire ~/.intellect/ directory
(excluding the intellect-agent repo and transient files).

`intellect import` restores from a backup zip, overlaying onto the current
INTELLECT_HOME root.
"""
...
from intellect_constants import get_default_intellect_root, get_intellect_home, display_intellect_home
...
_EXCLUDED_DIRS = {
    "intellect-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to intellect root) should be skipped."""
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside INTELLECT_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Intellect home directory."""
    intellect_root = get_default_intellect_root()

    if not intellect_root.is_dir():
        print(f"Error: Intellect home directory not found at {intellect_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"intellect-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"intellect-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_intellect_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(intellect_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(intellect_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(intellect_root)
    ...
    print(f"\nRestore with: intellect import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like an Intellect backup.
    ...
    # Look for telltale files that a intellect home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be an Intellect backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.intellect/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like an intellect dir name
        if prefix in {".intellect", "intellect"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_intellect_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (intellect_root / "config.yaml").exists()
        has_env = (intellect_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Intellect configuration.")
        ...
        for member in members:
            ...
            target = intellect_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(intellect_root.resolve())
            except ValueError:
        ...
        # Post-import: restore agent wrapper scripts
        ...
        try:
            ...
            for entry in iter_named_agent_homes(intellect_root):
            ...
            if restored_profiles:
                ...
                if created:
                    print(f"\n  Agent aliases restored: {', '.join(created)}")
                if skipped:
                    print(f"  Agent aliases skipped:  {', '.join(skipped)}")
                if not _is_wrapper_dir_in_path():
                    print(f"\n  Note: {_get_wrapper_dir()} is not in your PATH.")
                    print('  Add to your shell config (~/.bashrc or ~/.zshrc):')
                    print('    export PATH="$HOME/.local/bin:$PATH"')
        except ImportError:
            # intellect_cli.agents_home might not be available (fresh install)
            if iter_named_agent_homes(intellect_root):
                print(f"\n  Agents detected but aliases could not be created.")
                print(f"  Run: intellect agent list  (after installing intellect)")

        # Guidance
        print()
        if not (intellect_root / "intellect-agent").is_dir():
            print("Note: The intellect-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: intellect update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for agents:")
            for pname in gw_profiles:
                print(f"  intellect -a {pname} gateway install")

        print("Done. Your Intellect configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and intellect backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to INTELLECT_HOME).
...
# — `intellect update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(intellect_home: Optional[Path] = None) -> Path:
...
def create_quick_snapshot(
    ...
    intellect_home: Optional[Path] = None,
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(intellect_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside INTELLECT_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``intellect update``.
    ...
    Args:
        ...
        intellect_home: Override for the Intellect home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(intellect_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for intellect backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_intellect_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, intellect_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``intellect_root`` to ``out_path``.
...
def _pre_update_backup_dir(intellect_home: Optional[Path] = None) -> Path:
    home = intellect_home or get_intellect_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of INTELLECT_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<INTELLECT_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``intellect update``) should continue even if the backup fails.
    """
    intellect_root = intellect_home or get_default_intellect_root()
    if not intellect_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(intellect_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, intellect_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `intellect claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of INTELLECT_HOME under ``backups/`` before a
    ``intellect claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``intellect import <archive>``.  Writes to
    ``<INTELLECT_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `intellect import` and the
