...
* A list of **suggested actions** — structured entries the CLI
  turns into hints.
...
class DiagnosticAction:
    ...
    * ``cli_hint`` — print/copy a shell command (e.g.
      ``intellect -p <profile> auth``). No HTTP side effect.
...
def _main_model_visible(raw_config: Any) -> bool:
    ...
    Diagnostics runs in the kanban process which may not share the CLI's
...
def _rule_triage_aux_unavailable(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    With auto-decompose off, the user must run ``intellect kanban specify``,
    ...
    if auto_decompose:
        ...
        detail_path = (
            ...
            "`intellect kanban specify`, which uses auxiliary.triage_specifier."
        )
    ...
    actions = [
        DiagnosticAction(
            ...
            payload={
                "command": (
                    f"intellect config set {primary_slot}.provider auto"
                )
            },
            suggested=True,
        ),
    ...
    if not fallback_explicit and not main_visible:
        actions.append(DiagnosticAction(
            ...
            payload={
                "command": (
                    f"intellect config set {fallback_slot}.provider auto"
                )
            },
        ))
    if not auto_decompose:
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Specify manually: intellect kanban specify {task_id}",
            payload={"command": f"intellect kanban specify {task_id}"},
        ))
...
def _rule_repeated_failures(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    if most_recent_outcome == "spawn_failed" and assignee and assignee != "default":
        ...
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Verify profile: intellect -p {assignee} doctor",
            payload={"command": f"intellect -p {assignee} doctor"},
            suggested=True,
        ))
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Fix profile auth: intellect -p {assignee} auth",
            payload={"command": f"intellect -p {assignee} auth"},
        ))
        ...
        if task_id:
            actions.append(DiagnosticAction(
                kind="cli_hint",
                label=f"Check logs: intellect kanban log {task_id}",
                payload={"command": f"intellect kanban log {task_id}"},
                suggested=True,
            ))
...
def _rule_block_unblock_cycling(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    if task_id:
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Check block reasons: intellect kanban events {task_id}",
            payload={"command": f"intellect kanban events {task_id}"},
            suggested=True,
        ))
    return [Diagnostic(
...
def _rule_stranded_in_ready(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    * Dispatcher is misconfigured (wrong board, wrong INTELLECT_HOME).
    ...
    works for Intellect profiles, registered lanes, external workers, and
    ...
    actions = [
        ...
        DiagnosticAction(
            ...
            payload={"command": "intellect kanban diagnostics"},
        ),
    ]
...
def config_from_runtime_config(raw_config: Optional[dict]) -> dict:
    """Build diagnostics config from the full Intellect runtime config.
    ...
    *,
    ...
    for rule in _RULES:
        try:
            ...
            # A broken rule must never crash the kanban service. Rule bugs
