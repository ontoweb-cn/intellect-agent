"""``intellect portal`` — small CLI surface for ONTOWEB Portal users.
...
already in ``intellect auth`` or ``intellect tools``. It's a discovery + status
...
def _cmd_status(args) -> int:
    ...
    try:
        auth = get_ontoweb_auth_status() or {}
    except Exception:
        auth = {}
    ...
    print()
    print(color("  ONTOWEB Portal", Colors.MAGENTA))
    ...
    if logged_in:
        ...
        print(f"  Login:   intellect auth add ontoweb --type oauth")

    # Provider selection (independent of auth)
    model_cfg = config.get("model") if isinstance(config.get("model"), dict) else {}
    provider = str(model_cfg.get("provider") or "").strip().lower()
    if provider == "ontoweb":
        print(f"  Model:   {color('✓ using OntoWeb as inference provider', Colors.GREEN)}")
    elif provider:
        print(f"  Model:   currently {provider} (switch with `intellect model`)")
    ...
    for feat in features.items():
        if feat.managed_by_nous:
            state = color("via ONTOWEB Portal", Colors.GREEN)
...
def _cmd_tools(args) -> int:
    ...
    if not features.ontoweb_auth_present:
        print(color("  Not logged into ONTOWEB Portal — sign in with `intellect auth add ontoweb --type oauth`.", Colors.YELLOW))
        print()
...
def portal_command(args) -> int:
    """Top-level dispatch for `intellect portal <subcommand>`."""
    ...
    print("Run `intellect portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `intellect portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="ONTOWEB Portal status, subscription, and Tool Gateway routing",
        description=(
            "Inspect ONTOWEB Portal auth, Tool Gateway routing, and open the "
    ...
    portal_sub.add_parser(
        "tools",
        help="List Tool Gateway tools and which are routed via OntoWeb",
    )
