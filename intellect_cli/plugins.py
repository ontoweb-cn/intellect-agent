"""
Intellect Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with intellect-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.intellect/plugins/<name>/``
3. **Project plugins** – ``./.intellect/plugins/<name>/`` (opt-in via
   ``intellect_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``intellect_agent.plugins``
   entry-point group.
...
def get_bundled_plugins_dir() -> Path:
    ...
    Honours ``INTELLECT_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("INTELLECT_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``intellect_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.intellect/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("intellect_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When intellect_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Intellect loggers.
    """
    ...
    logger.debug(
        "intellect_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
ENTRY_POINTS_GROUP = "intellect_agent.plugins"

_NS_PARENT = "intellect_plugins"
...
class PluginManifest:
    ...
    #              in ~/.intellect/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``intellect plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``intellect honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``intellect <subcommand>``
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``intellect model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: intellect_cli.main imports plugins indirectly
        from intellect_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.intellect/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to intellect_cli/. Two layouts are
        ...
        # 2. User plugins (~/.intellect/plugins/)
        user_dir = get_intellect_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.intellect/plugins/)
        if _env_enabled("intellect_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".intellect" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set intellect_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # RAG provider plugins are loaded by plugins/rag/__init__.py
            # (parallel to memory providers). Record manifest only.
            if manifest.kind == "rag":
                loaded = LoadedPlugin(manifest=manifest, enabled=True)
                self._plugins[lookup_key] = loaded
                logger.debug(
                    "Skipping '%s' (rag, handled by plugins/rag/ discovery)",
                    lookup_key,
                )
                continue

            # Built-in backends auto-load — they ship with intellect and must
            # just work. Selection among them (e.g. which image_gen backend
            # services calls) is driven by ``<category>.provider`` config,
            # enforced by the tool wrapper.
            #
            # Bundled platform plugins (gateway adapters like IRC) auto-load
            # for the same reason: every platform Intellect ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `intellect plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
                ...
                if init_file.exists():
                    try:
                        ...
                        if (
                            ...
                            "register_rag_provider" in source_text
                            or "RAGProvider" in source_text
                        ):
                            kind = "rag"
                            logger.debug(
                                "Plugin %s: detected RAG provider, "
                                "treating as kind='rag'",
                                key,
                            )
                    except Exception:
                        pass
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``intellect_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``intellect_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="intellect-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``intellect tools`` TUI so plugin-provided toolsets appear
