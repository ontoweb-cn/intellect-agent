"""
Top-level argparse construction for the intellect CLI.
...
# argparse runs (it sets ``INTELLECT_HOME`` and strips itself from ``sys.argv``),
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``intellect_cli.relaunch`` should carry over when
...
_EPILOGUE = """
Examples:
    intellect                        Start interactive chat
    intellect chat -q "Hello"        Single query mode
    intellect -c                     Resume the most recent session
    intellect -c "my project"        Resume a session by name (latest in lineage)
    intellect --resume <session_id>  Resume a specific session by ID
    intellect setup                  Run setup wizard
    intellect logout                 Clear stored authentication
    intellect auth add <provider>    Add a pooled credential
    intellect auth list              List pooled credentials
    intellect auth remove <p> <t>    Remove pooled credential by index, id, or label
    intellect auth reset <provider>  Clear exhaustion status for a provider
    intellect model                  Select default model
    intellect fallback [list]        Show fallback provider chain
    intellect fallback add           Add a fallback provider (same picker as `intellect model`)
    intellect fallback remove        Remove a fallback provider from the chain
    intellect config                 View configuration
    ...
    intellect logs                   View agent.log (last 50 lines)
    intellect logs -f                Follow agent.log in real time
    intellect logs errors            View errors.log
    intellect logs --since 1h        Lines from the last hour
    intellect debug share             Upload debug report for support
    intellect update                 Update to latest version

For more help on a command:
    intellect <command> --help
"""
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="intellect",
        description="Intellect Agent - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `intellect chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via intellect_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
        ...
        help=(
            ...
            "under model.provider — use `intellect setup` or edit the file to change it."
        ),
    )
    parser.add_argument(
        "-t",
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to INTELLECT_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.intellect/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Intellect Agent",
    )
    ...
    chat_parser.add_argument(
        ...
        help=(
            ...
            "use; takes precedence over -q/--query; with "
            "INTELLECT_QUERY_FILE_DELETE=1 the file is deleted after reading)"
        ),
    )
    chat_parser.add_argument(
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also INTELLECT_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.intellect/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
