...
_OAUTH_CAPABLE_PROVIDERS = {"anthropic", "ontoweb", "openai-codex", "xai-oauth", "qwen-oauth", "google-gemini-cli", "minimax-oauth"}
...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_intellect_oauth_login_pure()
        if not creds:
            raise SystemExit("Anthropic OAuth login did not return credentials.")
        anthropic_mod.save_intellect_oauth_credentials(creds)
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:intellect_pkce",
    ...
    if provider == "ontoweb":
        # Codex-style auto-import: if a shared OntoWeb credential lives at
        # <intellect-root>/shared/ontoweb_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `intellect --profile <name>
        # auth add ontoweb --type oauth` a one-tap operation for users who
        # run multiple profiles.
        shared = auth_mod._read_shared_ontoweb_state()
        if shared:
            try:
                path = auth_mod._ontoweb_shared_store_path()
            except RuntimeError:
                path = None
            print()
            if path:
                print(f"Found existing OntoWeb OAuth credentials at {path}")
            else:
            ...
            if do_import in {"", "y", "yes"}:
                ...
                rehydrated = auth_mod._try_import_shared_ontoweb_state(
                    timeout_seconds=getattr(args, "timeout", None) or 15.0,
                )
                if rehydrated is not None:
                    custom_label = (getattr(args, "label", None) or "").strip() or None
                    entry = auth_mod.persist_ontoweb_credentials(rehydrated, label=custom_label)
        ...
        creds = auth_mod._ontoweb_device_code_login(
        ...
        # Honor `--label <name>` so ontoweb matches other providers' UX.  The
        # helper embeds this into providers.ontoweb so that label_from_token
        # doesn't overwrite it on every subsequent load_pool("ontoweb").
        custom_label = (getattr(args, "label", None) or "").strip() or None
        entry = auth_mod.persist_ontoweb_credentials(creds, label=custom_label)
        shown_label = entry.label if entry is not None else label_from_token(
    ...
    if provider == "openai-codex":
        # Clear any existing suppression marker so a re-link after `intellect auth
    ...
    raise SystemExit(f"`intellect auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Intellect reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `intellect auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `intellect auth` is called bare."""
