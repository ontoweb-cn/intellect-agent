"""CLI for the Intellect Kanban board — ``intellect kanban …`` subcommand.

Exposes the full Kanban command surface documented in the design spec
(``docs/intellect-kanban-v1-spec.pdf``).  All DB work is delegated to
...
from intellect_cli import kanban_swarm as ks
from intellect_cli.profiles import get_active_profile_name, get_profile_dir, seed_profile_skills
...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    - ``running=True``: a gateway is alive for this INTELLECT_HOME and its
    ...
    Used by ``intellect kanban create`` (and callers) to warn when a task
    ...
    if pid and not dispatch_on:
        return (
            ...
            "standalone daemon (`intellect kanban daemon --force`)."
        )
    return (
        False,
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    # INTELLECT_KANBAN_BOARD env var, then the persisted current-board
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "(set via `intellect kanban boards switch <slug>` or the "
            "INTELLECT_KANBAN_BOARD env var). Use `intellect kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    b_create.add_argument("--icon", default=None,
                          help="Optional emoji or single-character icon")
    b_create.add_argument("--color", default=None,
                          help="Optional hex color (e.g. '#8b5cf6')")
    ...
    p_list.add_argument("--mine", action="store_true",
                        help="Filter by $intellect_PROFILE as assignee")
    ...
    p_comment.add_argument("--author", default=None,
                           help="Author name (default: $intellect_PROFILE or 'user')")
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="DEPRECATED — dispatcher now runs in the gateway. Use `intellect gateway start`.",
    )
    ...
    p_specify.add_argument(
        ...
        help="Author name recorded on the audit comment "
             "(default: $intellect_PROFILE or 'specifier')",
    )
    p_specify.add_argument(
        "--json",
        action="store_true",
    ...
    p_decompose.add_argument(
        ...
        default=None,
        help="Author name recorded on the audit comment "
             "(default: $intellect_PROFILE or 'decomposer')",
    )
    p_decompose.add_argument(
        "--json",
        action="store_true",
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``intellect kanban …`` argparse dispatch.
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: intellect kanban <action> [options]\n"
                "Run 'intellect kanban --help' for the full list of actions.",
                file=sys.stderr,
            )
        return 0
    ...
    # env-var pin for the duration of this call. Using INTELLECT_KANBAN_BOARD
    ...
    prev_board_env = os.environ.get("INTELLECT_KANBAN_BOARD")
    ...
    def _restore_board_env() -> None:
        ...
        if prev_board_env is None:
            ...
            os.environ["INTELLECT_KANBAN_BOARD"] = prev_board_env
    if board_override:
        try:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            print(
                ...
                f"Create it with `intellect kanban boards create {normed}`.",
                file=sys.stderr,
            )
            return 1
        os.environ["INTELLECT_KANBAN_BOARD"] = normed
    ...
    # INTELLECT_HOME. Previously only `init` and `daemon` triggered
...
def _profile_author() -> str:
    ...
    for env in ("intellect_PROFILE_NAME", "intellect_PROFILE"):
...
# ---------------------------------------------------------------------------
# Boards management (intellect kanban boards …)
# ---------------------------------------------------------------------------

def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``intellect kanban boards <action>``.
    ...
    DB, so a fresh INTELLECT_HOME that has never called ``kanban init``
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if not boards:
        print("(no boards — create one with `intellect kanban boards create <slug>`)")
        return 0
    ...
    if len(boards) > 1:
        print("Switch boards with `intellect kanban boards switch <slug>`.")
    return 0
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "switch", False):
        ...
        print(f"  Use `intellect kanban boards switch {meta['slug']}` to make it current.")
    return 0


def _cmd_boards_rm(args: argparse.Namespace) -> int:
    # When the user runs `intellect kanban boards delete <slug>` (alias), the
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    # the kanban dispatcher can use them without a separate `intellect profile
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    if not data:
        print("(no assignees — create a profile with `intellect -p <name> setup`)")
        return 0
    # Header
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    if len(all_boards) > 1:
        ...
        print(
            ...
            f"`intellect kanban boards list`)\n"
        )
    if not tasks:
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    # comments / runs.
    from intellect_cli import kanban_diagnostics as kd
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``intellect kanban show <id>``
...
def _cmd_diagnostics(args: argparse.Namespace) -> int:
    ...
    the kanban service uses, so CLI output matches what the UI shows.
    """
    from intellect_cli import kanban_diagnostics as kd
    from intellect_cli.config import load_config
...
def _worker_run_id_for(task_id: str) -> Optional[int]:
    if os.environ.get("INTELLECT_KANBAN_TASK") != task_id:
        return None
    raw = os.environ.get("intellect_KANBAN_RUN_ID")
    if not raw:
        return None
    try:
        return int(raw)
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "force", False):
        print(
            "intellect kanban daemon: DEPRECATED — the dispatcher now runs\n"
            "inside the gateway. To use kanban:\n"
            "\n"
            "    intellect gateway start       # starts the gateway + embedded dispatcher\n"
            "\n"
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"`intellect kanban list --status ready` / "
                    f"`intellect kanban list --status blocked` for recent "
    ...
    def _ready_queue_nonempty() -> bool:
        ...
        task whose assignee maps to a real Intellect profile (i.e. one the
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from intellect_cli import kanban_specify as spec
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    from intellect_cli import kanban_decompose as decomp
