"""`intellect checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.intellect/checkpoints/``.  Actions:

    intellect checkpoints               # same as `status`
    intellect checkpoints status        # total size, project count, breakdown
    intellect checkpoints list          # per-project checkpoint counts + workdir
    intellect checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    intellect checkpoints clear [-f]    # nuke the entire base (asks first)
    intellect checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    intellect checkpoints
    intellect checkpoints prune --retention-days 3 --max-size-mb 200
    intellect checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: intellect checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``intellect checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `intellect checkpoints` → status
