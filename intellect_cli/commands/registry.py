"""Slash command definitions and autocomplete for the Intellect CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Intellect config/state", "Session",
    ...
    CommandDef("moa", "Mixture of Agents — multi-model aggregation (N+1 calls per request)", "Models",
               aliases=("mixture-of-agents",), args_hint="[list|preset]",
               subcommands=("list",)),
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Intellect works on across turns until achieved", "Session",
               args_hint="[text | pause | resume | clear | status]"),
    CommandDef("subgoal", "Add or manage extra criteria on the active goal", "Session",
               args_hint="[text | remove N | clear]"),
    CommandDef("status", "Show session info", "Session"),
    CommandDef("whoami", "Show your slash command access (admin / user)", "Info"),
    CommandDef("agent", "Show active agent name and home directory", "Info",
               aliases=("profile",)),
    ...
    CommandDef("busy", "Control what Enter does while Intellect is working", "Configuration",
    ...
    CommandDef("blueprint", "List, search, and manage automation blueprints", "Tools & Skills",
               cli_only=True, args_hint="[subcommand]",
               subcommands=("list", "show", "search")),
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.intellect/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Intellect Agent to the latest version", "Info"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`intellect_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/intellect`` subcommand mapping, and
...
Telegram only displays a small BotCommand menu in practice.  The full Intellect
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``intellect skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``intellect skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``intellect skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/intellect <verb>`` subcommand.
    ...
    can still reach them via ``/intellect <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/intellect`` is always reserved as the first entry so the
    legacy ``/intellect <subcommand>`` form keeps working for anything that
    ...
    # Reserve /intellect as the catch-all top-level command.
    entries.append(("intellect", "Talk to Intellect or run a subcommand", "[subcommand] [args]"))
    seen.add("intellect")
    ...
    # Second pass: aliases.  Only add if slots remain.
    for cmd in COMMAND_REGISTRY:
        if not _is_gateway_available(cmd, overrides):
            continue
        for alias in cmd.aliases:
...
def slack_app_manifest(request_url: str = "https://intellect-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /intellect handler.

    Maps both canonical names and aliases so /intellect bg do stuff works
    the same as /intellect background do stuff.

    Plugin-registered slash commands are included so ``/intellect <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from intellect_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
