...
Pure logic: walks a Intellect config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `intellect doctor` and a future `intellect migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Intellect config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Intellect config and return retirement issues.
