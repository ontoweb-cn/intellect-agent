"""CLI subcommand: ``intellect send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"intellect send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"intellect send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"intellect send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"intellect send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"intellect send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `intellect gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.intellect/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_intellect_env() -> None:
    """Populate ``os.environ`` from ``~/.intellect/.env`` AND bridge top-level
    ...
    startup that ``intellect send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.intellect/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.intellect/config.yaml`` into
       ...
       via ``intellect config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``intellect send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.intellect/.env and ~/.intellect/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "intellect send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  intellect send --to telegram \"hello\"\n"
            "  intellect send --to discord:#ops --file report.md\n"
            "  intellect send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "intellect send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `intellect send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Intellect "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.intellect/.env + ~/.intellect/config.yaml) — no LLM, "
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `intellect send --list telegram`.",
    )
