"""Normalized ONTOWEB Portal account entitlement helpers."""
...
OntowebAccountInfoSource = Literal["jwt", "account_api", "inference_key", "none", "error"]

_ACCOUNT_INFO_CACHE_TTL = 60
_account_info_cache: tuple[str, float, "OntowebPortalAccountInfo"] | None = None
_ACCOUNT_INFO_CACHE_LOCK = threading.Lock()


@dataclass(frozen=True)
class OntowebPortalSubscriptionInfo:
...
@dataclass(frozen=True)
class OntowebPaidServiceAccessInfo:
...
@dataclass(frozen=True)
class OntowebPortalAccountInfo:
    logged_in: bool
    source: OntowebAccountInfoSource
    fresh: bool
    ...
    subscription: Optional[OntowebPortalSubscriptionInfo] = None
    paid_service_access: Optional[bool] = None
    paid_service_access_info: Optional[OntowebPaidServiceAccessInfo] = None
...
def ontoweb_portal_billing_url(account_info: Optional[OntowebPortalAccountInfo] = None) -> str:
    """Return the billing URL for a normalized OntoWeb account snapshot."""
    try:
    ...
    if not isinstance(base, str) or not base.strip():
        base = DEFAULT_ONTOWEB_PORTAL_URL
    return f"{base.rstrip('/')}/billing"


def format_ontoweb_portal_entitlement_message(
    account_info: Optional[OntowebPortalAccountInfo],
    *,
    capability: str = "this feature",
    include_refresh_hint: bool = True,
) -> Optional[str]:
    """Return user-facing guidance for a missing OntoWeb paid entitlement.
    ...
    credits does not.
    """
    billing_url = ontoweb_portal_billing_url(account_info)
    ...
    if account_info is None:
        return (
            f"Intellect could not verify your ONTOWEB Portal entitlement, so {capability} "
            f"is unavailable. Run `intellect model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"OntoWeb inference credentials are configured, but Intellect cannot verify "
                f"your ONTOWEB Portal paid access for {capability}. Log in with "
                f"`intellect model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to ONTOWEB Portal to use {capability}: run `intellect model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"Intellect could not verify your ONTOWEB Portal paid access, so {capability} "
            f"is unavailable."
        )
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `intellect model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"Intellect could not find a ONTOWEB Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `intellect model` to "
            f"authenticate again; if the problem persists, contact OntoWeb support."
        )
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `intellect model` to refresh Intellect."
        return message

    return (
        f"Your ONTOWEB Portal account does not currently have paid service access, "
...
def _no_paid_access_message(
    ...
    if has_active_subscription and active_subscription_is_paid:
        ...
        return (
            f"Your ONTOWEB Portal credits are exhausted{credit_detail}, so {capability} "
    ...
    if has_active_subscription and active_subscription_is_paid is False:
        return (
            f"Your current ONTOWEB Portal plan does not include paid service access, "
    ...
    if has_active_subscription is False:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your ONTOWEB Portal account has no active subscription or usable credits"
    ...
    credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
    return (
        f"Your ONTOWEB Portal account has no usable paid credits{credit_detail}, so "
...
def _fresh_account_info(
    *,
    ...
    try:
        from intellect_cli.auth import get_provider_auth_state, resolve_ontoweb_access_token

        access_token = resolve_ontoweb_access_token()
        ...
        payload = _fetch_ontoweb_account_info(access_token, portal_base_url)
        if not payload:
            return _error_info(
...
def _info_from_inference_key_pool(
    portal_base_url: Optional[str],
) -> Optional[OntowebPortalAccountInfo]:
    """Return an explicit unknown-entitlement snapshot for opaque OntoWeb keys."""
    try:
        entry = _select_ontoweb_pool_entry()
        if entry is None:
            return None
...
def _info_from_oauth_pool(
    *,
    ...
    try:
        payload = _fetch_ontoweb_account_info(access_token, entry_portal_url)
    except Exception as exc:
        return _error_info(
            error=exc,
            logged_in=True,
...
def _select_ontoweb_pool_entry() -> Optional[Any]:
...
def _fetch_ontoweb_account_info(
    access_token: str,
    portal_base_url: Optional[str] = None,
) -> dict[str, Any]:
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from intellect_cli.auth import _decode_jwt_claims
    except Exception:
        return None
    ...
    access_info = OntowebPaidServiceAccessInfo(
        allowed=paid_access,
...
def _subscription_from_payload(value: Any) -> Optional[OntowebPortalSubscriptionInfo]:
    if not isinstance(value, dict):
        return None
    return OntowebPortalSubscriptionInfo(
...
def _paid_service_access_from_payload(value: Any) -> Optional[OntowebPaidServiceAccessInfo]:
    if not isinstance(value, dict):
        return None
    allowed = _coerce_bool(value.get("allowed"))
    paid_access = _coerce_bool(value.get("paid_access"))
    return OntowebPaidServiceAccessInfo(
        allowed=allowed,
        paid_access=paid_access,
