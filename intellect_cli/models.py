...
_INTELLECT_USER_AGENT = f"intellect-cli/{_INTELLECT_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    """
    from intellect_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $INTELLECT_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $INTELLECT_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Intellect refreshes the cache from
...
_VOLCENGINE_BASE_MODELS: list[str] = [
    "doubao-1.5-pro-256k",
    "doubao-1.5-pro-32k",
    "doubao-pro-256k",
    "doubao-pro-128k",
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    "volcengine": list(_VOLCENGINE_BASE_MODELS),
    "volcengine-coding-plan": [
        "doubao-seed-2.0-code",
        *_VOLCENGINE_BASE_MODELS,
    ],
    "volcengine-agent-plan": list(_VOLCENGINE_BASE_MODELS),
    "tencent-tokenhub": [
        "hy3-preview",
    ],
    "arcee": [
...
# ---------------------------------------------------------------------------
# OntoWeb Portal free-model helper
# ---------------------------------------------------------------------------
# The OntoWeb Portal models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# OntoWeb Portal account tier detection
# ---------------------------------------------------------------------------
def is_ontoweb_free_tier(account_info: dict[str, Any]) -> bool:
...
def partition_ontoweb_models_by_tier(
    model_ids: list[str],
    pricing: dict[str, dict[str, str]],
    free_tier: bool,
) -> tuple[list[str], list[str]]:
    """Split OntoWeb models into (selectable, unavailable) based on user tier.
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    The Portal's ``/api/ontoweb/recommended-models`` endpoint advertises which
    models are free *right now* — independent of what the in-repo
    ``_PROVIDER_MODELS["ontoweb"]`` list happens to contain or whether the
    ...
    older Intellect that doesn't ship that model in its hardcoded curated
    ...
    * ``pricing`` gets a synthetic ``{"prompt": "0", "completion": "0"}``
      ...
      map, so :func:`partition_ontoweb_models_by_tier` keeps it.

    Failures (network, parse, missing field) are silent and degrade to
    returning the inputs unchanged.
    """
    try:
        payload = fetch_ontoweb_recommended_models(
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    users. The Portal's ``/api/ontoweb/recommended-models`` endpoint advertises
    which paid models are blessed *right now* — independent of what the
    in-repo ``_PROVIDER_MODELS["ontoweb"]`` list happens to contain or whether
    ...
    picker even if the user is running an older Intellect that doesn't ship
    ...
    * ``pricing`` is left untouched — we deliberately do NOT synthesize
      ...
      :func:`partition_ontoweb_models_by_tier` keeps free models selectable;
...
def check_ontoweb_free_tier(*, force_fresh: bool = False) -> bool:
    """Check if the current OntoWeb Portal user is on a free (unpaid) tier.
    ...
    try:
        ...
        account_info = get_ontoweb_portal_account_info(force_fresh=force_fresh)
...
# ---------------------------------------------------------------------------
# OntoWeb Portal recommended models
#
...
ONTOWEB_RECOMMENDED_MODELS_PATH = "/api/ontoweb/recommended-models"
_ONTOWEB_RECOMMENDED_CACHE_TTL: int = 600  # seconds (10 minutes)
# (result_dict, timestamp) keyed by portal_base_url so staging vs prod don't collide.
_ontoweb_recommended_cache: dict[str, tuple[dict[str, Any], float]] = {}


def fetch_ontoweb_recommended_models(
    portal_base_url: str = "",
    timeout: float = 5.0,
    *,
    force_refresh: bool = False,
) -> dict[str, Any]:
    """Fetch the OntoWeb Portal's curated recommended-models payload.

    Hits ``<portal>/api/ontoweb/recommended-models``. The endpoint is public —
    no auth is required. Results are cached per portal URL for
    ``_ONTOWEB_RECOMMENDED_CACHE_TTL`` seconds; pass ``force_refresh=True`` to
    bypass the cache.
    ...
    """
    base = (portal_base_url or "https://portal.ontoweb.cn").rstrip("/")
    now = time.monotonic()
    cached = _ontoweb_recommended_cache.get(base)
    if not force_refresh and cached is not None:
        payload, cached_at = cached
        if now - cached_at < _ONTOWEB_RECOMMENDED_CACHE_TTL:
            return payload

    url = f"{base}{ONTOWEB_RECOMMENDED_MODELS_PATH}"
    try:
    ...
    _ontoweb_recommended_cache[base] = (data, now)
    return data


def _resolve_ontoweb_portal_url() -> str:
    """Best-effort lookup of the Portal base URL the user is authed against."""
    try:
        from intellect_cli.auth import (
            DEFAULT_ONTOWEB_PORTAL_URL,
            get_provider_auth_state,
        )
        state = get_provider_auth_state("ontoweb") or {}
        portal = str(state.get("portal_base_url") or "").strip()
        if portal:
            return portal.rstrip("/")
        return str(DEFAULT_ONTOWEB_PORTAL_URL).rstrip("/")
    except Exception:
        return "https://portal.ontoweb.cn"
...
def get_ontoweb_recommended_aux_model(
    *,
    vision: bool = False,
    ...
    * ``vision=False`` → ``paidRecommendedCompactionModel`` or
    ...
    via :func:`check_ontoweb_free_tier`. Pass an explicit bool to bypass the
    ...
    ``google/gemini-3-flash-preview``).
    """
    base = portal_base_url or _resolve_ontoweb_portal_url()
    payload = fetch_ontoweb_recommended_models(base, force_refresh=force_refresh)
    if not payload:
        return None

    if free_tier is None:
        try:
            free_tier = check_ontoweb_free_tier()
        except Exception:
...
CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("ontoweb",           "OntoWeb Portal",              "OntoWeb Portal (OntoWeb Research subscription)"),
    ...
    ProviderEntry("volcengine",         "Volcano Engine",           "Volcano Engine (火山引擎) — Doubao models direct API"),
    ProviderEntry("volcengine-coding-plan", "Volcano Engine Coding Plan", "Volcano Engine Coding Plan — dedicated coding tier"),
    ProviderEntry("volcengine-agent-plan",  "Volcano Engine Agent Plan",  "Volcano Engine Agent Plan — dedicated agent tier"),
...
#
# Some vendors expose several Intellect provider slugs (one per endpoint /
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (OntoWeb Portal, private mirrors, older catalog snapshots) don't populate
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from intellect_cli.model_catalog import get_curated_openrouter_models
...
def get_curated_ontoweb_model_ids() -> list[str]:
    """Return the curated OntoWeb Portal model-id list.

    Prefers the remotely-hosted catalog manifest (published under
    ``website/static/api/model-catalog.json``); falls back to the in-repo
    snapshot in ``_PROVIDER_MODELS["ontoweb"]`` when the manifest is
    unreachable. Always returns a list (never None).
    """
    try:
        from intellect_cli.model_catalog import get_curated_ontoweb_models
        remote = get_curated_ontoweb_models()
    except Exception:
        remote = None
    if remote:
        return list(remote)
    return list(_PROVIDER_MODELS.get("ontoweb", []))
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, OntoWeb Portal).
    """
...
_DEFAULT_ONTOWEB_INFERENCE_BASE = "https://inference.ontoweb.cn"


def _resolve_ontoweb_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for OntoWeb Portal pricing.

    The OntoWeb inference ``/v1/models`` endpoint exposes pricing without
    ...
    try:
        ...
        pass
    return ("", _DEFAULT_ONTOWEB_INFERENCE_BASE)


# ---------------------------------------------------------------------------
# Deprecated aliases (pre-OntoWeb rename — remove after downstream plugins migrate)
# ---------------------------------------------------------------------------
is_nous_free_tier = is_ontoweb_free_tier
partition_nous_models_by_tier = partition_ontoweb_models_by_tier
fetch_nous_recommended_models = fetch_ontoweb_recommended_models
get_nous_recommended_aux_model = get_ontoweb_recommended_aux_model
get_curated_nous_model_ids = get_curated_ontoweb_model_ids
_resolve_nous_portal_url = _resolve_ontoweb_portal_url
_resolve_nous_pricing_credentials = _resolve_ontoweb_pricing_credentials
NOUS_RECOMMENDED_MODELS_PATH = ONTOWEB_RECOMMENDED_MODELS_PATH


def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    """Return live pricing for providers that support it (openrouter, ontoweb, novita)."""
    ...
    if normalized == "ontoweb":
        api_key, base_url = _resolve_ontoweb_pricing_credentials()
        if base_url:
            # OntoWeb base_url typically looks like https://inference.ontoweb.cn/v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from intellect_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    Supports ``provider:model`` syntax to switch providers at runtime::
        ...
        ontoweb:hermes-3                           →  ("ontoweb", "hermes-3")
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (Codex, OntoWeb, etc. all support /models)
...
_AGGREGATOR_PROVIDERS = frozenset(
    {"ontoweb", "openrouter", "copilot", "kilocode"}
)
...
def _resolve_static_model_alias(
    ...
    try:
        from intellect_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_static_provider_for_model(
    ...
    # If someone types `/model ontoweb` or `/model anthropic`, treat it as a
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Intellect' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``intellect_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Intellect should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``intellect auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.intellect/.env``.
    ...
    try:
        from intellect_cli.auth import read_credential_pool
        from intellect_cli.copilot_auth import (
...
#   - "ontoweb": curated list and Portal /models endpoint are the source of
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (Codex, OntoWeb),
    falling back to static lists. For providers in ``_MODELS_DEV_PREFERRED``
    (opencode-go/zen, xiaomi, deepseek, smaller inference providers, etc.),
    models.dev entries are merged on top of curated so new models released
    on the platform appear in ``/model`` without a Intellect release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Intellect release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "ontoweb":
        # Try live OntoWeb Portal /models endpoint
        try:
            from intellect_cli.auth import fetch_ontoweb_models, resolve_ontoweb_runtime_credentials
            creds = resolve_ontoweb_runtime_credentials()
            if creds:
                live = fetch_ontoweb_models(api_key=creds.get("api_key", ""), inference_base_url=creds.get("base_url", ""))
                if live:
                    return live
        except Exception:
            pass
        # Live failed (or no creds). Fall back to the docs-hosted manifest
        # — NOT the in-repo _PROVIDER_MODELS["ontoweb"] snapshot — so newly
        # added Portal models still surface without a Intellect release.
        manifest_ids = get_curated_ontoweb_model_ids()
        if manifest_ids:
            return manifest_ids
...
#
# Cache strategy:
#   - One JSON file at $INTELLECT_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    ...
    return get_intellect_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    (codex, copilot, anthropic-via-claude-code, ontoweb portal), the
    relevant tokens live in ``$INTELLECT_HOME/auth.json`` and external
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _INTELLECT_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Intellect' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _INTELLECT_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from intellect_constants import get_intellect_home
    return get_intellect_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Intellect will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Intellect cannot verify the model name."
