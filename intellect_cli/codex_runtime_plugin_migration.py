"""Migrate Intellect' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Intellect' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Intellect mcp_servers.<n>.command/args/env  → codex stdio transport
  Intellect mcp_servers.<n>.url/headers       → codex streamable_http transport
  Intellect mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Intellect mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Intellect-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Intellect' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Intellect config.")
...
_KEYS_DROPPED_WITH_WARNING = {
    # Intellect' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, intellect_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Intellect MCP server config to the codex inline-table dict
    ...
    if not isinstance(intellect_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(intellect_cfg.get("command"))
    has_url = bool(intellect_cfg.get("url"))
    ...
    if has_command:
        # Stdio transport
        out["command"] = str(intellect_cfg["command"])
        args = intellect_cfg.get("args") or []
        if args:
            out["args"] = [str(a) for a in args]
        env = intellect_cfg.get("env") or {}
        if env:
            # Codex expects string values
            out["env"] = {str(k): str(v) for k, v in env.items()}
        cwd = intellect_cfg.get("cwd")
        if cwd:
            out["cwd"] = str(cwd)
    elif has_url:
        # streamable_http transport (codex covers both http and SSE here)
        out["url"] = str(intellect_cfg["url"])
        headers = intellect_cfg.get("headers") or {}
        if headers:
            out["http_headers"] = {str(k): str(v) for k, v in headers.items()}
        # Intellect' transport: sse hint is informational; codex auto-negotiates
        if intellect_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]

    # Timeouts
    if "timeout" in intellect_cfg:
        try:
            out["tool_timeout_sec"] = float(intellect_cfg["timeout"])
        except (TypeError, ValueError):
            skipped.append("timeout (not numeric)")
    if "connect_timeout" in intellect_cfg:
        try:
            out["startup_timeout_sec"] = float(intellect_cfg["connect_timeout"])
        except (TypeError, ValueError):
            skipped.append("connect_timeout (not numeric)")

    # Enabled flag (codex defaults to true so we only emit when explicitly false)
    if intellect_cfg.get("enabled") is False:
        out["enabled"] = False

    # Detect keys we explicitly drop with warning
    for key in intellect_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            skipped.append(f"{key} (no codex equivalent)")
        elif key not in _KNOWN_intellect_KEYS:
            skipped.append(f"{key} (unknown Intellect key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (INTELLECT_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Intellect)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Intellect' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Intellect' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Intellect
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="intellect-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a INTELLECT_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed intellect-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.intellect`` symlink that happens to live under ``/private/var/folders``
...
def _build_intellect_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Intellect' own
    ...
    so an intellect installed under /opt/, /usr/local/, or a venv all work.
    INTELLECT_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # INTELLECT_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_intellect_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever INTELLECT_HOME its launcher sets
    ...
    # — that would override the launcher's INTELLECT_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("INTELLECT_HOME", tmp_path) would
    ...
    intellect_home = os.environ.get("INTELLECT_HOME") or ""
    if intellect_home and _looks_like_test_tempdir(intellect_home):
        intellect_home = ""
    if intellect_home:
        env["INTELLECT_HOME"] = intellect_home
    # PYTHONPATH passes through so a worktree-launched intellect finds the
    # branch's modules instead of the installed package.
    pythonpath = os.environ.get("PYTHONPATH")
    if pythonpath:
        env["PYTHONPATH"] = pythonpath
    # Quiet mode + redaction defaults so the MCP wire stays clean.
    env["INTELLECT_QUIET"] = "1"
    env["INTELLECT_REDACT_SECRETS"] = env.get("INTELLECT_REDACT_SECRETS", "true")
...
def migrate(
    intellect_config: dict,
    *,
    codex_home: Optional[Path] = None,
    dry_run: bool = False,
    discover_plugins: bool = True,
    default_permission_profile: Optional[str] = ":workspace",
    expose_intellect_tools: bool = True,
) -> MigrationReport:
    """Translate Intellect mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        intellect_config: full ~/.intellect/config.yaml dict
        ...
        expose_intellect_tools: when True (default), register Intellect' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Intellect for tools
    ...
    intellect_servers = (intellect_config or {}).get("mcp_servers") or {}
    if not isinstance(intellect_servers, dict):
        report.errors.append(
            "mcp_servers in Intellect config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in intellect_servers.items():
    ...
    # Inject Intellect' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Intellect for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/intellect_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_intellect_tools:
        translated["intellect-tools"] = _build_intellect_tools_mcp_entry()
        if "intellect-tools" not in report.migrated:
            report.migrated.append("intellect-tools")
