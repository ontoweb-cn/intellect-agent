"""
Intellect Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.intellect/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Intellect PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing intellect-agent or intellect PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Intellect Agent" comment and following line
                if '# Intellect Agent' in line or '# intellect-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('intellect' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing intellect
                if 'intellect' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the intellect wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "intellect",
        Path("/usr/local/bin/intellect"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains intellect_cli reference)
                content = wrapper.read_text()
                if 'intellect_cli' in content or 'intellect-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(intellect_home: Path) -> list:
    ...
    creates::
        ...
        ~/.local/bin/node -> $INTELLECT_HOME/node/bin/node
        ~/.local/bin/npm  -> $INTELLECT_HOME/node/bin/npm
        ~/.local/bin/npx  -> $INTELLECT_HOME/node/bin/npx
    ...
    when the link still resolves into this Intellect home's ``node`` directory.
    A link the user has since repointed at nvm (or anything else outside
    Intellect) is left untouched so we never break unrelated tooling.
    """
    node_dir = (intellect_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``intellect gateway run`` processes
    ...
    try:
        from intellect_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from intellect_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``INTELLECT_HOME`` and ``intellect_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\intellect\git\cmd``, ``%LOCALAPPDATA%\intellect\git\bin``,
#      ``%LOCALAPPDATA%\intellect\git\usr\bin``, ``%LOCALAPPDATA%\intellect\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\intellect\git\`` and Node to
#      ``%LOCALAPPDATA%\intellect\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the gateway path, drops files into
#      ``%LOCALAPPDATA%\intellect\gateway-service\`` and sometimes
...
def _intellect_path_markers(intellect_home: Path) -> list[str]:
    """Path-entry substrings that identify Intellect-owned User-PATH entries."""
    root = str(intellect_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare intellect-agent install dir.
    markers = [root + "\\intellect-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if INTELLECT_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "intellect".  We don't want to catch
    # unrelated entries like "cintellect-foo" or "ephermeral", so we look for
    # backslash-intellect as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(intellect_home: Path) -> list[str]:
    """Strip Intellect-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _intellect_path_markers(intellect_home)
            kept: list[str] = []
...
def remove_intellect_env_vars_windows() -> list[str]:
    """Delete INTELLECT_HOME and intellect_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("INTELLECT_HOME", "intellect_GIT_BASH_PATH"):
                try:
...
def _is_default_intellect_home(intellect_home: Path) -> bool:
    """Return True when ``intellect_home`` points at the default (non-profile) root."""
    try:
        from intellect_constants import get_default_intellect_root
        return intellect_home.resolve() == get_default_intellect_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its INTELLECT_HOME directory.

    We shell out to ``intellect -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current INTELLECT_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m intellect_cli.main` so we don't depend on a `intellect`
    #    wrapper that may be half-removed mid-uninstall.
    intellect_invocation = [_sys.executable, "-m", "intellect_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                intellect_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's INTELLECT_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.intellect/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.intellect/ for future reinstall
    """
    project_root = get_project_root()
    intellect_home = get_intellect_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie INTELLECT_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_intellect_home(intellect_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Intellect Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {intellect_home / 'config.yaml'}")
    print(f"  Secrets: {intellect_home / '.env'}")
    print(f"  Data:    {intellect_home / 'cron/'}, {intellect_home / 'sessions/'}, {intellect_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their INTELLECT_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Intellect data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Intellect code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in intellect_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\intellect\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(intellect_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Intellect-owned PATH entries in User environment")

        log_info("Removing INTELLECT_HOME / intellect_GIT_BASH_PATH User env vars...")
        removed_env = remove_intellect_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Intellect-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing intellect command...")
    ...
    #     (only when they still point into this Intellect home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Intellect-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(intellect_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Intellect-managed node/npm/npx symlinks found")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.intellect/, just remove the intellect-agent subdir
            if intellect_home in project_root.parents or project_root.parent == intellect_home:
    ...
    #     under INTELLECT_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(intellect_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(intellect_home)
    ...
    # 5. Optionally remove ~/.intellect/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile INTELLECT_HOME dirs live under
        ...
        try:
            if intellect_home.exists():
                shutil.rmtree(intellect_home)
                log_success(f"Removed {intellect_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {intellect_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {intellect_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {intellect_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  iex (irm https://raw.giteeusercontent.com/ontoweb/intellect-agent/raw/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.giteeusercontent.com/ontoweb/intellect-agent/raw/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Intellect Agent! ⚕")
    print()
