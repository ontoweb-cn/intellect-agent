"""
Gateway subcommand for intellect CLI.

Handles: intellect gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_intellect_home is imported lazily at call sites to avoid ImportError
# when intellect_constants is cached from a pre-update version during `intellect update`.
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "intellect-gateway*",
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "intellect CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``intellect gateway status`` from falsely counting the ``intellect`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``intellect gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    current_home = str(get_intellect_home().resolve())
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``intellect update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Intellect profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Intellect profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from intellect_cli.profiles import list_profiles
    except Exception:
        return processes
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``intellect update`` users
    # with no running gateway until they re-invoke ``intellect gateway``
...
def _sync_intellect_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``INTELLECT_HOME``.

    Under ``sudo``, ``INTELLECT_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_intellect_home` falls back to ``/root/.intellect`` — the wrong
    profile. The unit file pins ``INTELLECT_HOME`` for the actual gateway
    ...
    unit_home = env.get("INTELLECT_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("INTELLECT_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["INTELLECT_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}intellect gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}intellect gateway restart{scope_flag}")
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `intellect gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``intellect gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from intellect_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (INTELLECT_HOME-scoped).
...
from intellect_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Intellect Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``intellect gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``intellect_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("intellect_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "intellect-gateway"
SERVICE_DESCRIPTION = "Intellect Agent Gateway - Messaging Platform Integration"
...
def _profile_arg(intellect_home: str | None = None) -> str:
    ...
    Args:
        intellect_home: Optional explicit INTELLECT_HOME path. Defaults to the current
            ``get_intellect_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from intellect_constants import get_default_intellect_root
    home = Path(intellect_home or str(get_intellect_home())).resolve()
    default = get_default_intellect_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this INTELLECT_HOME.

    Default ``~/.intellect`` returns ``intellect-gateway`` (backward compatible).
    Profile ``~/.intellect/profiles/coder`` returns ``intellect-gateway-coder``.
    Any other INTELLECT_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Intellect installs that predate the
# intellect-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (intellect-gateway-*.service) and unrelated third-party
# "intellect" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("intellect.service",)
...
def _find_legacy_intellect_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Intellect gateway units.

    Detects unit files installed by older Intellect versions that used a
    different service name (e.g. ``intellect.service`` before the rename to
    ``intellect-gateway.service``). When both a legacy unit and the current
    ``intellect-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``intellect-gateway-coder.service`` and unrelated third-party
      ``intellect-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``intellect.service`` running an unrelated
...
def has_legacy_intellect_units() -> bool:
    """Return True when any legacy Intellect gateway unit files exist."""
    return bool(_find_legacy_intellect_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Intellect gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_intellect_units()
    if not legacy:
        return
    print_warning("Legacy Intellect gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current intellect-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    intellect gateway migrate-legacy")


def remove_legacy_intellect_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Intellect gateway unit files.

    Iterates over whatever ``_find_legacy_intellect_units()`` returns — which is
    ...
    legacy = _find_legacy_intellect_units()
    if not legacy:
        print("No legacy Intellect gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Intellect gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: intellect gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo intellect gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    intellect gateway uninstall")
    print_info("    sudo intellect gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning("  System service install requires sudo, so Intellect can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo intellect gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo intellect gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo intellect gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Intellect often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.intellect`` → ``ai.intellect.gateway.plist`` (backward compatible).
    Profile ``~/.intellect/profiles/coder`` → ``ai.intellect.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.intellect.gateway-{suffix}" if suffix else "ai.intellect.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.intellect/intellect-agent  -> /home/alice/.intellect/intellect-agent
      /opt/intellect                 -> /opt/intellect  (kept as-is)
...
def _intellect_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current INTELLECT_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_intellect_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.intellect                    → /home/alice/.intellect
      /root/.intellect/profiles/coder     → /home/alice/.intellect/profiles/coder
      /opt/custom-intellect               → /opt/custom-intellect  (kept as-is)
    """
    current_intellect = get_intellect_home().resolve()
    current_default = (Path.home() / ".intellect").resolve()
    target_default = Path(target_home_dir) / ".intellect"

    # Default ~/.intellect → remap to target user's default
    if current_intellect == current_default:
        return str(target_default)

    # Profile or subdir of ~/.intellect → preserve the relative structure
    try:
        relative = current_intellect.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.intellect) — keep as-is
        return str(current_intellect)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    intellect_node = intellect_home / "node" / "bin"
    if _is_dir(intellect_node):
        candidates.append(str(intellect_node))
    intellect_nm = intellect_home / "node_modules" / ".bin"
    if _is_dir(intellect_nm):
        candidates.append(str(intellect_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m intellect_cli.main``, so module
    ...
    dir, or a clone that ``intellect update`` later relocates/removes — the path
    ...
    ``INTELLECT_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if INTELLECT_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        intellect_home = _intellect_home_for_target_user(home_dir)
        profile_arg = _profile_arg(intellect_home)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's INTELLECT_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(intellect_home) if intellect_home else _remap_path_for_user(working_dir, home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m intellect_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="INTELLECT_HOME={intellect_home}"
Restart=always
RestartSec=5
...
"""
    ...
    intellect_home = str(get_intellect_home().resolve())
    profile_arg = _profile_arg(intellect_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only INTELLECT_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp INTELLECT_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../intellect_test`` path into
    # ``Environment="INTELLECT_HOME=..."``. Writing that to the developer's
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Intellect install")
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo intellect gateway uninstall --system")
    print_info("         intellect gateway install")
    print_info("         intellect gateway start")
...
def systemd_install(
    ...
    # Offer to remove legacy units (intellect.service from pre-rename installs)
    # before installing the new intellect-gateway.service. If both remain, they
    ...
    if has_legacy_intellect_units():
        print()
        print_legacy_unit_warning()
        print()
        if prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_intellect_units(interactive=False)
            print()
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}intellect gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}intellect gateway status{scope_flag}             # Check status")
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}intellect gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    _sync_intellect_home_from_systemd_unit(system=system)
    try:
    ...
    try:
        ...
        print(
            ...
            "check `intellect gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `intellect gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}intellect gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}intellect gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {'sudo ' if system else ''}intellect gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}intellect gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.intellect.gateway-{suffix}" if suffix else "ai.intellect.gateway"
...
def generate_launchd_plist() -> str:
    ...
    working_dir = _stable_service_working_dir()
    intellect_home = str(get_intellect_home().resolve())
    log_dir = get_intellect_home() / "logs"
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>intellect_cli.main</string>",
    ]
    if profile_arg:
...
<dict>
    ...
    <dict>
        ...
        <key>INTELLECT_HOME</key>
        <string>{intellect_home}</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Intellect install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  intellect gateway status             # Check status")
...
def launchd_stop():
    ...
    # `intellect gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Intellect install")
    else:
        print("⚠ Service definition is stale relative to the current Intellect install")
        print("  Run: intellect gateway start")
    ...
    if deep:
        log_file = get_intellect_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _is_official_docker_checkout() -> bool:
    return (
        str(PROJECT_ROOT) == "/opt/intellect"
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("intellect_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Intellect gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'intellect' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/intellect/docker/entrypoint.sh before the Intellect command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$INTELLECT_HOME and break later non-root gateway runs."
    )
    print("  Set intellect_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk.")
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `intellect gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set intellect_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `intellect gateway restart` which already
    ...
    # `intellect gateway start/restart` — leaving the gateway vulnerable to
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ Intellect Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
    ...
    # intellect_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("intellect_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from intellect_constants import get_intellect_home as _ghh
            log_dir = _ghh() / "logs"
            log_dir.mkdir(parents=True, exist_ok=True)
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Intellect will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @intellect:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. intellect) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Intellect delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Intellect agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Intellect will use (e.g., intellect@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Intellect connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: intellect pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Intellect will connect automatically over WebSocket",
        ],
        "vars": [
...
def _all_platforms() -> list[dict]:
    ...
    ``intellect setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run intellect under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.intellect/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_intellect_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'intellect pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: intellect pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'intellect gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from intellect_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from intellect_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Intellect will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. Intellect will store the returned account_id/token in ~/.intellect/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `intellect gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_intellect_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `intellect pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `intellect pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"IntellectAgent\"")
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``intellect_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from intellect_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.intellect/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.intellect/.env: {', '.join(required)}")
    else:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: intellect gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: intellect gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: intellect gateway install")
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo intellect gateway install --system")
                    print_info("  Or run in foreground:  intellect gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: intellect gateway run")
                print_info("  For persistence:   tmux new -s intellect 'intellect gateway run'")
                print_info("  To enable systemd: add systemd=true to /etc/wsl.conf, then 'wsl --shutdown'")
            elif is_termux():
                from intellect_constants import display_intellect_home as _dhh
                print_info("  Termux does not use systemd/launchd services.")
                print_info("  Run in foreground: intellect gateway run")
                print_info(f"  Or start it manually in the background (best effort): nohup intellect gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
            else:
                print_info("  Service install not supported on this platform.")
                print_info("  Run in foreground: intellect gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'intellect gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`intellect_cli.service_manager` are caught and surfaced as
    ...
    if profile is None:
        ...
        # INTELLECT_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``intellect gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``intellect gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    (auto-restart on crash, multiple profile gateways under the same
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``intellect gateway run`` are unaffected.
      2. ``intellect_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``intellect gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``intellect_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("intellect_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("intellect_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${INTELLECT_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  Use `--no-supervise` (or intellect_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    # gateway flaps. `sleep infinity` matches the static main-intellect
    # service's pattern (see docker/s6-rc.d/main-intellect/run): the CMD
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: intellect gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: intellect gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s intellect 'intellect gateway run'")
                print()
            ...
            print()
            print("  intellect gateway run                              # direct foreground")
            print("  tmux new -s intellect 'intellect gateway run'         # persistent via tmux")
            print("  nohup intellect gateway run > ~/.intellect/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  intellect profile create <name>     # creates the s6 service slot")
                print("  intellect -p <name> gateway start   # bring it up via s6")
                print("  intellect status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: intellect gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: intellect gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: intellect gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  intellect profile delete <name>     # tears down the s6 service slot")
                print("  intellect -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `intellect -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: intellect gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if supports_systemd_services():
                ...
                if linger_ok is not True:
                    ...
                    print()
                    print("  Then restart the gateway:")
                    print("    intellect gateway restart")
                    return
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: intellect gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  intellect gateway install")
                else:
                    print("To install as a service:")
                    print("  intellect gateway install")
                    print("  sudo intellect gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  intellect gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup intellect gateway run > ~/.intellect/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s intellect 'intellect gateway run'         # persistent via tmux")
                    print("  nohup intellect gateway run > ~/.intellect/logs/gateway.log 2>&1 &  # background")
                elif is_windows():
                    print("  intellect gateway install  # Install as Windows Scheduled Task (auto-start on login)")
                else:
                    print("  intellect gateway install  # Install as user service")
                    print("  sudo intellect gateway install --system  # Install as boot-time system service")
        ...
        # Stop, disable, and remove legacy Intellect gateway unit files from
        # pre-rename installs (e.g. intellect.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_intellect_units(interactive=not yes, dry_run=dry_run)
