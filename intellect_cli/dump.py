"""
Dump command for intellect CLI.

Outputs a compact, plain-text summary of the user's Intellect setup
...
from intellect_cli.config import get_intellect_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.intellect_build_sha`` by the Dockerfile's
    ``INTELLECT_GIT_SHA`` build-arg (see ``intellect_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``intellect dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from intellect_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(intellect_home: Path) -> int:
...
def _cron_summary(intellect_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = intellect_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    load_intellect_dotenv(
        intellect_home=env_path.parent,
    ...
    lines.append(f"intellect_home:      {display_intellect_home()}")
    ...
    toolsets = config.get("toolsets", ["intellect-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(intellect_home)}")
    lines.append(f"  skills:             {_count_skills(intellect_home)}")
