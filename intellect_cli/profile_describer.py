...
Used by ``intellect profile describe <name> --auto``. Reads the profile's
installed skills, model+provider, name, and optionally a small slice of
memory, then asks the auxiliary LLM to produce a 1-2 sentence description
of what the profile is good at.

Result is written to ``<profile_dir>/profile.yaml`` with
``description_auto: true`` so UIs can surface a "review" badge. User can
edit afterward to confirm.

Design notes
------------
- Mirrors the shape of ``intellect_cli/kanban_specify.py``: lazy aux
...
from intellect_cli import profiles as profiles_mod
...
_SYSTEM_PROMPT = """You are a profile-describer for the Intellect Agent kanban board.
...
Rules:
  ...
  - Never write "Intellect Agent profile" or other meta-narration.
...
def describe_profile(
    ...
    *,
    ...
    try:
        if canon == "default":
            from intellect_constants import get_intellect_home  # type: ignore
            profile_dir = Path(get_intellect_home())
        else:
