...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or intellect -c.",
    ...
    "Ctrl+Z suspends Intellect to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "intellect -c resumes your most recent CLI session. intellect -c \"project name\" resumes by title.",
    "intellect -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "intellect -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "intellect chat -t web,terminal enables only specific toolsets for a focused session.",
    "intellect chat -s github-pr-workflow preloads a skill at launch.",
    "intellect chat -q \"query\" runs a single non-interactive query and exits.",
    "intellect chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "intellect chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "intellect --yolo bypasses all dangerous command approval prompts for the entire session.",
    "intellect chat --source telegram tags the session for filtering in intellect sessions list.",
    "intellect -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "intellect doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "intellect dump outputs a compact setup summary — great for bug reports.",
    "intellect config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "intellect config edit opens config.yaml in your default editor.",
    "intellect config check scans for missing or stale configuration options.",
    "intellect sessions browse opens an interactive session picker with search.",
    "intellect sessions stats shows session counts by platform and database size.",
    "intellect sessions prune --older-than 30 cleans up old sessions.",
    "intellect skills search react --source skills-sh searches the skills.sh public directory.",
    "intellect skills check scans installed hub skills for upstream updates.",
    "intellect skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "intellect skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "intellect mcp add github --command npx adds MCP servers from the command line.",
    "intellect mcp serve runs Intellect itself as an MCP server for other agents.",
    "intellect auth add lets you add multiple API keys for credential pool rotation.",
    "intellect completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "intellect logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "intellect backup creates a zip backup of your entire Intellect home directory.",
    "intellect profile create coder creates an isolated profile that becomes its own command.",
    "intellect profile create work --clone copies your current config and keys to a new profile.",
    "intellect update syncs new bundled skills to ALL profiles automatically.",
    "intellect gateway install sets up Intellect as a system service (systemd/launchd).",
    "intellect memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "intellect webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: intellect tools disables unused tools, intellect skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "intellect models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as intellect -w).",
    ...
    "INTELLECT_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Intellect tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'intellect profile create coder' creates the 'coder' command.",
    "intellect profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Intellect prints a resume command with session ID and stats.",
    "intellect sessions export backup.jsonl exports all sessions for backup or analysis.",
    "intellect -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "intellect skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .intellect/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: intellect cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Intellect runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "intellect gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Intellect via a local macOS server.",
    ...
    "Context files (.intellect.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Intellect loads project context from .intellect.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "intellect mcp opens an interactive picker of ONTOWEB-approved MCPs you can install in one keystroke.",
    "intellect mcp catalog lists ONTOWEB-approved MCP servers shipped with the repo.",
    "intellect mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — intellect tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.intellect/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "intellect chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "intellect plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "intellect acp runs Intellect as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "INTELLECT_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "intellect auth add ontoweb or intellect auth add openai-codex sets up OAuth-based providers.",
    ...
    "intellect status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.intellect/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Intellect your own.",
    ...
    "Before a gateway session reset, Intellect auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.intellect/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at INTELLECT_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "INTELLECT_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in INTELLECT_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.intellect/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'intellect profile create ls' would be rejected.",
    "intellect profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "intellect claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "intellect_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via intellect tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.intellect/.env.",
    "Skills from trusted repos (ONTOWEB) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.intellect/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Intellect auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Intellect config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Intellect is working.',
    ...
    '/reload reloads ~/.intellect/.env into the running session — pick up new API keys without restarting.',

    # --- Multi-User: Members, Teams & Projects ---
    'intellect members bootstrap --admin alice --team core --project web-app creates a full multi-user environment in one shot.',
    'intellect members create bob --login bob --email bob@example.com adds a member and prints an invite code.',
    'intellect members invite --login charlie --email charlie@example.com generates a redeemable invite code.',
    'intellect members login alice sets the active CLI member — subsequent /team and /project commands scope to that member.',
    'intellect members list shows all members; intellect members show <id> displays identities, teams, and projects.',
    '/team <team_id> or /teams sets and lists your teams from chat — sticky per session, scoped to your member.',
    '/project <project_id> or /projects sets and lists your projects from chat — the agent then auto-scopes its work.',
    '/join <team_id> requests membership in a team — team admins approve via insight members bind or the dashboard.',
    'intellect members bind alice --provider github links a GitHub OAuth identity to a member for portal auth.',
    'Teams are who (collaboration groups), projects are what (work contexts) — link teams to projects for full scoping.',
    'INTELLECT_TENANT=<name> pins a namespace shared by kanban boards, cron jobs, and the multi-user dashboard.',
    ...
    "intellect_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.intellect/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.intellect/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'intellect curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "intellect curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'intellect curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'intellect auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    'credential_pool_strategies.<provider>: round_robin cycles keys evenly instead of the fill_first default.',
    'use_gateway: true per-tool routes web, image, tts, or browser through your OntoWeb subscription — no extra keys.',
    ...
    # --- TUI ---
    'intellect_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "INTELLECT_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'intellect_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'intellect_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'INTELLECT_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'INTELLECT_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.intellect/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.intellect/skills/ so drop-in skills appear without restarting the session.',
    ...
    'intellect -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'intellect chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'intellect chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'intellect chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "intellect chat --source tool tags programmatic chats so they don't clutter intellect sessions list.",
    'intellect dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'intellect sessions rename <ID> "new title" renames any past session; intellect sessions delete <ID> removes one.',
    'intellect import restores a session export or profile archive produced by sessions export or profile export.',
    'intellect fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'intellect pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'intellect setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'intellect status --deep runs the full health sweep across every component; plain intellect status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'intellect_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'intellect_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.intellect/plugins/ — trust-gated by design.',
    "intellect_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'intellect_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'INTELLECT_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'INTELLECT_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'intellect_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'intellect_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'intellect_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'intellect_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'intellect_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'intellect_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'intellect_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Intellect block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from intellect sessions list by default — set --source explicitly to see them.',
