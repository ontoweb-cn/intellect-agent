...
(the stage2 hook) has chowned the volume and seeded $INTELLECT_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$INTELLECT_HOME``,
    not under ``profiles/``). The dispatcher in ``intellect_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``intellect gateway start`` (no ``-p``) targets. Without it,
    bare ``intellect gateway start`` inside the container would land on
    ...
    ``$INTELLECT_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        intellect_home: The container's INTELLECT_HOME (typically /opt/data).
            Profiles live under ``<intellect_home>/profiles/<name>/``;
            the default profile lives at ``<intellect_home>`` itself.
    ...
    # named profiles).
    default_prior_state = _read_prior_state(intellect_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(intellect_home)
    ...
    if not dry_run:
        _write_reconcile_log(intellect_home, actions)
    return actions
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `intellect -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit intellect
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the intellect user) won't hit EACCES. See
...
def _write_reconcile_log(
    intellect_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $INTELLECT_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    intellect_home = Path(os.environ.get("INTELLECT_HOME", "/opt/data"))
    scandir = Path(os.environ.get("S6_PROFILE_GATEWAY_SCANDIR", "/run/service"))
    actions = reconcile_profile_gateways(
        intellect_home=intellect_home, scandir=scandir,
    )
    for a in actions:
        print(
