...
DEFAULT_CODEX_MODELS: List[str] = [
    ...
    # in intellect_cli/models.py — only the openai-codex (OAuth) provider
    ...
    # Intellect uses. Removed here. If OpenAI re-enables them on Codex backend,
...
_FORWARD_COMPAT_TEMPLATE_MODELS: List[tuple[str, tuple[str, ...]]] = [
    ...
    # entitlement; Intellect does not.
...
def _read_cache_models(codex_home: Path) -> List[str]:
    ...
    if isinstance(entries, list):
        for item in entries:
            ...
            # public OpenAI API, while Intellect openai-codex talks to the same
