"""``intellect slack ...`` CLI subcommands.

Today only ``intellect slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ intellect slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(bot_name: str, bot_description: str) -> dict:
    ...
    it stays in sync with the rest of Intellect. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for an Intellect deployment — users can tweak them in the Slack UI after
    pasting.
    """
    from intellect_cli.commands import slack_app_manifest
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your Intellect agent on Slack")[:140],
        ...
        "features": {
            ...
            "assistant_view": {
                "assistant_description": "Chat with Intellect in threads and DMs.",
            },
        },
        "oauth_config": {
            "scopes": {
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``intellect_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$INTELLECT_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Intellect")
      --description DESC  Override the bot description
      --slashes-only  Emit only the ``features.slash_commands`` array (for
                      merging into an existing manifest manually)
    """
    name = getattr(args, "name", None) or "Intellect"
    description = getattr(args, "description", None) or "Your Intellect agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("INTELLECT_HOME") or str(Path.home() / ".intellect")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Intellect app\n"
