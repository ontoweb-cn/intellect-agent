...
Usage:
    intellect pairing list              # Show all pending + approved users
    intellect pairing approve <platform> <code>  # Approve a pairing code
    intellect pairing revoke <platform> <user_id> # Revoke user access
    intellect pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle intellect pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: intellect pairing {list|approve|revoke|clear-pending}")
        print("Run 'intellect pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.intellect/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'intellect pairing list' to see pending codes.\n")
