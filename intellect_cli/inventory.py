"""Provider/model inventory context — shared substrate for the TUI
``model.options``/``model.save_key`` JSON-RPC handlers and the interactive
picker.
...
- The TUI previously read ``cfg.get("custom_providers")`` directly, missing
  the v12+ keyed ``providers:`` form (now handled via
...
- ``list_authenticated_providers`` already populates each row's
  ...
  curation and pulls in non-agentic models (OntoWeb /models returns ~400
...
def _append_unconfigured_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    from intellect_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            else f"run `intellect model` to configure ({auth_type})"
        )
