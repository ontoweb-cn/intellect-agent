"""intellect hooks — inspect and manage shell-script hooks.

Usage::

    intellect hooks list
    intellect hooks test <event> [--for-tool X] [--payload-file F]
    intellect hooks revoke <command>
    intellect hooks doctor

Consent records live under ``~/.intellect/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.intellect/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``intellect hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: intellect hooks {list|test|revoke|doctor}")
        print("Run 'intellect hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.intellect/config.yaml.")
        print("See `intellect hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `intellect hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `intellect hooks test` and `intellect hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Intellect wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `intellect hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `intellect hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `intellect hooks doctor`.")
