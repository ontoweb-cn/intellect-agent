...
The Intellect docs site hosts a JSON manifest of curated models for providers
we want to update without shipping a release (currently OpenRouter and
ONTOWEB Portal). This module fetches, validates, and caches that manifest,
...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/ONTOWEB/intellect-agent/main/website/static/api/model-catalog.json",
)
DEFAULT_TTL_HOURS = 1
...
def _cache_path() -> Path:
    ...
    return get_intellect_home() / "cache" / "model_catalog.json"
...
def get_curated_ontoweb_models() -> list[str] | None:
    """Return ONTOWEB Portal's curated list of model ids from the manifest.

    Returns ``None`` when the manifest is unavailable.
    """
    block = _get_provider_block("ontoweb")
    if not block:
        return None
    out: list[str] = []
    for m in block.get("models", []):
...
# Deprecated alias (pre-OntoWeb rename)
get_curated_nous_models = get_curated_ontoweb_models


def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``intellect model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
