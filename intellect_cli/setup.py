"""
Interactive setup wizard for Intellect Agent.
...
from intellect_cli.ontoweb_subscription import get_ontoweb_subscription_features
...
_DOCS_BASE = "https://intellect-agent.ontoweb.cn/docs"
...
# display_intellect_home imported lazily at call sites (stale-module safety during intellect update)
...
from intellect_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from intellect_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Intellect Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Intellect using environment variables or config commands:")
    print_info("  intellect config set model.provider custom")
    print_info("  intellect config set model.base_url http://localhost:8080/v1")
    print_info("  intellect config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'intellect setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from intellect_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'intellect setup')")


def _seed_oauth_builtin_catalog() -> dict:
    """Load built-in OAuth providers into state.db (idempotent)."""
    try:
        from agent.oauth.builtin_catalog import (
        ...
        db = SessionDB()
        try:
            stats = seed_builtin_oauth_providers(db._conn.cursor())
            db._conn.commit()
            catalog_id = stats.get("catalog_id") or ""
            if catalog_id:
                write_catalog_marker(catalog_id)
            return stats
        finally:
            db.close()
    except Exception as exc:
        logger.debug("oauth builtin catalog seed failed: %s", exc)
        return {}


def _print_setup_summary(config: dict, intellect_home, oauth_seed_stats: dict | None = None):
    """Print the setup completion summary."""
    if oauth_seed_stats:
        total = oauth_seed_stats.get("total")
        inserted = oauth_seed_stats.get("inserted", 0)
        if total:
            print_info(
                f"OAuth: {total} built-in providers in state.db"
                + (f" ({inserted} newly added)" if inserted else "")
            )
    ...
    tool_status = []
    subscription_features = get_ontoweb_subscription_features(config)
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'intellect setup' to configure"))
    ...
    if subscription_features.web.managed_by_nous:
        tool_status.append(("Web Search & Extract (OntoWeb subscription)", True, None))
    ...
    if subscription_features.browser.managed_by_nous:
        tool_status.append(("Browser Automation (OntoWeb Browser Use)", True, None))
    ...
    # Image generation — FAL (direct or via OntoWeb), or any plugin-registered
    # provider (OpenAI, etc.)
    if subscription_features.image_gen.managed_by_nous:
        tool_status.append(("Image Generation (OntoWeb subscription)", True, None))
        ...
        try:
            ...
            from intellect_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `intellect tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    if subscription_features.video_gen.managed_by_nous:
        tool_status.append(("Video Generation (FAL via OntoWeb subscription)", True, None))
    else:
        try:
            from agent.video_gen_registry import list_providers as _list_video_providers
            from intellect_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        tool_status.append(("Text-to-Speech (OpenAI via OntoWeb subscription)", True, None))
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'intellect setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'intellect setup tts'"))
    else:
        tool_status.append(("Text-to-Speech (Edge TTS)", True, None))

    if subscription_features.modal.managed_by_nous:
        tool_status.append(("Modal Execution (OntoWeb subscription)", True, None))
    elif cfg_get(config, "terminal", "backend") == "modal":
        if subscription_features.modal.direct_override:
            tool_status.append(("Modal Execution (direct Modal)", True, None))
        else:
            tool_status.append(("Modal Execution", False, "run 'intellect setup terminal'"))
    elif managed_ontoweb_tools_enabled() and subscription_features.ontoweb_auth_present:
        tool_status.append(("Modal Execution (optional via OntoWeb subscription)", True, None))

    # Home Assistant
    if get_env_value("HASS_TOKEN"):
        tool_status.append(("Smart Home (Home Assistant)", True, None))

    # Spotify (OAuth vian intellect auth spotify — check auth.json, not env vars)
    try:
        from intellect_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'intellect setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {intellect_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('intellect setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('intellect setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('intellect setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('intellect setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('intellect setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('intellect config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('intellect config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('intellect config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('intellect', Colors.GREEN)}              Start chatting")
    print(f"   {color('intellect gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('intellect doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `intellect tools` and `intellect setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``intellect model``)
    ...
    provider added to ``intellect model`` is automatically available here.
    ...
    # Delegate to the shared intellect model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from intellect_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: intellect model")
    ...
    # on demand via `intellect auth add`, `intellect setup` vision, and
    # `intellect setup tts`. This keeps both quick and full setup thin.

    # Tool Gateway prompt is already shown by _model_flow_ontoweb() above.
    save_config(config)
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``intellect model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def _setup_tts_provider(config: dict):
    ...
    choices = []
    providers = []
    if managed_ontoweb_tools_enabled() and subscription_features.ontoweb_auth_present:
        choices.append("OntoWeb Subscription (managed OpenAI TTS, billed to your subscription)")
        providers.append("ontoweb-openai")
    choices.extend(
        [
    ...
    selected_via_nous = selected == "ontoweb-openai"
    if selected == "ontoweb-openai":
        selected = "openai"
        print_info("OpenAI TTS will use the managed OntoWeb gateway and bill to your subscription.")
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.intellect/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Intellect auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via intellect setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'intellect setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Intellect runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `intellect setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `intellect setup terminal`.
        ...
        managed_modal_available = bool(
            ...
            and
            get_ontoweb_subscription_features(config).ontoweb_auth_present
        ...
        if use_managed_modal:
            ...
            print_info("Modal execution will use the managed OntoWeb gateway and bill to your subscription.")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into INTELLECT_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("INTELLECT_MAX_ITERATIONS")
    ...
    print_info("  Run `intellect setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # INTELLECT_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Intellect delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `intellect update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://intellect-agent.ontoweb.cn/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under INTELLECT_HOME, and print
    ...
    """
    try:
        from intellect_cli.slack_cli import _build_full_manifest
        from intellect_constants import get_intellect_home

        manifest = _build_full_manifest(
            bot_name="Intellect",
            bot_description="Your Intellect agent on Slack",
        )
        target = Path(get_intellect_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `intellect slack manifest --write` anytime to refresh after "
            "Intellect adds new commands."
        )
        ...
        print_info(
            ...
            "intellect slack manifest --write"
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Intellect delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Intellect to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from intellect_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://intellect-agent.ontoweb.cn/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://intellect-agent.ontoweb.cn/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  intellect config edit")
    print_info("   Open config in your editor:  intellect config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from intellect_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Intellect from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'intellect setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     intellect config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_intellect_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  intellect gateway")
        else:
            from intellect_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   intellect gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   intellect gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `intellect setup tools` and `intellect tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``intellect_cli.auth`` — lists every supported
        provider along with its ``api_key_env_vars``.
      * ``active_provider`` in the auth store — covers OAuth device-code /
        external-OAuth providers (OntoWeb, Codex, Qwen, Gemini CLI, ...).
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from intellect_cli.config
    ...
    if section_key == "model":
        ...
        from intellect_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_intellect.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_intellect migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_intellect", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Intellect.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Intellect to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Intellect at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Intellect at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Intellect at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Intellect at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Intellect equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Intellect config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Intellect.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Intellect's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(intellect_home: Path) -> bool:
    ...
    print_info("Intellect can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: intellect claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=intellect_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: intellect claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Intellect configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Intellect config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Intellect (use intellect claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot ONTOWEB Portal setup — OAuth + provider switch + Tool Gateway.

    Wired into ``intellect setup --portal``. Does NOT prompt for anything
    besides what the underlying OAuth + Tool Gateway prompts already need.
    Designed to be shareable as a single command (``intellect setup --portal``)
    that gets a brand-new user from zero to a fully working Intellect session
    with web/image/tts/browser tools all routed via their Portal sub.
    """
    from types import SimpleNamespace

    from intellect_cli.auth_commands import auth_add_command
    from intellect_cli.config import save_config
    from intellect_cli.auth import get_ontoweb_auth_status
    ...
    print(color("│     ⚕ Intellect Setup — ONTOWEB Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    print_info("    — all routed through your ONTOWEB Portal sub.")
    print()
    print_info("  Sign up: https://portal.ontoweb.cn/manage-subscription")
    print()

    # Skip OAuth if already logged in (don't re-prompt every time the user
    # runs `intellect setup --portal` after a successful first run).
    already_logged_in = False
    try:
        already_logged_in = bool((get_ontoweb_auth_status() or {}).get("logged_in"))
    except Exception:
        already_logged_in = False

    if already_logged_in:
        print_success("  Already logged into ONTOWEB Portal.")
    else:
        # Hand off to the shared auth wiring so the device-code flow is
        # identical to `intellect auth add ontoweb --type oauth`. SimpleNamespace
        ...
        try:
            ...
            print()
            print_error(f"  ONTOWEB Portal login failed (exit {e.code}).")
            print_info("  You can retry later with `intellect auth add ontoweb --type oauth`.")
            return
        except (KeyboardInterrupt, EOFError):
            print()
            ...
            print()
            print_error(f"  ONTOWEB Portal login failed: {exc}")
            print_info("  You can retry later with `intellect auth add ontoweb --type oauth`.")
            return

    # Set provider → ontoweb so the model picker, status surfaces, and
    # managed-tool gating all light up. Leave model.model empty so the
    # runtime picks OntoWeb's default model; the user can change it later
    # with `intellect model`.
    ...
    save_config(config)
    print()
    print_success("  OntoWeb set as your inference provider.")

    # Offer the Tool Gateway opt-in (single Y/n) — same flow that fires
    # from `intellect model` after picking OntoWeb.
    print()
    try:
    ...
    print_info("  Run `intellect portal status` to inspect routing.")
    print_info("  Run `intellect` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      intellect setup           — full or quick (auto-detected)
      intellect setup model     — just model/provider
      intellect setup tts       — just text-to-speech
      intellect setup terminal  — just terminal backend
      intellect setup gateway   — just messaging platforms
      intellect setup tools     — just tool configuration
      intellect setup agent     — just agent settings
    """
    from intellect_cli.config import is_managed, managed_error
    if is_managed():
        managed_error("run setup wizard")
        return
    ensure_intellect_home()
    oauth_seed_stats = _seed_oauth_builtin_catalog()
    ...
    # --portal: one-shot ONTOWEB Portal setup. Skips the rest of the wizard.
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Intellect Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Intellect Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Intellect Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, intellect_home, oauth_seed_stats)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Intellect configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'intellect setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(intellect_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up Intellect?", [
            "Quick Setup (OntoWeb Portal) — OAuth login, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, intellect_home, is_existing, oauth_seed_stats)
            return
    ...
    print_info(f"Data folder:  {intellect_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'intellect config edit'")
    ...
    # Tune later with `intellect setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, intellect_home, oauth_seed_stats)


def _run_first_time_quick_setup(config: dict, intellect_home, is_existing: bool, oauth_seed_stats=None):
    """Streamlined first-time setup via OntoWeb Portal: OAuth, model, terminal & messaging.

    Routes straight to the OntoWeb Portal provider — runs the device-code OAuth
    login, picks a OntoWeb model, then configures the terminal backend and (optionally)
    a messaging platform. Applies sensible defaults for everything else (agent
    settings, tools); the user can customize later via ``intellect setup <section>``
    or switch providers with ``intellect model``.
    """
    from intellect_cli.config import load_config

    # Step 1: OntoWeb Portal — OAuth login + model selection.
    # _model_flow_ontoweb() handles both the logged-out path (device-code OAuth,
    # which selects a model internally) and the already-logged-in path (curated
    # OntoWeb model picker). Provider is set to "ontoweb" by the login/model save.
    print()
    print_header("OntoWeb Portal")
    print_info("One subscription, 300+ models, plus the Tool Gateway:")
    print_info("  web search, image generation, TTS, browser automation.")
    print_info("Sign up: https://portal.ontoweb.cn/manage-subscription")
    print()
    try:
        from intellect_cli.main import _model_flow_ontoweb
        _model_flow_ontoweb(config)
    except (KeyboardInterrupt, EOFError):
        print()
        print_info("OntoWeb Portal setup cancelled.")
    except Exception as exc:
        logger.debug("_model_flow_ontoweb error during quick setup: %s", exc)
        print_warning(f"OntoWeb Portal setup encountered an error: {exc}")
        print_info("You can try again later with: intellect model")

    # Re-sync the wizard's config dict from disk — _model_flow_ontoweb (and the
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'intellect setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    intellect setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  intellect setup gateway")
    print()

    _print_setup_summary(config, intellect_home, oauth_seed_stats)


def _run_quick_setup(config: dict, intellect_home, oauth_seed_stats: dict | None = None):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'intellect setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Intellect to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'intellect setup gateway'.")
