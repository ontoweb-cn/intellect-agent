"""
Baked-in build metadata for Intellect Agent.

Source installs report their git revision live via ``git rev-parse`` (see
``intellect_cli/dump.py`` and ``intellect_cli/banner.py``).  That doesn't work inside
...
To make ``intellect dump`` and the startup banner identify the exact commit the
image was built from, the Docker build writes the build-time ``$INTELLECT_GIT_SHA``
arg into ``<project_root>/.intellect_build_sha``.  This module is the single
...
- Returns ``None`` when the file is absent.  Source installs and dev images
  built without the ``INTELLECT_GIT_SHA`` build-arg fall through to live-git
...
_BUILD_SHA_FILE = Path(__file__).parent.parent / ".intellect_build_sha"


def get_build_sha(short: int = 8) -> Optional[str]:
    """Return the baked-in build SHA, truncated to ``short`` chars, or None.

    Reads ``<project_root>/.intellect_build_sha`` if present.  The file is
    written by the Dockerfile's ``INTELLECT_GIT_SHA`` build-arg and contains
