...
# xAI's public API is OpenAI-compatible for the endpoints Intellect commonly
# uses. The Responses endpoint is included because Intellect' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok vian Intellect-managed OAuth credentials."""

    auth_hint = "intellect auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`intellect auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`intellect auth reset xai-oauth` or re-authenticate with "
                    "`intellect auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `intellect auth add xai-oauth --type oauth`."
            )
