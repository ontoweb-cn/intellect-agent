"""CLI handlers for the ``intellect proxy`` subcommand."""
...
from intellect_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "intellect proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    provider = getattr(args, "provider", None) or "ontoweb"
    try:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"intellect auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Intellect proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Intellect proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: intellect proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``intellect proxy <subcommand>``."""
    ...
    print(
        "intellect proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  intellect proxy start [--provider ontoweb|xai] [--host 127.0.0.1] [--port 8645]\n"
