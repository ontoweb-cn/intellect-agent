...
- ``intellect_cli.providers``        -- canonical provider identity + overlays
- ``intellect_cli.model_normalize``  -- per-provider name formatting
...
_intellect_MODEL_WARNING = (
    "ONTOWEB intellect 3 & 4 models are NOT agentic and are not designed "
    "for use with Intellect Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real ONTOWEB intellect 3 / Intellect 4 chat families.
# The previous substring check (`"intellect" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``intellect-brain:qwen3-14b-ctx16k`` that just
# happen to carry "intellect" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   ONTOWEB/Intellect-3-Llama-3.1-70B, intellect-4-405b, openrouter/intellect3:70b
# Negative examples it must NOT match:
#   intellect-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_intellect_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])intellect[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_ontoweb_intellect_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Ontoweb Intellect 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_intellect_NON_AGENTIC_RE.search(model_name))


def _check_intellect_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Ontoweb Intellect 3/4 chat model."""
    if is_ontoweb_intellect_non_agentic(model_name):
        return _intellect_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``intellect config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from intellect_cli.model_switch import
...
def _resolve_alias_fallback(
    ...
    Falls back to ``("openrouter", "ontoweb")`` only when no authenticated
    providers are supplied (backwards compat for non-interactive callers).
    """
    providers = authenticated_providers or ("openrouter", "ontoweb")
...
def resolve_display_context_length(
    ...
    about Codex OAuth, Copilot, OntoWeb, and falls back to models.dev for the
    rest.
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'intellect model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'intellect doctor' — config issues detected:"
    ...
    # Mirrors the same logic in intellect_cli.runtime_provider.resolve_runtime_provider;
    ...
    intellect_warn = _check_intellect_model_warning(new_model)
    if intellect_warn:
        warnings.append(intellect_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from intellect_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from intellect_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by intellect provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "ontoweb" pulls from the remote model-catalog manifest published at
    # https://intellect-agent.ontoweb.cn/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring an Intellect release. Falls back to the in-repo
    # _PROVIDER_MODELS["ontoweb"] snapshot when the manifest is unreachable.
    curated["ontoweb"] = get_curated_ontoweb_model_ids()
    ...
    for intellect_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(intellect_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (intellect_OVERLAYS) with proper auth store checking.
        ...
        # /model picker sees the SAME list `intellect model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(intellect_id)
        if not model_ids:
            model_ids = curated.get(intellect_id, [])
            if intellect_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(intellect_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Intellect-only providers (ontoweb, openai-codex, copilot, opencode-go) ---
    from intellect_cli.providers import intellect_OVERLAYS
    from intellect_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Intellect provider ID.
    # intellect_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Intellect IDs ("copilot").
    _mdev_to_intellect = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in intellect_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Intellect slug — e.g. "github-copilot" → "copilot"
        intellect_slug = _mdev_to_intellect.get(pid, pid)
        if intellect_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(intellect_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, intellect_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or intellect_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(intellect_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", intellect_slug, exc)
        ...
        # configured.
        if not has_creds and intellect_slug == "anthropic":
            try:
                from agent.anthropic_adapter import (
                    read_claude_code_credentials,
                    read_intellect_oauth_credentials,
                )
                intellect_creds = read_intellect_oauth_credentials()
                cc_creds = read_claude_code_credentials()
                if (intellect_creds and intellect_creds.get("accessToken")) or \
        ...
        if intellect_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(intellect_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(intellect_slug)
                model_ids = _ids if _ids else (curated.get(intellect_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(intellect_slug, []) or curated.get(pid, [])
        else:
            ...
            if not model_ids:
                model_ids = curated.get(intellect_slug, []) or curated.get(pid, [])
                if intellect_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(intellect_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": intellect_slug,
            "name": get_label(intellect_slug),
            "is_current": intellect_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "intellect",
        ...
        seen_slugs.add(intellect_slug.lower())
        _record_builtin_endpoint(intellect_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or intellect_OVERLAYS (keeps /model in sync
    # with `intellect model`).
    try:
        from intellect_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Intellect's canonical write key (matches
            ...
            # Intellect writes ``models:`` as a dict keyed by model id
            # (see intellect_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Intellect's own writer uses; a hyphen variant is accepted
            ...
            # active model. Intellect's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, intellect_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`intellect_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from intellect_cli.models import fetch_openrouter_models
