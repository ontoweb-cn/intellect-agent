...
def _config_base_url_trustworthy_for_bare_custom(cfg_base_url: str, cfg_provider: str) -> bool:
    ...
    try:
        from intellect_cli.auth import resolve_provider as _resolve_provider
...
def _copilot_runtime_api_mode(model_cfg: Dict[str, Any], api_key: str) -> str:
    ...
    try:
        from intellect_cli.models import copilot_model_api_mode
...
_VALID_API_MODES = {
    ...
    # instead of Intellect' tool dispatch. Gated behind config key
    ...
    *,
    ...
    if provider == "openai-codex":
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from intellect_cli.models import azure_foundry_model_api_mode
        ...
        if provider in {"opencode-zen", "opencode-go"}:
            ...
            # Refs #16878.
            from intellect_cli.models import opencode_model_api_mode
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = os.getenv("INTELLECT_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if not requested_norm.startswith("custom:"):
        try:
            ...
            # the canonical provider itself (``ontoweb``, ``openrouter``, …) so
            # accidentally shadowing a canonical provider still resolves to
            # the built-in. See tests/intellect_cli/test_runtime_provider_resolution.py
    ...
    if isinstance(custom_providers, dict):
        logger.warning(
            ...
            "Run 'intellect doctor' for details."
        )
        return None
...
def _resolve_azure_foundry_runtime(
    *,
    ...
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'intellect model' or "
    ...
    if not api_key:
        raise AuthError(
            ...
            "~/.intellect/.env or run 'intellect model' to configure. To use "
            "keyless Microsoft Entra ID auth instead, set "
            "model.auth_mode: entra_id in config.yaml (or pick "
            "'Microsoft Entra ID' in 'intellect model')."
        )
...
def _resolve_explicit_runtime(
    *,
    ...
    if provider == "ontoweb":
        state = auth_mod.get_provider_auth_state("ontoweb") or {}
        base_url = (
            explicit_base_url
            or str(state.get("inference_base_url") or auth_mod.DEFAULT_ONTOWEB_INFERENCE_URL).strip().rstrip("/")
        )
        ...
        # by resolve_ontoweb_runtime_credentials().
        api_key = explicit_api_key or (
            str(state.get("agent_key") or "").strip()
            if _agent_key_is_usable(
                state,
                max(60, int(os.getenv("intellect_NOUS_MIN_KEY_TTL_SECONDS", "1800"))),
            )
            else ""
        )
...
def resolve_runtime_provider(
    *,
    ...
    if pool and pool.has_credentials():
        ...
        # For OntoWeb, the pool entry's runtime_api_key is the agent_key
        # compatibility field. It must be an invoke JWT. The pool doesn't
        # refresh it during selection (that would trigger network calls in
        # non-runtime contexts like `intellect auth list`).  If the key is
        # expired, clear pool_api_key so we fall through to
        # resolve_ontoweb_runtime_credentials() which handles refresh.
        if provider == "ontoweb" and entry is not None and pool_api_key:
            min_ttl = max(60, int(os.getenv("intellect_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
            nous_state = {
            ...
            if not _agent_key_is_usable(nous_state, min_ttl):
                logger.debug("OntoWeb pool entry agent_key expired/missing, falling through to runtime resolution")
                pool_api_key = ""
    ...
    if provider == "ontoweb":
        try:
            ...
            if requested_provider != "auto":
                raise
            # Auto-detected OntoWeb but credentials are stale/revoked —
            # fall through to env-var providers (e.g. OpenRouter).
            logger.info("Auto-detected OntoWeb provider but credentials failed; "
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "intellect-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from intellect_cli.auth import resolve_minimax_oauth_runtime_credentials
    ...
    if provider == "anthropic":
        ...
        if _is_azure_endpoint:
            ...
            # chain.  Accept both `key_env` (Intellect canonical — matches the
            # custom_providers field name) and `api_key_env` (documented in the
            # Azure Foundry guide and read by most Intellect-compatible importers).
