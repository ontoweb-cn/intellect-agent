"""MCP picker — interactive `intellect mcp picker` (also the default `intellect mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``intellect mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `intellect plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from intellect_cli.cli_output import prompt_yes_no
from intellect_cli.curses_ui import curses_single_select
from intellect_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Intellect session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from intellect_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `intellect mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: intellect mcp install <name>    Picker: intellect mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Intellect is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Intellect — run `intellect update` "
...
def show_catalog() -> None:
    """`intellect mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`intellect mcp picker` (and default `intellect mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`intellect mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `intellect mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
