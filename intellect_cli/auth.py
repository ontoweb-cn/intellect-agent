...
Supports OAuth device code flows (ONTOWEB Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.intellect/auth.json with cross-process file locking.
...
from intellect_cli.config import get_intellect_home, get_config_path, read_raw_config
from intellect_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
# ONTOWEB Portal defaults
DEFAULT_ONTOWEB_PORTAL_URL = "https://portal.ontoweb.cn"
DEFAULT_ONTOWEB_INFERENCE_URL = "https://inference-api.ontoweb.cn/v1"
DEFAULT_ONTOWEB_CLIENT_ID = "intellect-cli"
ONTOWEB_INFERENCE_INVOKE_SCOPE = "inference:invoke"
DEFAULT_ONTOWEB_SCOPE = ONTOWEB_INFERENCE_INVOKE_SCOPE
ONTOWEB_DEVICE_CODE_SOURCE = "device_code"
ONTOWEB_AUTH_PATH_INVOKE_JWT = "invoke_jwt"
ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120       # refresh 2 min before expiry
ONTOWEB_INVOKE_JWT_MIN_TTL_SECONDS = ACCESS_TOKEN_REFRESH_SKEW_SECONDS
...
SPOTIFY_DOCS_URL = "https://intellect-agent.ontoweb.cn/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

XAI_OAUTH_DOCS_URL = "https://intellect-agent.ontoweb.cn/docs/guides/xai-grok-oauth"
OAUTH_OVER_SSH_DOCS_URL = "https://intellect-agent.ontoweb.cn/docs/guides/oauth-over-ssh"
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    "ontoweb": ProviderConfig(
        id="ontoweb",
        name="ONTOWEB Portal",
        auth_type="oauth_device_code",
        portal_base_url=DEFAULT_ONTOWEB_PORTAL_URL,
        inference_base_url=DEFAULT_ONTOWEB_INFERENCE_URL,
        client_id=DEFAULT_ONTOWEB_CLIENT_ID,
        scope=DEFAULT_ONTOWEB_SCOPE,
    ),
    ...
    "spotify": ProviderConfig(
        ...
        inference_base_url=DEFAULT_SPOTIFY_API_BASE_URL,
        extra={
    ...
    "volcengine-coding-plan": ProviderConfig(
        id="volcengine-coding-plan",
        name="Volcano Engine Coding Plan",
        auth_type="api_key",
        inference_base_url="https://ark.cn-beijing.volces.com/api/coding/v3",
        api_key_env_vars=("VOLCENGINE_CODING_PLAN_API_KEY",),
        base_url_env_var="VOLCENGINE_CODING_PLAN_BASE_URL",
    ),
    "volcengine-agent-plan": ProviderConfig(
        id="volcengine-agent-plan",
        name="Volcano Engine Agent Plan",
        auth_type="api_key",
        inference_base_url="https://ark.cn-beijing.volces.com/api/plan/v3",
        api_key_env_vars=("VOLCENGINE_AGENT_PLAN_API_KEY",),
        base_url_env_var="VOLCENGINE_AGENT_PLAN_BASE_URL",
    ),
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from intellect_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
...
def _resolve_zai_base_url(api_key: str, default_url: str, env_override: str) -> str:
    ...
    try:
        from agent.oauth.model_tokens import read_provider_extra_metadata
        cached = read_provider_extra_metadata("zai").get("detected_endpoint")
        if isinstance(cached, dict) and cached.get("base_url"):
            if cached.get("key_hash", "") == key_hash:
    ...
    try:
        ...
        if isinstance(cached, dict) and cached.get("base_url"):
            if cached.get("key_hash", "") == key_hash:
                logger.debug("Z.AI: using auth.json cached endpoint %s", cached["base_url"])
                # Migrate to DB
                try:
    ...
    if detected and detected.get("base_url"):
        ...
        # Write to DB extra_metadata (primary)
        try:
            from agent.oauth.model_tokens import write_provider_extra_metadata
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``intellect auth``.
    """
    return (
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `intellect model` to re-authenticate."

    if error.code == "subscription_required":
        if error.provider == "ontoweb":
            return _format_ontoweb_entitlement_auth_error(error)
...
def _format_ontoweb_entitlement_auth_error(error: AuthError) -> str:
    try:
        ...
        message = format_ontoweb_portal_entitlement_message(
            account_info,
            capability="OntoWeb model access",
        )
        if message:
            return message
    except Exception:
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("intellect_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.intellect/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_intellect_home() / "auth.json"
    # Seat belt: if pytest is running and INTELLECT_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch INTELLECT_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".intellect" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set INTELLECT_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom INTELLECT_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_intellect_root()
    except Exception:
        return None
    profile_home = get_intellect_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.intellect/auth.json`` even when INTELLECT_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_intellect_root()`` for a profile-shaped INTELLECT_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".intellect" / "auth.json"
            try:
...
def _file_lock(
    ...
    (e.g. profile auth.json vs shared OntoWeb store) don't share reentrancy
...
def _auth_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    ``_ontoweb_shared_store_lock``, acquire ``_auth_store_lock`` FIRST
    (outer) and the shared OntoWeb lock SECOND (inner). All runtime
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        # Use auth_file's parent as home so profile mode doesn't resolve
        # the global auth.json to the profile directory (F5).
        store = SecretStore(home=str(auth_file.parent))
        raw = store.read_json(auth_file.name)
        # One-shot migration: if read_json returned empty (plaintext file),
        # fall back to legacy read and schedule encryption.
        if not raw:
            try:
    ...
    if isinstance(raw, dict) and isinstance(raw.get("systems"), dict):
        systems = raw["systems"]
        providers = {}
        if "ontoweb_portal" in systems:
            providers["ontoweb"] = systems["ontoweb_portal"]
        return {"version": AUTH_STORE_VERSION, "providers": providers,
                "active_provider": "ontoweb" if providers else None}
...
def _save_auth_store(auth_store: Dict[str, Any]) -> Path:
    ...
    # Write encrypted via SecretStore — Fernet AES-128-CBC + HMAC at rest.
    # The encryption key lives at ~/.intellect/.oauth-key (0o600).
    # Use auth_file's parent as home so profile mode doesn't write the
    # global auth store to the profile directory (F5).
    from agent.secret_store import SecretStore
    store = SecretStore(home=str(auth_file.parent))
    store.write_json(auth_file.name, auth_store)
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    profile can see providers (e.g. ``ontoweb``) that were only authenticated at
    global scope. Once the user runs ``intellect auth login <provider>`` inside
...
def _save_provider_state(auth_store: Dict[str, Any], provider_id: str, state: Dict[str, Any]) -> None:
    ...
    _set_active_provider_id(provider_id, auth_store=auth_store)
...
def _read_credential_pool_db_first(provider_id: str) -> List[Dict[str, Any]]:
    """PR-A7: multi-entry pool from ``oauth_pool_entries`` only (not singleton row)."""
    try:
        from agent.oauth.runtime_settings import get_oauth_runtime_settings

        if not get_oauth_runtime_settings().db_read_first():
            return []
        from agent.oauth.pool_storage import try_read_pool_entries

        return try_read_pool_entries(provider_id)
    except Exception:
        return []
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``intellect auth add <provider>`` inside the profile, profile entries
    fully shadow global for that provider on the next read.

    PR-A5/A10: DB pool table + ``oauth_tokens`` before ``auth.json`` (when enabled).

    Writes always go to the profile (``write_credential_pool`` is unchanged).
    See issue #18594 follow-up.
    """
    try:
        from agent.oauth.runtime_settings import should_read_auth_json
    except Exception:
        should_read_auth_json = lambda: True  # noqa: E731

    if provider_id:
        db_entries = _read_credential_pool_db_first(provider_id)
        if db_entries:
            return list(db_entries)
    elif not should_read_auth_json():
        try:
            from agent.oauth.pool_storage import read_all_pool_entries_grouped

            return read_all_pool_entries_grouped()
        except Exception:
            return {}
    ...
    if not should_read_auth_json():
        if provider_id:
            try:
                from agent.oauth.model_tokens import load_credential_pool_entries_from_db

                return load_credential_pool_entries_from_db(provider_id)
            except Exception:
                return []
        return {}
...
def write_credential_pool(provider_id: str, entries: List[Dict[str, Any]]) -> Path:
    """Persist credential pool to state.db and optionally auth.json (A7/A6)."""
    try:
        from agent.oauth.runtime_settings import get_oauth_runtime_settings
...
def get_provider_auth_state(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    OAuth or OntoWeb device-code session). See issue #18594 follow-up.
    """
    auth_store = _load_auth_store()
    return _load_provider_state(auth_store, provider_id)


def _normalize_active_provider_id(provider_id: Optional[str]) -> Optional[str]:
    pid = (provider_id or "").strip().lower()
    return pid or None


def _read_active_provider_from_config() -> Optional[str]:
    """Canonical active OAuth provider id (PR-A7): ``config.yaml`` ``auth.active_provider``."""
    try:
...
def _write_active_provider_to_config(provider_id: Optional[str]) -> None:
    try:
        ...
        if not isinstance(auth, dict):
            auth = {}
        normalized = _normalize_active_provider_id(provider_id)
        if normalized:
            auth["active_provider"] = normalized
        else:
        ...
        logger.debug("write auth.active_provider to config failed: %s", exc)


def _set_active_provider_id(
    provider_id: Optional[str],
    *,
    auth_store: Optional[Dict[str, Any]] = None,
) -> None:
    """Persist active provider to config; mirror to auth.json only when legacy write is on."""
    normalized = _normalize_active_provider_id(provider_id)
    _write_active_provider_to_config(normalized)
    try:
        from agent.oauth.runtime_settings import should_write_auth_json

        mirror_auth_json = should_write_auth_json()
    except Exception:
        mirror_auth_json = False
    if not mirror_auth_json:
        return
    if auth_store is None:
        with _auth_store_lock():
            store = _load_auth_store()
            if normalized:
                store["active_provider"] = normalized
            else:
...
def get_active_provider() -> Optional[str]:
    """Return the active OAuth provider id (config first, optional auth.json fallback)."""
    active = _read_active_provider_from_config()
    if active:
        return active
    try:
        from agent.oauth.runtime_settings import should_read_auth_json
        ...
        return None
    with _auth_store_lock():
        legacy = _normalize_active_provider_id(_load_auth_store().get("active_provider"))
    if legacy:
        _write_active_provider_to_config(legacy)
    return legacy
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # 1. Check canonical active_provider (config + legacy auth.json)
    try:
    ...
    # not by the user explicitly configuring anthropic in Intellect.
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `intellect logout`.
    ...
    with _auth_store_lock():
        ...
        if get_active_provider() == _normalize_active_provider_id(target):
            _set_active_provider_id(None, auth_store=auth_store)
            cleared = True
    ...
    try:
        ...
        try:
            if delete_model_token(store, target):
                cleared = True
            if delete_pool_entries(store, target):
                cleared = True
        finally:
            store.close()
    except Exception:
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    try:
        ...
        lines = ["Config issue detected — run 'intellect doctor' for full diagnostics:"]
        for ci in issues:
...
def resolve_provider(
    ...
    *,
    ...
    1. ``auth.active_provider`` / legacy auth.json with valid credentials
    ...
    _PROVIDER_ALIASES = {
        ...
        "doubao": "volcengine", "volc": "volcengine", "bytedance": "volcengine",
        "volc-coding": "volcengine-coding-plan", "doubao-coding": "volcengine-coding-plan",
        "volcengine-coding": "volcengine-coding-plan",
        "volc-agent": "volcengine-agent-plan", "doubao-agent": "volcengine-agent-plan",
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'intellect model' for available providers, or run 'intellect doctor' to diagnose config issues."
    ...
    # Check config / legacy auth store for an active OAuth provider
    try:
    ...
    raise AuthError(
        "No inference provider configured. Run 'intellect model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.intellect/.env.",
...
# Allowlist of hosts the ONTOWEB Portal proxy is willing to forward inference
...
# (ONTOWEB_INFERENCE_BASE_URL) bypass validation — that's the documented
# dev/staging escape hatch and the env source is already trusted (the
# user set it themselves).
_ALLOWED_ONTOWEB_INFERENCE_HOSTS: FrozenSet[str] = frozenset({
    "inference-api.ontoweb.cn",
})


def _validate_ontoweb_inference_url_from_network(url: Optional[str]) -> Optional[str]:
    ...
    The env-var override path (``ONTOWEB_INFERENCE_BASE_URL``) bypasses
    ...
    if parsed.scheme != "https":
        logger.warning(
            "ontoweb: refusing non-https inference URL scheme %r from Portal response",
            parsed.scheme,
        )
        return None
    if parsed.hostname not in _ALLOWED_ONTOWEB_INFERENCE_HOSTS:
        logger.warning(
            "ontoweb: refusing inference URL host %r from Portal response "
...
def _ontoweb_invoke_jwt_status(
    token: Any,
    *,
    scope: Any = None,
    expires_at: Any = None,
    min_ttl_seconds: int = ONTOWEB_INVOKE_JWT_MIN_TTL_SECONDS,
) -> Optional[str]:
    ...
    if ONTOWEB_INFERENCE_INVOKE_SCOPE not in scopes:
...
def _ontoweb_invoke_jwt_is_usable(
    token: Any,
    *,
    scope: Any = None,
    expires_at: Any = None,
...
def _assert_ontoweb_inference_jwt_usable(
    ...
    *,
    ...
    if reason is None:
        return
    raise AuthError(
        "ONTOWEB Portal access token is not a usable inference JWT "
        f"({reason}). Re-authenticate with: intellect auth add ontoweb",
        provider="ontoweb",
        code=reason,
...
def _log_ontoweb_invoke_jwt_selected(
    *,
    access_token: Any,
    sequence_id: Optional[str] = None,
) -> None:
    logger.info("OntoWeb inference auth: using NAS invoke JWT")
    _oauth_trace(
...
def _ontoweb_jwt_expires_at(token: Any, fallback_expires_at: Any = None) -> Optional[str]:
    claims = _decode_jwt_claims(token)
    exp = claims.get("exp")
    if isinstance(exp, (int, float)):
        try:
...
def _set_ontoweb_agent_key_from_invoke_jwt(
    state: Dict[str, Any],
    *,
    ...
    expires_at = _ontoweb_jwt_expires_at(access_token, state.get("expires_at"))
...
def _select_ontoweb_invoke_jwt(
    state: Dict[str, Any],
    *,
    access_token: Any = None,
    sequence_id: Optional[str] = None,
    ...
    _set_ontoweb_agent_key_from_invoke_jwt(state)
    _log_ontoweb_invoke_jwt_selected(
        access_token=state.get("access_token"),
        sequence_id=sequence_id,
    )


_ONTOWEB_EFFECTIVE_STATE_IGNORED_KEYS = frozenset({
...
def _ontoweb_effective_provider_state(state: Dict[str, Any]) -> Dict[str, Any]:
    return {
        key: value
        for key, value in state.items()
        if key not in _ONTOWEB_EFFECTIVE_STATE_IGNORED_KEYS
    }
...
def _read_qwen_cli_tokens() -> Dict[str, Any]:
    try:
        from agent.oauth.model_tokens import try_load_model_token_row

        row = try_load_model_token_row("qwen-oauth")
        if row and row.get("access_token"):
            meta = row.get("metadata") if isinstance(row.get("metadata"), dict) else {}
            expires_at = row.get("expires_at")
            expiry_ms = int(expires_at * 1000) if expires_at else int(meta.get("expiry_date") or 0)
            out: Dict[str, Any] = {
                "access_token": row["access_token"],
                "refresh_token": row.get("refresh_token") or "",
            }
            if expiry_ms:
                out["expiry_date"] = expiry_ms
            for key in ("resource_url", "token_type"):
                if key in meta:
                    out[key] = meta[key]
            return out
    except Exception:
...
def _save_qwen_cli_tokens(tokens: Dict[str, Any]) -> Path:
    ...
    if access:
        try:
            ...
            if expiry_ms:
                try:
                    expires_in = max(60, int((int(expiry_ms) / 1000.0) - time.time()))
                except Exception:
                    expires_in = 0
            from agent.oauth.model_tokens import try_persist_model_token

            meta = {
                k: tokens[k]
                for k in ("resource_url", "token_type", "expiry_date")
            ...
            logger.debug("persist Qwen token to oauth_tokens failed", exc_info=True)

    # File write (conditional on should_write_auth_json, default False)
    try:
        from agent.oauth.runtime_settings import should_write_auth_json
        if not should_write_auth_json():
            return auth_path
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("INTELLECT_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    try:
        from agent.oauth.model_tokens import try_model_token_auth_status

        db_status = try_model_token_auth_status("qwen-oauth")
        if db_status:
            return db_status
    except Exception:
        pass
    ...
    try:
        ...
        # and `intellect model` walks users into a broken Qwen setup flow.
...
#
# Tokens live in ~/.intellect/auth/google_oauth.json (managed by agent.google_oauth).
...
def resolve_gemini_oauth_runtime_credentials(
    *,
    ...
    """Resolve runtime OAuth creds for google-gemini-cli (DB-first via google_oauth)."""
    try:
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `intellect auth list` / `intellect status`."""
    try:
        from agent.oauth.model_tokens import try_model_token_auth_status

        db_status = try_model_token_auth_status("google-gemini-cli")
        if db_status:
            try:
                ...
                creds = load_credentials()
                if creds:
                    db_status.setdefault("auth_file", str(_credentials_path()))
                    db_status["expires_at_ms"] = creds.expires_ms
...
# Spotify auth — PKCE tokens stored in ~/.intellect/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("intellect_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set intellect_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("intellect_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("intellect_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("intellect_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _make_xai_callback_handler(expected_path: str) -> tuple[type[BaseHTTPRequestHandler], dict[str, Any]]:
    ...
    class _XAICallbackHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            # (#27385 — "callback received but Intellect times out") can produce
            # actionable evidence without a code change.  Logged values are
            # fingerprints / booleans only; no actual code/state strings leak
            # into the log file.  Run with ``intellect_LOG_LEVEL=INFO`` (or check
            # ``~/.intellect/logs/agent.log`` which captures INFO+ unconditionally).
            try:
                logger.info(
            ...
            if incoming["code"] is None and incoming["error"] is None:
                ...
                body = (
                    ...
                    "<code>intellect auth add xai-oauth</code> to retry.</p>"
                    "</body></html>"
                )
                self.wfile.write(body.encode("utf-8"))
                return
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `intellect auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `intellect auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def _spotify_save_state(spotify_state: Dict[str, Any]) -> None:
    """Persist Spotify OAuth state to DB (primary) and optionally auth.json."""
    access_token = spotify_state.get("access_token", "")
    refresh_token = spotify_state.get("refresh_token", "")
    expires_in = spotify_state.get("expires_in", 0)
    metadata = {
        "client_id": spotify_state.get("client_id", ""),
        "redirect_uri": spotify_state.get("redirect_uri", ""),
        "accounts_base_url": spotify_state.get("accounts_base_url", ""),
        "api_base_url": spotify_state.get("api_base_url", ""),
        "scope": spotify_state.get("scope", ""),
        "granted_scope": spotify_state.get("granted_scope", ""),
        "token_type": spotify_state.get("token_type", "Bearer"),
        "obtained_at": spotify_state.get("obtained_at", ""),
        "expires_at": spotify_state.get("expires_at", ""),
        "auth_type": spotify_state.get("auth_type", "oauth_pkce"),
    }
    # DB write (primary)
    if access_token:
        try:
            ...
            try_persist_model_token(
                ...
                expires_in=expires_in if expires_in else None,
                metadata=metadata,
            )
        except Exception:
            pass
...
def _spotify_state_from_db_row(row: Dict[str, Any]) -> Dict[str, Any]:
    """Convert a DB oauth_tokens row to Spotify state dict format."""
    ...
    return {
        ...
        "accounts_base_url": meta.get("accounts_base_url", DEFAULT_SPOTIFY_ACCOUNTS_BASE_URL),
        "api_base_url": meta.get("api_base_url", DEFAULT_SPOTIFY_API_BASE_URL),
        "scope": meta.get("scope", ""),
        "granted_scope": meta.get("granted_scope", ""),
        ...
        "expires_in": 0,
        "auth_type": meta.get("auth_type", "oauth_pkce"),
    }
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    try:
        from agent.oauth.model_tokens import try_load_model_token_row
        row = try_load_model_token_row("spotify")
        if row and row.get("access_token"):
    ...
    if not state or not state.get("access_token"):
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `intellect auth spotify` first.",
                provider="spotify",
    ...
    if should_refresh:
        ...
        _spotify_save_state(state)

    access_token = str(state.get("access_token", "") or "").strip()
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `intellect auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.intellect/.env, and return it.
    ...
    print("       App name:     anything (e.g. intellect-agent)")
    ...
    # Persist so subsequent `intellect auth spotify` runs skip the wizard.
    save_env_value("intellect_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("intellect_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved intellect_SPOTIFY_CLIENT_ID to ~/.intellect/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "intellect_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize Intellect:")
    print(authorize_url)
    print()
    ...
    _spotify_save_state(spotify_state)

    print("Spotify login successful!")
    print("  Provider state saved to DB")
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"Intellect is waiting for the OAuth callback on {redirect_uri}")
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.intellect/auth.json (not ~/.codex/)
#
# Intellect maintains its own Codex OAuth session separate from the Codex CLI
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from state.db or Intellect auth store (~/.intellect/auth.json).
    
    Returns dict with 'tokens' (access_token, refresh_token) and 'last_refresh'.
    Raises AuthError if no Codex tokens are stored.
    """
    db_data = _read_codex_tokens_from_db()
    if db_data:
        tokens = db_data.get("tokens") or {}
        access_token = tokens.get("access_token")
        refresh_token = tokens.get("refresh_token")
        if isinstance(access_token, str) and access_token.strip():
            if isinstance(refresh_token, str) and refresh_token.strip():
                return db_data
            allow_auth_json_fallback = False
            try:
                from agent.oauth.runtime_settings import should_read_auth_json
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `intellect auth` to authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `intellect auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `intellect auth` to re-authenticate.",
            provider="openai-codex",
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``intellect setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``intellect auth add openai-codex``
      ...
      ``intellect auth add`` workaround for #33000 would silently leave that
...
def _persist_codex_pool_after_token_save(
    tokens: Dict[str, str],
    last_refresh: Optional[str],
) -> None:
    """Sync device-code pool rows to state.db when auth.json writes are off (A7)."""
    access_token = tokens.get("access_token")
    if not access_token:
        return
    refresh_token = tokens.get("refresh_token")
    ...
    if not entries:
        return
    refreshable = {"device_code", "manual:device_code"}
    for entry in entries:
        if not isinstance(entry, dict):
            continue
        if entry.get("source") not in refreshable:
            continue
        entry["access_token"] = access_token
        if refresh_token:
            entry["refresh_token"] = refresh_token
        ...
        entry["last_error_reset_at"] = None
    try:
        write_credential_pool("openai-codex", entries)
    except Exception:
        logger.debug("persist Codex pool to DB failed", exc_info=True)


def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to state.db; optionally mirror to auth.json (A6)."""
    ...
    if access:
        try:
            ...
            logger.debug("persist Codex token to oauth_tokens failed", exc_info=True)
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Intellect auth state."""
    ...
    if response.status_code == 429:
        ...
        # "run intellect auth" prompt (see issue #32790).
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `intellect auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Intellect auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Intellect's own Codex token store.
    ...
    if not force_refresh:
        try:
            from agent.oauth.model_tokens import resolve_runtime_access_token

            db_token = resolve_runtime_access_token("openai-codex")
            if db_token and not _codex_access_token_is_expiring(db_token, refresh_skew_seconds):
                base_url = (
                    os.getenv("intellect_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("intellect_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Intellect processes
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.intellect/auth.json
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `intellect model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `intellect model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `intellect model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `intellect model`.",
            provider="xai-oauth",
    ...
    *,
    ...
    if access:
        try:
            ...
            logger.debug("persist xAI token to oauth_tokens failed", exc_info=True)
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.intellect/auth.json``. A single MITM during initial
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `intellect model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``intellect_XAI_BASE_URL``
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `intellect model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older Intellect (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `intellect model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``intellect model``
    ...
    *,
    ...
    if not force_refresh:
        try:
            ...
            db_token = resolve_runtime_access_token("xai-oauth")
            if db_token and not _xai_access_token_is_expiring(db_token, refresh_skew_seconds):
                base_url = _xai_validate_inference_base_url(
                    os.getenv("intellect_XAI_BASE_URL", "").strip().rstrip("/")
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("intellect_XAI_REFRESH_TIMEOUT_SECONDS", "20"))
...
def _resolve_verify(
    *,
    ...
    effective_ca = (
        ...
        or os.getenv("intellect_CA_BUNDLE")
...
# =============================================================================
# ONTOWEB Portal — token refresh and model discovery
# =============================================================================

# -----------------------------------------------------------------------------
# Shared OntoWeb token store — lets OAuth credentials persist across profiles
# so a new `intellect --profile <name> auth add ontoweb --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${intellect_SHARED_AUTH_DIR}/ontoweb_auth.json, defaulting to
# ``<intellect-root>/shared/ontoweb_auth.json`` where ``<intellect-root>`` is what
# ``get_default_intellect_root()`` returns — ``~/.intellect`` on Linux/macOS,
# ``%LOCALAPPDATA%\intellect`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's INTELLECT_HOME so named profiles (which
# typically live under ``<intellect-root>/profiles/<name>/``) all see the
# same file.
#
...
ONTOWEB_SHARED_STORE_FILENAME = "ontoweb_auth.json"
_ontoweb_shared_lock_holder = threading.local()


def _ontoweb_shared_auth_dir() -> Path:
    """Resolve the directory that holds the shared OntoWeb token store.

    Honors ``intellect_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<intellect-root>/shared/``, where ``<intellect-root>`` is what
    :func:`intellect_constants.get_default_intellect_root` returns — so
    Linux/macOS classic installs land at ``~/.intellect/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\intellect\\shared\\``, and
    Docker / custom ``INTELLECT_HOME`` deployments at
    ``<INTELLECT_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("intellect_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from intellect_constants import get_default_intellect_root
    return get_default_intellect_root() / "shared"


def _ontoweb_shared_store_path() -> Path:
    path = _ontoweb_shared_auth_dir() / ONTOWEB_SHARED_STORE_FILENAME
    # Seat belt: if pytest is running and this resolves to a path under the
    # real user's Intellect root, refuse rather than silently corrupt cross-profile
    # state. Tests must set intellect_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from intellect_constants import get_default_intellect_root
        real_home_shared = (
            get_default_intellect_root() / "shared" / ONTOWEB_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                f"Refusing to touch real user shared OntoWeb auth store during test run: "
                f"{path}. Set intellect_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path


@contextmanager
def _ontoweb_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    """Cross-profile lock for the shared OntoWeb OAuth store.
    ...
    ``_try_import_shared_ontoweb_state``, which holds this lock alone for
    ...
    """
    try:
        lock_path = _ontoweb_shared_store_path().with_suffix(".lock")
    except RuntimeError:
        # No INTELLECT_HOME yet (pre-setup): fall through without locking.
        yield
        return

    with _file_lock(
        lock_path,
        _ontoweb_shared_lock_holder,
        timeout_seconds,
        "Timed out waiting for shared OntoWeb auth lock",
    ):
        yield


def _merge_shared_ontoweb_oauth_state(state: Dict[str, Any]) -> bool:
    """Copy fresher shared OAuth tokens into a profile-local OntoWeb state."""
    shared = _read_shared_ontoweb_state()
    if not shared:
        return False
...
def _write_shared_ontoweb_state(state: Dict[str, Any]) -> None:
    """Persist a minimal copy of the OntoWeb OAuth state to the shared store.
    ...
    shared = {
        ...
        "scope": state.get("scope") or DEFAULT_ONTOWEB_SCOPE,
        "client_id": state.get("client_id") or DEFAULT_ONTOWEB_CLIENT_ID,
        "portal_base_url": state.get("portal_base_url") or DEFAULT_ONTOWEB_PORTAL_URL,
        "inference_base_url": state.get("inference_base_url") or DEFAULT_ONTOWEB_INFERENCE_URL,
        "obtained_at": state.get("obtained_at"),
        "expires_at": state.get("expires_at"),
        "updated_at": datetime.now(timezone.utc).isoformat(),
    }
    try:
        with _ontoweb_shared_store_lock():
            path = _ontoweb_shared_store_path()
            path.parent.mkdir(parents=True, exist_ok=True)
            # secure_parent_dir refuses to chmod / or top-level dirs (#25821).
            secure_parent_dir(path)
            tmp = path.with_name(f"{path.name}.tmp.{os.getpid()}.{uuid.uuid4().hex}")
            # Create with 0o600 atomically via os.open(O_EXCL) — closes the TOCTOU
            # window where write_text() + post-write chmod briefly exposed OntoWeb
        ...
        logger.debug("Failed to write shared OntoWeb auth store: %s", exc)


def _read_shared_ontoweb_state() -> Optional[Dict[str, Any]]:
    """Return the shared OntoWeb OAuth state if present and well-formed.
    ...
    try:
        ...
        logger.debug("Shared OntoWeb auth store at %s is unreadable: %s", path, exc)
        return None
    if not isinstance(payload, dict):
        return None
...
def _clear_shared_ontoweb_state(reason: str) -> None:
    """Remove the shared OntoWeb OAuth store after a terminal token failure."""
    try:
        with _ontoweb_shared_store_lock():
            path = _ontoweb_shared_store_path()
            try:
        ...
        logger.debug("Failed to clear shared OntoWeb auth store: %s", exc)


def _is_terminal_ontoweb_refresh_error(exc: Exception) -> bool:
    """True when retrying the same OntoWeb refresh token cannot succeed."""
    return (
        isinstance(exc, AuthError)
        and exc.provider == "ontoweb"
...
def _quarantine_ontoweb_oauth_state(
    state: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
    ...
    _clear_shared_ontoweb_state(reason)
    invalidate_ontoweb_auth_status_cache()


def _quarantine_ontoweb_pool_entries(
    auth_store: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
) -> bool:
    """Remove singleton-seeded OntoWeb pool entries that contain dead OAuth state."""
    pool = auth_store.get("credential_pool")
    if not isinstance(pool, dict):
        return False
    entries = pool.get("ontoweb")
    if not isinstance(entries, list):
        return False

    retained = []
    removed = False
    singleton_sources = {ONTOWEB_DEVICE_CODE_SOURCE, f"manual:{ONTOWEB_DEVICE_CODE_SOURCE}"}
    for entry in entries:
...
def _try_import_shared_ontoweb_state(
    *,
    timeout_seconds: float = 15.0,
) -> Optional[Dict[str, Any]]:
    """Attempt to rehydrate OntoWeb OAuth state from the shared store.
    ...
    for ``persist_ontoweb_credentials()``.
    ...
    """
    try:
        with _ontoweb_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            shared = _read_shared_ontoweb_state()
            if not shared:
                return None

            # Build a full state dict so refresh_ontoweb_oauth_from_state has every
            ...
            state: Dict[str, Any] = {
                ...
                "client_id": shared.get("client_id") or DEFAULT_ONTOWEB_CLIENT_ID,
                "portal_base_url": shared.get("portal_base_url") or DEFAULT_ONTOWEB_PORTAL_URL,
                "inference_base_url": shared.get("inference_base_url") or DEFAULT_ONTOWEB_INFERENCE_URL,
                "token_type": shared.get("token_type") or "Bearer",
                "scope": shared.get("scope") or DEFAULT_ONTOWEB_SCOPE,
            ...
            def _persist_shared_refresh(updated_state: Dict[str, Any], _reason: str) -> None:
                _write_shared_ontoweb_state(updated_state)

            refreshed = refresh_ontoweb_oauth_from_state(
                state,
                timeout_seconds=timeout_seconds,
                force_refresh=True,
                on_state_update=_persist_shared_refresh,
            )
            _write_shared_ontoweb_state(refreshed)
    except AuthError as exc:
        _oauth_trace(
        ...
        if _is_terminal_ontoweb_refresh_error(exc):
            _clear_shared_ontoweb_state("shared_import_terminal_refresh_failure")
        logger.debug("Shared OntoWeb import failed: %s", exc)
        return None
    except Exception as exc:
        _oauth_trace(
            "nous_shared_import_failed",
...
def _refresh_access_token(
    *,
    ...
    response = client.post(
        f"{portal_base_url}/api/oauth/token",
        headers={"x-ontoweb-refresh-token": refresh_token},
        data={
            "grant_type": "refresh_token",
            "client_id": client_id,
        },
    ...
    if response.status_code == 200:
        ...
        if "access_token" not in payload:
            raise AuthError("Refresh response missing access_token",
                            provider="ontoweb", code="invalid_token", relogin_required=True)
        return payload
    ...
    try:
        ...
        raise AuthError("Refresh token exchange failed",
                        provider="ontoweb", relogin_required=True) from exc

    code = str(error_payload.get("error", "invalid_grant"))
    description = str(error_payload.get("error_description") or "Refresh token exchange failed")
    relogin = code in {"invalid_grant", "invalid_token", "refresh_token_reused"}

    # Detect the OAuth 2.1 "refresh token reuse" signal from the OntoWeb portal
    ...
    # called POST /api/oauth/token with Intellect's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, Intellect's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            "ONTOWEB Portal detected refresh-token reuse and revoked this session.\n"
            "This usually means an external process (monitoring script, "
            "custom self-heal hook, or another Intellect install sharing "
            "~/.intellect/auth.json) called POST /api/oauth/token with Intellect's "
            "refresh token without persisting the rotated token back.\n"
            "OntoWeb refresh tokens are single-use — only Intellect may call the "
            "refresh endpoint. For health checks, use `intellect auth status` "
            "instead.\n"
            "Re-authenticate with: intellect auth add ontoweb"
        )
        relogin = True

    raise AuthError(description, provider="ontoweb", code=code, relogin_required=relogin)


def fetch_ontoweb_models(
    *,
    inference_base_url: str,
    api_key: str,
    timeout_seconds: float = 15.0,
    ...
    """Fetch available model IDs from the OntoWeb inference API."""
    ...
    if response.status_code != 200:
        ...
        raise AuthError(description, provider="ontoweb", code="models_fetch_failed")
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Intellect models — they're not reliable for agentic tool-calling
            if "intellect" in mid.lower():
                continue
...
def _agent_key_is_usable(state: Dict[str, Any], min_ttl_seconds: int) -> bool:
    ...
    if not isinstance(key, str) or not key.strip():
        return False
    return _ontoweb_invoke_jwt_is_usable(
        key,
        scope=state.get("scope"),
...
def resolve_ontoweb_access_token(
    *,
    timeout_seconds: float = 15.0,
    insecure: Optional[bool] = None,
    ca_bundle: Optional[str] = None,
    ...
    """Resolve a refresh-aware ONTOWEB Portal access token for managed tool gateways."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
        state = _load_provider_state(auth_store, "ontoweb")

        if not state:
            raise AuthError(
                "Intellect is not logged into ONTOWEB Portal.",
                provider="ontoweb",
                relogin_required=True,
            )
        ...
        portal_base_url = (
            ...
            or os.getenv("intellect_PORTAL_BASE_URL")
            or os.getenv("ONTOWEB_PORTAL_BASE_URL")
            or DEFAULT_ONTOWEB_PORTAL_URL
        ).rstrip("/")
        client_id = str(state.get("client_id") or DEFAULT_ONTOWEB_CLIENT_ID)
        verify = _resolve_verify(insecure=insecure, ca_bundle=ca_bundle, auth_state=state)

        with _ontoweb_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            merged_shared = _merge_shared_ontoweb_oauth_state(state)
            access_token = state.get("access_token")
            refresh_token = state.get("refresh_token")
            if not isinstance(access_token, str) or not access_token:
                raise AuthError(
                    "No access token found for ONTOWEB Portal login.",
                    provider="ontoweb",
                    relogin_required=True,
                )
            ...
            if not _is_expiring(state.get("expires_at"), refresh_skew_seconds):
                if merged_shared:
                    _ontoweb_save_state_to_db(state)
                    try:
                        from agent.oauth.runtime_settings import should_write_auth_json
                        if should_write_auth_json():
            ...
            with httpx.Client(
                ...
                try:
                    ...
                    if _is_terminal_ontoweb_refresh_error(exc):
                        _quarantine_ontoweb_oauth_state(
                            state,
                            exc,
                            reason="managed_access_token_refresh_failure",
                        )
                        _quarantine_ontoweb_pool_entries(
                            auth_store,
                            exc,
                            reason="managed_access_token_refresh_failure",
                        )
...
def refresh_ontoweb_oauth_pure(
    access_token: str,
    refresh_token: str,
    client_id: str,
    portal_base_url: str,
    ...
    *,
    ...
    scope: str = DEFAULT_ONTOWEB_SCOPE,
    obtained_at: Optional[str] = None,
    ...
    """Refresh OntoWeb OAuth state without mutating auth.json directly.
    ...
    state: Dict[str, Any] = {
        "access_token": access_token,
        "refresh_token": refresh_token,
        "client_id": client_id or DEFAULT_ONTOWEB_CLIENT_ID,
        "portal_base_url": (portal_base_url or DEFAULT_ONTOWEB_PORTAL_URL).rstrip("/"),
        "inference_base_url": (inference_base_url or DEFAULT_ONTOWEB_INFERENCE_URL).rstrip("/"),
        "token_type": token_type or "Bearer",
        "scope": scope or DEFAULT_ONTOWEB_SCOPE,
    ...
    with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
        current_invoke_jwt_status = _ontoweb_invoke_jwt_status(
        ...
        if force_refresh or current_invoke_jwt_status is not None:
            ...
            if not isinstance(refresh_token_value, str) or not refresh_token_value:
                ...
                raise AuthError(
                    "No refresh token is available for ONTOWEB Portal.",
                    provider="ontoweb",
                    relogin_required=True,
                )
            refreshed = _refresh_access_token(
            ...
            state["token_type"] = refreshed.get("token_type") or state.get("token_type") or "Bearer"
            state["scope"] = refreshed.get("scope") or state.get("scope")
            refreshed_url = _validate_ontoweb_inference_url_from_network(refreshed.get("inference_base_url"))
            if refreshed_url:
        ...
        _assert_ontoweb_inference_jwt_usable(state)
        _select_ontoweb_invoke_jwt(state)

    return state


def refresh_ontoweb_oauth_from_state(
    state: Dict[str, Any],
    *,
    timeout_seconds: float = 15.0,
    force_refresh: bool = False,
    ...
    """Refresh OntoWeb OAuth from a state dict. Thin wrapper around refresh_ontoweb_oauth_pure."""
    tls = state.get("tls") or {}
    return refresh_ontoweb_oauth_pure(
        state.get("access_token", ""),
        state.get("refresh_token", ""),
        state.get("client_id", "intellect-cli"),
        state.get("portal_base_url", DEFAULT_ONTOWEB_PORTAL_URL),
        state.get("inference_base_url", DEFAULT_ONTOWEB_INFERENCE_URL),
        token_type=state.get("token_type", "Bearer"),
        scope=state.get("scope", DEFAULT_ONTOWEB_SCOPE),
        obtained_at=state.get("obtained_at"),
        expires_at=state.get("expires_at"),
...
def _ontoweb_save_state_to_db(state: Dict[str, Any]) -> None:
    """Persist OntoWeb OAuth state to DB (best-effort)."""
    access_token = state.get("access_token", "")
    if not access_token:
        return
    try:
...
def persist_ontoweb_credentials(
    ...
    *,
    ...
    """Persist OntoWeb OAuth credentials as the singleton provider state
    and ensure the credential pool is in sync.

    OntoWeb credentials are read at runtime from two independent locations:

    - ``providers.ontoweb``: singleton state read by
      ``resolve_ontoweb_runtime_credentials()`` during 401 recovery and by
      ``_seed_from_singletons()`` during pool load.
    - ``credential_pool.ontoweb``: used by the runtime ``pool.select()`` path.

    Historically ``intellect auth add ontoweb`` wrote a ``manual:device_code`` pool
    entry only, skipping ``providers.ontoweb``. When the runtime credential
    expired, the recovery path read the empty singleton state and raised
    ``AuthError`` silently (``logger.debug`` at INFO level).

    This helper writes ``providers.ontoweb`` then calls ``load_pool("ontoweb")`` so
    ...
    ``intellect auth add ontoweb --label <name>``).  It gets embedded in the
    singleton state so that ``_seed_from_singletons`` uses it as the pool
    entry's label on every subsequent ``load_pool("ontoweb")`` instead of the
    ...
    return next(
        (e for e in pool.entries() if e.source == ONTOWEB_DEVICE_CODE_SOURCE),
        None,
    )


def _sync_ontoweb_pool_from_auth_store() -> None:
    """Best-effort pool reseed after providers.ontoweb changes; never fail login."""
    try:
        from agent.credential_pool import load_pool

        load_pool("ontoweb")
    except Exception as exc:
        logger.debug("Failed to sync OntoWeb credential pool from auth store: %s", exc)


def resolve_ontoweb_runtime_credentials(
    *,
    timeout_seconds: float = 15.0,
    insecure: Optional[bool] = None,
    ca_bundle: Optional[str] = None,
    ...
    """
    Resolve OntoWeb inference credentials for runtime use.
    ...
    # DB first (best-effort)
    state = None
    try:
        from agent.oauth.model_tokens import try_load_model_token_row
        row = try_load_model_token_row("ontoweb")
        if row and row.get("access_token"):
            meta = row.get("metadata") or {}
            if isinstance(meta, str):
                try:
            ...
            state = {
                "access_token": row.get("access_token", ""),
                "refresh_token": row.get("refresh_token", ""),
                "portal_base_url": meta.get("portal_base_url", ""),
                "inference_base_url": meta.get("inference_base_url", ""),
                "client_id": meta.get("client_id", ""),
                "scope": meta.get("scope", ""),
                "token_type": meta.get("token_type", "Bearer"),
                "obtained_at": meta.get("obtained_at", ""),
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Intellect is not logged into ONTOWEB Portal.",
                            provider="ontoweb", relogin_required=True)
        ...
        inference_base_url = (
            ...
            or os.getenv("ONTOWEB_INFERENCE_BASE_URL")
            or DEFAULT_ONTOWEB_INFERENCE_URL
        ).rstrip("/")
        client_id = str(state.get("client_id") or DEFAULT_ONTOWEB_CLIENT_ID)
        ...
        def _persist_state(reason: str) -> None:
            ...
            # the mtime-keyed OntoWeb auth-status cache warm during read paths.
            if (
                _ontoweb_effective_provider_state(state)
                == _ontoweb_effective_provider_state(persisted_state)
            ):
                _oauth_trace(
        ...
        with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError("No access token found for ONTOWEB Portal login.",
                                provider="ontoweb", relogin_required=True)

            invoke_jwt_status = _ontoweb_invoke_jwt_status(
                access_token,
                scope=state.get("scope"),
                expires_at=state.get("expires_at"),
            )
            if force_refresh or invoke_jwt_status is not None:
                with _ontoweb_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
                    if _merge_shared_ontoweb_oauth_state(state):
                        access_token = state.get("access_token")
                        refresh_token = state.get("refresh_token")
                        invoke_jwt_status = _ontoweb_invoke_jwt_status(
                            access_token,
            ...
            _assert_ontoweb_inference_jwt_usable(
                state,
                access_token=access_token,
            )
            _select_ontoweb_invoke_jwt(
                state,
                access_token=access_token,
                sequence_id=sequence_id,
            )
        ...
        _persist_state("resolve_ontoweb_runtime_credentials_final")

    if state_persisted:
        _sync_ontoweb_pool_from_auth_store()

    api_key = state.get("agent_key")
    if not isinstance(api_key, str) or not api_key:
        raise AuthError("Failed to resolve a OntoWeb inference API key",
                        provider="ontoweb", code="server_error")
    ...
    return {
        ...
        "expires_at": expires_at,
        "expires_in": expires_in,
        "source": ONTOWEB_AUTH_PATH_INVOKE_JWT,
        "auth_path": ONTOWEB_AUTH_PATH_INVOKE_JWT,
    }
...
def _empty_ontoweb_auth_status() -> Dict[str, Any]:
    return {
        "logged_in": False,
...
def _snapshot_ontoweb_pool_status() -> Dict[str, Any]:
    """Best-effort status from the credential pool.

    This is a fallback only. The auth-store provider state is the runtime source
    of truth because it is what ``resolve_ontoweb_runtime_credentials()`` refreshes.
    """
    try:
        from agent.credential_pool import load_pool
        ...
        if not pool or not pool.has_credentials():
            return _empty_ontoweb_auth_status()
...
# ── Process-level memo for get_ontoweb_auth_status() ──
# get_ontoweb_auth_status() validates state by calling resolve_ontoweb_runtime_credentials(),
# which does a synchronous OAuth refresh POST to portal.ontoweb.cn. That can take
# ~350ms even on the failure path, and read-only UI surfaces (`intellect tools`, status panels,
# subscription-feature checks) call it many times per render — `intellect tools` → "All Platforms"
# was firing the refresh ~31× during one menu paint, racking up >13s of HTTP and burning
# single-use refresh tokens. Cache the snapshot for a few seconds, keyed on the auth.json
# mtime so that `intellect auth login/logout/add/remove` invalidate naturally on the next call.
_NOUS_AUTH_STATUS_CACHE_TTL = 15.0  # seconds
_ontoweb_auth_status_cache: Optional[Tuple[float, Optional[float], Dict[str, Any]]] = None
...
def invalidate_ontoweb_auth_status_cache() -> None:
    """Clear the get_ontoweb_auth_status() process-level memo.

    Call this from any code path that mutates OntoWeb auth state without going
    through resolve_ontoweb_runtime_credentials() (e.g. tests). Login/logout
    ...
    """
    global _ontoweb_auth_status_cache
    _ontoweb_auth_status_cache = None


def get_ontoweb_auth_status() -> Dict[str, Any]:
    """Status snapshot for OntoWeb auth.
    ...
    ``invalidate_ontoweb_auth_status_cache()`` explicitly.
    """
    global _ontoweb_auth_status_cache
    now = time.monotonic()
    mtime = _auth_file_mtime()
    cached = _ontoweb_auth_status_cache
    ...
    status = _compute_ontoweb_auth_status()
    _ontoweb_auth_status_cache = (now, mtime, dict(status))
    return status


def _compute_ontoweb_auth_status() -> Dict[str, Any]:
    """Uncached implementation of get_ontoweb_auth_status(). See that function."""
    try:
        from agent.oauth.model_tokens import try_model_token_auth_status

        db_status = try_model_token_auth_status("ontoweb")
        if db_status and db_status.get("logged_in"):
            return {
                "logged_in": True,
                "credential_source": "oauth_tokens",
                "source": "oauth_tokens",
                "inference_credential_present": True,
            }
    except Exception:
    ...
    if state:
        ...
        try:
            ...
            refreshed_state = get_provider_auth_state("ontoweb") or state
            base_status.update(
                {
                    "logged_in": True,
    ...
    return _snapshot_ontoweb_pool_status()


# ---------------------------------------------------------------------------
# Deprecated aliases (pre-OntoWeb rename — remove after downstream plugins migrate)
# ---------------------------------------------------------------------------
_format_nous_entitlement_auth_error = _format_ontoweb_entitlement_auth_error
_nous_invoke_jwt_status = _ontoweb_invoke_jwt_status
_nous_invoke_jwt_is_usable = _ontoweb_invoke_jwt_is_usable
_assert_nous_inference_jwt_usable = _assert_ontoweb_inference_jwt_usable
_log_nous_invoke_jwt_selected = _log_ontoweb_invoke_jwt_selected
_nous_jwt_expires_at = _ontoweb_jwt_expires_at
_set_nous_agent_key_from_invoke_jwt = _set_ontoweb_agent_key_from_invoke_jwt
_select_nous_invoke_jwt = _select_ontoweb_invoke_jwt
_nous_effective_provider_state = _ontoweb_effective_provider_state
_nous_shared_lock_holder = _ontoweb_shared_lock_holder
_nous_shared_store_path = _ontoweb_shared_store_path
_nous_shared_store_lock = _ontoweb_shared_store_lock
_merge_shared_nous_oauth_state = _merge_shared_ontoweb_oauth_state
_is_terminal_nous_refresh_error = _is_terminal_ontoweb_refresh_error
_quarantine_nous_oauth_state = _quarantine_ontoweb_oauth_state
_quarantine_nous_pool_entries = _quarantine_ontoweb_pool_entries
fetch_nous_models = fetch_ontoweb_models
refresh_nous_oauth_pure = refresh_ontoweb_oauth_pure
refresh_nous_oauth_from_state = refresh_ontoweb_oauth_from_state
persist_nous_credentials = persist_ontoweb_credentials
_snapshot_nous_pool_status = _snapshot_ontoweb_pool_status


def get_codex_auth_status() -> Dict[str, Any]:
    """Status snapshot for Codex auth.
    
    Checks the credential pool first (where `intellect auth` stores credentials),
    then falls back to the legacy provider state.
    """
    try:
        from agent.oauth.model_tokens import try_model_token_auth_status

        db_status = try_model_token_auth_status("openai-codex")
        if db_status:
            return db_status
    except Exception:
        pass
...
def get_xai_oauth_auth_status() -> Dict[str, Any]:
    try:
        ...
        db_status = try_model_token_auth_status("xai-oauth")
        if db_status:
            return db_status
    except Exception:
        pass
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("intellect_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("intellect_COPILOT_ACP_ARGS", "").strip()
...
def get_auth_status(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    if target == "ontoweb":
        return get_ontoweb_auth_status()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``intellect doctor`` runs the live
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Intellect' "
                    "lazy-install at first use)."
                )
            else:
                info["hint"] = (
                    "azure-identity is installed; live credential validation "
                    "is skipped here. Run `intellect doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set intellect_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _update_config_for_provider(
    ...
    """Update config.yaml (``auth.active_provider`` + model) for the active provider.
    ...
    MiniMax's API).
    """
    _set_active_provider_id(provider_id)
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `intellect logout` historically keyed off auth.json.active_provider only.
    ...
    if provider in {"ontoweb", "openai-codex", "xai-oauth"}:
        return provider
    return None
...
def _prompt_model_selection(
    ...
    """
    from intellect_cli.models import _format_price_per_mtok
    ...
    try:
        ...
        _upgrade_url = (portal_url or DEFAULT_ONTOWEB_PORTAL_URL).rstrip("/")
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from intellect_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'intellect model' or 'intellect setup' instead."""
    print("The 'intellect login' command has been removed.")
    print("Use 'intellect auth' to manage credentials,")
    print("'intellect model' to select a provider, or 'intellect setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.intellect/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing Intellect-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Intellect auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Intellect will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("intellect_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("Credentials imported. Note: if Codex CLI refreshes its token,")
                print("Intellect will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Intellect gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Intellect creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Intellect auth store
    _save_codex_tokens(creds["tokens"], creds.get("last_refresh"))
    config_path = _update_config_for_provider("openai-codex", creds.get("base_url", DEFAULT_CODEX_BASE_URL))
    print()
    print("Login successful!")
    from intellect_constants import display_intellect_home as _dhh
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Intellect auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(Intellect creates its own local OAuth session)")
    print()
...
def _xai_oauth_build_authorize_url(
    *,
    ...
    # `referrer=intellect-agent` lets xAI attribute Intellect-originated logins
    ...
    *,
    ...
    if not code_verifier:
        raise AuthError(
            ...
            "This is a bug in Intellect — please report at "
            "https://gitee.com/ontoweb/intellect-agent/issues/26990.",
            provider="xai-oauth",
...
def _xai_oauth_loopback_login(
    *,
    ...
    if manual_paste:
        ...
        print("Open this URL to authorize Intellect with xAI:")
        print(authorize_url)
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to DB (primary) and optionally auth.json."""
    access_token = auth_state.get("access_token", "")
    refresh_token = auth_state.get("refresh_token", "")
    expires_in = auth_state.get("expires_in", 0)
    metadata = {
        "region": auth_state.get("region", ""),
        "portal_base_url": auth_state.get("portal_base_url", ""),
        "inference_base_url": auth_state.get("inference_base_url", ""),
        "client_id": auth_state.get("client_id", ""),
        "scope": auth_state.get("scope", ""),
        "token_type": auth_state.get("token_type", "Bearer"),
        "resource_url": auth_state.get("resource_url", ""),
        "obtained_at": auth_state.get("obtained_at", ""),
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Intellect login via MiniMax ({region}) OAuth...")
...
def _minimax_oauth_quarantine_on_terminal_refresh(state: Dict[str, Any], exc: AuthError) -> None:
    ...
    provider. Mirrors the OntoWeb / xAI-OAuth / Codex-OAuth quarantine pattern
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `intellect model` and select "
                "MiniMax (OAuth).",
...
def _minimax_state_from_db_row(row: Dict[str, Any]) -> Dict[str, Any]:
    """Convert a DB oauth_tokens row to MiniMax auth_state dict format."""
    meta = row.get("metadata") or {}
    if isinstance(meta, str):
        try:
            import json as _json
    ...
    return {
        ...
        "portal_base_url": meta.get("portal_base_url", MINIMAX_OAUTH_GLOBAL_BASE),
        "inference_base_url": meta.get("inference_base_url", MINIMAX_OAUTH_GLOBAL_INFERENCE),
        "client_id": meta.get("client_id", MINIMAX_OAUTH_CLIENT_ID),
        "scope": meta.get("scope", MINIMAX_OAUTH_SCOPE),
        "token_type": meta.get("token_type", "Bearer"),
        "access_token": row.get("access_token", ""),
        "refresh_token": row.get("refresh_token", ""),
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``intellect status`` that just want to know
    ...
    try:
        from agent.oauth.model_tokens import try_load_model_token_row
        row = try_load_model_token_row("minimax-oauth")
        if row and row.get("access_token"):
...
def get_minimax_oauth_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        db_status = try_model_token_auth_status("minimax-oauth")
        if db_status:
            return db_status
    except Exception:
        pass
...
def _ontoweb_device_code_login(
    *,
    ...
    """Run the OntoWeb device-code flow and return full OAuth state without persisting."""
    pconfig = PROVIDER_REGISTRY["ontoweb"]
    portal_base_url = (
        portal_base_url
        or os.getenv("intellect_PORTAL_BASE_URL")
        or os.getenv("ONTOWEB_PORTAL_BASE_URL")
    ...
    print(f"Starting Intellect login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        return refresh_ontoweb_oauth_from_state(
            auth_state,
            timeout_seconds=timeout_seconds,
            force_refresh=False,
        )
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `intellect model` again to finish setup.")
            raise SystemExit(1)
        raise


def _login_ontoweb(args, pconfig: ProviderConfig) -> None:
    """ONTOWEB Portal device authorization flow."""
    ...
    try:
        ...
        # flow, check the shared store for an existing OntoWeb credential
        # from any other profile. If present, offer to rehydrate it.
        shared = _read_shared_ontoweb_state()
        if shared:
            try:
                shared_path = _ontoweb_shared_store_path()
            except RuntimeError:
                shared_path = None
            print()
            if shared_path:
                print(f"Found existing OntoWeb OAuth credentials at {shared_path}")
            else:
                print("Found existing shared OntoWeb OAuth credentials")
            try:
                do_import = input("Import these credentials? [Y/n]: ").strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
            if do_import in {"", "y", "yes"}:
                print("Rehydrating OntoWeb session from shared credentials...")
                auth_state = _try_import_shared_ontoweb_state(
                    timeout_seconds=timeout_seconds,
                )
        ...
        if auth_state is None:
            auth_state = _ontoweb_device_code_login(
        ...
        # overwrites it to "ontoweb".  If the user picks "Skip (keep current)"
        ...
        _ontoweb_save_state_to_db(auth_state)
        ...
        _write_shared_ontoweb_state(auth_state)
        _sync_ontoweb_pool_from_auth_store()
        ...
        # leave the config in a half-updated state (provider=ontoweb but model
        ...
        try:
            ...
            from intellect_cli.models import (
                get_curated_ontoweb_model_ids, get_pricing_for_provider,
                check_ontoweb_free_tier, partition_ontoweb_models_by_tier,
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older Intellect builds still see
            ...
            if model_ids:
                ...
                _url = (_portal or DEFAULT_ONTOWEB_PORTAL_URL).rstrip("/")
        ...
        # them to the OntoWeb Portal with a mismatched model.  The OntoWeb OAuth tokens
        # stay saved for future use.
        if not selected_model:
            # Restore the prior active_provider that _save_provider_state
            # overwrote to "ontoweb".  config.yaml model.provider is left
            # untouched, so the user's previous provider is fully preserved.
            _set_active_provider_id(prior_active_provider)
            print()
            print("No provider change. OntoWeb credentials saved for future use.")
            print("  Run `intellect model` again to switch to ONTOWEB Portal.")
            return

        config_path = _update_config_for_provider(
            "ontoweb", inference_base_url, default_model=selected_model,
        )
        if selected_model:
            _save_model_choice(selected_model)
            print(f"Default model set to: {selected_model}")
        print(f"  Config updated: {config_path} (model.provider=ontoweb)")
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Intellect will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `intellect model` or configure an API key to use Intellect.")
        else:
