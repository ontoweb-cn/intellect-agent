...
There are two ways to add a platform to the Intellect gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.intellect/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Intellect code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `intellect gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `intellect_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Intellect core.
...
And add it to the `intellect-gateway` composite:

```python
"intellect-gateway": {
    "includes": [..., "intellect-your-platform"]
}
```
...
## 12. Status Display (`intellect_cli/status.py`)
...
## 13. Gateway Setup Wizard (`intellect_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ intellect_cli/ toolsets.py \
  --include="*.py" -l | sort -u
