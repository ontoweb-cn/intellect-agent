"""Shim — canonical implementation moved to plugins/platforms/api_server/adapter.py (P3-10).
...
``gateway.platforms.api_server`` and ``plugins.platforms.api_server.adapter`` are the
...
* the gateway's built-in adapter chain imports module-level helpers
  (``check_api_server_requirements``, private regexes and constants),
...
from plugins.platforms.api_server import adapter as _impl
