"""Shim — canonical implementation moved to plugins/platforms/whatsapp/adapter.py (P3-10).
...
``gateway.platforms.whatsapp`` and ``plugins.platforms.whatsapp.adapter`` are the
...
* the gateway's built-in adapter chain imports module-level helpers
  (``check_whatsapp_requirements``, private regexes and constants),
...
from plugins.platforms.whatsapp import adapter as _impl
