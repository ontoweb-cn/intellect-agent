"""Shim — canonical implementation moved to plugins/platforms/weixin/adapter.py (P3-10).
...
``gateway.platforms.weixin`` and ``plugins.platforms.weixin.adapter`` are the
...
* the gateway's built-in adapter chain imports module-level helpers
  (``check_weixin_requirements``, private regexes and constants),
...
from plugins.platforms.weixin import adapter as _impl
