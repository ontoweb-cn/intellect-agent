"""Shim — canonical implementation moved to plugins/platforms/feishu/adapter.py (P3-10).
...
``gateway.platforms.feishu`` and ``plugins.platforms.feishu.adapter`` are the
...
* the gateway's built-in adapter chain imports module-level helpers
  (``check_feishu_requirements``, private regexes and constants),
...
from plugins.platforms.feishu import adapter as _impl
