"""Shim — canonical implementation moved to plugins/platforms/bluebubbles/adapter.py (P3-10).
...
``gateway.platforms.bluebubbles`` and ``plugins.platforms.bluebubbles.adapter`` are the
...
* the gateway's built-in adapter chain imports module-level helpers
  (``check_bluebubbles_requirements``, private regexes and constants),
...
from plugins.platforms.bluebubbles import adapter as _impl
