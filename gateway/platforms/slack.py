"""Shim — canonical implementation moved to plugins/platforms/slack/adapter.py (P3-10).
...
``gateway.platforms.slack`` and ``plugins.platforms.slack.adapter`` are the
...
* the gateway's built-in adapter chain imports module-level helpers
  (``check_slack_requirements``, private regexes and constants),
...
from plugins.platforms.slack import adapter as _impl
