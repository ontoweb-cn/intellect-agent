"""Shim — canonical implementation moved to plugins/platforms/dingtalk/adapter.py (P3-10).
...
``gateway.platforms.dingtalk`` and ``plugins.platforms.dingtalk.adapter`` are the
...
* the gateway's built-in adapter chain imports module-level helpers
  (``check_dingtalk_requirements``, private regexes and constants),
...
from plugins.platforms.dingtalk import adapter as _impl
