"""Shim — canonical implementation moved to plugins/platforms/wecom/adapter.py (P3-10).
...
``gateway.platforms.wecom`` and ``plugins.platforms.wecom.adapter`` are the
...
* the gateway's built-in adapter chain imports module-level helpers
  (``check_wecom_requirements``, private regexes and constants),
...
from plugins.platforms.wecom import adapter as _impl
