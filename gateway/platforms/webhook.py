"""Shim — canonical implementation moved to plugins/platforms/webhook/adapter.py (P3-10).
...
``gateway.platforms.webhook`` and ``plugins.platforms.webhook.adapter`` are the
...
* the gateway's built-in adapter chain imports module-level helpers
  (``check_webhook_requirements``, private regexes and constants),
...
from plugins.platforms.webhook import adapter as _impl
