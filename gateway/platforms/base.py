...
# Audio file extensions Intellect recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Intellect' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Intellect-created Telegram private-chat
...
from intellect_constants import get_default_intellect_root, get_intellect_dir, get_intellect_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.intellect/.env manually."
)
...
# Default location: {INTELLECT_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_intellect_dir("cache/images", "image_cache")
...
async def cache_image_from_url(url: str, ext: str = ".jpg", retries: int = 2) -> str:
    ...
    """
    from tools.url_safety import is_safe_url
    from tools.safe_http import safe_async_http_transport
    ...
    async with httpx.AsyncClient(
        transport=safe_async_http_transport(),
        timeout=30.0,
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; IntellectAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
...
AUDIO_CACHE_DIR = get_intellect_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_intellect_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_intellect_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_intellect_dir("cache/screenshots", "browser_screenshots")
_INTELLECT_HOME = get_intellect_home()
_INTELLECT_ROOT = get_default_intellect_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "intellect_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "intellect_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "intellect_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "intellect_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _INTELLECT_HOME / "image_cache",
    _INTELLECT_HOME / "audio_cache",
    _INTELLECT_HOME / "video_cache",
    _INTELLECT_HOME / "document_cache",
    _INTELLECT_HOME / "browser_screenshots",
    ...
    _INTELLECT_HOME / "cache" / "images",
    _INTELLECT_HOME / "cache" / "audio",
    _INTELLECT_HOME / "cache" / "videos",
    _INTELLECT_HOME / "cache" / "documents",
    _INTELLECT_HOME / "cache" / "screenshots",
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Intellect profile and shared Intellect root both contain control
    # files and credentials. Only cache subdirectories under them are
    # explicitly allowlisted above.
    for intellect_root in (_INTELLECT_HOME, _INTELLECT_ROOT):
        denied.append(intellect_root / ".env")
        denied.append(intellect_root / "auth.json")
        denied.append(intellect_root / "credentials")
        denied.append(intellect_root / "config.yaml")
    return denied
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``intellect_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Intellect-managed cache, under an operator-allowlisted root
    (``intellect_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # The denylist still blocks /etc, /proc, ~/.ssh, ~/.aws, ~/.intellect/.env,
    # ~/.intellect/auth.json, etc. — so the obvious prompt-injection sites
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?intellect\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+intellect[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("intellect_GATEWAY_BUSY_TEXT_MODE", "queue").strip().lower()
            or "queue"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "intellect_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "intellect_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from intellect_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          INTELLECT_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          INTELLECT_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          INTELLECT_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("INTELLECT_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("INTELLECT_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("INTELLECT_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _intellect_run_generation is set on
