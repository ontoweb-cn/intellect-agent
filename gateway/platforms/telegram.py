"""Shim — canonical implementation moved to plugins/platforms/telegram/adapter.py (P3-10).
...
``gateway.platforms.telegram`` and ``plugins.platforms.telegram.adapter`` are the
...
* the gateway's built-in adapter chain imports module-level helpers
  (``check_telegram_requirements``, private regexes and constants),
...
from plugins.platforms.telegram import adapter as _impl
