"""Shim — canonical implementation moved to plugins/platforms/signal/adapter.py (P3-10).
...
``gateway.platforms.signal`` and ``plugins.platforms.signal.adapter`` are the
...
* the gateway's built-in adapter chain imports module-level helpers
  (``check_signal_requirements``, private regexes and constants),
...
from plugins.platforms.signal import adapter as _impl
