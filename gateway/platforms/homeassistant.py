"""Shim — canonical implementation moved to plugins/platforms/homeassistant/adapter.py (P3-10).
...
``gateway.platforms.homeassistant`` and ``plugins.platforms.homeassistant.adapter`` are the
...
* the gateway's built-in adapter chain imports module-level helpers
  (``check_homeassistant_requirements``, private regexes and constants),
...
from plugins.platforms.homeassistant import adapter as _impl
