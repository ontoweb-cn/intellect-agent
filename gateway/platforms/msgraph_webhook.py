"""Shim — canonical implementation moved to plugins/platforms/msgraph_webhook/adapter.py (P3-10).
...
``gateway.platforms.msgraph_webhook`` and ``plugins.platforms.msgraph_webhook.adapter`` are the
...
* the gateway's built-in adapter chain imports module-level helpers
  (``check_msgraph_webhook_requirements``, private regexes and constants),
...
from plugins.platforms.msgraph_webhook import adapter as _impl
