"""Shim — canonical implementation moved to plugins/platforms/yuanbao/adapter.py (P3-10).
...
``gateway.platforms.yuanbao`` and ``plugins.platforms.yuanbao.adapter`` are the
...
* the gateway's built-in adapter chain imports module-level helpers
  (``check_yuanbao_requirements``, private regexes and constants),
...
from plugins.platforms.yuanbao import adapter as _impl
