"""Shim — canonical implementation moved to plugins/platforms/matrix/adapter.py (P3-10).
...
``gateway.platforms.matrix`` and ``plugins.platforms.matrix.adapter`` are the
...
* the gateway's built-in adapter chain imports module-level helpers
  (``check_matrix_requirements``, private regexes and constants),
...
from plugins.platforms.matrix import adapter as _impl
