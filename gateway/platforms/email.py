"""Shim — canonical implementation moved to plugins/platforms/email/adapter.py (P3-10).
...
``gateway.platforms.email`` and ``plugins.platforms.email.adapter`` are the
...
* the gateway's built-in adapter chain imports module-level helpers
  (``check_email_requirements``, private regexes and constants),
...
from plugins.platforms.email import adapter as _impl
