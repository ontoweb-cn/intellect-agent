...
shared ``state.db`` (same file and conventions as ``tools.async_delegation``
— WAL, owner pid + process-start-time liveness, bounded retention). The
gateway writes three checkpoints around the send:
...
semantics are explicit about ambiguity:
...
def _db_path():
    return get_intellect_home() / "state.db"
...
def _transaction() -> Iterator[sqlite3.Connection]:
    ...
    gateway that exhausts ``RLIMIT_NOFILE``, this leaks per call.
    """
    conn = _connect()
    try:
        with conn:
