...
def _discord_tools_loaded() -> bool:
    ...
    Two conditions must hold:
      1. The `discord` or `discord_admin` toolset is enabled for the
         Discord platform via `intellect tools` (opt-in, default OFF).
...
def build_session_context_prompt(
    ...
    *,
    ...
    if context.source.platform == Platform.SLACK:
        ...
        # `discord` / `discord_admin` via `intellect tools` AND the bot
    ...
    lines.append(
        f"- `\"local\"` → Save to local files only ({display_intellect_home()}/cron/output/)"
    )
...
def build_session_key(
    ...
    Multi-user / multi-project extensions:
      - When *member_id* is set, ``:member:<id>`` is appended.
      - When *team_id* is set, ``:team:<id>`` is appended.
      - When *project_id* is set, ``:project:<id>`` is appended.
