...
(``_running_agents`` in the runner, the adapter-level session guard), but the
durable transcript is owned by SESSION_ID — and ``switch_session``
(``gateway/session.py:1329``) makes the key→id mapping many-to-one (``/resume``
of a named session from a second chat/topic, CLI-continuity rebinding,
async-delegation completion pinning, Telegram topic-binding tip-walks). Two
routing keys mapped to one session_id run concurrent turns on two different
agent objects, so no per-key guard ever sees the collision. The two turns then
interleave their flushes on one transcript: rows persist in completion order
instead of arrival order, the identity-marker dedup over shared history dicts
can swallow a row outright, and the second turn runs on a history base that
never saw the first turn's exchange — leaving a permanent ``user;user``
alternation wedge that ``repair_message_sequence`` re-repairs on every request
forever.
...
- **Generation-scoped, identity-checked release.** A token records its owner
  ...
  newer turn's lease. Release is idempotent.
...
- Mid-turn compression rotation leaves a small alias window: the tip-walk can
  ...
  ``rebind`` closes the common case; the mid-turn binding-sync sites are the
  right place to alias the lease in a follow-up.
"""
...
# INTELLECT_TURN_LEASE_TIMEOUT bridge because lease contention is not agent
...
class SessionTurnLeaseRegistry:
        ...
        *,
        ...
        if lease.lock.locked():
            ...
            logger.warning(
                ...
                "serializing this turn behind the previous turn's flush",
                session_id,
                owner_key,
                generation,
    ...
    def rebind(self, token: Optional[TurnLeaseToken], new_session_id: str) -> bool:
        ...
        Edge: if the new id already has a live lease of its own (another turn
        is running on the target session), the two serialization domains
        cannot be merged mid-wait — log loudly and keep the token on the old
        id. Fail-open, never deadlock: a holder cannot wait mid-turn.
        """
        if (
            token is None
            or token.released
