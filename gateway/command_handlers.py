...
class GatewayCommandHandlers:
    """Mixin providing slash-command handler methods for GatewayRunner.

    Extracted from ``gateway/run.py`` to reduce the monolithic class.
    Methods use ``self`` to access GatewayRunner attributes at runtime
    via Python MRO.
    """
        ...
        try:
            ...
            if _mid:
                ...
                try:
                    ...
                    _mdb = MembershipDB(config=_cfg)
                    _mdb.cleanup_expired_sessions()
                    _m = _mdb.get_member(_mid)
                    _online = _m.get("online_status") if _m else "offline"
                    login = (_m.get("login_name") if _m else None) or _mid[:12]
                    result += f"\n\n**Member:** {login}"
                    if _role:
                        result += f" ({_role})"
                    if _online == "online":
                        _sessions = _mdb.list_active_sessions(_mid)
                        _plat_list = ", ".join(s["platform"] for s in _sessions)
                        result += f"\n**Status:** online ({_plat_list})"
                    else:
                        result += "\n**Status:** offline"
                except Exception:
                    _log_non_critical()
    ...
    async def _handle_oauth_command(self, event: MessageEvent) -> str:
        """Handle /oauth <login|status|unlink>."""
        args = event.get_command_args().strip().lower()
        sub = args.split()[0] if args else "status"
        target = args.split()[1] if len(args.split()) > 1 else ""

        if sub == "login" and target:
            # /oauth login github
            return (
                f"To bind {target} OAuth, open:\n"
                f"  http://127.0.0.1:18923/oauth/start?provider={target}\n"
                f"Then run /bind {target} to complete."
            )
        elif sub == "status":
            return await self._handle_oauth_status(event)
        elif sub == "unlink" and target:
            return await self._handle_oauth_unlink(event, target)
        else:
            return "Usage: /oauth <login <provider> | status | unlink <provider>>"


    async def _handle_oauth_status(self, event: MessageEvent) -> str:
        """Show OAuth bindings for the current member."""
        source = event.source
        _cfg = _load_gateway_config()
        member_id = None
        try:
            from agent.runtime_context import resolve_member_id
            member_id, _ = resolve_member_id(
                platform=source.platform.value if source.platform else "cli",
                external_id=source.user_id or source.user_id_alt,
                config=_cfg, db=self._session_db,
            )
        ...
        try:
            from agent.membership import MembershipDB
            db = MembershipDB(config=_cfg)
            identities = db.list_identities(member_id)
            if not identities:
                return "No OAuth providers linked. Use /oauth login <provider> to link one."
            lines = ["Linked accounts:"]
            for ident in identities:
                provider = ident.get("provider", "").replace("oauth:", "")
                lines.append(f"  {provider}: {ident.get('email', ident.get('provider_id', '')[:20])}")
            return "\n".join(lines)
    ...
    async def _handle_oauth_unlink(self, event: MessageEvent, provider: str) -> str:
        """Unlink an OAuth provider from the current member."""
        source = event.source
        _cfg = _load_gateway_config()
        member_id = None
        try:
        ...
        try:
            from agent.membership import MembershipDB
            db = MembershipDB(config=_cfg)
            db.unbind_identity(f"oauth:{provider}", "", member_id)
            return f"Unlinked {provider}."
        except Exception as e:
            return f"Error: {e}"
        finally:
            if db:
    ...
    async def _handle_bind_command(self, event: MessageEvent) -> str:
        """Handle /bind <provider> — bind current platform identity to a member's OAuth."""
        args = event.get_command_args().strip()
        provider = args.split()[0] if args else ""
        if not provider:
            return "Usage: /bind <provider>  (e.g. /bind github)"

        source = event.source
        platform = source.platform.value if source.platform else "unknown"
        external_id = source.user_id or source.user_id_alt or ""

        if not external_id:
            return "Cannot bind: no platform user ID available."

        # Store binding metadata in identities
        _cfg = _load_gateway_config()
        db = None
        try:
            from agent.membership import MembershipDB
            ...
            if not member_id:
                return "Not logged in as a member."
            ...
            found = None
            for ident in identities:
                if ident.get("provider", "").endswith(provider):
                    found = ident
                    break
            if not found:
                return f"No {provider} OAuth linked yet. Link it first via CLI: intellect members bind <login> --provider {provider}"
            # Merge binding into existing metadata (don't overwrite)
            now = time.time()
            existing_meta = {}
            try:
                existing_meta = json.loads(found.get("metadata", "{}") or "{}")
            except (json.JSONDecodeError, TypeError):
                pass
            existing_meta[f"{platform}_id"] = external_id
            existing_meta["bound_via"] = "gateway"
            existing_meta["bound_at"] = now
            db._conn.execute(
                "UPDATE identities SET metadata=? WHERE provider=? AND member_id=?",
                (json.dumps(existing_meta), found["provider"], member_id),
            )
            db._conn.commit()
            return f"Bound {platform} to {provider} OAuth. Future messages on {platform} will be recognized as your account."
        except Exception as e:
            return f"Error: {e}"
        finally:
            if db:
    ...
    async def _handle_agents_command(self, event: MessageEvent) -> str:
        ...
        for session_key, agent in running_agents.items():
            ...
            is_pending = agent is _get_pending_sentinel()
            agent_rows.append(
                {
                    "session_key": session_key,
                    "elapsed": elapsed,
        ...
        if action in {"pause", "resume"}:
            ...
            if action == "pause":
                ...
                return (
                    ...
                    f"`intellect gateway restart` to reset."
                )
            # action == "resume"
            if platform not in failed:
                return (
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        try:
            ...
            atomic_json_write(
                _intellect_home / ".restart_notify.json",
                notify_data,
                indent=None,
            )
        except Exception as e:
        ...
        try:
            ...
            atomic_json_write(
                _intellect_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        from intellect_cli.commands import gateway_help_lines
        lines = [
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        user_provs = None
        custom_provs = None
        config_path = _intellect_home / "config.yaml"
        try:
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — Intellect default runtime
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return t("gateway.personality.none_configured", path=display_intellect_home())
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        ...
        try:
            import yaml as _y
            _cfg_path = _intellect_home / "config.yaml"
            if _cfg_path.exists():
    ...
    async def _handle_resume_command(self, event: MessageEvent) -> str:
        ...
        def _list_titled_sessions() -> list[dict]:
            ...
            # Resolve member_id for session isolation in multi-user mode.
            _mid = None
            try:
                from agent.runtime_context import resolve_member_id
                cfg = self.config if isinstance(self.config, dict) else _load_gateway_config()
                _mid, _ = resolve_member_id(
                    platform=user_source or "cli",
                    external_id=source.user_id or source.user_id_alt,
                    config=cfg,
                    db=self._session_db,
                )
        ...
        if name.isdigit():
            ...
            # Resolve member context so /resume can't load another member's session.
            _actor_mid = None
            try:
                from agent.runtime_context import resolve_member_id
                cfg = self.config if isinstance(self.config, dict) else _load_gateway_config()
                _actor_mid, _ = resolve_member_id(
                    platform=source.platform.value if source.platform else "cli",
                    external_id=source.user_id or source.user_id_alt,
                    config=cfg,
                    db=self._session_db,
    ...
    async def _handle_usage_command(self, event: MessageEvent) -> str:
        ...
        agent = self._running_agents.get(session_key)
        if not agent or agent is _get_pending_sentinel():
            _cache_lock = getattr(self, "_agent_cache_lock", None)
            _cache = getattr(self, "_agent_cache", None)
            if _cache_lock and _cache is not None:
                with _cache_lock:
        ...
        provider = getattr(agent, "provider", None) if agent and agent is not _get_pending_sentinel() else None
        base_url = getattr(agent, "base_url", None) if agent and agent is not _get_pending_sentinel() else None
        api_key = getattr(agent, "api_key", None) if agent and agent is not _get_pending_sentinel() else None
        ...
        if provider:
            try:
                from agent.account_usage import fetch_account_usage  # deferred import (~230ms)
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return (
                ...
                "  `intellect bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        full log uploads should use ``intellect debug share`` from the CLI.
        """
        import asyncio
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Intellect Agent to the latest version.

        Spawns ``intellect update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``intellect update`` may trigger. Marker
        ...
        if is_managed():
            return f"✗ {format_managed_message('update Intellect Agent')}"
        ...
        if not intellect_cmd:
            return t("gateway.update.intellect_cmd_not_found")
        ...
        # Spawn `intellect update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `intellect update --gateway`
        ...
        try:
            if sys.platform == "win32":
                ...
                # intellect_cmd is a list of argv parts we can pass directly
                ...
                intellect_cmd_str = " ".join(shlex.quote(part) for part in intellect_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {intellect_cmd_str} update --gateway"
