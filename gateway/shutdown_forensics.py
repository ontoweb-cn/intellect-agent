    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if intellect_home_str:
            takeover_path = Path(intellect_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(intellect_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded intellect-agent but never re-ran ``intellect setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../intellect-gateway.service"
    ...
    # that's the common case for intellect.
    ...
    # constant in intellect_cli/gateway.py.
    headroom = 30.0
