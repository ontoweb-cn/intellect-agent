"""Shared utility functions for intellect-agent."""
...
def atomic_replace(tmp_path: Union[str, Path], target: Union[str, Path]) -> str:
    ...
    ``~/.intellect/`` to a git-tracked profile package or dotfiles repo
    (GitHub #16743).
...
def atomic_text_write(
    ...
    *,
    ...
    """Write plain text atomically with optional permission mode.

    Uses temp file + fsync + :func:`atomic_replace`.  When *mode* is set the
    temp fd is ``fchmod``'d before writing so secret-bearing files never
    briefly inherit the process umask (TOCTOU between create and chmod).
    """
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
