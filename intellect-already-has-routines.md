# Intellect Agent Has Had "Routines" Since March
...
Intellect equivalent — works today:
```bash
...
Intellect equivalent — works today:
```bash
intellect webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Intellect implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Intellect Agent |
|---|---|---|
...
## Things Intellect Does That Routines Can't
...
Intellect has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Intellect Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
intellect gateway setup    # enable webhooks
intellect webhook subscribe pr-review \
  --events "pull_request" \
...
*Intellect Agent is built by [ONTOWEB](https://ontoweb.cn). Open source, model-agnostic, runs on your infrastructure.*
