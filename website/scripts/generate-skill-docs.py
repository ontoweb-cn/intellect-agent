...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://gitee.com/ontoweb/intellect-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    intellect_meta = (fm.get("metadata") or {}).get("intellect") or {}
    tags = intellect_meta.get("tags") or []
    related = intellect_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `intellect skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Intellect Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Intellect ships with a large built-in skill library copied into `~/.intellect/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Intellect also syncs bundled skills on `intellect update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.intellect/skills/` tree, it is still shipped with Intellect; restore it with `intellect skills reset <name> --restore`.",
        "",
