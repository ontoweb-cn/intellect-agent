...
description: "Frequently asked questions and solutions to common issues with Intellect Agent"
---
...
### What LLM providers work with Intellect?

Intellect Agent works with any OpenAI-compatible API. Supported providers include:

- **[OpenRouter](https://openrouter.ai/)** — access hundreds of models through one API key (recommended for flexibility)
- **[ONTOWEB Portal](/integrations/ontoweb-portal)** — ONTOWEB's subscription gateway — 300+ models plus web/image/TTS/browser through one OAuth login (recommended for newcomers)
- **OpenAI** — GPT-5.4, GPT-5-codex, GPT-4.1, GPT-4o, etc.
- **Anthropic** — Claude models (direct API, OAuth via `intellect auth add anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `intellect model` or by editing `~/.intellect/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.

### Does it work on Windows?

**Not natively.** Intellect Agent requires a Unix-like environment. On Windows, install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Intellect from inside it. The standard install command works perfectly in WSL2:

```bash
curl -fsSL https://raw.giteeusercontent.com/ontoweb/intellect-agent/raw/main/scripts/install.sh | bash
```

### I run Intellect in WSL2. What's the best way to control my normal Windows Chrome?

Prefer an MCP bridge over `/browser connect`.

Recommended pattern:

- run Intellect inside WSL2
...
- let Intellect use the resulting MCP browser tools

This is more reliable than trying to force Intellect core browser transport to attach directly across the WSL2/Windows boundary.

See:

- [Use MCP with Intellect](../guides/use-mcp-with-intellect.md#wsl2-bridge-intellect-in-wsl-to-windows-chrome)
...
Yes — Intellect now has a tested Termux install path for Android phones.
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Intellect Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.intellect/`.

### Can I use it offline / with local models?

Yes. Run `intellect model`, select **Custom endpoint**, and enter your server's URL:
...
# Context length: 64000   ← Intellect minimum; set this to match your server's actual context window
```
...
Intellect persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 64000`), make sure to set the matching context length in Intellect — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
Intellect auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `intellect_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

Intellect Agent itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Intellect Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Intellect programmatically:
...
#### `intellect: command not found` after installation
...
**Cause:** Intellect requires Python 3.11 or newer.
...
**Cause:** Intellect builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when Intellect runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** Intellect auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.intellect/config.yaml`:
...
sudo rm /usr/local/bin/intellect
...
**Solution:** Exit your session and use `intellect model` from your terminal to add new providers:

```bash
# Exit the Intellect chat session first (Ctrl+C or /quit)
...
After adding a new provider via `intellect model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `intellect model` (from terminal) |
| Enter/change API keys | `intellect model` (from terminal) |
...
intellect config show

# Re-configure your provider
intellect model

# Or set directly
intellect config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.intellect/.env` for conflicting entries.
:::
...
# Set a valid model
intellect config set intellect_MODEL anthropic/claude-opus-4.7

# Or specify per-session
intellect chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `intellect chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Intellect detected the wrong context length for your model.
...
intellect chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, Intellect may have the wrong context length for your model. Check what it detected:
...
**Cause:** Intellect detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Intellect never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `intellect chat`
...
cat ~/.intellect/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `intellect gateway setup`
- Check gateway logs: `cat ~/.intellect/logs/gateway.log | tail -50`
...
Configure in `~/.intellect/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
pip install "intellect-agent[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
#### WSL: Gateway keeps disconnecting or `intellect gateway start` fails
...
tmux new -s intellect 'intellect gateway run'
# Reattach later: tmux attach -t intellect

# Option 3: Background via nohup
nohup intellect gateway run > ~/.intellect/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'intellect gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `intellect gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
intellect gateway install    # Re-snapshots your current PATH
intellect gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.intellect.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `intellect chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `intellect chat -t "terminal"`
...
cd ~/.intellect/intellect-agent && uv pip install -e ".[mcp]"
...
Verify your `~/.intellect/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
intellect config show | grep -A 12 mcp_servers

# Restart Intellect or reload MCP after config changes
intellect chat
```

See also:
- [MCP (Model Context Protocol)](/user-guide/features/mcp)
- [Use MCP with Intellect](/guides/use-mcp-with-intellect)
...
:::warning
If an MCP server crashes mid-request, Intellect will report a timeout. Check the server's own logs (not just Intellect logs) to diagnose the root cause.
:::
...
`intellect update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `intellect update` once — it covers every profile on the machine.
...
**Solution: Delegation config.** Intellect can route subagents to a different model automatically. Set this in `~/.intellect/config.yaml`:
...
Now when you tell Intellect "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Intellect do this?

**Current limitation:** Intellect profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Intellect reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `intellect skills config` settings don't seem to take effect.

**Solution:** Use `intellect skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`intellect gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Intellect creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Intellect to another machine
...
1. Install Intellect Agent on the new machine:
   ```bash
   curl -fsSL https://raw.giteeusercontent.com/ontoweb/intellect-agent/raw/main/scripts/install.sh | bash
   ```
...
2. On the **source machine**, create a full backup:
   ...
   intellect backup
   ```
   This creates a zip of your entire `~/.intellect/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/intellect-backup-<timestamp>.zip`.

3. Copy the zip to the new machine and import it:
   ```bash
   # On the source machine
   scp ~/intellect-backup-<timestamp>.zip newmachine:~/

   # On the new machine
   intellect import ~/intellect-backup-<timestamp>.zip
   ```

4. On the new machine, run `intellect setup` to verify API keys and provider config are working.
...
```bash
rsync -av --exclude='intellect-agent' ~/.intellect/ newmachine:~/.intellect/
```

:::tip
`intellect backup` produces a consistent snapshot even while Intellect is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::

### Permission denied when reloading shell after install

**Scenario:** After running the Intellect installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not an Intellect-specific issue — it's a shell config permissions problem.
...
intellect config show | head -20

# Re-run model selection
intellect model

# Or test with a known-good model
intellect chat -q "hello" --model anthropic/claude-opus-4.7
```
...
1. **Search existing issues:** [GitHub Issues](https://gitee.com/ontoweb/intellect-agent/issues)
2. **Ask the community:** [ONTOWEB Discord](https://discord.gg/ontoweb)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Intellect version (`intellect --version`), and the full error message
