...
description: "How Intellect stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `intellect_cli/cron.py` | CLI `intellect cron` subcommands |
...
Jobs are stored in `~/.intellect/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `intellect cron` commands are run or during active CLI sessions.
...
```python
# ~/.intellect/scripts/check_competitors.py
...
2. **Environment variable** — `intellect_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.intellect/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `intellect cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `intellect cron` CLI provides direct job management:

```bash
intellect cron list                    # Show all jobs
intellect cron create                  # Interactive job creation (alias: add)
intellect cron edit <job_id>           # Edit job configuration
intellect cron pause <job_id>          # Pause a running job
intellect cron resume <job_id>         # Resume a paused job
intellect cron run <job_id>            # Trigger immediate execution
intellect cron remove <job_id>         # Delete a job
```
