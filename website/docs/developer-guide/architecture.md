...
sidebar_position: 1
title: "Architecture"
description: "Intellect Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Intellect Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ intellect_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # IntellectCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── intellect_state.py           # SQLite session/state database with FTS5
├── intellect_constants.py       # INTELLECT_HOME, profile-aware paths
...
│
├── intellect_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `intellect` subcommands (large file)
...
│   ├── skills_config.py      # intellect skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # intellect tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # intellect gateway start/stop
│
...
```text
User input → IntellectCLI.process_input()
...
Three discovery sources: `~/.intellect/plugins/` (user), `.intellect/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `intellect plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-an-intellect-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Intellect as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`intellect -p <name>`) gets its own INTELLECT_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
