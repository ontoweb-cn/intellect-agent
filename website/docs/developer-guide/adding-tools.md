...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Intellect Agent — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in Intellect tool** to the repository itself.
...
2. **`toolsets.py`** — add tool name to `_INTELLECT_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `intellect_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `intellect_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `intellect chat -q "Use the weather tool for London"`
