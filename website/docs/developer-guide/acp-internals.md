...
The ACP adapter wraps Intellect' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
intellect acp / intellect-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.intellect/.env
  -> configure stderr logging
  -> construct intellectACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'intellect-agent[acp]==<version>' intellect-acp`, pointed at the `intellect-agent` PyPI release.
...
- `allow_once` -> Intellect `once`
- `allow_always` -> Intellect `always`
- reject options -> Intellect `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Intellect tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["intellect-acp"])
...
Instead it reuses Intellect' runtime resolver:

- `acp_adapter/auth.py`
- `intellect_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Intellect provider/credentials. It also always advertises a terminal setup auth method (`intellect-setup`, args `--setup`) so first-run registry clients can open Intellect' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.intellect/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `intellect-acp` toolset definition
- `intellect_cli/main.py` — `intellect acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `intellect-acp` script
