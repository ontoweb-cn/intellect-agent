...
description: "Diagnose and fix common Intellect cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`intellect gateway` for foreground, or `intellect gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `intellect cron tick`.
...
```bash
date
intellect cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.intellect/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.intellect/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.intellect/.env` |
...
| `local` | Write access to `~/.intellect/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `intellect cron list` for updated `last_error` field (if available).
...
```bash
intellect skills list
```

Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `intellect skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `intellect skills list`.
...
2. `~/.intellect/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `intellect cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Intellect config directory). Verify:
```bash
ls ~/.intellect/scripts/your-script.py   # Must exist
intellect cron edit <job_id> --script ~/.intellect/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `intellect skills install <skill-name>`.
...
```bash
ps aux | grep intellect
# Kill duplicate processes, keep only one
```

### Check 4: Permissions on jobs.json

Jobs are stored in `~/.intellect/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.intellect/cron/jobs.json
chmod 600 ~/.intellect/cron/jobs.json   # Your user should own it
```
...
```bash
intellect cron list                    # Show all jobs, states, next_run times
intellect cron run <job_id>            # Schedule for next tick (for testing)
intellect cron edit <job_id>           # Fix configuration issues
intellect logs                         # View recent Intellect logs
intellect skills list                  # Verify installed skills
```
...
1. Run the job with `intellect cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.intellect/logs/agent.log` for scheduler messages and `~/.intellect/logs/errors.log` for warnings
3. Open an issue at [gitee.com/ontoweb/intellect-agent](https://gitee.com/ontoweb/intellect-agent) with:
