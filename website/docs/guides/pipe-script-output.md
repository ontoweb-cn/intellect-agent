...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `intellect send`."
---

# Pipe Script Output to Messaging Platforms

`intellect send` is a small, scriptable CLI that pushes a message to any
messaging platform Intellect is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | intellect send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `intellect
...
intellect send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
intellect send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
intellect send --list

# Filter by platform
intellect send --list telegram
```
...
Any platform Intellect ships adapters for works as a target:
...
`intellect send` resolves the message body in this order:

1. **Positional argument** — `intellect send --to telegram "hi"`
2. **`--file PATH`** — `intellect send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | intellect send --to telegram`

When stdin is a TTY (no pipe), Intellect does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  intellect send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `intellect send` reuses your Intellect config, the same script works on
any host where Intellect is installed — no need to export bot tokens into
...
`intellect send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  intellect send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | intellect send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.intellect/bin/intellect send \
...
```bash
./train.py --epochs 200 && \
  intellect send --to telegram "training done" || \
  intellect send --to telegram "training failed (exit $?)"
```
...
intellect send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(intellect send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `intellect send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `intellect send` calls
the platform's REST endpoint directly using credentials from
`~/.intellect/.env` and `~/.intellect/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `intellect gateway start` and retry.
...
# Machine-readable
intellect send --list --json
```

The listing is built from `~/.intellect/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`intellect gateway
...
| Approach | Multi-platform | Reuses Intellect creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `intellect send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`intellect send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `intellect send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `intellect send` shares with cron delivery.
