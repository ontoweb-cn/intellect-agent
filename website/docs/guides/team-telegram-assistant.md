...
This tutorial walks you through setting up a Telegram bot powered by Intellect Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Intellect Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.intellect/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Intellect itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Intellect Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_intellect_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Intellect Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.intellect/.env`:
...
```
[Gateway] Starting Intellect Gateway...
...
# View live logs
journalctl --user -u intellect-gateway -f
...
intellect gateway start
intellect gateway stop
tail -f ~/.intellect/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `intellect gateway install` to update the plist.
:::
...
```bash
intellect gateway stop && intellect gateway start
```
...
intellect pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.intellect/.env`:
...
Control how much detail the bot shows when using tools. In `~/.intellect/config.yaml`:
...
Customize how the bot communicates by editing `~/.intellect/SOUL.md`:

For a full guide, see [Use SOUL.md with Intellect](/guides/use-soul-with-intellect).
...
```bash
# From the CLI
intellect cron list          # View all scheduled jobs
intellect cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u intellect-gateway` (Linux) or `~/.intellect/logs/gateway.log` (macOS) |
| Cron job output | `~/.intellect/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.intellect/cron/jobs.json` |
| Pairing data | `~/.intellect/pairing/` |
| Session history | `~/.intellect/sessions/` |
