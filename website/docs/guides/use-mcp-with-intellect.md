---
sidebar_position: 6
title: "Use MCP with Intellect"
description: "A practical guide to connecting MCP servers to Intellect Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Intellect

This guide shows how to actually use MCP with Intellect Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Intellect tool
- you want Intellect to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Intellect to internal APIs, databases, or company systems without modifying Intellect core

Do not use MCP when:
- a built-in Intellect tool already solves the job well
...
- Intellect remains the agent
- MCP servers contribute tools
- Intellect discovers those tools at startup or reload time
...
If you installed Intellect with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Intellect banner/status should show MCP integration when configured
- ask Intellect what tools it has available
...
## WSL2: bridge Intellect in WSL to Windows Chrome

This is the practical setup when:

- Intellect runs inside WSL2
...
In this setup, Intellect does **not** connect to Chrome directly. Instead:

- Intellect runs in WSL
- Intellect starts a local stdio MCP server
...
```text
Intellect (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Intellect stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Intellect core browser transport
...
```bash
intellect mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
intellect mcp test chrome-devtools-win
```

Then start a fresh Intellect session or run:
...
Once loaded, Intellect can use the MCP-prefixed browser tools directly. For example:
...
If Intellect runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Intellect from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Intellect from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Intellect:
...
2. Intellect-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Intellect will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Intellect to reason over a bounded workspace.
...
Now Intellect can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Intellect core.
...
Because Intellect now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
