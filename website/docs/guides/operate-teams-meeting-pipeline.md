...
```bash
intellect teams-pipeline validate
```
...
```bash
intellect teams-pipeline token-health
intellect teams-pipeline token-health --force-refresh
```
...
```bash
intellect teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
intellect teams-pipeline maintain-subscriptions
intellect teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Intellect cron (recommended if you already run the Intellect gateway)

Intellect ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.intellect/scripts/`. First create the script:

```bash
mkdir -p ~/.intellect/scripts
cat > ~/.intellect/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec intellect teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.intellect/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
intellect cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/intellect-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Intellect Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=intellect
EnvironmentFile=/etc/intellect/env
ExecStart=/usr/local/bin/intellect teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/intellect-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Intellect Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now intellect-teams-pipeline-maintain.timer
systemctl list-timers intellect-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/intellect teams-pipeline maintain-subscriptions >> /var/log/intellect/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.intellect/.env` at the top of a wrapper script that crontab calls.
...
```bash
intellect teams-pipeline subscriptions   # should show expirationDateTime advanced
intellect teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
intellect teams-pipeline list
intellect teams-pipeline list --status failed
intellect teams-pipeline show <job-id>
```

### Replay a stored job

```bash
intellect teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
intellect teams-pipeline fetch --meeting-id <meeting-id>
intellect teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `intellect teams-pipeline maintain-subscriptions --dry-run`
- inspect `intellect teams-pipeline list --status failed`
...
- run `intellect teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `intellect teams-pipeline run`
...
- [ ] `intellect teams-pipeline validate` returns an OK snapshot
- [ ] `intellect teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Intellect cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
