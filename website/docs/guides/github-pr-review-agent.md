...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Intellect Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Intellect instantly when PRs are opened or updated.
:::
...
- **Intellect Agent installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  intellect gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.intellect/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Intellect can access GitHub. Start a chat:
...
```
Run: gh pr list --repo ONTOWEB/intellect-agent --state open --limit 3
```
...
Still in the chat, ask Intellect to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo ONTOWEB/intellect-agent
```
...
A skill gives Intellect consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.intellect/skills/code-review
```

Create `~/.intellect/skills/code-review/SKILL.md`:
...
Verify it loaded — start `intellect` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Intellect your team's standards:
...
2. Teach Intellect your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
intellect gateway status    # Is the gateway running?
intellect cron list         # Is the job enabled?
```
...
- **[Build a Plugin](/guides/build-an-intellect-plugin)** — wrap the review logic into a shareable plugin
