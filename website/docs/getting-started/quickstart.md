...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Intellect Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Intellect setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Intellect Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Intellect Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Intellect for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Intellect working on my machine | `intellect setup` | Run a real chat and verify it responds |
| I already know my provider | `intellect model` | Save the config, then start chatting |
| I want a bot or always-on setup | `intellect gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `intellect model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `intellect model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Intellect cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Intellect Agent

**Option A — pip (simplest):**

```bash
pip install intellect-agent
intellect postinstall     # optional: installs Node.js, browser, ripgrep, ffmpeg + runs setup
```
...
The single most important setup step. Use `intellect model` to walk through the choice interactively:

```bash
intellect model
```

:::tip Easiest path: ONTOWEB Portal
...
That logs you in, sets ONTOWEB as your provider, and turns on the Tool Gateway in one command.
:::
...
|----------|-----------|---------------|
| **ONTOWEB Portal** | Subscription-based, zero-config | OAuth login via `intellect model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `intellect model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `intellect model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `intellect_cli/models.py` may change between releases) | `intellect model` → MiniMax (OAuth) |
...
| **Google Gemini (OAuth)** | Gemini via the `google-gemini-cli` OAuth flow — no key needed | `intellect model` → Google Gemini (OAuth) |
| **xAI** | Grok models via direct API | Set `XAI_API_KEY` |
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `intellect model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `intellect model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `intellect model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `intellect model` (requires `copilot` CLI + `copilot login`) |
...
Intellect Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `intellect model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Intellect separates secrets from normal config:

- **Secrets and tokens** → `~/.intellect/.env`
- **Non-secret settings** → `~/.intellect/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
intellect config set model anthropic/claude-opus-4.6
intellect config set terminal.backend docker
intellect config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
intellect            # classic CLI
intellect --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Intellect ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `intellect` vs `intellect --tui`.
:::
...
- Intellect replies without error
...
```bash
intellect --continue    # Resume the most recent session
intellect -c            # Short form
```
...
```bash
intellect gateway setup    # Interactive platform configuration
```
...
- `intellect tools` — tune tool access per platform
- `intellect skills` — browse and install reusable workflows
...
```bash
intellect config set terminal.backend docker    # Docker isolation
intellect config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the Intellect install directory (the curl installer placed it at
# ~/.intellect/intellect-agent on Linux/macOS or %LOCALAPPDATA%\intellect\intellect-agent on Windows):
...
```bash
intellect skills search kubernetes
...
(If you installed without `[all]`, run `cd ~/.intellect/intellect-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Intellect opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `intellect model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `intellect gateway setup` and check `intellect gateway status` |
| `intellect --continue` can't find old session | Switched profiles or session never saved | Check `intellect sessions list` and confirm you're in the right profile |
...
| `intellect doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `intellect model`
3. `intellect setup`
4. `intellect sessions list`
5. `intellect --continue`
6. `intellect gateway status`
...
| `intellect tools` | Configure which tools are enabled per platform |
| `intellect setup` | Full setup wizard (configures everything at once) |
| `intellect doctor` | Diagnose issues |
| `intellect update` | Update to latest version |
| `intellect gateway` | Start the messaging gateway |
| `intellect --continue` | Resume last session |
