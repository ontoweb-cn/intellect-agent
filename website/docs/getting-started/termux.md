...
description: "Run Intellect Agent directly on an Android phone with Termux"
---

# Intellect on Android with Termux

This is the tested path for running Intellect Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Intellect from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
- links `intellect` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `intellect` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/intellect" "$PREFIX/bin/intellect"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `intellect` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.intellect/.env`.
...
### `intellect doctor` says ripgrep or Node is missing
