...
title: "Git Worktrees"
description: "Run multiple Intellect agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Intellect Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Intellect so each session has a clean, isolated working directory.

## Why Use Worktrees with Intellect?

Intellect treats the **current working directory** as the project root:

- CLI: the directory where you run `intellect` or `intellect chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.intellect/config.yaml`
...
git worktree add ../repo-feature feature/intellect-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/intellect-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Intellect there:

```bash
cd ../repo-feature

# Start Intellect in the worktree
intellect
```
...
git worktree add ../repo-experiment-a feature/intellect-a
git worktree add ../repo-experiment-b feature/intellect-b
```
...
Each Intellect process:

- Works on its own branch (`feature/intellect-a` vs `feature/intellect-b`).
...
- Intellect checkpoint data under `~/.intellect/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Intellect experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/intellect-checkpoints-docs`, `feature/intellect-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Intellect from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `intellect -w` (Automatic Worktree Mode)

Intellect has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `intellect/intellect-<hash>`).
...
```bash
intellect -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `intellect -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Intellect session its own clean checkout.
