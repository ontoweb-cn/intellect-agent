...
A **profile distribution** packages a complete Intellect agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./agents.md) is a local agent, a distribution is that agent made shareable.

## What this means

Before distributions, sharing an Intellect agent meant sending someone:
...
The tradeoff: recipients need git installed. On any machine running Intellect in 2026, that's already true.
...
- **You just want to back up a profile on your own machine.** Use [`intellect agent export` / `import`](../reference/agent-commands.md#intellect-profile-export) — that's what those are for.
...
You built Intellect-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
For implementation details, precise CLI behavior, and all flags, see the [Profile Commands reference](../reference/agent-commands.md#distribution-commands).
...
- Reserved profile names (`intellect`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.

## See also

- [Profiles: Running Multiple Agents](./agents.md) — the base concept
- [Profile Commands reference](../reference/agent-commands.md) — every flag, every option
- [`intellect agent export` / `import`](../reference/agent-commands.md#intellect-profile-export) — local backup / restore (not distribution)
