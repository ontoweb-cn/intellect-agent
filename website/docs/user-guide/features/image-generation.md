...
description: Generate images via FAL.ai — 11 models including FLUX 2, GPT Image (1.5 & 2), Nano Banana Pro, Ideogram, Recraft V4 Pro, Krea 2, and more, selectable via `intellect tools`.
...
Intellect Agent generates images from text prompts via FAL.ai. Eleven models are supported out of the box, each with different speed, quality, and cost tradeoffs. The active model is user-configurable via `intellect tools` and persists in `config.yaml`.
...
If you have a paid [ONTOWEB Portal](https://portal.ontoweb.cn) subscription, you can use image generation through the **[Tool Gateway](tool-gateway.md)** without a FAL API key. Your model selection persists across both paths. New installs can run `intellect setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **OntoWeb Subscription** as the image-gen backend via `intellect tools`.
...
Navigate to **🎨 Image Generation**, pick your backend (OntoWeb Subscription or FAL.ai), then the picker shows all supported models in a column-aligned table — arrow keys to navigate, Enter to select:
...
image_gen:
  ...
  use_gateway: false            # true if using OntoWeb Subscription
```

### GPT-Image Quality

The `fal-ai/gpt-image-1.5` and `fal-ai/gpt-image-2` request quality is pinned to `medium` (~$0.034–$0.06/image at 1024×1024). We don't expose the `low` / `high` tiers as a user-facing option so that ONTOWEB Portal billing stays predictable across all users — the cost spread between tiers is 3–22×. If you want a cheaper option, pick Klein 9B or Z-Image Turbo; if you want higher quality, use Nano Banana Pro or Recraft V4 Pro.
...
3. **Submission** — `_submit_fal_request()` routes via direct FAL credentials or the managed OntoWeb gateway.
...
- **Requires FAL credentials** (direct `FAL_KEY` or OntoWeb Subscription)
