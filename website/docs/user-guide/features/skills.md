...
All skills live in **`~/.intellect/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Intellect at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Intellect to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.intellect/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
intellect chat --toolsets skills -q "What skills do you have?"
intellect chat --toolsets skills -q "Show me the axolotl skill"
```
...
/home/user/.intellect/cache/chart-q4-2025.png
...
When a missing value is encountered, Intellect asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `intellect setup` or `~/.intellect/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `intellect config migrate` prompts for unconfigured settings, and `intellect config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.intellect/skills/                  # Single source of truth
...
If you maintain skills outside of Intellect — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Intellect to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.intellect/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.intellect/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the Intellect process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Intellect ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.intellect/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.intellect/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `intellect bundles list`.
...
intellect bundles create research

# Overwrite an existing bundle
intellect bundles create backend-dev --skill ... --force

# Delete a bundle
intellect bundles delete backend-dev

# Re-scan ~/.intellect/skill-bundles/ and report changes
intellect bundles reload
```
...
- **Bundles work in every surface** — interactive CLI, TUI, and every gateway platform (Telegram, Discord, Slack, …) — because dispatch is centralized in the same place as individual skill commands.
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.intellect/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.intellect/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
```bash
intellect skills browse                              # Browse all hub skills (official first)
intellect skills browse --source official            # Browse only official optional skills
intellect skills search kubernetes                   # Search all sources
intellect skills search react --source skills-sh     # Search the skills.sh directory
intellect skills search https://mintlify.com/docs --source well-known
intellect skills inspect openai/skills/k8s           # Preview before installing
intellect skills install openai/skills/k8s           # Install with security scan
intellect skills install official/security/1password
intellect skills install skills-sh/vercel-labs/json-render/json-render-react --force
intellect skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
intellect skills list --source hub                   # List hub-installed skills
intellect skills check                               # Check installed hub skills for upstream updates
intellect skills update                              # Reinstall hub skills with upstream changes when needed
intellect skills audit                               # Re-scan all hub skills for security
intellect skills uninstall k8s                       # Remove a hub skill
intellect skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
intellect skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
intellect skills publish skills/my-skill --to github --repo owner/repo
intellect skills snapshot export setup.json          # Export skill config
intellect skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Intellect. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `intellect skills search <query> --source skills-sh`. Intellect resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Intellect currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Intellect repository itself and install with built-in trust.
...
This is Vercel's public skills directory. Intellect can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
intellect skills search https://mintlify.com/docs --source well-known
intellect skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
intellect skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Intellect can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
intellect skills install openai/skills/k8s
intellect skills tap add myorg/skills-repo
```
...
[Skills Hub](https://intellect-agent.ontoweb.cn/docs) page — instead of a
tag-derived guess. This is generic: any tap that ships the file gets real
categorization, no Intellect-side changes required.
...
- Intellect source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

Intellect supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

Intellect source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

Intellect can search and convert agent entries from LobeHub's public catalog into installable Intellect skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Intellect source id: `lobehub`

#### 8. browse.sh (`browse-sh`)

Intellect integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Intellect' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- Intellect source id: `browse-sh`
- Trust level: `community`

```bash
intellect skills search airbnb --source browse-sh
intellect skills inspect browse-sh/airbnb.com/search-listings-ddgioa
intellect skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Intellect fetches the URL, parses the YAML frontmatter, security-scans it, and installs.

- Intellect source id: `url`
...
```bash
intellect skills install https://sharethis.chat/SKILL.md
intellect skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
intellect skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `intellect skills update` re-fetches from the same URL automatically when you want to refresh.
...
`intellect skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Intellect | Always trusted |
...
```bash
intellect skills check          # Report which installed hub skills changed upstream
intellect skills update         # Reinstall only the skills with updates available
intellect skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Intellect users add with `intellect skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.intellect.tags`, `version`, `author`, `platforms`, `metadata.intellect.config`).
...
Intellect discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.

#### Minimal tap example

```
my-org/intellect-skills
└── skills/
    └── deploy-runbook/
        └── SKILL.md
```
...
After pushing that to GitHub, any Intellect user can subscribe and install:

```bash
intellect skills tap add my-org/intellect-skills
intellect skills search deploy
intellect skills install my-org/intellect-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.intellect/.hub/taps.json`:
...
The `intellect skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `intellect skills tap list` shows the effective path per tap.
...
```bash
intellect skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.intellect/.hub/taps.json` (created on demand).

## Bundled skill updates (`intellect skills reset`)

Intellect ships with a set of bundled skills in `skills/` inside the repo. On install and on every `intellect update`, a sync pass copies those into `~/.intellect/skills/` and records a manifest at `~/.intellect/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Intellect recomputes the hash of your local copy and compares it to the origin hash:
...
intellect skills reset google-workspace --restore
