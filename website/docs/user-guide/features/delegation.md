...
If a subagent times out having made **zero** API calls (usually: provider unreachable, auth failure, or tool-schema rejection), `delegate_task` writes a structured diagnostic to `~/.intellect/logs/subagent-timeout-<session>-<timestamp>.log` containing the subagent's config snapshot, credential-resolution trace, and any early error messages. Much easier to root-cause than the previous silent-timeout behavior.
:::
...
## Session identity (`intellect_SESSION_ID`)

Each agent instance has its own `session_id` in SQLite and in logs. Subagents are constructed with **`publish_session_context=False`** so the parent process is not switched to the child's id on the main thread during child setup.

When a subagent runs, its session id is published in the **worker thread** only. When delegation finishes (or a worker exits), the parent's `intellect_SESSION_ID` is restored (ContextVar + `os.environ`).

| Runtime | How tools should read the active session |
|---------|------------------------------------------|
| Gateway (async) | `get_session_env("intellect_SESSION_ID")` — task-local ContextVar |
| CLI / sync tools | Same helper; falls back to `os.environ` when no ContextVar is set |
| Subprocess shells (`terminal`) | Inherit merged env; under concurrent batch delegation, prefer in-process tools using `get_session_env` |

Library code should use `from gateway.session_context import get_session_env` instead of raw `os.environ.get("intellect_SESSION_ID")`.

For temporary overrides in synchronous code, use `session_id_scope(session_id)` from the same module.

:::note Parallel batch caveat
With `tasks=[...]` and multiple concurrent children, `os.environ` is still process-global. Each worker sets the ContextVar for its child; shell commands that only read `$intellect_SESSION_ID` from the environment may see whichever child last wrote the global env. In-process tools that use `get_session_env` see the correct id for their thread.
:::
