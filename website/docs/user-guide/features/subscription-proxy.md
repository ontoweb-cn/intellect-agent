...
description: "Use your ONTOWEB Portal subscription (or other OAuth provider) as an OpenAI-compatible endpoint for external apps"
---
...
chat completions — use your Intellect-managed provider subscription as their
...
| Use case | "Use Intellect as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the ONTOWEB Portal OAuth flow. Intellect stores
the refresh token in `~/.intellect/auth.json` — the same place all Intellect
...
Model:      Intellect-4-70B    # or Intellect-4.3-36B, Intellect-4-405B
```
...
Currently shipped: `ontoweb` (ONTOWEB Portal) and `xai` (xAI / Grok). More
OAuth providers can be added by implementing the `UpstreamAdapter`
interface in `intellect_cli/proxy/adapters/`.
...
If you see `not logged in`, run `intellect auth add ontoweb`. If you see
...
The proxy only forwards paths the upstream actually serves. For OntoWeb
Portal:
...
supports; check `portal.ontoweb.cn/models`.
...
[portal.ontoweb.cn](https://portal.ontoweb.cn).
...
`intellect_cli/proxy/adapters/<provider>.py` and registering it in
