...
- **Plugin / integration authors** wanting to add a new lane shape (a CLI worker that wraps Codex / Claude Code / OpenCode, a containerised review worker, a non-Intellect service that pulls tasks via the API).

If you're writing the worker code itself — the agent that runs *inside* a lane — the [`kanban-worker`](https://gitee.com/ontoweb/intellect-agent/blob/main/skills/devops/kanban-worker/SKILL.md) skill is the deeper procedural detail.

## The hierarchy

```text
Intellect Kanban  =  canonical task lifecycle + audit trail
...
Intellect Kanban owns lifecycle truth — `ready` → `running` → `blocked` / `done` / `archived`. Worker lanes execute work but never own that truth; everything they do flows back through the kanban kernel via the `kanban_*` tools (or, for non-Intellect external workers, via the API). Reviewers gate the transition from "code change written" to "task done."
...
The dispatcher matches `task.assignee` against either a Intellect profile name (the default lane shape) or a registered non-spawnable identifier (the plugin lane shape — see [Adding an external CLI worker lane](#adding-an-external-cli-worker-lane) below). Tasks whose assignee doesn't resolve are left on `ready` with a `skipped_nonspawnable` event so a board operator can fix them; they are not silently dropped or executed by an arbitrary fallback.
...
| Variable | Carries |
|---|---|
| `INTELLECT_KANBAN_TASK` | the task id the worker is operating on |
| `intellect_KANBAN_DB` | absolute path to the per-board SQLite file |
| `INTELLECT_KANBAN_BOARD` | board slug |
| `intellect_KANBAN_WORKSPACES_ROOT` | root of the board's workspace tree |
| `intellect_KANBAN_WORKSPACE` | absolute path to *this* task's workspace |
| `intellect_KANBAN_RUN_ID` | the current run's id (for the lifecycle gate) |
| `intellect_KANBAN_CLAIM_LOCK` | the claim lock string (`<host>:<pid>:<uuid>`) |
| `intellect_PROFILE` | the worker's own profile name (for `kanban_comment` author attribution) |
| `intellect_TENANT` | tenant namespace, if the task has one |

For non-Intellect lanes (registered via a plugin), the plugin supplies its own `spawn_fn` callable that gets `task`, `workspace`, and `board` and returns an optional pid for crash detection.
...
- **Block instead of complete**, with `reason` prefixed `review-required: ` so `intellect kanban show` surfaces the row as awaiting review.
...
The [`kanban-worker`](https://gitee.com/ontoweb/intellect-agent/blob/main/skills/devops/kanban-worker/SKILL.md) skill has worked examples for both `kanban_complete` (truly terminal tasks — typo fixes, docs changes, research writeups) and the `review-required` block pattern.
...
- `kanban_show` returns both, so a reviewer (or a follow-up worker) reading the task gets the full history.

CLI users can run `intellect kanban tail <task_id>` to follow live, or `intellect kanban runs <task_id>` for the historical attempt list.
...
The historical issue for this is [#19931](https://gitee.com/ontoweb/intellect-agent/issues/19931) and the closed-not-merged Codex-specific PR [#19924](https://gitee.com/ontoweb/intellect-agent/pull/19924) — those describe the original architecture proposal but didn't land a runner.
...
- **Stranded-task detection** — a ready task whose assignee never produces a claim within `kanban.stranded_threshold_seconds` (default 30 min) shows up in `intellect kanban diagnostics` as a `stranded_in_ready` warning. Severity escalates to error at 2x the threshold and critical at 6x. Catches typo'd assignees, deleted profiles, and down external worker pools in one signal — identity-agnostic, no per-board allowlist to curate.

## Related

- [Kanban overview](./kanban) — the user-facing intro.
- [Kanban tutorial](./kanban-tutorial) — step-by-step walkthrough.
- [`kanban-worker`](https://gitee.com/ontoweb/intellect-agent/blob/main/skills/devops/kanban-worker/SKILL.md) — the skill the worker process loads.
- [`kanban-orchestrator`](https://gitee.com/ontoweb/intellect-agent/blob/main/skills/devops/kanban-orchestrator/SKILL.md) — the orchestrator side.
