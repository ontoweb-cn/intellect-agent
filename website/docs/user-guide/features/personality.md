...
description: "Customize Intellect Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Intellect Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `INTELLECT_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Intellect is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Intellect now seeds a default `SOUL.md` automatically in:

```text
~/.intellect/SOUL.md
```

More precisely, it uses the current instance's `INTELLECT_HOME`, so if you run Intellect with a custom home directory, it will use:

```text
$INTELLECT_HOME/SOUL.md
```

### Important behavior

- **SOUL.md is the agent's primary identity.** It occupies slot #1 in the system prompt, replacing the hardcoded default identity.
- Intellect creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Intellect loads `SOUL.md` only from `INTELLECT_HOME`
- Intellect does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Intellect falls back to a built-in default identity
...
If Intellect loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `INTELLECT_HOME`, the personality belongs to the Intellect instance itself.

That also makes it easier to teach users:
- "Edit `~/.intellect/SOUL.md` to change Intellect' default personality."
...
- how Intellect should handle uncertainty, disagreement, or ambiguity
...
## What Intellect injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Intellect falls back to a built-in default identity ("You are Intellect Agent, an intelligent AI assistant created by ONTOWEB..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Intellect ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Intellect, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Intellect its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.intellect/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.intellect/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-intellect)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Intellect speaks
- `display.skin` and `/skin` affect how Intellect looks in the terminal
