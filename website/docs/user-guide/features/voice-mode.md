...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Intellect Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Intellect Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Intellect](/guides/use-voice-mode-with-intellect).

## Prerequisites

Before using voice features, make sure you have:

1. **Intellect Agent installed** — `pip install intellect-agent` (see [Installation](/getting-started/installation))
2. **An LLM provider configured** — run `intellect model` or set your preferred provider credentials in `~/.intellect/.env`
3. **A working base setup** — run `intellect` to verify the agent responds to text before enabling voice

:::tip
The `~/.intellect/` directory and default `config.yaml` are created automatically the first time you run `intellect`. You only need to create `~/.intellect/.env` manually for API keys.
:::

:::tip ONTOWEB Portal covers both
A paid [ONTOWEB Portal](/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `intellect setup --portal` wires both up at once.
:::
...
pip install "intellect-agent[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
pip install "intellect-agent[messaging]"

# Premium TTS (ElevenLabs)
pip install "intellect-agent[tts-premium]"
...
Voice mode is available in both the **classic CLI** (`intellect chat`) and the **TUI** (`intellect --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.intellect/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
intellect                # Start the interactive CLI
```
...
1. Start the CLI with `intellect` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.intellect/config.yaml` (default: `ctrl+b`).
:::
...
```bash
intellect gateway        # Start the gateway (connects to configured platforms)
intellect gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@intellectbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.intellect/.env`:
```bash
...
```bash
intellect gateway        # Start with existing configuration
```
...
If you are running Intellect inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.intellect/.env`
...
- Review gateway logs: `tail -f ~/.intellect/logs/gateway.log`
