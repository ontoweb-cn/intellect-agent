...
When Intellect Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.intellect/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.intellect/config.yaml` (switchable per session
via `/personality`).
...
If you use Intellect' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.intellect/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Intellect Agent's deliverable
