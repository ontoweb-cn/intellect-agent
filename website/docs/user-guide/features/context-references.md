...
Type `@` followed by a reference to inject content directly into your message. Intellect expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Intellect env: `$INTELLECT_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$INTELLECT_HOME/skills/.hub/`
