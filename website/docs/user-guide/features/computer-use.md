...
Intellect Agent can drive your Mac's desktop — clicking, typing, scrolling,
...
Use `intellect computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `intellect tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
4. Start a session with the toolset enabled:
   ...
   ```
   or add `computer_use` to your enabled toolsets in `~/.intellect/config.yaml`.

## Keeping cua-driver up to date

The cua-driver project ships fixes regularly (e.g. v0.1.6 fixed a Safari
window-focus bug for UTM workflows). Intellect refreshes the binary in two
places so you don't get stuck on a stale release:

- **`intellect update`** — when you update Intellect itself, if `cua-driver` is
...
- **`intellect computer-use install --upgrade`** — manual force-refresh.
...
`intellect computer-use status` shows the installed version next to the
binary path.
...
Intellect applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.intellect/config.yaml` if you want every action confirmed.

## Token efficiency

Screenshots are expensive. Intellect applies four layers of optimisation:
...
- **Private SPI risk.** Apple can change SkyLight's symbol surface in any
  OS update. Pin the driver version with the `intellect_CUA_DRIVER_VERSION`
...
`intellect computer-use install` to fetch the cua-driver binary, or run
`intellect tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://gitee.com/ontoweb/intellect-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
