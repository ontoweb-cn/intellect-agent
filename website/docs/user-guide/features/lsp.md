...
Intellect runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Intellect
...
1. Intellect captures a baseline of current diagnostics for the file.
...
…). Intellect auto-detects the binary on PATH or in
`<INTELLECT_HOME>/lsp/bin/`.
...
`node_modules` tree — Intellect installs both packages together when you
run `intellect lsp install typescript` or auto-install fires on first
use.

## CLI

```
intellect lsp status          # service state + per-server install status
intellect lsp list            # registry, optionally --installed-only
intellect lsp install <id>    # eagerly install one server
intellect lsp install-all     # try every server with a known recipe
intellect lsp restart         # tear down running clients
intellect lsp which <id>      # print resolved binary path
```

`intellect lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <INTELLECT_HOME>/lsp/bin
...
When `install_strategy: auto`, Intellect installs binaries into
`<INTELLECT_HOME>/lsp/bin/`. NPM packages land in
`<INTELLECT_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Intellect-owned and is
...
Servers are kept alive for the life of the Intellect process. There's
...
**`intellect lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<INTELLECT_HOME>/lsp/bin/`. Run
`intellect lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `intellect lsp status`**
...
When `intellect lsp status` shows a `Backend warnings` section, install
...
`~/.intellect/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.intellect/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `intellect lsp restart` to clear the set;
