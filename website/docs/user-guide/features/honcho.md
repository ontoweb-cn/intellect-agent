...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Intellect's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Intellect instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
intellect memory setup    # select "honcho" from the provider list
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$INTELLECT_HOME/honcho.json` (profile-local). The setup wizard handles this for you.

### Self-Hosted Honcho with Authentication

When pointing Intellect at a self-hosted Honcho server, `intellect honcho setup` (and `intellect memory setup`) ask for a **local JWT / bearer token** after the base URL. Paste a JWT signed with the server's `AUTH_JWT_SECRET` (the Honcho compose env var) to enable authenticated access; leave it blank for servers running with `AUTH_USE_AUTH=false`. The local token is stored under the host block (`hosts.<host>.apiKey` in `honcho.json`), separate from any cloud `apiKey`, so you can flip the `Cloud or local?` prompt back to `cloud` later without losing either credential.
...
- `per-session` — each `intellect` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Intellect syncs them back at session init.
...
The `intellect honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `intellect memory setup honcho` (or run `intellect memory setup` and pick it from the list); the `intellect honcho` subcommand then appears on the next invocation.

```bash
intellect memory setup honcho    # Configure Honcho directly (works before activation)
intellect honcho status          # Connection status, config, and key settings
intellect honcho setup           # Redirects to `intellect memory setup` (post-activation alias)
intellect honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
intellect honcho peer            # Show or update peer names + dialectic reasoning level
intellect honcho mode            # Show or set recall mode (hybrid/context/tools)
intellect honcho tokens          # Show or set token budget for context and dialectic
intellect honcho identity        # Seed or show the AI peer's Honcho identity
intellect honcho sync            # Sync Honcho config to all existing profiles
intellect honcho peers           # Show peer identities across all profiles
intellect honcho sessions        # List known Honcho session mappings
intellect honcho map             # Map current directory to a Honcho session name
intellect honcho enable          # Enable Honcho for the active profile
intellect honcho disable         # Disable Honcho for the active profile
intellect honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `intellect honcho`

If you previously used the standalone `intellect honcho setup`:
...
No re-login or re-setup needed. Run `intellect memory setup` and select "honcho" — the wizard detects your existing config.
