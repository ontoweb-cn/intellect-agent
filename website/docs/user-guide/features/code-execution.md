...
The `execute_code` tool lets the agent write Python scripts that call Intellect tools programmatically, collapsing multi-step workflows into a single LLM turn. The script runs in a child process on the agent host, communicating with Intellect over a Unix domain socket RPC.

## How It Works

1. The agent writes a Python script using `from intellect_tools import ...`
2. Intellect generates a `intellect_tools.py` stub module with RPC functions
3. Intellect opens a Unix domain socket and starts an RPC listener thread
4. The script runs in a child process — tool calls travel over the socket back to Intellect
5. Only the script's `print()` output is returned to the LLM; intermediate tool results never enter the context window

```python
# The agent can write scripts like:
from intellect_tools import web_search, web_extract
...
```python
from intellect_tools import search_files, read_file
import json
...
```python
from intellect_tools import search_files, read_file, patch
...
```python
from intellect_tools import terminal, read_file
import json
...
`execute_code` has two execution modes controlled by `code_execution.mode` in `~/.intellect/config.yaml`:

| Mode | Working directory | Python interpreter |
|------|-------------------|--------------------|
| **`project`** (default) | The session's working directory (same as `terminal()`) | Active `VIRTUAL_ENV` / `CONDA_PREFIX` python, falling back to Intellect's own python |
| `strict` | A temp staging directory isolated from the user's project | `sys.executable` (Intellect's own python) |
...
### `intellect_*` variables in the child

The child process receives only a small, fixed set of operational `intellect_*`
...
(plus `intellect_RPC_DIR` / `intellect_RPC_SOCKET` / `TZ` / `HOME`, which Intellect
injects explicitly so the RPC channel works).

:::note Behavior change
Earlier versions passed **any** variable whose name began with `intellect_`
through to the child. That broad prefix was removed for security hardening: it
could leak `intellect_*`-named configuration that doesn't match a secret substring
(for example `intellect_BASE_URL`, `intellect_KANBAN_DB`, or a `intellect_*_WEBHOOK`
endpoint) into arbitrary sandboxed code.

If an `execute_code` script — or a repo/plugin module it imports at import time
— relied on a `intellect_*` variable outside the four operational names above, it
...
the secret-stripping guarantee (Intellect-managed provider credentials can never
...
**Diagnosing it.** When the child drops one or more non-allowlisted `intellect_*`
variables, Intellect emits a one-line `debug` log naming them and pointing at the
`env_passthrough` escape hatch. Run with debug logging (`intellect logs --level
DEBUG`, or check `~/.intellect/logs/agent.log`) and look for
`execute_code: dropped N non-allowlisted intellect_* var(s)` if a script behaves
as though a `intellect_*` variable is missing.

Intellect always writes the script and the auto-generated `intellect_tools.py` RPC stub into a temp staging directory that is cleaned up after execution. In `strict` mode the script also *runs* there; in `project` mode it runs in the session's working directory (the staging directory stays on `PYTHONPATH` so imports still resolve). The child process runs in its own process group so it can be cleanly killed on timeout or interruption.
...
**Rule of thumb:** Use `execute_code` when you need to call Intellect tools programmatically with logic between calls. Use `terminal` for running shell commands, builds, and processes.
