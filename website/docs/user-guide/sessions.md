...
Intellect Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.intellect/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Intellect stores session history so it can resume conversations, but it does not
...
Intellect explicitly injects for that turn.
...
For example, if a user sends an image and asks Intellect to make a meme from it,
Intellect may inspect that image once with vision and run an image-processing
...
`intellect sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`intellect` or `intellect chat`) |
...
intellect -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
intellect -c "my project"   # → resumes "my project #3"
```
...
intellect --resume 20250305_091523_a1b2c3d4
intellect -r 20250305_091523_a1b2c3d4

# Resume by title
intellect --resume "refactoring auth"

# Or with the chat subcommand
intellect chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `intellect sessions list`.

### Conversation Recap on Resume

When you resume a session, Intellect displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Intellect session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.intellect/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `intellect -r "<title>"` from the shell) and pick up where the platform left off.
...
Intellect automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `intellect sessions list` or `intellect sessions browse`.
...
```bash
intellect sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Intellect creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`intellect -c "my project"`), it automatically picks the most recent session in the lineage.
...
Intellect provides a full set of session management commands via `intellect sessions`:
...
# Filter by platform
intellect sessions list --source telegram

# Show more sessions
intellect sessions list --limit 50
```
...
intellect sessions export backup.jsonl

# Export sessions from a specific platform
intellect sessions export telegram-history.jsonl --source telegram

# Export a single session
intellect sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
intellect sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
intellect sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
intellect sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
intellect sessions prune

# Custom age threshold
intellect sessions prune --older-than 30

# Only prune sessions from a specific platform
intellect sessions prune --source telegram --older-than 60

# Skip confirmation
intellect sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`intellect insights`](/reference/cli-commands#intellect-insights).
...
When Intellect cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Intellect uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Intellect in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.intellect/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.intellect/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.intellect/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |
...
`*.jsonl` files in `~/.intellect/sessions/`. They are no longer written or
read by Intellect. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Intellect process in the same `INTELLECT_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.intellect/config.yaml`:
...
intellect sessions delete <session_id>
