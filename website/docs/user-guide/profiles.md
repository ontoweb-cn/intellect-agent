...
A profile is a separate Intellect home directory. Each profile gets its own directory containing its own `config.yaml`, `.env`, `SOUL.md`, memories, sessions, skills, cron jobs, and state database. Profiles let you run separate agents for different purposes — a coding assistant, a personal bot, a research agent — without mixing up Intellect state.
...
```bash
intellect profile create coder       # creates profile + "coder" command alias
coder setup                       # configure API keys and model
coder chat                        # start chatting
```

That's it. `coder` is now its own Intellect profile with its own config, memory, and state.

:::caution Temporary: profile management disabled by default
As of 2026-06, **`profiles.management_enabled` defaults to `false`**. While disabled:

- **CLI:** `intellect profile create|use|delete|rename|import|install|alias` are blocked; **`intellect -p <existing>`** still works.
- **WebUI:** Profiles panel and profile switch/create/delete are hidden; sessions are scoped to the **default** profile only.
...
:::tip
Quickest setup: run `intellect setup --portal` inside the new profile to wire up models + tools at once. See [ONTOWEB Portal](/integrations/ontoweb-portal).
:::
...
You can also set or auto-generate the description later with `intellect profile describe` — see the [Kanban guide](./features/kanban#auto-vs-manual-orchestration) for the full routing model.
...
Copies your current profile's `config.yaml`, `.env`, and `SOUL.md` into the new profile. Same API keys and model, but fresh sessions and memory. Edit `~/.intellect/profiles/work/.env` for different API keys, or `~/.intellect/profiles/work/SOUL.md` for a different personality.
...
The alias works with every intellect subcommand — it's just `intellect -p <name>` under the hood.
...
- **`intellect profile`**: Shows current profile name, path, model, gateway status
...
Using `cwd: "."` on the local backend means "the directory Intellect was launched from", not "the profile directory".
...
Per-profile gateways are supervised by [s6-overlay](https://github.com/just-containers/s6-overlay) (PID 1 in the container), so `intellect profile create <name>` automatically registers an s6 service slot at `/run/service/gateway-<name>/`. `intellect -p <name> gateway start/stop/restart` dispatches to `s6-svc` instead of spawning a bare process — crashes are auto-restarted and `docker restart` preserves the previously-running set of gateways. See [Per-profile gateway supervision](/user-guide/docker#per-profile-gateway-supervision) for details.
:::
...
```bash
intellect profile list           # show all profiles with status
intellect profile show coder     # detailed info for one profile
intellect profile rename coder dev-bot   # rename (updates alias + service)
intellect profile export coder   # export to coder.tar.gz
intellect profile import coder.tar.gz   # import from archive
```
...
Use `--yes` to skip confirmation: `intellect profile delete coder --yes`
...
Profiles use the `INTELLECT_HOME` environment variable. When you run `coder chat`, the wrapper script sets `INTELLECT_HOME=~/.intellect/profiles/coder` before launching intellect. Since 119+ files in the codebase resolve paths via `get_intellect_home()`, Intellect state automatically scopes to the profile's directory — config, sessions, memory, skills, state database, gateway PID, logs, and cron jobs.
