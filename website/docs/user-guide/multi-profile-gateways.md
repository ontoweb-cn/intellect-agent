...
You want this setup when you have two or more Intellect agents that should all
...
(`ai.intellect.gateway-<name>.plist`) or systemd user service
(`intellect-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
`~/.local/bin/intellect-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: intellect-gateways {start|stop|restart|status|list}"
}
...
```bash
intellect-gateways start      # start every configured profile
intellect-gateways stop       # stop every configured profile
intellect-gateways restart    # restart all
intellect-gateways status     # status across all
intellect-gateways list       # delegates to `intellect gateway list`
```
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.intellect.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/intellect-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.intellect.gateway.plist` /
`intellect-gateway.service`.
...
The default profile uses `~/.intellect/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
intellect config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.intellect/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=intellect --why="gateways running" \
  sleep infinity &
...
`intellect-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
`intellect update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `intellect gateway start` after a previous `intellect gateway stop`. The
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.intellect.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.intellect.gateway-<profile>.plist

# Linux
systemctl --user restart intellect-gateway-<profile>.service
```
