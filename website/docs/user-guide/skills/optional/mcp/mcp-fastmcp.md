...
| | |
|---|---|
| Source | Optional — install with `intellect skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Intellect or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Intellect. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.intellect/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.intellect/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Intellect integration, either:

- configure the server in `~/.intellect/config.yaml` using the `native-mcp` skill, or
...
### Intellect cannot see the deployed server

The server-building part may be correct while the Intellect config is not. Load the `native-mcp` skill and configure the server in `~/.intellect/config.yaml`, then restart Intellect.
