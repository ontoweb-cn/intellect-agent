...
| | |
|---|---|
| Source | Optional — install with `intellect skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not an Intellect core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Intellect native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Intellect native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Intellect native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
Recommended Intellect workflow:
...
## Recommended Intellect usage patterns
...
- Do not silently choose Parallel when Intellect native tools are already sufficient.
