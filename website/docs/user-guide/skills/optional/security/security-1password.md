...
| | |
|---|---|
| Source | Optional — install with `intellect skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Intellect Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Intellect terminal calls (desktop app flow only)
...
### Service Account (recommended for Intellect)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.intellect/.env` (the skill will prompt for this on first load).
...
## Intellect Execution Pattern (desktop app flow)

Intellect terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/intellect-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/intellect-op.sock"
