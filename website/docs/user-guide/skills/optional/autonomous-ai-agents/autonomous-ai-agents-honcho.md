...
description: "Configure and use Honcho memory with Intellect -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session..."
---
...
Configure and use Honcho memory with Intellect -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `intellect skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Intellect

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Intellect profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/intellect#running-honcho-locally-with-intellect

### Verify

```bash
intellect honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Intellect profile's AI peer
...
Honcho models conversations as interactions between **peers**. Intellect creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Intellect instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Intellect run |
| `global` | Single session across all directories |

Manual override: `intellect honcho map my-project-name`
...
Each Intellect profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
1. Creates a `intellect.coder` host block in `honcho.json`
...
```bash
intellect honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Intellect when Honcho memory is active.
...
Config file: `$INTELLECT_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `intellect honcho setup`. Ensure `memory.provider: honcho` is in `~/.intellect/config.yaml`.
...
| Command | Description |
|---------|-------------|
| `intellect honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `intellect honcho status` | Show resolved config, connection test, peer info for active profile |
| `intellect honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `intellect honcho disable` | Disable Honcho for the active profile |
| `intellect honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `intellect honcho peers` | Show peer identities across all profiles |
| `intellect honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `intellect honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `intellect honcho sessions` | List known directory-to-session-name mappings |
| `intellect honcho map <name>` | Map current working directory to a Honcho session name |
| `intellect honcho identity` | Seed AI peer identity or show both peer representations |
| `intellect honcho sync` | Create host blocks for all Intellect profiles that don't have one yet |
| `intellect honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Intellect + Honcho |
| `intellect memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `intellect memory status` | Show active memory provider and config |
| `intellect memory off` | Disable external memory provider |
