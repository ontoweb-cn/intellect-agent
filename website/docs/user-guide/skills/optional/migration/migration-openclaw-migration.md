---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Intellect Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Intellect Agent"
---
...
Migrate a user's OpenClaw customization footprint into Intellect Agent. Imports Intellect-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `intellect skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Intellect Agent (ONTOWEB) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Intellect`, `Memory`, `Persona`, `Import` |
| Related skills | [`intellect-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-intellect-agent) |
...
# OpenClaw -> Intellect Migration

Use this skill when a user wants to move their OpenClaw setup into Intellect Agent with minimal manual cleanup.
...
```bash
intellect claw migrate              # Full interactive migration
...
intellect claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `intellect setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_intellect.py` to:

- import `SOUL.md` into the Intellect home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Intellect memory entries
- merge OpenClaw command approval patterns into Intellect `command_allowlist`
- migrate Intellect-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Intellect working-directory configuration
- copy OpenClaw skills into `~/.intellect/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Intellect workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.intellect/tts/`
- archive non-secret docs that do not have a direct Intellect destination
...
- `scripts/openclaw_to_intellect.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.intellect/skills/migration/openclaw-migration/scripts/openclaw_to_intellect.py`

Do not guess a shorter path like `~/.intellect/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.intellect/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Intellect-compatible secrets, currently:
...
Intellect CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Intellect. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into an Intellect workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Intellect skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.intellect/skills/migration/openclaw-migration/scripts/openclaw_to_intellect.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.intellect/skills/migration/openclaw-migration/scripts/openclaw_to_intellect.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.intellect/skills/migration/openclaw-migration/scripts/openclaw_to_intellect.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.intellect/skills/migration/openclaw-migration/scripts/openclaw_to_intellect.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.intellect/skills/migration/openclaw-migration/scripts/openclaw_to_intellect.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.intellect/skills/migration/openclaw-migration/scripts/openclaw_to_intellect.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Intellect unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Intellect targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Intellect destination. Unsupported auth blobs must still be reported as skipped.
...
- Intellect persona state imported
- Intellect memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.intellect/skills/openclaw-imports/`
