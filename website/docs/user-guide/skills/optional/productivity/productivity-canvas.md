...
| | |
|---|---|
| Source | Optional — install with `intellect skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Intellect Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.intellect/.env`:
...
```bash
CANVAS="python $INTELLECT_HOME/skills/productivity/canvas/scripts/canvas_api.py"
