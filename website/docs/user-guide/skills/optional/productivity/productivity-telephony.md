---
title: "Telephony — Give Intellect phone capabilities without core tool changes"
sidebar_label: "Telephony"
description: "Give Intellect phone capabilities without core tool changes"
---
...
Give Intellect phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `intellect skills install official/productivity/telephony` |
...
This optional skill gives Intellect practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `~/.intellect/.env`
...
It does **not** turn Intellect into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Intellect memory
...
### 1) "I want Intellect to own a real phone number"
Use **Twilio**.
...
- pairs well with Intellect `text_to_speech` plus a public file host or tunnel
...
### `~/.intellect/telephony_state.json`
...
```bash
SCRIPT="$(find ~/.intellect/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
intellect skills search telephony
intellect skills install official/productivity/telephony
```
...
Then save credentials into Intellect:
...
3. Buy it and save it into `~/.intellect/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Intellect calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Intellect's existing `text_to_speech` support.

Use this when:
- you want the call to use Intellect's configured TTS voice rather than Twilio `<Say>`
...
Recommended Intellect TTS -> Twilio Play workflow:

1. Generate the audio with Intellect `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Intellect to create the message audio with `text_to_speech`
...
Important note:
- Intellect TTS is great for prerecorded outbound messages
...
- Intellect STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Intellect memory.
...
- Do not store arbitrary third-party phone numbers in Intellect memory.
...
3. persist that number to `~/.intellect/.env`
