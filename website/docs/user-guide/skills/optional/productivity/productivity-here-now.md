...
| | |
|---|---|
| Source | Optional — install with `intellect skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${intellect_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${intellect_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${intellect_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client intellect
```
...
```bash
PUBLISH="${intellect_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client intellect
```
...
```bash
DRIVE="${intellect_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `intellect`)    |
