...
- Give it a name like "intellect-agent"
- Select scopes:
...
- Give it a title like "intellect-agent-&lt;machine-name>"
...
elif [ -f ~/.intellect/.env ] && grep -q "^GITHUB_TOKEN=" ~/.intellect/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.intellect/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
