...
- `intellect profile list` — prints the table of profiles configured on this machine. Run it through your terminal tool if you have one; otherwise ask the user.
...
t1 = kanban_create(
    ...
    tenant=os.environ.get("intellect_TENANT"),
)["task_id"]
...
>
> The dispatcher will pick up T1 and T2 now. T3 starts when both finish. You'll get a gateway ping when T4 completes. Use the dashboard or `intellect kanban tail <id>` to follow along.
...
**Tenant inheritance.** If `intellect_TENANT` is set in your env, pass `tenant=os.environ.get("intellect_TENANT")` on every `kanban_create` call so child tasks stay in the same namespace.
...
1. **Reclaim** (or `intellect kanban reclaim <task_id>`) — abort the running worker immediately and reset the task to `ready`. The existing claim TTL is ~15 min; this is the fast path out.
2. **Reassign** (or `intellect kanban reassign <task_id> <new-profile> --reclaim`) — switch the task to a different profile (one that exists on this setup) and let the dispatcher pick it up with a fresh worker.
3. **Change profile model** — the dashboard prints a copy-paste hint for `intellect -p <profile> model` since profile config lives on disk; edit it in a terminal, then Reclaim to retry with the new model.
