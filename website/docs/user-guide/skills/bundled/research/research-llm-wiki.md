...
The wiki directory is **scoped to your current context**. Before every tool call the
runtime injects:
...
| `WIKI_SCOPE_ID` | slug/id; empty for global |
| `WIKI_WRITE_MODE` | `read_write` or `read_only` |

**Resolution order** (default session — not explicit global):
...
4. **Legacy global** — `~/wiki` or config (single-user mode only)

**Global org wiki** (multi-user): `$INTELLECT_HOME/wiki/global/` — **read-only** for
ordinary members; only `owner` / `admin` may write when `WIKI_TARGET_SCOPE=global`.

Directories use **intended paths** — missing folders are OK; create on first write or init.

**Always orient using env vars:**
