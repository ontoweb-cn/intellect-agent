---
title: "Intellect S6 Container Supervision"
sidebar_label: "Intellect S6 Container Supervision"
description: "Modify, debug, or extend the s6-overlay supervision tree inside the Intellect Agent Docker image — adding new services, debugging profile gateways, understan..."
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Intellect S6 Container Supervision

Modify, debug, or extend the s6-overlay supervision tree inside the Intellect Agent Docker image — adding new services, debugging profile gateways, understanding the Architecture B main-program pattern.
...
| Path | `skills/software-development/intellect-s6-container-supervision` |
| Version | `1.0.0` |
| Author | Intellect Agent |
| License | MIT |
| Tags | `docker`, `s6`, `supervision`, `gateway`, `profiles` |
| Related skills | [`intellect-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-intellect-agent), `intellect-agent-dev` |
...
# Intellect s6-overlay Container Supervision

## When to use this skill

Load this skill when you're working on:
- Adding or removing a static service in the Intellect Docker image (something that should be supervised at every container start, like the dashboard)
- Diagnosing why a per-profile gateway isn't starting, restarting, or surviving `docker restart`
- Understanding why the container's CMD is `/opt/intellect/docker/main-wrapper.sh` and how leading-dash args reach the user's program
...
If you're just running the Intellect Agent and want to use Docker, see `website/docs/user-guide/docker.md` instead.
...
│   ├── 01-intellect-setup                ← docker/stage2-hook.sh
│   │   ├── UID/GID remap
│   │   ├── chown /opt/data
│   │   ├── chown /opt/data/profiles (every boot)
│   │   ├── seed .env / config.yaml / SOUL.md
│   │   └── skills_sync.py
│   └── 02-reconcile-profiles          ← intellect_cli.container_boot
│       ├── chown /run/service (intellect-writable for runtime register)
│       └── walk $INTELLECT_HOME/profiles/<name>/gateway_state.json
│           → recreate /run/service/gateway-<name>/
│           → auto-start only those with prior_state == "running"
│
├── s6-rc.d (static services, in /etc/s6-overlay/s6-rc.d/)
│   └── main-intellect/run                ← exec sleep infinity (no-op slot)
│
...
│   └── ...
│
└── CMD ("main program")               ← /opt/intellect/docker/main-wrapper.sh
    └── routes user args: bare exec | intellect subcommand | intellect (no args)
...
| Path | Role |
|---|---|
| `Dockerfile` | s6-overlay install + cont-init.d wiring + `ENTRYPOINT ["/init", "/opt/intellect/docker/main-wrapper.sh"]` |
| `docker/stage2-hook.sh` | The "old entrypoint logic" — UID remap, chown, seed, skills sync. Runs as cont-init.d/01-intellect-setup. |
| `docker/cont-init.d/02-reconcile-profiles` | Calls `intellect_cli.container_boot` on every boot to restore profile gateway slots from the persistent volume. |
| `docker/main-wrapper.sh` | The container's CMD. Routes user args, drops to intellect via `s6-setuidgid`, exec's the chosen program. |
| `docker/s6-rc.d/main-intellect/run` | No-op `sleep infinity` — slot exists so the s6-rc user bundle is valid; main intellect runs as the CMD, not as a supervised service. |
...
| `intellect_cli/service_manager.py` | `S6ServiceManager`: `register_profile_gateway`, `unregister_profile_gateway`, `start/stop/restart/is_running`, `list_profile_gateways`. |
| `intellect_cli/container_boot.py` | `reconcile_profile_gateways()` — walks persistent profiles, regenerates s6 slots, emits `container-boot.log`. |
| `intellect_cli/gateway.py::_dispatch_via_service_manager_if_s6` | Intercepts `intellect gateway start/stop/restart` and routes to s6 when running in a container. |

## Why Architecture B (CMD as main program, not s6-supervised)

The original plan (v1–v3) called for main intellect to run as a supervised s6-rc service. Two real s6-overlay v3 mechanics blocked that:

1. **cont-init.d scripts receive no CMD args** — so the stage2 hook can't parse `docker run <image> chat -q "hi"` to set `intellect_ARGS` for a service `run` script to consume.
...
So we use the s6-overlay-native CMD pattern: `ENTRYPOINT ["/init", "/opt/intellect/docker/main-wrapper.sh"]`. /init prepends the wrapper to user args automatically — so `docker run <image> --version` becomes `/init main-wrapper.sh --version`, and `--version` doesn't get intercepted by /init's POSIX shell. The wrapper drops to intellect via `s6-setuidgid`, then exec's the chosen program. The program's exit code becomes the container exit code, exactly matching the pre-s6 tini contract.

Trade-off: main intellect is unsupervised under s6. That exactly matches its behavior under tini (the pre-s6 image). Dashboard supervision is the only **new** guarantee — and per-profile gateways under `/run/service/` get full supervision.
...
2. Drop to intellect via `s6-setuidgid intellect` at the top of run (unless you specifically need root).
...
Edit `S6ServiceManager._render_run_script` in `intellect_cli/service_manager.py`. The function is also called by `intellect_cli/container_boot.py::_register_service` during boot reconciliation, so it's the single source of truth. Update the corresponding assertion in `tests/intellect_cli/test_service_manager.py::test_s6_register_creates_service_dir_and_triggers_scan`.

### Run the docker test harness

```sh
docker build -t intellect-agent-harness:latest .
INTELLECT_TEST_IMAGE=intellect-agent-harness:latest scripts/run_tests.sh tests/docker/ -v
...
`docker exec` defaults to root. Either pass `--user intellect` or rely on the stage2 chown sweep next reboot. Don't write files under `$INTELLECT_HOME/profiles/<name>/` as root manually — the next reconcile pass will sweep them but in-flight operations may hit perm errors.
...
- `intellect-agent-dev`: General intellect-agent codebase navigation
- `intellect-tool-quirks`: Specific Intellect-tool workarounds (sed/grep/etc.) — load when debugging the s6 stack's interaction with intellect built-in tools.
