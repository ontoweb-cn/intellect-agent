...
Connect Intellect to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Intellect supports two WeCom integration modes:
...
> Run `intellect gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Intellect decrypts the message, queues it for the agent
...
(Use `intellect gateway start` only after `intellect gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Intellect and the token the
...
in `~/.intellect/.env` values around `=` will also break signature checks. After
fixing, restart `intellect gateway run`.
...
Check `intellect gateway run` logs for the bound host/port. If the adapter bound to
