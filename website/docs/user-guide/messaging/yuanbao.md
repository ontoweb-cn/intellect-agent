...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Intellect Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Intellect to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.intellect/.env`:
...
All standard Intellect commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.intellect/logs/gateway.log`
...
4. Consider enabling verbose logging: `intellect_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Intellect automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
intellect_LOG_LEVEL=debug intellect gateway
```
...
```bash
intellect chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
