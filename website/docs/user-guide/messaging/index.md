...
description: "Chat with Intellect from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Intellect from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Intellect](/guides/use-voice-mode-with-intellect).

:::tip
Bots need both a model provider and tool providers (TTS, web). A [ONTOWEB Portal](/integrations/ontoweb-portal) subscription bundles all of them.
:::
...
```mermaid
flowchart TB
    subgraph Gateway["Intellect Gateway"]
...
```bash
intellect gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
intellect gateway              # Run in foreground
intellect gateway setup        # Configure messaging platforms interactively
intellect gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo intellect gateway install --system   # Linux only: install a boot-time system service
intellect gateway start        # Start the default service
intellect gateway stop         # Stop the default service
intellect gateway status       # Check default service status
intellect gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Intellect Agent to the latest version |
...
Configure per-platform overrides in `~/.intellect/gateway.json`:
...
intellect pairing approve telegram XKGH5N7P

# Other pairing commands:
intellect pairing list          # View pending + approved users
intellect pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Intellect appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.intellect/config.yaml`:
...
Intellect confirms immediately:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.intellect/config.yaml`:
...
```bash
intellect_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
intellect gateway install               # Install as user service
intellect gateway start                 # Start the service
intellect gateway stop                  # Stop the service
intellect gateway status                # Check status
journalctl --user -u intellect-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo intellect gateway install --system
sudo intellect gateway start --system
sudo intellect gateway status --system
journalctl -u intellect-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Intellect will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Intellect installations on the same machine (with different `INTELLECT_HOME` directories), each gets its own systemd service name. The default `~/.intellect` uses `intellect-gateway`; other installations use `intellect-gateway-<hash>`. The `intellect gateway` commands automatically target the correct service for your current `INTELLECT_HOME`.
:::

### macOS (launchd)

```bash
intellect gateway install               # Install as launchd agent
intellect gateway start                 # Start the service
intellect gateway stop                  # Stop the service
intellect gateway status                # Check status
tail -f ~/.intellect/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.intellect.gateway.plist`. It includes three environment variables:
...
- **INTELLECT_HOME** — scopes the gateway to your Intellect installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `intellect gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `INTELLECT_HOME` directory gets its own launchd label. The default `~/.intellect` uses `ai.intellect.gateway`; other installations use `ai.intellect.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `intellect-cli` | Full access |
| Telegram | `intellect-telegram` | Full tools including terminal |
| Discord | `intellect-discord` | Full tools including terminal |
| WhatsApp | `intellect-whatsapp` | Full tools including terminal |
| Slack | `intellect-slack` | Full tools including terminal |
| Google Chat | `intellect-google_chat` | Full tools including terminal |
| Signal | `intellect-signal` | Full tools including terminal |
| SMS | `intellect-sms` | Full tools including terminal |
| Email | `intellect-email` | Full tools including terminal |
| Home Assistant | `intellect-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `intellect-mattermost` | Full tools including terminal |
| Matrix | `intellect-matrix` | Full tools including terminal |
| DingTalk | `intellect-dingtalk` | Full tools including terminal |
| Feishu/Lark | `intellect-feishu` | Full tools including terminal |
| WeCom | `intellect-wecom` | Full tools including terminal |
| WeCom Callback | `intellect-wecom-callback` | Full tools including terminal |
| Weixin | `intellect-weixin` | Full tools including terminal |
| BlueBubbles | `intellect-bluebubbles` | Full tools including terminal |
| QQBot | `intellect-qqbot` | Full tools including terminal |
| Yuanbao | `intellect-yuanbao` | Full tools including terminal |
| Microsoft Teams | `intellect-teams` | Full tools including terminal |
| API Server | `intellect-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `intellect-webhook` | Full tools including terminal |
...
1. **Gateway log** (`~/.intellect/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
