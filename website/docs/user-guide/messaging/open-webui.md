...
sidebar_position: 8
title: "Open WebUI"
description: "Connect Open WebUI to Intellect Agent via the OpenAI-compatible API server"
---

# Open WebUI Integration

[Open WebUI](https://github.com/open-webui/open-webui) (126k★) is the most popular self-hosted chat interface for AI. With Intellect Agent's built-in API server, you can use Open WebUI as a polished web frontend for your agent — complete with conversation management, user accounts, and a modern chat interface.
...
Open WebUI connects to Intellect Agent's API server just like it would connect to OpenAI. Intellect handles the requests with its full toolset — terminal, file operations, web search, memory, skills — and returns the final response.

:::important Runtime location
The API server is a **Intellect agent runtime**, not a pure LLM proxy. For each request, Intellect creates a server-side `AIAgent` on the API-server host. Tool calls run where that API server is running.

For example, if a laptop points Open WebUI or another OpenAI-compatible client at an Intellect API server on a remote machine, `pwd`, file tools, browser tools, local MCP tools, and other workspace tools run on the remote API-server host, not on the laptop.
:::

Open WebUI talks to Intellect server-to-server, so you do not need `API_SERVER_CORS_ORIGINS` for this integration.

## Quick Setup

### One-command local bootstrap (macOS/Linux, no Docker)

If you want Intellect + Open WebUI wired together locally with a reusable launcher, run:
...
- ensures `~/.intellect/.env` contains `API_SERVER_ENABLED`, `API_SERVER_HOST`, `API_SERVER_KEY`, `API_SERVER_PORT`, and `API_SERVER_MODEL_NAME`
- restarts the Intellect gateway so the API server comes up
- installs Open WebUI into `~/.local/open-webui-venv`
- writes a launcher at `~/.local/bin/start-open-webui-intellect.sh`
...
```bash
OPEN_WEBUI_NAME='My Intellect UI' \
OPEN_WEBUI_ENABLE_SIGNUP=true \
intellect_API_MODEL_NAME='My Intellect Agent' \
bash scripts/setup_open_webui.sh
```
...
```bash
intellect config set API_SERVER_ENABLED true
intellect config set API_SERVER_KEY your-secret-key
```

`intellect config set` auto-routes the flag to `config.yaml` and the secret to `~/.intellect/.env`. If the gateway is already running, restart it so the change takes effect:

```bash
intellect gateway stop && intellect gateway
```

### 2. Start Intellect Agent gateway
...
Go to **http://localhost:3000**. Create your admin account (the first user becomes admin). You should see your agent in the model dropdown (named after your profile, or **intellect-agent** for the default profile). Start chatting!
...
Your agent model should now appear in the model dropdown (named after your profile, or **intellect-agent** for the default profile).
...
This is the default and requires no extra configuration. Open WebUI sends standard OpenAI-format requests and Intellect Agent responds accordingly. Each request includes the full conversation history.
...
2. Edit your intellect-agent connection
3. Change **API Type** from "Chat Completions" to **"Responses (Experimental)"**
4. Save

With the Responses API, Open WebUI sends requests in the Responses format (`input` array + `instructions`), and Intellect Agent can preserve full tool call history across turns via `previous_response_id`. When `stream: true`, Intellect also streams spec-native `function_call` and `function_call_output` items, which enables custom structured tool-call UI in clients that render Responses events.
...
2. Intellect Agent creates a server-side `AIAgent` instance using the API server's profile, model/provider config, memory, skills, and configured API-server toolsets
...
Your agent has access to the same tools and capabilities as that API-server Intellect instance. If the API server is remote, those tools are remote too.

If you need tools to run against your **local** workspace today, run Intellect locally and point it at a pure LLM provider or pure OpenAI-compatible model proxy (for example vLLM, LiteLLM, Ollama, llama.cpp, OpenAI, OpenRouter, etc.). A future split-runtime mode for "remote brain, local hands" is being tracked in [#18715](https://gitee.com/ontoweb/intellect-agent/issues/18715); it is not the behavior of the current API server.
...
|----------|-------------|
| `OPENAI_API_BASE_URL` | Intellect Agent's API URL (include `/v1`) |
...
Intellect Agent may be executing multiple tool calls (reading files, running commands, searching the web) before producing its final response. This is normal for complex queries. The response appears all at once when the agent finishes.

### "Invalid API key" errors

Make sure your `OPENAI_API_KEY` in Open WebUI matches the `API_SERVER_KEY` in Intellect Agent.
...
To run separate Intellect instances per user — each with their own config, memory, and skills — use [profiles](/user-guide/profiles). Each profile runs its own API server on a different port and automatically advertises the profile name as the model in Open WebUI.
...
The model dropdown will show `alice` and `bob` as distinct models. You can assign models to Open WebUI users via the admin panel, giving each user their own isolated Intellect agent.
