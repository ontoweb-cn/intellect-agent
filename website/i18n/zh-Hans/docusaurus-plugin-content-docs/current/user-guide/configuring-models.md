...
- **右列** — 所选提供商的精选模型列表。这些是 Intellect 针对该提供商推荐的 agentic 模型，而非原始的 `/models` 接口返回结果（OpenRouter 的原始列表包含 400+ 个模型，涵盖 TTS、图像生成器和重排序器）。

在过滤框中输入提供商名称、slug 或模型 ID 进行筛选。

选择模型后点击 **Switch**，Intellect 会将其写入 `~/.intellect/config.yaml` 的 `model` 部分。**此操作仅对新会话生效** — 已打开的聊天标签页将继续使用启动时的模型。如需在当前聊天中热切换，请在聊天内使用 `/model` 斜杠命令。
...
每个辅助任务默认为 `auto`，即 Intellect 对该任务也使用主模型。当某个边缘任务需要更便宜或更快的模型时，可单独覆盖该槽位。
...
通过仪表板保存时，Intellect 写入 `~/.intellect/config.yaml`：
...
Intellect 仅列出具有有效凭据的提供商。检查侧边栏中的 **Keys** — 应存在以下之一：API key、成功的 OAuth 或自定义端点 URL。若所需提供商不在列表中，运行 `intellect setup` 进行配置，或前往 **Keys** 添加环境变量。
...
```bash
intellect model            # 交互式提供商 + 模型选择器（切换默认值的标准方式）
```
...
如需在不启动选择器的情况下列出提供商/模型，请使用仪表板或下方的 REST 端点。查看 CLI 当前实际使用的配置：`intellect config get model` 和 `intellect status`。

### 直接编辑配置文件

编辑 `~/.intellect/config.yaml` 后重启相关服务。完整 schema 请见[配置参考](./configuration.md)。
...
```bash
# 列出已认证的提供商及精选模型列表
curl -H "X-Intellect-Session-Token: $TOKEN" http://localhost:PORT/api/model/options

# 读取当前主模型及辅助任务分配
curl -H "X-Intellect-Session-Token: $TOKEN" http://localhost:PORT/api/model/auxiliary

# 设置主模型
curl -X POST -H "Content-Type: application/json" -H "X-Intellect-Session-Token: $TOKEN" \
...
session token 在启动时注入仪表板 HTML，每次服务器重启后轮换。如需对运行中的仪表板编写脚本，可从浏览器开发者工具中获取（`window.__intellect_SESSION_TOKEN__`）。