...
description: 通过 FAL.ai 文生图；支持 8 个模型，含 FLUX 2、GPT-Image、Nano Banana Pro、Ideogram、Recraft V4 Pro 等，可用 intellect tools 切换。
sidebar_label: 文生图
sidebar_position: 6
---
...
Intellect Agent 通过 FAL.ai 根据文字提示生成图像。默认内置 8 个模型，在速度、画质与成本上各有取舍。当前模型可通过 `intellect tools` 配置，并持久化在 `config.yaml`。
...
:::tip OntoWeb 订阅用户
若你持有付费 [ONTOWEB Portal](https://portal.ontoweb.cn) 订阅，可通过 **[Tool Gateway](tool-gateway.md)** 使用文生图，**无需** `FAL_KEY`。模型选择在「直连 FAL」与「订阅网关」两条路径下保持一致。
...
进入 **🎨 Image Generation**，选择后端（OntoWeb Subscription 或 FAL.ai），随后在表格中用方向键选择模型，回车确认：
...
image_gen:
  ...
  use_gateway: false            # 使用 OntoWeb Subscription 时为 true
```

### GPT-Image 画质档位

`fal-ai/gpt-image-1.5` 的请求画质固定为 `medium`（约 1024×1024 下 $0.034/张）。面向用户**不开放** `low` / `high` 档位，以便 ONTOWEB Portal 侧计费在全体用户间更可预期（档位价差约 22×）。若需要更便宜的 GPT-Image 路线，请换其他模型；若追求更高画质，可考虑 Klein 9B 或同类 Imagen 系模型。
...
3. **提交** — `_submit_fal_request()` 根据凭据走直连 FAL 或 OntoWeb 托管网关。  
...
- **需要 FAL 凭据**（直连 `FAL_KEY` 或 OntoWeb 订阅网关）  
