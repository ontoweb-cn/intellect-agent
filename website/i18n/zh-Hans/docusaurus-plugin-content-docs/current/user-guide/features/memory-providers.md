...
description: "外部记忆提供者插件 — Honcho、Graphiti、OpenViking、Mem0、Hindsight、Holographic、RetainDB、ByteRover、Supermemory"
---

# Memory Providers

Intellect Agent 内置 9 个外部记忆提供者插件，为 Agent 提供跨会话的持久化知识，超越内置的 MEMORY.md 和 USER.md。同一时间只能激活**一个**外部提供者——内置记忆始终与其并行工作。

## 快速开始

```bash
intellect memory setup      # 交互式选择器 + 配置
intellect memory status     # 查看当前激活状态
intellect memory off        # 禁用外部提供者
```

也可以通过 `intellect plugins` → Provider Plugins → Memory Provider 选择激活的记忆提供者。

或在 `~/.intellect/config.yaml` 中手动设置：

```yaml
memory:
  provider: openviking   # 或 honcho, graphiti, mem0, hindsight, holographic, retaindb, byterover, supermemory
```

## 工作原理

当记忆提供者激活时，Intellect 会自动：
...
**安装向导：**
```bash
intellect memory setup        # 选择 "honcho" — 运行 Honcho 专属的安装后配置
```

旧版 `intellect honcho setup` 命令仍然有效（现在会重定向到 `intellect memory setup`），但只有在 Honcho 被选为激活记忆提供者后才会注册。

**配置：** `$INTELLECT_HOME/honcho.json`（profile 本地）或 `~/.honcho/config.json`（全局）。解析顺序：`$INTELLECT_HOME/honcho.json` > `~/.intellect/honcho.json` > `~/.honcho/config.json`。参见[配置参考](https://github.com/intellect-ai/intellect-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrations/intellect)。
...
:::tip 从 `intellect honcho` 迁移
如果你之前使用过 `intellect honcho setup`，你的配置和所有服务端数据均完好无损。只需通过安装向导重新启用，或手动设置 `memory.provider: honcho`，即可通过新系统重新激活。
:::

**多 peer 配置：**

Honcho 将对话建模为 peer 之间的消息交换——每个 Intellect profile 对应一个用户 peer 加一个 AI peer，共享同一个 workspace。workspace 是共享环境：用户 peer 在各 profile 间全局共享，每个 AI peer 拥有独立身份。每个 AI peer 从自身的观察中独立构建表示/card，因此 `coder` profile 保持代码导向，而 `writer` profile 针对同一用户保持编辑导向。

映射关系：

| 概念 | 含义 |
|---------|-----------|
| **Workspace** | 共享环境。同一 workspace 下的所有 Intellect profile 共享同一用户身份。 |
| **用户 peer**（`peerName`） | 人类用户。在 workspace 内跨 profile 共享。 |
| **AI peer**（`aiPeer`） | 每个 Intellect profile 一个。host key `intellect` → 默认；其他 profile 使用 `intellect.<profile>`。 |
...
扫描所有 Intellect profile，为没有 host 块的 profile 创建 host 块，从默认 `intellect` 块继承设置，并提前创建新的 AI peer。幂等操作——跳过已有 host 块的 profile。
...
参见[配置参考](https://github.com/intellect-ai/intellect-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrations/intellect)。
...
# 然后配置 Intellect
intellect memory setup    # 选择 "openviking"
# 或手动配置：
...
**安装：**
```bash
intellect memory setup    # 选择 "mem0"
# 或手动配置：
intellect config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.intellect/.env
```

**配置：** `$INTELLECT_HOME/mem0.json`

| 键 | 默认值 | 描述 |
|-----|---------|-------------|
| `user_id` | `intellect-user` | 用户标识符 |
| `agent_id` | `intellect` | Agent 标识符 |
...
**安装：**
```bash
intellect memory setup    # 选择 "hindsight"
# 或手动配置：
...
**本地模式 UI：** `hindsight-embed -p intellect ui start`

**配置：** `$INTELLECT_HOME/hindsight/config.json`

| 键 | 默认值 | 描述 |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` 或 `local` |
| `bank_id` | `intellect` | 记忆库标识符 |
...
| `retain_context` | `conversation between Intellect Agent and the User` | 保留记忆的上下文标签 |
...
完整配置参考参见[插件 README](https://gitee.com/ontoweb/intellect-agent/blob/main/plugins/memory/hindsight/README.md)。

---

### Graphiti

具备双时态有效性（「何时说出」vs「何时为真」）、episode 溯源与混合检索（语义 + BM25 + 图遍历）的时序知识图谱记忆。对话轮次可自动入库为 episode；上下文压缩前会将即将被压缩的历史持久化到图中。

| | |
|---|---|
| **适合场景** | 将对话记忆建成可查询图谱——偏好、关系、决策、事件及其时间线 |
| **依赖** | `pip install 'intellect-agent[graphiti]'` + FalkorDB（推荐 Docker）或 Neo4j |
| **数据存储** | FalkorDB（默认）或 Neo4j——按 member / team / project 划分 scope 图 |
| **费用** | 免费（自托管图库）+ 实体抽取用 LLM API（或通过 `llm_base_url` 接本地 Ollama） |

**工具（5 个）：** `graphiti_add_episode`（持久化观察）、`graphiti_search_facts`（混合召回）、`graphiti_search_nodes`（实体查找）、`graphiti_get_node_timeline`（双时态时间线）、`graphiti_delete_episode`（带审计的删除）
...
intellect memory setup    # 选择 "graphiti"
# 或手动：
intellect config set memory.provider graphiti
```

Graphiti 为当前 provider 时，`intellect graphiti status` 会按 scope 图 ping FalkorDB；`intellect doctor` 包含 Graphiti 健康检查。

**CLI（仅激活时可用）：** `intellect graphiti setup | status | stats | timeline | dump | rebuild-communities | mcp start | mcp config`

**配置：** `$INTELLECT_HOME/graphiti/config.json`

| 键 | 默认 | 说明 |
|----|------|------|
| `backend` | `falkordb` | `falkordb`（推荐）或 `neo4j` |
| `falkordb_host` | `localhost` | FalkorDB 主机（Docker 栈内为 `falkordb`） |
| `falkordb_port` | `6380` | 主机端口（容器内为 6379） |
| `embedding_provider` | `local` | `local`（fastembed / bge-m3，无需 API key）或 `openai` |
| `llm_provider` | `openai` | 抽取用 LLM；`llm_base_url` 可指向 Ollama / vLLM / LiteLLM |
| `auto_ingest` | `true` | 每轮对话自动写入 episode |
| `ingest_every_n_turns` | `1` | `auto_ingest` 开启时的入库频率 |
| `default_max_nodes` | `10` | 预取 / 搜索默认返回事实数上限 |

**特点：**
- 双时态事实——追踪「已知内容」与「有效时间窗」
- Scope 图——member / team / project 分图，搜索可合并
- 预压缩钩子——压缩前将旧轮次写入图谱
- 默认 fastembed 本地向量（`bge-m3`）——无需 embedding API key

**与文档 RAG 共存：** Graphiti 管**对话**记忆；文档语料库请配合 [RAG Providers](/user-guide/features/rag-providers)（如 `rag.provider: lightrag`）。内置 `memory` 工具行为不变。
...
**安装：**
```bash
intellect memory setup    # 选择 "holographic"
# 或手动配置：
intellect config set memory.provider holographic
```

**配置：** `plugins.intellect-memory-store` 下的 `config.yaml`

| 键 | 默认值 | 描述 |
|-----|---------|-------------|
| `db_path` | `$INTELLECT_HOME/memory_store.db` | SQLite 数据库路径 |
...
**安装：**
```bash
intellect memory setup    # 选择 "retaindb"
# 或手动配置：
...
# 然后配置 Intellect
intellect memory setup    # 选择 "byterover"
# 或手动配置：
...
- 知识树存储于 `$INTELLECT_HOME/byterover/`（profile 范围隔离）
...
**安装：**
```bash
intellect memory setup    # 选择 "supermemory"
# 或手动配置：
intellect config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.intellect/.env
```

**配置：** `$INTELLECT_HOME/supermemory.json`

| 键 | 默认值 | 描述 |
|-----|---------|-------------|
| `container_tag` | `intellect` | 用于搜索和写入的容器标签。支持 `{identity}` 模板用于 profile 范围隔离。 |
...
- **Profile 范围容器**——在 `container_tag` 中使用 `{identity}`（例如 `intellect-{identity}` → `intellect-coder`），按 Intellect profile 隔离记忆
...
| **Graphiti** | 自托管 | 免费 | 5 | `graphiti-core` + FalkorDB/Neo4j | 双时态知识图谱 + 混合检索 |
...
- **图谱类提供者**（Graphiti）在 FalkorDB / Neo4j 上按 scope（member / team / project）隔离数据
