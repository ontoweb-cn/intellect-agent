...
:::tip 零调用超时时的诊断转储
如果子智能体在**零次** API 调用的情况下超时（通常原因：provider 不可达、认证失败或工具 schema 被拒绝），`delegate_task` 会将结构化诊断信息写入 `~/.intellect/logs/subagent-timeout-<session>-<timestamp>.log`，其中包含子智能体的配置快照、凭据解析追踪以及早期错误消息。比之前的静默超时行为更易于定位根因。
:::
...
## 会话标识（`intellect_SESSION_ID`）

每个智能体实例在 SQLite 与日志中都有各自的 `session_id`。子智能体构造时使用 **`publish_session_context=False`**，避免在主线程搭建子智能体时把父进程的会话 id 切换成子 id。

子智能体在**工作线程**中运行时会发布自己的 session id；委派结束（或工作线程退出）后，会恢复父级的 `intellect_SESSION_ID`（ContextVar + `os.environ`）。

| 运行环境 | 工具应如何读取当前会话 |
|---------|------------------------|
| Gateway（异步） | `get_session_env("intellect_SESSION_ID")` — 任务级 ContextVar |
| CLI / 同步工具 | 同上；无 ContextVar 时回退到 `os.environ` |
| 子进程 shell（`terminal`） | 继承合并后的环境；并发批量委派时，进程内工具应优先用 `get_session_env` |

库代码应使用 `from gateway.session_context import get_session_env`，不要直接 `os.environ.get("intellect_SESSION_ID")`。

同步代码中需要临时覆盖时，使用同模块的 `session_id_scope(session_id)`。

:::note 并行批量注意事项
使用 `tasks=[...]` 且多个子智能体并发时，`os.environ` 仍是进程全局的。每个工作线程会为自己的子 id 设置 ContextVar；仅读取环境变量 `$intellect_SESSION_ID` 的 shell 命令可能看到「最后被写入」的子 id。使用 `get_session_env` 的进程内工具能看到各自线程的正确 id。
:::
