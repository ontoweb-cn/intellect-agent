...
:::info
以下是 Intellect 在触发此 skill 时加载的完整 skill 定义。这是 skill 激活时 agent 看到的指令内容。
:::
...
维基目录**与当前上下文作用域绑定**。每次工具调用前，运行时会注入：

| 变量 | 含义 |
|------|------|
| `WIKI_PATH` | 目标维基目录（可能尚不存在） |
| `WIKI_SCOPE` | `project` \| `team` \| `member` \| `global` |
| `WIKI_SCOPE_ID` | slug/id；global 为空 |
| `WIKI_WRITE_MODE` | `read_write` 或 `read_only` |

**解析顺序**（默认会话，非显式 global）：

1. **项目** — `$INTELLECT_HOME/projects/{slug}/wiki/`
2. **团队** — `$INTELLECT_HOME/teams/{slug}/wiki/`（无活跃项目）
3. **成员** — `$INTELLECT_HOME/members/{id}/wiki/`（已登录且无团队/项目）
4. **旧版全局** — `~/wiki` 或配置（仅单用户模式）

**组织 Global 维基**（多用户）：`$INTELLECT_HOME/wiki/global/` — 普通成员**只读**；仅 `owner` / `admin` 在 `WIKI_TARGET_SCOPE=global` 时可写。

目录为**预期路径**——缺失文件夹正常，首次写入或初始化时创建。
