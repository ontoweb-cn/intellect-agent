...
旧命令 `intellect agent`、`-p`/`--profile`，以及 `~/.intellect/agents/` 仍兼容（会迁移到 `agents/`）。
...
- **`intellect agent`**：显示当前 profile 名称、路径、模型、gateway 状态

## Profile vs 工作区 vs 沙箱

profile 常与工作区或沙箱混淆，但它们是不同的概念：

- **profile** 为 Intellect 提供独立的状态目录：`config.yaml`、`.env`、`SOUL.md`、会话、记忆、日志、cron 任务和 gateway 状态。
...
在 local 后端使用 `cwd: "."` 表示"Intellect 启动时所在的目录"，而非"profile 目录"。
...
```bash
coder gateway install         # 创建 intellect-gateway-coder systemd/launchd 服务
assistant gateway install     # 创建 intellect-gateway-assistant 服务
```
...
`intellect update` 拉取一次代码（共享），并自动将新的内置技能同步到**所有** profile：
...
默认 profile 就是 `~/.intellect` 本身。无需迁移——现有安装的工作方式完全不变。
...
完整指南请参阅 **[Profile 发行版：共享完整 Agent](./agent-distributions.md)**——包括编写、发布、更新语义、安全模型和使用场景。