...
slug: /
sidebar_position: 0
title: "Intellect Agent 文档"
description: "由 ONTOWEB 构建的自我改进 AI 智能体。内置学习循环，从经验中创建技能，在使用过程中持续改进，并跨会话保持记忆。"
...
由 [ONTOWEB](https://ontoweb.cn) 构建的自我改进 AI 智能体。唯一内置学习循环的智能体——它从经验中创建技能，在使用过程中持续改进，主动提示自身持久化知识，并在会话间不断深化对你的建模。
...
<div style={{display: 'flex', gap: '1rem', marginBottom: '2rem', flexWrap: 'wrap'}}>
  ...
  <a href="https://gitee.com/ontoweb/intellect-agent" style={{display: 'inline-block', padding: '0.6rem 1.2rem', border: '1px solid rgba(255,215,0,0.2)', borderRadius: '8px', textDecoration: 'none'}}>在 GitHub 上查看</a>
</div>
...
| 🔌 **[MCP 集成](/user-guide/features/mcp)** | 连接 MCP 服务器、过滤其工具，并安全扩展 Intellect |
| 🧭 **[在 Intellect 中使用 MCP](/guides/use-mcp-with-intellect)** | 实用的 MCP 配置模式、示例与教程 |
| 🎙️ **[语音模式](/user-guide/features/voice-mode)** | 在 CLI、Telegram、Discord 及 Discord 语音频道中进行实时语音交互 |
| 🗣️ **[在 Intellect 中使用语音模式](/guides/use-voice-mode-with-intellect)** | Intellect 语音工作流的实操配置与使用模式 |
| 🎭 **[个性与 SOUL.md](/user-guide/features/personality)** | 通过全局 SOUL.md 定义 Intellect 的默认风格 |
| 📄 **[上下文文件](/user-guide/features/context-files)** | 影响每次对话的项目上下文文件 |
| 🔒 **[安全](/user-guide/security)** | 命令审批、授权与容器隔离 |
| 💡 **[技巧与最佳实践](/guides/tips)** | 快速上手，充分发挥 Intellect 的潜力 |
...
- **由模型训练者构建** — 由 [ONTOWEB](https://ontoweb.cn) 创建，该实验室是 Intellect、Nomos 和 Psyche 背后的团队。支持 [ONTOWEB Portal](https://portal.ontoweb.cn)、[OpenRouter](https://openrouter.ai)、OpenAI 或任意端点
...
- **研究就绪** — 批处理、轨迹导出、基于 Atropos 的 RL 训练。由 [ONTOWEB](https://ontoweb.cn) 构建——该实验室是 Intellect、Nomos 和 Psyche 模型背后的团队
