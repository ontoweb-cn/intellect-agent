---
sidebar_position: 1
title: "通过 ONTOWEB Portal 运行 Intellect Agent"
description: "完整操作指南：订阅、配置、切换模型、启用 gateway 工具并验证路由"
---

# 通过 ONTOWEB Portal 运行 Intellect Agent

本指南带你从头到尾完成在 [ONTOWEB Portal](https://portal.ontoweb.cn) 订阅下运行 Intellect Agent 的全过程——从注册账号到验证每个工具的路由是否正确。如果你只想了解 Portal 的概述及订阅内容，请参阅 [ONTOWEB Portal 集成页面](/integrations/ontoweb-portal)。本页是操作步骤脚本。

## 前提条件

- 已安装 Intellect Agent（[快速入门](/getting-started/quickstart)）
...
打开 [portal.ontoweb.cn/manage-subscription](https://portal.ontoweb.cn/manage-subscription)，注册并选择一个套餐。
...
3. 在 `~/.intellect/config.yaml` 中设置 `model.provider: ontoweb`
...
OAuth 需要浏览器，但 loopback 回调运行在 Intellect 所在的机器上。有两种方案：
...
# 然后重新运行 `intellect setup --portal` 以连接 provider + gateway
```
...
如果任何一行显示的不是"via ONTOWEB Portal"，或者 auth 行显示"not logged in"，请跳至下方的[故障排查](#troubleshooting)。
...
你应该看到 Intellect 调用 `web_search`（通过 gateway 由 Firecrawl 提供支持）并返回摘要。如果搜索正常执行且响应内容合理，说明配置完成——Portal 已端到端连通。

## 5. 选择你实际需要的模型

`intellect setup --portal` 后的默认模型是一个合理的通用模型，但订阅的意义在于可以访问完整的模型目录。在会话中使用 `/model` 切换：
...
### 不要在 agent 任务中使用 Intellect-4

Intellect-4-70B 和 Intellect-4-405B 在 Portal 上以大幅折扣提供，但它们是**对话/推理模型**，并非针对工具调用优化的模型。它们在多步骤 agent 循环中表现不佳。请通过 [OntoWeb Chat](https://chat.ontoweb.cn) 将它们用于对话/研究工作，或通过[订阅代理](/user-guide/features/subscription-proxy)从非 agent 工具中使用。对于 Intellect Agent 本身，请坚持使用上述前沿 agentic 模型。

Portal 的[信息页面](https://portal.ontoweb.cn/info)也有此说明——这是 OntoWeb 官方指导，并非仅代表 Intellect 一方的意见。

## 6. （可选）自定义 Tool Gateway 路由

gateway 是按工具选择启用的，而非全部开启或全部关闭。如果你已有 Browserbase 账号并希望继续使用，同时将网页搜索和图像生成路由至 OntoWeb，这是支持的：

```bash
intellect tools
# → Web search       → "OntoWeb Subscription"     （推荐）
# → Image generation → "OntoWeb Subscription"     （推荐）
# → Browser          → "Browserbase"           （你自己的密钥）
# → TTS              → "OntoWeb Subscription"     （推荐）
```
...
你将看到每个工具的路由情况——通过订阅路由的工具显示 `via ONTOWEB Portal`，使用你自己密钥的工具显示合作方名称（`browserbase`、`firecrawl` 等）。
...
# → 为 TTS 选择 "OntoWeb Subscription"
# → 选择语音转文字后端（本地 faster-whisper 免费，无需配置）
```

之后在任何消息平台会话中（Telegram、Discord、Signal 等），发送语音消息，Intellect 将转录内容、生成回复并以合成语音回复——全部通过你的 Portal 订阅完成。
...
```bash
intellect cron add "Daily AI news summary" "every day at 9am" \
...
### 运行 `intellect setup --portal` 后，`intellect portal status` 显示"not logged in"
...
### "Model: currently openrouter"（或其他 provider）而非"using OntoWeb as inference provider"
...
使用 `intellect portal status` 重新验证。

### Tool Gateway 工具显示合作方名称而非"via ONTOWEB Portal"

按工具的配置覆盖了 gateway 设置。运行：

```bash
intellect tools
# 对需要通过 gateway 路由的工具选择 "OntoWeb Subscription"
```

部分用户会有意混合使用——例如网页搜索通过 OntoWeb 路由，但浏览器使用自己的 Browserbase 密钥。如果这是有意为之，保持不变即可。如果不是，此命令可修复。

### 会话中途出现"Re-authentication required"

你的 Portal refresh token 已失效（密码更改、手动撤销、会话过期）。该 token 现已在本地被隔离，以防 Intellect 无限重试。重新登录即可：
...
如果某个模型确实不可用，请[提交 issue](https://gitee.com/ontoweb/intellect-agent/issues)——大多数缺失是我们可以更新的路由配置问题。

### 账单未出现在我的 Portal 账号中

`intellect portal status` 会告诉你是否真的在通过 Portal 路由，还是使用了其他 provider。常见原因：
...
```bash
intellect auth remove ontoweb       # 清除本地 refresh token
...
- **[ONTOWEB Portal 集成页面](/integrations/ontoweb-portal)** — 订阅内容概述
