...
:::tip 想要实时审查？
如果你有可用的公开端点，请查看[使用 Webhook 自动化 GitHub PR 评论](./webhook-github-pr-review.md)——GitHub 会在 PR 被打开或更新时立即向 Intellect 推送事件。
:::
...
- **已安装 Intellect Agent** — 参见[安装指南](/getting-started/installation)
...
:::tip 没有消息通知？没关系
使用 `deliver: "local"` 将审查结果保存到 `~/.intellect/cron/output/`。在接入通知之前用于测试非常方便。
:::
...
Skill 为 Intellect 提供一致的审查准则，在会话和 cron 运行之间持久保存。没有 skill，审查质量会参差不齐。

```bash
mkdir -p ~/.intellect/skills/code-review
```

创建 `~/.intellect/skills/code-review/SKILL.md`：
...
验证是否已加载——启动 `intellect`，你应该能在启动时的 skill 列表中看到 `code-review`。

---

## 第四步：教会它你的团队规范

这才是让审查器真正有用的关键。启动一个会话，向 Intellect 传授你的团队标准：
...
2. 通过 memory（记忆）向 Intellect 传授你的团队规范（第四步）
...
- **[构建 Plugin](/guides/build-an-intellect-plugin)** — 将审查逻辑封装为可共享的 plugin
- **[Profiles](/user-guide/agents)** — 运行一个专属审查器 profile，拥有独立的 memory 和配置
