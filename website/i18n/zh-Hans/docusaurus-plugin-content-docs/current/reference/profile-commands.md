...
:::info 已从 Profile 改名
规范命令为 `intellect agent`。`intellect profile` 仍为兼容别名。磁盘路径见 [Agents](../user-guide/profiles.md)。
:::

本页涵盖所有与 [Intellect agents](../user-guide/profiles.md) 相关的命令。通用 CLI 命令请参阅 [CLI 命令参考](./cli-commands.md)。
...
| `install` | 从 git URL 或本地目录安装 agent 发行版。参见 [Profile 发行版](../user-guide/profile-distributions.md)。 |
...
| `--alias` | 同时创建 shell wrapper（例如 `telemetry` → `intellect -p telemetry`）。 |
