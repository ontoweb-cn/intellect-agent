...
本页涵盖所有与 [Intellect profiles](../user-guide/profiles.md) 相关的命令。通用 CLI 命令请参阅 [CLI 命令参考](./cli-commands.md)。
...
管理 profile 的顶级命令。不带子命令运行 `intellect profile` 将显示帮助信息。
...
| `--description "<text>"` | 一到两句话描述该 profile 的用途。供 kanban 编排器根据角色而非仅凭 profile 名称来路由任务。可跳过，稍后通过 `intellect profile describe` 添加。持久化保存在 `<profile_dir>/profile.yaml` 中。 |
...
:::info
`intellect profile export` / `import` 仍是在**本机进行 profile 本地备份和恢复**的正确命令。发行版（`install` / `update` / `info`）是独立概念：通过 git 分发 profile，供他人安装。
:::
...
`intellect profile list` 也会在 `Distribution` 列中显示发行版名称和版本，`intellect profile show <name>` / `delete <name>` 会显示来源 URL，让你一眼看出哪些 profile 来自 git 仓库，哪些是本地创建的。
...
3. 告知接收方运行 `intellect profile install <your-repo-url>`。
...
安装后，Tab 补全适用于：
- `intellect profile <TAB>` — 子命令（list、use、create 等）
- `intellect profile use <TAB>` — profile 名称
