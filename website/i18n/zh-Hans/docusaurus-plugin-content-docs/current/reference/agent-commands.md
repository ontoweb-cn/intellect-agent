...
:::info 已从 Profile 改名
规范命令为 `intellect agent`。`intellect agent` 仍为兼容别名。磁盘路径见 [Agents](../user-guide/agents.md)。
:::

本页涵盖所有与 [Intellect agents](../user-guide/agents.md) 相关的命令。通用 CLI 命令请参阅 [CLI 命令参考](./cli-commands.md)。
...
管理 agent home 的顶级命令。不带子命令运行将显示当前活跃 agent 状态。
...
| `delete` | 删除 agent。 |
| `show` | 显示 agent 详情。 |
| `alias` | 重新生成 agent 的 shell alias。 |
| `rename` | 重命名 agent。 |
| `export` | 将 agent 导出为 tar.gz 归档文件。 |
| `import` | 从 tar.gz 归档文件导入 agent。 |
| `install` | 从 git URL 或本地目录安装 agent 发行版。参见 [Profile 发行版](../user-guide/agent-distributions.md)。 |
| `update` | 重新拉取发行版管理的 agent 并重新应用其 bundle。 |
| `info` | 显示 agent 的发行版元数据（来源 URL、commit、最后更新时间）。 |
...
列出所有 agent。当前活跃的 agent 以标记突出显示。
...
将 `<name>` 设为活跃 profile。此后所有 `intellect` 命令（不带 `-p`）都将使用该 profile。
...
| `--description "<text>"` | 一到两句话描述该 profile 的用途。供 kanban 编排器根据角色而非仅凭 profile 名称来路由任务。可跳过，稍后通过 `intellect agent describe` 添加。持久化保存在 `<profile_dir>/profile.yaml` 中。 |
| `--no-skills` | 创建一个**空** profile，不启用任何内置 skill。会在 profile 目录中写入 `.no-skills` 标记，使后续 `intellect update` 不再重新植入内置 skill 集，且拒绝与 `--clone` / `--clone-all` 组合使用（因为后者会复制 skill）。适用于不应继承完整 skill 目录的窄化编排器 profile 或沙箱 profile。 |
...
此处显示的是 profile 的 Intellect 主目录，而非终端工作目录。终端命令从 `terminal.cwd` 启动（或在本地后端 `cwd: "."` 时从启动目录启动）。
...
重新生成位于 `~/.local/bin/<name>` 的 shell alias 脚本。适用于 alias 被意外删除，或移动 Intellect 安装目录后需要更新的情况。
...
:::tip
**初次接触发行版？** 请先阅读 [Profile 发行版用户指南](../user-guide/agent-distributions.md) — 其中通过完整示例介绍了原因、时机和方法。以下章节是在你已知需求时使用的简明 CLI 参考。
:::
...
:::info
`intellect agent export` / `import` 仍是在**本机进行 profile 本地备份和恢复**的正确命令。发行版（`install` / `update` / `info`）是独立概念：通过 git 分发 profile，供他人安装。
:::
...
| `--alias` | 同时创建 shell wrapper（例如 `telemetry` → `intellect -a telemetry`）。 |
...
打印 profile 的发行版 manifest — 名称、版本、所需 Intellect 版本、作者、环境变量要求、来源 URL/路径，以及发行版最后一次 `install` 或 `update` 时记录的 `Installed:` 时间戳。适用于安装前检查共享 profile 的需求，以及发现"该 profile 已安装 6 个月未更新"等情况。

`intellect agent list` 也会在 `Distribution` 列中显示发行版名称和版本，`intellect agent show <name>` / `delete <name>` 会显示来源 URL，让你一眼看出哪些 profile 来自 git 仓库，哪些是本地创建的。
...
`intellect_requires` 支持 `>=`、`<=`、`==`、`!=`、`>`、`<`，或裸版本号（视为 `>=`）。若当前 Intellect 版本不满足规格，安装将失败并给出明确错误。
...
2. 初始化 git 仓库（或使用已有仓库），推送到 GitHub / GitLab / 任何 Intellect 可克隆的托管平台。
3. 告知接收方运行 `intellect agent install <your-repo-url>`。
...
全局标志，用于在不更改默认 profile 的情况下，在指定 profile 下运行任意 Intellect 命令。仅在该命令执行期间覆盖活跃 profile。
...
安装后，Tab 补全适用于：
- `intellect agent <TAB>` — 子命令（list、use、create 等）
- `intellect agent use <TAB>` — profile 名称
- `intellect -a <TAB>` — profile 名称

## 另请参阅

- [Profiles 用户指南](../user-guide/agents.md)
