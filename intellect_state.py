#!/usr/bin/env python3
"""
SQLite State Store for Intellect Agent.
...
# intellect_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def allow_explicit_session_db_path() -> bool:
    """Return True when ``SessionDB(db_path=…)`` overrides are permitted.

    Tests and one-off migrate tooling may set ``INTELLECT_ALLOW_SQLITE_DB_PATH=1``.
    """
    import os
    import sys

    if os.environ.get("INTELLECT_ALLOW_SQLITE_DB_PATH", "").strip().lower() in (
        "1",
        "true",
        "yes",
    ):
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``intellect_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see intellect_cli/kanban_db.py) would
...
def _seed_oauth_providers(cursor) -> None:
    """Seed built-in OAuth providers from ``agent/oauth/catalog/builtin_providers.json``."""
...
class SessionDB:
    ...
    # With multiple intellect processes (gateway + CLI sessions + worktree agents)
    ...
    def _query_conn(self):
        """Read connection for SELECT-heavy paths."""
        return self._conn
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. This usually means Intellect is running on an "
            ...
            "https://www.intellect-agent.ai (underlying error: %s)",
            self.db_path,
            exc,
        )

    # Process-level FTS5 support cache (probing creates+drops a temp table).
    _fts5_support_cached: bool | None = None

    @classmethod
    def reset_process_caches(cls) -> None:
        """Clear process-level caches (tests, env-switching fixtures)."""
        cls._fts5_support_cached = None
        cls._schema_columns_cache = None

    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        if SessionDB._fts5_support_cached is not None:
            return SessionDB._fts5_support_cached
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._intellect_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._intellect_fts5_probe")
            SessionDB._fts5_support_cached = True
            return True
        except sqlite3.OperationalError as exc:
    ...
    def _try_wal_checkpoint(self) -> None:
        ...
        if self._storage_backend is not None:
            self._storage_backend.try_wal_checkpoint()
    ...
    # Process-level schema column cache — SCHEMA_SQL is a module constant.
    ...
    def _parse_schema_columns(schema_sql: str) -> Dict[str, Dict[str, str]]:
        ...
        Uses an in-memory SQLite database to parse. Results are cached
        per process since SCHEMA_SQL never changes at runtime.
        """
        if SessionDB._schema_columns_cache is not None:
            return SessionDB._schema_columns_cache
        ...
        try:
            ...
            SessionDB._schema_columns_cache = table_columns
    ...
    def _init_schema(self):
        ...
        # ── Membership / teams / projects indexes ─────────────────────
        # Created after _reconcile_columns so WHERE clauses referencing
        # reconciler-added columns (member_id, team_id, project_id) work.
        for _idx_sql in [
            "CREATE INDEX IF NOT EXISTS idx_members_login_name ON members(login_name) WHERE login_name IS NOT NULL",
            "CREATE INDEX IF NOT EXISTS idx_members_enabled ON members(enabled)",
            "CREATE INDEX IF NOT EXISTS idx_identities_member ON identities(member_id)",
            "CREATE INDEX IF NOT EXISTS idx_member_api_tokens_hash ON member_api_tokens(token_hash)",
            ...
            "ON members(display_name) WHERE display_name IS NOT NULL AND display_name != ''",
            ...
            try:
                ...
                logger.debug("Index create skipped: %s — %s", _idx_sql[:60], exc)
        ...
        if row is None:
            ...
            if current_version < 15:
                # v15: members, teams, and projects tables (multi-user foundation).
                # All new tables are created via IF NOT EXISTS in SCHEMA_SQL above;
                # all new session columns (member_id, team_id, project_id,
                # project_workspace) are added by _reconcile_columns().
                # No row backfill — pre-members sessions keep NULL member_id until migration
                # for all four columns, which is the correct legacy representation.
                pass
            if current_version < 16:
                # v16: member_role_bindings table — schema placeholder for v2
                # database-driven RBAC.  Table is created via IF NOT EXISTS in
                # SCHEMA_SQL above.  No row backfill needed; v1 uses hard-coded
                # ROLE_PERMISSIONS in agent/membership.py.
                pass
            if current_version < 17:
                # v17: member password authentication columns.
                # password_hash / password_salt: scrypt-hashed password
                # (hashlib.scrypt, n=32768, r=8, p=1 — Python 3.12+ stdlib)
                # password_reset_code / password_reset_expiry: owner-initiated reset
                # password_set_at: timestamp of last password change
                # failed_login_count / locked_until: brute-force lockout
                # role: global role (owner / admin / member) — NULL = member
                # All columns are added by _reconcile_columns(); no backfill needed.
                pass
            if current_version < 18:
                # v18: member_sessions table for online presence tracking.
                # Table is created via IF NOT EXISTS in SCHEMA_SQL above.
                # members.last_active_at / last_active_platform / online_status
                # are added by _reconcile_columns(); no backfill needed.
                pass
            if current_version < 19:
                # v19: oauth_providers + oauth_tokens tables for unified OAuth.
                # Tables are created via IF NOT EXISTS in SCHEMA_SQL above.
                # identities.metadata column added by _reconcile_columns().
                # Seed built-in providers so the table is immediately usable.
                _seed_oauth_providers(cursor)
            if current_version < 25:
                # v25: wiki_contributions — member → global LLM Wiki merge queue.
                cursor.executescript(
                    ...
                    CREATE INDEX IF NOT EXISTS idx_wiki_contributions_status
            ...
            if current_version < 23:
                # v23: oauth_pending_states — member login OAuth state (multi-worker safe).
                cursor.executescript(
                    """
            ...
            if current_version < 22:
                # v22: oauth_pool_entries for multi-credential model OAuth (PR-A7).
                cursor.executescript(
                    """
            ...
            if current_version < 21:
                # v21: role_definitions (single-user: skip RBAC seeding)
                # Built-in RBAC roles are not needed in single-user mode.
                ...
                # seed_builtin_role_definitions removed (single-user mode)
            if current_version < 20:
                # v20: member_admin_audit_log + unique display_name per profile.
                cursor.executescript(
                    """
                ...
                try:
                    cursor.execute(
                        "CREATE UNIQUE INDEX IF NOT EXISTS idx_members_display_name_unique "
                        "ON members(display_name) WHERE display_name IS NOT NULL "
                        "AND display_name != ''"
                    )
                except sqlite3.IntegrityError:
                    pass  # existing duplicates — doctor should report; app-layer still enforces
        ...
        # Unique title among root sessions only — compression continuations often
        # reuse the parent title (e.g. default "Cli Session") and must not collide.
        try:
            cursor.execute("DROP INDEX IF EXISTS idx_sessions_title_unique")
            cursor.execute(
                "CREATE UNIQUE INDEX IF NOT EXISTS idx_sessions_title_unique "
                "ON sessions(title) "
                "WHERE title IS NOT NULL AND parent_session_id IS NULL"
            )
        except sqlite3.OperationalError:
    ...
    def get_session(
        ...
        *,
        ...
        if row is None:
            return None
        session = dict(row)
        # (single-user: no session visibility filtering)
        return session
    ...
    def list_sessions_rich(
        ...
        When *member_id* is provided (multi-user mode), sessions are scoped per
        ``members.session_isolation`` (strict by default; owner/admin may see all).
        """
        where_clauses = []
        params = []

        # (single-user: no member isolation filtering)
    ...
    def get_messages_as_conversation(
        ...
        for row in rows:
            ...
            # that replay reasoning (OpenRouter, OpenAI, OntoWeb) receive
    ...
    def _is_duplicate_replayed_user_message(messages: List[Dict[str, Any]], msg: Dict[str, Any]) -> bool:
        """Check for duplicate user content within the current turn.

        Uses an O(1) hash-set per-turn rather than O(n) reverse scan,
        resetting when an assistant response with content/tool_calls is seen.
        """
        ...
        # Walk backward to find the turn boundary (last assistant with content).
        # Collect seen user-contents in a set for O(1) lookup vs the O(n) scan.
        seen: set[str] = set()
        for prev in reversed(messages):
            ...
            if prev.get("role") == "user":
                ...
                if isinstance(prev_content, str) and prev_content:
                    seen.add(prev_content)
        return content in seen
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Intellect, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Intellect session.

        A Intellect session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Intellect session is already bound to any Telegram DM topic.
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``intellect_DISABLE_FTS_TRIGRAM`` or not yet created), so
