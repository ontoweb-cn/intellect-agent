"""Tests for intellect_constants module."""
...
class TestGetDefaultintellectRoot:
    """Tests for get_default_intellect_root() — Docker/custom deployment awareness."""

    def test_no_intellect_home_returns_native(self, tmp_path, monkeypatch):
        """When INTELLECT_HOME is not set, returns ~/.intellect."""
        monkeypatch.delenv("INTELLECT_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_intellect_root() == tmp_path / ".intellect"

    def test_intellect_home_is_native(self, tmp_path, monkeypatch):
        """When INTELLECT_HOME = ~/.intellect, returns ~/.intellect."""
        native = tmp_path / ".intellect"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("INTELLECT_HOME", str(native))
        assert get_default_intellect_root() == native

    def test_intellect_home_is_profile(self, tmp_path, monkeypatch):
        """When INTELLECT_HOME is a profile under ~/.intellect, returns ~/.intellect."""
        native = tmp_path / ".intellect"
    ...
    def test_intellect_home_is_docker(self, tmp_path, monkeypatch):
        """When INTELLECT_HOME points outside ~/.intellect (Docker), returns INTELLECT_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("INTELLECT_HOME", str(docker_home))
        assert get_default_intellect_root() == docker_home

    def test_intellect_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any INTELLECT_HOME outside ~/.intellect is treated as the root."""
        custom = tmp_path / "my-intellect-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("INTELLECT_HOME", str(custom))
        assert get_default_intellect_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (INTELLECT_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("INTELLECT_HOME", str(profile))
        assert get_default_intellect_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(intellect_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(intellect_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".intellect"
