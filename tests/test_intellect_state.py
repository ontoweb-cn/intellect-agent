"""Tests for intellect_state.py — SessionDB SQLite CRUD, FTS5 search, export."""

import sqlite3
import time
import pytest

from intellect_state import SCHEMA_SQL, SessionDB
...
def db(tmp_path):
    ...
    SessionDB.reset_process_caches()  # clear FTS5/schema caches across env changes
...
class TestSessionLifecycle:
    ...
    def test_db_initializes_without_fts5_module(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("intellect_state.sqlite3.connect", connect_without_fts)
    ...
    def test_fts_runtime_restores_triggers_after_no_fts_open(
        ...
        monkeypatch.setattr("intellect_state.sqlite3.connect", real_connect)
        SessionDB.reset_process_caches()
...
class TestSchemaInit:
    ...
    def test_schema_version(self, db):
        from intellect_state import SCHEMA_VERSION
    ...
    def test_topic_mode_schema_is_not_auto_migrated_on_open(self, tmp_path):
        ...
        The gateway must remain rollback-safe: simply upgrading Intellect and starting
    ...
    def test_schema_sql_is_source_of_truth(self, db):
        ...
        """
        from intellect_state import SCHEMA_SQL
...
# (single-user: member session isolation tests removed — all sessions visible)

    def test_no_actor_returns_all(self, db):
        db.create_session("s1", source="cli", member_id="bob")
        session = db.get_session("s1")
        assert session is not None
...
class TestConcurrentWriteSafety:
    ...
    def test_sqlite_short_timeout_with_jitter_retry(self, db):
        """Short connect timeout; lock contention retried in SQLiteBackend.execute_write."""
        import inspect

        from agent.storage.sqlite_backend import SQLiteBackend

        init_src = inspect.getsource(SQLiteBackend.initialize)
        assert "timeout=1.0" in init_src
        write_src = inspect.getsource(SQLiteBackend.execute_write)
        assert "BEGIN IMMEDIATE" in write_src
        assert "WRITE_MAX_RETRIES" in write_src
