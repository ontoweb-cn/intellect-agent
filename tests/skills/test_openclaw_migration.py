...
"""
    ...
    assert "Tyler Williams > Active Projects: Intellect Agent" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `intellect cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # intellect_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.intellect/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "intellect_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``Intellect``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Intellect prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Intellect to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Intellect config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Intellect uses tools well"
    # All-lowercase matches → lowercase ``intellect``; this preserves the
    # real filesystem path ``~/.intellect`` (Intellect home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "intellect should always respond concisely"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    # Same case-preservation rule as above.
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Intellect remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "intellect prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Intellect was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "intellect uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Intellect said to ask Intellect about Intellect settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.intellect`` (the real Intellect home), not the broken
    ``.Intellect``.
    ...
    rewritten to ``~/.Intellect/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.intellect/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .intellect directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.intellect'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "intellect config path: ~/.intellect/"
...
def test_migrate_memory_rebrands_entries(tmp_path):
    ...
    target_root = tmp_path / "intellect"
    target_root.mkdir()
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are Intellect" in result
