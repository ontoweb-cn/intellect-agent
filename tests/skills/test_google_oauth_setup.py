...
class TestIntellectConstantsFallback:
    """Tests for _intellect_home.py fallback when intellect_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_intellect_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _intellect_home.py with intellect_constants blocked."""
        monkeypatch.setitem(sys.modules, "intellect_constants", None)
        spec = importlib.util.spec_from_file_location("_intellect_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_intellect_home_env_var(self, monkeypatch, tmp_path):
        """When intellect_constants is missing, INTELLECT_HOME comes from env var."""
        monkeypatch.setenv("INTELLECT_HOME", str(tmp_path / "custom-intellect"))
        module = self._load_helper(monkeypatch)
        assert module.get_intellect_home() == tmp_path / "custom-intellect"

    def test_fallback_defaults_to_dot_intellect(self, monkeypatch):
        """When intellect_constants is missing and INTELLECT_HOME unset, default to ~/.intellect."""
        monkeypatch.delenv("INTELLECT_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_intellect_home() == Path.home() / ".intellect"

    def test_fallback_ignores_empty_intellect_home(self, monkeypatch):
        """Empty/whitespace INTELLECT_HOME is treated as unset."""
        monkeypatch.setenv("INTELLECT_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_intellect_home() == Path.home() / ".intellect"

    def test_fallback_display_intellect_home_shortens_path(self, monkeypatch):
        """Fallback display_intellect_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("INTELLECT_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_intellect_home() == "~/.intellect"

    def test_fallback_display_intellect_home_profile_path(self, monkeypatch):
        """Fallback display_intellect_home() handles profile paths under ~/."""
        monkeypatch.setenv("INTELLECT_HOME", str(Path.home() / ".intellect/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_intellect_home() == "~/.intellect/profiles/coder"

    def test_fallback_display_intellect_home_custom_path(self, monkeypatch):
        """Fallback display_intellect_home() returns full path for non-home locations."""
        monkeypatch.setenv("INTELLECT_HOME", "/opt/intellect-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_intellect_home() == "/opt/intellect-custom"

    def test_delegates_to_intellect_constants_when_available(self):
        """When intellect_constants IS importable, _intellect_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_intellect_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import intellect_constants
        assert module.get_intellect_home is intellect_constants.get_intellect_home
        assert module.display_intellect_home is intellect_constants.display_intellect_home
