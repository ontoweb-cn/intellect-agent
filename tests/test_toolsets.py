...
class TestToolsetConsistency:
    ...
    def test_intellect_platforms_share_core_tools(self):
        """All intellect-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on intellect-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["intellect-cli", "intellect-telegram", "intellect-discord", "intellect-whatsapp", "intellect-slack", "intellect-signal", "intellect-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_intellect_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("intellect-whatsapp")

    def test_intellect_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("intellect-api-server")
