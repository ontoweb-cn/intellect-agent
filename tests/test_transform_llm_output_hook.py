...
from intellect_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(intellect_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <intellect_home>/plugins/<name> and opt it in."""
    plugin_dir = intellect_home / "plugins" / name
...
def test_hook_receives_expected_kwargs(tmp_path, monkeypatch):
    ...
    intellect_home.mkdir(exist_ok=True)
    _make_enabled_plugin(
        intellect_home, "capture_hook",
        register_body=(
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("INTELLECT_HOME", str(tmp_path / "intellect_empty"))
