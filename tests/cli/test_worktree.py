...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("intellect/intellect-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned intellect/* and pr-* branches."""

    def test_prunes_orphaned_intellect_branch(self, git_repo):
        """intellect/intellect-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "intellect/intellect-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "intellect/intellect-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "intellect/intellect-deadbeef" in result.stdout
        ...
        assert "intellect/intellect-deadbeef" in orphaned
        ...
        assert "intellect/intellect-deadbeef" not in result.stdout
