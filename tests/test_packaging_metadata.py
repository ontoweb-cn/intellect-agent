...
def test_every_on_disk_subpackage_is_covered_by_packages_find():
    ...
    top-level package is listed twice — bare (``intellect_cli``) for the package
    itself and ``intellect_cli.*`` for its subpackages — EXCEPT when someone
    forgets the wildcard. v0.15.x listed ``intellect_cli`` without ``intellect_cli.*``,
    so the wheel shipped ``intellect_cli/*.py`` but dropped subpackages
    like ``proxy``, causing ``ModuleNotFoundError`` on every install.
...
def test_bundled_plugin_manifests_ship_in_both_wheel_and_sdist():
    ...
    Plugin discovery (intellect_cli/plugins.py) registers each bundled plugin by
