...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate INTELLECT_HOME under tmp_path."""
    monkeypatch.setenv("INTELLECT_HOME", str(tmp_path / ".intellect"))
    (tmp_path / ".intellect").mkdir()

    # Force the constants/image cache helpers to re-read INTELLECT_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("intellect_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_intellect_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under INTELLECT_HOME — gateway cleanup
