...
class TestDefaultContextLengths:
    ...
    def test_deepseek_v4_models_1m_context(self):
        ...
        # / ONTOWEB Portal) to 1M without probing down to the legacy 128K
...
# =========================================================================
# ONTOWEB Portal context-window resolution (provider="ontoweb")
# =========================================================================

class TestNousPortalContextResolution:
    """ONTOWEB Portal /v1/models is authoritative for what OntoWeb infra enforces
    ...
    def test_portal_value_wins_over_openrouter_catalog(
        ...
        the OntoWeb portal correctly enforces 262144.  Portal must win."""
        import agent.model_metadata as mm
        cache_file = tmp_path / "context_length_cache.yaml"
        monkeypatch.setattr(mm, "_get_context_cache_path", lambda: cache_file)
    ...
    def test_stale_cache_is_bypassed_and_overwritten_by_portal(
        ...
        """Users upgrading from pre-fix builds have ``qwen3.6-plus@…ontoweb… =
    ...
    def test_bypass_keyed_on_url_not_provider_string(
        ...
        when the user is really on ONTOWEB Portal (e.g. cred-pool fallback).
        The OntoWeb-URL bypass must trigger off the URL host, not the provider
        ...
        for provider_arg in ("", "openrouter", "custom"):
            ...
            assert ctx == 262_144, (
                f"URL-based OntoWeb detection must fire for provider={provider_arg!r}; "
                f"got {ctx}"
            )
