...
class TestChatCompletionsBasic:
    ...
    def test_convert_messages_strips_internal_scaffolding_markers(self, transport):
        """Intellect-internal ``_``-prefixed markers must never reach the wire.
...
class TestChatCompletionsBuildKwargs:
    ...
    def test_nous_tags(self, transport):
        ...
        assert kw["extra_body"]["tags"] == ontoweb_portal_tags()
    ...
    def test_nous_omits_disabled_reasoning(self, transport):
        ...
        # OntoWeb rejects enabled=false; reasoning omitted entirely
    ...
    def test_gemma_does_not_receive_thinking_config(self, transport):
        ...
        # when Intellect has reasoning enabled, the field must be omitted for
...
class TestChatCompletionsKimi:
    ...
    def test_moonshot_tool_schemas_are_sanitized_by_model_name(self, transport):
        """Aggregator routes (OntoWeb, OpenRouter) hit Moonshot by model name, not base URL."""
        tools = [
            {
                "type": "function",
                "function": {
