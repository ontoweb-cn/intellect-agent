...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Intellect passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("INTELLECT_HOME", "/users/alice/.intellect/profiles/backend-worker")
        monkeypatch.setenv("INTELLECT_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "intellect_KANBAN_DB",
            "/users/alice/.intellect/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.intellect/kanban/boards/smoke"]'
            in cmd
        )
