"""Tests for the profile-scoped credential primitive."""
import pytest
...
class TestMultiplexActiveFailClosed:
    ...
    def test_global_env_still_reads_environ_under_multiplex(self, monkeypatch):
        monkeypatch.setenv("INTELLECT_HOME", "/opt/data")
        ss.set_multiplex_active(True)
        assert ss.get_secret("INTELLECT_HOME") == "/opt/data"

    def test_kanban_prefix_is_global(self, monkeypatch):
        monkeypatch.setenv("INTELLECT_KANBAN_DB", "/x/kanban.db")
        ss.set_multiplex_active(True)
        assert ss.get_secret("INTELLECT_KANBAN_DB") == "/x/kanban.db"
