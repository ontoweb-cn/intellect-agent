...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Intellect Tests")
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_intellect_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("INTELLECT_HOME", str(tmp_path / ".intellect"))

    intellect_env = tmp_path / ".intellect" / ".env"
    intellect_env.parent.mkdir(parents=True)
    intellect_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.intellect/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
