...
def curator_env(tmp_path, monkeypatch):
    """Isolated INTELLECT_HOME + freshly reloaded curator + skill_usage modules."""
...
def test_dry_run_does_not_advance_state(curator_env, monkeypatch):
    ...
    `intellect curator status`. Fixes #18373.
    """
    c = curator_env["curator"]
    u = curator_env["usage"]
    skills_dir = curator_env["home"] / "skills"
...
def test_cli_unpin_refuses_bundled_skill(curator_env, capsys):
    """intellect curator unpin must refuse bundled/hub skills too (matches pin)."""
    from intellect_cli import curator as cli
    skills_dir = curator_env["home"] / "skills"
...
# `intellect model` → auxiliary picker, the dashboard Models tab, and the full
...
def test_review_model_new_slot_wins_over_legacy(curator_env):
    ...
    cfg = {
        "model": {"provider": "openrouter", "default": "openai/gpt-5.5"},
        "auxiliary": {
            "curator": {"provider": "ontoweb", "model": "new-winner"},
        },
        "curator": {
            "auxiliary": {"provider": "openrouter", "model": "legacy-loser"},
        },
    }
    assert curator._resolve_review_model(cfg) == ("ontoweb", "new-winner")
...
def test_curator_slot_is_canonical_aux_task():
    ...
    # 2. intellect_cli/main.py _AUX_TASKS — CLI picker
