...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_intellect_env(self):
        """Intellect' own .env inside INTELLECT_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: intellect internal .env is in ~/.intellect/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.intellect/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.intellect/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Intellect cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        intellect_home = tmp_path / ".intellect"
        cache = intellect_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._intellect_home_path", return_value=intellect_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Intellect cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        intellect_home = tmp_path / ".intellect"
        hub = intellect_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_intellect_home(self, tmp_path):
        """The env basename guard does not depend on INTELLECT_HOME resolution."""
        intellect_home = tmp_path / ".intellect"
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        intellect_home = tmp_path / ".intellect"
        cache = intellect_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
