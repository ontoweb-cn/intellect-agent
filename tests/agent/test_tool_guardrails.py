...
def test_merge_enables_hard_stop_for_gateway_platform():
    merged = merge_tool_loop_guardrails_for_platform(
        {"hard_stop_enabled": False},
        "telegram",
    )
    cfg = ToolCallGuardrailConfig.from_mapping(merged)
    assert cfg.hard_stop_enabled is True


def test_merge_leaves_cli_soft_by_default():
    merged = merge_tool_loop_guardrails_for_platform(
        {"hard_stop_enabled": False},
        "cli",
    )
...
def test_merge_enables_hard_stop_for_cron():
    cfg = ToolCallGuardrailConfig.from_mapping(
        merge_tool_loop_guardrails_for_platform({}, "cron")
    )
    assert cfg.hard_stop_enabled is True
