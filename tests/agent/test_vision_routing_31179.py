...
def isolated_home(monkeypatch):
    """Temp INTELLECT_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="intellect_test_31179_")
    intellect_home = os.path.join(test_home, ".intellect")
    os.makedirs(intellect_home)
...
def _fresh_modules():
    """Drop cached intellect modules so each test reloads against current env."""
