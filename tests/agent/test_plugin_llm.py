...
""",
        ...
        from intellect_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        intellect_home = tmp_path / ".intellect"
        intellect_home.mkdir()
        (intellect_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))
        from intellect_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from intellect_cli.plugins import PluginContext, PluginManifest, PluginManager
