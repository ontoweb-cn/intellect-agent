...
Prior to this change, aggregator users (OpenRouter / ONTOWEB Portal) had aux
...
class TestResolveAutoMainFirst:
    ...
    def test_nous_main_uses_main_model_for_aux(self, monkeypatch):
        """ONTOWEB Portal main user → aux uses their picked OntoWeb model, not free-tier MiMo."""
        # No OPENROUTER_API_KEY → ensures if main failed we'd fall to chain
        with patch(
            "agent.auxiliary_client._read_main_provider", return_value="ontoweb",
        ), patch(
            "agent.auxiliary_client._read_main_model",
        ...
        assert mock_resolve.call_args.args[0] == "ontoweb"
...
class TestResolveVisionMainFirst:
    ...
    def test_nous_main_vision_uses_paid_nous_vision_backend(self):
        """Paid OntoWeb main → aux vision uses the dedicated OntoWeb vision backend."""
        with patch(
            "agent.auxiliary_client._read_main_provider", return_value="ontoweb",
        ), patch(
            "agent.auxiliary_client._read_main_model",
    ...
    def test_nous_main_vision_uses_free_tier_nous_vision_backend(self):
        """Free-tier OntoWeb main → aux vision uses MiMo omni, not the text main model."""
        with patch(
            "agent.auxiliary_client._read_main_provider", return_value="ontoweb",
        ), patch(
            "agent.auxiliary_client._read_main_model",
    ...
    def test_copilot_vision_sets_vision_header(self, monkeypatch):
        ...
        with patch(
            ...
            "intellect_cli.copilot_auth.copilot_request_headers",
    ...
    def test_main_unavailable_vision_falls_through_to_aggregators(self):
        """Main provider fails → fall back to OpenRouter/OntoWeb strict backends."""
        ...
        assert provider in {"openrouter", "ontoweb"}
    ...
    def test_explicit_provider_override_still_wins(self):
        ...
        with patch(
            ...
            "agent.auxiliary_client._resolve_task_provider_model",
            return_value=("ontoweb", None, None, None, None),  # explicit override
        ), patch(
            "agent.auxiliary_client._resolve_strict_vision_backend"
        ) as mock_strict:
            mock_strict.return_value = (MagicMock(), "ontoweb-default-model")

            from agent.auxiliary_client import resolve_vision_provider_client

            provider, client, model = resolve_vision_provider_client()

        # Explicit "ontoweb" override → uses strict backend, NOT main model path
        assert provider == "ontoweb"
        mock_strict.assert_called_once_with("ontoweb", None)
