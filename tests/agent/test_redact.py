...
def _ensure_redaction_enabled(monkeypatch):
    """Ensure INTELLECT_REDACT_SECRETS is not disabled by prior test imports."""
...
class TestKnownPrefixes:
    ...
    def test_slack_app_level_token(self):
        token = "xapp-1-A" + "B" * 24
        result = redact_sensitive_text(token)
...
class TestWebUrlsNotRedacted:
    """Web-URL query-param redaction is now **enabled** for sensitive params
    (access_token, refresh_token, code, password, token, api_key, secret,
    etc.).  Redaction only affects LOG OUTPUT — the actual HTTP requests
    the agent sends are unmodified.  Non-sensitive params (state, magic,
    format, id, customer) pass through unchanged.

    This protects credentials from leaking into agent.log / errors.log /
    gateway.log while keeping OAuth callbacks, magic-link checkouts, and
    pre-signed share URLs fully functional.
    """

    def test_oauth_callback_code_is_redacted(self):
    ...
    def test_access_token_query_is_redacted(self):
        text = "Fetching https://example.com/api?access_token=opaque_value_here_1234&format=json"
        result = redact_sensitive_text(text)
        assert "access_token=***" in result
        assert "format=json" in result  # format is not sensitive
        assert "opaque_value" not in result

    def test_magic_link_checkout_passes_through(self):
        # "magic" is NOT a sensitive query param — passes through unchanged
        text = "Open https://checkout.example.com/resume?magic=ABCDEF123456&customer=42"
        assert redact_sensitive_text(text) == text

    def test_presigned_signature_is_redacted(self):
        text = "https://s3.amazonaws.com/bucket/k?signature=LONG_PRESIGNED_SIG&id=public"
        result = redact_sensitive_text(text)
        assert "signature=***" in result
        assert "id=public" in result  # id is not sensitive

    def test_https_userinfo_passes_through(self):
        text = "URL: https://user:supersecretpw@host.example.com/path"
        assert redact_sensitive_text(text) == text

    def test_websocket_url_query_is_redacted(self):
        text = "wss://api.example.com/ws?token=opaqueWsToken123"
        result = redact_sensitive_text(text)
        assert "token=***" in result
        assert "opaqueWsToken123" not in result

    def test_full_url_password_query_is_redacted(self):
        # Full URLs (with scheme) are matched and have sensitive params redacted.
        text = "GET https://bluebubbles.example.com/api?password=webhookSecret123&event=new-message"
        result = redact_sensitive_text(text)
        assert "password=***" in result
        assert "webhookSecret123" not in result
        assert "event=new-message" in result  # event is not sensitive

    def test_bare_path_query_not_redacted(self):
        # The URL-with-query regex requires a scheme (https:// etc.) to avoid
        # false positives on bare paths in access logs.  Access-log-level
        # webhook endpoints with password params are expected to use aiohttp's
        # access_log=None (which BlueBubbles already does).
        text = (
