...
def _allow_auth_json_credential_pool_io(monkeypatch):
    """These tests exercise legacy auth.json pool sync paths (pre-A10 defaults)."""
...
def test_round_robin_strategy_rotates_priorities(tmp_path, monkeypatch):
    ...
    config_path = tmp_path / "intellect" / "config.yaml"
...
def test_explicit_reset_timestamp_overrides_default_429_ttl(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "intellect_cli.auth._import_codex_cli_tokens",
        lambda: None,
    )
    _write_auth_store(
        tmp_path,
...
def test_load_pool_persists_bitwarden_origin_metadata_without_secret(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OPENROUTER_API_KEY", sentinel)
    monkeypatch.setattr(
        "intellect_cli.env_loader.get_secret_source",
...
def test_write_credential_pool_sanitizes_borrowed_payload_at_disk_boundary(tmp_path, monkeypatch):
    ...
    from intellect_cli.auth import write_credential_pool
    ...
    write_credential_pool("openrouter", [
        {
            ...
            "auth_type": "api_key",
            "priority": 0,
            "source": "systemd://intellect/openrouter",
            "access_token": sentinel,
            "refresh_token": f"refresh-{sentinel}",
            "agent_key": f"agent-{sentinel}",
            "api_key": f"extra-{sentinel}",
...
def test_load_pool_prefers_dotenv_over_stale_os_environ(tmp_path, monkeypatch):
    ...
    from a parent shell) must NOT shadow the fresh key in ~/.intellect/.env when
    ...
    # User edited ~/.intellect/.env with the fresh key
...
def test_load_pool_falls_back_to_os_environ_when_dotenv_empty(tmp_path, monkeypatch):
    """When ~/.intellect/.env does not define OPENROUTER_API_KEY (typical Docker /
    ...
    (intellect_home / ".env").write_text("SOME_OTHER_VAR=unrelated\n")
...
def test_load_pool_migrates_ontoweb_provider_state(tmp_path, monkeypatch):
    monkeypatch.setenv("INTELLECT_HOME", str(tmp_path / "intellect"))
    _write_auth_store(
        tmp_path,
        {
...
def test_nous_pool_terminal_refresh_removes_device_code_entry(tmp_path, monkeypatch):
    monkeypatch.setenv("INTELLECT_HOME", str(tmp_path / "intellect"))
    monkeypatch.setenv("intellect_SHARED_AUTH_DIR", str(tmp_path / "shared"))
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
    ...
    pool.add_entry(PooledCredential.from_dict("ontoweb", {
        "id": "legacy-seeded",
        "source": "manual:device_code",
        "auth_type": "oauth",
    ...
    pool.add_entry(PooledCredential.from_dict("ontoweb", {
        ...
        "source": "manual",
        "auth_type": "api_key",
        "access_token": "manual-ontoweb-key",
    }))

    monkeypatch.setattr(auth_mod, "resolve_ontoweb_runtime_credentials", _terminal_refresh_failure)
...
def test_load_pool_removes_stale_file_backed_singleton_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.read_intellect_oauth_credentials",
        lambda: None,
    )
    monkeypatch.setattr(
...
def test_load_pool_migrates_ontoweb_provider_state_preserves_tls(tmp_path, monkeypatch):
    monkeypatch.setenv("INTELLECT_HOME", str(tmp_path / "intellect"))
    _write_auth_store(
        tmp_path,
        {
...
def test_singleton_seed_does_not_clobber_manual_oauth_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("ANTHROPIC_TOKEN", raising=False)
    monkeypatch.delenv("CLAUDE_CODE_OAUTH_TOKEN", raising=False)
    monkeypatch.setattr("intellect_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
    ...
    assert {entry.source for entry in entries} == {"manual:intellect_pkce", "intellect_pkce"}
...
def test_load_pool_api_key_path_skips_oauth_autodiscovery(tmp_path, monkeypatch):
    ...
    When the user picks "Anthropic API key" at `intellect setup`,
    ...
    + claude-cli user-agent).  Autodiscovered Claude Code / Intellect PKCE
    ...
    monkeypatch.setattr("agent.anthropic_adapter.read_intellect_oauth_credentials", _fake_pkce)
...
def test_load_pool_api_key_path_prunes_stale_oauth_entries(tmp_path, monkeypatch):
    ...
    `intellect_pkce` into auth.json) and later switches to the API key at
    `intellect setup` would still have those OAuth entries dormant on disk.
    ...
    monkeypatch.setattr("intellect_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    monkeypatch.setattr("agent.anthropic_adapter.read_intellect_oauth_credentials", lambda: None)
...
def test_load_pool_oauth_path_still_autodiscovers(tmp_path, monkeypatch):
    ...
    OAuth path at `intellect setup`.  When ANTHROPIC_TOKEN is set (and
...
def test_load_pool_seeds_qwen_oauth_via_cli_tokens(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "intellect_cli.auth.resolve_qwen_runtime_credentials",
        lambda **kw: {
            "provider": "qwen-oauth",
...
def test_nous_seed_from_singletons_preserves_obtained_at_timestamps(tmp_path, monkeypatch):
    ...
    the ``providers.ontoweb`` singleton, it must carry the mint/refresh
    ...
    assert e.agent_key == "sk-ontoweb-AAAA"
...
# ── PR #10160 salvage: OntoWeb OAuth cross-process sync tests ─────────────────

def test_sync_ontoweb_entry_from_auth_store_adopts_newer_tokens(tmp_path, monkeypatch):
    ...
    synced = pool._sync_ontoweb_entry_from_auth_store(entry)
...
def test_nous_exhausted_entry_recovers_via_auth_store_sync(tmp_path, monkeypatch):
    """An exhausted OntoWeb entry should recover when auth.json has newer tokens."""
    monkeypatch.setenv("INTELLECT_HOME", str(tmp_path / "intellect"))
    from agent.credential_pool import load_pool, STATUS_EXHAUSTED
...
def test_sync_codex_entry_from_auth_store_adopts_newer_tokens(tmp_path, monkeypatch):
    ...
    # Simulate `intellect auth openai-codex` replacing the token pair on disk.
...
def test_codex_exhausted_entry_recovers_via_auth_store_sync(tmp_path, monkeypatch):
    ...
    rate-limit reset the user ran `intellect model` to reauth, but the pool
    ...
    # Simulate `intellect model` / `intellect auth` refreshing the tokens.
...
def test_is_terminal_xai_oauth_refresh_error():
    from intellect_cli.auth import AuthError, _is_terminal_xai_oauth_refresh_error
    ...
    # OntoWeb error does not trigger xAI check
    assert not _is_terminal_xai_oauth_refresh_error(
        AuthError("Revoked", provider="ontoweb", code="invalid_grant", relogin_required=True)
    )
    # Generic exception
...
def test_is_terminal_codex_oauth_refresh_error():
    from intellect_cli.auth import AuthError, _is_terminal_codex_oauth_refresh_error
