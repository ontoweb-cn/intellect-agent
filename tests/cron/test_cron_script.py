...
- Path resolution (absolute, relative to INTELLECT_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp INTELLECT_HOME."""
    intellect_home = tmp_path / ".intellect"
    intellect_home.mkdir()
    (intellect_home / "cron").mkdir()
    (intellect_home / "cron" / "output").mkdir()
    (intellect_home / "scripts").mkdir()
    monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "intellect_DIR", intellect_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", intellect_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", intellect_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", intellect_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.intellect/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("intellect_SESSION_PLATFORM") is None
        assert os.environ.get("intellect_SESSION_CHAT_ID") is None
        assert os.environ.get("intellect_SESSION_CHAT_NAME") is None
