...
INTELLECT_HOME scoping, and tick() serialization for profile jobs.
"""
...
def isolated_cron_profile_home(tmp_path, monkeypatch):
    """Create an isolated Intellect root with a named profile and temp cron store."""
    root = tmp_path / "intellect-root"
...
class TestCronjobToolProfile:
    ...
    def test_schema_advertises_profile(self):
        ...
        assert "intellect profile" in desc_lower
        assert "context-local" in desc_lower
        assert "subprocess" in desc_lower
        assert "temporarily sets intellect_home" not in desc_lower
...
class TestRunJobProfileContext:
    ...
    def _install_agent_stubs(monkeypatch, observed: dict):
        ...
        class FakeAgent:
            def __init__(self, **kwargs):
                ...
                observed["env_home_during_init"] = os.environ.get("INTELLECT_HOME")
                ...
                observed["intellect_home_during_init"] = str(get_intellect_home())
                observed["scheduler_home_during_init"] = str(sched._get_intellect_home())
                observed["skip_context_files"] = kwargs.get("skip_context_files")

            def run_conversation(self, *_a, **_kw):
                from intellect_constants import get_intellect_home

                observed["env_home_during_run"] = os.environ.get("INTELLECT_HOME")
                ...
                observed["intellect_home_during_run"] = str(get_intellect_home())
                observed["scheduler_home_during_run"] = str(sched._get_intellect_home())
    ...
    def test_run_job_sets_and_restores_profile_home(
        ...
        assert observed["intellect_home_during_init"] == str(profile_home.resolve())
        assert observed["intellect_home_during_run"] == str(profile_home.resolve())
    ...
    def test_profile_dotenv_environment_is_restored(
        ...
        def fake_load_dotenv(path, *_a, **_kw):
            observed.setdefault("dotenv_paths", []).append(str(path))
            os.environ["intellect_PROFILE_TEST_SHARED"] = "profile-value"
            os.environ["intellect_PROFILE_TEST_ONLY"] = "profile-only"
            os.environ["intellect_CRON_TIMEOUT"] = "123"
            return True
        ...
        assert os.environ["intellect_CRON_TIMEOUT"] == "0"
        assert os.environ["INTELLECT_HOME"] == str(root)
        assert sched._get_intellect_home() == root
    ...
    def test_no_agent_profile_uses_profile_scripts_dir_and_restores_env(
        ...
        (scripts_dir / "print_home.py").write_text(
            "import os\nprint(os.environ.get('INTELLECT_HOME', ''))\n",
            encoding="utf-8",
        )
        monkeypatch.setattr(sched, "_intellect_home", None)
    ...
    def test_run_job_without_profile_leaves_intellect_home_untouched(
        self, isolated_cron_profile_home, monkeypatch
    ):
        import cron.scheduler as sched
        ...
        assert observed["intellect_home_during_init"] == str(root)
        assert os.environ["INTELLECT_HOME"] == str(root)
