...
class TestTickWorkdirPartition:
    ...
    def test_workdir_jobs_run_sequentially(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(sched, "get_due_jobs", lambda *_a, **_kw: [workdir_job, parallel_job])
...
class TestRunJobTerminalCwd:
    ...
    def _install_stubs(monkeypatch, observed: dict):
        ...
        # Bypass the real provider resolver — it reads ~/.intellect and credentials.
        from intellect_cli import runtime_provider as _rtp
        monkeypatch.setattr(
            _rtp,
        ...
        # run_job calls load_dotenv(~/.intellect/.env, override=True), which will
