"""
Tests for mcp_serve — Intellect MCP server.
...
def _isolate_intellect_home(tmp_path, monkeypatch):
    """Redirect INTELLECT_HOME to a temp directory."""
    monkeypatch.setenv("INTELLECT_HOME", str(tmp_path))
    try:
        import intellect_constants
        monkeypatch.setattr(intellect_constants, "get_intellect_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking intellect_state schema."""
...
class TestEventBridgePollE2E:
    ...
    def test_poll_always_queries_pg_backend(self, tmp_path, monkeypatch):
        """PostgreSQL has no state.db mtime — every poll must query the store."""
        import mcp_serve
        ...
        session_id = "20260329_150000_pg_poll"
        sessions_data = {
            "agent:main:telegram:dm:pg": {
                "session_key": "agent:main:telegram:dm:pg",
                "session_id": session_id,
                "platform": "telegram",
                "updated_at": "2026-03-29T15:00:05",
                "origin": {"platform": "telegram", "chat_id": "pg"},
            }
        }
        (sessions_dir / "sessions.json").write_text(json.dumps(sessions_data))
        ...
        bridge = mcp_serve.EventBridge()
        monkeypatch.setattr(
            "agent.storage.factory.get_storage_backend_name",
            lambda _cfg: "postgresql",
        )
