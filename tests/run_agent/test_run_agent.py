...
def test_aiagent_reuses_existing_errors_log_handler():
    ...
    error_log_path = (run_agent._intellect_home / "logs" / "errors.log").resolve()
...
class TestSaveSessionLogRedactsSecrets:
    ...
    def _ensure_redaction_enabled(self, monkeypatch):
        """Force redaction on regardless of host INTELLECT_REDACT_SECRETS state.
...
class TestBuildSystemPrompt:
    ...
    def test_includes_ontoweb_subscription_prompt(self, agent, monkeypatch):
        monkeypatch.setattr(run_agent, "build_ontoweb_subscription_prompt", lambda tool_names: "ONTOWEB SUBSCRIPTION BLOCK")
        prompt = agent._build_system_prompt()
        assert "ONTOWEB SUBSCRIPTION BLOCK" in prompt
...
class TestBuildApiKwargs:
    ...
    def test_reasoning_sent_for_nous_route(self, agent):
        agent.provider = "ontoweb"
        agent.base_url = "https://inference-api.ontoweb.cn/v1"
        agent.model = "minimax/minimax-m2.5"
        messages = [{"role": "user", "content": "hi"}]
        kwargs = agent._build_api_kwargs(messages)
        assert kwargs["extra_body"]["reasoning"]["effort"] == "medium"
...
class TestConcurrentToolExecution:
    ...
    def test_invoke_tool_blocked_returns_error_and_skips_execution(self, agent, monkeypatch):
        ...
        monkeypatch.setattr(
            "intellect_cli.plugins.get_pre_tool_call_block_message",
...
class TestRunConversation:
    ...
    def test_ollama_small_runtime_context_fails_before_api_call(self, agent, caplog):
        ...
        assert "Ollama runtime context too small for Intellect tool use" in caplog.text
    ...
    def test_request_scoped_api_hooks_fire_for_each_api_call(self, agent):
        ...
        with (
            ...
            patch("intellect_cli.plugins.invoke_hook", side_effect=_record_hook),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_nous_401_refreshes_after_remint_and_retries(self, agent):
        ...
        with (
            ...
            patch.object(agent, "_interruptible_api_call", side_effect=_fake_api_call),
            patch.object(
                agent, "_try_refresh_ontoweb_client_credentials", side_effect=_fake_refresh
            ),
        ):
            result = agent.run_conversation("hello")
    ...
    def test_kanban_block_called_on_iteration_exhaustion(self, agent, monkeypatch):
        ...
        monkeypatch.setenv("INTELLECT_KANBAN_TASK", "t_test_task_123")
        ...
        with (
            ...
            patch("intellect_cli.kanban_db._record_task_failure",
                  mock_record_failure),
            patch("intellect_cli.kanban_db.connect", mock_connect),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_no_kanban_block_when_not_in_kanban_mode(self, agent, monkeypatch):
        """The exhaustion bridge must NOT fire when INTELLECT_KANBAN_TASK
...
class TestNousCredentialRefresh:
    """Verify OntoWeb credential refresh rebuilds the runtime client."""

    def test_try_refresh_ontoweb_client_credentials_rebuilds_client(
        ...
        monkeypatch.setattr(
            "intellect_cli.auth.resolve_ontoweb_runtime_credentials", _fake_resolve
        )

        agent.client = _ExistingClient()
        with patch("run_agent.OpenAI", side_effect=_fake_openai):
            ok = agent._try_refresh_ontoweb_client_credentials(force=True)

        assert ok is True
        assert closed["value"] is True
        assert captured["force_refresh"] is True
        assert rebuilt["kwargs"]["api_key"] == "new-ontoweb-key"
        assert (
            rebuilt["kwargs"]["base_url"] == "https://inference-api.ontoweb.cn/v1"
        )
...
class TestGpt5ApiModeRouting:
    ...
    def test_nous_gpt5_stays_on_chat_completions(self, agent):
        """OntoWeb serves gpt-5.x on /chat/completions — must not upgrade to codex_responses."""
        agent.provider = "ontoweb"
        agent.base_url = "https://inference-api.ontoweb.cn/v1"
        agent.api_mode = "chat_completions"
...
class TestSystemPromptStability:
    ...
    def test_fresh_build_when_no_history(self, agent):
        ...
        assert "Intellect Agent" in agent._cached_system_prompt
