...
def _make_agent(
    *tool_names: str,
    ...
    with (
        ...
        patch("intellect_cli.config.load_config", return_value=config or {}),
...
def test_plugin_pre_tool_block_wins_without_counting_as_toolguard_block():
    ...
    with (
        patch("intellect_cli.plugins.get_pre_tool_call_block_message", return_value="plugin policy"),
...
def test_non_interactive_platform_enables_hard_stop_at_agent_init():
    """Gateway/cron surfaces merge hard_stop even when config disables it."""
    agent = _make_agent(
        "web_search",
        platform="telegram",
...
def test_cli_platform_keeps_config_hard_stop_default():
    agent = _make_agent(
        "web_search",
        platform="cli",
        config={"tool_loop_guardrails": {"hard_stop_enabled": False}},
