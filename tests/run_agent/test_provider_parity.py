...
class TestDeveloperRoleSwap:
    ...
    def test_developer_role_via_ontoweb_portal(self, monkeypatch):
        agent = _make_agent(
            monkeypatch,
            "ontoweb",
...
class TestBuildApiKwargsNousPortal:
    def test_includes_nous_product_tags(self, monkeypatch):
        ...
        kwargs = agent._build_api_kwargs(messages)
        extra = kwargs.get("extra_body", {})
        assert extra.get("tags") == ontoweb_portal_tags()
...
# ── Chat completions response handling (OpenRouter/OntoWeb) ─────────────────────
...
class TestAuxiliaryClientProviderPriority:
    ...
    def test_nous_when_no_openrouter(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._read_ontoweb_auth", return_value=ontoweb_auth), \
    ...
    def test_custom_endpoint_when_no_nous(self, monkeypatch):
        """Custom endpoint is used when no OpenRouter/OntoWeb keys are available.
