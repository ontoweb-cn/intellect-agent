...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    intellect_state = ModuleType("intellect_state")
    intellect_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "intellect_state", intellect_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Intellect"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Intellect"
