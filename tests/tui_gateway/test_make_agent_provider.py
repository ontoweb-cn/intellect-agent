...
def test_make_agent_honors_tui_launch_env_flags():
    ...
    with (
        patch.dict(
            os.environ,
            {
                "intellect_TUI_MAX_TURNS": "7",
                "intellect_TUI_CHECKPOINTS": "1",
                "intellect_TUI_PASS_SESSION_ID": "1",
                "INTELLECT_IGNORE_RULES": "1",
            },
        ),
        patch("tui_gateway.server._load_cfg", return_value=fake_cfg),
        patch("tui_gateway.server._get_db", return_value=MagicMock()),
...
def test_make_agent_tolerates_null_config_sections():
    """Bare `agent:` / `display:` keys in ~/.intellect/config.yaml parse as
