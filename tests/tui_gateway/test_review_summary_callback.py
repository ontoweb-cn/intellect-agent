...
def server():
    with patch.dict(
        "sys.modules",
        {
            "intellect_constants": MagicMock(
                get_intellect_home=MagicMock(return_value="/tmp/intellect_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'intellect-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'intellect-release' patched"
    }
