"""Tests for intellect_cli configuration management."""
...
class TestGetintellectHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("INTELLECT_HOME", None)
            home = get_intellect_home()
            assert home == Path.home() / ".intellect"

    def test_env_override(self):
        with patch.dict(os.environ, {"INTELLECT_HOME": "/custom/path"}):
            home = get_intellect_home()
            assert home == Path("/custom/path")


class TestEnsureintellectHome:
    def test_creates_subdirs(self, tmp_path):
        with patch.dict(os.environ, {"INTELLECT_HOME": str(tmp_path)}):
            ensure_intellect_home()
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``intellect logs`` surfaces it)
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        with patch.dict(os.environ, {"INTELLECT_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="intellect_cli.config"):
                config = load_config()
            ...
            # stands out at intellect startup before logging is configured)
            captured = capsys.readouterr()
            assert "intellect config:" in captured.err
    ...
    def test_dedup_on_repeated_load_same_file(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"INTELLECT_HOME": str(tmp_path)}):
            ...
            assert "intellect config:" in first
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"INTELLECT_HOME": str(tmp_path)}):
            ...
            assert "intellect config:" in after_edit, "edited file should re-warn"
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"INTELLECT_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"INTELLECT_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"INTELLECT_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"INTELLECT_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"INTELLECT_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from intellect_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or any ``intellect_*`` runtime flag.

    The ``PUT /api/env`` endpoint exposes this to any authed caller (and
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Intellect
    process on next startup via the dotenv → ``os.environ`` chain in
    ``intellect_cli/env_loader.py``.

    Regression test for the pentest finding filed alongside the
    ``web-pentest`` skill (PR #32265 / issue #32267).
    """

    @pytest.fixture(autouse=True)
    def _intellect_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "intellect_SPOTIFY_CLIENT_ID",
            "INTELLECT_QWEN_BASE_URL",
            "INTELLECT_MAX_ITERATIONS",
        ],
    )
    def test_intellect_integration_keys_still_writable(self, allowed_key):
        """``intellect_*`` overall is NOT blocked — only the four runtime
        location names (HOME/PROFILE/CONFIG/ENV) are. Integration
        credentials following the ``intellect_*`` convention must keep
    ...
    def test_arbitrary_user_key_still_works(self):
        ...
        denylist and outside ``intellect_*``) keep working. The denylist
