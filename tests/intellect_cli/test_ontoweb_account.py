"""Tests for normalized ONTOWEB Portal account entitlement helpers."""
...
def test_valid_jwt_with_paid_access_true(monkeypatch):
    token = _jwt(
        {
            ...
            "org_id": "org_123",
            "client_id": "intellect-cli",
            "product_id": "ontoweb-intellect-agent",
            "nous_client": "intellect-agent",
            "exp": int(time.time()) + 900,
            "paid_access": True,
            "subscription_tier": 2,
        }
    )
    monkeypatch.setattr("intellect_cli.auth.get_provider_auth_state", lambda provider: _state(token))

    info = get_ontoweb_portal_account_info()
    ...
    assert info.product_id == "ontoweb-intellect-agent"
...
def test_expired_jwt_falls_back_to_fresh_account(monkeypatch):
    ...
    monkeypatch.setattr("intellect_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    monkeypatch.setattr("intellect_cli.auth.resolve_ontoweb_access_token", lambda: "fresh-token")
    monkeypatch.setattr("intellect_cli.ontoweb_account._fetch_ontoweb_account_info", lambda *a, **kw: payload)
...
def test_fresh_account_payload_normalization(monkeypatch, payload, expected_paid):
    ...
    assert isinstance(info, OntowebPortalAccountInfo)
    assert info.source == "account_api"
    assert info.fresh is True
...
def test_no_oauth_token_reports_inference_key_present(monkeypatch):
    monkeypatch.setattr("intellect_cli.auth.get_provider_auth_state", lambda provider: {})

    class _Entry:
        label = "manual-ontoweb"
        access_token = ""
    ...
    assert info.credential_source == "pool:manual-ontoweb"
    assert info.paid_service_access is None
...
def test_entitlement_message_returns_none_for_paid_access():
    ...
    assert format_ontoweb_portal_entitlement_message(info, capability="paid models") is None
...
def test_entitlement_message_for_inference_key_without_portal_login():
    ...
    assert "OntoWeb inference credentials are configured" in message
    assert "cannot verify your ONTOWEB Portal paid access" in message
    assert "Log in with `intellect model`" in message
...
def test_entitlement_message_for_no_subscription_or_credits():
    ...
    message = format_ontoweb_portal_entitlement_message(info, capability="paid models")
...
def test_entitlement_message_for_unknown_entitlement_is_explicit():
    ...
    message = format_ontoweb_portal_entitlement_message(info, capability="Tool Gateway")

    assert message is not None
    assert "could not verify" in message
    assert "account_api_timeout" in message
    assert "Run `intellect model`" in message
...
def test_entitlement_message_for_account_missing():
    ...
    assert message is not None
    assert "could not find a ONTOWEB Portal account or organisation" in message
