...
`INTELLECT_REDACT_SECRETS` at module-import time. `intellect_cli/main.py` at
...
in .env), the toggle was silently ignored in both `intellect chat` and
`intellect gateway run`.

Fix: bridge `security.redact_secrets` from config.yaml → `INTELLECT_REDACT_SECRETS`
env var in `intellect_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports intellect_cli.main and checks
    # _REDACT_ENABLED. Must be a subprocess — we need a clean module state.
    probe = textwrap.dedent(
        """\
        import sys, os
        # Make absolutely sure the env var is not pre-set
        os.environ.pop("INTELLECT_REDACT_SECRETS", None)
        sys.path.insert(0, %r)
        import intellect_cli.main  # triggers the bridge + setup_logging
        import agent.redact
        print(f"REDACT_ENABLED={agent.redact._REDACT_ENABLED}")
        print(f"ENV_VAR={os.environ.get('INTELLECT_REDACT_SECRETS', '<unset>')}")
        """
    ) % str(REPO_ROOT)

    env = dict(os.environ)
    env["INTELLECT_HOME"] = str(intellect_home)
    env.pop("INTELLECT_REDACT_SECRETS", None)
...
def test_redact_secrets_default_true_when_unset(tmp_path):
    ...
    `INTELLECT_REDACT_SECRETS=false`).
    """
    intellect_home = tmp_path / ".intellect"
    intellect_home.mkdir()
    (intellect_home / "config.yaml").write_text("{}\n")  # empty config
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env INTELLECT_REDACT_SECRETS takes precedence over config.yaml."""
    intellect_home = tmp_path / ".intellect"
    intellect_home.mkdir()
    (intellect_home / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (intellect_home / ".env").write_text("INTELLECT_REDACT_SECRETS=true\n")
