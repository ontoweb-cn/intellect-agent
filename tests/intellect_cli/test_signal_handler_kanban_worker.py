...
for interactive ``intellect chat -q`` invocations, but kanban workers spawned by
...
The fix: when the process is a dispatcher-spawned worker (``INTELLECT_KANBAN_TASK``
...
def _synthetic_worker_script() -> str:
    ...
    shape without needing the full intellect_cli boot path (config, providers,
...
def _is_alive_like_dispatcher(pid: int) -> bool:
    """Mirrors intellect_cli/kanban_db.py:_pid_alive on Linux.
...
def test_sigterm_with_kanban_task_env_terminates_quickly():
    """With INTELLECT_KANBAN_TASK set, SIGTERM should kill the process in <2s
    even when a non-daemon thread is still alive."""
    proc = _spawn_synthetic({"INTELLECT_KANBAN_TASK": "t_test_28181"})
    try:
        t0 = time.time()
        ...
        while time.time() < deadline:
            ...
            time.sleep(0.02)
        pytest.fail(
            f"process still alive 2s after SIGTERM with INTELLECT_KANBAN_TASK set "
...
def test_sigterm_without_kanban_task_env_uses_keyboard_interrupt_path():
    """Without INTELLECT_KANBAN_TASK, the original KeyboardInterrupt path runs.

    This is the contrast case proving the fix is gated on the env var: in
    interactive ``intellect chat -q`` (no env var), behavior is unchanged. The
...
def test_real_handler_uses_os_exit_for_kanban_workers():
    ...
    os._exit(0) when INTELLECT_KANBAN_TASK is set.
    ...
    assert "INTELLECT_KANBAN_TASK" in body, (
        "_signal_handler_q must gate its kanban-worker exit path on "
        "INTELLECT_KANBAN_TASK — see #28181"
    )
