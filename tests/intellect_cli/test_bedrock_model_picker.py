...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (intellect_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via intellect_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in intellect_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from intellect_cli.providers import intellect_OVERLAYS
        assert "bedrock" in intellect_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from intellect_cli.providers import intellect_OVERLAYS
        assert intellect_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from intellect_cli.providers import intellect_OVERLAYS
        assert intellect_OVERLAYS["bedrock"].auth_type == "aws_sdk"
