"""Tests for IRC gateway configuration via `intellect setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Intellect install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Intellect install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#intellect")
            monkeypatch.setenv("IRC_NICKNAME", "intellect-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `intellect setup gateway` experience with IRC present."""
