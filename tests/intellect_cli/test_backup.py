"""Tests for intellect backup and import commands."""
...
def _make_intellect_tree(root: Path) -> None:
    """Create a realistic ~/.intellect directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    (root / "memory_store.db").write_bytes(b"fake-sqlite")
    (root / "intellect_state.db").write_bytes(b"fake-state")
    ...
    # intellect-agent repo (should be EXCLUDED)
    (root / "intellect-agent").mkdir(exist_ok=True)
    (root / "intellect-agent" / "run_agent.py").write_text("# big file\n")
    (root / "intellect-agent" / ".git").mkdir()
    (root / "intellect-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_intellect_agent(self):
        from intellect_cli.backup import _should_exclude
        assert _should_exclude(Path("intellect-agent/run_agent.py"))
        assert _should_exclude(Path("intellect-agent/.git/HEAD"))
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        _make_intellect_tree(intellect_home)

        monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))
        # get_default_intellect_root needs this
    ...
    def test_excludes_intellect_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include intellect-agent/ directory."""
        intellect_home = tmp_path / ".intellect"
        intellect_home.mkdir()
        _make_intellect_tree(intellect_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "intellect-agent" in n]
            assert agent_files == [], f"intellect-agent files leaked into backup: {agent_files}"
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/intellect-backup-*.zip."""
        intellect_home = tmp_path / ".intellect"
        intellect_home.mkdir()
        (intellect_home / "config.yaml").write_text("model: test\n")
        ...
        zips = list(tmp_path.glob("intellect-backup-*.zip"))
        assert len(zips) == 1

    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside INTELLECT_HOME."""
        intellect_home = tmp_path / ".intellect"
        intellect_home.mkdir()
        _make_intellect_tree(intellect_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(intellect_home / "skills" / "outside-link.txt", outside)
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Intellect backup."""
        from intellect_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only intellect_state.db (old wrong name) is rejected."""
        from intellect_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["intellect_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into intellect home."""
        intellect_home = tmp_path / ".intellect"
        intellect_home.mkdir()
        monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (intellect_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (intellect_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (intellect_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (intellect_home / "profiles" / "coder" / "config.yaml").exists()

    def test_strips_intellect_prefix(self, tmp_path, monkeypatch):
        """Import strips .intellect/ prefix if all entries share it."""
        intellect_home = tmp_path / ".intellect"
        intellect_home.mkdir()
        monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".intellect/config.yaml": "model: test\n",
            ".intellect/skills/a/SKILL.md": "# A\n",
        })
        ...
        assert (intellect_home / "config.yaml").read_text() == "model: test\n"
        assert (intellect_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_intellect_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like an intellect backup."""
        intellect_home = tmp_path / ".intellect"
        intellect_home.mkdir()
        monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        assert (intellect_home / "config.yaml").exists()
        # traversal file should NOT exist outside intellect home
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (intellect_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (intellect_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (intellect_home / "config.yaml").read_text() == "model: restored\n"
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        self._make_backup_zip(zip_path, {
            ...
            "auth.json": '{"providers": {"ontoweb": "token"}}',
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (intellect_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".intellect"
        src_home.mkdir(parents=True)
        _make_intellect_tree(src_home)

        monkeypatch.setenv("INTELLECT_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from intellect_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".intellect"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("INTELLECT_HOME", str(dst_home))
        ...
        # intellect-agent should NOT be present
        assert not (dst_home / "intellect-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from intellect_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without intellect markers fails validation."""
        import io
        from intellect_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_intellect(self):
        """Detects .intellect/ prefix wrapping all entries."""
        import io
        from intellect_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".intellect/config.yaml", "test")
            zf.writestr(".intellect/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".intellect/"
    ...
    def test_detect_prefix_only_dirs(self):
        ...
        with zipfile.ZipFile(buf, "w") as zf:
            ...
            zf.writestr(".intellect/", "")
            zf.writestr(".intellect/skills/", "")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ""
...
class TestBackupEdgeCases:
    def test_nonexistent_intellect_home(self, tmp_path, monkeypatch):
        """Backup exits when intellect home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".intellect"
        monkeypatch.setenv("INTELLECT_HOME", str(fake_home))
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("intellect-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_intellect_home(self, tmp_path, monkeypatch):
        """Backup handles empty intellect home (no files to back up)."""
        intellect_home = tmp_path / ".intellect"
        intellect_home.mkdir()
        # Only excluded dirs, no actual files
        (intellect_home / "__pycache__").mkdir()
        (intellect_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_permission_error_during_backup(self, tmp_path, monkeypatch):
        ...
        bad_file = intellect_home / "secret.db"
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        old_file = intellect_home / "ancient.txt"
    ...
    def test_skips_output_zip_inside_intellect(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside intellect root."""
        intellect_home = tmp_path / ".intellect"
        intellect_home.mkdir()
        (intellect_home / "config.yaml").write_text("model: test\n")
        ...
        # Output inside intellect home
        out_zip = intellect_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        intellect_home = tmp_path / ".intellect"
        intellect_home.mkdir()
        (intellect_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        intellect_home = tmp_path / ".intellect"
        intellect_home.mkdir()
        (intellect_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_permission_error_during_import(self, tmp_path, monkeypatch):
        ...
        locked_dir = intellect_home / "locked"
        locked_dir.mkdir()
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (intellect_home / "config.yaml").exists()
        assert (intellect_home / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (intellect_home / "profiles" / "coder" / "config.yaml").exists()
        assert (intellect_home / "profiles" / "researcher" / "config.yaml").exists()
    ...
    def test_import_without_profiles_module(self, tmp_path, monkeypatch):
        ...
        def fake_import(name, *a, **kw):
            if name == "intellect_cli.profiles":
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from intellect_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    @pytest.fixture
    def intellect_home(self, tmp_path):
        """Create a fake INTELLECT_HOME with critical state files."""
    ...
    def test_creates_snapshot(self, intellect_home):
        from intellect_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(intellect_home=intellect_home)
        assert snap_id is not None
        snap_dir = intellect_home / "state-snapshots" / snap_id
        assert snap_dir.is_dir()
        assert (snap_dir / "manifest.json").exists()

    def test_label_in_id(self, intellect_home):
        from intellect_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", intellect_home=intellect_home)
        assert "before-upgrade" in snap_id

    def test_state_db_safely_copied(self, intellect_home):
        from intellect_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(intellect_home=intellect_home)
        db_copy = intellect_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_copies_nested_files(self, intellect_home):
        from intellect_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(intellect_home=intellect_home)
        assert (intellect_home / "state-snapshots" / snap_id / "cron" / "jobs.json").exists()

    def test_missing_files_skipped(self, intellect_home):
        from intellect_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(intellect_home=intellect_home)
        with open(intellect_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(intellect_home=empty) is None

    def test_list_snapshots(self, intellect_home):
        from intellect_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", intellect_home=intellect_home)
        id2 = create_quick_snapshot(label="second", intellect_home=intellect_home)

        snaps = list_quick_snapshots(intellect_home=intellect_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, intellect_home):
        from intellect_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", intellect_home=intellect_home)
        snaps = list_quick_snapshots(limit=3, intellect_home=intellect_home)
        assert len(snaps) == 3

    def test_restore_config(self, intellect_home):
        from intellect_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(intellect_home=intellect_home)

        (intellect_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (intellect_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, intellect_home=intellect_home)
        assert result is True
        assert "openrouter" in (intellect_home / "config.yaml").read_text()

    def test_restore_state_db(self, intellect_home):
        from intellect_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(intellect_home=intellect_home)

        conn = sqlite3.connect(str(intellect_home / "state.db"))
        conn.execute("INSERT INTO sessions VALUES ('s2', 'new')")
        conn.commit()
        conn.close()

        restore_quick_snapshot(snap_id, intellect_home=intellect_home)
    ...
    def test_restore_nonexistent(self, intellect_home):
        from intellect_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", intellect_home=intellect_home) is False

    def test_auto_prune(self, intellect_home):
        from intellect_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", intellect_home=intellect_home)
        snaps = list_quick_snapshots(limit=100, intellect_home=intellect_home)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, intellect_home):
        from intellect_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", intellect_home=intellect_home)
        deleted = prune_quick_snapshots(keep=3, intellect_home=intellect_home)
        assert deleted == 7
        assert len(list_quick_snapshots(intellect_home=intellect_home)) == 3

    def test_snapshot_includes_pairing_directories(self, intellect_home):
        ...
        (intellect_home / "platforms" / "pairing").mkdir(parents=True)
        (intellect_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (intellect_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (intellect_home / "pairing").mkdir()
        (intellect_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (intellect_home / "feishu_comment_pairing.json").write_text(
    ...
    def test_restore_recovers_pairing_data(self, intellect_home):
        ...
        assert restore_quick_snapshot(snap_id, intellect_home=intellect_home) is True
        assert approved.exists()
        assert '"alice"' in approved.read_text()
        assert feishu.exists()
        assert '"user_xyz"' in feishu.read_text()

    def test_empty_pairing_dir_does_not_fail(self, intellect_home):
...
# ---------------------------------------------------------------------------
# Pre-update backup (intellect update safety net)
# ---------------------------------------------------------------------------

class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``intellect update``
    ...
    def intellect_home(self, tmp_path):
        ...
        root.mkdir()
        _make_intellect_tree(root)
        return root

    def test_creates_backup_under_backups_dir(self, intellect_home):
        from intellect_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(intellect_home=intellect_home)
        assert out is not None
        assert out.exists()
        assert out.parent == intellect_home / "backups"
        assert out.name.startswith("pre-update-")
        assert out.suffix == ".zip"

    def test_backup_contents_match_full_backup(self, intellect_home):
        """Pre-update backup should include the same user data that
        ``intellect backup`` would, and should exclude the same directories."""
        from intellect_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(intellect_home=intellect_home)
        assert out is not None
        ...
        assert "skills/my-skill/SKILL.md" in names
        assert "profiles/coder/config.yaml" in names
        # intellect-agent repo excluded
        assert not any(n.startswith("intellect-agent/") for n in names)
        # __pycache__ excluded
        assert not any("__pycache__" in n for n in names)
        # pid files excluded
        assert "gateway.pid" not in names

    def test_does_not_recurse_into_prior_backups(self, intellect_home):
        """The ``backups/`` directory must be excluded so that each backup
        doesn't grow exponentially by including all prior backups."""
        from intellect_cli.backup import create_pre_update_backup
        # First backup
        out1 = create_pre_update_backup(intellect_home=intellect_home)
        assert out1 is not None
        # Second backup — must not include the first
        out2 = create_pre_update_backup(intellect_home=intellect_home)
    ...
    def test_rotation_keeps_only_n(self, intellect_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(intellect_home=intellect_home, keep=3)
            created.append(out)
            _t.sleep(1.05)  # ensure distinct seconds in timestamp

        remaining = sorted(
            p.name for p in (intellect_home / "backups").iterdir()
    ...
    def test_rotation_preserves_manual_files(self, intellect_home):
        """Hand-dropped zips in ``backups/`` must not be touched by
        rotation — it only prunes files matching ``pre-update-*.zip``."""
        import time as _t
        from intellect_cli.backup import create_pre_update_backup

        (intellect_home / "backups").mkdir(exist_ok=True)
        manual = intellect_home / "backups" / "my-manual.zip"
        manual.write_bytes(b"manual backup")

        for _ in range(5):
            create_pre_update_backup(intellect_home=intellect_home, keep=2)
            _t.sleep(1.05)
    ...
    def test_returns_none_if_root_missing(self, tmp_path):
        from intellect_cli.backup import create_pre_update_backup
        assert create_pre_update_backup(intellect_home=tmp_path / "does-not-exist") is None

    def test_keep_zero_does_not_delete_freshly_created_backup(self, intellect_home):
        ...
        """
        from intellect_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(intellect_home=intellect_home, keep=0)
        assert out is not None
        assert out.exists(), (
    ...
    def test_keep_negative_does_not_delete_freshly_created_backup(self, intellect_home):
    ...
    def test_keep_zero_still_prunes_older_backups(self, intellect_home):
        ...
        first = create_pre_update_backup(intellect_home=intellect_home, keep=5)
        _t.sleep(1.05)
        second = create_pre_update_backup(intellect_home=intellect_home, keep=5)
        _t.sleep(1.05)
        third = create_pre_update_backup(intellect_home=intellect_home, keep=0)
    ...
    def test_skips_symlinked_files(self, intellect_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside INTELLECT_HOME."""
        from intellect_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def intellect_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".intellect"
        root.mkdir()
        _make_intellect_tree(root)
        # Point INTELLECT_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("INTELLECT_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for intellect_cli.config + intellect_constants so they pick up INTELLECT_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("intellect_cli.config") or mod == "intellect_constants":
                del __import__("sys").modules[mod]
        return root

    def test_backup_flag_creates_backup(self, intellect_home, capsys):
        """--backup forces the pre-update backup for one run even when config is off."""
        from intellect_cli.main import _run_pre_update_backup
        _run_pre_update_backup(Namespace(no_backup=False, backup=True))
        out = capsys.readouterr().out
        assert "Creating pre-update backup" in out
        assert "Saved:" in out
        assert "Restore:" in out
        assert "intellect import" in out
        assert "Disable:" in out
        # Actual backup was created
        backups = list((intellect_home / "backups").glob("pre-update-*.zip"))
        assert len(backups) == 1

    def test_default_disabled_is_silent(self, intellect_home, capsys):
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not (intellect_home / "backups").exists() or not list(
            (intellect_home / "backups").glob("pre-update-*.zip")
        )

    def test_no_backup_flag_skips(self, intellect_home, capsys):
        from intellect_cli.main import _run_pre_update_backup
    ...
    def test_config_enabled_creates_backup(self, intellect_home, capsys):
        ...
        import yaml
        (intellect_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": True},
        }))
        import sys as _sys
        for mod in list(_sys.modules.keys()):
            if mod.startswith("intellect_cli.config"):
    ...
    def test_config_disabled_is_silent(self, intellect_home, capsys):
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not list((intellect_home / "backups").glob("pre-update-*.zip")) \
            if (intellect_home / "backups").exists() else True

    def test_cli_flag_overrides_enabled_config(self, intellect_home, capsys):
...
# ---------------------------------------------------------------------------
# Pre-migration backup (intellect claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``intellect claw migrate`` runs before mutating ~/.intellect/."""
    ...
    def test_creates_backup_under_backups_dir(self, intellect_home):
        from intellect_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(intellect_home=intellect_home)
        assert out is not None
        assert out.exists()
        # Shares the backups/ directory with pre-update backups so `intellect
        # import` and the update-backup listing both pick them up.
        assert out.parent == intellect_home / "backups"
        assert out.name.startswith("pre-migration-")
        assert out.suffix == ".zip"

    def test_backup_uses_shared_exclusion_rules(self, intellect_home):
        """Pre-migration backup reuses the same exclusion rules as
        ``intellect backup`` / ``create_pre_update_backup`` — no drift."""
        from intellect_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(intellect_home=intellect_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_restorable_with_intellect_import(self, intellect_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Intellect
        backup — `intellect import` should accept it."""
        from intellect_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(intellect_home=intellect_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_recurse_into_prior_backups(self, intellect_home):
        from intellect_cli.backup import create_pre_migration_backup
        out1 = create_pre_migration_backup(intellect_home=intellect_home)
        assert out1 is not None
        out2 = create_pre_migration_backup(intellect_home=intellect_home)
        assert out2 is not None
        with zipfile.ZipFile(out2) as zf:
            names = zf.namelist()
        assert not any(n.startswith("backups/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, intellect_home):
        ...
        created = []
        for _ in range(7):
            out = create_pre_migration_backup(intellect_home=intellect_home, keep=3)
            if out is not None:
                created.append(out)
            _t.sleep(1.05)  # timestamp resolution

        remaining = sorted((intellect_home / "backups").glob("pre-migration-*.zip"))
        assert len(remaining) <= 3, f"expected <=3 backups retained, got {len(remaining)}"

    def test_missing_intellect_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.intellect yet — nothing to back up."""
        from intellect_cli.backup import create_pre_migration_backup
        missing = tmp_path / "does-not-exist"
        out = create_pre_migration_backup(intellect_home=missing)
        assert out is None

    def test_does_not_touch_pre_update_backups(self, intellect_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from intellect_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(intellect_home=intellect_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        import time as _t
        for _ in range(3):
            out = create_pre_migration_backup(intellect_home=intellect_home, keep=1)
            assert out is not None
            _t.sleep(1.05)
...
class TestRestoreCronJobsIfEmptied:
    """`intellect update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, intellect_home: Path, label="pre-update"):
        from intellect_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, intellect_home=intellect_home, keep=5)
    ...
    def test_restores_when_emptied_after_migration(self, tmp_path):
        ...
        intellect_home = tmp_path / ".intellect"
        jobs_path = intellect_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(intellect_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, intellect_home=intellect_home)
    ...
    def test_noop_when_snapshot_id_missing(self, tmp_path):
        ...
        jobs_path = intellect_home / "cron" / "jobs.json"
        self._seed_jobs(jobs_path, [])
        assert restore_cron_jobs_if_emptied(None, intellect_home=intellect_home) is None
        assert restore_cron_jobs_if_emptied("", intellect_home=intellect_home) is None
