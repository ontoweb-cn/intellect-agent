...
Covers:
 - Normal path (tokens already in Intellect auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Intellect
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `intellect auth openai-codex`. The old
...
@pytest.fixture()
def intellect_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Intellect auth store (no Codex CLI needed)."""
    ...
    for var in [
        ...
        "ONTOWEB_API_KEY", "DEEPSEEK_API_KEY",
    ]:
...
def test_normal_path_still_works(intellect_auth_only_env):
    """openai-codex appears when tokens are already in Intellect auth store."""
...
def test_codex_picker_uses_live_codex_catalog(intellect_auth_only_env, tmp_path, monkeypatch):
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Intellect
    auth store."""
    intellect_home = tmp_path / ".intellect"
    intellect_home.mkdir()
...
def test_no_codex_when_no_credentials(tmp_path, monkeypatch):
    ...
    for var in [
        "OPENROUTER_API_KEY", "OPENAI_API_KEY", "ANTHROPIC_API_KEY",
        "ONTOWEB_API_KEY", "DEEPSEEK_API_KEY", "COPILOT_GITHUB_TOKEN",
