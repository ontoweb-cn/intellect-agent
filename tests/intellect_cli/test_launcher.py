"""Tests for the top-level `./intellect` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./intellect` should use `intellect_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "intellect"
    called = []

    fake_main_module = types.ModuleType("intellect_cli.main")

    def fake_main():
        called.append("intellect_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "intellect_cli.main", fake_main_module)
    ...
    assert called == ["intellect_cli.main"]
