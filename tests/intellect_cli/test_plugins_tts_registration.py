...
``$INTELLECT_HOME/plugins/``, runs ``PluginManager().discover_and_load()``,
and asserts the registration result.

Mirrors the structure of
``tests/intellect_cli/test_plugin_scanner_recursion.py::TestRegisterImageGenProvider``.
"""
...
class TestRegisterTTSProvider:
    ...
    def test_accepts_valid_provider(self):
        ...
        intellect_home = Path(os.environ["INTELLECT_HOME"])
        _write_plugin(
        ...
        _enable(intellect_home, "my-tts-plugin")
    ...
    def test_rejects_non_provider(self, caplog):
        ...
        _enable(intellect_home, "bad-tts-plugin")
    ...
    def test_rejects_builtin_shadow(self, caplog):
        ...
        _enable(intellect_home, "shadow-tts-plugin")
