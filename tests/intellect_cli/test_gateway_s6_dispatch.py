"""Tests for the Phase 4 s6 dispatch helper in intellect_cli.gateway.

`_dispatch_via_service_manager_if_s6` decides whether a
`intellect gateway start/stop/restart` invocation should be routed to
...
def test_dispatch_returns_false_on_host(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from intellect_cli import gateway as gw
    monkeypatch.setattr(
        "intellect_cli.service_manager.detect_service_manager", lambda: "systemd",
    )
    # Should not even attempt to construct a manager.
    monkeypatch.setattr(
        "intellect_cli.service_manager.get_service_manager",
...
def test_dispatch_returns_true_and_calls_start_on_s6(
    ...
    monkeypatch.setattr(
        "intellect_cli.service_manager.get_service_manager", lambda: rec,
    )
...
def test_dispatch_defaults_profile_to_default(
    ...
    monkeypatch.setattr(
        "intellect_cli.gateway._profile_suffix", lambda: "",
    )
...
def test_dispatch_renders_gateway_not_registered_friendly(
    ...
    """`intellect -p typo gateway start` should print a clear message and
    exit 1 — not dump a traceback at the user."""
    from intellect_cli import gateway as gw
    from intellect_cli.service_manager import GatewayNotRegisteredError
    ...
    monkeypatch.setattr(
        "intellect_cli.service_manager.get_service_manager", lambda: _RaisesMissing(),
    )
    ...
    assert "intellect profile create typo" in out
...
def test_dispatch_renders_s6_command_error_friendly(
    ...
    monkeypatch.setattr(
        "intellect_cli.service_manager.get_service_manager", lambda: _RaisesS6Error(),
    )
...
def _stub_s6(monkeypatch: pytest.MonkeyPatch, *, on_s6: bool) -> _CallRecorder:
    ...
    rec = _CallRecorder()
    monkeypatch.setattr(
        "intellect_cli.service_manager.detect_service_manager",
...
def _stub_execvp(monkeypatch: pytest.MonkeyPatch) -> list[list[str]]:
    ...
    monkeypatch.setattr("intellect_cli.gateway.os.execvp", fake_execvp)
    return calls
...
def test_redirect_noop_on_host(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "intellect_cli.gateway.os.execvp",
        lambda *a, **kw: pytest.fail("execvp should not be called on host"),
    )
    monkeypatch.delenv("intellect_S6_SUPERVISED_CHILD", raising=False)
    monkeypatch.delenv("intellect_GATEWAY_NO_SUPERVISE", raising=False)
...
def test_redirect_fires_inside_s6_container(
    ...
    monkeypatch.setattr("intellect_cli.gateway._profile_suffix", lambda: "")
    ...
    assert "intellect_GATEWAY_NO_SUPERVISE" in err
...
def test_redirect_short_circuits_supervised_child(
    ...
    running execs `intellect gateway run --replace`, the
    intellect_S6_SUPERVISED_CHILD sentinel must short-circuit the redirect
    ...
    monkeypatch.setenv("intellect_S6_SUPERVISED_CHILD", "1")
    monkeypatch.delenv("intellect_GATEWAY_NO_SUPERVISE", raising=False)
...
def test_redirect_respects_no_supervise_env(
    ...
    """`intellect_GATEWAY_NO_SUPERVISE=1` (env var) must skip the redirect.

    Truthiness mirrors the s6 service's own env var parsing —
    ...
    monkeypatch.delenv("intellect_S6_SUPERVISED_CHILD", raising=False)
    monkeypatch.setenv("intellect_GATEWAY_NO_SUPERVISE", value)
...
def test_redirect_no_supervise_env_falsy_values_dont_opt_out(
    ...
    """Falsy / unrecognized values of intellect_GATEWAY_NO_SUPERVISE must
    NOT opt out. We're strict about what counts as "yes" so a typo
    like `intellect_GATEWAY_NO_SUPERVISE=0` doesn't silently enable the
    ...
    for falsy in ("", "0", "false", "no", "off", "garbage"):
        monkeypatch.setenv("intellect_GATEWAY_NO_SUPERVISE", falsy)
