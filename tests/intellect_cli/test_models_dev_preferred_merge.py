...
These guard the contract:
  ...
  * OpenRouter and ONTOWEB Portal are NEVER merged — they keep their curated
    (OpenRouter) or live-Portal (OntoWeb) semantics.
...
appear in ``/model`` without an Intellect release.
"""
...
class TestProviderModelIdsPreferred:
    ...
    def test_opencode_go_offline_falls_back_to_curated(self):
        ...
        with patch("agent.models_dev.list_agentic_models", return_value=[]):
            out = provider_model_ids("opencode-go")
        # Curated floor (see intellect_cli/models.py _PROVIDER_MODELS["opencode-go"])
        assert "mimo-v2-pro" in out
        assert "kimi-k2.6" in out
...
class TestOpenRouterAndNousUnchanged:
    """Per Teknium: openrouter and ontoweb are NEVER merged with models.dev."""
