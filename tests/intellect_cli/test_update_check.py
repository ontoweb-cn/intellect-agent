"""Tests for the update check mechanism in intellect_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    repo_dir = tmp_path / "intellect-agent"
    repo_dir.mkdir()
    (repo_dir / ".git").mkdir()
    ...
    with patch("intellect_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_invalidates_on_version_change(tmp_path, monkeypatch):
    ...
    """
    import intellect_cli.banner as banner

    # No local git checkout -> the PyPI path is exercised (pip-install class).
    fake_banner = tmp_path / "intellect_cli" / "banner.py"
    ...
    monkeypatch.setenv("INTELLECT_HOME", str(tmp_path))
    monkeypatch.delenv("INTELLECT_REVISION", raising=False)
    with patch("intellect_cli.banner.subprocess.run") as mock_run, \
         patch("intellect_cli.banner.check_via_pypi", return_value=0) as mock_pypi:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("INTELLECT_HOME", str(tmp_path))
    with patch("intellect_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("INTELLECT_HOME", str(tmp_path))
    with patch("intellect_cli.banner.subprocess.run") as mock_run:
        with patch("intellect_cli.banner.check_via_pypi", return_value=0):
            result = banner.check_for_updates()
    assert result == 0
    mock_run.assert_not_called()
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when INTELLECT_HOME has no git repo."""
    import intellect_cli.banner as banner
    ...
    # Point INTELLECT_HOME at a temp dir with no intellect-agent/.git
    monkeypatch.setenv("INTELLECT_HOME", str(tmp_path))
    with patch("intellect_cli.banner.subprocess.run") as mock_run:
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from intellect_cli.main import _invalidate_update_cache

    # Build a fake ~/.intellect with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"INTELLECT_HOME": str(default_home)}):
