...
empty `env_vars` — causing `intellect tools` to write the toolset to config
...
class TestPostSetupGate:
    def test_cua_driver_missing_forces_setup(self, monkeypatch, tmp_path):
        ...
        from intellect_cli import tools_config
    ...
    def test_cua_driver_installed_skips_setup(self, monkeypatch, tmp_path):
        ...
        so a re-save through `intellect tools` doesn't re-prompt the user."""
        from intellect_cli import tools_config
