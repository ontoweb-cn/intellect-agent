"""Tests for intellect_cli.mcp_catalog and intellect_cli.mcp_picker.
...
def catalog_dir(tmp_path, monkeypatch):
    ...
    cat.mkdir()
    monkeypatch.setenv("INTELLECT_OPTIONAL_MCPS", str(cat))
    return cat


@pytest.fixture(autouse=True)
def _isolate_intellect_home(tmp_path, monkeypatch):
    """Redirect all config I/O to a temp INTELLECT_HOME."""
    hh = tmp_path / "intellect-home"
    hh.mkdir()
    monkeypatch.setenv("INTELLECT_HOME", str(hh))
    monkeypatch.setattr(
        "intellect_cli.config.get_intellect_home", lambda: hh
    )
    monkeypatch.setattr(
        "intellect_cli.config.get_config_path", lambda: hh / "config.yaml"
    )
    monkeypatch.setattr(
        "intellect_cli.config.get_env_path", lambda: hh / ".env"
    )
    # mcp_catalog grabs get_intellect_home() lazily through intellect_constants
    monkeypatch.setattr(
        "intellect_constants.get_intellect_home", lambda: hh
    )
    return hh
...
def _entry(name: str):
    ...
    from intellect_cli.mcp_catalog import get_entry
...
class TestManifestParsing:
    def test_minimal_valid(self, catalog_dir):
        ...
        from intellect_cli.mcp_catalog import list_catalog
...
class TestInstall:
    def test_install_simple_stdio_writes_config(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from intellect_cli.mcp_catalog import install_entry
    ...
    def test_install_with_install_dir_substitution(self, catalog_dir, tmp_path):
        ...
        from intellect_cli import mcp_catalog
        from intellect_cli.mcp_catalog import install_entry
        from intellect_cli.config import load_config
    ...
    def test_install_required_env_missing_raises(self, catalog_dir, monkeypatch):
        ...
        from intellect_cli import mcp_catalog
        from intellect_cli.mcp_catalog import install_entry, CatalogError
...
class TestUninstall:
    def test_uninstall_removes_server_block(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from intellect_cli.mcp_catalog import install_entry, uninstall_entry
        from intellect_cli.config import load_config
    ...
    def test_uninstall_missing_returns_false(self):
        from intellect_cli.mcp_catalog import uninstall_entry
...
class TestCatalogDiagnostics:
    def test_future_manifest_version_skipped_with_diagnostic(self, catalog_dir):
        ...
        from intellect_cli.mcp_catalog import list_catalog, catalog_diagnostics
    ...
    def test_picker_surfaces_future_manifest_warning(self, catalog_dir, capsys, monkeypatch):
        ...
        entries so users running headless or after `intellect setup` know to update."""
        body = _basic_manifest()
        ...
        assert "requires a newer Intellect" in out
...
class TestGitInstallShaRef:
    def test_sha_ref_skips_branch_attempt(self, catalog_dir, monkeypatch, tmp_path):
        ...
        from intellect_cli import mcp_catalog
        from intellect_cli.mcp_catalog import _do_git_install
    ...
    def test_branch_ref_uses_branch_clone(self, catalog_dir, monkeypatch):
        ...
        from intellect_cli import mcp_catalog
        from intellect_cli.mcp_catalog import _do_git_install, get_entry
...
class TestToolsConfigIncludeMode:
    def test_configure_mcp_writes_include_not_exclude(self, monkeypatch, tmp_path):
        ...
        import intellect_cli.tools_config as tc
        ...
        monkeypatch.setattr(
            "intellect_cli.curses_ui.curses_checklist",
...
class TestShippedCatalog:
    def test_all_shipped_manifests_parse(self, monkeypatch):
        ...
        """
        # Use the actual repo's optional-mcps directory (no INTELLECT_OPTIONAL_MCPS
        # override) so this test catches real manifests.
        monkeypatch.delenv("INTELLECT_OPTIONAL_MCPS", raising=False)
        from intellect_cli.mcp_catalog import _catalog_root, _parse_manifest
