"""Tests for the `intellect memory reset` CLI command.
...
- Profile-scoped reset (uses INTELLECT_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake INTELLECT_HOME with memory files."""
    intellect_home = tmp_path / ".intellect"
    memories = intellect_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nintellect repo is at ~/.intellect/intellect-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
    ...
    return intellect_home, memories
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `intellect memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        intellect_home = tmp_path / ".intellect"
        (intellect_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's INTELLECT_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_intellect_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
