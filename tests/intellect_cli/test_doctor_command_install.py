"""Tests for the Command Installation check in intellect doctor."""
...
def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal INTELLECT_HOME + PROJECT_ROOT for doctor tests."""
    ...
    intellect_bin = venv_bin_dir / "intellect"
    intellect_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    intellect_bin.chmod(0o755)
    ...
    return home, project, intellect_bin
...
class TestDoctorCommandInstallation:
    ...
    def test_correct_symlink_shows_ok(self, monkeypatch, tmp_path):
        home, project, intellect_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create the command link dir with correct symlink
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "intellect"
        cmd_link.symlink_to(intellect_bin)
    ...
    def test_missing_symlink_shows_fail(self, monkeypatch, tmp_path):
        ...
        assert "intellect doctor --fix" in out
    ...
    def test_fix_creates_missing_symlink(self, monkeypatch, tmp_path):
        ...
        cmd_link = tmp_path / ".local" / "bin" / "intellect"
        assert cmd_link.is_symlink()
        assert cmd_link.resolve() == intellect_bin.resolve()
    ...
    def test_wrong_target_symlink_shows_warn(self, monkeypatch, tmp_path):
        ...
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "intellect"
        wrong_target = tmp_path / "wrong_intellect"
    ...
    def test_dot_venv_dir_is_found(self, monkeypatch, tmp_path):
        ...
        intellect_bin = project / ".venv" / "bin" / "intellect"
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "intellect"
        cmd_link.symlink_to(intellect_bin)
        ...
        out = _run_doctor(fix=False)
        assert "Venv entry point exists" in out
        assert ".venv/bin/intellect" in out

    @pytest.mark.skipif(sys.platform == "win32", reason="Symlink check is Unix-only")
    def test_non_symlink_regular_file_shows_ok(self, monkeypatch, tmp_path):
        """If ~/.local/bin/intellect is a regular file (not symlink), accept it."""
        home, project, intellect_bin = _setup_doctor_env(monkeypatch, tmp_path)

        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "intellect"
        cmd_link.write_text("#!/bin/sh\nexec python -m intellect_cli.main \"$@\"\n")
