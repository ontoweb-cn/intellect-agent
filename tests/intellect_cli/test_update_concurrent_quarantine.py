"""Tests for issue #26670 — concurrent intellect.exe detection and improved
quarantine retry / reboot-deferred fallback during `intellect update` on Windows.
...
# Tests in this module either exercise the REAL _detect_concurrent_intellect_instances
# helper (and need the autouse stub in tests/intellect_cli/conftest.py disabled),
...
# ---------------------------------------------------------------------------
# _detect_concurrent_intellect_instances
# ---------------------------------------------------------------------------


def _make_proc(pid: int, exe: str, name: str = "intellect.exe"):
...
def test_detect_concurrent_returns_empty_when_no_other_processes(_winp, tmp_path):
    ...
    (scripts_dir / "intellect.exe").write_bytes(b"")
    (scripts_dir / "intellect-gateway.exe").write_bytes(b"")

    fake_psutil = types.SimpleNamespace(process_iter=lambda attrs: iter([]))
    with patch.dict(sys.modules, {"psutil": fake_psutil}):
        result = cli_main._detect_concurrent_intellect_instances(scripts_dir)
...
def test_detect_concurrent_excludes_self_pid(_winp, tmp_path):
    ...
    procs = [_make_proc(my_pid, str(shim), "intellect.exe")]
...
@patch.object(cli_main, "_is_windows", return_value=True)
def test_detect_concurrent_finds_other_intellect_process(_winp, tmp_path):
    scripts_dir = tmp_path
    shim = scripts_dir / "intellect.exe"
    shim.write_bytes(b"")
    ...
    procs = [
        _make_proc(other_pid, str(shim), "intellect.exe"),
    ...
    assert result == [(other_pid, "intellect.exe")]
...
def test_detect_concurrent_matches_case_insensitively(_winp, tmp_path):
    ...
    # Simulate the desktop spawning intellect.EXE (uppercase ext) from same path
    upper = str(shim).replace("intellect.exe", "INTELLECT.EXE")
    procs = [_make_proc(9999, upper, "INTELLECT.EXE")]
    fake_psutil = types.SimpleNamespace(process_iter=lambda attrs: iter(procs))
    with patch.dict(sys.modules, {"psutil": fake_psutil}):
        result = cli_main._detect_concurrent_intellect_instances(scripts_dir)

    assert result == [(9999, "INTELLECT.EXE")]
...
def test_detect_concurrent_is_noop_off_windows(_winp, tmp_path):
    ...
    assert cli_main._detect_concurrent_intellect_instances(tmp_path) == []
...
def test_detect_concurrent_excludes_parent_chain(_winp, tmp_path):
    ...
    Simulates the real Windows topology: intellect.exe launcher (PID L) spawns
    ...
    rows = [
        ...
        _make_proc(launcher_pid, str(shim), "intellect.exe"),
    ]
...
@patch.object(cli_main, "_is_windows", return_value=True)
def test_detect_concurrent_still_finds_unrelated_other_intellect(_winp, tmp_path):
    """A sibling intellect.exe outside our ancestor chain must still be reported."""
    scripts_dir = tmp_path
    shim = scripts_dir / "intellect.exe"
    shim.write_bytes(b"")
    me = os.getpid()
    launcher_pid = me + 100  # our .exe launcher (parent — must be excluded)
    sibling_pid = me + 200  # an UNRELATED intellect.exe (must still be reported)

    rows = [
        _make_proc(me, str(shim), "python.exe"),
        _make_proc(launcher_pid, str(shim), "intellect.exe"),
        _make_proc(sibling_pid, str(shim), "intellect.exe"),
    ]
    fake_psutil = _fake_psutil_with_parent_chain(
        parent_chain=[launcher_pid],
        proc_iter_rows=rows,
    ...
    assert result == [(sibling_pid, "intellect.exe")]
...
def test_detect_concurrent_parent_chain_walks_deep(_winp, tmp_path):
    ...
    rows = [
        _make_proc(me, str(shim), "python.exe"),
        _make_proc(parent_pid, str(shim), "intellect.exe"),
        _make_proc(grandparent_pid, str(shim), "intellect.exe"),
        _make_proc(greatgrandparent_pid, str(shim), "intellect.exe"),
    ]
    fake_psutil = _fake_psutil_with_parent_chain(
...
def test_detect_concurrent_parents_call_robust_to_one_bad_hop(_winp, tmp_path):
    ...
    assert result == [(launcher_pid, "intellect.exe")]
...
def test_detect_concurrent_parent_walk_handles_stub_without_process(_winp, tmp_path):
    ...
    rows = [
        _make_proc(me, str(shim), "intellect.exe"),
        _make_proc(other_pid, str(shim), "intellect.exe"),
    ]
...
def test_format_message_mentions_pids_and_remediation(tmp_path):
    matches = [(1234, "intellect.exe"), (5678, "intellect.exe")]
    msg = cli_main._format_concurrent_instances_message(matches, tmp_path)

    assert "1234" in msg
    assert "5678" in msg
    assert "intellect.exe" in msg
    assert "gateway" in msg.lower() or "webui" in msg.lower()
    assert "--force" in msg
    # Mentions the file that would have been overwritten
    assert str(tmp_path / "intellect.exe") in msg
...
# ---------------------------------------------------------------------------
# _quarantine_running_intellect_exe — retry + reboot-deferred fallback
# ---------------------------------------------------------------------------
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    shim = tmp_path / "intellect.exe"
    shim.write_bytes(b"old")

    pairs = cli_main._quarantine_running_intellect_exe(tmp_path)

    assert len(pairs) == 1
    orig, quarantine = pairs[0]
    assert orig == shim
    assert quarantine.name.startswith("intellect.exe.old.")
...
def test_quarantine_retries_then_succeeds(_winp, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cli_main, "_intellect_exe_shims", lambda d: [shim])
...
def test_quarantine_actionable_warning_when_everything_fails(
    ...
    assert "gateway" in captured.lower() or "webui" in captured.lower()
...
def test_cmd_update_aborts_on_concurrent_instance(_winp, tmp_path, capsys):
    """If another intellect.exe is running, the update bails out before
    ...
    with patch.object(
        ...
        cli_main,
        "_detect_concurrent_intellect_instances",
        return_value=[(4242, "intellect.exe")],
    ), patch.object(
...
def test_cmd_update_force_bypasses_concurrent_check(_winp, tmp_path):
    ...
    detect = MagicMock(return_value=[(9, "intellect.exe")])
    ...
    with patch.object(
        ...
        cli_main, "_detect_concurrent_intellect_instances", detect
    ), patch.object(
