"""Tests for intellect_cli/bundles.py — the `intellect bundles` CLI subcommand."""

import argparse

import pytest

from intellect_cli.bundles import (
    bundles_command,
    register_cli,
)
...
def bundles_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("intellect_BUNDLES_DIR", str(bundles_dir))
