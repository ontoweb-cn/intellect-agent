"""Comprehensive tests for intellect_cli.profiles module.
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and INTELLECT_HOME for profile tests
# ---------------------------------------------------------------------------
...
class TestValidateProfileName:
    ...
    @pytest.mark.parametrize("name", ["intellect", "test", "tmp", "root", "sudo"])
    def test_reserved_names_rejected(self, name):
        """Reserved names collide with the Intellect install itself or with
...
class TestGetProfileDir:
    ...
    def test_default_returns_intellect_home(self, profile_env):
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes intellect-agent, .worktrees,
        ...
        (default_home / "intellect-agent" / ".git").mkdir(parents=True)
...
class TestNoSkillsOptOut:
    """Tests for `intellect profile create --no-skills` and the opt-out marker."""
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `intellect update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("intellect_cli.profiles._cleanup_gateway_service"):
...
class TestGetActiveProfileName:
    ...
    def test_default_intellect_home_returns_default(self, profile_env):
        # INTELLECT_HOME points to tmp_path/.intellect which is the default
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom INTELLECT_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("INTELLECT_HOME", str(custom))
        # With Docker-aware roots, a custom INTELLECT_HOME is the default —
...
class TestResolveProfileEnv:
    ...
    def test_default_returns_default_home(self, profile_env):
        ...
        assert result == str(tmp_path / ".intellect")
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("intellect")
        assert result is not None
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from intellect_cli.profiles import create_wrapper_script
    ...
    def test_remove_finds_bat_on_windows(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "win32")
        from intellect_cli.profiles import create_wrapper_script, remove_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
    ...
    def test_remove_returns_false_when_absent(self, profile_env):
        from intellect_cli.profiles import remove_wrapper_script
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        with patch("intellect_cli.profiles.check_alias_collision", return_value="skip"):
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        honcho_path = tmp_path / ".intellect" / "honcho.json"
        honcho_path.write_text(json.dumps({
            "hosts": {
                "intellect.ssi_health": {
                    "recallMode": "hybrid",
        ...
        assert "intellect.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["intellect_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["intellect_heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "intellect.ssi_health": {"workspace": "intellect", "enabled": True}
            }
        }))
        ...
        assert "intellect.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["intellect_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["intellect_heimdall"]["workspace"] == "intellect"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "intellect.ssi_health": {"aiPeer": "ssi_health"},
                "intellect_heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["intellect.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["intellect_heimdall"]["aiPeer"] == "heimdall"
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultintellectHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_intellect_home()."""
    ...
    def test_default_intellect_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_intellect_home()
...
class TestEdgeCases:
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from intellect_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".intellect"
