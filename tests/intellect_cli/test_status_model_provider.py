"""Tests for intellect_cli.status model/provider display."""

from types import SimpleNamespace

from intellect_cli.ontoweb_account import OntowebPaidServiceAccessInfo, OntowebPortalAccountInfo
...
def _patch_common_status_deps(monkeypatch, status_mod, tmp_path, *, openai_base_url=""):
    ...
    monkeypatch.setattr(status_mod, "get_intellect_home", lambda: tmp_path, raising=False)
    ...
    monkeypatch.setattr(auth_mod, "get_ontoweb_auth_status", lambda: {}, raising=False)
...
def test_show_status_displays_configured_dict_model_and_provider_label(monkeypatch, capsys, tmp_path):
    from intellect_cli import status as status_mod
...
def test_show_status_reports_managed_nous_features(monkeypatch, capsys, tmp_path):
    monkeypatch.setattr("intellect_cli.status.managed_ontoweb_tools_enabled", lambda: True)
    from intellect_cli import status as status_mod
    ...
    monkeypatch.setattr(
        status_mod,
        "load_config",
        lambda: {"model": {"default": "claude-opus-4-6", "provider": "ontoweb"}},
        raising=False,
    )
    monkeypatch.setattr(status_mod, "resolve_requested_provider", lambda requested=None: "ontoweb", raising=False)
    monkeypatch.setattr(status_mod, "resolve_provider", lambda requested=None, **kwargs: "ontoweb", raising=False)
    monkeypatch.setattr(status_mod, "provider_label", lambda provider: "ONTOWEB Portal", raising=False)
    monkeypatch.setattr(
        status_mod,
        "get_ontoweb_subscription_features",
        lambda config: OntowebSubscriptionFeatures(
            subscribed=True,
    ...
    out = capsys.readouterr().out
    assert "OntoWeb Tool Gateway" in out
    assert "Browser automation" in out
    assert "active via OntoWeb subscription" in out


def test_show_status_hides_ontoweb_subscription_section_when_feature_flag_is_off(monkeypatch, capsys, tmp_path):
    monkeypatch.setattr("intellect_cli.status.managed_ontoweb_tools_enabled", lambda: False)
    from intellect_cli import status as status_mod
    ...
    out = capsys.readouterr().out
    assert "OntoWeb Tool Gateway" not in out
...
def test_show_status_reports_exhausted_nous_credits(monkeypatch, capsys, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        lambda: OntowebPortalAccountInfo(
            ...
            portal_base_url="https://portal.example.test",
            paid_service_access_info=OntowebPaidServiceAccessInfo(
                allowed=False,
    ...
    monkeypatch.setattr(status_mod, "load_config", lambda: {"model": {"provider": "ontoweb"}}, raising=False)
    monkeypatch.setattr(status_mod, "resolve_requested_provider", lambda requested=None: "ontoweb", raising=False)
    monkeypatch.setattr(status_mod, "resolve_provider", lambda requested=None, **kwargs: "ontoweb", raising=False)
    monkeypatch.setattr(status_mod, "provider_label", lambda provider: "ONTOWEB Portal", raising=False)
    ...
    assert "free-tier OntoWeb account" not in out
...
def test_show_status_reports_empty_lmstudio_listing_as_reachable(monkeypatch, capsys, tmp_path):
    ...
    monkeypatch.setattr(
        "intellect_cli.models.probe_lmstudio_models",
