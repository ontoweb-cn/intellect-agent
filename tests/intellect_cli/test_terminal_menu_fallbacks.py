...
def test_prompt_model_selection_falls_back_on_menu_runtime_error(monkeypatch):
    ...
    monkeypatch.setattr("intellect_cli.curses_ui.curses_radiolist", _raise_menu)
    responses = iter(["2"])
...
def test_prompt_reasoning_effort_falls_back_on_menu_runtime_error(monkeypatch):
    from intellect_cli.main import _prompt_reasoning_effort_selection
...
def test_remove_custom_provider_falls_back_on_menu_runtime_error(tmp_path, monkeypatch):
    from intellect_cli.main import _remove_custom_provider
...
def test_named_custom_provider_model_picker_falls_back_on_menu_runtime_error(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("INTELLECT_HOME", str(tmp_path))
    monkeypatch.setattr("intellect_cli.curses_ui.curses_radiolist", _raise_menu)
    monkeypatch.setattr("intellect_cli.models.fetch_api_models", lambda *args, **kwargs: ["model-a", "model-b"])
