...
from intellect_cli import runtime_provider as rp
...
def test_named_custom_provider_does_not_shadow_builtin_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: {
            ...
            "api_key": "ontoweb-runtime-key",
            "source": "portal",
            "expires_at": None,
        },
    )

    resolved = rp.resolve_runtime_provider(requested="ontoweb")

    assert resolved["provider"] == "ontoweb"
    assert resolved["base_url"] == "https://inference-api.ontoweb.cn/v1"
    assert resolved["api_key"] == "ontoweb-runtime-key"
    assert resolved["requested_provider"] == "ontoweb"
...
def test_named_custom_provider_skipped_for_canonical_built_in(monkeypatch):
    """Companion to the test above: ``ontoweb`` is a canonical provider name
    (``resolve_provider('ontoweb') == 'ontoweb'``), so a custom entry with that name
    ...
    entry = rp._get_named_custom_provider("ontoweb")
...
def test_resolve_requested_provider_precedence(monkeypatch):
    ...
    monkeypatch.setattr(rp, "_get_model_config", lambda: {})
    assert rp.resolve_requested_provider() == "ontoweb"
...
def test_resolve_provider_custom_returns_custom():
    ...
    from intellect_cli.auth import resolve_provider
...
def test_auto_detected_ontoweb_auth_failure_falls_through_to_openrouter(monkeypatch):
    """When auto-detect picks OntoWeb but credentials are revoked, fall through to OpenRouter."""
    from intellect_cli.auth import AuthError
    ...
    # resolve_provider returns "ontoweb" (stale active_provider in auth.json)
    monkeypatch.setattr(rp, "resolve_provider", lambda *a, **k: "ontoweb")
    ...
    # OntoWeb credential resolution fails with revoked token
    monkeypatch.setattr(
        rp, "resolve_ontoweb_runtime_credentials",
        lambda **kw: (_ for _ in ()).throw(
            AuthError("Refresh session has been revoked",
                      provider="ontoweb", code="invalid_grant", relogin_required=True)
        ),
    )
...
def test_explicit_ontoweb_auth_failure_still_raises(monkeypatch):
    """When user explicitly requests OntoWeb and auth fails, the error should propagate."""
    from intellect_cli.auth import AuthError
    import pytest
    ...
    # With explicit "ontoweb", should raise — don't silently switch providers
    with pytest.raises(AuthError, match="Refresh session has been revoked"):
        rp.resolve_runtime_provider(requested="ontoweb")
...
class TestAzureFoundryResolution:
    ...
    def test_azure_foundry_missing_api_key_raises(self, monkeypatch):
        ...
        # `get_env_value` reads from ~/.intellect/.env — mock it to return None
...
class TestProviderEntryApiKeyEnvAlias:
    ...
    def test_snake_case_api_key_env_normalizes_to_key_env(self):
        from intellect_cli.config import _normalize_custom_provider_entry
        entry = {
            "name": "vendor",
    ...
    def test_valid_fields_set_lists_key_env(self):
        ...
        from intellect_cli.config import _VALID_CUSTOM_PROVIDER_FIELDS
...
def test_minimax_oauth_runtime_returns_anthropic_messages_mode(monkeypatch):
    ...
    from intellect_cli.auth import MINIMAX_OAUTH_GLOBAL_INFERENCE
...
def test_minimax_oauth_runtime_uses_inference_base_url(monkeypatch):
    ...
    from intellect_cli.auth import MINIMAX_OAUTH_CN_INFERENCE
