...
from intellect_cli.main import cmd_update, PROJECT_ROOT
...
class TestCmdUpdatePip:
    ...
    def test_update_pip_exports_virtualenv_from_sys_prefix(
        ...
        monkeypatch.setattr(hm.sys, "prefix", "/tmp/intellect-launcher-venv")
        monkeypatch.setattr(hm.sys, "base_prefix", "/usr")

        hm._cmd_update_pip(mock_args)

        assert mock_run.call_count == 1
        assert mock_run.call_args.args[0] == ["/usr/bin/uv", "pip", "install", "--upgrade", "intellect-agent"]
        assert mock_run.call_args.kwargs["env"]["VIRTUAL_ENV"] == "/tmp/intellect-launcher-venv"
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_on_fork_checks_upstream_when_origin_up_to_date(
        ...
        origin but behind ONTOWEB/intellect-agent silently misses updates.
        """
        from intellect_cli import main as hm
        ...
        with patch.object(
            hm,
            "_get_origin_url",
            return_value="https://github.com/example/intellect-agent.git",
    ...
    def test_update_refreshes_repo_and_tui_node_dependencies(
        ...
        # v0.4.1: web/ dashboard was removed. _update_node_dependencies()
        # only manages the ui-tui/ Ink TUI dependencies.
        update_flags = [
            "/usr/bin/npm",
            "ci",
            "--no-fund",
        ...
        # Regression for #18840: ui-tui install must stream
        # output (capture_output=False) so postinstall progress is visible
        # to the user.
        tui_call = npm_calls[0]
        tui_run_call = next(
            call
            for call in mock_run.call_args_list
            if call.args
            ...
            and call.kwargs.get("cwd") == PROJECT_ROOT / "ui-tui"
        )
        assert tui_run_call.kwargs.get("capture_output") is False, (
            "ui-tui npm install must stream output "
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "intellect_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "intellect_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch("intellect_cli.config.check_config_version", return_value=(1, 2)), patch(
            "intellect_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ), patch("intellect_cli.main.sys") as mock_sys:
            ...
            from intellect_cli.config import migrate_config
...
class TestCmdUpdateMigrationPrompt:
    ...
    def test_version_bump_only_applies_silently_without_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "intellect_cli.config.get_missing_env_vars", return_value=[]
        ), patch(
            "intellect_cli.config.get_missing_config_fields", return_value=[]
        ), patch(
            "intellect_cli.config.check_config_version", return_value=(5, 24)
        ), patch(
            "intellect_cli.config.migrate_config",
    ...
    def test_new_options_are_listed_by_name_before_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "intellect_cli.config.get_missing_env_vars", return_value=env_items
        ), patch(
            "intellect_cli.config.get_missing_config_fields", return_value=cfg_items
        ), patch(
            "intellect_cli.config.check_config_version", return_value=(1, 24)
        ), patch(
            "intellect_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": [], "warnings": []},
        ), patch("intellect_cli.main.sys") as mock_sys:
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        default_p = SimpleNamespace(name="default", path=Path("/fake/.intellect"))
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.intellect/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.intellect/profiles/work"))
        ...
        with (
            patch("intellect_cli.profiles.list_profiles", return_value=all_profiles),
            patch("intellect_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("intellect_cli.profiles.list_profiles", return_value=[default_p]),
            patch("intellect_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
...
class TestCmdUpdateBranchFlag:
    """``intellect update --branch <name>`` targets the requested branch.
...
class TestCmdUpdateCheckBranchFlag:
    """``intellect update --check --branch <name>`` honors the branch override.
    ...
    @patch("intellect_cli.config.detect_install_method", return_value="git")
    @patch("subprocess.run")
    ...
    @patch("intellect_cli.config.detect_install_method", return_value="pip")
    @patch("intellect_cli.banner.check_via_pypi", return_value=0)
    @patch("subprocess.run")
...
class TestCmdUpdateZipBranchRefusal:
    """``intellect update --branch=<non-main>`` must refuse on the ZIP fallback path.
    ...
    def test_zip_fallback_refuses_non_main_branch(self, capsys):
        from intellect_cli.main import _update_via_zip
