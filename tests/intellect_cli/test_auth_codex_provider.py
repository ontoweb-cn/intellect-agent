"""Tests for Codex auth — tokens stored in Intellect auth store (~/.intellect/auth.json)."""
...
def _legacy_codex_tests_mirror_auth_json(monkeypatch):
    """These tests assert auth.json pool sync; PR-A6 default is DB-only writes."""
...
def _setup_intellect_auth(intellect_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Intellect auth store."""
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_intellect_auth(intellect_home)
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_intellect_auth(intellect_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))
    ...
    monkeypatch.setattr("intellect_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    intellect_home = tmp_path / "intellect"
    _setup_intellect_auth(intellect_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    would have run ``intellect auth add openai-codex`` as a workaround, leaving
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Intellect auth store, not ~/.codex/."""
    intellect_home = tmp_path / "intellect"
    codex_home = tmp_path / "codex-cli"
    intellect_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "intellect-at", "refresh_token": "intellect-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Intellect store
    assert not (codex_home / "auth.json").exists()

    # Intellect auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "intellect-at"


def test_resolve_returns_intellect_auth_store_source(tmp_path, monkeypatch):
    intellect_home = tmp_path / "intellect"
    _setup_intellect_auth(intellect_home)
    monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run intellect auth".
    """
    from intellect_cli.auth import (
    ...
    assert "intellect auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from intellect_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from intellect_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "intellect_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr("intellect_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("intellect_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
