"""Tests for intellect_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestGetDisabledSkills:
    def test_empty_config(self):
        from intellect_cli.skills_config import get_disabled_skills
...
class TestSaveDisabledSkills:
    @patch("intellect_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from intellect_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("intellect_cli.config.load_config")
    ...
    @patch("intellect_cli.config.load_config")
    @patch.dict("os.environ", {"intellect_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """intellect_SESSION_PLATFORM should be used when intellect_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_intellect_platform_takes_precedence(self, tmp_path, monkeypatch):
        """intellect_PLATFORM should win over intellect_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("INTELLECT_HOME", str(tmp_path))
        monkeypatch.setenv("intellect_PLATFORM", "telegram")
        monkeypatch.setenv("intellect_SESSION_PLATFORM", "telegram")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from intellect_cli.skills_config import _get_categories
        skills = [
