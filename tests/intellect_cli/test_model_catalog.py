"""Tests for intellect_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate INTELLECT_HOME + reset any module-level catalog cache per test."""
    ...
    import importlib
    from intellect_cli import model_catalog
...
def _valid_manifest() -> dict:
    return {
        ...
        "providers": {
            ...
            "ontoweb": {
                "metadata": {"display_name": "ONTOWEB Portal"},
                "models": [
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from intellect_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    ...
    PRIMARY = "https://intellect-agent.ontoweb.cn/docs/api/model-catalog.json"
    FALLBACK = (
...
class TestCuratedAccessors:
    ...
    def test_nous_returns_ids(self, isolated_home):
        ...
        with patch.object(
            ...
            result = model_catalog.get_curated_ontoweb_models()
    ...
    def test_nous_returns_none_when_catalog_empty(self, isolated_home):
        from intellect_cli import model_catalog
        with patch.object(model_catalog, "_fetch_manifest", return_value=None):
            assert model_catalog.get_curated_ontoweb_models() is None
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in intellect_cli.models."""

    def test_curated_nous_ids_falls_back_to_hardcoded_on_empty_catalog(
        self, isolated_home
    ):
        from intellect_cli import model_catalog
        from intellect_cli.models import get_curated_ontoweb_model_ids, _PROVIDER_MODELS

        with patch.object(model_catalog, "_fetch_manifest", return_value=None):
            result = get_curated_ontoweb_model_ids()

        assert result == list(_PROVIDER_MODELS["ontoweb"])

    def test_curated_nous_ids_prefers_manifest(self, isolated_home):
        from intellect_cli import model_catalog
        from intellect_cli.models import get_curated_ontoweb_model_ids
    ...
    def test_picker_nous_row_uses_manifest(self, tmp_path, monkeypatch):
        """The /model picker must surface the manifest's ontoweb list, not the
        in-repo _PROVIDER_MODELS["ontoweb"] snapshot. Regression: before this
        ...
        the slash-command picker until the next Intellect release.
        """
        ...
        # ``INTELLECT_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` INTELLECT_HOME directly instead.
        import importlib
        from intellect_cli import model_catalog
        importlib.reload(model_catalog)
        try:
            from intellect_cli.model_switch import list_picker_providers

            active_home = Path(os.environ["INTELLECT_HOME"])
            (active_home / "auth.json").write_text(
                json.dumps(
                    {
                        "providers": {"ontoweb": {"access_token": "fake"}},
                        "credential_pool": {},
                    }
                )
            )
            ...
            with patch.object(
                ...
                picker = list_picker_providers(
                    current_provider="ontoweb", max_models=99
                )
        finally:
            model_catalog.reset_cache()

        nous_row = next((r for r in picker if r["slug"] == "ontoweb"), None)
        assert nous_row is not None, "ontoweb row must appear when authed"
...
#
# History: qwen/qwen3.6-plus was added to _PROVIDER_MODELS["ontoweb"] in commit
...
class TestManifestMatchesInRepoLists:
    ...
    def test_in_repo_lists_match_manifest(self):
        ...
        assert self._strip_volatile(actual) == self._strip_volatile(expected), (
            ...
            "_PROVIDER_MODELS['ontoweb'] / OPENROUTER_MODELS. "
