...
The notice prints the most recent curator run summary on `intellect update`,
...
session start) so users normally never see the rename map. `intellect update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    importlib.reload(intellect_main)
