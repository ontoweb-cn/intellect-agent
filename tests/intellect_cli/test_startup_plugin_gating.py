...
``intellect_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``intellect --help``, ``intellect version``,
``intellect logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
from intellect_cli.main import (
...
def _live_subcommand_names() -> set[str]:
    """Run ``intellect --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from intellect_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["intellect", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["intellect", "-w", "--tui"], None),
        (["intellect", "version"], "version"),
        (["intellect", "--tui", "chat"], "chat"),
        (["intellect", "-w", "logs"], "logs"),
        (["intellect", "chat", "hello world"], "chat"),
        (["intellect", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["intellect", "-m", "gpt5", "chat"], "chat"),
        (["intellect", "--model", "gpt5", "chat", "hi"], "chat"),
        (["intellect", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["intellect", "-z", "hello world"], None),
        (["intellect", "-z", "hello", "chat"], "chat"),
        (["intellect", "--model=gpt5", "chat"], "chat"),     # inline form
        (["intellect", "--", "chat"], "chat"),               # -- terminator
        (["intellect", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["intellect", "some-plugin-cmd"], "some-plugin-cmd"),
        (["intellect", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["intellect"],                          # bare → chat
        ["intellect", "--help"],                # top-level help
        ["intellect", "-h"],
        ["intellect", "version"],               # known built-in
        ["intellect", "logs"],
        ["intellect", "gateway", "run"],
        ["intellect", "--tui"],
        ["intellect", "-w", "--tui"],
        ["intellect", "chat", "hi"],
        ["intellect", "help"],                  # accepted built-in-ish
        ["intellect", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["intellect", "meet", "join"],          # potential google_meet plugin
        ["intellect", "honcho", "status"],      # potential memory plugin
        ["intellect", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``intellect help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"intellect_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
