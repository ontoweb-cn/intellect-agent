...
from intellect_cli.plugins import PluginManager
...
def _enable(intellect_home: Path, name: str) -> None:
    """Append ``name`` to ``plugins.enabled`` in ``<intellect_home>/config.yaml``."""
...
class TestCategoryNamespaceRecursion:
    def test_category_namespace_discovered(self, tmp_path, monkeypatch):
        ...
        import os
        intellect_home = Path(os.environ["INTELLECT_HOME"])  # set by hermetic conftest fixture
        user_plugins = intellect_home / "plugins"

        _write_plugin(user_plugins, ["image_gen", "openai"])
        _enable(intellect_home, "image_gen/openai")
    ...
    def test_flat_plugin_key_matches_name(self, tmp_path, monkeypatch):
        ...
        _enable(intellect_home, "my-plugin")
    ...
    def test_category_dir_with_manifest_is_leaf(self, tmp_path, monkeypatch):
        ...
        # at the parent.
        _write_plugin(user_plugins, ["image_gen", "openai"])
        _enable(intellect_home, "image_gen")
        _enable(intellect_home, "image_gen/openai")
...
class TestKindField:
    def test_default_kind_is_standalone(self, tmp_path, monkeypatch):
        import os
        intellect_home = Path(os.environ["INTELLECT_HOME"])  # set by hermetic conftest fixture
        _write_plugin(intellect_home / "plugins", ["p1"])
        _enable(intellect_home, "p1")
...
class TestBackendGate:
    ...
    def test_user_backend_loads_when_enabled(self, tmp_path, monkeypatch):
        ...
        _enable(intellect_home, "image_gen/fancy")
    ...
    def test_exclusive_kind_skipped(self, tmp_path, monkeypatch):
        ...
        _enable(intellect_home, "some-backend")
...
class TestRegisterImageGenProvider:
    def test_accepts_valid_provider(self, tmp_path, monkeypatch):
        ...
        _enable(intellect_home, "my-img-plugin")
    ...
    def test_rejects_non_provider(self, tmp_path, monkeypatch, caplog):
        ...
        _enable(intellect_home, "bad-img-plugin")
