"""Tests for intellect_cli.tools_config platform tool persistence."""
...
def test_get_platform_tools_x_search_auto_enabled_when_xai_oauth_present(monkeypatch):
    ...
    to also click through `intellect tools` → X (Twitter) Search to flip the
    toolset on. Tool's check_fn still gates schema registration if creds
    later go missing.
    """
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "intellect_cli.tools_config._xai_credentials_present", lambda: True
    )
...
def test_get_platform_tools_x_search_off_when_no_xai_credentials(monkeypatch):
    ...
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "intellect_cli.tools_config._xai_credentials_present", lambda: False
    )
...
def test_get_platform_tools_x_search_respects_explicit_config(monkeypatch):
    """Once the user has saved an explicit toolset list via `intellect tools`,
    ...
    # User explicitly opted into spotify but not x_search via `intellect tools`.
    config = {"platform_toolsets": {"cli": ["intellect-cli", "spotify"]}}
    enabled = _get_platform_tools(config, "cli")
...
def test_get_platform_tools_expands_composite_when_mixed_with_configurable():
    """``[intellect-cli, spotify]`` (composite + configurable) must keep the full
    ``intellect-cli`` toolset alongside the explicit Spotify opt-in. The
    has_explicit_config branch used to drop ``intellect-cli`` on the floor,
    ...
    for ts in ("terminal", "file", "web", "browser", "memory", "delegation",
        ...
        assert ts in enabled, f"{ts} should be enabled when intellect-cli is listed"
...
def test_get_platform_tools_composite_only_unchanged():
    ...
    composite_only = _get_platform_tools(
        {"platform_toolsets": {"cli": ["intellect-cli"]}},
        "cli",
...
def test_get_platform_tools_mixed_does_not_resurrect_default_off():
    ...
    pull-in. Without this, ``intellect-cli`` expansion would re-enable
    ``moa`` / ``rl`` / ``homeassistant`` for users who never opted in."""
    config = {"platform_toolsets": {"cli": ["intellect-cli", "terminal"]}}
...
def test_get_platform_tools_preserves_explicit_empty_selection():
    ...
    # live in _INTELLECT_CORE_TOOLS but aren't user-toggleable) are still
    # auto-recovered by _get_platform_tools so saving via `intellect tools`
...
def test_apply_toolset_change_from_default_does_not_enable_default_off_toolsets():
    ...
    with patch("intellect_cli.tools_config.save_config"):
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://gitee.com/ontoweb/intellect-agent/issues/1247
    """
    config = {
        "platform_toolsets": {
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    """Platform default toolsets (intellect-cli, intellect-telegram, etc.) must NOT
    ...
    ``intellect tools``, but intellect-cli stays in the config and re-enables
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                ...
                "delegation", "file", "intellect-cli",  # <-- the culprit
    ...
    # intellect-cli must NOT survive — it's a platform default, not an MCP server
    assert "intellect-cli" not in saved
...
def test_save_platform_tools_does_not_preserve_intellect_telegram():
    """Same bug for Telegram — intellect-telegram must not be preserved."""
    config = {
        "platform_toolsets": {
            "telegram": [
                "browser", "file", "intellect-telegram", "terminal", "web",
            ]
        }
    }
    ...
    assert "intellect-telegram" not in saved
    assert "web" in saved
...
def test_save_platform_tools_still_preserves_mcp_with_platform_default_present():
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                "web", "terminal", "intellect-cli", "my-mcp-server", "github-tools",
            ]
        }
    }
...
def test_visible_providers_include_ontoweb_subscription_when_logged_in(monkeypatch):
    config = {"model": {"provider": "ontoweb"}}

    monkeypatch.setattr(
        "intellect_cli.ontoweb_subscription.get_ontoweb_portal_account_info",
        lambda: OntowebPortalAccountInfo(
            logged_in=True,
            source="jwt",
            fresh=False,
    ...
    assert providers[0]["name"].startswith("OntoWeb Subscription")


def test_visible_providers_force_fresh_shows_ontoweb_subscription_after_upgrade(monkeypatch):
    calls = []

    def fake_subscription_features(config, *, force_fresh=False):
        calls.append(("features", force_fresh))
        return SimpleNamespace(
            ontoweb_auth_present=True,
            account_info=OntowebPortalAccountInfo(
                logged_in=True,
    ...
    monkeypatch.setattr(
        "intellect_cli.tools_config.get_ontoweb_subscription_features",
...
def test_visible_providers_show_ontoweb_subscription_when_logged_out(monkeypatch):
    """OntoWeb-managed Tool Gateway rows are always listed, even logged out.
    ...
    assert any(p["name"].startswith("OntoWeb Subscription") for p in providers)


def test_visible_providers_show_ontoweb_subscription_when_paid_access_is_false(monkeypatch):
    ...
    shown; ``ensure_ontoweb_portal_access`` blocks activation if still unpaid.
    """
    config = {"model": {"provider": "ontoweb"}}
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("intellect_cli.tools_config._run_post_setup", lambda key: None)
...
def test_reconfigure_lists_enabled_web_without_existing_provider_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "intellect_cli.tools_config._toolset_has_keys",
    ...
    monkeypatch.setattr("intellect_cli.tools_config._prompt_choice", fake_prompt_choice)
    monkeypatch.setattr(
        "intellect_cli.tools_config._configure_tool_category_for_reconfig",
        lambda ts_key, cat, config, **kwargs: configured.append(ts_key),
    )
    monkeypatch.setattr("intellect_cli.tools_config.save_config", lambda config: None)
...
def test_first_install_nous_auto_configures_managed_defaults(monkeypatch):
    monkeypatch.setattr("intellect_cli.ontoweb_subscription.managed_ontoweb_tools_enabled", lambda: True)
    config = {
        "model": {"provider": "ontoweb"},
        "platform_toolsets": {"cli": []},
    }
    ...
    monkeypatch.setattr(
        "intellect_cli.tools_config._prompt_toolset_checklist",
    ...
    # apply_ontoweb_managed_defaults per platform; a second iteration sees values
    # set by the first as "explicit" and skips them.
    monkeypatch.setattr(
        "intellect_cli.tools_config._get_enabled_platforms",
        lambda: ["cli"],
    )
    monkeypatch.setattr(
        "intellect_cli.ontoweb_subscription.get_ontoweb_portal_account_info",
        lambda *args, **kwargs: OntowebPortalAccountInfo(
            logged_in=True,
            source="jwt",
            fresh=False,
            paid_service_access=True,
    ...
    configured = []
    monkeypatch.setattr(
        "intellect_cli.tools_config._configure_toolset",
...
def test_first_install_nous_auto_configures_video_gen(monkeypatch):
    """When a OntoWeb subscriber checks video_gen in the toolset checklist,
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from intellect_cli.tools_config import PLATFORMS
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """intellect-gateway includes list should cover all messaging platforms."""
        from intellect_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["intellect-gateway"]["includes"])
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from intellect_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from intellect_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://gitee.com/ontoweb/intellect-agent/issues/6901
    """
    config = {
...
def test_computer_use_needs_configuration_when_cua_driver_post_setup_pending():
    ...
    Returning users enabling Computer Use through `intellect tools` must reach the
...
def test_computer_use_respects_custom_cua_driver_command():
    """The setup gate should match runtime's intellect_CUA_DRIVER_CMD override."""
    def fake_which(name: str):
        return "/opt/bin/custom-cua" if name == "custom-cua" else None

    with patch.dict("os.environ", {"intellect_CUA_DRIVER_CMD": "custom-cua"}), \
...
def test_computer_use_blank_custom_driver_command_falls_back_to_default():
    ...
    with patch.dict("os.environ", {"intellect_CUA_DRIVER_CMD": "   "}), \
         patch("shutil.which", side_effect=fake_which):
        assert _toolset_needs_configuration_prompt("computer_use", {}) is False
...
class TestImagegenBackendRegistry:
    ...
    def test_fal_backend_registered(self):
        from intellect_cli.tools_config import IMAGEGEN_BACKENDS
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        """Both OntoWeb Subscription and FAL.ai providers must carry the
        imagegen_backend tag so _configure_provider fires the picker."""
        from intellect_cli.tools_config import TOOL_CATEGORIES
...
class TestImagegenModelPicker:
    ...
    def test_picker_writes_chosen_model_to_config(self):
        from intellect_cli.tools_config import _configure_imagegen_model
        config = {}
        # Force _prompt_choice to pick index 1 (second-in-ordered-list).
        with patch("intellect_cli.tools_config._prompt_choice", return_value=1):
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch("intellect_cli.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
    ...
    def test_picker_repairs_corrupt_config_section(self):
        ...
        with patch("intellect_cli.tools_config._prompt_choice", return_value=0):
            _configure_imagegen_model("fal", config)
...
def test_save_platform_tools_clears_no_mcp_sentinel():
    """`intellect tools` has no UI for no_mcp, so saving from the picker clears
...
def test_get_platform_tools_recovers_non_configurable_toolsets_from_composite():
    ...
    fake_toolsets["intellect-_test_platform"] = {
    ...
    test_platforms = {
        "_test_platform": {"label": "Test", "default_toolset": "intellect-_test_platform"},
    }

    with mock_patch("intellect_cli.tools_config.PLATFORMS", {**PLATFORMS, **test_platforms}):
...
def test_get_platform_tools_second_pass_skips_fully_claimed_toolsets():
    ...
    be added by the second pass (prevents 'search', 'intellect-acp' noise).
    """
...
def test_get_platform_tools_discord_both_off_by_default():
    """Both `discord` and `discord_admin` are opt-in via `intellect tools`,
...
def test_discord_toolsets_not_available_on_other_platforms():
    ...
    from intellect_cli.tools_config import _toolset_allowed_for_platform
...
def test_discord_toolsets_user_enabled_are_honored():
    """When the user opts in via `intellect tools`, the toolset appears."""
...
def test_save_platform_tools_strips_restricted_toolsets():
    ...
    from intellect_cli.tools_config import _save_platform_tools
    config = {}
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    them twice — otherwise `intellect tools` → "reconfigure existing" shows
    the same toolset two rows in a row.
    """
    from intellect_cli.tools_config import _get_effective_configurable_toolsets
...
def test_reconfigure_provider_syncs_use_gateway(monkeypatch, provider, config_key, expected):
    ...
    monkeypatch.setattr(
        "intellect_cli.ontoweb_subscription.ensure_ontoweb_portal_access",
        lambda **kwargs: True,
    )
    config = {}
...
def test_reconfigure_provider_runs_post_setup_for_env_var_providers(
    ...
    called = []
    monkeypatch.setattr("intellect_cli.tools_config._run_post_setup", lambda key: called.append(key))
    monkeypatch.setattr("intellect_cli.tools_config.get_env_value", lambda k: None)
    monkeypatch.setattr("intellect_cli.tools_config._prompt", lambda *a, **kw: "")
    monkeypatch.setattr("intellect_cli.tools_config.save_env_value", lambda k, v: None)
...
# ---------------------------------------------------------------------------
# Inline OntoWeb Portal login gate on managed-provider selection
# ---------------------------------------------------------------------------


def test_configure_managed_provider_blocks_when_not_entitled(monkeypatch):
    """Selecting a OntoWeb-managed backend without paid access writes no config."""
    monkeypatch.setattr(
        "intellect_cli.ontoweb_subscription.ensure_ontoweb_portal_access",
        lambda **kwargs: False,
    )
    provider = {
        "name": "OntoWeb Subscription (Firecrawl)",
...
def test_configure_non_managed_provider_skips_portal_gate(monkeypatch):
    """A self-hosted provider must never trigger the OntoWeb Portal login gate."""
    called = {"gate": False}
    ...
    monkeypatch.setattr(
        "intellect_cli.ontoweb_subscription.ensure_ontoweb_portal_access", _boom
    )
