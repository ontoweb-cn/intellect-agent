"""Tests for the Kanban DB layer (intellect_cli.kanban_db)."""
...
def test_connect_honors_kanban_busy_timeout_env(kanban_home, monkeypatch):
    ...
    """
    monkeypatch.setenv("intellect_KANBAN_BUSY_TIMEOUT_MS", "123456")
...
def test_connect_rejects_tls_record_in_sqlite_header(tmp_path, monkeypatch):
    ...
    home.mkdir()
    monkeypatch.setenv("INTELLECT_HOME", str(home))
    monkeypatch.delenv("intellect_KANBAN_DB", raising=False)
    monkeypatch.delenv("intellect_KANBAN_HOME", raising=False)
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
...
def test_claim_uses_env_default_ttl(kanban_home, monkeypatch):
    monkeypatch.setenv("intellect_KANBAN_CLAIM_TTL_SECONDS", "3600")
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x", assignee="a")
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import intellect_cli.kanban_db as _kb
...
def test_detect_crashed_workers_skips_freshly_claimed_tasks(
    ...
    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.delenv("intellect_KANBAN_CRASH_GRACE_SECONDS", raising=False)
...
def test_detect_crashed_workers_grace_period_env_override(
    ...
    """intellect_KANBAN_CRASH_GRACE_SECONDS env var adjusts the window."""
    import intellect_cli.kanban_db as _kb

    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("intellect_KANBAN_CRASH_GRACE_SECONDS", "5")
...
def test_resolve_crash_grace_seconds_handles_bad_env(monkeypatch):
    ...
    for bad_val in ("notanumber", "-5", ""):
        monkeypatch.setenv("intellect_KANBAN_CRASH_GRACE_SECONDS", bad_val)
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Intellect profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="intellect-task", assignee="daily")
...
def test_scratch_workspace_created_under_intellect_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
def test_cleanup_workspace_removes_managed_scratch_dir(kanban_home):
    ...
    with kb.connect() as conn:
        ...
        assert ws.is_dir()
        kb.complete_task(conn, t, result="ok")
    assert not ws.exists(), "Intellect-managed scratch dir should be cleaned up"
...
def test_cleanup_workspace_honors_workspaces_root_env_override(tmp_path, monkeypatch):
    """``intellect_KANBAN_WORKSPACES_ROOT`` extends the managed-scratch set.
    ...
    monkeypatch.setenv("intellect_KANBAN_WORKSPACES_ROOT", str(workspaces_override))
    kb.init_db()
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """Intellect' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` Intellect' DB/metadata/logs on task completion.
    """
...
# spawned with `intellect -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's INTELLECT_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's INTELLECT_HOME."""

    def _set_home(self, monkeypatch, tmp_path, intellect_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))
        monkeypatch.delenv("intellect_KANBAN_HOME", raising=False)

    def test_default_install_anchors_at_home_dot_intellect(
        self, tmp_path, monkeypatch
    ):
        # Standard install: INTELLECT_HOME == ~/.intellect, no profile active.
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.intellect/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.intellect/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.intellect/kanban.db.
        default_home = tmp_path / ".intellect"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # INTELLECT_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `intellect -p coder`).
        monkeypatch.setenv("INTELLECT_HOME", str(profile_home))
    ...
    def test_docker_custom_intellect_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: INTELLECT_HOME points outside ~/.intellect.
        # `get_default_intellect_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".intellect"`.
        custom_root = tmp_path / "opt" / "intellect"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: INTELLECT_HOME=/opt/intellect/profiles/coder;
        # `get_default_intellect_root()` walks up to /opt/intellect because
    ...
    def test_explicit_override_via_intellect_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Explicit override: intellect_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".intellect"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("INTELLECT_HOME", str(profile_home))
        monkeypatch.setenv("intellect_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("intellect_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # INTELLECT_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile INTELLECT_HOME and reads.
        monkeypatch.setenv("INTELLECT_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_intellect_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # intellect_KANBAN_DB pins the file path directly and beats both
        # intellect_KANBAN_HOME and the `get_default_intellect_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("INTELLECT_HOME", str(default_home))
        monkeypatch.setenv("intellect_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("intellect_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows intellect_KANBAN_HOME -- the pins
        # are independent.
        assert kb.workspaces_root() == umbrella / "kanban" / "workspaces"

    def test_intellect_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # intellect_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".intellect"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("INTELLECT_HOME", str(default_home))
        monkeypatch.setenv("intellect_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("intellect_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows intellect_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        # intellect_KANBAN_HOME.
        default_home = tmp_path / ".intellect"
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("INTELLECT_HOME", str(default_home))
        monkeypatch.setenv("intellect_KANBAN_DB", "   ")
        monkeypatch.setenv("intellect_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject intellect_KANBAN_DB
        # and intellect_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites INTELLECT_HOME.
        default_home = tmp_path / ".intellect"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
        ...
        env = captured["env"]
        assert env["intellect_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["intellect_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["INTELLECT_KANBAN_TASK"] == "t_dispatch_env"
        assert env["intellect_KANBAN_BRANCH"] == "wt/t_dispatch_env"
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see intellect_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    (see intellect-agent issue #22032).
    ...
    with _patch("intellect_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="intellect_state"):
            conn = kb.connect()
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'intellect kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_intellect_argv()
#
# Workers spawned by the dispatcher must use a `intellect` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["intellect", ...]`
...
def test_resolve_intellect_argv_prefers_path_shim(monkeypatch):
    """When `intellect` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import intellect_cli.kanban_db as kb

    monkeypatch.delenv("INTELLECT_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/intellect")
    argv = kb._resolve_intellect_argv()
    assert argv == ["/usr/local/bin/intellect"]


def test_resolve_intellect_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    """A relative executable override must not remain workspace-cwd-dependent."""
    import intellect_cli.kanban_db as kb

    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("INTELLECT_BIN", ".\\intellect.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_intellect_argv() == [os.path.abspath(".\\intellect.exe")]


def test_resolve_intellect_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "intellect.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("INTELLECT_BIN", raising=False)
    monkeypatch.setenv("PATH", str(bin_dir))
    monkeypatch.setenv("PATHEXT", ".CMD")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_intellect_argv() == [sys.executable, "-m", "intellect_cli.main"]


def test_resolve_intellect_argv_honors_intellect_bin_path_override(monkeypatch, tmp_path):
    """An explicit path-like INTELLECT_BIN lets service managers pin the executable."""
    import shutil
    import intellect_cli.kanban_db as kb

    shim = tmp_path / "bin" / "intellect"
    shim.parent.mkdir()
    shim.write_text("#!/bin/sh\n", encoding="utf-8")
    monkeypatch.setenv("INTELLECT_BIN", str(shim))
    monkeypatch.setattr(shutil, "which", lambda name: None)

    assert kb._resolve_intellect_argv() == [str(shim)]


def test_resolve_intellect_argv_intellect_bin_bare_name_uses_path(monkeypatch, tmp_path):
    """Bare INTELLECT_BIN values keep PATH semantics instead of cwd shadowing."""
    import stat
    import intellect_cli.kanban_db as kb

    cwd_intellect = tmp_path / "intellect"
    cwd_intellect.write_text("wrong\n", encoding="utf-8")
    cwd_intellect.chmod(cwd_intellect.stat().st_mode | stat.S_IXUSR)
    path_intellect = tmp_path / "bin" / "intellect"
    path_intellect.parent.mkdir()
    path_intellect.write_text("right\n", encoding="utf-8")
    path_intellect.chmod(path_intellect.stat().st_mode | stat.S_IXUSR)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("PATH", str(path_intellect.parent))
    monkeypatch.setenv("INTELLECT_BIN", "intellect")

    assert kb._resolve_intellect_argv() == [str(path_intellect)]


def test_resolve_intellect_argv_intellect_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    """Bare INTELLECT_BIN does not accept current-directory shadow executables."""
    import sys
    import intellect_cli.kanban_db as kb

    (tmp_path / "intellect.exe").write_text("wrong\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
...
def test_resolve_intellect_argv_intellect_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
    """A PATH-resolved INTELLECT_BIN batch shim is not used as worker argv[0]."""
    import sys
    import intellect_cli.kanban_db as kb
...
def test_resolve_intellect_argv_intellect_bin_unresolved_bare_name_falls_back(monkeypatch):
    """Unresolved INTELLECT_BIN command names do not delegate cwd search to Popen."""
    import sys
    import intellect_cli.kanban_db as kb
...
def test_resolve_intellect_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m intellect_cli.main`.

    Pins the correct module name (NOT `intellect` — there is no top-level
    `intellect` package). Regression for #23198: the original PR shipped
    `python -m intellect` which fails with `No module named intellect` on every
    invocation.
    """
    import shutil
    import sys
    ...
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_intellect_argv()
    assert argv == [sys.executable, "-m", "intellect_cli.main"]


def test_resolve_intellect_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `intellect_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m intellect_cli.main --version`
    ...
    with mock.patch.dict(os.environ, {}, clear=False):
        os.environ.pop("INTELLECT_BIN", None)
    ...
    assert "Intellect Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_task_dict_survives_corrupt_created_at(tmp_path, monkeypatch):
    """Defense in depth: even if task_age ever raised, the task API must not 500.

    Verify a single corrupt row doesn't turn the whole board response into an
    error.
    """
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="intellect_cli.kanban_db"):
        with kb.connect() as conn:
...
def test_write_txn_healthy_commit_no_exception(tmp_path):
    ...
    from intellect_cli.kanban_db import connect, write_txn
    db = tmp_path / "test.db"
...
def test_write_txn_raises_on_truncated_file(tmp_path):
    ...
    with pytest.raises(sqlite3.DatabaseError, match="torn-extend|page count mismatch"):
        with unittest.mock.patch("intellect_cli.kanban_db.os.path.getsize", side_effect=fake_getsize):
            with write_txn(conn) as c:
                c.execute(
                    "INSERT INTO tasks (id, title, assignee, status, priority, created_at) "
...
def test_write_txn_post_commit_check_fires_every_call(tmp_path):
    ...
    from intellect_cli.kanban_db import connect, write_txn
    import intellect_cli.kanban_db as kanban_db_module
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
    call_count = 0
...
def test_connect_sets_wal_autocheckpoint_100(tmp_path):
    ...
    from intellect_cli.kanban_db import connect
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    import struct
    from intellect_cli.kanban_db import connect, _check_file_length_invariant
...
def test_reap_worker_zombies_returns_count():
    ...
    with patch("intellect_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("intellect_cli.kanban_db._record_worker_exit"):
...
def test_reap_worker_zombies_noop_on_windows(monkeypatch):
    ...
    monkeypatch.setattr("intellect_cli.kanban_db.os.name", "nt")
    with patch("intellect_cli.kanban_db.os.waitpid") as mock_waitpid:
...
def test_reap_worker_zombies_noop_no_children():
    ...
    with patch("intellect_cli.kanban_db.os.waitpid", side_effect=ChildProcessError):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_reap_worker_zombies_records_exit_status():
    ...
    with patch("intellect_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch(
            "intellect_cli.kanban_db._record_worker_exit",
...
def test_reap_worker_zombies_handles_waitpid_os_error():
    ...
    with patch("intellect_cli.kanban_db.os.waitpid", side_effect=OSError("test error")):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_zombie_reaper_survives_all_boards_failing():
    ...
    for tick in range(5):
        ...
        with patch(
            "intellect_cli.kanban_db.os.waitpid", side_effect=make_fake_waitpid(pids)
        ):
            with patch("intellect_cli.kanban_db._record_worker_exit"):
                pids = kb.reap_worker_zombies()
...
def test_dispatch_once_still_reaps_via_extracted_fn(kanban_home):
    ...
    with patch("intellect_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("intellect_cli.kanban_db._record_worker_exit"):
            with patch("intellect_cli.kanban_db.os.name", "posix"):
                pids = kb.reap_worker_zombies()
