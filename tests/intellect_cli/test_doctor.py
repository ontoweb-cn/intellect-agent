"""Tests for intellect_cli.doctor."""
...
import intellect_cli.doctor as doctor
import intellect_cli.gateway as gateway_cli
from intellect_cli import doctor as doctor_mod
from intellect_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `intellect doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "INTELLECT_HOME", intellect_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("INTELLECT_KANBAN_TASK", "probe")
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("intellect_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_intellect_home(self, tmp_path, provider=""):
        """Create a minimal INTELLECT_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_intellect_home(tmp_path, provider)
...
def test_run_doctor_accepts_intellect_provider_ids_that_catalog_aliases(
...
def _run_doctor_with_healthy_oauth_fallback(
    ...
    *,
    ...
    monkeypatch.setattr(_auth_mod, "get_ontoweb_auth_status", lambda: {"logged_in": True})
    monkeypatch.setattr(_auth_mod, "get_codex_auth_status", lambda: {})
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from intellect_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("intellect_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "intellect_cli.doctor", raising=False)
        from intellect_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
...
class TestDoctorXaiOAuthStatus:
    ...
    failure (or runtime exception) cannot silence the OntoWeb / Codex / Gemini /
    ...
    def _run(self, monkeypatch, tmp_path, *, xai_auth_fn) -> str:
        ...
        from intellect_cli import auth as _auth_mod
        monkeypatch.setattr(_auth_mod, "get_ontoweb_auth_status", lambda: {"logged_in": False})
    ...
    def test_import_failure_does_not_affect_other_providers(self, monkeypatch, tmp_path):
        """OntoWeb / Codex / Gemini / MiniMax rows must survive an xAI import failure."""
        home = tmp_path / ".intellect"
        home.mkdir(parents=True, exist_ok=True)
        (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
        project = tmp_path / "project"
        ...
        with contextlib.redirect_stdout(buf):
            doctor_mod.run_doctor(Namespace(fix=False))
        out = buf.getvalue()
        assert "ONTOWEB Portal auth" in out
        assert "logged in" in out
