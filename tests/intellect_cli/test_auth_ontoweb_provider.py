"""Regression tests for OntoWeb OAuth refresh and inference JWT interactions."""
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from intellect_cli.auth import _resolve_verify
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("intellect_CA_BUNDLE", raising=False)
        result = _resolve_verify(auth_state={"tls": {}})
        assert result is True

    def test_missing_intellect_ca_bundle_env_falls_back(self, monkeypatch):
        from intellect_cli.auth import _resolve_verify

        monkeypatch.setenv("intellect_CA_BUNDLE", "/nonexistent/intellect-ca.pem")
...
def _setup_ontoweb_auth(
    ...
    *,
    ...
    auth_store = {
        "version": 1,
        "active_provider": "ontoweb",
        "providers": {
            "ontoweb": {
                "portal_base_url": "https://portal.example.com",
                "inference_base_url": "https://inference.example.com/v1",
                "client_id": "intellect-cli",
                "token_type": "Bearer",
                "scope": scope,
    ...
    (intellect_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_resolve_nous_runtime_credentials_prefers_invoke_jwt_and_mirrors(
    ...
    import intellect_cli.auth as auth_mod
    ...
    _setup_ontoweb_auth(
        intellect_home,
        access_token=token,
        scope=auth_mod.DEFAULT_ONTOWEB_SCOPE,
    ...
    pool_entries = payload["credential_pool"]["ontoweb"]
    assert len(pool_entries) == 1
    assert pool_entries[0]["agent_key"] == token
    assert pool_entries[0]["source"] == auth_mod.ONTOWEB_DEVICE_CODE_SOURCE
...
def test_resolve_nous_runtime_credentials_invoke_jwt_is_idempotent(
    ...
    auth_path = intellect_home / "auth.json"
    ...
    monkeypatch.setattr(auth_mod, "_write_shared_ontoweb_state", _unexpected_shared_write)
    monkeypatch.setattr(
        auth_mod,
    ...
    assert (
        payload["providers"]["ontoweb"]["agent_key_obtained_at"]
...
def test_resolve_nous_runtime_credentials_refreshes_legacy_agent_key_to_invoke_jwt(
    ...
    assert payload["credential_pool"]["ontoweb"][0]["agent_key"] == refreshed_token
...
def test_resolve_nous_runtime_credentials_reauths_when_invoke_scope_missing(
    ...
    with pytest.raises(AuthError) as exc:
        auth_mod.resolve_ontoweb_runtime_credentials()
...
def test_nous_device_code_login_does_not_retry_legacy_scope_when_invoke_refused(monkeypatch):
    ...
    with pytest.raises(httpx.HTTPStatusError):
        auth_mod._ontoweb_device_code_login(
            portal_base_url="https://portal.example.com",
            inference_base_url="https://inference.example.com/v1",
            open_browser=False,
            timeout_seconds=1,
    ...
    assert scopes == [auth_mod.DEFAULT_ONTOWEB_SCOPE]
...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))
    monkeypatch.setenv("intellect_AGENT_USE_LEGACY_SESSION_KEYS", "true")
    ...
    result = auth_mod._ontoweb_device_code_login(
        portal_base_url="https://portal.example.com",
        inference_base_url="https://inference.example.com/v1",
        open_browser=False,
        timeout_seconds=1,
    ...
    assert requested_scopes == [auth_mod.DEFAULT_ONTOWEB_SCOPE]
...
def test_nous_inference_auth_logs_do_not_include_secret_values(
    ...
    caplog.set_level(logging.INFO, logger="intellect_cli.auth")
    auth_mod.resolve_ontoweb_runtime_credentials(
        force_refresh=True,
    )
...
def test_get_ontoweb_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    """get_ontoweb_auth_status() should find OntoWeb credentials in the pool
    even when the auth store has no OntoWeb provider entry — this is the
    ...
    intellect_home = tmp_path / "intellect"
    intellect_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store — no OntoWeb provider entry
    (intellect_home / "auth.json").write_text(json.dumps({
        "version": 1, "providers": {},
    }))
    monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))

    # Seed the credential pool with a OntoWeb entry
    from agent.credential_pool import PooledCredential, load_pool
    pool = load_pool("ontoweb")
    token = _invoke_jwt(seconds=3600)
    expires_at = _future_iso(3600)
    entry = PooledCredential.from_dict("ontoweb", {
        "access_token": token,
    ...
    status = get_ontoweb_auth_status()
    assert status["logged_in"] is True
    assert "example.com" in str(status.get("portal_base_url", ""))


def test_get_ontoweb_auth_status_pool_opaque_key_is_not_inference_credential(tmp_path, monkeypatch):
    from intellect_cli.auth import get_ontoweb_auth_status, invalidate_ontoweb_auth_status_cache
...
def test_get_ontoweb_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    """get_ontoweb_auth_status() falls back to auth store when credential
    pool is empty.
    """
    from intellect_cli.auth import get_ontoweb_auth_status

    intellect_home = tmp_path / "intellect"
    _setup_ontoweb_auth(intellect_home, access_token="at-123")
    monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))
    monkeypatch.setattr(
        "intellect_cli.auth.resolve_ontoweb_runtime_credentials",
        lambda **kwargs: {
            "base_url": "https://inference.example.com/v1",
...
def test_get_ontoweb_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    from intellect_cli.auth import get_ontoweb_auth_status
    from agent.credential_pool import PooledCredential, load_pool

    intellect_home = tmp_path / "intellect"
    _setup_ontoweb_auth(intellect_home, access_token="at-fresh")
    monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))

    pool = load_pool("ontoweb")
    stale = PooledCredential.from_dict("ontoweb", {
...
def test_get_ontoweb_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    from intellect_cli.auth import get_ontoweb_auth_status
    ...
    def _boom(**kwargs):
        raise AuthError("Refresh session has been revoked", provider="ontoweb", relogin_required=True)

    monkeypatch.setattr("intellect_cli.auth.resolve_ontoweb_runtime_credentials", _boom)
...
def test_get_ontoweb_auth_status_empty_returns_not_logged_in(tmp_path, monkeypatch):
    """get_ontoweb_auth_status() returns logged_in=False when both pool
...
def test_refresh_token_persisted_when_refreshed_jwt_lacks_invoke_scope(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("intellect_cli.auth._refresh_access_token", _fake_refresh_access_token)

    with pytest.raises(AuthError) as exc:
        resolve_ontoweb_runtime_credentials()
    assert exc.value.code == "missing_inference_invoke_scope"

    state_after_failure = get_provider_auth_state("ontoweb")
...
def test_terminal_refresh_failure_quarantines_tokens(
    ...
    """A revoked/invalid OntoWeb refresh token must not be replayed forever."""
    from intellect_cli import auth as auth_mod
    ...
    assert load_pool("ontoweb").select() is not None
    ...
    auth_mod._write_shared_ontoweb_state(shared_state)
    ...
    state_after_failure = auth_mod.get_provider_auth_state("ontoweb")
    assert state_after_failure is not None
...
def test_managed_access_token_refresh_failure_quarantines_tokens(
    ...
    intellect_home = tmp_path / "intellect"
    _setup_ontoweb_auth(intellect_home, refresh_token="refresh-old")
    monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))
    from agent.credential_pool import load_pool
    ...
    with pytest.raises(AuthError, match="Invalid refresh token"):
        auth_mod.resolve_ontoweb_access_token()
...
# =============================================================================
# _login_ontoweb: "Skip (keep current)" must preserve prior provider + model
# =============================================================================


class TestLoginNousSkipKeepsCurrent:
    """When a user runs `intellect model` → ONTOWEB Portal → Skip (keep current) after
    ...
    unconditionally after login, which flipped model.provider to "ontoweb" while
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        return intellect_home, config_path, auth_path

    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        """Patch OAuth + model-list + prompt so _login_ontoweb doesn't hit network."""
        import intellect_cli.auth as auth_mod
        import intellect_cli.models as models_mod
        import intellect_cli.ontoweb_subscription as ns

        fake_auth_state = {
            "access_token": "fake-ontoweb-token",
            "agent_key": "fake-agent-key",
            "inference_base_url": "https://inference-api.ontoweb.cn",
            "portal_base_url": "https://portal.ontoweb.cn",
        ...
        monkeypatch.setattr(models_mod, "check_ontoweb_free_tier", _check_nous_free_tier)
        monkeypatch.setattr(
            models_mod, "partition_ontoweb_models_by_tier",
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        """User picks Skip → config.yaml untouched, OntoWeb creds still saved."""
        import argparse
        import yaml
        from intellect_cli.auth import PROVIDER_REGISTRY, _login_ontoweb

        intellect_home, config_path, auth_path = self._setup_home_with_openrouter(
        ...
        _login_ontoweb(args, PROVIDER_REGISTRY["ontoweb"])
        ...
        # auth.json: active_provider restored to openrouter, but OntoWeb creds saved
        auth_after = json.loads(auth_path.read_text())
        assert auth_after["active_provider"] == "openrouter"
        assert "ontoweb" in auth_after["providers"]
        assert auth_after["providers"]["ontoweb"]["access_token"] == "fake-ontoweb-token"
    ...
    def test_picking_model_switches_to_nous(self, tmp_path, monkeypatch):
        """User picks a OntoWeb model → provider flips to ontoweb with that model."""
        import argparse
        import yaml
        from intellect_cli.auth import PROVIDER_REGISTRY, _login_ontoweb
    ...
    def test_skip_with_no_prior_active_provider_clears_it(self, tmp_path, monkeypatch):
        ...
        instead of leaving it as ontoweb."""
        import argparse
        import yaml
        from intellect_cli.auth import PROVIDER_REGISTRY, _login_ontoweb
        ...
        auth_path = intellect_home / "auth.json"
        auth_after = json.loads(auth_path.read_text())
        # active_provider should NOT be set to "ontoweb" after Skip
        assert auth_after.get("active_provider") in {None, ""}
        # But OntoWeb creds are still saved
        assert "ontoweb" in auth_after.get("providers", {})


# =============================================================================
# persist_ontoweb_credentials: shared helper for CLI + web dashboard login paths
# =============================================================================


def _full_state_fixture() -> dict:
    """Shape of the dict returned by _ontoweb_device_code_login /
    refresh_ontoweb_oauth_from_state. Used as helper input."""
    token = _invoke_jwt(seconds=3600)
    expires_at = _future_iso(3600)
    return {
        "portal_base_url": "https://portal.example.com",
...
def test_persist_ontoweb_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    """Helper must populate BOTH credential_pool.ontoweb AND providers.ontoweb.

    Regression guard: before this helper existed, `intellect auth add ontoweb`
    wrote only the pool. After the OntoWeb agent_key's 24h TTL expired, the
    401-recovery path in run_agent.py called resolve_ontoweb_runtime_credentials
    which reads providers.ontoweb, found it empty, raised AuthError, and the
    agent failed with "Non-retryable client error". Both stores must stay
    in sync at write time.
    """
    from intellect_cli.auth import persist_ontoweb_credentials, ONTOWEB_DEVICE_CODE_SOURCE
    ...
    entry = persist_ontoweb_credentials(state)
    ...
    # credential_pool.ontoweb has exactly one canonical device_code entry
    pool_entries = payload["credential_pool"]["ontoweb"]
    assert len(pool_entries) == 1, pool_entries
    pool_entry = pool_entries[0]
    assert pool_entry["source"] == ONTOWEB_DEVICE_CODE_SOURCE
    assert pool_entry["agent_key"] == state["agent_key"]
    assert pool_entry["inference_base_url"] == "https://inference.example.com/v1"


def test_persist_ontoweb_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    """End-to-end: after persisting via the helper, resolve_ontoweb_runtime_credentials
    must succeed (not raise "Intellect is not logged into ONTOWEB Portal").

    This is the exact path that run_agent.py's `_try_refresh_ontoweb_client_credentials`
    calls after a OntoWeb 401 — before the fix it would raise AuthError because
    providers.ontoweb was empty.
    """
    from intellect_cli.auth import (
        persist_ontoweb_credentials,
        resolve_ontoweb_runtime_credentials,
    )
    ...
    # doesn't raise "Intellect is not logged into ONTOWEB Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_ontoweb_credentials_idempotent_no_duplicate_pool_entries(tmp_path, monkeypatch):
    ...
    ``load_pool()``. The helper now writes providers.ontoweb and lets seeding
    ...
    persist_ontoweb_credentials(first)
    ...
    assert payload["providers"]["ontoweb"]["agent_key"] == second_token

    # credential_pool.ontoweb has exactly one entry, carrying the latest agent_key
    pool_entries = payload["credential_pool"]["ontoweb"]
    assert len(pool_entries) == 1, pool_entries
    assert pool_entries[0]["source"] == ONTOWEB_DEVICE_CODE_SOURCE
...
def test_persist_ontoweb_credentials_reloads_pool_after_singleton_write(tmp_path, monkeypatch):
...
def test_persist_ontoweb_credentials_embeds_custom_label(tmp_path, monkeypatch):
    """User-supplied ``--label`` round-trips through providers.ontoweb and the pool.

    Previously `intellect auth add ontoweb --type oauth --label <name>` silently
    dropped the label because persist_ontoweb_credentials() ignored it and
    _seed_from_singletons always auto-derived via label_from_token().  The
    fix stashes the label inside providers.ontoweb so seeding prefers it.
    """
    from intellect_cli.auth import persist_ontoweb_credentials, ONTOWEB_DEVICE_CODE_SOURCE
    ...
    entry = persist_ontoweb_credentials(_full_state_fixture(), label="my-personal")
    assert entry is not None
    assert entry.source == ONTOWEB_DEVICE_CODE_SOURCE
...
def test_persist_ontoweb_credentials_custom_label_survives_reseed(tmp_path, monkeypatch):
    ...
    """
    from intellect_cli.auth import persist_ontoweb_credentials
    from agent.credential_pool import load_pool
    ...
    persist_ontoweb_credentials(_full_state_fixture(), label="work-acct")
...
def test_persist_ontoweb_credentials_no_label_uses_auto_derived(tmp_path, monkeypatch):
    ...
    entry = persist_ontoweb_credentials(_full_state_fixture())
    assert entry is not None
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    When the ONTOWEB Portal server returns ``invalid_grant`` with
    ``error_description`` containing "reuse detected", Intellect must surface an
    ...
    re-authenticate" string led users to report this as an Intellect persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from intellect_cli.auth import _refresh_access_token
    ...
    assert "intellect auth add ontoweb" in message.lower()
...
def test_refresh_token_reuse_error_code_is_terminal():
    """OntoWeb may return refresh_token_reused as the OAuth error code itself."""
    from intellect_cli import auth as auth_mod
    ...
    assert exc_info.value.relogin_required is True
    assert auth_mod._is_terminal_ontoweb_refresh_error(exc_info.value) is True


def test_refresh_token_exchange_sends_refresh_token_header():
    """OntoWeb refresh tokens must be sent in a header so sandbox proxies can
    ...
    assert client.kwargs["headers"]["x-ontoweb-refresh-token"] == "refresh-1"
...
# =============================================================================
# Shared OntoWeb token store — cross-profile persistence (Codex-style auto-import)
# =============================================================================


@pytest.fixture
def shared_store_env(tmp_path, monkeypatch):
    """Redirect intellect_SHARED_AUTH_DIR to a tmp_path.

    Required for every test that exercises the shared OntoWeb store — the
    ...
    monkeypatch.setenv("intellect_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without intellect_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.intellect/shared/`` across CI runs.
    """
    from intellect_cli.auth import _ontoweb_shared_store_path

    monkeypatch.delenv("intellect_SHARED_AUTH_DIR", raising=False)

    with pytest.raises(RuntimeError, match="shared OntoWeb auth store"):
        _ontoweb_shared_store_path()


def test_shared_store_honors_env_override(tmp_path, monkeypatch):
    """intellect_SHARED_AUTH_DIR must redirect the path."""
    from intellect_cli.auth import _ontoweb_shared_store_path, ONTOWEB_SHARED_STORE_FILENAME

    custom_dir = tmp_path / "custom_shared"
    monkeypatch.setenv("intellect_SHARED_AUTH_DIR", str(custom_dir))

    path = _ontoweb_shared_store_path()
    assert path == custom_dir / ONTOWEB_SHARED_STORE_FILENAME


def test_shared_store_read_missing_returns_none(shared_store_env):
    """Missing file → ``_read_shared_ontoweb_state()`` returns None."""
    from intellect_cli.auth import _read_shared_ontoweb_state

    assert _read_shared_ontoweb_state() is None


def test_shared_store_read_malformed_returns_none(shared_store_env):
    """Unreadable / non-JSON file → None, not an exception."""
    from intellect_cli.auth import _ontoweb_shared_store_path, _read_shared_ontoweb_state
...
def test_shared_store_write_and_read_roundtrip(shared_store_env):
    ...
    loaded = _read_shared_ontoweb_state()
...
def test_shared_store_write_skips_when_refresh_token_missing(shared_store_env):
    ...
    from intellect_cli.auth import _ontoweb_shared_store_path, _write_shared_ontoweb_state
    ...
    assert not _ontoweb_shared_store_path().is_file()


def test_persist_ontoweb_credentials_mirrors_to_shared_store(
    tmp_path, monkeypatch, shared_store_env,
):
    """persist_ontoweb_credentials must populate BOTH per-profile auth.json
    AND the shared store, so a future profile's `intellect auth add ontoweb
    ...
    intellect_home = tmp_path / "intellect"
    intellect_home.mkdir(parents=True, exist_ok=True)
    (intellect_home / "auth.json").write_text(
    ...
    assert str(_ontoweb_shared_store_path()).startswith(str(shared_store_env))
...
def test_try_import_shared_returns_none_when_store_missing(shared_store_env):
    ...
    from intellect_cli.auth import _try_import_shared_ontoweb_state

    assert _try_import_shared_ontoweb_state() is None
...
def test_try_import_shared_returns_none_on_refresh_failure(
    ...
    portal down), _try_import_shared_ontoweb_state must return None so the
    login flow falls back to a fresh device-code run.
    """
    from intellect_cli import auth as auth_mod

    # Seed the shared store
    auth_mod._write_shared_ontoweb_state(_full_state_fixture())
    ...
    monkeypatch.setattr(auth_mod, "refresh_ontoweb_oauth_from_state", _boom)

    assert auth_mod._try_import_shared_ontoweb_state() is None
    assert auth_mod._read_shared_ontoweb_state() is None
...
def test_try_import_shared_persists_rotated_token_when_jwt_validation_fails(
    ...
    shared_after = auth_mod._read_shared_ontoweb_state()
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    every field persist_ontoweb_credentials() needs.
    """
    from intellect_cli import auth as auth_mod
    ...
    monkeypatch.setattr(auth_mod, "refresh_ontoweb_oauth_from_state", _fake_refresh)

    result = auth_mod._try_import_shared_ontoweb_state()
    ...
    assert result["client_id"] == "intellect-cli"
...
def test_shared_store_survives_across_profile_switch(
    ...
    (different INTELLECT_HOME) sees the same shared state and can rehydrate
    ...
    monkeypatch.setenv("INTELLECT_HOME", str(profile_a))
    ...
    # persists — _read_shared_ontoweb_state() must still return the tokens.
    ...
    shared = auth_mod._read_shared_ontoweb_state()
    assert shared is not None
    assert shared["refresh_token"] == "refresh-tok"

    # And a successful rehydrate + persist lands ontoweb into profile B
...
def test_runtime_refresh_uses_newer_shared_token_before_local_stale_token(
    ...
    """A sibling profile may rotate the single-use OntoWeb refresh token.
    ...
    profile_state = auth_mod.get_provider_auth_state("ontoweb")
...
def test_managed_gateway_access_token_uses_newer_shared_token(
    ...
    assert auth_mod.resolve_ontoweb_access_token() == "shared-fresh-access"
