"""Tests for the `intellect proxy` subcommand and its upstream adapters."""
...
from intellect_cli.proxy.adapters import ADAPTERS, get_adapter
from intellect_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from intellect_cli.proxy.adapters.ontoweb_portal import OntowebPortalAdapter
from intellect_cli.proxy.adapters.xai import XAIGrokAdapter
...
def test_get_adapter_returns_instance():
    adapter = get_adapter("ontoweb")
    assert isinstance(adapter, OntowebPortalAdapter)
...
def test_get_adapter_case_insensitive():
    assert isinstance(get_adapter("ONTOWEB"), OntowebPortalAdapter)
    assert isinstance(get_adapter("  OntoWeb  "), OntowebPortalAdapter)
...
# ---------------------------------------------------------------------------
# OntowebPortalAdapter
# ---------------------------------------------------------------------------


def _write_auth_store(intellect_home: Path, nous_state: Dict[str, Any]) -> Path:
    """Write an auth.json with the given ontoweb state into a hermetic INTELLECT_HOME."""
    auth_path = intellect_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {"ontoweb": nous_state},
    }))
    return auth_path


def test_nous_adapter_metadata():
    adapter = OntowebPortalAdapter()
    assert adapter.name == "ontoweb"
    assert adapter.display_name == "ONTOWEB Portal"
...
def test_nous_adapter_not_authenticated_when_no_auth_file(tmp_path, monkeypatch):
    # INTELLECT_HOME is already set by conftest, but make doubly sure
...
def test_nous_adapter_not_authenticated_when_provider_missing(tmp_path, monkeypatch):
    ...
    assert not OntowebPortalAdapter().is_authenticated()
...
def test_nous_adapter_authenticated_with_agent_key(tmp_path, monkeypatch):
    ...
    _write_auth_store(tmp_path, {
        ...
        "inference_base_url": "https://inference-api.ontoweb.cn/v1",
    })
    assert OntowebPortalAdapter().is_authenticated()
...
def test_nous_adapter_get_credential_uses_runtime_resolver(tmp_path, monkeypatch):
    ...
    with patch(
        "intellect_cli.proxy.adapters.ontoweb_portal.resolve_ontoweb_runtime_credentials",
    ...
    assert cred.base_url == "https://inference-api.ontoweb.cn/v1"
...
def test_nous_adapter_get_credential_raises_when_not_logged_in(tmp_path, monkeypatch):
    monkeypatch.setenv("INTELLECT_HOME", str(tmp_path))
    adapter = OntowebPortalAdapter()
    with pytest.raises(RuntimeError, match="intellect auth add ontoweb"):
...
def test_nous_adapter_quarantines_terminal_refresh_failure(tmp_path, monkeypatch):
    ...
    assert stored.get("credential_pool", {}).get("ontoweb") == []
...
def _write_xai_pool_entry(
    ...
    *,
    ...
    """Write an xai-oauth pool entry into a hermetic INTELLECT_HOME."""
    auth_path = intellect_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {},
...
def test_xai_adapter_retry_refreshes_current_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("intellect_cli.auth.refresh_xai_oauth_pure", fake_refresh)
...
def test_xai_adapter_retry_rotates_pool_entry_on_429(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("intellect_cli.auth.refresh_xai_oauth_pure", _refresh_must_not_run)
...
def test_server_forwards_chat_completions():
    async def run():
        ...
        try:
            ...
            assert "Intellect-4-70B" in req["body"]
        finally:
...
def test_cmd_proxy_status_runs(capsys, tmp_path, monkeypatch):
    monkeypatch.setenv("INTELLECT_HOME", str(tmp_path))
    from intellect_cli.proxy.cli import cmd_proxy_status

    args = MagicMock()
    rc = cmd_proxy_status(args)
    assert rc == 0
    out = capsys.readouterr().out
    assert "ontoweb" in out
    assert "ONTOWEB Portal" in out
    assert "not logged in" in out


def test_cmd_proxy_providers_runs(capsys):
    from intellect_cli.proxy.cli import cmd_proxy_list_providers
...
def test_cmd_proxy_start_refuses_unknown_provider(capsys):
    from intellect_cli.proxy.cli import cmd_proxy_start
...
def test_cmd_proxy_start_refuses_when_unauthenticated(capsys, tmp_path, monkeypatch):
    ...
    args = MagicMock()
    args.provider = "ontoweb"
    args.host = None
    args.port = None
    rc = cmd_proxy_start(args)
    assert rc == 2
    err = capsys.readouterr().err
    assert "intellect auth add ontoweb" in err
