"""Tests for intellect_cli/completion.py — shell completion script generation."""
...
from intellect_cli.completion import _walk, generate_bash, generate_zsh, generate_fish
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "_intellect_completion()" in out
        assert "complete -F _intellect_completion intellect" in out
...
class TestGenerateZsh:
    def test_contains_compdef_header(self):
        ...
        assert "#compdef intellect" in out
    ...
    def test_registers_compdef_instead_of_invoking_completion_function(self):
        out = generate_zsh(_make_parser())
        assert 'compdef _intellect intellect' in out
        assert '_intellect "$@"' not in out
        ...
        try:
            result = subprocess.run(
                [
                    "zsh",
                    "-fc",
                    f"autoload -Uz compinit && compinit -D; source {path}; [[ ${{_comps[intellect]}} == _intellect ]]",
                ],
                capture_output=True,
                text=True,
            )
...
class TestGenerateFish:
    def test_disables_file_completion(self):
        ...
        assert "complete -c intellect -f" in out
...
class TestProfileCompletion:
    ...
    def test_fish_has_profiles_helper(self):
        out = generate_fish(_make_parser())
        assert "__intellect_profiles" in out
        assert "$HOME/.intellect/profiles" in out
    ...
    def test_fish_profile_actions_complete_names(self):
        ...
        assert "__intellect_profiles" in out
        count = out.count("(__intellect_profiles)")
