...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.run_intellect_oauth_login_pure",
        lambda: {
            "access_token": token,
    ...
    assert entry["source"] == "manual:intellect_pkce"
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    monkeypatch.setenv("INTELLECT_HOME", str(tmp_path / "intellect"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    token = _jwt_with_email("ontoweb@example.com")
    monkeypatch.setattr(
        "intellect_cli.auth._ontoweb_device_code_login",
        lambda **kwargs: {
    ...
    entries = payload["credential_pool"]["ontoweb"]
    device_code_entries = [
    ...
    # `intellect auth add ontoweb` must also populate providers.ontoweb so the
    # 401-recovery path (resolve_ontoweb_runtime_credentials) can refresh an
    # invoke JWT when the token expires. If this mirror is missing, recovery
    # raises "Intellect is not logged into ONTOWEB Portal" and the agent dies.
...
def test_auth_add_minimax_oauth_starts_login_and_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "intellect_cli.auth._minimax_oauth_login",
        lambda **kwargs: {
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`intellect auth add ontoweb --type oauth --label <name>` must preserve the
    custom label end-to-end — it was silently dropped in the first cut of the
    persist_ontoweb_credentials helper because `--label` wasn't threaded through.
    """
    monkeypatch.setenv("INTELLECT_HOME", str(tmp_path / "intellect"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    token = _jwt_with_email("ontoweb@example.com")
    ...
    assert pool_entry["label"] == "my-ontoweb"

    # … and survives in providers.ontoweb so a subsequent load_pool() re-seeds
    # it without reverting to the auto-derived fingerprint.
    assert payload["providers"]["ontoweb"]["label"] == "my-ontoweb"
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "intellect_cli.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from intellect_cli.auth_commands import auth_reset_command
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`intellect logout --provider openai-codex` must still clear model.provider.
...
def test_logout_defaults_to_configured_codex_when_no_active_provider(tmp_path, monkeypatch, capsys):
    """Bare `intellect logout` should target configured Codex if auth has no active provider."""
    intellect_home = tmp_path / "intellect"
    monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}, "credential_pool": {}})
    (intellect_home / "config.yaml").write_text(
...
def test_reset_config_provider_uses_atomic_yaml_write(tmp_path, monkeypatch):
    ...
    from intellect_cli.auth import _reset_config_provider
    ...
    with patch("intellect_cli.auth.atomic_yaml_write", side_effect=_boom) as mock_write:
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from intellect_cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "intellect_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("intellect_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("intellect_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("intellect_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "intellect_cli.auth_commands.time.time",
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from intellect_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
def test_unsuppress_credential_source_returns_false_when_absent(tmp_path, monkeypatch):
    ...
    from intellect_cli.auth import unsuppress_credential_source
...
def test_auth_add_codex_clears_suppression_marker(tmp_path, monkeypatch):
    """Re-linking codex via `intellect auth add openai-codex` must clear any suppression marker."""
    monkeypatch.setenv("INTELLECT_HOME", str(tmp_path / "intellect"))
    monkeypatch.setattr(
        "agent.credential_pool._seed_from_singletons",
        lambda provider, entries: (False, set()),
    ...
    # Pre-existing suppression (simulating a prior `intellect auth remove`)
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("intellect_cli.auth._import_codex_cli_tokens", _fake_import)
...
def test_auth_remove_env_seeded_suppresses_shell_exported_var(tmp_path, monkeypatch, capsys):
    """`intellect auth remove xai 1` must stick even when the env var is exported
    by the shell (not written into ~/.intellect/.env).  Before PR for #13371 the
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    """When the env var lives only in ~/.intellect/.env (not the shell), the
    ...
    (intellect_home / ".env").write_text("DEEPSEEK_API_KEY=sk-ds-only\n")
    ...
    assert (intellect_home / ".env").read_text().strip() == ""


def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    """Re-adding a credential via `intellect auth add <provider>` clears any
    ...
    from types import SimpleNamespace
    from intellect_cli.auth import is_source_suppressed
    from intellect_cli.auth_commands import auth_add_command
...
def test_seed_from_env_respects_env_suppression(tmp_path, monkeypatch):
    ...
    via `intellect auth remove`.  This is the gate that prevents shell-exported
...
# =============================================================================
# Unified credential-source stickiness — every source Intellect reads from has a
...
def test_seed_from_singletons_respects_nous_suppression(tmp_path, monkeypatch):
    """ontoweb device_code must not re-seed from auth.json when suppressed."""
    intellect_home = tmp_path / "intellect"
    intellect_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))
    ...
    (intellect_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {"ontoweb": {"access_token": "tok", "refresh_token": "r", "expires_at": 9999999999}},
        "suppressed_sources": {"ontoweb": ["device_code"]},
    }))

    from agent.credential_pool import _seed_from_singletons
    entries = []
    changed, active = _seed_from_singletons("ontoweb", entries)
    assert changed is False
    assert entries == []
    assert active == set()
...
def test_seed_from_singletons_respects_copilot_suppression(tmp_path, monkeypatch):
    ...
    import intellect_cli.copilot_auth as ca
...
def test_seed_from_singletons_respects_qwen_suppression(tmp_path, monkeypatch):
    ...
    import intellect_cli.auth as ha
...
def test_seed_from_singletons_respects_intellect_pkce_suppression(tmp_path, monkeypatch):
    """anthropic intellect_pkce must not re-seed from ~/.intellect/.anthropic_oauth.json when suppressed."""
    intellect_home = tmp_path / "intellect"
    intellect_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))

    import yaml
    (intellect_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (intellect_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["intellect_pkce"]},
    }))

    # Stub the readers so only intellect_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_intellect_oauth_credentials", lambda: {
    ...
    # intellect_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "intellect_pkce" not in active
...
def test_seed_custom_pool_respects_config_suppression(tmp_path, monkeypatch):
    ...
    import yaml
    (intellect_home / "config.yaml").write_text(yaml.dump({
        "model": {},
        "custom_providers": [
...
def test_credential_sources_registry_has_expected_steps():
    ...
    required = {
        ...
        "~/.intellect/.anthropic_oauth.json",
        "auth.json providers.ontoweb",
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    with patch(
        "intellect_cli.copilot_auth.resolve_copilot_token",
...
def test_auth_add_clears_all_suppressions_including_non_env(tmp_path, monkeypatch):
    """Re-adding a credential via `intellect auth add <provider>` clears ALL
...
def test_auth_remove_codex_manual_device_code_suppresses_canonical(tmp_path, monkeypatch):
    """Removing a manual:device_code entry (from `intellect auth add openai-codex`)
