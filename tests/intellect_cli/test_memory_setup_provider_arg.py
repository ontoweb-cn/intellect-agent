"""Tests for `intellect memory setup [provider]` routing.
...
``intellect memory setup honcho``) without the interactive picker — which matters
because the per-provider ``intellect <provider>`` subcommand is only registered
...
from intellect_cli import memory_setup
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "intellect memory setup" in out
