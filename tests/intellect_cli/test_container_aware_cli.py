...
def container_env(tmp_path, monkeypatch):
    """Set up a fake INTELLECT_HOME with .container-mode file."""
    intellect_home = tmp_path / ".intellect"
    intellect_home.mkdir()
    monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))
    monkeypatch.delenv("intellect_DEV", raising=False)

    container_mode = intellect_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=intellect-agent\n"
        "exec_user=intellect\n"
        "intellect_bin=/data/current-package/bin/intellect\n"
    )
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("intellect_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "intellect-agent"
    assert info["exec_user"] == "intellect"
    assert info["intellect_bin"] == "/data/current-package/bin/intellect"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("intellect_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_intellect_dev(container_env, monkeypatch):
    """Returns None when intellect_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("intellect_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_intellect_dev_zero(container_env, monkeypatch):
    """intellect_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("intellect_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        intellect_home = Path(tmpdir) / ".intellect"
        intellect_home.mkdir()
        (intellect_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("intellect_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_intellect_home": lambda: intellect_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("intellect_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=intellect-custom\n"
        "exec_user=myuser\n"
        "intellect_bin=/opt/intellect/bin/intellect\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "intellect-custom"
    assert info["exec_user"] == "myuser"
    assert info["intellect_bin"] == "/opt/intellect/bin/intellect"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from intellect_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "intellect"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "intellect-agent" in cmd
    assert "/data/current-package/bin/intellect" in cmd
    assert "chat" in cmd
