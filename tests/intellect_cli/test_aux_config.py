"""Tests for the auxiliary-model configuration UI in ``intellect model``.
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"provider": "ontoweb", "model": "gemini-3-flash"}, "ontoweb · gemini-3-flash"),
        (
...
def test_save_aux_choice_preserves_timeout(tmp_path, monkeypatch):
    ...
    _save_aux_choice("vision", provider="ontoweb", model="gemini-3-flash")
...
def test_save_aux_choice_creates_missing_task_entry(tmp_path, monkeypatch):
    ...
    _save_aux_choice("vision", provider="ontoweb", model="gemini-3-flash")
    cfg = load_config()
    assert cfg["auxiliary"]["vision"]["provider"] == "ontoweb"
...
def test_reset_aux_to_auto_clears_routing_preserves_timeouts(tmp_path, monkeypatch):
    ...
    _save_aux_choice("compression", provider="ontoweb", model="gemini-3-flash")
    from intellect_cli.config import save_config
