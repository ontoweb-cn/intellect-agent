...
def test_pip_install_detected_when_no_git_dir(tmp_path):
    ...
    with patch("intellect_cli.config.get_managed_system", return_value=None), \
         patch("intellect_cli.config.get_intellect_home", return_value=tmp_path):
...
def test_managed_install_takes_precedence(tmp_path):
    """When intellect_MANAGED is set, that takes precedence over git detection."""
    (tmp_path / ".git").mkdir()
    with patch("intellect_cli.config.get_managed_system", return_value="NixOS"), \
         patch("intellect_cli.config.get_intellect_home", return_value=tmp_path):
        from intellect_cli.config import detect_install_method
        method = detect_install_method(project_root=tmp_path)
        assert method == "nixos"
...
def test_recommended_update_command_pip():
    ...
    from intellect_cli.config import recommended_update_command_for_method
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    image, so ``intellect update`` refused to run. With a ``.git`` checkout it
    must resolve to ``git``.
    """
    (tmp_path / ".git").mkdir()
    with patch("intellect_cli.config.get_managed_system", return_value=None), \
         patch("intellect_cli.config.get_intellect_home", return_value=tmp_path), \
         patch("intellect_constants.is_container", return_value=True):
        from intellect_cli.config import detect_install_method
...
def test_banner_warns_on_pip_install(tmp_path):
    ...
    from intellect_cli import banner

    hh = tmp_path / ".intellect"
    hh.mkdir()
    (hh / ".install_method").write_text("pip\n")

    with patch("intellect_cli.config.get_intellect_home", return_value=hh), \
         patch("intellect_constants.get_intellect_home", return_value=hh):
        buf = io.StringIO()
