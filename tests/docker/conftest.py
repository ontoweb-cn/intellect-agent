...
Override the image with ``INTELLECT_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("INTELLECT_TEST_IMAGE", "intellect-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``INTELLECT_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("INTELLECT_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged intellect user
# ---------------------------------------------------------------------------
#
# Background: every Intellect runtime path inside the container drops to UID
# 10000 (the ``intellect`` user) via ``s6-setuidgid intellect``. ``docker exec``
...
# fails (PermissionError swallowed) for intellect, so a test that ran as root
...
# helpers below run every probe under ``-u intellect`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: intellect).
