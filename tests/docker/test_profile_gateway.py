...
Phase 4 wires `intellect -p <profile> gateway start/stop` through the s6
...
def test_profile_create_then_gateway_start(
    ...
    r = _sh(container_name, f"intellect -p {PROFILE} gateway start", timeout=60)
    ...
    r = _sh(container_name, f"intellect -p {PROFILE} gateway stop", timeout=30)
    assert r.returncode == 0
...
def test_profile_delete_stops_gateway(
    ...
    r = _sh(
        container_name,
        f"intellect profile delete {PROFILE} --yes",
        timeout=30,
    )
