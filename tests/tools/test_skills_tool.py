...
class TestParseFrontmatter:
    ...
    def test_nested_yaml(self):
        ...
        fm, body = _parse_frontmatter(content)
        assert fm["metadata"]["intellect"]["tags"] == ["a", "b"]
    ...
    def test_skill_view_applies_template_vars(self, tmp_path):
        with (
            ...
            skill_dir = _make_skill(
                tmp_path,
                "templated",
                body="Run ${intellect_SKILL_DIR}/scripts/do.sh in ${intellect_SESSION_ID}",
            )
        ...
        assert "${intellect_SKILL_DIR}" not in result["content"]
