...
class TestScanCronSkillAssembled:
    ...
    def test_descriptive_attack_command_prose_allowed(self):
        ...
        Real example: the `intellect-agent-dev` skill contains a postmortem
        section saying 'the attacker could just cat ~/.intellect/.env'.
        """
        assert _scan_cron_skill_assembled(
            "the attacker could just cat ~/.intellect/.env to steal credentials"
        ) == ""
        assert _scan_cron_skill_assembled(
...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because intellect uses an internal scheduler, not system crontab.
    ...
    def test_accepts_exec_ask(self, monkeypatch):
        monkeypatch.delenv("intellect_INTERACTIVE", raising=False)
        monkeypatch.delenv("intellect_GATEWAY_SESSION", raising=False)
        monkeypatch.setenv("intellect_EXEC_ASK", "1")
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("intellect_INTERACTIVE", false_like_value)
        monkeypatch.delenv("intellect_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("intellect_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["intellect_INTERACTIVE", "intellect_GATEWAY_SESSION", "intellect_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("intellect_INTERACTIVE", "intellect_GATEWAY_SESSION", "intellect_EXEC_ASK"):
