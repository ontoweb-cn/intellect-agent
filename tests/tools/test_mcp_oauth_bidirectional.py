"""Regression test for the ``intellectMCPOAuthProvider.async_auth_flow`` bidirectional
generator bridge.
...
@pytest.mark.asyncio
async def test_intellect_provider_forwards_asend_values(tmp_path, monkeypatch):
    ...
    from tools.mcp_oauth_manager import _intellect_PROVIDER_CLS, reset_manager_for_tests

    assert _intellect_PROVIDER_CLS is not None, "SDK OAuth types must be available"
    ...
    storage = intellectTokenStorage("srv")
    ...
    metadata = OAuthClientMetadata(
        redirect_uris=[AnyUrl("http://127.0.0.1:12345/callback")],
        client_name="Intellect Agent",
    )
    provider = _intellect_PROVIDER_CLS(
        server_name="srv",
...
@pytest.mark.asyncio
async def test_intellect_provider_forwards_401_triggers_refresh(tmp_path, monkeypatch):
