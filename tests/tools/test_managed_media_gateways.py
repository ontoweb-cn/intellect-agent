...
@pytest.fixture(autouse=True)
def _enable_managed_ontoweb_tools(monkeypatch):
    """Patch the source modules so managed_ontoweb_tools_enabled() returns True
...
def test_managed_fal_submit_uses_gateway_origin_and_nous_token(monkeypatch):
    ...
    monkeypatch.setenv("TOOL_GATEWAY_USER_TOKEN", "ontoweb-token")
    ...
    assert captured["client_key"] == "ontoweb-token"
...
def test_openai_tts_uses_managed_audio_gateway_when_direct_key_absent(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("TOOL_GATEWAY_DOMAIN", "ontoweb.cn")
    monkeypatch.setenv("TOOL_GATEWAY_USER_TOKEN", "ontoweb-token")
    ...
    assert captured["api_key"] == "ontoweb-token"
    assert captured["base_url"] == "https://openai-audio-gateway.ontoweb.cn/v1"
...
def test_transcription_uses_model_specific_response_formats(monkeypatch, tmp_path):
    ...
    assert whisper_capture["base_url"] == "https://openai-audio-gateway.ontoweb.cn/v1"
...
def test_video_gen_managed_fal_submit_uses_gateway(monkeypatch):
    ...
    monkeypatch.delenv("FAL_KEY", raising=False)
    monkeypatch.setenv("FAL_QUEUE_GATEWAY_URL", "http://127.0.0.1:3009")
    monkeypatch.setenv("TOOL_GATEWAY_USER_TOKEN", "ontoweb-video-token")
    ...
    assert captured["submit_via"] == "managed_client"
    assert captured["client_key"] == "ontoweb-video-token"
