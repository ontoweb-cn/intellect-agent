...
def test_intellect_provider_subclass_exists():
    """intellectMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _intellect_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _intellect_PROVIDER_CLS is not None
    assert issubclass(_intellect_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_intellect_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns intellectMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _intellect_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("INTELLECT_HOME", str(tmp_path))
    ...
    assert _intellect_PROVIDER_CLS is not None
    assert isinstance(provider, _intellect_PROVIDER_CLS)
    assert provider._intellect_server_name == "srv"
