...
class TestGptQualityPinnedToMedium:
    ...
    and cannot be overridden via config. Pinning keeps ONTOWEB Portal billing
...
class TestModelResolution:
    ...
    def test_no_config_falls_back_to_default(self, image_tool):
        with patch("intellect_cli.config.load_config", return_value={}):
...
class TestManagedGatewayErrorTranslation:
    """4xx from the OntoWeb managed gateway should be translated to a user-actionable message."""

    def test_4xx_translates_to_value_error_with_remediation(self, image_tool, monkeypatch):
        """403 from managed gateway → ValueError mentioning FAL_KEY + intellect tools."""
        ...
        managed_gateway.ontoweb_user_token = "test-token"
