...
Tests cover:
- managed_ontoweb_tools_enabled() subscription-based gate
...
class TestManagedNousToolsEnabled:
    """Subscription-based gate: True for paid OntoWeb subscribers."""

    def test_disabled_when_not_logged_in(self, monkeypatch):
        monkeypatch.setattr(
            "intellect_cli.ontoweb_account.get_ontoweb_portal_account_info",
            lambda: OntowebPortalAccountInfo(logged_in=False, source="none", fresh=False),
        )
        assert managed_ontoweb_tools_enabled() is False
    ...
    def test_enabled_for_paid_subscriber(self, monkeypatch):
        ...
        assert managed_ontoweb_tools_enabled() is True
    ...
    def test_force_fresh_is_forwarded(self, monkeypatch):
        ...
        assert managed_ontoweb_tools_enabled(force_fresh=True) is True
        assert calls == [True]
...
class TestNousToolGatewayUnavailableMessage:
    def test_uses_entitlement_reason_for_logged_in_user(self, monkeypatch):
        monkeypatch.setattr(
            "intellect_cli.ontoweb_account.get_ontoweb_portal_account_info",
            lambda force_fresh=False: OntowebPortalAccountInfo(
                logged_in=True,
                source="account_api",
                fresh=True,
                paid_service_access=False,
        ...
        message = ontoweb_tool_gateway_unavailable_message("managed image generation")
...
class TestResolveModalBackendState:
    ...
    def _resolve(monkeypatch, mode, *, has_direct, managed_ready, nous_enabled=False):
        ...
        monkeypatch.setattr(
            "tools.tool_backend_helpers.managed_ontoweb_tools_enabled",
            lambda: nous_enabled,
        )
