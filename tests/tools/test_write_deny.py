...
class TestWriteDenyExactPaths:
    ...
    def test_intellect_env(self):
        # ``.env`` under the active INTELLECT_HOME (profile-aware, not just
        # ``~/.intellect``) must be write-denied. The hermetic test conftest
        # points INTELLECT_HOME at a tempdir — resolve via get_intellect_home()
        # to match the denylist.
        from intellect_constants import get_intellect_home
        path = str(get_intellect_home() / ".env")
        assert _is_write_denied(path) is True

    def test_intellect_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.intellect/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "intellect_root"
        ...
        # Sanity check: INTELLECT_HOME does point to the profile dir, not the root.
        from intellect_constants import get_intellect_home, get_default_intellect_root
        assert get_intellect_home() == profile_home
        assert get_default_intellect_root() == root
...
class TestWriteAllowed:
    ...
    def test_intellect_config_not_env(self):
        path = os.path.join(str(Path.home()), ".intellect", "config.yaml")
