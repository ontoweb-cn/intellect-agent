...
def _restore_tool_modules():
    original_intellect_home = os.environ.get("INTELLECT_HOME")
    original_modules = {
        name: module
        for name, module in sys.modules.items()
        if name == "tools"
        or name.startswith("tools.")
        or name == "intellect_cli"
        or name.startswith("intellect_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_intellect_home is None:
            os.environ.pop("INTELLECT_HOME", None)
        else:
            os.environ["INTELLECT_HOME"] = original_intellect_home
        _reset_modules(("tools", "intellect_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    intellect_cli = types.ModuleType("intellect_cli")
    intellect_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["intellect_cli"] = intellect_cli
    intellect_home = tmp_path / "intellect-home"
    os.environ["INTELLECT_HOME"] = str(intellect_home)
    sys.modules["intellect_cli.config"] = types.SimpleNamespace(
        get_intellect_home=lambda: intellect_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="intellect-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": intellect_home / "modal_snapshots.json",
