...
def test_non_persistent_cleanup_removes_container(monkeypatch):
    ...
    docker rm so containers don't leak across intellect processes.
...
def _make_execute_only_env(forward_env=None):
    ...
    env._snapshot_path = "/tmp/intellect-snap-test123.sh"
    env._cwd_file = "/tmp/intellect-cwd-test123.txt"
    env._cwd_marker = "__intellect_CWD_test123__"
...
def test_init_env_args_uses_intellect_dotenv_for_allowlisted_env(monkeypatch):
    """_build_init_env_args picks up forwarded env vars from .env file at init time."""
    # Use a var that is NOT in _intellect_PROVIDER_ENV_BLOCKLIST (GITHUB_TOKEN
    # is in the copilot provider's api_key_env_vars and gets stripped).
    env = _make_execute_only_env(["DATABASE_URL"])

    monkeypatch.delenv("DATABASE_URL", raising=False)
    monkeypatch.setattr(docker_env, "_load_intellect_env_vars", lambda: {"DATABASE_URL": "value_from_dotenv"})
...
def test_init_env_args_prefers_shell_env_over_intellect_dotenv(monkeypatch):
...
def test_forward_env_overrides_docker_env_in_init_args(monkeypatch):
    ...
    monkeypatch.setattr(docker_env, "_load_intellect_env_vars", lambda: {})
...
def test_security_args_include_setuid_setgid_for_privdrop(monkeypatch):
    ...
    (e.g. via ``s6-setuidgid`` in the bundled Intellect image, or ``gosu``/``su``
...
def test_run_command_tags_intellect_agent_label(monkeypatch):
    """Every container intellect-agent starts must carry the intellect-agent=1 label
    so the orphan reaper (and external operators) can identify them with a
    single ``docker ps --filter label=intellect-agent=1`` call. Regression test
    ...
    assert "intellect-agent=1" in labels, (
        f"intellect-agent=1 label missing; got labels: {sorted(labels)}"
    )
...
def test_run_command_tags_task_and_profile_labels(monkeypatch):
    ...
    returns ``"default"`` for the root Intellect home."""
    monkeypatch.setattr(docker_env, "find_docker", lambda: "/usr/bin/docker")
    ...
    labels = _labels_in_run_args(_run_args_from_calls(calls))
    assert "intellect-task-id=kanban-42" in labels, (
        f"intellect-task-id=kanban-42 missing; got: {sorted(labels)}"
    )
    assert "intellect-profile=research-bot" in labels, (
        f"intellect-profile=research-bot missing; got: {sorted(labels)}"
    )
...
def test_run_command_sanitizes_unsafe_task_id(monkeypatch):
    ...
    assert "intellect-task-id=task_with_weird_chars" in labels, (
...
def test_labels_attribute_populated_after_init(monkeypatch):
    ...
    assert env._labels == {
        "intellect-agent": "1",
        "intellect-task-id": "abc",
        "intellect-profile": "default",
    }
...
def test_reuse_attaches_to_running_container_without_docker_run(monkeypatch):
    ...
    issue #20561 root cause: every Intellect process spawning a new container
...
def test_reuse_starts_stopped_container_before_attaching(monkeypatch):
    ...
    ``docker start`` before the new Intellect process uses it. Without this
...
def test_cleanup_with_persist_is_noop_for_container(monkeypatch):
    ...
    Intellect startup, not on graceful exit. Issue #20561 — the first iteration
...
def test_reap_orphan_removes_stale_exited_container(monkeypatch):
    ...
    SIGKILL'd Intellect processes leak containers permanently."""
...
def test_reap_orphan_spares_recently_exited_container(monkeypatch):
    ...
    container belongs to a Intellect process that just finished and may be
...
def test_reap_orphan_scopes_to_profile_filter_via_label(monkeypatch):
    """The reaper must pass ``--filter label=intellect-profile=<profile>`` to
    ...
    assert "label=intellect-profile=research-bot" in flat, (
        f"profile filter not applied to docker ps; got args: {ps_calls[0][0]}"
    )
    assert "label=intellect-agent=1" in flat, (
        f"intellect-agent label filter must also be applied; got: {ps_calls[0][0]}"
    )
    assert "status=exited" in flat, (
        "must filter to exited containers only — running containers may "
        "belong to a sibling Intellect process and must NEVER be reaped"
    )
...
def test_credential_mount_skipped_when_source_is_directory(monkeypatch, tmp_path, caplog):
    ...
    fake_mounts = [
        {"host_path": str(corrupted_dir), "container_path": "/root/.intellect/google_token.json"},
    ]
    monkeypatch.setattr(
...
def test_credential_mount_skipped_when_source_missing(monkeypatch, tmp_path, caplog):
    ...
    fake_mounts = [
        {"host_path": str(missing_path), "container_path": "/root/.intellect/deleted_token.json"},
    ]
    monkeypatch.setattr(
        "tools.credential_files.get_credential_file_mounts",
        lambda: fake_mounts,
...
def test_credential_mount_works_when_source_is_valid_file(monkeypatch, tmp_path):
    ...
    fake_mounts = [
        {"host_path": str(valid_file), "container_path": "/root/.intellect/token.json"},
    ]
    monkeypatch.setattr(
        "tools.credential_files.get_credential_file_mounts",
        lambda: fake_mounts,
