...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("intellect_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("intellect_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"INTELLECT_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_intellect_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.intellect/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_custom_intellect_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $INTELLECT_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_intellect_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$INTELLECT_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_intellect_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $INTELLECT_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m intellect_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "intellect_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart intellect-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "stop/restart" in desc

    def test_pkill_intellect_detected(self):
        """pkill targeting intellect/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_intellect_detected(self):
        cmd = "killall intellect"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep intellect) bypasses the pkill/killall name-matching
    ...
    def test_pkill_intellect_still_detected(self):
        """Existing pkill pattern must not regress."""
        cmd = "pkill -9 intellect"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("intellect_GATEWAY_SESSION", "intellect_YOLO_MODE",
                      "INTELLECT_SESSION_KEY", "intellect_INTERACTIVE")
        }
        os.environ.pop("intellect_YOLO_MODE", None)
        os.environ.pop("intellect_INTERACTIVE", None)
        os.environ["intellect_GATEWAY_SESSION"] = "1"
        os.environ["INTELLECT_SESSION_KEY"] = self.SESSION_KEY
