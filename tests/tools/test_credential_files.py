...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        (intellect_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(intellect_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.intellect/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        intellect_home = tmp_path / ".intellect"
        intellect_home.mkdir()
        (intellect_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        intellect_home = tmp_path / ".intellect"
        intellect_home.mkdir()
        (intellect_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        intellect_home = tmp_path / ".intellect"
        intellect_home.mkdir()
        (intellect_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        intellect_home = tmp_path / ".intellect"
        skills_dir = intellect_home / "skills"
        ...
        assert mounts[0]["container_path"] == "/root/.intellect/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        intellect_home = tmp_path / ".intellect"
        (intellect_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"INTELLECT_HOME": str(intellect_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.intellect")

        assert mounts[0]["container_path"] == "/home/user/.intellect/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.intellect/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.intellect/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape INTELLECT_HOME."""
        monkeypatch.setenv("INTELLECT_HOME", str(tmp_path / ".intellect"))
        (tmp_path / ".intellect").mkdir()

        # Create a sensitive file one level above intellect_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside intellect_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside INTELLECT_HOME must still be registered."""
        intellect_home = tmp_path / ".intellect"
        intellect_home.mkdir()
        monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))
        (intellect_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_intellect_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of INTELLECT_HOME must be allowed."""
        intellect_home = tmp_path / ".intellect"
        intellect_home.mkdir()
        subdir = intellect_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside INTELLECT_HOME pointing outside must be rejected."""
        intellect_home = tmp_path / ".intellect"
        intellect_home.mkdir()
        monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))

        # Create a sensitive file outside intellect_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside intellect_home pointing outside
        symlink = intellect_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes INTELLECT_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, intellect_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape INTELLECT_HOME."""
        intellect_home = tmp_path / ".intellect"
        intellect_home.mkdir()
        monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))
        ...
        sensitive.write_text("{}")
        self._write_config(intellect_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(intellect_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside INTELLECT_HOME via config must still mount."""
        intellect_home = tmp_path / ".intellect"
        intellect_home.mkdir()
        monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))

        (intellect_home / "oauth.json").write_text("{}")
        self._write_config(intellect_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        intellect_home = tmp_path / ".intellect"
        intellect_home.mkdir()
        (intellect_home / "cache" / "documents").mkdir(parents=True)
        (intellect_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.intellect/cache/documents" in paths
        assert "/root/.intellect/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.intellect/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        intellect_home = tmp_path / ".intellect"
        intellect_home.mkdir()
        # Use legacy dir name — get_intellect_dir prefers old if it exists
        (intellect_home / "document_cache").mkdir()
        (intellect_home / "image_cache").mkdir()
        monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(intellect_home / "document_cache") in host_paths
        assert str(intellect_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.intellect/cache/documents" in container_paths
        assert "/root/.intellect/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        intellect_home = tmp_path / ".intellect"
        doc_dir = intellect_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        intellect_home = tmp_path / ".intellect"
        ss_dir = intellect_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.intellect/cache/screenshots/session_abc/screen1.png"
