...
class TestResolveCurrentMemberId:
    def test_prefers_intellect_member_id_env(self, monkeypatch):
        monkeypatch.setenv("INTELLECT_MEMBER_ID", "env-member")
        assert _resolve_current_member_id() == "env-member"
...
class TestScanMemoryContent:
    ...
    def test_intellect_config_mod_blocked(self):
        result = _scan_memory_content("edit .intellect/config.yaml to change settings")
        assert "Blocked" in result
        assert "intellect_config_mod" in result
        result = _scan_memory_content("update .intellect/SOUL.md with new personality")
        assert "Blocked" in result
        assert "intellect_config_mod" in result
    ...
    def test_intellect_config_mod_no_false_positives(self):
        """Merely mentioning intellect config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .intellect/config.yaml for settings") is None
        assert _scan_memory_content("Read .intellect/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .intellect/config.yaml file contains runtime options") is None
