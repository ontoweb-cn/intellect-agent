...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._intellect_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(intellect_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(intellect_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_intellect_bin(self):
        """After _INSTALL_FAILED, manual install in INTELLECT_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        intellect_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(intellect_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(intellect_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._intellect_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == intellect_bin
            assert _tirith_mod._resolved_path == intellect_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# INTELLECT_HOME isolation
# ---------------------------------------------------------------------------

class TestintellectHomeIsolation:
    def test_intellect_bin_dir_respects_intellect_home(self):
        """_intellect_bin_dir must use INTELLECT_HOME, not hardcoded ~/.intellect."""
        from tools.tirith_security import _intellect_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"INTELLECT_HOME": tmpdir}):
            result = _intellect_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_intellect_home(self):
        """_failure_marker_path must use INTELLECT_HOME, not hardcoded ~/.intellect."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"INTELLECT_HOME": "/custom/intellect"}):
            result = _failure_marker_path()
        assert result == "/custom/intellect/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets INTELLECT_HOME; verify it's active."""
        intellect_home = os.getenv("INTELLECT_HOME")
        assert intellect_home is not None, "INTELLECT_HOME should be set by conftest"
        assert "intellect_test" in intellect_home, "Should point to test temp dir"

    def test_get_intellect_home_fallback(self):
        """Without INTELLECT_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_intellect_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove INTELLECT_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".intellect")
            result = _get_intellect_home()
