...
def _clean_state():
    ...
    saved = {}
    for k in ("intellect_INTERACTIVE", "intellect_GATEWAY_SESSION", "intellect_EXEC_ASK", "intellect_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["intellect_INTERACTIVE"] = "1"
        session_key = os.getenv("INTELLECT_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No intellect_INTERACTIVE or intellect_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
