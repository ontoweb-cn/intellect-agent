...
# ---------------------------------------------------------------------------
# Toolset: discord / discord_admin only in intellect-discord
# ---------------------------------------------------------------------------

class TestToolsetInclusion:
    def test_discord_tools_in_intellect_discord_toolset(self):
        from toolsets import TOOLSETS
        assert "discord" in TOOLSETS["intellect-discord"]["tools"]
        assert "discord_admin" in TOOLSETS["intellect-discord"]["tools"]

    def test_discord_tools_not_in_core_tools(self):
        from toolsets import _INTELLECT_CORE_TOOLS
        assert "discord" not in _INTELLECT_CORE_TOOLS
        assert "discord_admin" not in _INTELLECT_CORE_TOOLS

    def test_discord_tools_not_in_other_toolsets(self):
        from toolsets import TOOLSETS
        for name, ts in TOOLSETS.items():
            if name in {"intellect-discord", "intellect-gateway", "discord", "discord_admin"}:
                continue
            tools = ts.get("tools", [])
...
class TestConfigAllowlist:
    ...
    def test_config_load_failure_is_permissive(self, monkeypatch):
        ...
        monkeypatch.setattr("intellect_cli.config.load_config", bad_load)
        assert _load_allowed_actions_config() is None
