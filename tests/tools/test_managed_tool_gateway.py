...
def test_resolve_managed_tool_gateway_derives_vendor_origin_from_shared_domain():
    with patch.dict(
        ...
        clear=False,
    ), patch.object(managed_tool_gateway, "managed_ontoweb_tools_enabled", return_value=True):
        result = resolve_managed_tool_gateway(
            "firecrawl",
            token_reader=lambda: "ontoweb-token",
        )

    assert result is not None
    assert result.gateway_origin == "https://firecrawl-gateway.ontoweb.cn"
    assert result.ontoweb_user_token == "ontoweb-token"
...
def test_resolve_managed_tool_gateway_is_disabled_without_subscription():
    with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "ontoweb.cn"}, clear=False), \
         patch.object(managed_tool_gateway, "managed_ontoweb_tools_enabled", return_value=False):
        result = resolve_managed_tool_gateway(
            "firecrawl",
            token_reader=lambda: "ontoweb-token",
        )
...
def test_read_ontoweb_access_token_refreshes_expiring_cached_token(tmp_path, monkeypatch):
    ...
    assert managed_tool_gateway.read_ontoweb_access_token() == "fresh-token"
