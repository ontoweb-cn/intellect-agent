...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad intellect_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: intellect_GATEWAY_SESSION set, a bound session
    key, and isolated gateway queues/callbacks. Yields the session_key."""
    monkeypatch.setenv("intellect_GATEWAY_SESSION", "1")
    monkeypatch.delenv("intellect_INTERACTIVE", raising=False)
    monkeypatch.delenv("intellect_CRON_SESSION", raising=False)
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("intellect_CRON_SESSION", "1")
    monkeypatch.delenv("intellect_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_intellect_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "INTELLECT_HOME": "/h", "intellect_PROFILE": "p",
        "intellect_CONFIG": "/c.yaml", "intellect_ENV": "/e",
        # other intellect_* → dropped (broad prefix removed)
        "intellect_BASE_URL": "https://x", "intellect_INTERACTIVE": "1",
        "intellect_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("INTELLECT_HOME", "intellect_PROFILE", "intellect_CONFIG", "intellect_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "intellect_BASE_URL", "intellect_INTERACTIVE", "intellect_KANBAN_DB",
...
def test_env_scrub_logs_dropped_intellect_vars(caplog):
    """Dropping a non-allowlisted, non-secret intellect_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset intellect_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "INTELLECT_HOME": "/h",          # allowlisted → kept, not logged
        "intellect_BASE_URL": "https://x",   # dropped → logged
        "intellect_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "intellect_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "INTELLECT_HOME" in out and "PATH" in out
    assert "intellect_BASE_URL" not in out and "intellect_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "intellect_BASE_URL" in msgs and "intellect_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "intellect_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped intellect_* vars."""
    import logging
