...
class TestRegistry:
    ...
    def test_alias_lookup(self):
        ...
        assert get_provider_profile("ontoweb-portal").name == "ontoweb"
...
class TestOpenRouterProfile:
    ...
    def test_reasoning_disabled_still_passes(self):
        """OpenRouter passes disabled reasoning through (unlike OntoWeb)."""
        p = get_provider_profile("openrouter")
        eb, _ = p.build_api_kwargs_extras(
...
class TestOntowebProfile:
    def test_tags(self):
        from agent.portal_tags import ontoweb_portal_tags
        p = get_provider_profile("ontoweb")
        body = p.build_extra_body()
