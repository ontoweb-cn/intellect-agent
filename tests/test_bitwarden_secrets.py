...
binary download are exercised manually by `intellect secrets bitwarden
...
def intellect_home(tmp_path, monkeypatch):
    """Point Intellect at an isolated home directory."""
    home = tmp_path / ".intellect"
    home.mkdir()
    monkeypatch.setenv("INTELLECT_HOME", str(home))
    # Some modules cache get_intellect_home; clear if needed.
    import intellect_constants
    if hasattr(intellect_constants, "_INTELLECT_HOME_CACHE"):
        intellect_constants._INTELLECT_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(intellect_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(intellect_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(intellect_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from intellect_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under intellect_home/cache/."""
    home = tmp_path / ".intellect"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
