"""Tests for intellect_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Intellect entry point
(intellect, intellect-agent, intellect-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Intellect entry point imports intellect_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported intellect_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("intellect_bootstrap", None)
    import intellect_bootstrap  # noqa: WPS433
    return intellect_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Intellect CLI
...
class TestEntryPointsImportBootstrap:
    """Every Intellect entry point must import intellect_bootstrap as its
    ...
    # Entry points that invoke Intellect as a process.  Each one must
    # import intellect_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "intellect_cli/main.py",   # intellect CLI (console_script)
        "run_agent.py",          # intellect-agent (console_script)
        "acp_adapter/entry.py",  # intellect-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import intellect_bootstrap' and that
        ...
        half-finished ``intellect update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``intellect_bootstrap`` as a top-level module) leaves intellect
        ...
        """
        # Resolve relative to the intellect-agent repo root.  Tests live
        # at tests/test_intellect_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # intellect start even when intellect_bootstrap hasn't been
        ...
        assert first_import_name == "intellect_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'intellect_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import intellect_bootstrap' line to be the first import."
        )
