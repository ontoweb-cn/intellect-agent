...
def test_observed_group_context_uses_shared_source_and_prompt_for_later_mentions():
    async def _run():
        ...
        text = "@intellect_bot what did Alice say?"
        msg = _group_message(
            text,
            from_user_id=222,
...
def test_observed_group_context_preserves_slash_command_text_for_dispatch():
    ...
    event = MessageEvent(
        ...
        raw_message=_group_message(
            "/new@intellect_bot",
            entities=[_bot_command_entity("/new@intellect_bot", "/new@intellect_bot")],
        ),
    )

    attributed = adapter._apply_telegram_group_observe_attribution(event)

    assert attributed.text == "/new@intellect_bot"
...
def test_group_messages_can_require_direct_trigger_via_config():
    ...
    assert adapter._should_process_message(_group_message("hi @intellect_bot", entities=[_mention_entity("hi @intellect_bot")])) is True
    ...
    assert adapter._should_process_message(
        _group_message(
            "/status@intellect_bot",
            entities=[_bot_command_entity("/status@intellect_bot", "/status@intellect_bot")],
        ),
        is_command=True,
    ) is True
...
def test_raw_bot_mention_fallback_does_not_match_email_or_substring():
    adapter = _make_adapter(require_mention=True, bot_username="intellect_bot")

    assert adapter._should_process_message(_group_message("email ops@intellect_bot.example")) is False
    assert adapter._should_process_message(_group_message("prefix@intellect_bot hi")) is False
    assert adapter._should_process_message(_group_message("hi @intellect_bot")) is True
...
def test_guest_mode_allows_only_direct_mentions_outside_allowed_chats():
    ...
    mentioned = _group_message(
        "hi @intellect_bot",
        chat_id=-201,
        entities=[_mention_entity("hi @intellect_bot")],
    )
...
def test_allowed_topics_drop_other_forum_topics_before_other_gates():
    ...
    assert adapter._should_process_message(
        _group_message("hi @intellect_bot", chat_id=-100, thread_id=11, entities=[_mention_entity("hi @intellect_bot")])
    ) is False
