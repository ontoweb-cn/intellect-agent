...
class TestDefaultInteractionDispatch:
    ...
    async def test_update_prompt_click_writes_response_file(self, tmp_path, monkeypatch):
        """update_prompt:y click writes 'y' to ~/.intellect/.update_response."""
        adapter = self._make_adapter()
        intellect_home = tmp_path / "intellect_home"
        intellect_home.mkdir()
        monkeypatch.setattr(
            "intellect_constants.get_intellect_home",
            lambda: intellect_home,
        )
        ...
        response = intellect_home / ".update_response"
