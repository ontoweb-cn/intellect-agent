...
class TestStartRun:
    ...
    async def test_start_returns_202(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_create_agent") as mock_create:
                ...
                assert status["object"] == "intellect.run"
    ...
    async def test_start_invalid_history_does_not_allocate_run(self, adapter):
        ...
        assert adapter._run_status_store.get("missing") is None
...
class TestRunEvents:
    ...
    async def test_approval_string_false_does_not_resolve_all(self, adapter):
        ...
        adapter._run_status_store.set(run_id, {"run_id": run_id, "status": "running"})
