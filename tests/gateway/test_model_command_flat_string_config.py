...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "intellect_cli.model_switch.switch_model",
        lambda **kw: _fake_switch_result(),
    )
    # save_config writes to ``get_intellect_home() / config.yaml`` — point it here.
    monkeypatch.setattr("intellect_constants.get_intellect_home", lambda: intellect_home)
    monkeypatch.setattr("intellect_cli.config.get_intellect_home", lambda: intellect_home)
    return cfg_path
