"""Shared fixtures for the intellect-agent test suite.
...
2. **Isolated INTELLECT_HOME.** INTELLECT_HOME points to a per-test tempdir so
   code reading ``~/.intellect/*`` via ``get_intellect_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".intellect"`` instead of the canonical
   ``get_intellect_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No intellect_SESSION_* inheritance** — the agent's current gateway
...
# intellect_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_intellect_BEHAVIORAL_VARS = frozenset({
    "intellect_YOLO_MODE",
    "intellect_INTERACTIVE",
    "INTELLECT_QUIET",
    "intellect_TOOL_PROGRESS",
    "intellect_TOOL_PROGRESS_MODE",
    "INTELLECT_MAX_ITERATIONS",
    "intellect_SESSION_PLATFORM",
    "intellect_SESSION_CHAT_ID",
    "intellect_SESSION_CHAT_NAME",
    "intellect_SESSION_THREAD_ID",
    "INTELLECT_SESSION_SOURCE",
    "INTELLECT_SESSION_KEY",
    "intellect_GATEWAY_SESSION",
    "_INTELLECT_GATEWAY",
    "intellect_PLATFORM",
    "intellect_MODEL",
    "intellect_INFERENCE_MODEL",
    "INTELLECT_INFERENCE_PROVIDER",
    "intellect_TUI_PROVIDER",
    "intellect_MANAGED",
    "intellect_DEV",
    "intellect_CONTAINER",
    "INTELLECT_EPHEMERAL_SYSTEM_PROMPT",
    "INTELLECT_TIMEZONE",
    "INTELLECT_REDACT_SECRETS",
    "intellect_BACKGROUND_NOTIFICATIONS",
    "intellect_EXEC_ASK",
    "INTELLECT_HOME_MODE",
    "intellect_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.intellect/kanban.db instead of the per-test INTELLECT_HOME.
    "intellect_KANBAN_DB",
    "INTELLECT_KANBAN_BOARD",
    "intellect_KANBAN_HOME",
    "intellect_KANBAN_WORKSPACES_ROOT",
    "intellect_KANBAN_LOGS_ROOT",
    "INTELLECT_KANBAN_TASK",
    "intellect_KANBAN_WORKSPACE",
    "intellect_KANBAN_RUN_ID",
    "intellect_KANBAN_CLAIM_LOCK",
    "intellect_KANBAN_DISPATCH_IN_GATEWAY",
    "intellect_TENANT",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
    ...
    # exercise notification toggles must opt in explicitly or they
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and INTELLECT_HOME to per-test tempdirs so code that
    reads ``~/.intellect/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral intellect_* vars that could change test semantics.
    for name in _intellect_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect INTELLECT_HOME to a per-test tempdir. Code that reads
    #    ``~/.intellect/*`` via ``get_intellect_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.intellect/*`` via
    #    ``Path.home() / ".intellect"`` instead of ``get_intellect_home()``
    #    is a bug to fix at the callsite.
    fake_intellect_home = tmp_path / "intellect_test"
    fake_intellect_home.mkdir()
    (fake_intellect_home / "sessions").mkdir()
    (fake_intellect_home / "cron").mkdir()
    (fake_intellect_home / "memories").mkdir()
    (fake_intellect_home / "skills").mkdir()
    monkeypatch.setenv("INTELLECT_HOME", str(fake_intellect_home))
    ...
    #    ~/.intellect/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import intellect_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_intellect_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal intellect config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``intellect-gateway`` process
...
# correlated with a ``tests/intellect_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> intellect-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart intellect-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting intellect/python patterns are also blocked.
    """
    ...
    def _matches_intellect_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _intellect_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_intellect_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch intellect-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m intellect_cli.main".
                if (
                    "intellect" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live intellect-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting intellect/python could hit the live gateway. "
