...
See: https://gitee.com/ontoweb/intellect-agent/issues/4426
"""
...
class TestGetSubprocessHome:
    """Unit tests for intellect_constants.get_subprocess_home()."""

    def test_returns_none_when_intellect_home_unset(self, monkeypatch):
    ...
    def test_returns_path_when_home_dir_exists(self, tmp_path, monkeypatch):
        intellect_home = tmp_path / ".intellect"
        intellect_home.mkdir()
        profile_home = intellect_home / "home"
        profile_home.mkdir()
        monkeypatch.setenv("INTELLECT_HOME", str(intellect_home))
        from intellect_constants import get_subprocess_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        base = tmp_path / ".intellect" / "profiles"
        ...
        monkeypatch.setenv("INTELLECT_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("INTELLECT_HOME", str(base / "beta"))
    ...
    def test_context_override_is_thread_local(self, tmp_path, monkeypatch):
        ...
        from intellect_constants import (
            get_intellect_home,
            reset_intellect_home_override,
            set_intellect_home_override,
        )
        ...
        def read_from_other_thread():
            ...
            seen.append(str(get_intellect_home()))

        thread = threading.Thread(target=read_from_other_thread)
        thread.start()
        assert ready.wait(timeout=5)

        token = set_intellect_home_override(profile)
        try:
            assert get_intellect_home() == profile
            release.set()
        ...
        assert get_intellect_home() == root
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_injects_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(intellect_home / "home")
    ...
    def test_no_injection_when_intellect_home_unset(self, monkeypatch):
        monkeypatch.delenv("INTELLECT_HOME", raising=False)
    ...
    def test_context_override_bridges_to_subprocess_env(self, tmp_path, monkeypatch):
        ...
        assert result["INTELLECT_HOME"] == str(profile)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from intellect_cli.profiles import _PROFILE_DIRS
