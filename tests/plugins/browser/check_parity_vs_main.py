...
Run from the PR worktree:
    ...
    cd ~/.intellect/intellect-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.intellect/intellect-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.intellect/intellect-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a intellect-agent checkout"
)
...
# Isolated INTELLECT_HOME for the config write.
