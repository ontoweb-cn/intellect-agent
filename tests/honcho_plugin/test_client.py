...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "intellect"
        assert config.workspace_id == "intellect"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.intellect/honcho.json
        monkeypatch.setenv("INTELLECT_HOME", str(tmp_path / "isolated"))
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"intellect": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.context_tokens == 2000
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"intellect": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"intellect": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="intellect-agent"
        ):
            result = config.resolve_session_name("/home/user/intellect-agent/subdir")
        assert result == "intellect-agent"
...
class TestResolveConfigPath:
    def test_prefers_intellect_home_when_exists(self, tmp_path):
        intellect_home = tmp_path / "intellect"
        intellect_home.mkdir()
        local_cfg = intellect_home / "honcho.json"
        local_cfg.write_text('{"apiKey": "local"}')

        with patch.dict(os.environ, {"INTELLECT_HOME": str(intellect_home)}):
            result = resolve_config_path()
        assert result == local_cfg

    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: INTELLECT_HOME points at ~/.intellect/profiles/<name>, so
        # _get_default_intellect_home() must resolve back to ~/.intellect — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".intellect"
        ...
        assert _get_default_intellect_home() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_intellect_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".intellect".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".intellect"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "intellect_coder"
        assert profile_host_key("default") == "intellect"

    def test_default_returns_intellect(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("intellect_HONCHO_HOST", None)
            os.environ.pop("INTELLECT_HOME", None)
            assert resolve_active_host() == "intellect"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"intellect_HONCHO_HOST": "intellect.coder"}):
            assert resolve_active_host() == "intellect.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("intellect_HONCHO_HOST", None)
            with patch("intellect_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "intellect_coder"

    def test_default_profile_returns_intellect(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("intellect_HONCHO_HOST", None)
            with patch("intellect_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "intellect"

    def test_custom_profile_returns_intellect(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("intellect_HONCHO_HOST", None)
            with patch("intellect_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "intellect"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("intellect_HONCHO_HOST", None)
            # Temporarily remove intellect_cli.profiles to simulate import failure
            saved = sys.modules.get("intellect_cli.profiles")
            sys.modules["intellect_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "intellect"
            finally:
                if saved is not None:
                    sys.modules["intellect_cli.profiles"] = saved
                else:
                    sys.modules.pop("intellect_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="intellect_coder")
        assert config.host == "intellect_coder"
        assert config.workspace_id == "intellect"  # shared workspace
        assert config.ai_peer == "intellect_coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="intellect")
        assert config.host == "intellect"
        assert config.workspace_id == "intellect"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "intellect": {"aiPeer": "intellect", "peerName": "alice"},
                "intellect_coder": {
                    "aiPeer": "intellect_coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="intellect_coder", config_path=config_file,
        )
        assert config.host == "intellect_coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "intellect_dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="intellect_dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "intellect_dreamer"
    ...
    def test_from_global_config_reads_legacy_dot_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "intellect.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        config = HonchoClientConfig.from_global_config(
            host="intellect_dreamer",
            config_path=config_file,
        )
        assert config.host == "intellect_dreamer"
        assert config.peer_name == "dreamer-user"
        assert config.workspace_id == "intellect_dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"intellect": {"enabled": True, "aiPeer": "intellect"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"intellect": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_intellect_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="intellect",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("intellect_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_defaults_to_30s_when_no_timeout_configured(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("intellect_cli.config.load_config", return_value={}):
            client = get_honcho_client(cfg)
    ...
    def test_intellect_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="intellect",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("intellect_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
...
class TestDialecticDepthParsing:
    ...
    def test_depth_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "***",
            "dialecticDepth": 1,
            "hosts": {"intellect": {"dialecticDepth": 3}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
