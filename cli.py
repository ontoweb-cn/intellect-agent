#!/usr/bin/env python3
"""
Intellect Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Intellect Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills intellect-agent-dev,github-auth
...
def _resolve_cli_member_id() -> str | None:
    """Read the active member_id from .cli-session.json, if any."""
    try:
        from agent.runtime_context import load_cli_session
        data = load_cli_session()
        return data.get("member_id") if data else None
    except Exception:
        return None


# Suppress startup messages for clean CLI experience
os.environ["INTELLECT_QUIET"] = "1"  # Our own modules
...
try:
    from intellect_cli.pt_input_extras import (
...
from intellect_cli.banner import _format_context_length, format_banner_version_label
...
from intellect_cli.browser_connect import (
...
load_intellect_dotenv(intellect_home=_intellect_home, project_env=_project_env)
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.intellect/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If INTELLECT_IGNORE_USER_CONFIG=1 is set (via ``intellect chat --ignore-user-config``),
    the user config at ``~/.intellect/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({INTELLECT_HOME}/config.yaml)
    user_config_path = _intellect_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("INTELLECT_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Intellect, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Intellect - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see intellect_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
    ...
    if config_path.exists():
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                from intellect_cli.config import _normalize_root_model_keys
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and IntellectCLI.__init__ checks "default" first.
    ...
    # Normalize config key: the new config system (intellect_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && intellect` to control it.
    ...
    # UNLESS we're inside a gateway process (detected by _INTELLECT_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_INTELLECT_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["INTELLECT_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.intellect/logs/.
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``intellect --help`` paths inside the chat command flow).  See
...
def get_tool_definitions(*args, **kwargs):
    from intellect_cli.mcp_startup import wait_for_mcp_discovery
...
from intellect_cli.banner import build_welcome_banner
from intellect_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from intellect_cli.callbacks import prompt_for_secret
...
def _prepare_deferred_agent_startup() -> None:
    ...
    if _deferred_agent_startup_done:
        return
    if os.environ.get("INTELLECT_DEFER_AGENT_STARTUP") != "1":
        return
    _deferred_agent_startup_done = True
    _accept_hooks = os.environ.get("INTELLECT_ACCEPT_HOOKS", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run intellect -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"intellect-{short_id}"
    branch_name = f"intellect/{wt_name}"
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`intellect_cli.config.load_config` (the authoritative loader that
    ...
    try:
        ...
        from intellect_constants import get_intellect_home as _get_intellect_home
        _intellect_home_maint = _get_intellect_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_intellect_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`intellect_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``intellect/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("intellect-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``intellect/intellect-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``intellect -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("intellect/intellect-") or b.startswith("pr-"))
    ]
...
#
# Detection priority:
#   1. intellect_LIGHT / intellect_TUI_LIGHT env (true/false) — explicit override
#   2. INTELLECT_TUI_THEME=light|dark — explicit theme
#   3. INTELLECT_TUI_BACKGROUND=#RRGGBB — explicit bg hint
#   4. COLORFGBG env (set by xterm/Konsole/urxvt) — bg slot 7/15 = light
#   5. OSC 11 query (\x1b]11;?\x1b\\) — ask the terminal directly
#   6. Default: assume dark (matches the legacy Intellect assumption)
#
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        for var in ("intellect_LIGHT", "intellect_TUI_LIGHT"):
            ...
            if _FALSE_RE.match(v):
                ...
                return result
        # 2. Theme hint
        theme = (os.environ.get("INTELLECT_TUI_THEME") or "").strip().lower()
        if theme == "light":
            result = True
            _LIGHT_MODE_CACHE = result
            return result
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("INTELLECT_TUI_BACKGROUND") or ""
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from intellect_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
    if getattr(SkinConfig, "_intellect_light_mode_hook_installed", False):
        return
    ...
    SkinConfig._intellect_light_mode_hook_installed = True  # type: ignore[attr-defined]
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in Intellect output it's common to display cron schedules verbatim.
    #
...
def _sanitize_ansi_for_display(text: str) -> str:
    """Strip potentially dangerous ANSI escape sequences from LLM output.

    Keeps SGR color codes (\\\\x1b[...m) used by prompt_toolkit for
    rendering. Strips OSC sequences (\\\\x1b]...BEL/ST) which can set
    clipboard, window title, or hyperlink targets, and DCS/PM/APC/SOS
    sequences which are rarely used legitimately in agent output.
    """
    import re

    _OSC_RE = re.compile(r"\x1b\][^\x07\x1b]*(?:\x07|\x1b\\)")
    _OTHER_CONTROL_SEQ_RE = re.compile(
        r"\x1b[PX^_]"  # DCS, SOS, PM, APC  introducers
        r"[^\x1b]*"      # payload
        r"(?:\x1b\\)?"   # optional ST terminator
    )
    text = _OSC_RE.sub("", text)
    text = _OTHER_CONTROL_SEQ_RE.sub("", text)
    return text
...
def _cprint(text: str):
    ...
    text = _sanitize_ansi_for_display(text)
...
from intellect_constants import is_termux as _is_termux_environment
...
def _apply_bracketed_paste_timeout_patch() -> None:
    ...
    ``_intellect_bp_timeout_patched`` sentinel on the module.
    """
    try:
        ...
        if getattr(_vt100_mod, "_intellect_bp_timeout_patched", False):
            return
        ...
        def _patched_vt100_feed(self_parser, data: str) -> None:
            if self_parser._in_bracketed_paste:
                ...
                if end_mark in self_parser._paste_buffer:
                    ...
                    self_parser._intellect_bp_start = None
                    if remaining:
                        _patched_vt100_feed(self_parser, remaining)
                else:
                    bp_start = getattr(self_parser, "_intellect_bp_start", None)
                    now = time.monotonic()
                    if bp_start is None:
                        self_parser._intellect_bp_start = now
        ...
        _vt100_mod._intellect_bp_timeout_patched = True
...
# ASCII Art - Intellect Caduceus (compact, fits in left panel)
intellect_CADUCEUS = """[#CD7F32]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣀⡀⠀⣀⣀⠀⢀⣀⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
[#CD7F32]⠀⠀⠀⠀⠀⠀⢀⣠⣴⣾⣿⣿⣇⠸⣿⣿⠇⣸⣿⣿⣷⣦⣄⡀⠀⠀⠀⠀⠀⠀[/]
[#FFBF00]⠀⢀⣠⣴⣶⠿⠋⣩⡿⣿⡿⠻⣿⡇⢠⡄⢸⣿⠟⢿⣿⢿⣍⠙⠿⣶⣦⣄⡀⠀[/]
[#FFBF00]⠀⠀⠉⠉⠁⠶⠟⠋⠀⠉⠀⢀⣈⣁⡈⢁⣈⣁⡀⠀⠉⠀⠙⠻⠶⠈⠉⠉⠀⠀[/]
[#FFD700]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣴⣿⡿⠛⢁⡈⠛⢿⣿⣦⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
...
def _build_compact_banner() -> str:
    ...
    if os.environ.get("INTELLECT_FAST_STARTUP_BANNER") == "1":
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from intellect_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.intellect/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.intellect/config.yaml on first use)
...
class IntellectCLI:
    ...
    def __init__(
        ...
        """
        Initialize the Intellect CLI.

        Args:
            model: Model to use (default: from env or claude-sonnet)
            toolsets: List of toolsets to enable (default: all)
            provider: Inference provider ("auto", "openrouter", "ontoweb", "openai-codex", "zai", "kimi-coding", "minimax", "minimax-cn")
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from intellect_cli.runtime_provider import _auto_detect_local_model
        ...
        self.requested_provider = (
            ...
            or os.getenv("INTELLECT_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("INTELLECT_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("INTELLECT_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        # by `intellect chat --ignore-rules` in intellect_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("INTELLECT_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("INTELLECT_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        # it's shared across all Intellect processes for this INTELLECT_HOME.
        ...
        self._history_file = _intellect_home / ".intellect_history"
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from intellect_cli.voice import format_voice_record_key_for_status
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            return f"⚕ {self.model if getattr(self, 'model', None) else 'Intellect'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        if resolved_provider == "copilot":
            try:
                from intellect_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from intellect_cli.models import normalize_opencode_model_id, opencode_model_api_mode
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - IntellectCLI._status_bar_display_width(label)
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if not _is_callable_provider and (not isinstance(api_key, str) or not api_key):
            ...
            if _has_custom_base:
                ...
                print("\n⚠️  Provider resolver returned an empty API key. "
                      "Set OPENROUTER_API_KEY or run: intellect setup")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: intellect setup")
            return False
        ...
        # `intellect chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `intellect auth add openai-codex`
        # without `intellect model`), fall back to the provider's first catalog
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                if _quiet_mode:
                    ...
                    print(
                        "Use a session ID from a previous CLI run (intellect sessions list).",
                        file=sys.stderr,
                    )
                else:
                    _cprint(f"\033[1;31mSession not found: {self.session_id}{_RST}")
                    _cprint(f"{_DIM}Use a session ID from a previous CLI run (intellect sessions list).{_RST}")
                return False
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``intellect doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``intellect doctor`` so the banner stays
        small.
        """
        try:
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Intellect needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        # Warn if the configured model is a Ontoweb Intellect LLM (not agentic)
        from intellect_cli.model_switch import is_ontoweb_intellect_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_ontoweb_intellect_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  ONTOWEB intellect 3 & 4 models are NOT agentic and are not "
                "designed for use with Intellect Agent.[/]"
            )
            self._console_print(
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ Intellect: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ Intellect: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.intellect/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from intellect_cli.clipboard import save_clipboard_image

        img_dir = get_intellect_home() / "images"
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: intellect --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Intellect config/state.
        ...
        """
        from intellect_cli.backup import (
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_intellect_home()}/state-snapshots/):\n")
    ...
    def _handle_paste_command(self):
        ...
        from intellect_cli.clipboard import has_clipboard_image
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run intellect chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'intellect setup' to configure[/]")
        except Exception:
    ...
    def _show_session_status(self):
        ...
        lines = [
            "Intellect CLI Status",
            "",
            f"Session ID: {self.session_id}",
            f"Path: {display_intellect_home()}",
        ]
        if title:
        ...
        # 2.1.114's /recap.
        try:
            from intellect_cli.session_recap import build_recap
            recap = build_recap(
    ...
    def _fast_command_available(self) -> bool:
        try:
            from intellect_cli.models import model_supports_fast_mode
        except Exception:
            return False
        agent = getattr(self, "agent", None)
    ...
    def show_help(self):
        ...
        from intellect_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from intellect_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Intellect!{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in intellect_cli/colors.py
    ...
    def _handle_profile_command(self):
        ...
        display = display_intellect_home()
    ...
    def _list_recent_sessions(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        if not self._session_db:
            return []
        try:
            # Scope to current member in multi-user mode.
            _mid = _resolve_cli_member_id()
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from intellect_cli.main import _relative_time
    ...
    def new_session(self, silent=False, title=None):
        ...
        if self.agent:
            ...
            if self._session_db:
                try:
                    # Resolve member_id for multi-user session isolation.
                    _member_id = os.environ.get("INTELLECT_MEMBER_ID")
                    if not _member_id:
                        try:
                            from intellect_constants import get_intellect_home
                            import json as _json
                            _sf = get_intellect_home() / ".cli-session.json"
                            if _sf.exists():
                                _member_id = _json.loads(_sf.read_text()).get("member_id")
                        except Exception:
                            pass
                    self.agent._session_db_created = False
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("INTELLECT_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
                            "max_iterations": self.max_turns,
                            "reasoning_config": self.reasoning_config,
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `intellect gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            _cprint("  Tip:   Use /history or `intellect sessions list` to find sessions.")
            return
        ...
        if target.isdigit():
            ...
            from intellect_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `intellect sessions list` to see available sessions.")
            return
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.intellect/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``intellect --resume <id>``
        ...
        saved_dir = get_intellect_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"intellect_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: intellect --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from intellect_cli.curses_ui import curses_single_select
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        # Copilot, and OntoWeb-enforced caps win over the raw models.dev entry
        # (e.g. gpt-5.5 is 1.05M on openai but 272K on Codex OAuth).
        mi = result.model_info
        try:
            from intellect_cli.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `intellect model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from intellect_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from intellect_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from intellect_cli.model_switch import switch_model, parse_model_flags
        from intellect_cli.providers import get_label
        ...
        # TUI used to duplicate. Overlay live session state
        # via with_overrides (truthy-only) so empty self.* attrs don't
        # clobber disk config.
        from intellect_cli.inventory import build_models_payload, load_picker_context
    ...
    def _handle_codex_runtime(self, cmd_original: str) -> None:
        ...
        Usage:
            ...
            /codex-runtime auto                  — Intellect default (chat_completions)
            /codex-runtime codex_app_server      — hand turns to codex subprocess
            /codex-runtime on / off              — synonyms for the above
        """
        from intellect_cli import codex_runtime_switch as crs
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to intellect_cli.curator so the CLI and the `intellect curator`
        ...
        try:
            from intellect_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_kanban_command(self, cmd: str):
        ...
        """
        from intellect_cli.kanban import run_slash
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to intellect_cli.skills_hub."""
        from intellect_cli.skills_hub import handle_skills_slash
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_intellect_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_intellect_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in intellect_cli/commands.py instead of touching every dispatch site.
        from intellect_cli.commands import resolve_command as _resolve_cmd
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``intellect bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: intellect bundles create "
        ...
        _cprint(
            ...
            f"Manage with `intellect bundles`.{_RST}"
        )
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically an Intellect-managed isolated debug "
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Intellect keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from intellect_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_intellect_home()}/skins/\n")
            return
    ...
    def _handle_footer_command(self, cmd_original: str) -> None:
        ...
        """
        from intellect_cli.config import load_config
        from intellect_cli.colors import Colors as _Colors
    ...
    def _is_session_yolo_active(self) -> bool:
        ...
        ``intellect_YOLO_MODE`` into ``_YOLO_MODE_FROZEN`` before tool imports
        happen.
        """
        try:
            from tools.approval import (
    ...
    def _toggle_yolo(self):
        ...
        ``intellect_YOLO_MODE`` here — that env var is read once at module import
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Intellect is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Intellect is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while Intellect is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from intellect_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
            model = getattr(agent, "model", None) or getattr(self, "model", None)
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        * ``/compress here [N]`` — boundary-aware compression. Summarize
        ...
        from intellect_cli.partial_compress import (
    ...
    def _handle_debug_command(self):
        ...
        from intellect_cli.debug import run_debug_share
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Intellect Agent to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``intellect update`` so the user sees update output directly and gets
        ...
        when cancelled.
        """
        from intellect_cli.config import is_managed, format_managed_message

        if is_managed():
            print(f"  ✗ {format_managed_message('update Intellect Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Intellect Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update Intellect Agent",
            detail="This will exit the current session and run `intellect update`.",
            choices=choices,
        )
        if raw is None:
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by intellect_logging not
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from intellect_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.intellect/skills/ and queue a note for the
        next user turn.
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "intellect_voice"), exist_ok=True)
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard INTELLECT_HOME, so no -p needed.
            try:
                from intellect_cli.profiles import get_active_profile_name
            ...
            print(f"  intellect --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  intellect -c \"{session_title}\"{profile_flag}")
            print()
            ...
            try:
                from intellect_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from intellect_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from intellect_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Intellect Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Intellect Agent! Type your message or /help for commands."
        ...
        try:
            ...
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    ...
                    f"(INTELLECT_REDACT_SECRETS={_redact_raw}). "
        ...
        # after an OpenClaw→Intellect migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from intellect_cli.config import get_config_path as _get_cfg_path_resid
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in intellect_cli.pt_input_extras
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Intellect binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from intellect_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Intellect Agent")
        ...
        try:
            from intellect_cli.config import load_config
            from intellect_cli.voice import (
        ...
        def handle_paste(event):
            ...
            if pasted_text:
                ...
                if (lines_hit or chars_hit) and not buf.text.strip().startswith('/'):
                    _paste_counter[0] += 1
                    paste_dir = _intellect_home / "pastes"
        ...
        # Dynamic prompt: shows Intellect symbol when agent is working,
        ...
        try:
            ...
            if not getattr(_pt_renderer, "_intellect_osd_patched", False):
                ...
                _pt_renderer._intellect_osd_patched = True
        except Exception:
            pass
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the intellect process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: intellect setup"
            )
            _run_cleanup()
...
def main(
    ...
    """
    Intellect Agent CLI - Interactive AI Assistant
    ...
    os.environ["intellect_INTERACTIVE"] = "1"
    
    # Handle gateway mode (messaging + cron)
    if gateway:
        import asyncio
        from gateway.run import start_gateway
        print("Starting Intellect Gateway (messaging platforms)...")
    ...
    # Default to intellect-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
    ...
    # mode registers its own inside IntellectCLI.run(), but `-q` runs
    ...
    if query or image:
        ...
        # Kanban workers spawn with ``intellect chat -q "work kanban task <id>"``;
        ...
        _kanban_task_id = os.environ.get("INTELLECT_KANBAN_TASK", "").strip()
        if _kanban_task_id:
            try:
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if cli._init_agent(
                    ...
                    # machine-readable (no styled "Intellect" box, no tool-gen
            ...
            # Single-query mode (`intellect chat -q "…"`): skip the welcome
